...
trtmc::RobotObservation observation(std::vector<float>& image, std::vector<float>& state) {
    return {image.data(), 2, 2, 3, state.data(), 2};
}

void test_chunk_contract_and_bounds() {
    auto module = std::make_unique<FakeActModule>();
    auto* module_ptr = module.get();
    trtmc::LeRobotActPipeline pipeline(std::move(module), 2, 2, 3, 2, 2, 3, {-1.0F, -1.0F},
                                       {1.0F, 1.0F}, "act-test");
...
void test_action_queue_and_reset() {
    auto module = std::make_unique<FakeActModule>();
    auto* module_ptr = module.get();
    trtmc::LeRobotActPipeline pipeline(std::move(module), 2, 2, 3, 2, 2, 3, {-2.0F, -2.0F},
                                       {2.0F, 2.0F});
    std::vector<float> image(12, 0.5F);
    std::vector<float> state{0.1F, -0.2F};
...
void test_invalid_construction() {
    bool threw = false;
    try {
        trtmc::LeRobotActPipeline pipeline(std::make_unique<FakeActModule>(), 2, 2, 3, 2, 2, 3,
                                           {-1.0F}, {1.0F});
