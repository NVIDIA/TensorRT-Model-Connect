...
int main() {
    try {
        const auto options = trtmc::parse_cosmos3_options(
            R"({"negative_prompt":"{}","num_inference_steps":35,"guidance_scale":6.0,"flow_shift":10.0,"video_height":720,"video_width":1280,"video_num_frames":189,"frame_rate":24,"text_seq_len":4096,"seed":42})");
        trtmc::GenerateConfig config;
        config.seed = 7;
        const auto request = trtmc::resolve_cosmos3_request(options, config);
        if (request.seed != 7 || request.video_num_frames != 189 ||
            request.num_inference_steps != 35) {
