...
// =============================================================================
//
// Tests the RecurrentPipeline with mock engines and Qwen3.5-owned recurrent
...
static void test_mamba_pipeline() {
    ...
    std::vector<trtmc::Qwen35RecurrentState::TensorSpec> specs = {
        {"conv_state", {12}},
        {"ssm_state", {32}},
    };
    auto rs = std::make_unique<trtmc::Qwen35RecurrentState>(1, specs, stream);
...
static void test_rwkv_pipeline() {
    ...
    auto rs = std::make_unique<trtmc::Qwen35RecurrentState>(2, specs, stream);
...
static void test_hybrid_pipeline() {
    ...
    auto kv = std::make_unique<trtmc::Qwen35KvCache>(1, 4, 2, stream);
    std::vector<trtmc::Qwen35RecurrentState::TensorSpec> specs = {{"ssm", {4}}};
    auto ssm = std::make_unique<trtmc::Qwen35RecurrentState>(1, specs, stream);
    auto hybrid = std::make_unique<trtmc::Qwen35HybridState>(std::move(kv), std::move(ssm));
