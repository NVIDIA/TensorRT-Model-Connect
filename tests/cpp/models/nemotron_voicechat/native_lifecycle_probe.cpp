...
validate_event_contracts(const std::vector<std::vector<const Capture*>>& sessions) {
    ...
    for (const auto& session : sessions) {
        ...
        std::int64_t next_media_sample = 0;
...
bool required_lifecycle_contract(bool baseline, bool irregular, bool barge_in, bool cancel,
                                 ...
                                 bool backpressure_concurrency) {
    return baseline && irregular && barge_in && cancel && reset && tail &&
           events.sequence_monotonic && events.media_contiguous && normal_multiturn &&
           function_channel && backpressure_concurrency;
}

int run_host_self_check() {
    Capture valid;
    std::vector<trtmc::SpeechSessionEvent> events(4);
    events[0].kind = EventKind::kTurnStarted;
    events[0].epoch = 2;
    events[0].sequence = 0;
    events[1].kind = EventKind::kAgentAudio;
    events[1].epoch = 2;
    events[1].sequence = 1;
    events[1].audio_samples = {0.1F, -0.1F};
    events[1].sample_rate = 22050;
    events[1].media_start_sample = 0;
    events[1].media_end_sample = 2;
    events[2].kind = EventKind::kAgentText;
    events[2].epoch = 2;
    events[2].sequence = 2;
    events[2].text = "ok";
    events[3].kind = EventKind::kAgentAudio;
    events[3].epoch = 2;
    events[3].sequence = 3;
    events[3].audio_samples = {0.2F};
    events[3].sample_rate = 22050;
    events[3].media_start_sample = 2;
    events[3].media_end_sample = 3;
    valid.absorb(std::move(events));
    const auto valid_evidence = validate_event_contracts({{&valid}});
    ...
    Capture invalid;
    std::vector<trtmc::SpeechSessionEvent> malformed(2);
    for (auto& event : malformed) {
        event.kind = EventKind::kAgentAudio;
        event.epoch = 3;
        event.sequence = 0;
        event.audio_samples = {0.0F};
        event.sample_rate = 22050;
    }
    malformed[0].media_start_sample = 0;
    malformed[0].media_end_sample = 1;
    malformed[1].media_start_sample = 7;
    malformed[1].media_end_sample = 8;
    invalid.absorb(std::move(malformed));
    const auto invalid_evidence = validate_event_contracts({{&invalid}});
    ...
    Capture multiturn;
    std::vector<trtmc::SpeechSessionEvent> turns(10);
    turns[0].kind = EventKind::kTurnStarted;
    turns[0].epoch = 1;
    turns[0].sequence = 0;
    turns[1].kind = EventKind::kAgentText;
    turns[1].epoch = 1;
    turns[1].sequence = 1;
    turns[1].text = "initial greeting";
    turns[1].is_final = true;
    turns[2].kind = EventKind::kTurnFinished;
    turns[2].epoch = 1;
    turns[2].sequence = 2;
    turns[3].kind = EventKind::kUserTranscript;
    turns[3].epoch = 2;
    turns[3].sequence = 0;
    turns[3].text = "first user";
    turns[3].is_final = true;
    turns[4].kind = EventKind::kTurnStarted;
    turns[4].epoch = 3;
    turns[4].sequence = 0;
    turns[5].kind = EventKind::kAgentText;
    turns[5].epoch = 3;
    turns[5].sequence = 1;
    turns[5].text = "first agent";
    turns[5].is_final = true;
    turns[6].kind = EventKind::kTurnFinished;
    turns[6].epoch = 3;
    turns[6].sequence = 2;
    turns[7].kind = EventKind::kUserTranscript;
    turns[7].epoch = 4;
    turns[7].sequence = 0;
    turns[7].text = "second user";
    turns[7].is_final = true;
    turns[8].kind = EventKind::kTurnStarted;
    turns[8].epoch = 5;
    turns[8].sequence = 0;
    turns[9].kind = EventKind::kAgentText;
    turns[9].epoch = 5;
    turns[9].sequence = 1;
    turns[9].text = "second agent";
    turns[9].is_final = true;
    multiturn.absorb(std::move(turns));
    std::vector<trtmc::SpeechSessionEvent> completion(2);
    completion[0].kind = EventKind::kTurnFinished;
    completion[0].epoch = 5;
    completion[0].sequence = 2;
    completion[1].kind = EventKind::kInputFinished;
    completion[1].epoch = 6;
    completion[1].sequence = 0;
    multiturn.absorb(std::move(completion));
    ...
    backpressure.producer_thread_completed = true;
    backpressure.consumer_thread_completed = true;
    backpressure.events_observed_while_producing = true;
    backpressure.bounded_queue = true;
    backpressure.overflow_error_observed = true;
    backpressure.no_deadlock = true;
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const auto event_contract =
            validate_event_contracts({{&baseline},
                                      {&irregular},
        ...
        receipt << "    \"recognized_interrupt_frames\": " << barge_interrupt_frames << ",\n";
        ...
        receipt << "    \"pass\": " << json_bool(normal_multiturn_pass) << ",\n";
        ...
        receipt << "    \"pass\": " << json_bool(function_channel_pass) << ",\n";
        ...
        receipt << "    \"call_id\": \"" << json_escape(function_channel.call_id) << "\",\n";
        receipt << "    \"stale_function_payloads\": " << function_channel.stale_function_payloads
                << "\n";
        receipt << "  },\n";
        receipt << "  \"backpressure_concurrency\": {\n";
        receipt << "    \"implemented\": true,\n";
        receipt << "    \"pass\": " << json_bool(backpressure_concurrency_pass) << ",\n";
