...
voicechat::FunctionToolCatalog make_tools() {
...
void test_exact_markers_and_tool_catalog() {
    ...
    check(tools.tools().size() == 2 && tools.find("get_weather") != nullptr &&
              tools.find("missing") == nullptr,
          "flat and nested OpenAI-style tools are indexed by name");
    check(tools.find("get_weather")->ack_messages ==
                  std::vector<std::string>({"Let me check the weather."}) &&
              tools.find("add")->ack_messages ==
                  std::vector<std::string>({"I will add those numbers."}),
...
void test_natural_and_multi_call_observation() {
    ...
    check(state.awaiting_response_end(), "completed call waits for EOTR");
...
void test_bounds_and_reset() {
    ...
    bounded.reset();
    check(!bounded.capturing_call() && !bounded.awaiting_response_end() &&
              bounded.buffered_tokens() == 0,
...
void test_tool_response_tokens() {
    ...
    check(forced.text == R"(<TOOL_RESPONSE>[{"temperature":72}]</TOOL_RESPONSE>)" &&
              forced.text == encoded_text &&
              forced.token_ids == std::vector<int32_t>({501, 502, 503}),
    ...
    const auto already_array = voicechat::build_tool_response_tokens(
        R"([{"first":1},{"second":2}])", [](std::string_view) { return std::vector<int32_t>{7}; });
    check(already_array.text == R"(<TOOL_RESPONSE>[{"first":1},{"second":2}]</TOOL_RESPONSE>)",
          "tool result arrays are not nested a second time");

    const auto plain = voicechat::build_tool_response_tokens(
        "ready", [](std::string_view) { return std::vector<int32_t>{8}; });
    check(plain.text == R"(<TOOL_RESPONSE>["ready"]</TOOL_RESPONSE>)",
...
void test_on_hold_lookup() {
    const auto messages = voicechat::FunctionOnHoldMessages::from_json(
        R"({"get_weather":["Let me check.","One moment."],"default":"Please wait."})");
    check(messages.lookup("get_weather") ==
              std::vector<std::string>({"Let me check.", "One moment."}),
          "on-hold lookup returns the per-tool messages");
    check(messages.lookup("add") == std::vector<std::string>({"Please wait."}),
          "on-hold lookup falls back to default");

    const auto no_default =
        voicechat::FunctionOnHoldMessages::from_json(R"({"get_weather":"Checking."})");
    check(no_default.lookup("add").empty(), "on-hold lookup permits no default");
    expect_invalid([] { (void)voicechat::FunctionOnHoldMessages::from_json(R"({"default":[]})"); },
                   "on-hold lookup rejects empty message lists");
}
