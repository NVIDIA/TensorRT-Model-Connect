...
int main() {
    ...
    const auto check = [&](bool condition, const char* message) {
        if (!condition) {
            std::cerr << "FAIL [InternVL]: " << message << '\n';
            ++failures;
        }
    };
    ...
    check(trtmc::test::rejects_native_contract<trtmc::InternvlKvCache>(
              stream,
              [](auto& module) {
    ...
    {
        trtmc::InternvlKvCache cache(1, 11, 2, stream, trtmc::DType::kFloat16);
...

    {
        ...
        trtmc::InternvlKvCache rank_cache(1, 16, 128, stream, trtmc::DType::kBFloat16);
