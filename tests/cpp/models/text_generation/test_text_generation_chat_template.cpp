// Model-owned chat-template coverage for text-generation-specific formats.
...
static void test_apply_nemotron_h_no_thinking() {
    auto result = trtmc::apply_chat_template("nemotron_h", "hello", false);
...
static void test_apply_nemotron_labs_diffusion_no_thinking() {
    auto result = trtmc::apply_chat_template("nemotron_labs_diffusion", "hello", false);
...
static void test_apply_chatml_no_thinking() {
    auto result = trtmc::apply_chat_template("chatml", "What is 2+2?", false);
...
static void test_apply_mistral_no_thinking_ignored() {
    auto result = trtmc::apply_chat_template("mistral", "hello", false);
    check(result == "[INST] hello [/INST]", "mistral no-thinking ignored");
}

static void test_apply_phi() {
    auto result = trtmc::apply_chat_template("phi", "hello");
    check(result == "<|user|>\nhello<|end|>\n<|assistant|>\n", "phi application");
}

static void test_apply_gemma() {
    auto result = trtmc::apply_chat_template("gemma", "hello");
    check(result == "<start_of_turn>user\nhello<end_of_turn>\n<start_of_turn>model\n",
          "gemma application");
}

static void test_apply_llama3() {
    auto result = trtmc::apply_chat_template("llama3", "hello");
...
int main() {
    trtmc::register_text_generation_chat_templates();
    ...
    std::cerr << "All text_generation chat_template tests passed.\n";
    return 0;
}
