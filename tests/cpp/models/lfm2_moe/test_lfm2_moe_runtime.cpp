...
class FakeLfm2MoeModule final : public trtmc::ITrtModule {
  public:
    struct Call {
        int32_t token{0};
        int32_t position{0};
    ...
    FakeLfm2MoeModule(cudaStream_t stream, trtmc::DType state_dtype, int32_t vocab_size = 6)
...
void test_native_kv_and_double_buffered_conv(cudaStream_t stream) {
    FakeLfm2MoeModule module(stream, trtmc::DType::kFloat16);
...
void test_native_contract_rejection_and_capacity(cudaStream_t stream) {
    {
        FakeLfm2MoeModule malformed(stream, trtmc::DType::kFloat16);
        malformed.set_tensor("key_value_lengths", {2}, trtmc::DType::kInt32);
        trtmc::Lfm2MoeKvCache cache(1, 8, 2, 4, stream, trtmc::DType::kFloat16);
        bool rejected = false;
        try {
...
void test_pipeline_uses_safe_one_token_progression(cudaStream_t stream) {
    ...
    auto kv = std::make_unique<trtmc::Lfm2MoeKvCache>(1, 8, 2, 4, stream, trtmc::DType::kFloat16);
    auto conv = std::make_unique<trtmc::Lfm2MoeConvState>(1, 4, 3, stream, trtmc::DType::kFloat16);
    auto state = std::make_unique<trtmc::Lfm2MoeHybridState>(std::move(kv), std::move(conv));

    trtmc::Lfm2MoeTextGenConfig config;
    config.vocab_size = 6;
    config.id_eos = 5;
    config.id_eos_ids = {5};
