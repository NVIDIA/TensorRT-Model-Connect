...
void test_decoder_detection() {
    ...
    check(trtmc::lfm2_moe_uses_sequence_byte_level_decoder(sequence.data(), sequence.size()),
          "detects exact LFM2 Sequence ByteLevel decoder");
    check(!trtmc::lfm2_moe_uses_sequence_byte_level_decoder(direct.data(), direct.size()),
          "does not double-decode a direct ByteLevel tokenizer");
    check(!trtmc::lfm2_moe_uses_sequence_byte_level_decoder(replace.data(), replace.size()),
          "does not alter unrelated Sequence decoders");
    check(!trtmc::lfm2_moe_uses_sequence_byte_level_decoder("not-json", 8),
...
void test_reported_space_and_newline_regression() {
    ...
    check(trtmc::lfm2_moe_decode_gpt2_byte_level(encoded) == ":\nA) Paris",
...
void test_multilingual_utf8_round_trip() {
    ...
    check(trtmc::lfm2_moe_decode_gpt2_byte_level(byte_level_encode(original)) == original,
          "decodes complete UTF-8 byte sequences across eight languages");
    const std::string emoji = "🙂";
    check(trtmc::lfm2_moe_decode_gpt2_byte_level(byte_level_encode(emoji)) == emoji,
...
void test_every_raw_byte_round_trip() {
    ...
    const auto decoded = trtmc::lfm2_moe_decode_gpt2_byte_level(byte_level_encode(raw));
    ...
    check(trtmc::lfm2_moe_decode_gpt2_byte_level(malformed) == malformed,
          "preserves malformed raw UTF-8 bytes verbatim");
}

void test_wrapper_preserves_special_token_contract() {
    auto wrapped = trtmc::lfm2_moe_wrap_byte_level_decoder(std::make_unique<FilteringTokenizer>());
...
void test_bundle_tokenizer_wraps_sequence_decoder() {
    ...
    bundle.sections.push_back(trtmc::BundleSection{
        "tokenizer.json", std::vector<char>(tokenizer_json.begin(), tokenizer_json.end())});

    auto tokenizer = trtmc::create_lfm2_moe_tokenizer(bundle);
