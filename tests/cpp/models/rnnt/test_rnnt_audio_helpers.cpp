...
#include "runtime/models/rnnt/audio_helpers.h"
...
void test_rnnt_mel_matches_configured_generic_options() {
    ...
    trtmc::MelSpectrogramOptions options;
    options.n_fft = n_fft;
    ...
    options.log_scale = trtmc::MelLogScale::kNaturalLog;
    const auto generic = trtmc::extract_configured_mel_spectrogram(
        samples.data(), static_cast<int32_t>(samples.size()), mel_filters.data(), n_freq_bins,
        n_mel_bins, options);

    check(rnnt.n_mels == generic.n_mels, "rnnt mel keeps generic mel count");
    check(rnnt.n_frames == generic.n_frames, "rnnt mel keeps generic frame count");
    check(rnnt.data.size() == generic.data.size(), "rnnt mel keeps generic data size");
    for (std::size_t i = 0; i < rnnt.data.size(); ++i) {
        check_close(rnnt.data[i], generic.data[i], 1e-6F, "rnnt mel value matches generic");
    }
}
...
int main() {
    test_rnnt_mel_matches_configured_generic_options();
