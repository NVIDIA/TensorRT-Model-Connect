...
// Intent:         LocateAnythingPipeline text-only generation with mock engines; constructor
//                 validation (null decoder/cache); LocateAnythingConfig sync from
//                 LocateAnythingPreprocessConfig; zero max_tokens early exit; string-based
...
static void test_vl_text_only() {
    ...
    trtmc::LocateAnythingPipeline pipeline(std::move(decoder), nullptr, std::move(cache), cfg,
                                           vl_pp, stream);
    check(std::string(pipeline.pipeline_type()) == "LocateAnythingPipeline", "vl name");
...
static void test_vl_validates_decoder() {
    ...
    bool threw = false;
    try {
        trtmc::LocateAnythingPipeline p(nullptr, nullptr, std::move(cache), cfg, vl_pp, stream);
...
static void test_vl_validates_cache() {
    ...
    bool threw = false;
    try {
        trtmc::LocateAnythingPipeline p(std::move(decoder), nullptr, nullptr, cfg, vl_pp, stream);
    } catch (const std::exception&) {
        threw = true;
    }
...
static void test_vl_config_sync() {
    // LocateAnythingPreprocessConfig has image_token_id=1 and vision_output_dim=64,
    // LocateAnythingConfig has image_token_id=-1 and vision_output_dim=0.
    ...
    check(std::string(pipeline.pipeline_type()) == "LocateAnythingPipeline",
...
static void test_vl_generate_with_vision_encoder() {
    ...
    //   convert_float_to_decoded (lines 64-79), locateanything_preprocess_decoded_image with
...
static void test_vl_generate_with_embed_decoder() {
    // Covers LocateAnythingPipeline::run_text_step_with_embed() full body (lines 285-361).
