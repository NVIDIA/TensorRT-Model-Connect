...
static void test_forward_device() {
    // Covers TrtModule::forward_device() with a required device input.
    // The empty-map form remains available to callers that populate device_ptr()
    // directly, while supplied DeviceTensor entries are validated.
    auto engine = build_identity_engine();
    if (!engine)
        return;
    ...
    auto out = module.forward_device(inputs);
