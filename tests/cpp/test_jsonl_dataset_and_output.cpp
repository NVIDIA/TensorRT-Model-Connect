...
bool test_parse_basic() {
    const std::string line =
        R"({"sample_id":"q1","answer":"42","prompt":"What is 6*7?"})";
...
bool test_parse_with_seed_index() {
    const std::string line =
        R"({"sample_id":"q2","answer":"7","prompt":"3+4","seed_index":5})";
    auto s = trtmc::cli::parse_dataset_line(line, 1);
...
bool test_parse_malformed_json() {
    ...
    try {
        ...
        std::string msg = e.what();
        return msg.find("42") != std::string::npos && msg.find("Malformed JSON") != std::string::npos;
    }
}
...
bool test_multiline_dataset_processing() {
    ...
    return parsed_samples.size() == 1 &&
           parsed_samples[0].sample_id == "s1" &&
...
bool test_build_text_sample_record_roundtrip() {
    ...
    return parsed["id"].get<int32_t>() == 42 &&
           ...
           parsed["generated"].get<std::string>() == "Paris" &&
           parsed["token_ids"].size() == 3 && parsed["token_ids"][0].get<int32_t>() == 101 &&
