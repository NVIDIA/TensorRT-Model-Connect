...
void test_generation_plan_preserves_acceptance_artifact_name() {
    ...
    check(plan.stages[2].required_side_input_count == 2,
          "RALM requires mask and preserved feature side tensors");
    check(std::string(plan.stages[2].required_side_inputs[0]) == "audio_mask",
...
void test_generation_plan_description_includes_stage_order_and_artifact() {
    ...
    check(description.find("ralm(semantic_lm_states=>residual_hidden") != std::string::npos,
