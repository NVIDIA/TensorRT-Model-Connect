"""Segmentation and prompted segmentation strategy runners.

Handles:
  - SegmentationRunner: semantic segmentation (e.g. SegFormer)
  - PromptedSegmentationRunner: prompted instance segmentation (e.g. SAM)

Both use subprocess isolation for GPU operations.

Auto-discovered by the registry via the module-level ``plugin`` attribute.
For this module we register TWO runners, so we use a ``plugins`` list
pattern recognized by the registry scanner (each element is registered
individually by checking for ``plugins`` after ``plugin``).
"""
...
# we register the primary one as ``plugin`` and register the second manually
# at import time via the registry.
plugin = SegmentationRunner()

# Register PromptedSegmentationRunner via direct registry call.
# This runs at import time during auto-discovery.
def _register_prompted():
    try:
        from ..registry import register_runner
        register_runner(PromptedSegmentationRunner())
    except Exception:
        try:
            from tests.e2e_harness.registry import register_runner
            register_runner(PromptedSegmentationRunner())
        except Exception:
            pass
