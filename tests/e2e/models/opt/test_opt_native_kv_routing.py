...
"""CPU-only contracts for OPT's TensorRT native KV path."""
...
from tensorrt_model_connect.families.opt.build_routing import (
...
from tensorrt_model_connect.families.opt.config import ModelConfig
from tensorrt_model_connect.families.opt.native_kv_contract import (
...
def _config(*, raw_updates: dict | None = None, **overrides) -> ModelConfig:
    ...
    raw = {
        ...
        "word_embed_proj_dim": values["hidden_size"],
        "layerdrop": 0.0,
    }
...
def test_opt_125m_uses_full_context_native_kv():
    config = _config()
...
def test_native_graph_uses_update_and_fused_attention_without_concat():
    family_dir = Path(__file__).resolve().parents[4] / (
        "python/tensorrt_model_connect/families/opt"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ({"model_type": "opt2"}, {}, "model_type"),
        ({"architectures": ["OtherForCausalLM"]}, {}, "architectures"),
        ({"hidden_size": 769}, {}, "divisible"),
        ({"num_key_value_heads": 6}, {}, "requires MHA"),
        ({"hidden_act": "gelu"}, {}, "activation_function='relu'"),
        ({}, {"do_layer_norm_before": False}, "do_layer_norm_before=true"),
        ({}, {"word_embed_proj_dim": 512}, "word_embed_proj_dim == hidden_size"),
        ({}, {"scale_embedding": True}, "scaled token embeddings"),
        ({}, {"layerdrop": 0.1}, "layerdrop=0"),
        ({}, {"use_parallel_residual": True}, "sequential residuals"),
        ({}, {"sliding_window": 1024}, "unsupported"),
    ],
)
...
def _weights(config: ModelConfig) -> dict[str, object]:
    ...
    weights: dict[str, object] = {
        ...
        "position_embedding": _Tensor((config.max_position_embeddings, hidden)),
...
def test_weight_contract_rejects_missing_shape_and_foreign_weights():
    ...
    foreign["layer.0.w_gate"] = _Tensor((64, 128))
...
def test_plugin_builds_only_the_requested_native_split_role(monkeypatch):
    ...
    plugin_module = importlib.import_module("tensorrt_model_connect.families.opt.plugin")
