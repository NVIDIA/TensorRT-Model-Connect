...
class HfTransformersReference:
    ...
    def _run_speech_to_text_ref(
        ...
        """Run Nemotron speech streaming reference."""
        return self._run_nemo_speech_ref(case, stage, ctx)
    def _run_nemo_speech_ref(
        self, case: E2ECase, stage: StageSpec, ctx: RunContext
    ) -> StageOutput:
        """Run NeMo speech model for speech-to-text reference."""
        artifacts_dir = ctx.artifacts_dir or tempfile.gettempdir()
        model_dir = _case_artifact_dir(artifacts_dir, case.name) if ctx.artifacts_dir else artifacts_dir
        ...
        script = textwrap.dedent(f"""\
            ...
            try:
                ...
                # Convert to mono WAV if needed (NeMo ASR expects mono)
        ...
        return run_reference_subprocess(
            ...
            failure_label="NeMo speech reference",
        )
