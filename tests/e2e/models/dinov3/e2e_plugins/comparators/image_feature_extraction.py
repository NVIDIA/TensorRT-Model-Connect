...
class ImageFeatureExtractionComparator:
    ...
    def compare(
        ...
        metrics["register_count_match"] = _exact_metric(
            register_count_match,
            _threshold(threshold, "register_count_match"),
        )
        ...
        trt_pooler_invariant = np.array_equal(trt_pooler, trt_hidden[:, 0, :])
        ref_pooler_invariant = np.array_equal(ref_pooler, ref_hidden[:, 0, :])
        metrics["pooler_token_invariant"] = _exact_metric(
            trt_pooler_invariant and ref_pooler_invariant,
        ...
        metrics["full_cosine"] = _minimum_metric(full_cosine, _threshold(threshold, "full_cosine"))
        metrics["cls_cosine"] = _minimum_metric(cls_cosine, _threshold(threshold, "cls_cosine"))
        metrics["pooler_cosine"] = _minimum_metric(
            pooler_cosine, _threshold(threshold, "pooler_cosine")
        )
        ...
        patch_cosines = np.asarray(
            [
                _cosine(trt_patch, ref_patch)
                for trt_patch, ref_patch in zip(trt_patches, ref_patches)
            ],
            dtype=np.float64,
        )
        ...
        if trt_registers > 0:
            register_cosine = _cosine(
                trt_hidden[:, 1:patch_start, :],
                ref_hidden[:, 1:patch_start, :],
            )
            metrics["register_cosine"] = _minimum_metric(
                register_cosine,
                _threshold(threshold, "register_cosine"),
            )
...
plugin = ImageFeatureExtractionComparator()
