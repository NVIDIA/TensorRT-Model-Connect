...
Handles gpt_neox's gpt_neox_decoder_kv_cache runtime strategy, which maps to
...
def _format_native_trace_error(case: E2ECase, phase: str, meta: dict) -> str:
    ...
    return (
        f"Native trace logits requested for {case.name} phase={phase}, but {detail}"
    )
...
class TextGenerationCausalRunner:
    ...
    def _run_native_trace_logits(
        ...
        """Collect full-vocabulary logits from GPT-NeoX's native C++ JSONL trace."""
            ...
            with trace_path.open("r", encoding="utf-8") as trace_file:
                for line_number, line in enumerate(trace_file, start=1):
                    ...
                    if len(ids) != len(values):
                        raise ValueError(
                            f"trace line {line_number} has mismatched ids/logits"
                        )
                    ...
                    if (
                        ...
                        raise ValueError(
                            f"trace line {line_number} is not a complete vocabulary"
                        )
