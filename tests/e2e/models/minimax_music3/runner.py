...
"""Model-owned E2E entrypoint for MiniMax-Music3.

The harness owns the sequence -- resolve the bundle, run preflight, drive the
runner, then the reference, then the contract -- and this supplies the paths
and the case filter for that. An earlier revision parsed the manifests here and
returned case names without ever reaching the harness, so the entrypoint
collected zero tests and the family's E2E never ran at all.
"""
...
def _resolve_ld_library_path() -> str:
    """Put the TensorRT wheel's libraries ahead of whatever is inherited."""
...
def _load_waives(platform: str = "") -> dict[str, tuple[str, str]]:
    """Read this family's waiver list, if it has one."""
    ...
    with open(_WAIVES_FILE, encoding="utf-8") as handle:
        for line in handle:
            line = line.strip()
            ...
            if len(parts) < 2:
                continue
            name_part, action = parts[0], parts[1].upper()
