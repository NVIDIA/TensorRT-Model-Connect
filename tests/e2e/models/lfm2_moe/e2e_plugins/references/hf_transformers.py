...
"""Pinned Hugging Face Transformers oracle for the LFM2-MoE family."""
...
class Lfm2MoeHfTransformersReference:
    """Run the exact checkpoint revision with native Transformers LFM2-MoE code."""
    ...
    def run_stage(self, case: E2ECase, stage: StageSpec, ctx: RunContext) -> StageOutput:
        if stage.name != "full_generation":
            raise ValueError(
                f"LFM2-MoE reference supports only full_generation, got {stage.name!r}"
            )
        ...
        script = textwrap.dedent(
            ...
            model_kwargs = {{
                "device_map": "auto",
                "torch_dtype": {_reference_dtype(case)},
            }}
            if revision:
        ...
        try:
            ...
            raise RuntimeError(f"Pinned LFM2-MoE reference timed out after {elapsed:.0f}s") from exc
        elapsed = time.monotonic() - started
        ...
        if result.returncode != 0:
            ...
            message = f"Pinned LFM2-MoE reference failed (rc={result.returncode}): {truncated}"
            if log_path:
...
plugin = Lfm2MoeHfTransformersReference()
