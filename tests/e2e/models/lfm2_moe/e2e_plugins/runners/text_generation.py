...
"""LFM2-MoE causal-generation runner using the pure-C++ CLI plus local debug adapter."""
...
class TextGenerationCausalRunner:
    """Execute LFM2-MoE generation and retain exact token/flag evidence."""
    ...
    def run_stage(self, case: E2ECase, stage: StageSpec, ctx: RunContext) -> StageOutput:
        if stage.name != "full_generation":
            raise ValueError(f"LFM2-MoE runner supports only full_generation, got {stage.name!r}")
    ...
    def _run_cpp_binary(
        ...
        logger.info("LFM2-MoE C++ inference: %s", " ".join(command))
    ...
    def _run_debug_logits(
        ...
        script = textwrap.dedent(
            ...
            from tensorrt_model_connect.families.lfm2_moe.debug_runner import (
