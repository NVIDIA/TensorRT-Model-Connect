"""Family-owned decoder tensor-parallel tests for phi."""
...
def test_phi_plugin_routes_tp_build(monkeypatch) -> None:
...
def test_phi_plugin_rejects_quantized_tp(monkeypatch) -> None:
    plugin_mod = importlib.import_module(
        f"tensorrt_model_connect.families.{FAMILY}.plugin")
    monkeypatch.setattr(
        plugin_mod,
