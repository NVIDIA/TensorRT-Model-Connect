...
Handles internlm's internlm_decoder_kv_cache runtime strategy, which maps to
...
class TextGenerationCausalRunner:
    ...
    def _run_native_trace_logits(
        ...
        """Collect full-vocabulary logits from InternLM's native C++ JSONL trace."""
        ...
        try:
            ...
            if not selected:
                raise ValueError(
                    f"native C++ trace contains no rows for phase={phase}; "
