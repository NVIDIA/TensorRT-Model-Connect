...
"""InternLM native C++ runner contracts."""
...
from tests.e2e.models.internlm.e2e_plugins.contracts import E2ECase, RunContext, StageSpec
from tests.e2e.models.internlm.e2e_plugins.runners import text_generation


def _case() -> E2ECase:
    return E2ECase(
        name="internlm-native-runner",
        hf_id="internlm/test",
        family="internlm",
...
def test_local_cpp_run_fails_closed_on_runtime_error_with_zero_exit(
    tmp_path: Path,
...
def test_native_trace_collects_full_vocab_logits_locally(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    commands: list[list[str]] = []
