...
"""Compare raw TensorRT API and ONNX->trtexec engines for timm SE-ResNet.

The raw API path uses TensorRT-Model-Connect's timm_seresnet family plugin.  The
...
DEFAULT_MODEL_ID = "timm/seresnet50.a1_in1k"
...
def _build_api_engine(model_id: str, plan_path: Path, *, verbose: bool) -> None:
    ...
    if plugin is None or plugin.name != "timm_seresnet":
        raise RuntimeError(
            f"Expected timm_seresnet plugin for {config.model_type!r}, got {plugin}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        default=str(_repo_root() / "artifacts" / "timm_seresnet_trt_path_comparison"),
    )
