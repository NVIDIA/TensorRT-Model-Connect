...
"""PixArt-owned diffusion image contract plugin."""
...
def _make_pass(stage_name: str, metrics, rule: str) -> CompareResult:
    return CompareResult(
        ...
        metrics=metrics,
        composite_rule=rule,
        message="PixArt diffusion image contract verified",
    )
...
class PixArtDiffusionImagePlugin:
    ...
    def verify(self, trt_output, ref_output, case, threshold):
        ...
        return _make_fail(stage, metrics, rule, "PixArt diffusion image contract failed")


plugin = PixArtDiffusionImagePlugin()
