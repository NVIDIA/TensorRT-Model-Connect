...
def test_supported_probe_selects_profile_without_hidden_opt_in(
    ...
    probe = run_probe(
        manifest,
        _request(parameters={**parameters, "public_options": MC_DEFAULT_DEPLOYMENT}),
    )
...
def test_established_mc_defaults_select_the_qualified_edge_profile() -> None:
    import inspect
...
def test_public_cli_accepts_default_and_explicit_qualified_profiles_only(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_build_stages_test_only_engine_runtime_and_plugin_payloads(
    ...
    assert build.descriptor["bundle_info"]["hidden_size"] == 2048
