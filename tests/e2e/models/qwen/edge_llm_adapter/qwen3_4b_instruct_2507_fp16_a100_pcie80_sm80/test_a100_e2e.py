...
def test_public_build_inspect_and_run_delegate_to_edgellm(tmp_path: Path) -> None:
    ...
    assert header["hidden_size"] == 2560
    assert header["num_layers"] == 36
    ...
    assert "Hidden size:        2560" in inspect.stdout
    assert "Layers:             36" in inspect.stdout
    assert "Attention heads:    32" in inspect.stdout
