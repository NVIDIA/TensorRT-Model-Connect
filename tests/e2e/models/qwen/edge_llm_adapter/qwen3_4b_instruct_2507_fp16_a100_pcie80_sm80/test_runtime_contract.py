...
def test_runtime_rejects_wrong_model_vocab_size_metadata(
    tmp_path: Path, fake_runtime: Path
) -> None:
    artifact, _ = _staged_capsule(tmp_path, fake_runtime)
    metadata = _mutable_descriptor(artifact)
    metadata["limits"]["vocab_size"] = 151935

    assert _rejected_create(artifact, metadata) == (
        b"implementation metadata limits field 'vocab_size' mismatch: expected 151936, got 151935"
    )
...
int main(int argc, char** argv) {
    ...
    request.model_id = "Qwen/Qwen3-4B-Instruct-2507";
    request.profile_id = "qwen3-4b-instruct-2507-fp16--a100-pcie80-sm80--edgellm0.9-trt11";
    ...
    if (first.text != "fake:hello" || second.text != "fake:world" ||
        ...
        pipeline->model_id() != std::string("Qwen/Qwen3-4B-Instruct-2507") ||
