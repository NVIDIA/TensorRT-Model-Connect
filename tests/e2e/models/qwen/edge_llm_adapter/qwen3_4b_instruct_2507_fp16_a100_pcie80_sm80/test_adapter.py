...
"""Model-local contracts for the Qwen3-4B-Instruct-2507 TensorRT Edge-LLM capsule."""
...
def test_installed_layout_compiles_the_packaged_qwen_runtime(
    ...
    monkeypatch.setenv("_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_ALLOW_FAKE_RUNTIME_BUILD", "1")
...
def _nlohmann_json_include() -> Path:
    ...
    pytest.fail(
        ...
        "set _TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_NLOHMANN_JSON_INCLUDE_DIR"
    )
...
def _fake_engine(
    ...
    *,
    ...
    builder_config: dict[str, object] | None = None,
) -> Path:
    root.mkdir()
...
def test_engine_validation_rejects_each_wrong_model_field(tmp_path: Path, field: str) -> None:
    adapter = _load_adapter_module()
    ...
    with pytest.raises(adapter.AdapterError, match=rf"engine config\.{field} must be exactly"):
...
def test_engine_validation_rejects_tensor_parallel_fields(
    ...
    with pytest.raises(adapter.AdapterError, match=rf"engine config\.{field} must be absent"):
        adapter._validate_engine_directory(engine)
...
@pytest.mark.parametrize(
    "sibling_config", (QWEN3_06B_ENGINE_MODEL_CONFIG, QWEN3_17B_ENGINE_MODEL_CONFIG)
)
def test_qwen3_4b_adapter_rejects_sibling_qwen3_engine(
    tmp_path: Path, sibling_config: dict[str, object]
) -> None:
    adapter = _load_adapter_module()
    sibling_engine = _fake_engine(tmp_path / "sibling-engine", model_config=sibling_config)

    with pytest.raises(adapter.AdapterError, match=r"engine config\.max_position_embeddings"):
        adapter._validate_engine_directory(sibling_engine)
...
def test_fake_probe_and_build_require_the_same_internal_authorization(
    ...
    adapter = _load_adapter_module()
    monkeypatch.delenv(
        "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_ALLOW_FAKE_RUNTIME_BUILD", raising=False
    )
