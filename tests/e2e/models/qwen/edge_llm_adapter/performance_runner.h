...
namespace qwen_edge_performance {

namespace fs = std::filesystem;
using Json = nlohmann::json;

inline constexpr char kSchema[] = "trtmc.qwen.edgellm.performance-runner.v1";
...
inline void validate_sample(const Sample& sample) {
    if (sample.generated.empty() || sample.token_ids.empty())
        throw std::runtime_error("runtime returned an empty performance response");
...
Measurements measure(Generate&& generate, Synchronize&& synchronize) {
    ...
    for (int32_t index = 0; index < kMeasuredRequests; ++index) {
        ...
        iterations.push_back({latency_ms, std::move(sample)});
    }
