...
The raw API path uses TensorRT-Model-Connect's timm_hrnet family plugin.  The
...
def _build_api_engine(model_id: str, plan_path: Path, *, verbose: bool) -> None:
    ...
    if plugin is None or plugin.name != "timm_hrnet":
        raise RuntimeError(
            f"Expected timm_hrnet plugin for {config.model_type!r}, got {plugin}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        default=str(_repo_root() / "artifacts" / "timm_hrnet_trt_path_comparison"),
    )
