...
try:
    ...
    from tensorrt_model_connect.families.timm_mnasnet import plugin
...
def test_model_config_uses_timm_architecture_when_model_type_absent(tmp_path: Path):
    ...
    assert cfg.model_type == "mnasnet_100"
    assert plugin.matches(cfg.model_type)


@pytest.mark.parametrize(
    "model_type", ["mnasnet_100", "mnasnet_small", "semnasnet_100", "timm_mnasnet"]
)
def test_plugin_matches_mnasnet_variants(model_type: str):
    assert plugin.matches(model_type)


@pytest.mark.parametrize(
    "model_type", ["resnet50", "efficientnet_b0", "mobilenetv3_large_100", ""]
)
...
def test_bundle_config_reads_nested_pretrained_cfg(tmp_path: Path):
    ...
    assert bundle_config["runtime_strategy"] == "timm_mnasnet_image_classification"
