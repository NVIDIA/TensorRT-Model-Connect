...
"""Compare raw TensorRT API and ONNX->trtexec engines for timm MNASNet.

The raw API path uses TensorRT-Model-Connect's timm_mnasnet family plugin.  The
...
DEFAULT_MODEL_ID = "timm/mnasnet_100.rmsp_in1k"
...
def _build_api_engine(model_id: str, plan_path: Path, *, verbose: bool) -> None:
    ...
    if plugin is None or plugin.name != "timm_mnasnet":
        raise RuntimeError(
            f"Expected timm_mnasnet plugin for {config.model_type!r}, got {plugin}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        default=str(_repo_root() / "artifacts" / "timm_mnasnet_trt_path_comparison"),
    )
