...
def _verified_archive(path: Path, expected_sha256: str) -> Path:
    actual = _sha256(path)
    if actual != expected_sha256:
        raise ValueError(
            f"archive checksum mismatch for {path}: expected {expected_sha256}, got {actual}"
        )
    return path
...
def prepare_archives(
    ...
    *,
    ...
    ground_truth_sha256: str = GROUND_TRUTH_SHA256,
    target_height: int = TARGET_HEIGHT,
    target_width: int = TARGET_WIDTH,
) -> Path:
    """Verify, extract, transform, and describe all 15 official trainingQ scenes."""
    _verified_archive(data_archive, data_sha256)
    _verified_archive(ground_truth_archive, ground_truth_sha256)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ground_truth = (
        arguments.ground_truth_archive or archive_root / "MiddEval3-GT0-Q.zip"
    )
