...
def test_full_volume_conv_bn_folding_is_default_on_with_explicit_fallback(monkeypatch) -> None:
    from tensorrt_model_connect.families.fast_foundation_stereo.native_post import (
        _full_volume_conv_bn_folding_enabled,
    )

    variable = "TRTMC_FAST_FOUNDATION_STEREO_FOLD_FULL_VOLUME_BN"
    monkeypatch.delenv(variable, raising=False)
    assert _full_volume_conv_bn_folding_enabled()

    monkeypatch.setenv(variable, "1")
    assert _full_volume_conv_bn_folding_enabled()

    monkeypatch.setenv(variable, "0")
    assert not _full_volume_conv_bn_folding_enabled()


def test_post16_to_8_conv_bn_folding_is_default_on_with_explicit_fallback(monkeypatch) -> None:
    from tensorrt_model_connect.families.fast_foundation_stereo.native_post import (
        _post16_to_8_conv_bn_folding_enabled,
    )

    variable = "TRTMC_FAST_FOUNDATION_STEREO_FOLD_POST16_TO_8_BN"
    monkeypatch.delenv(variable, raising=False)
    assert _post16_to_8_conv_bn_folding_enabled()

    monkeypatch.setenv(variable, "1")
    assert _post16_to_8_conv_bn_folding_enabled()

    monkeypatch.setenv(variable, "0")
    assert not _post16_to_8_conv_bn_folding_enabled()


def test_feature_att_8_conv_bn_folding_is_default_on_with_explicit_fallback(monkeypatch) -> None:
    from tensorrt_model_connect.families.fast_foundation_stereo.native_post import (
        _feature_att_8_conv_bn_folding_enabled,
    )

    variable = "TRTMC_FAST_FOUNDATION_STEREO_FOLD_FEATURE_ATT_8_BN"
    monkeypatch.delenv(variable, raising=False)
    assert _feature_att_8_conv_bn_folding_enabled()

    monkeypatch.setenv(variable, "1")
    assert _feature_att_8_conv_bn_folding_enabled()

    monkeypatch.setenv(variable, "0")
    assert not _feature_att_8_conv_bn_folding_enabled()


def test_remaining_safe_conv_bn_folding_is_default_on_with_explicit_fallback(monkeypatch) -> None:
    from tensorrt_model_connect.families.fast_foundation_stereo.native_post import (
        _remaining_safe_conv_bn_folding_enabled,
    )

    variable = "TRTMC_FAST_FOUNDATION_STEREO_FOLD_REMAINING_SAFE_BN"
    monkeypatch.delenv(variable, raising=False)
    assert _remaining_safe_conv_bn_folding_enabled()

    monkeypatch.setenv(variable, "1")
    assert _remaining_safe_conv_bn_folding_enabled()

    monkeypatch.setenv(variable, "0")
    assert not _remaining_safe_conv_bn_folding_enabled()
