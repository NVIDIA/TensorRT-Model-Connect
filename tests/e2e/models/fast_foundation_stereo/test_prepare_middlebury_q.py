...
def test_transform_scene_center_crops_and_marks_edge_padding_invalid() -> None:
    ...
    left_out, right_out, disparity_out, valid_out, plan = (
        prepare_middlebury_q.transform_scene(
            left,
            right,
            disparity,
            mask,
...
def test_prepare_archives_writes_all_15_scenes_and_transform_provenance(
    tmp_path: Path,
) -> None:
    ...
    assert [row["inputs"]["scene"] for row in requests] == sorted(
        prepare_middlebury_q.SCENES
    )
