...
"""Compare raw TensorRT API and ONNX->trtexec engines for timm EfficientNet.

The raw API path uses TensorRT-Model-Connect's timm_efficientnet family plugin.  The
...
def _build_api_engine(model_id: str, plan_path: Path, *, verbose: bool) -> None:
    ...
    if plugin is None or plugin.name != "timm_efficientnet":
        raise RuntimeError(
            f"Expected timm_efficientnet plugin for {config.model_type!r}, got {plugin}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        default=str(_repo_root() / "artifacts" / "timm_efficientnet_trt_path_comparison"),
    )
