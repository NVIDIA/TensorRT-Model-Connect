...
class HfTransformersReference:
    ...
    def _run_image_classification_ref(
        ...
        script = textwrap.dedent(f"""\
            ...
            second_class = int(order[1])
            result = {{
                "top_class": top_class,
                "top_score": float(logits[top_class]),
                "num_classes": int(logits.shape[0]),
                # Reported so the comparator can tell a near-tie from a real
                # disagreement; see the top1_margin_atol threshold.
                "second_class": second_class,
                "top1_margin": float(logits[top_class] - logits[second_class]),
            }}
