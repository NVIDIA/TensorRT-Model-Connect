...
def test_workspace_limits_preserve_defaults_or_apply_exact_override() -> None:
    ...
    assert defaults == {
        ...
        "adaln_precompute.plan": "trt_default_max",
        "denoiser_head.plan": "trt_default_max",
        "denoiser_tail.plan": "trt_default_max",
        "denoiser_finish.plan": "trt_default_max",
        "fl2va_keyframe_vae_encoder.plan": 32 << 30,
        "vae_tile_decoder.plan": 96 << 30,
        "audio_vae_decoder.plan": 96 << 30,
    }
    assert _workspace_limits(None) == defaults
    assert _workspace_limits(8) == {filename: 8 << 30 for filename in defaults}
...
def test_main_passes_cli_workspace_to_every_builder(
    ...
    assert receipt["profile"]["first_block_cache"] is True
    assert receipt["denoiser_mode"] == "first_block"
