...
class HfTransformersReference:
    ...
    def _run_embedding_ref(
        ...
        script = textwrap.dedent(f"""\
            ...
            # The manifest carries the official query-side instruction format.
            # Preserve it verbatim and ensure the required EOD token is final.
            inputs = tokenizer(prompt, return_tensors="pt", padding=True,
                               truncation=True)
            eos_id = int(model.config.eos_token_id)
            if int(inputs["input_ids"][0, -1]) != eos_id:
                eos = torch.tensor([[eos_id]], dtype=inputs["input_ids"].dtype)
                one = torch.ones((1, 1), dtype=inputs["attention_mask"].dtype)
                inputs["input_ids"] = torch.cat([inputs["input_ids"], eos], dim=1)
                inputs["attention_mask"] = torch.cat(
                    [inputs["attention_mask"], one], dim=1)
            with torch.no_grad():
            ...
            if bool((last_indices < 0).any()):
                raise RuntimeError(
                    "Embedding reference received an empty mask row")
            pooled = hidden[
