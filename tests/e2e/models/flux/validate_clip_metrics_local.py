...
Usage examples:
  ...
  python tests/e2e/models/flux/validate_clip_metrics_local.py \\
      --trt-dir  /tmp/e2e_artifacts/image-diffusion/trt_frames \\
      --ref-dir  /tmp/e2e_artifacts/image-diffusion/hf_frames \\
...
def _load_prompt_from_manifest(name: str) -> str | None:
    path = _REPO_ROOT / "tests" / "e2e" / "models" / "flux" / "manifests" / f"{name}.json"
    if not path.exists():
        return None
...
def main() -> None:
    ...
    parser.add_argument("--manifest", metavar="NAME",
                        help="Manifest name (e.g. image-diffusion) to auto-load prompt")
