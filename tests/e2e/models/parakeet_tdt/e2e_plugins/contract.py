...
"""ParakeetTDT-owned speech-to-text contract plugin."""
...
class ParakeetTDTASRPlugin:
    reference_families = ["asr_parakeet_tdt"]
    user_contract = "exact_transcript"

    def configure_reference(self, case):
        return {"auto_class": "AutoModelForTDT", "parakeet_tdt": True}
    ...
    def verify(self, trt_output, ref_output, case, threshold):
        ...
        metrics = {
            "normalized_transcript_exact_match": MetricResult(
                value=1.0 if exact_match else 0.0,
                threshold=1.0,
                operator=">=",
                passed=exact_match,
                note="Composite gate for the exact_transcript user contract",
            ),
        ...
        passed = exact_match
        rule = "normalized TRT transcript == normalized reference transcript"
        if passed:
