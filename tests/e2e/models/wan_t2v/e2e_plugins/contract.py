"""Wan T2V-owned diffusion video contract plugin."""
...
def _make_pass(stage_name: str, metrics, rule: str) -> CompareResult:
    return CompareResult(
        ...
        metrics=metrics,
        composite_rule=rule,
        message="Wan T2V diffusion video contract verified",
    )
...
class WanT2VDiffusionVideoPlugin:
    ...
    def verify(self, trt_output, ref_output, case, threshold):
        ...
        return _make_fail(stage, metrics, rule, "Wan T2V diffusion video contract failed")


plugin = WanT2VDiffusionVideoPlugin()
