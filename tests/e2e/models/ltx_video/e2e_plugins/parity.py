...
"""Shared deterministic inputs for LTX Video HF-to-TRTMC parity."""
...
def ensure_initial_latents(case: E2ECase, ctx: RunContext) -> InitialLatents:
    ...
    patch = case.inputs.get("patch_size", [1, 1, 1])
    pt, ph, pw = (int(value) for value in patch)
    if min(frames, height, width, z_dim, scale_t, scale_s, pt, ph, pw) <= 0:
        raise ValueError(
            "LTX shared latent dimensions and scale factors must be positive"
        )
    ...
    if t_lat % pt or h_lat % ph or w_lat % pw:
        raise ValueError(
            f"LTX latent shape {t_lat}x{h_lat}x{w_lat} is not divisible by patch {pt}x{ph}x{pw}"
        )
    ...
    output_path = output_dir / (
        f"{case.name}.seed-{seed}.{frames}x{height}x{width}.f32"
    )
