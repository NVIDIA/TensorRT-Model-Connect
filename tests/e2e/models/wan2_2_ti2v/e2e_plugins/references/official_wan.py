...
class Wan22OfficialWanReference:
    ...
    def run_stage(self, case: E2ECase, stage: StageSpec, ctx: RunContext) -> StageOutput:
        ...
        if model_revision not in {"local", HF_REFERENCE_REVISION}:
            raise RuntimeError(
        ...
        if text_max_length != 512:
            raise ValueError("The pinned official Wan2.2 TI2V-5B reference requires text_max_length=512")
...
"""
        ...
        data: dict[str, object] = {
            ...
            "stderr": stderr,
            "prompt": prompt,
            "reference_implementation": "official_wan",
        }
        if stderr_log:
