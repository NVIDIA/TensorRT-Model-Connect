"""Focused tests for GPT-OSS tensor-parallel support."""
...
try:
    gpt_oss_module = importlib.import_module(
        "tensorrt_model_connect.families.gpt_oss.plugin")
    from tensorrt_model_connect.families.gpt_oss import tp_builder
...
def _weights() -> dict:
    ...
    weights: dict[str, object] = {
        ...
        "layer.0.router_bias": np.zeros((2,), dtype=np.float32),
        "layer.0.o_bias": np.arange(hidden, dtype=np.float32),
        "layer.0.sinks": np.arange(4, dtype=np.float32),
    }
    ...
    for key in ("q_bias", "k_bias", "v_bias"):
        weights[f"layer.0.{key}"] = np.arange(8, dtype=np.float32)
    ...
    for expert_idx in range(2):
        ...
        weights[f"{prefix}.gate_bias"] = np.arange(inter, dtype=np.float32)
        weights[f"{prefix}.up_bias"] = np.arange(inter, dtype=np.float32)
        weights[f"{prefix}.down_bias"] = np.arange(hidden, dtype=np.float32)
    return weights


def test_gpt_oss_tp_slices_attention_expert_biases_and_sinks():
    parallel = ParallelConfig(mode="tensor_parallel", tp_size=4, rank=2)
    weights = _weights()

    sharded = tp_builder.shard_gpt_oss_weights(
        _config(), weights, parallel=parallel)

    np.testing.assert_array_equal(
        sharded["layer.0.w_q"], weights["layer.0.w_q"][:, 4:6])
    np.testing.assert_array_equal(
        sharded["layer.0.q_bias"], weights["layer.0.q_bias"][4:6])
    np.testing.assert_array_equal(
        sharded["layer.0.w_o"], weights["layer.0.w_o"][4:6, :])
    np.testing.assert_array_equal(
        sharded["layer.0.o_bias"], weights["layer.0.o_bias"])
    np.testing.assert_array_equal(
        sharded["layer.0.sinks"], weights["layer.0.sinks"][2:3])
    np.testing.assert_array_equal(
        sharded["layer.0.expert.0.w_gate"],
        weights["layer.0.expert.0.w_gate"][:, 8:12],
    )
    np.testing.assert_array_equal(
        sharded["layer.0.expert.0.gate_bias"],
        weights["layer.0.expert.0.gate_bias"][8:12],
    )
    np.testing.assert_array_equal(
    ...
    np.testing.assert_array_equal(
        sharded["layer.0.expert.0.down_bias"],
        weights["layer.0.expert.0.down_bias"],
    )
    assert sharded["_attention_size"] == 2
    assert sharded["_moe_intermediate_size"] == 4


def test_gpt_oss_tp_validation_rejects_non_divisible_kv_heads():
    with pytest.raises(ValueError, match="num_key_value_heads"):
        tp_builder._validate_gpt_oss_tp(
...
def test_gpt_oss_plugin_routes_parallel_builds(monkeypatch):
    ...
    def fake_build(config, weights, max_cache_length, **kwargs):
        ...
        return b"gpt-oss-tp-plan"

    monkeypatch.setattr(
        gpt_oss_module, "require_tensorrt_11_for_tensor_parallel", fake_require)
    monkeypatch.setattr(tp_builder, "build_gpt_oss_tp_engine", fake_build)

    parallel = ParallelConfig(mode="tensor_parallel", tp_size=4, rank=1)
    result = gpt_oss_module.GptOssPlugin().build_engine(
        _config(), _weights(), 17,
        verbose=True,
    ...
    assert result == b"gpt-oss-tp-plan"
    assert calls["require"][0] == parallel
    assert "GPT-OSS tensor-parallel" in calls["require"][1]
