...
"""Focused tests for Granite tensor-parallel dispatch."""
...
def test_granite_plugin_routes_parallel_builds(monkeypatch) -> None:
    module = importlib.import_module(
        "tensorrt_model_connect.families.granite.plugin")
    calls: dict[str, object] = {}

    def fake_build(config, weights, max_cache_length, **kwargs):
        calls["build"] = (config, weights, max_cache_length, kwargs)
        return b"granite-tp-plan"
    ...
    result = module.GranitePlugin().build_engine(
        _config(),
    ...
    assert result == b"granite-tp-plan"
