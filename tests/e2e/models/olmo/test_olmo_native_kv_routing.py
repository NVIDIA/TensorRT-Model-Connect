...
"""CPU-only contracts for OLMo's TensorRT native KV path."""
...
from tensorrt_model_connect.families.olmo.build_routing import (
...
from tensorrt_model_connect.families.olmo.config import ModelConfig
from tensorrt_model_connect.families.olmo.native_kv_contract import (
...
def test_olmo_1b_uses_full_context_native_kv():
    config = _config()
    ...
    assert resolved_rotary_dim(config) == 128
    assert row_bytes == 2 * 16 * 16 * 128 * 2
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({}, {"partial_rotary_factor": 0.5}, "full-head RoPE"),
...
def test_weight_contract_rejects_missing_shape_and_foreign_weights():
    ...
    foreign["layer.0.q_bias"] = _Tensor((64,))
...
def test_plugin_builds_only_the_requested_native_split_role(monkeypatch):
    ...
    plugin_module = importlib.import_module("tensorrt_model_connect.families.olmo.plugin")
