...
"""Pinned Hugging Face Transformers oracle for the dense LFM2 family."""
...
class Lfm2HfTransformersReference:
    """Run the exact checkpoint revision with native Transformers LFM2 code."""
    ...
    def run_stage(self, case: E2ECase, stage: StageSpec, ctx: RunContext) -> StageOutput:
        if stage.name != "full_generation":
            raise ValueError(f"LFM2 reference supports only full_generation, got {stage.name!r}")
        ...
        try:
            ...
            raise RuntimeError(f"Pinned LFM2 reference timed out after {elapsed:.0f}s") from exc
        elapsed = time.monotonic() - started
        ...
        if result.returncode != 0:
            ...
            message = f"Pinned LFM2 reference failed (rc={result.returncode}): {truncated}"
            if log_path:
...
plugin = Lfm2HfTransformersReference()
