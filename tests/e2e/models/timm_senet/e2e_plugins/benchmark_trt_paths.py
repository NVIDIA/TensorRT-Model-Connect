...
"""Compare raw TensorRT API and ONNX->trtexec engines for timm SENet.

The raw API path uses TensorRT-Model-Connect's timm_senet family plugin.  The
...
DEFAULT_MODEL_ID = "timm/senet154.gluon_in1k"
...
def _build_api_engine(model_id: str, plan_path: Path, *, verbose: bool) -> None:
    ...
    if plugin is None or plugin.name != "timm_senet":
        raise RuntimeError(
            f"Expected timm_senet plugin for {config.model_type!r}, got {plugin}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-dir",
        default=str(_repo_root() / "artifacts" / "timm_senet_trt_path_comparison"),
    )
