...
"""Tests for the timm SENet image-classification family plugin."""
...
try:
    ...
    from tensorrt_model_connect.families.timm_senet import plugin
...
def _write_tiny_repvgg(tmp_path: Path, *, blocks_per_stage: tuple[int, ...] = (2, 1)) -> None:
    """Write a miniature SENet in its multi-branch training form."""
    classes = 5
    config = {
...
def test_model_config_uses_timm_architecture_when_model_type_absent(tmp_path: Path):
    ...
    assert cfg.model_type == "senet154"
    assert plugin.matches(cfg.model_type)


@pytest.mark.parametrize("model_type", ["senet154", "repvgg_b0", "timm_senet"])
...
def test_stride_is_derived_from_the_identity_branch(tmp_path: Path):
    ...
    blocks = cfg.raw["_timm_senet_config"]["blocks"]
...
def test_load_weights_fuses_every_block_to_one_kernel(tmp_path: Path):
    ...
    for block in cfg.raw["_timm_senet_config"]["blocks"]:
