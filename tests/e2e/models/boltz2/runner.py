...
"""Model-owned E2E entrypoint support for Boltz-2."""
...
def _resolve_engine_dir(config) -> str:
    ...
    directory = (
        Path(configured)
        if configured
        else Path.home() / ".cache" / "trtmc" / "engines"
    )
...
def _resolve_artifacts_dir(config) -> str:
    ...
    return str(Path(configured)) if configured else str(Path("/tmp/e2e_artifacts") / "boltz2")
...
def _resolve_ld_library_path() -> str:
    ...
    return ":".join(path for path in (trt_lib_dir, "/usr/local/cuda/lib64", existing) if path)
