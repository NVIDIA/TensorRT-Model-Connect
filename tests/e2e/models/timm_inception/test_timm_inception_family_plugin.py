...
try:
    ...
    from tensorrt_model_connect.families.timm_inception import plugin
...
def test_model_config_uses_timm_architecture_when_model_type_absent(tmp_path: Path):
    ...
    assert cfg.model_type == "inception_v3"
    assert plugin.matches(cfg.model_type)


@pytest.mark.parametrize("model_type", ["inception_v3", "timm_inception"])
...
def test_bundle_config_uses_the_inception_input_size_and_normalisation(tmp_path: Path):
    ...
    assert bundle_config["runtime_strategy"] == "timm_inception_image_classification"
