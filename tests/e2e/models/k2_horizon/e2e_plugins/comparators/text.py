...
def _contains_expected_answer(text: str, answer: str) -> bool:
    ...
    if norm_answer.isalnum():
        return (
            re.search(rf"(?<![a-z0-9]){re.escape(norm_answer)}(?![a-z0-9])", norm_text) is not None
        )
...
def _check_numerical_health(arr: np.ndarray, label: str) -> list[str]:
    ...
    if arr.size > 0:
        abs_max = (
            float(np.nanmax(np.abs(arr[np.isfinite(arr)]))) if np.any(np.isfinite(arr)) else 0.0
        )
        if abs_max > 1e6:
    ...
    def compare(
        ...
        if trt_logits is None or ref_logits is None:
            return CompareResult(
                ...
                status=StageStatus.ERROR.value,
                metrics=metrics,
                message="K2-Horizon parity requires both TRT and HF logits",
            )
        ...
        if trt_logits.shape != ref_logits.shape:
            return CompareResult(
                stage_name=stage.name,
                status=StageStatus.ERROR.value,
                metrics=metrics,
                message=(
                    "K2-Horizon logits must have identical shapes: "
                    f"TRT={trt_logits.shape}, HF={ref_logits.shape}"
                ),
            )
        n_steps, vocab_size = trt_logits.shape
        if n_steps == 0 or vocab_size < 2:
            return CompareResult(
                stage_name=stage.name,
                status=StageStatus.ERROR.value,
                metrics=metrics,
                message=f"K2-Horizon logits must be nonempty with vocab >= 2: {trt_logits.shape}",
            )
        if not np.isfinite(trt_logits).all() or not np.isfinite(ref_logits).all():
            return CompareResult(
                stage_name=stage.name,
                status=StageStatus.ERROR.value,
                metrics=metrics,
                message="K2-Horizon logits must contain only finite values",
            )

        trt_clean = trt_logits
        ref_clean = ref_logits
        notes: list[str] = []

        # Numerical health
        health_warnings = []
        health_warnings.extend(_check_numerical_health(trt_clean, "TRT logits"))
        health_warnings.extend(_check_numerical_health(ref_clean, "HF logits"))
        notes.extend(health_warnings)

        thresh = threshold.metrics

        # --- Metric 1: logit_cosine_p5 ---
        cosines = np.array([cosine_similarity(trt_clean[i], ref_clean[i]) for i in range(n_steps)])
        ...
        metrics["logit_cosine_p5"] = MetricResult(
            value=logit_cosine_p5,
            threshold=cosine_thresh,
            operator=">=",
            passed=logit_cosine_p5 >= cosine_thresh,
        )

        # --- Metric 2: logit_rel_l2_p95 ---
        rel_l2s = np.array([_relative_l2(trt_clean[i], ref_clean[i]) for i in range(n_steps)])
        ...
        metrics["logit_rel_l2_p95"] = MetricResult(
            value=logit_rel_l2_p95,
            threshold=rel_l2_thresh,
            operator="<=",
            passed=logit_rel_l2_p95 <= rel_l2_thresh,
        )
        ...
        metrics["stable_top1_match_rate"] = MetricResult(
            value=stable_top1_match_rate,
            threshold=stable_thresh,
            operator=">=",
            passed=stable_top1_match_rate >= stable_thresh,
        ...
        metrics["unstable_topk_hit_rate"] = MetricResult(
            value=unstable_topk_hit_rate,
            threshold=topk_thresh,
            operator=">=",
            passed=unstable_topk_hit_rate >= topk_thresh,
        ...
        metrics["token_agreement_rate"] = MetricResult(
            value=token_agreement_rate,
            threshold=ta_thresh,
            operator=">=",
            passed=token_agreement_rate >= ta_thresh,
        )
        ...
        ref_text_for_ned = _normalize_for_ned(
            _truncate_after_first_turn(_strip_leading_role_prefix(ref_text))
        )
        ...
        # --- Composite gating ---
        logit_quality_ok = metrics["logit_cosine_p5"].passed or metrics["logit_rel_l2_p95"].passed

        token_level_ok = metrics["token_agreement_rate"].passed or (
            metrics["stable_top1_match_rate"].passed and metrics["unstable_topk_hit_rate"].passed
        )

        text_ok = metrics["normalized_text_edit_distance"].passed or expected_answer_ok
        ...
        return CompareResult(
            ...
            metrics=metrics,
            composite_rule=(
                _COMPOSITE_RULE_WITH_EXPECTED_ANSWER if expected_answers else _COMPOSITE_RULE
            ),
            message=message,
        )
