...
"""K2-Horizon causal-generation runner using the pure-C++ CLI plus local debug adapter."""
...
class TextGenerationCausalRunner:
    ...
    def run_stage(self, case: E2ECase, stage: StageSpec, ctx: RunContext) -> StageOutput:
        if stage.name != "full_generation":
            raise ValueError(f"K2-Horizon runner supports only full_generation, got {stage.name!r}")
    ...
    def _run_cpp_binary(
        ...
        logger.info("K2-Horizon C++ inference: %s", " ".join(command))
    ...
    def _run_debug_logits(
        ...
        script = textwrap.dedent(
            ...
            from tensorrt_model_connect.families.k2_horizon.debug_runner import (
            ...
            # HF ``generate(..., output_scores=True)`` exposes the logits that
            # select each generated token. The first such row is produced by
            # the final prompt token, so align the native one-token trace to
