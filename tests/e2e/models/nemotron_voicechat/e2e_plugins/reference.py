        ...
        if stage.name == "realtime_websocket_interop":
            return StageOutput(
                ...
                data={
                    ...
                    "expected_input_wire_samples": (
                        round(case.metadata["function_speech_source_num_samples"] * 24000 / 16000)
                        + 6 * 24000
                    ),
                    "expected_input_wire_samples_by_scenario": {
                        "function": round(
                            case.metadata["function_speech_source_num_samples"] * 24000 / 16000
                        )
                        + 2 * 24000,
                        "truncate": 48000,
                        "cancel": 48000,
                    ...
                    "audio_output_path": str(
                        _persist_reference_audio(
                            case, ctx, "realtime_websocket_interop_reference.flac"
                        )
                    ),
                },
                metadata={"source": "model_owned_l4_realtime_transport_invariants"},
            )
