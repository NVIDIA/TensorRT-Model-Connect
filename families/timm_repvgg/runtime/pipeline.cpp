...
const Tensor& require_logits(const TensorMap& outputs) {
    for (const auto& [name, tensor] : outputs) {
        ...
        if (tensor.data == nullptr || tensor.dtype != DType::kFloat32 || tensor.numel() <= 0)
            throw std::runtime_error("timm RepVGG engine must return nonempty float32 logits");
        return tensor;
    }
    throw std::runtime_error("timm RepVGG engine did not return logits");
}
...
TimmRepvggImageClassificationPipeline::TimmRepvggImageClassificationPipeline(
    ...
    if (num_classes_ <= 0 ||
        ...
        throw std::runtime_error("timm RepVGG class metadata does not match its output size");
}

internal::LabelScoresResult
TimmRepvggImageClassificationPipeline::run(const internal::ImageToClassScoresRequest& request,
                                           internal::ConfigView config) {
    if (!config.empty())
        throw internal::ConfigError("timm RepVGG has no runtime configuration");
    ...
    if (image.format != internal::ImageFormat::Float32 || image.channels != 3 ||
        ...
        throw std::invalid_argument("timm RepVGG requires contiguous float32 RGB input");
    auto pixel_values = preprocess_timm_repvgg_image(
    ...
    if (n != static_cast<std::size_t>(num_classes_))
        throw std::runtime_error("timm RepVGG logits do not match its configured class count");
