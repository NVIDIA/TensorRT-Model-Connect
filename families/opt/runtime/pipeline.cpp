...
#include "families/opt/runtime/pipeline.h"

#include "families/opt/runtime/chat_templates.h"
#include "families/opt/runtime/kv_cache.h"
#include "families/opt/runtime/tensor_names.h"
...
OptTextGenerationPipeline::OptTextGenerationPipeline(std::unique_ptr<ITrtModule> decoder,
                                                     std::unique_ptr<OptInferenceState> state,
                                                     OptTextGenConfig config,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("OptTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("OptTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
TextResult OptTextGenerationPipeline::run(const internal::TextContinuationRequest& request,
                                          internal::ConfigView supplied) {
    const auto cfg = opt::parse_text_config(supplied);
    ...
    if (const auto* prompt = std::get_if<std::string_view>(&request.prefix)) {
        ...
        input_ids = opt::copy_token_prefix(std::get<Span<const std::int32_t>>(request.prefix));
    }
...
void require_prefill_kv_pointers(ITrtModule& prefill, const OptTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(opt_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(opt_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "OptTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const OptTextGenConfig& cfg, int32_t sq,
                                      OptInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("OptTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error("OptTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("OptTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error("OptTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<OptKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("OptTextGenerationPipeline: prefill requires OptKvCache");
}

} // namespace

void OptTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
...
std::string
OptTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
OptTextGenerationPipeline::TimedGenResult OptTextGenerationPipeline::generate_from_ids(
    ...
    if (mode != "ar")
        throw std::runtime_error("OptTextGenerationPipeline: unsupported generation mode '" + mode +
                                 "'");
    auto active_sampler = create_opt_sampler(params);
...
OptTextGenerationPipeline::run_decode_loop(OptISampler* sampler, const OptSamplingParams& params,
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const OptSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& OptTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
