...
// Unit tests for TDT-owned audio feature extraction helpers.
...
void test_tdt_fft_matches_direct_dft() {
    ...
    check_vectors_close(trtmc::tdt::detail::rfft_power(input), reference_rfft_power(input), 1e-5F,
                        "tdt 512-point FFT matches direct DFT");
}

trtmc::tdt::MelSpectrogramOptions make_incremental_options() {
    trtmc::tdt::MelSpectrogramOptions options;
    options.n_fft = 8;
    ...
    options.log_scale = trtmc::tdt::MelLogScale::kNaturalLog;
...
std::vector<float> materialize_incremental(const trtmc::tdt::IncrementalMelSpectrogram& state) {
...
std::vector<float> run_resampled_chunks(const std::vector<float>& source_audio,
                                        ...
                                        trtmc::tdt::IncrementalMelStats& stats) {
    const auto options = make_incremental_options();
    const auto filters = make_identity_filterbank(5);
    trtmc::tdt::IncrementalMelSpectrogram state(filters.data(), 5, 5, options, 96);
    int32_t accepted = 0;
...
void test_incremental_mel_processes_each_frame_once() {
    ...
    const auto offline = trtmc::tdt::extract_configured_mel_spectrogram(
        audio.data(), static_cast<int32_t>(audio.size()), filters.data(), 5, 5, options);

    trtmc::tdt::IncrementalMelSpectrogram state(filters.data(), 5, 5, options, 32);
...
void test_incremental_resample_matches_one_shot() {
    ...
    const auto resampled = trtmc::tdt::resample_linear(source_audio.data(), 96, 96, 32);
    const auto offline = trtmc::tdt::extract_configured_mel_spectrogram(
...
void test_incremental_resample_is_chunk_size_independent() {
    const auto source_audio = make_test_audio(96);
    trtmc::tdt::IncrementalMelStats coarse_stats;
    trtmc::tdt::IncrementalMelStats irregular_stats;
...
void test_tdt_mel_matches_configured_owned_options() {
    ...
    const auto tdt = trtmc::tdt::extract_tdt_mel_spectrogram(
    ...
    const auto configured = trtmc::tdt::extract_configured_mel_spectrogram(
        samples.data(), static_cast<int32_t>(samples.size()), mel_filters.data(), n_freq_bins,
        n_mel_bins, options);

    check(tdt.n_mels == configured.n_mels, "tdt mel keeps configured mel count");
    check(tdt.n_frames == configured.n_frames, "tdt mel keeps configured frame count");
    check(tdt.data.size() == configured.data.size(), "tdt mel keeps configured data size");
    for (std::size_t i = 0; i < tdt.data.size(); ++i) {
        check_close(tdt.data[i], configured.data[i], 1e-6F, "tdt mel value matches configured");
    }
}
...
int main() {
    ...
    test_tdt_mel_matches_configured_owned_options();
    ...
    if (g_failures != 0) {
        std::cerr << g_failures << " tdt audio helper test(s) failed\n";
        return 1;
    }
    return 0;
}
