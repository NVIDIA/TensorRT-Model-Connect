...
T require_value(const nlohmann::json& json, const char* name) {
    if (!json.contains(name))
        throw std::runtime_error(std::string("phi_moe runtime.json missing '") + name + "'");
    try {
        return json.at(name).get<T>();
    } catch (const nlohmann::json::exception&) {
        throw std::runtime_error(std::string("phi_moe runtime.json has invalid '") + name + "'");
    }
}
...
RuntimeConfig parse_runtime_config(const BundleReader& bundle) {
    ...
    try {
        ...
        throw std::runtime_error("phi_moe invalid runtime.json: " + std::string(error.what()));
    }
    if (!json.is_object())
        throw std::runtime_error("phi_moe runtime.json must be an object");
    if (json.size() != 14)
        throw std::runtime_error("phi_moe runtime.json has an unexpected field set");
    ...
    if (config.hidden_size <= 0 || config.num_layers <= 0 || config.num_heads <= 0 ||
        ...
        throw std::runtime_error("phi_moe runtime.json contains invalid dimensions");
    }
    if (config.num_key_value_heads % config.tensor_parallel_size != 0)
        throw std::runtime_error("phi_moe KV heads must be divisible by tensor parallel size");
    if (config.precision != "fp16" && config.precision != "bf16" && config.precision != "fp32") {
        throw std::runtime_error("phi_moe runtime.json contains invalid precision");
    }
    if (config.decoder_engine_layout != "single" && config.decoder_engine_layout != "split" &&
        config.decoder_engine_layout != "dual_profile") {
        throw std::runtime_error("phi_moe runtime.json contains invalid decoder_engine_layout");
    }
    if (config.decoder_engine_layout != "dual_profile" && config.tensor_parallel_size != 1)
        throw std::runtime_error("phi_moe non-dual-profile engines require tensor_parallel_size=1");
    const std::string expected_mode =
        config.tensor_parallel_size > 1 ? "tensor_parallel" : "single";
    if (config.tensor_parallel_mode != expected_mode)
        throw std::runtime_error("phi_moe runtime.json has inconsistent tensor_parallel_mode");
    return config;
}
...
PhiMoeKvCacheNames make_kv_names(std::int32_t num_layers) {
    PhiMoeKvCacheNames names;
    for (std::int32_t layer = 0; layer < num_layers; ++layer) {
        names.cache_k.push_back(phi_moe_layer_tensor_name("cache_k", layer));
        names.cache_v.push_back(phi_moe_layer_tensor_name("cache_v", layer));
        names.present_k.push_back(phi_moe_layer_tensor_name("present_k", layer));
        names.present_v.push_back(phi_moe_layer_tensor_name("present_v", layer));
    }
    return names;
}
...
std::string chat_template(const BundleReader& bundle) {
    ...
    if (section == nullptr || section->length == 0)
        throw std::runtime_error("phi_moe bundle requires chat_template.jinja");
...
DecoderModules load_modules(const FamilyContext& context, const RuntimeConfig& config) {
    ...
    if (config.decoder_engine_layout == "single") {
        modules.decode = load_engine(
            context.backend, require_section(context.reader, "engine.plan"), "phi_moe decoder");
        return modules;
    }
    if (config.decoder_engine_layout == "split") {
        ...
        modules.prefill = load_engine(
            context.backend, require_section(context.reader, "prefill.plan"), "phi_moe prefill");
        return modules;
    }
    ...
    if (dual.decode == nullptr || !dual.decode->ok() || dual.prefill == nullptr ||
        !dual.prefill->ok()) {
        throw std::runtime_error("phi_moe dual-profile engine did not create both contexts");
    }
...
ITask* create(const FamilyContext& context) {
    ...
    auto state = std::make_unique<PhiMoeKvCache>(config.num_layers, config.max_cache_length, kv_dim,
                                                 stream, cache_dtype(config.precision),
                                                 make_kv_names(config.num_layers));
    if (!state->ok())
        throw std::runtime_error("phi_moe failed to create KV cache");
    ...
    return new PhiMoeTextGenerationPipeline(
...
TRTMC_DEFINE_FAMILY_PLUGIN_V1("phi_moe")
