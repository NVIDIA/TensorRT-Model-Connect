...
//
// Composes: ITrtModule (decoder) + PhiMoeKvCache + ITokenizer for this runtime
...
#include "families/phi_moe/runtime/inference_state.h"
#include "families/phi_moe/runtime/sampler.h"
...
class PhiMoeTextGenerationPipeline final : public ITextGeneration {
  public:
    PhiMoeTextGenerationPipeline(std::unique_ptr<ITrtModule> decoder,
                                 std::unique_ptr<PhiMoeInferenceState> state,
                                 PhiMoeTextGenConfig config, std::shared_ptr<ITokenizer> tokenizer,
