...
def _official_reference(model_dir: Path, manifest: dict, case: dict, tmp_path: Path):
    ...
    processor = processor_class.from_pretrained(model_dir, trust_remote_code=True, use_fast=False)
...
def test_hf_reference_uses_the_slow_segformer_processor() -> None:
    source = Path(__file__).read_text(encoding="utf-8")
    assert "model_dir, trust_remote_code=True, use_fast=False" in source
...
def test_official_checkpoint_e2e(case_name: str, tmp_path: Path) -> None:
    ...
    record_mask_views(actual, expected, TEST_ROOT / case["test_image"], tmp_path / "report-views")
