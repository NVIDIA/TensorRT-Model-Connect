...
#include "families/modernbert/runtime/distributed_runtime.h"
#include "families/modernbert/runtime/pipeline.h"
#include "families/modernbert/runtime/plugin_helpers.h"
...
namespace trtmc::modernbert_factory {
namespace {
...
nlohmann::json require_config(const BundleReader& bundle) {
    ...
    if (!config.is_object())
        throw std::runtime_error("ModernBERT runtime.json must be an object");
    return config;
}
...
std::int32_t require_tensor_parallel_size(const nlohmann::json& config) {
    if (!config.contains("tensor_parallel_size") ||
        ...
        throw std::runtime_error("ModernBERT runtime.json requires tensor_parallel_size");
    }
    const auto value = config.at("tensor_parallel_size").get<std::int64_t>();
    if (value <= 0 || value > std::numeric_limits<std::int32_t>::max())
        throw std::runtime_error("ModernBERT tensor_parallel_size is invalid");
...
TRTMC_DEFINE_FAMILY_PLUGIN_V1("modernbert")
...
extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    ...
    const auto& plan = trtmc::modernbert_factory::require_section(context.reader, section.c_str());
