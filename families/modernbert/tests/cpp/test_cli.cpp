...
class FakeModernmodernbert final : public trtmc::IEncoding,
                                   public trtmc::IEmbedding,
                                   public trtmc::IReranking {
  public:
...
void modernbert_contract(const fs::path& root) {
    const auto path = root / "modernbert.bundle";
    write_bundle(path, "modernbert");
    ...
    check(encoded.first == 0 && Json::parse(encoded.second.output).at("values") == Json({3.0, 1.0}),
          "MODERNBERT owner converts the encoding request and preserves result fields");
    check(loaded_runtime == "/installed/runtime",
          "MODERNBERT owner uses the supplied installed runtime root");
    const auto embedded = invoke("embed", {{"bundle", path.string()},
                                           {"text", "abc"},
    ...
    check(embedded.first == 0 &&
          ...
          "MODERNBERT owner selects the embedding interface");
    check(loaded_runtime == "/override/runtime",
          "runtime_root override remains owned by MODERNBERT");
    check(loaded_cache == "/cache/runtime" && loaded_graphs,
          "owner forwards backend cache and graph settings");
    const auto ranked =
        invoke("rerank", {{"bundle", path.string()}, {"query", "a"}, {"document", "a"}});
    check(ranked.first == 0 && Json::parse(ranked.second.output).at("score") == 1.0,
          "MODERNBERT owner selects the reranking interface");
    write_bundle(path, "another_family");
    const auto before = task_loads;
    check(invoke("encode", {{"bundle", path.string()}, {"text", "a"}}).first != 0 &&
              task_loads == before,
          "MODERNBERT rejects another family's bundle before loading it");
    check(invoke("unknown", {{"bundle", path.string()}}).first != 0 && task_loads == before,
          "MODERNBERT rejects unknown handlers before loading a task");
}
...
std::unique_ptr<ITask> load_task(const BundleReader&, const std::string& runtime_root,
    ...
    return std::make_unique<FakeModernmodernbert>();
}
} // namespace trtmc

int main() {
    const auto root =
        fs::temp_directory_path() / ("trtmc-modernbert-cli-" + std::to_string(getpid()));
    fs::create_directories(root);
    try {
        modernbert_contract(root);
