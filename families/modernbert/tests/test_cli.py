...
from families.modernbert import cli, support
...
def test_cli_defaults_match_support_and_publish_a_bundle(monkeypatch, tmp_path):
    ...
    def select_backend(backend):
        assert backend == "trt"
        monkeypatch.setitem(
            sys.modules, "families.modernbert.model", SimpleNamespace(build=build_model)
        )

    monkeypatch.setattr(cli, "select_backend", select_backend)
    output = tmp_path / "model.bundle"
    declaration = support.describe(ModelMetadata({"model_type": "modernbert"}, {}))
    assert declaration is not None
    assert family_cli.main(["modernbert", "build", str(tmp_path), "-o", str(output)]) == 0
...
def test_failed_owner_preserves_existing_output(monkeypatch, tmp_path):
    ...
    monkeypatch.setitem(sys.modules, "families.modernbert.model", SimpleNamespace(build=fail))
...
def test_legacy_unsupported_inputs_are_still_rejected(tmp_path, changes):
    legacy = LegacyRequest(tmp_path, tmp_path / "out", "modernbert", "encoding", "fp32")
...
def test_help_is_lazy_and_owned(monkeypatch, capsys):
    ...
    def guarded(name, *args, **kwargs):
        assert name not in {
            "families.modernbert.cli",
            "families.modernbert.model",
            "tensorrt",
            "tensorrt_rtx",
            "huggingface_hub",
        }
    ...
    with pytest.raises(SystemExit) as error:
        family_cli.main(["modernbert", "build", "--help"])
