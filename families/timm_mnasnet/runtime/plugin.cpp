...
TimmMnasnetPreprocessConfig parse_config(const nlohmann::json& json) {
    TimmMnasnetPreprocessConfig config;
    ...
    if (config.input_image_h <= 0 || config.input_image_w <= 0 || config.crop_pct <= 0.0F ||
        ...
        throw std::runtime_error("timm MNASNet runtime.json does not match its runtime contract");
    }
    return config;
}
...
std::unique_ptr<ITrtModule> load_engine(IBackend& backend, const std::vector<char>& plan) {
    ...
    if (!engine || !engine->ok())
        throw std::runtime_error("timm MNASNet engine failed to load");
    return engine;
}
...
extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    if (context.kv_cache_size_bytes != 0)
        throw std::invalid_argument("timm_mnasnet does not support --kv-cache-size");
    const auto& config_data = trtmc::timm_mnasnet::require_section(context.reader, "runtime.json");
    const auto& plan = trtmc::timm_mnasnet::require_section(context.reader, "engine.plan");
    const auto metadata = nlohmann::json::parse(config_data.begin(), config_data.end());
    auto config = trtmc::timm_mnasnet::parse_config(metadata);
    auto engine = trtmc::timm_mnasnet::load_engine(context.backend, plan);
    return new trtmc::TimmMnasnetImageClassificationPipeline(
