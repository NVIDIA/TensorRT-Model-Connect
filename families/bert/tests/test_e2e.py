...
"""Direct build, native-runtime, and official-reference E2E for bert."""
...
def test_encoder_only_cases_keep_the_cls_contract() -> None:
    expected = {
        "bge-small-en-v1.5": "encoding",
        "paraphrase-multilingual-minilm-l12-v2": "encoding",
    }
    assert {
        name: manifest["task"] for name, (_, manifest, _case) in CASES.items() if name in expected
    } == expected
