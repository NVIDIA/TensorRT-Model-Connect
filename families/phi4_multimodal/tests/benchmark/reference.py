...
"""Run the official Transformers Phi-4 Multimodal reference for qualification."""
...
def _prepare(reference: Reference, image_path: str, prompt: str) -> dict[str, Any]:
    ...
    rendered = reference.processor.tokenizer.apply_chat_template(
    ...
    if (
        ...
        or not any(
            marker in rendered
            for marker in ("<|image_1|>", "<|image_pad|>", "<|vision_start|>", "<image>")
        )
    ):
        raise ValueError("Phi-4 Multimodal chat template lost the image placeholder")
    encoded = reference.processor(text=rendered, images=image, return_tensors="pt")
    return {
        name: value.to(
            device="cuda",
...
def _performance(arguments: argparse.Namespace) -> int:
    ...
    if arguments.family != "phi4_multimodal" or arguments.operation != "generate":
        raise ValueError("Phi-4 Multimodal reference only supports phi4_multimodal generate")
    if arguments.mode != "pytorch-eager":
        raise ValueError("Phi-4 Multimodal reference only supports pytorch-eager")
