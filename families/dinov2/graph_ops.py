...
"""Family-owned strongly-typed TensorRT graph helpers for DINOv2."""
...
def gelu(network, tensor, dtype: np.dtype):
    """Exact PyTorch GELU used by released DINOv2 checkpoints."""
    rank = len(tuple(tensor.shape))
    scalar_shape = (1,) * rank
...
def activation(network, tensor, name: str, dtype: np.dtype):
    ...
    raise ValueError(f"Unsupported DINOv2 activation: {name}")
...
def attention(network, q, k, v, head_dim: int):
    """Scaled dot-product attention with float32 scores and softmax.

    DINOv2 develops high-norm tokens whose attention logits lose too much
    precision in half precision; only the probability-weighted sum of `v`
    runs at the value precision.
    """
    q = cast(network, q, trt.float32)
    k = cast(network, k, trt.float32)
    scalar = constant(network, np.asarray(1.0 / np.sqrt(float(head_dim))), (1, 1, 1, 1), np.float32)
    ...
    probs = cast(network, probs_layer.get_output(0), v.dtype)
    return network.add_matrix_multiply(
        probs,
