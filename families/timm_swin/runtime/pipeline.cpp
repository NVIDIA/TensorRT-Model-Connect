...
ClassificationResult TimmSwinImageClassificationPipeline::classify(const float* pixels,
    ...
    auto values = preprocess_timm_swin_image(pixels, height, width, preprocess_config_);
    Tensor input;
    ...
    if (logits == nullptr || logits->numel() <= 0)
        throw std::runtime_error("timm Swin engine returned no logits");
    if (logits->dtype != DType::kFloat32)
        throw std::runtime_error("timm Swin logits must be float32");
