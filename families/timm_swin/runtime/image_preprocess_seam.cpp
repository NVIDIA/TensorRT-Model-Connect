...
stbir_filter resize_filter(const std::string& interpolation) {
    ...
    throw std::invalid_argument("Unsupported timm Swin interpolation: " + interpolation);
}

void validate_config(const TimmSwinPreprocessConfig& config) {
    if (config.input_image_h <= 0 || config.input_image_w <= 0)
        throw std::invalid_argument("timm Swin input dimensions must be positive");
    if (config.crop_pct <= 0.0F || config.crop_pct > 1.0F)
        throw std::invalid_argument("timm Swin crop_pct must be in (0, 1]");
    if (config.image_mean.size() != 3 || config.image_std.size() != 3)
        throw std::invalid_argument("timm Swin image mean/std must contain three channels");
    for (const float value : config.image_std) {
        if (value == 0.0F)
            throw std::invalid_argument("timm Swin image std must be non-zero");
    }
}
...
TimmSwinResizeShape compute_timm_swin_resize_shape(int32_t image_height, int32_t image_width,
    ...
    if (image_height <= 0 || image_width <= 0)
        throw std::invalid_argument("timm Swin source dimensions must be positive");
...
std::vector<float> preprocess_timm_swin_image(const float* image_pixels, int32_t image_height,
    ...
    if (image_pixels == nullptr || image_height <= 0 || image_width <= 0)
        throw std::invalid_argument("timm Swin source image must be non-empty");
    validate_config(config);
    const auto shape = compute_timm_swin_resize_shape(image_height, image_width, config);
    ...
    if (stbir_resize(
        ...
        throw std::runtime_error("Failed to resize timm Swin input image");
    }
