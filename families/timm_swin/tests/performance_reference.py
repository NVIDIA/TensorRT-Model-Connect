...
"""Time Swin inference and host logits, not classification/reporting."""
...
def _load_reference(arguments, request):
    ...
    missing, unexpected = model.load_state_dict(
        load_file(str(checkpoint / "model.safetensors")), strict=False
    )
    if missing or any(name.rsplit(".", 1)[-1] not in {"attn_mask", "relative_position_index"} for name in unexpected):
        raise ValueError("Swin checkpoint is missing learned tensors or contains unknown tensors")
...
def _summary(scores):
    ...
    if scores.ndim != 2 or scores.shape[0] != 1 or scores.shape[1] == 0:
        raise ValueError("Swin must return one nonempty class-score vector")
    if not np.isfinite(scores).all():
        raise ValueError("Swin returned nonfinite class scores")
    return {
...
def run(arguments):
    ...
    if (arguments.family != "timm_swin" or arguments.operation != "classify"
            or arguments.selected_task != "image_to_class_scores"):
        raise ValueError("reference requires timm_swin image_to_class_scores/classify")
