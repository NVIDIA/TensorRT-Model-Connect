...
"""Small TensorRT graph vocabulary owned by timm CrossViT."""
...
def patch_convolution(
    ...
    *,
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected the CrossViT patch convolution")
...
def reshape(network, tensor, shape: tuple[int, ...]):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a CrossViT reshape")
...
def permute(network, tensor, first, permutation, second):
    ...
    layer = network.add_shuffle(tensor)
    if layer is None:
        raise RuntimeError("TensorRT rejected a CrossViT shuffle")
...
def matmul_constant(
    ...
    *,
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a CrossViT linear weight")
    values = layer.get_output(0)
    if values.dtype != tensor.dtype:
        cast = network.add_cast(values, tensor.dtype)
        if cast is None:
            raise RuntimeError("TensorRT rejected a CrossViT linear cast")
    ...
    if product is None:
        raise RuntimeError("TensorRT rejected a CrossViT matmul")
    ...
    if bias_layer is None:
        raise RuntimeError("TensorRT rejected a CrossViT linear bias")
    values = bias_layer.get_output(0)
    if values.dtype != output.dtype:
        cast = network.add_cast(values, output.dtype)
        if cast is None:
            raise RuntimeError("TensorRT rejected a CrossViT bias cast")
        values = cast.get_output(0)
    summed = network.add_elementwise(output, values, trt.ElementWiseOperation.SUM)
    if summed is None:
        raise RuntimeError("TensorRT rejected a CrossViT bias add")
...
def layer_norm(
    ...
    *,
    ...
    if scale is None or shift is None:
        raise RuntimeError("TensorRT rejected the CrossViT norm parameters")
    ...
    if scale_tensor.dtype != tensor.dtype:
        ...
        if scale_cast is None or shift_cast is None:
            raise RuntimeError("TensorRT rejected the CrossViT norm cast")
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a CrossViT layer norm")
...
def gelu(network, tensor, *, dtype: np.dtype):
    ...
    def constant(value: float):
        ...
        if layer is None:
            raise RuntimeError("TensorRT rejected a CrossViT GELU constant")
        output = layer.get_output(0)
        if output.dtype == tensor.dtype:
            return output
        cast = network.add_cast(output, tensor.dtype)
        if cast is None:
            raise RuntimeError("TensorRT rejected a CrossViT GELU cast")
    ...
    if scaled is None:
        raise RuntimeError("TensorRT rejected the CrossViT GELU scale")
    error = network.add_unary(scaled.get_output(0), trt.UnaryOperation.ERF)
    if error is None:
        raise RuntimeError("TensorRT rejected the CrossViT GELU erf")
    ...
    if shifted is None or half is None:
        raise RuntimeError("TensorRT rejected the CrossViT GELU combination")
    output = network.add_elementwise(
        half.get_output(0), shifted.get_output(0), trt.ElementWiseOperation.PROD
    )
    if output is None:
        raise RuntimeError("TensorRT rejected the CrossViT GELU product")
...
def attention(network, query, key, value, mask, *, dtype: np.dtype):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected the CrossViT attention scale")
    values = layer.get_output(0)
    if values.dtype != query.dtype:
        cast = network.add_cast(values, query.dtype)
        if cast is None:
            raise RuntimeError("TensorRT rejected the CrossViT attention scale cast")
        values = cast.get_output(0)
    scaled = network.add_elementwise(query, values, trt.ElementWiseOperation.PROD)
    if scaled is None:
        raise RuntimeError("TensorRT rejected the CrossViT query scale")
    core = network.add_attention(
        scaled.get_output(0), key, value, trt.AttentionNormalizationOp.SOFTMAX, False
    )
    if core is None:
        raise RuntimeError("TensorRT rejected the CrossViT attention")
...
def slice_tokens(network, tensor, start: int, count: int, axis: int = 1):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a CrossViT slice")
    return layer.get_output(0)
...
def constant(network, values: np.ndarray, *, dtype: np.dtype, like):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a CrossViT constant")
    output = layer.get_output(0)
    if output.dtype == like.dtype:
        return output
    cast = network.add_cast(output, like.dtype)
    if cast is None:
        raise RuntimeError("TensorRT rejected a CrossViT constant cast")
    return cast.get_output(0)
...
def concatenate_tokens(network, tensors):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a CrossViT token concatenation")
    layer.axis = 1
    return layer.get_output(0)


def bicubic_resize(network, tensor, size: tuple[int, int]):
    """Bicubic resize matching torch.nn.functional.interpolate(align_corners=False).

    The cubic coefficient and the coordinate convention both have to be set:
    TensorRT and PyTorch choose different defaults, and a mismatch shifts every
    sampled pixel slightly instead of failing.
    """
    layer = network.add_resize(tensor)
    if layer is None:
        raise RuntimeError("TensorRT rejected a CrossViT resize")
    shape = [int(value) for value in tensor.shape]
    layer.shape = trt.Dims((shape[0], shape[1], size[0], size[1]))
...
def mean_tokens(network, tensor):
    """Mean over the token axis of a [1, N, C] tensor, dropping it."""
    layer = network.add_reduce(tensor, trt.ReduceOperation.AVG, 1 << 1, False)
    if layer is None:
        raise RuntimeError("TensorRT rejected the CrossViT token mean")
    return layer.get_output(0)


def scaled_sum(network, tensors, factor: float, *, dtype: np.dtype):
    """Sum tensors then multiply by one constant."""
    total = tensors[0]
    for other in tensors[1:]:
        total = add(network, total, other)
    shape = (1,) * len(tuple(total.shape))
    layer = network.add_constant(
        shape, trt.Weights(np.array([factor], dtype=dtype).reshape(shape))
    )
    if layer is None:
        raise RuntimeError("TensorRT rejected the CrossViT averaging constant")
    values = layer.get_output(0)
    if values.dtype != total.dtype:
        cast = network.add_cast(values, total.dtype)
        if cast is None:
            raise RuntimeError("TensorRT rejected the CrossViT averaging cast")
        values = cast.get_output(0)
    product = network.add_elementwise(total, values, trt.ElementWiseOperation.PROD)
    if product is None:
        raise RuntimeError("TensorRT rejected the CrossViT averaging product")
...
def add(network, left, right):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a CrossViT add")
    return layer.get_output(0)
