...
TimmCrossViTImageClassificationPipeline::TimmCrossViTImageClassificationPipeline(
    ...
    if (!model_ || !model_->ok())
        throw std::runtime_error("TimmCrossViTImageClassificationPipeline: invalid model");
}

ClassificationResult TimmCrossViTImageClassificationPipeline::classify(const float* pixels,
                                                                       int32_t height,
                                                                       int32_t width) {
    auto values = preprocess_timm_crossvit_image(pixels, height, width, preprocess_config_);
    Tensor input;
    ...
    if (logits == nullptr || logits->numel() <= 0)
        throw std::runtime_error("timm CrossViT engine returned no logits");
    if (logits->dtype != DType::kFloat32)
        throw std::runtime_error("timm CrossViT logits must be float32");
