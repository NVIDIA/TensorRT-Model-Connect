...
"""stablelm command handlers and model-owned build inputs."""
...
class BuildRequest:
    ...
    def __post_init__(self) -> None:
        if self.task not in {"text_generation"}:
            raise ValueError("unsupported stablelm task")
        if str(self.precision).lower() not in {"fp16", "bf16", "fp32"}:
            raise ValueError("unsupported stablelm precision")
        ...
        _validate_layer_indices(self.fp32_layers)


def _validate_layer_indices(layers: tuple[int, ...]) -> None:
...
def coerce_request(request: object) -> BuildRequest:
    ...
    if unknown := set(vars(request)) - set(fields) - legacy:
        raise ValueError(f"unknown stablelm build inputs: {sorted(unknown)}")
