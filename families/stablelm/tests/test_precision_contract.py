...
from families.stablelm.checkpoint_mapper import WeightDict
from families.stablelm.config import ModelConfig
...
def test_stablelm_attention_accumulation_matches_reference_contract(
    ...
    model_module = importlib.import_module("families.stablelm.model")
    ...
    plan = model_module._StableLMModel().build_engine(
        _config(),
        WeightDict(),
        max_cache_length=64,
        precision=precision,
    ...
    assert captured["fp32_attention_accumulation"] is expected_fp32_accumulation
...
def test_precision_boundaries_support_asymmetric_split_engines(
    ...
    def fake_build(config, weights, max_cache_length, **kwargs):
        del weights, max_cache_length, kwargs
        roles.append(config.raw["_decoder_engine_role"])
        return b"engine"
    ...
    model = model_module._StableLMModel()
...
def test_fp16_prefill_keeps_the_dynamic_graph_homogeneous(monkeypatch) -> None:
    ...
    model_module._StableLMModel().build_engine(
        config,
        WeightDict(),
        max_cache_length=64,
        precision="fp16",
...
def test_only_fp16_terminal_precision_boundary_stabilizes_the_lm_head(
    ...
    builder_module = importlib.import_module("families.stablelm.default_decoder")

    assert (
        builder_module._use_fp32_lm_head_accumulation(
...
def test_decode_lm_head_stabilizes_accumulation_without_changing_output_precision(
    ...
    monkeypatch.setattr(builder_module.graph_ops, "add_matmul_rhs_constant", fake_matmul)
