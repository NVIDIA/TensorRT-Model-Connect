...
from families.stablelm import cli, support
...
def test_cli_defaults_match_support_and_publish_a_bundle(monkeypatch, tmp_path):
    ...
    def select_backend(backend):
        assert backend == "trt"
        monkeypatch.setitem(
            sys.modules, "families.stablelm.model", SimpleNamespace(build=build_model)
        )

    monkeypatch.setattr(cli, "select_backend", select_backend)
    output = tmp_path / "model.bundle"
    declaration = support.describe(ModelMetadata({"model_type": "stablelm"}, {}))
    assert declaration is not None
    assert family_cli.main(["stablelm", "build", str(tmp_path), "-o", str(output)]) == 0
...
def test_failed_owner_preserves_existing_output(monkeypatch, tmp_path):
    ...
    monkeypatch.setitem(sys.modules, "families.stablelm.model", SimpleNamespace(build=fail))
...
def test_legacy_unsupported_inputs_are_still_rejected(tmp_path, changes):
    legacy = LegacyRequest(tmp_path, tmp_path / "out", "stablelm", "text_generation", "fp32")
...
def test_help_is_lazy_and_owned(monkeypatch, capsys):
    ...
    def guarded(name, *args, **kwargs):
        assert name not in {
            "families.stablelm.cli",
            "families.stablelm.model",
            "tensorrt",
            "tensorrt_rtx",
            "huggingface_hub",
        }
    ...
    with pytest.raises(SystemExit) as error:
        family_cli.main(["stablelm", "build", "--help"])
...
def test_mixed_precision_layers_remain_owned(monkeypatch, tmp_path):
    requests = []
    monkeypatch.setattr(cli, "build_bundle", lambda request, output: requests.append(request))
    assert (
        family_cli.main(
            [
                "stablelm",
    ...
    assert requests[0].fp32_layers == (0, 2)
    with pytest.raises(ValueError, match="fp32_layers"):
        cli.BuildRequest(tmp_path, fp32_layers=(-1,))
