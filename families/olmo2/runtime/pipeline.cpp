...
#include "families/olmo2/runtime/pipeline.h"

#include "families/olmo2/runtime/chat_templates.h"
#include "families/olmo2/runtime/kv_cache.h"
#include "families/olmo2/runtime/tensor_names.h"
...
Olmo2TextGenerationPipeline::Olmo2TextGenerationPipeline(std::unique_ptr<ITrtModule> decoder,
                                                         std::unique_ptr<Olmo2InferenceState> state,
                                                         Olmo2TextGenConfig config,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("Olmo2TextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("Olmo2TextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
TextResult Olmo2TextGenerationPipeline::run(const internal::TextContinuationRequest& request,
                                            internal::ConfigView supplied) {
    const auto cfg = olmo2::parse_text_config(supplied);
    ...
    if (const auto* prompt = std::get_if<std::string_view>(&request.prefix)) {
        ...
        input_ids = olmo2::copy_token_prefix(std::get<Span<const std::int32_t>>(request.prefix));
    }
...
void require_prefill_kv_pointers(ITrtModule& prefill, const Olmo2TextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(olmo2_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(olmo2_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "Olmo2TextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const Olmo2TextGenConfig& cfg,
                                      int32_t sq, Olmo2InferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("Olmo2TextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "Olmo2TextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("Olmo2TextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error("Olmo2TextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<Olmo2KvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("Olmo2TextGenerationPipeline: prefill requires Olmo2KvCache");
}

} // namespace

void Olmo2TextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error(
            "Olmo2TextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(lt.numel()) < vocab)
        throw std::runtime_error(
            "Olmo2TextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
Olmo2TextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
Olmo2TextGenerationPipeline::TimedGenResult Olmo2TextGenerationPipeline::generate_from_ids(
    ...
    const Olmo2SamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("Olmo2TextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_olmo2_sampler(params);
...
int32_t Olmo2TextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const Olmo2SampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& Olmo2TextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
