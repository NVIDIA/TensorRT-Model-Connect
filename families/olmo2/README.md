...
```sh
trtmc build allenai/OLMo-2-0425-1B -o model.bundle
...
```sh
cmake --build build --target trtmc trtmc_backend_trt trtmc_model_olmo2 test_olmo2_task_config
ctest --test-dir build --output-on-failure -R '^olmo2_task_config$'
build/test_olmo2_sdk_c model.bundle build text "Hello" max_new_tokens=20
build/test_olmo2_sdk_cpp model.bundle build text "Hello" max_new_tokens=20
```
