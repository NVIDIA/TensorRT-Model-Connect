...
"""Run the official Transformers SAM reference for qualification."""
...
def _load_reference(model_dir: Path, precision: str) -> Reference:
    ...
    processor = SamProcessor.from_pretrained(model_dir, use_fast=False)
    model = SamModel.from_pretrained(model_dir, torch_dtype=dtype).eval().to("cuda")
    return Reference(processor, model, dtype)


def _prepare(reference: Reference, image_path: str, request: Mapping[str, Any]) -> dict[str, Any]:
    import torch
    from PIL import Image

    image = Image.open(image_path).convert("RGB")
    x = int(float(request.get("point_x", 0.5)) * image.width)
    y = int(float(request.get("point_y", 0.5)) * image.height)
    label = 1 if bool(request.get("is_foreground", True)) else 0
    encoded = reference.processor(
        image,
        input_points=[[[x, y]]],
        input_labels=[[label]],
        return_tensors="pt",
...
def _infer(reference: Reference, inputs: Mapping[str, Any]) -> dict[str, Any]:
    ...
    masks = reference.processor.image_processor.post_process_masks(
    ...
    count = int(masks.numel() // (height * width))
    scores = outputs.iou_scores.detach().float().cpu().reshape(-1).tolist()
    return {
        "segmented_images": 1,
...
def _accuracy(arguments: argparse.Namespace) -> int:
    ...
    for sample in samples:
        inputs = _prepare(reference, str(sample["image_path"]), controls)
...
def _performance(arguments: argparse.Namespace) -> int:
    ...
    if arguments.family != "sam" or arguments.operation != "segment_prompted":
        raise ValueError("SAM reference only supports sam segment_prompted")
    if arguments.mode != "pytorch-eager":
        raise ValueError("SAM reference only supports pytorch-eager")
    ...
    inputs = _prepare(reference, str(request["image_path"]), request)
