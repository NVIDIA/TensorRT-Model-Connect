...
from families.sam.tests import test_e2e as e2e
from families.sam.tests.reporting import record_mask_views
...
def test_mask_views_survive_real_comparator_failure(monkeypatch, tmp_path: Path):
    ...
    actual = {"masks": [-1.0, 1.0, -2.0, 3.0], "num_masks": 1, "height": 2, "width": 2}
    expected = {"masks": np.array([[[[-1.0, -1.0], [-2.0, 3.0]]]])}
    ...
    monkeypatch.setattr(
        e2e, "_thresholds", lambda case: {"iou_per_prompt": 1.0, "num_masks_consistency": True}
    )
    recorder = e2e_evidence.Evidence(
        tmp_path / "evidence", family="sam", case=name, source_revision="a" * 40, roots=(tmp_path,)
    )
    ...
    native_view = views["Native mask 0"]
    with Image.open(recorder.directory / native_view["image"]["artifact"]) as image:
        overlay = np.asarray(image)
    np.testing.assert_array_equal(overlay[0, 0], [20, 30, 40])
    assert not np.array_equal(overlay[0, 1], [20, 30, 40])
