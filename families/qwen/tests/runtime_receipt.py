...
def assert_native_kv_receipt(payload: dict, case: dict, prompt_tokens: int) -> None:
    ...
    expected_prompt = int(case.get("expected_prompt_tokens", prompt_tokens))
    ...
    assert expected_launches == (prompt_tokens + expected_limit - 1) // expected_limit
    assert observed_tokens == prompt_tokens
