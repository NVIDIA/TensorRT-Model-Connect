...
from . import embedding_graph_ops as graph_ops
from .embedding_weights import _target_np_dtype

if TYPE_CHECKING:
    from .embedding_weights import WeightDict
...
def _infer_kv_attention_size(weights: "WeightDict", *, num_kv_heads: int, head_dim: int) -> int:
    ...
    if isinstance(first_k, np.ndarray) and first_k.ndim == 2:
        ...
        if actual != expected:
            raise ValueError(f"layer.0.w_k must use compact K/V width {expected}, got {actual}")
    return expected
...
def build_qwen3_embedding_engine(
    ...
    *,
    ...
    if str(config.model_type).lower() != "qwen3" or tuple(config.architectures) != (
        ...
        raise ValueError("Qwen3-Embedding requires the dense Qwen3ForCausalLM backbone")
