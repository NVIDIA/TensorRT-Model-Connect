...
QwenEmbeddingPipeline::QwenEmbeddingPipeline(std::unique_ptr<ITrtModule> encoder,
...
EmbeddingResult QwenEmbeddingPipeline::embed(const std::string& text) {
    ...
    const auto maximum = encoder_->input_profile_shape("token_id", encoder_->profile_idx(),
                                                       ProfileShapeSelector::kMax);
    if (maximum.size() != 1 || maximum[0] < 1 ||
        input_ids.size() > static_cast<std::size_t>(maximum[0]))
        throw std::invalid_argument("Qwen embedding input exceeds engine sequence capacity");
...
QwenEmbeddingPipeline::run(const internal::TextToEmbeddingRequest& request,
                           internal::ConfigView config) {
    internal::validate_config({}, config);
    std::string text(request.text);
    if (request.role == internal::EmbeddingRole::Query)
        text = "Instruct: Given a web search query, retrieve relevant passages that answer the "
               "query\nQuery:" +
               text;
    else if (request.role != internal::EmbeddingRole::Default &&
             request.role != internal::EmbeddingRole::Document)
        throw std::invalid_argument("Qwen embedding input role is invalid");
    auto result = embed(text);
    return {std::move(result.data), model_id_, "last_token", "l2"};
}
