...
T require_value(const nlohmann::json& json, const char* name) {
    if (!json.contains(name))
        throw std::runtime_error(std::string("qwen runtime.json missing '") + name + "'");
    try {
        return json.at(name).get<T>();
    } catch (const nlohmann::json::exception&) {
        throw std::runtime_error(std::string("qwen runtime.json has invalid '") + name + "'");
    }
}

std::int32_t require_eos_token(const nlohmann::json& json) {
    if (!json.contains("eos_token_id"))
        throw std::runtime_error("qwen runtime.json missing 'eos_token_id'");
    ...
    throw std::runtime_error("qwen runtime.json has invalid 'eos_token_id'");
}
...
RuntimeConfig parse_runtime_config(const BundleReader& bundle) {
    ...
    try {
        ...
        throw std::runtime_error("qwen invalid runtime.json: " + std::string(error.what()));
    }
    if (!json.is_object())
    ...
    if (config.hidden_size <= 0 || config.num_layers <= 0 || config.num_heads <= 0 ||
        ...
        throw std::runtime_error("qwen runtime.json contains invalid dimensions");
    }
    if (config.num_key_value_heads % config.tensor_parallel_size != 0)
        throw std::runtime_error("qwen KV heads must be divisible by tensor parallel size");
    if (config.precision != "fp16" && config.precision != "bf16" && config.precision != "fp32") {
        throw std::runtime_error("qwen runtime.json contains invalid precision");
    }
    if (config.decoder_engine_layout != "split" && config.decoder_engine_layout != "dual_profile") {
        throw std::runtime_error("qwen runtime.json contains invalid decoder_engine_layout");
    }
    ...
    if (config.tensor_parallel_mode != expected_mode)
        throw std::runtime_error("qwen runtime.json has inconsistent tensor_parallel_mode");
    return config;
}
...
std::string chat_template(const BundleReader& bundle) {
    ...
    try {
        ...
        if (template_value == config.end() || !template_value->is_string() ||
            ...
            throw std::runtime_error("qwen tokenizer_config.json has no chat_template");
        }
        return template_value->get<std::string>();
    } catch (const nlohmann::json::exception& error) {
        throw std::runtime_error("qwen invalid tokenizer_config.json: " +
...
std::int32_t prefill_token_limit(const ITrtModule& module) {
    ...
    if (shape.size() != 1 || shape.front() <= 0) {
        throw std::runtime_error("qwen prefill engine has no valid token profile");
    }
...
DecoderModules load_modules(const FamilyContext& context, const RuntimeConfig& config) {
    ...
    if (dual.decode == nullptr || !dual.decode->ok() || dual.prefill == nullptr ||
        !dual.prefill->ok()) {
        throw std::runtime_error("qwen dual-profile engine did not create both contexts");
    }
...
TRTMC_DEFINE_FAMILY_PLUGIN_V1("qwen")
