...
def test_declared_build_uses_narrow_owner_request(monkeypatch, tmp_path):
    ...
    assert family_cli.main(["detr", "build", "checkpoint", "-o", str(output)]) == 0
    legacy = LegacyRequest(tmp_path, output, "detr", "object_detection", "fp32")
...
def test_legacy_rejects_unsupported_nondefaults(tmp_path, changes):
    legacy = LegacyRequest(tmp_path, tmp_path / "out", "detr", "object_detection", "fp32")
...
def test_unknown_legacy_fields_are_not_dropped(tmp_path):
    legacy = LegacyRequest(tmp_path, tmp_path / "out", "detr", "object_detection", "fp32")
    with pytest.raises(ValueError, match="unknown detr build inputs"):
...
def test_help_and_rejected_options_do_not_import_handlers(monkeypatch, capsys):
    ...
    for command in ("build", "detect"):
        with pytest.raises(SystemExit) as result:
            family_cli.main(["detr", command, "--help"])
        assert result.value.code == 0
    assert "--runtime-root" in capsys.readouterr().out
    with pytest.raises(SystemExit) as result:
        family_cli.main(["detr", "build", "checkpoint", "-o", "out.bundle", "--tensor-parallel-size", "2"])
...
def test_bundle_selects_backend_before_import_and_publishes_atomically(monkeypatch, tmp_path):
    ...
    def build(request, writer):
        events.append("build")
        writer.set_header(family="detr", task="object_detection", backend=request.backend)
        writer.add_bytes("engine.plan", b"fixture")
    def select(backend):
        events.append(backend)
        monkeypatch.setitem(sys.modules, "families.detr.model", SimpleNamespace(build=build))
    ...
    monkeypatch.setattr(cli, "select_backend", lambda backend: monkeypatch.setitem(
        sys.modules, "families.detr.model", SimpleNamespace(build=fail)))
...
def test_real_manifests_serialize_through_the_owner_handler(monkeypatch, tmp_path):
    ...
    spec = family_cli.load_family_cli("detr")["commands"][0]
    ...
    for path in manifests:
        ...
        assert family_cli.main(["detr", "build", *argv]) == 0
...
def test_image_dimensions_are_owner_build_inputs(monkeypatch, tmp_path):
    captured = []
    monkeypatch.setattr(cli, "resolve_model", lambda model, revision: tmp_path)
    monkeypatch.setattr(cli, "build_bundle", lambda request, output: captured.append(request))
    assert family_cli.main(["detr", "build", "checkpoint", "-o", str(tmp_path / "out"),
                            "--image-height", "384", "--image-width", "640"]) == 0
    assert (captured[0].image_height, captured[0].image_width) == (384, 640)
    legacy = LegacyRequest(tmp_path, tmp_path / "out", "detr", "object_detection", "fp32", image_height=384, image_width=640)
    assert cli.coerce_request(legacy) == captured[0]
    with pytest.raises(NotImplementedError):
        cli.coerce_request(replace(legacy, max_sequence_length=2))
    with pytest.raises(ValueError, match="positive integer"):
        cli.BuildRequest(tmp_path, image_height=0)


def test_e2e_build_preserves_single_device_rejection(monkeypatch, tmp_path):
    from families.detr.tests import test_e2e
