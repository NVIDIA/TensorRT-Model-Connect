...
// The threshold DetrImageProcessor.post_process_object_detection defaults to.
...
DetrPreprocessConfig parse_config(const std::vector<char>& data) {
    ...
    if (config.input_image_h <= 0 || config.input_image_w <= 0 || config.image_mean.size() != 3 ||
        ...
        throw std::runtime_error("DETR runtime.json does not match its contract");
    }
    for (const float value : config.image_std) {
        if (value == 0.0F)
            throw std::runtime_error("DETR runtime.json has a zero normalisation std");
    }
    return config;
}
