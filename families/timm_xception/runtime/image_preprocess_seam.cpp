...
#include "families/timm_xception/runtime/image_preprocess_seam.h"
...
void validate_config(const TimmXceptionPreprocessConfig& config) {
...
std::vector<float> preprocess_timm_xception_image(const float* image_pixels, int32_t image_height,
    ...
    validate_config(config);
    const auto shape = compute_timm_xception_resize_shape(image_height, image_width, config);
