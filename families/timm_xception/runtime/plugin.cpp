...
TimmXceptionPreprocessConfig parse_config(const nlohmann::json& json) {
    ...
    if (config.input_image_h <= 0 || config.input_image_w <= 0 || config.crop_pct <= 0.0F ||
        ...
        throw std::runtime_error("timm Xception runtime.json does not match its runtime contract");
    }
    return config;
}
...
extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    ...
    auto config = trtmc::timm_xception::parse_config(metadata);
    auto engine = trtmc::timm_xception::load_engine(context.backend, plan);
    return new trtmc::TimmXceptionImageClassificationPipeline(
