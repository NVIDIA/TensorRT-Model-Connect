...
#include "families/timm_xception/runtime/pipeline.h"
...
const Tensor& require_logits(const TensorMap& outputs) {
    for (const auto& [name, tensor] : outputs) {
        ...
        if (tensor.data == nullptr || tensor.dtype != DType::kFloat32 || tensor.numel() <= 0)
            throw std::runtime_error("timm Xception engine must return nonempty float32 logits");
        return tensor;
    }
    throw std::runtime_error("timm Xception engine did not return logits");
}

} // namespace

TimmXceptionImageClassificationPipeline::TimmXceptionImageClassificationPipeline(
    std::unique_ptr<ITrtModule> model, TimmXceptionPreprocessConfig preprocess_config,
    ...
    if (!model_ || !model_->ok())
        throw std::runtime_error("TimmXceptionImageClassificationPipeline: invalid model");
    if (num_classes_ <= 0 ||
        (!labels_.empty() && labels_.size() != static_cast<std::size_t>(num_classes_)))
        throw std::runtime_error("timm Xception class metadata does not match its output size");
}

internal::LabelScoresResult
TimmXceptionImageClassificationPipeline::run(const internal::ImageToClassScoresRequest& request,
                                             internal::ConfigView config) {
    if (!config.empty())
        throw internal::ConfigError("timm Xception has no runtime configuration");
    ...
    if (image.format != internal::ImageFormat::Float32 || image.channels != 3 ||
        ...
        throw std::invalid_argument("timm Xception requires contiguous float32 RGB input");
    auto pixel_values = preprocess_timm_xception_image(
    ...
    if (n != static_cast<std::size_t>(num_classes_))
        throw std::runtime_error("timm Xception logits do not match its configured class count");
