...
def _reference_pipeline(model_dir: Path):
    ...
    return ChronosBoltPipeline.from_pretrained(model_dir, device_map="cpu", dtype=torch.float32)
...
def _official_reference(model_dir: Path, manifest: dict, case: dict, tmp_path: Path):
    ...
    context = torch.tensor(inputs["past_values"], dtype=torch.float32)
    with torch.no_grad():
    ...
    return {"values": value.float().cpu().numpy()}
...
def test_forecast_contract_requires_multiple_quantiles() -> None:
    expected = {"values": np.asarray([[[0.1, 0.2]]], dtype=np.float32)}
    with pytest.raises(AssertionError):
        _assert_parity(
            {"values": np.asarray([0.1, 0.2], dtype=np.float32), "shape": [1, 1, 2]},
            expected,
...
def test_official_checkpoint_e2e(case_name: str, tmp_path: Path) -> None:
    ...
    if case_name == ETTH1_CASE:
        from families.chronos_bolt.tests.etth1 import GATES, windows
