...
TimmEfficientnetPreprocessConfig parse_config(const std::vector<char>& data) {
    const auto json = nlohmann::json::parse(data.begin(), data.end());
    TimmEfficientnetPreprocessConfig config;
    ...
    if (config.input_image_h <= 0 || config.input_image_w <= 0 || config.crop_pct <= 0.0F ||
        ...
        throw std::runtime_error(
            "timm EfficientNet runtime.json does not match its runtime contract");
    }
    return config;
}
...
std::unique_ptr<ITrtModule> load_engine(IBackend& backend, const std::vector<char>& plan) {
    ...
    if (!engine || !engine->ok())
        throw std::runtime_error("timm EfficientNet engine failed to load");
    return engine;
}
...
TRTMC_DEFINE_FAMILY_PLUGIN_V1("timm_efficientnet")
...
extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    ...
    auto config = trtmc::timm_efficientnet::parse_config(config_data);
    auto engine = trtmc::timm_efficientnet::load_engine(context.backend, plan);
    return new trtmc::TimmEfficientnetImageClassificationPipeline(std::move(engine),
                                                                  std::move(config));
}
