...
#include "families/timm_efficientnet/runtime/pipeline.h"
...
const Tensor& require_logits(const TensorMap& outputs) {
    ...
    if (found == outputs.end())
        throw std::runtime_error("timm EfficientNet engine did not return logits");
    if (found->second.data == nullptr || found->second.dtype != DType::kFloat32 ||
        found->second.numel() <= 0)
        throw std::runtime_error("timm EfficientNet engine must return nonempty float32 logits");
    return found->second;
}

} // namespace

TimmEfficientnetImageClassificationPipeline::TimmEfficientnetImageClassificationPipeline(
    ...
    if (!model_ || !model_->ok())
        throw std::runtime_error("TimmEfficientnetImageClassificationPipeline: invalid model");
    if (num_classes_ <= 0 ||
        (!labels_.empty() && labels_.size() != static_cast<std::size_t>(num_classes_)))
        throw std::runtime_error("timm EfficientNet class metadata does not match its output size");
}

internal::LabelScoresResult
TimmEfficientnetImageClassificationPipeline::run(const internal::ImageToClassScoresRequest& request,
                                                 internal::ConfigView config) {
    if (!config.empty())
        throw internal::ConfigError("timm EfficientNet has no runtime configuration");
    ...
    if (image.format != internal::ImageFormat::Float32 || image.channels != 3 ||
        ...
        throw std::invalid_argument("timm EfficientNet requires contiguous float32 RGB input");
    auto pixel_values = preprocess_timm_efficientnet_image(
    ...
    if (n != static_cast<std::size_t>(num_classes_))
        throw std::runtime_error(
            "timm EfficientNet logits do not match its configured class count");
