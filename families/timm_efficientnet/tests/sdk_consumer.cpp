    ...
    try {
        ...
        auto result = [&] {
            ...
            if (!task.config_fields().empty())
                throw std::runtime_error("EfficientNet must expose no runtime Config fields");
        ...
        if (scores.empty() || result.kind() != TRTMC_SCORE_LOGIT)
            throw std::runtime_error("EfficientNet must provide complete, unnormalized logits");
        std::size_t top = 0;
