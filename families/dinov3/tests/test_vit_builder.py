...
"""DINOv3 ViT family-owned checkpoint and metadata contracts."""
...
def test_vit_loader_accepts_hf_namespaces_and_mlp_variants(
    ...
    weights = dinov3_model.load_vit_weights(str(tmp_path), config, precision="fp16")
...
def test_vit_loader_maps_timm_dinov3_qkvb_checkpoint(tmp_path: Path) -> None:
    ...
    weights = dinov3_model.load_vit_weights(str(tmp_path), config, precision="fp32")
...
def test_vit_config_and_bundle_metadata_preserve_hf_contract(tmp_path: Path) -> None:
    ...
    resolved = dinov3_model.resolve_vit_config(raw)
    config = ModelConfig.from_dir(tmp_path)
    metadata = dinov3_model._Dinov3Model().get_bundle_config_overrides(config)
    ...
    assert dinov3_model._Dinov3Model().default_max_cache_length(config) == 1
