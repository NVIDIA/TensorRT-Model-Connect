...
class FakeModel final : public trtmc::IImageFeatureExtractor {
  public:
    trtmc::ImageFeaturesResult extract_image_features(const float* pixels, std::int32_t height,
        ...
        return {{pixels, pixels + 6},
                {1, 2, 3},
                {bad_output ? std::numeric_limits<float>::infinity() : pixels[0], pixels[5]},
                {1, 2}};
    }
};
...
void contract(const fs::path& root) {
    ...
    const auto first = invoke("extract_features", values);
    check(first.status == 0 && first.error.empty(), "valid owner request succeeds");
    const auto output = Json::parse(first.output);
    check(output.at("last_hidden_state_shape") == Json({1, 2, 3}), "feature shape is preserved");
    check(output.at("pooler_output_shape") == Json({1, 2}), "pooler shape is preserved");
    check(output.at("pooler_output").at(1).get<float>() == 64.0F / 255.0F,
          "feature output preserves normalized RGB");
    ...
    values.update({{"runtime_root", "/override/runtime"},
                   ...
                   {"cuda_graphs", true}});
    check(invoke("extract_features", values).status == 0, "explicit runtime options succeed");
    check(loaded_runtime == "/override/runtime" && loaded_cache == "runtime.cache" && loaded_graphs,
          "runtime options reach the loader");
    bad_output = true;
    const auto invalid_output = invoke("extract_features", values);
    ...
    check(invoke("extract_features", values).status != 0 && task_loads == before,
...
int main() {
    const auto root = fs::temp_directory_path() / ("trtmc-dinov3-cli-" + std::to_string(getpid()));
