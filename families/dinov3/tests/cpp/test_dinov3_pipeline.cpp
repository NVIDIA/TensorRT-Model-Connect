...
class PlainTask final : public trtmc::ITask {
  public:
    const char* task() const noexcept override { return "plain"; }
};
...
void test_pipeline_returns_both_named_outputs_with_shapes() {
    ...
    check(std::string(pipeline.task()) == trtmc::IImageFeatureExtractor::kTask,
          "DINOv3 image-feature task");
}
