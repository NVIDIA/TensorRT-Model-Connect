...
Dinov3PreprocessConfig parse_config(const std::vector<char>& data) {
...
std::unique_ptr<ITrtModule> load_engine(IBackend& backend, const std::vector<char>& plan) {
    ...
    if (!engine || !engine->ok())
        throw std::runtime_error("DINOv3 engine failed to load");
    return engine;
}
...
TRTMC_DEFINE_FAMILY_PLUGIN_V1("dinov3")

extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    if (context.kv_cache_size_bytes != 0)
        throw std::invalid_argument("dinov3 does not support --kv-cache-size");
    const auto& config_data = trtmc::dinov3::require_section(context.reader, "runtime.json");
    const auto& plan = trtmc::dinov3::require_section(context.reader, "engine.plan");
    auto config = trtmc::dinov3::parse_config(config_data);
    auto engine = trtmc::dinov3::load_engine(context.backend, plan);
    return new trtmc::Dinov3ImageFeaturePipeline(std::move(engine), std::move(config));
}
