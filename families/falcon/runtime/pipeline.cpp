...
#include "families/falcon/runtime/pipeline.h"

#include "families/falcon/runtime/chat_templates.h"
#include "families/falcon/runtime/kv_cache.h"
#include "families/falcon/runtime/tensor_names.h"
...
FalconTextGenerationPipeline::FalconTextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<FalconInferenceState> state,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("FalconTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("FalconTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
TextResult FalconTextGenerationPipeline::run(const internal::TextContinuationRequest& request,
                                             internal::ConfigView supplied) {
    const auto cfg = falcon::parse_text_config(supplied);
    ...
    if (const auto* prompt = std::get_if<std::string_view>(&request.prefix)) {
        ...
        input_ids = falcon::copy_token_prefix(std::get<Span<const std::int32_t>>(request.prefix));
    }
...
void require_prefill_kv_pointers(ITrtModule& prefill, const FalconTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(falcon_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(falcon_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "FalconTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const FalconTextGenConfig& cfg,
                                      int32_t sq, FalconInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("FalconTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "FalconTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("FalconTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "FalconTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<FalconKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("FalconTextGenerationPipeline: prefill requires FalconKvCache");
}

} // namespace

void FalconTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error(
            "FalconTextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(lt.numel()) < vocab)
        throw std::runtime_error(
            "FalconTextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
FalconTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
FalconTextGenerationPipeline::TimedGenResult FalconTextGenerationPipeline::generate_from_ids(
    ...
    const FalconSamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("FalconTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_falcon_sampler(params);
...
int32_t FalconTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const FalconSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& FalconTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
