...
```sh
trtmc build tiiuae/falcon-rw-1b -o model.bundle
...
```sh
cmake --build build --target trtmc trtmc_backend_trt trtmc_model_falcon test_falcon_task_config
ctest --test-dir build --output-on-failure -R '^falcon_task_config$'
build/test_falcon_sdk_c model.bundle build text "Hello" max_new_tokens=20
build/test_falcon_sdk_cpp model.bundle build text "Hello" max_new_tokens=20
```
