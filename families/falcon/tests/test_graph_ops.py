...
trt = pytest.importorskip("tensorrt", reason="TensorRT is required for Falcon graph tests")

from families.falcon import graph_ops  # noqa: E402
...
def test_falcon_alibi_matches_reference_bfloat16_rounding_path():
    ...
    assert all(
        operation != trt.ElementWiseOperation.SUB for operation, *_operands in network.elementwise
    )
