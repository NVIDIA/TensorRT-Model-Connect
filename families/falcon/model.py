...
def build(request: "BuildRequest", writer: "BundleWriter") -> None:
    """Build one Falcon bundle through family-owned code only."""
    if request.dynamic_kv_cache:
        raise NotImplementedError("falcon does not support dynamic_kv_cache")

    if request.image_height is not None:
        raise NotImplementedError("falcon does not support image_height")

    if request.image_width is not None:
        raise NotImplementedError("falcon does not support image_width")

    if request.video_num_frames is not None:
        raise NotImplementedError("falcon does not support video_num_frames")

    if request.max_batch_size != 1:
        raise NotImplementedError("falcon does not support max_batch_size")

    if request.context_parallel_size != 1:
        raise ValueError("this family does not support context parallelism")

    if request.task != "text_continuation":
        raise ValueError("falcon supports only task=text_continuation")

    model_dir = Path(request.model_dir)
    config = ModelConfig.from_dir(model_dir)
    if not (
        str(config.model_type).lower().startswith("falcon")
        or str(config.model_type).lower() in {"refinedweb", "refinedwebmodel"}
    ):
        raise ValueError(f"Falcon does not support model_type={config.model_type!r}")
    precision = str(request.precision).lower()
    if precision not in {"fp32", "fp16", "bf16"}:
        raise ValueError("Falcon precision must be fp32, fp16, or bf16")
    max_sequence_length = _positive_int(
        request.max_sequence_length or min(config.max_position_embeddings, 256),
        "max_sequence_length",
    )
    if max_sequence_length > config.max_position_embeddings:
        raise ValueError("Falcon max_sequence_length exceeds checkpoint context capacity")
    if request.quantization not in {None, "none"}:
        raise NotImplementedError("Falcon has no qualified family-owned quantized build")
    if request.fp32_layers:
        raise NotImplementedError("Falcon does not expose mixed-precision layer selection")
    ...
    writer.set_header(family="falcon", task=request.task, backend=request.backend)
    if parallel.enabled:
