...
#include "families/timm_senet/runtime/image_preprocess_seam.h"
...
void validate_config(const TimmSENetPreprocessConfig& config) {
...
std::vector<float> preprocess_timm_senet_image(const float* image_pixels, int32_t image_height,
    ...
    validate_config(config);
    const auto shape = compute_timm_senet_resize_shape(image_height, image_width, config);
