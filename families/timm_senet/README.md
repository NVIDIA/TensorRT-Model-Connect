...
```sh
trtmc build timm/senet154.gluon_in1k -o model.bundle
...
```sh
cmake --build build --target test_timm_senet_task_contract test_timm_senet_image_preprocess
ctest --test-dir build --output-on-failure -R '^timm_senet_(task_contract|image_preprocess)$'
```
...
```sh
build/test_timm_senet_sdk_c model.bundle build image.rgb.f32 480 640
build/test_timm_senet_sdk_cpp model.bundle build image.rgb.f32 480 640
```
...
`tests/performance.yaml` takes over the existing `timm_senet.classify`
