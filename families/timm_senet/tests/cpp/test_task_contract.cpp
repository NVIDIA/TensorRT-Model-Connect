...
class RecordingModule final : public trtmc::ITrtModule {
  public:
    trtmc::TensorMap forward(const trtmc::TensorMap& inputs) override {
        ...
        return {{output_name, {logits.data(), {1, static_cast<int64_t>(logits.size())}, dtype}}};
    }
...
trtmc::TimmSENetPreprocessConfig preprocessing() {
...
void test_binding_and_complete_owned_logits() {
    trtmc::internal::LabelScoresResult result;
    {
        ...
        trtmc::TimmSENetImageClassificationPipeline model(std::move(module), preprocessing(), 5, "",
                                                          {});
...
void test_class_identity_metadata() {
    ...
    for (const auto& labels : {partial, std::vector<std::string>(5)}) {
        rejects<std::runtime_error>(
            [&] {
                trtmc::TimmSENetImageClassificationPipeline model(
    ...
    for (const auto& [vocabulary, labels] : valid) {
        trtmc::TimmSENetImageClassificationPipeline model(std::make_unique<RecordingModule>(),
...
void test_existing_named_output() {
    auto module = std::make_unique<RecordingModule>();
    module->output_name = "class_scores";
    const auto expected = module->logits;
    trtmc::TimmSENetImageClassificationPipeline model(std::move(module), preprocessing(), 5, "",
                                                      {});
    const std::vector<float> pixels(12, 0.75F);
    require(model.run(request(pixels), {}).scores == expected,
            "a single named classifier output retains the existing runtime contract");
}
