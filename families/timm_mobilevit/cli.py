...
"""Family-owned timm_mobilevit commands; importing this module does not load TensorRT."""
...
class BuildRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if str(self.precision).lower() not in {"fp16", "fp32"}:
            raise ValueError("timm_mobilevit supports only fp16 and fp32 precision")
...
def coerce_request(request: object) -> BuildRequest:
    ...
    if unknown := set(vars(request)) - allowed:
        raise ValueError(f"unknown timm_mobilevit build inputs: {sorted(unknown)}")
...
def build(*, model: str, output: Path, revision: str | None = None,
          task: str = "classification", precision: str = "fp32", backend: str = "trt", verbose: bool = False) -> int:
