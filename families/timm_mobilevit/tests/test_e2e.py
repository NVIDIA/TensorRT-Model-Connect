...
"""Direct build, native runtime, and timm reference proof for MobileViT."""
...
from families.timm_mobilevit.cli import BuildRequest, build_bundle
from tools.e2e_evidence import evidence_stage, record_evidence


FAMILY = "timm_mobilevit"
...
def pytest_generate_tests(metafunc) -> None:
    ...
    if not selected:
        names = [
            pytest.param(
                name,
                marks=pytest.mark.skip(reason="real MobileViT E2E requires explicit selection"),
                id=name,
            )
            for name in names
        ]
...
def _required_path(value: str | None, label: str) -> Path:
    assert value, f"selected MobileViT E2E requires {label}"
    path = Path(value)
    assert path.exists(), f"selected MobileViT E2E {label} does not exist: {path}"
    return path


def _model_dir(manifest: dict) -> Path:
    explicit = os.environ.get("TRTMC_TIMM_TIMM_MOBILEVIT_MODEL_DIR")
    if explicit:
        return _required_path(explicit, "TRTMC_TIMM_TIMM_MOBILEVIT_MODEL_DIR")
...
def _asset(case: dict) -> Path:
    ...
    assert path.is_file(), f"selected MobileViT E2E image is missing: {path}"
