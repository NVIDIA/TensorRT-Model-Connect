...
def _selected_masks(output):
    ...
    return masks[:2].astype(bool), int(masks.shape[0])
            ...
            record_evidence(
                "mask_view_summary",
                {
                    role.lower(): {
                        ...
                        "boundary": "post-processed nonzero masks are foreground",
                    }
                },
            )
            if count == 0:
                ...
                views.append(
                    {
                        ...
                        "Display resized with nearest-neighbor masks; post-processed nonzero masks are foreground.",
                    }
                )
