...
static constexpr std::string_view kSam3ClipConfig = R"({
      "tokenizer_add_special_tokens": true,
      "tokenizer_prefix_ids": [10],
      "tokenizer_suffix_ids": [11]
    })";

static constexpr std::string_view kSam3ClipTokenizer = R"({
      "model": {
        "type": "BPE",
...
std::filesystem::path write_sam3_clip_tokenizer_bundle(std::string_view config,
                                                       std::string_view tokenizer_json) {
    char path[] = "/tmp/trtmc_sam3_pipeline_XXXXXX";
    ...
    const std::string header =
        ...
        std::to_string(config.size()) + R"(},"tokenizer.json":{"offset":)" +
        std::to_string(config.size()) + R"(,"length":)" + std::to_string(tokenizer_json.size()) +
        "}}}";
...
std::shared_ptr<trtmc::ITokenizer>
load_sam3_clip_tokenizer(std::string_view config = kSam3ClipConfig,
                         std::string_view tokenizer_json = kSam3ClipTokenizer) {
    const auto path = write_sam3_clip_tokenizer_bundle(config, tokenizer_json);
    try {
...
void test_sam3_clip_tokenizer_matches_meta_segmentation() {
    auto tokenizer = load_sam3_clip_tokenizer();
...
void test_sam3_clip_explicit_frame_overrides_generic_header() {
    ...
    auto tokenizer = load_sam3_clip_tokenizer(
        R"({"tokenizer_add_special_tokens":false,"tokenizer_prefix_ids":[10],"tokenizer_suffix_ids":[11]})");
...
void test_clip_non_removed_split_keeps_standalone_space_token() {
    std::string tokenizer_json(kSam3ClipTokenizer);
    ...
    tokenizer_json.replace(behavior, std::strlen("\"behavior\": \"Removed\""),
                           "\"behavior\": \"Isolated\"");
    auto tokenizer = load_sam3_clip_tokenizer(kSam3ClipConfig, tokenizer_json);
...
class FakeModule : public trtmc::ITrtModule {
  public:
    ...
    void bind_external(const std::string& name, void* ptr, const std::vector<int64_t>&) override {
...
class FakeSam3TextModule final : public FakeModule {
  public:
    bool ok() const override { return true; }
...
class FakeDeviceSam3VisionModule final : public FakeModule {
  public:
    ...
    trtmc::TensorMap forward(const trtmc::TensorMap& inputs) override {
        ...
        if (image == inputs.end() || image->second.data == nullptr)
            throw std::runtime_error("missing fake device SAM3 image input");
        const auto* pixels = static_cast<const float*>(image->second.data);
        saw_shape = image->second.shape == std::vector<int64_t>({1, 3, 4, 4});
        ...
        for (const auto& [name, pointer] : outputs_) {
            (void)pointer;
            auto& values = host_outputs_[name];
            values.assign(1, output_marker(name, calls));
            result[name] = trtmc::Tensor{values.data(), tensor_shape(name), trtmc::DType::kFloat32};
        }
        return result;
    }
  ...
  private:
    ...
    std::unordered_map<std::string, std::vector<float>> host_outputs_;
    std::shared_ptr<void> keep_alive_;
};

class FakeSam3CoreModule final : public FakeModule {
  public:
    bool ok() const override { return true; }
...
class FakeSam3TrackerModule : public FakeModule {
  public:
    explicit FakeSam3TrackerModule(bool recurrent, bool memory_only = false,
        ...
        try {
            if (recurrent_) {
                constexpr std::size_t max_memory_records = 10;
                allocate_device_tensor("memory_features",
                                       device_batch_size_ * max_memory_records * 4 * 64);
                allocate_device_tensor("memory_position",
                                       device_batch_size_ * max_memory_records * 4 * 64);
            }
            if (!memory_only_) {
...
make_parallel_tracker_init_fixture(const std::shared_ptr<ParallelTrackerInitSchedule>& schedule,
    ...
    auto init = std::make_unique<FakeSam3TrackerModule>(false, false, 2.0F, true, true);
    auto* init_ptr = init.get();
    auto sibling = std::make_unique<FakeSam3TrackerModule>(false, false, 2.0F, true, true);
    auto* sibling_ptr = sibling.get();
    if (schedule != nullptr) {
        init->on_forward = [schedule] { schedule->rendezvous(); };
        sibling->on_forward = [schedule] { schedule->rendezvous(); };
    }
    auto step = std::make_unique<FakeSam3TrackerModule>(true, false, 2.0F, true);
    auto step_batch2 = std::make_unique<FakeSam3TrackerModule>(true, false, 2.0F, true, false, 2);
    auto memory = std::make_unique<FakeSam3TrackerModule>(false, true, 2.0F, true);
    auto memory_batch2 = std::make_unique<FakeSam3TrackerModule>(false, true, 2.0F, true, false, 2);
    auto hard_memory =
        std::make_unique<FakeSam3TrackerModule>(false, true, 2.0F, true, false, 1, true);
    auto hard_memory_batch2 =
        std::make_unique<FakeSam3TrackerModule>(false, true, 2.0F, true, false, 2, true);
    ...
    auto pipeline = std::make_unique<trtmc::Sam3Pipeline>(
        ...
        std::move(init), std::move(step), std::move(memory), std::move(step_batch2),
        std::move(memory_batch2), std::move(sibling), std::move(hard_memory),
...
void test_preprocess_matches_customer_meta_pillow_resize() {
    ...
    const auto output = trtmc::preprocess_sam3_image(pixel.data(), 1, 1, config);
    bool exact = output.size() == 3U * 4U * 4U;
    for (std::size_t index = 0; exact && index < 16U; ++index) {
        exact = output[index] == -1.0F && output[16U + index] == 0.00390625F &&
                output[32U + index] == 1.0F;
    }
    check(exact,
          "sam3 resize preserves Meta Pillow uint8 quantization, normalization, and CHW layout");
}
...
struct VideoFixture {
    std::unique_ptr<trtmc::Sam3Pipeline> pipeline;
    FakeSam3TextModule* text{nullptr};
    FakeDeviceSam3VisionModule* vision{nullptr};
...
VideoFixture make_video_fixture(std::size_t detections = 1, bool device_recurrent = false,
                                float tracker_logit = 2.0F,
    ...
    auto init = std::make_unique<FakeSam3TrackerModule>(false, false, 2.0F, true);
    auto* init_ptr = init.get();
    auto step =
        std::make_unique<FakeSam3TrackerModule>(true, false, tracker_logit, true, device_recurrent);
    auto* step_ptr = step.get();
    auto memory =
        std::make_unique<FakeSam3TrackerModule>(false, true, 2.0F, true, device_recurrent);
    auto* memory_ptr = memory.get();
    auto hard_memory =
        std::make_unique<FakeSam3TrackerModule>(false, true, 2.0F, true, device_recurrent, 1, true);
    auto* hard_memory_ptr = hard_memory.get();
    auto hard_memory_batch2 =
        std::make_unique<FakeSam3TrackerModule>(false, true, 2.0F, true, device_recurrent, 2, true);
    auto* hard_memory_batch2_ptr = hard_memory_batch2.get();
    auto hard_mask_resize = std::make_unique<FakeSam3HardMaskResizeModule>(1);
    auto hard_mask_resize_batch2 = std::make_unique<FakeSam3HardMaskResizeModule>(2);
    auto* hard_mask_resize_ptr = hard_mask_resize.get();
    auto* hard_mask_resize_batch2_ptr = hard_mask_resize_batch2.get();

    auto step_batch2 = std::make_unique<FakeSam3TrackerModule>(true, false, tracker_logit, true,
                                                               device_recurrent, 2);
    auto memory_batch2 =
        std::make_unique<FakeSam3TrackerModule>(false, true, 2.0F, true, device_recurrent, 2);
    auto* step_batch2_ptr = step_batch2.get();
    auto* memory_batch2_ptr = memory_batch2.get();
    auto parallel_init = std::make_unique<FakeSam3TrackerModule>(false, false, 2.0F, true);
    ...
    auto pipeline = std::make_unique<trtmc::Sam3Pipeline>(
        ...
        std::move(step_batch2), std::move(memory_batch2), std::move(parallel_init),
...
void test_video_bundle_requires_both_hard_memory_plans() {
    ...
    try {
        trtmc::Sam3Pipeline pipeline(
            ...
            std::make_unique<FakeSam3CoreModule>(), std::make_shared<FakeTokenizer>(),
            make_config(), "facebook/sam3", std::make_unique<FakeSam3TrackerModule>(false),
            std::make_unique<FakeSam3TrackerModule>(true),
            std::make_unique<FakeSam3TrackerModule>(false, true),
            std::make_unique<FakeSam3TrackerModule>(true, false, 2.0F, false, false, 2),
            std::make_unique<FakeSam3TrackerModule>(false, true, 2.0F, false, false, 2),
            std::make_unique<FakeSam3TrackerModule>(false),
            std::make_unique<FakeSam3TrackerModule>(false, true, 2.0F, false, false, 1, true),
            nullptr, std::make_unique<FakeSam3HardMaskResizeModule>(1),
            std::make_unique<FakeSam3HardMaskResizeModule>(2));
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void test_prompt_only_session_cleanup_keeps_cuda_device() {
          ...
          "sam3 prompt-only cleanup queries the caller CUDA device");
    {
        ...
        check(cudaGetDevice(&current_device) == cudaSuccess && current_device == original_device &&
                  cudaGetLastError() == cudaSuccess,
              "sam3 prompt-only cleanup preserves the single selected CUDA device");
    }
}
#endif
...
void test_image_pcs_runs_native_preprocess_and_full_result() {
    ...
    const auto result = fixture.pipeline->segment_prompted_text(pixels.data(), 2, 4, "ear");
    check(fixture.text->saw_expected_ids, "sam3 image PCS encodes the text prompt");
    check(fixture.vision->saw_shape, "sam3 image PCS sends the exact B1 vision shape");
    check(fixture.vision->saw_normalized_pixels,
          "sam3 image PCS applies customer Meta image preprocessing");
    check(fixture.core->saw_text_shape && fixture.core->saw_mask,
          "sam3 image PCS passes the projected prompt to the core");
    check(fixture.core->saw_vision_inputs, "sam3 image PCS passes all FPN features to the core");
...
void test_frame_zero_soft_mask_cleanup_feeds_output_and_memory() {
    auto fixture = make_video_fixture(2, false, 2.0F, [](trtmc::Sam3Config& config) {
    ...
    check(propagated.size() == 1 && propagated.front().object_ids == std::vector<int32_t>({0}) &&
          ...
          "mask-input score");
    check(fixture.memory->calls == 1 && fixture.memory_batch2->calls == 0 &&
...
void test_frame_zero_zero_detections_is_stable() {
    auto fixture = make_video_fixture(
        1, false, 2.0F, [](trtmc::Sam3Config& config) { config.detection_threshold = 1.0F; });
    std::vector<float> pixels(12, 0.5F);
    auto session = fixture.pipeline->create_sam3_video_session("person");
    auto prompt = session->accept_prompt_frame(pixels.data(), 2, 2);
...
void test_late_new_track_uses_uncleaned_hard_memory() {
    auto fixture = make_video_fixture(2, false, 2.0F,
    ...
    check(fixture.memory->calls == 2 && fixture.memory_batch2->calls == 0 &&
...
void test_recurrent_tracker_uses_b2_pairs_and_b1_odd_tail() {
    ...
    {
        auto fixture = make_video_fixture(3, false, 2.0F, {});
        fixture.core->detections_first_frame_only = true;
        const auto results = run_video(*fixture.pipeline, 2);
...
void test_hard_mask_resize_stream_preserves_stable_global_ownership() {
    auto fixture = make_video_fixture(3);
...
void test_device_recurrent_memory_rounds_features_and_positions() {
    ...
    constexpr float rounded_position = 0.66796875F;
    {
        auto fixture = make_video_fixture(1, true);
...
void test_parallel_mask_cleanup_preserves_full_results() {
    ...
    auto first = make_video_fixture(3, false, 2.0F, enable_cleanup);
    ...
    auto second = make_video_fixture(3, false, 2.0F, enable_cleanup);
...
void test_conditioning_and_pointer_history() {
    ...
    auto config_fixture = make_video_fixture(1, false, 2.0F, configure);
    ...
    auto pointers = make_video_fixture(1, false, 2.0F, configure);
...
void test_memory_quality_keeps_later_appearance_cohort_independent() {
    ...
    const std::vector<std::vector<float>> recurrent_masks = {
        first_mask, first_mask, second_mask, first_mask, second_mask, first_mask, second_mask,
    };
    fixture.step->recurrent_masks_by_item = recurrent_masks;
    fixture.step_batch2->recurrent_masks_by_item = recurrent_masks;
    ...
    const std::vector<float> selected_ious{0.0F, 0.0F, 0.04F, 1.0F, 1.0F, 1.0F, 1.0F};
    fixture.step->scripted_selected_ious = selected_ious;
    fixture.step_batch2->scripted_selected_ious = selected_ious;
    const auto results = run_video(*fixture.pipeline, 5);
    const auto first_cohort =
        std::find(fixture.step->memory_offsets_history.begin(),
                  fixture.step->memory_offsets_history.end(), std::vector<int32_t>({0, 1}));
    const auto later_cohort =
        first_cohort == fixture.step->memory_offsets_history.end()
            ? first_cohort
            : std::find(std::next(first_cohort), fixture.step->memory_offsets_history.end(),
                        std::vector<int32_t>({0, 2, 1}));
    check(results.size() == 5 && first_cohort != fixture.step->memory_offsets_history.end() &&
              later_cohort != fixture.step->memory_offsets_history.end(),
...
void test_memory_quality_recomputes_after_cohort_member_removal() {
    auto fixture = make_video_fixture(
        2, false, 2.0F, [](trtmc::Sam3Config& config) { config.hotstart_duplicate_threshold = 1; });
    ...
    fixture.step_batch2->contained_pair_masks = true;
    ...
    fixture.step_batch2->scripted_selected_ious = {0.0F, 0.04F, 1.0F, 1.0F};
    const auto results = run_video(*fixture.pipeline, 4);
...
void test_reconditioning_uses_raw_tracker_logit() {
    auto fixture = make_video_fixture(
        1, false, 0.8F, [](trtmc::Sam3Config& config) { config.high_confidence_threshold = 0.7F; });
...
void test_recent_occlusion_and_hotstart_policy() {
    ...
    occlusion.step_batch2->scripted_occlusion = true;
    ...
    check(hotstart.hard_memory->calls == 0 && hotstart.hard_memory_batch2->calls == 1 &&
              hotstart.memory->calls == 1 && hotstart.memory_batch2->calls == 2 &&
              hotstart.memory_batch2->final_masks_history.size() == 4 &&
              hotstart.memory_batch2->final_masks_history[2] ==
                  std::vector<float>({2.0F, -2.0F, -2.0F, -2.0F}) &&
              hotstart.memory_batch2->memory_suppressions_history[2] == 1 &&
              close(hotstart.memory_batch2->memory_scores_history[2], -10.0F),
...
int main() {
    ...
    test_prompt_only_session_cleanup_keeps_cuda_device();
#endif
