...
"""Wan2.2-owned fused QKV graph contracts."""
...
from families.wan2_2_ti2v import trt_ops as op
...
def test_fused_qkv_linear_packs_qkv_in_order_and_preserves_bf16_slices(
    ...
    def linear(network, actual_x, weight, bias):
        calls.append((network, actual_x, weight, bias))
        return _Tensor((rows, 3 * hidden_size))

    monkeypatch.setattr(op, "linear", linear)
    network = _Network()
