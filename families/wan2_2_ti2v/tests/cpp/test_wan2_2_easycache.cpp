...
std::string runtime_json(double threshold = 0.08, int first = 7, int last = 2, int max_reuse = 4) {
    return std::string("{\"easycache_enabled\":true,\"easycache_threshold\":") +
           std::to_string(threshold) + ",\"easycache_first_exact_steps\":" + std::to_string(first) +
           ",\"easycache_last_exact_steps\":" + std::to_string(last) +
           ",\"easycache_max_consecutive_reuse\":" + std::to_string(max_reuse) +
           ",\"late_cfg_enabled\":true}";
}

void test_defaults_are_inert() {
    const EasyCacheConfig settings;
    check(!settings.enabled, "EasyCache is disabled by default");
    check(close(settings.threshold, 0.02), "EasyCache threshold default is preserved");
    check(settings.first_exact_steps == 7 && settings.last_exact_steps == 2 &&
              settings.max_consecutive_reuse == 1,
          "EasyCache integer defaults are preserved");
    check(!trtmc::wan2_2_ti2v::validate_late_cfg_request(false, settings),
          "late-CFG is disabled by default");
}

void test_runtime_values_reach_controller_contract() {
    auto settings = trtmc::wan2_2_ti2v::parse_runtime_config(runtime_json());
    settings.easycache.total_steps = 50;
    check(settings.late_cfg_enabled, "late-CFG value reaches the runtime API");
    check(settings.easycache.enabled && close(settings.easycache.threshold, 0.08) &&
              settings.easycache.first_exact_steps == 7 &&
              settings.easycache.last_exact_steps == 2 &&
              settings.easycache.max_consecutive_reuse == 4,
          "EasyCache values reach the runtime API");
...
void test_runtime_parser_rejects_invalid_values() {
    check_throws([] { (void)trtmc::wan2_2_ti2v::parse_runtime_config(runtime_json(0.08, -1)); },
                 "runtime parser rejects a negative exact-step count");
    check_throws(
        [] { (void)trtmc::wan2_2_ti2v::parse_runtime_config(runtime_json(0.08, 7, 2, -1)); },
        "runtime parser rejects a negative reuse cap");
}
...
void record_simple_actual(LateCfgController& controller, std::int32_t step) {
...
void test_late_cfg_exact_windows_cadence_and_prediction() {
    ...
    for (std::int32_t step = 0; step < 50; ++step) {
...
void test_prediction_fails_closed_to_actual() {
    LateCfgController controller;
    for (std::int32_t step = 0; step <= 22; ++step) {
        const bool compute = step == 0 || step == 19 || step == 20 || step == 21;
        std::int64_t timestep = 1000 - step;
        if (step == 19)
            timestep = 100;
        if (step == 20)
            timestep = 99;
        ...
        if (!compute) {
            check(action == LateCfgAction::kEasyCacheReuse,
                  "late-CFG continues after prediction rejection");
            continue;
        }
        if (step == 21) {
            check(action == LateCfgAction::kPredictUnconditional,
    ...
    check(stats.actual_unconditional_calls == 4 && stats.predicted_unconditional_reuses == 0 &&
          ...
          "late-CFG accounts for an actual-unconditional fail-close");
}
...
void test_synthetic_unconditional_updates_easycache() {
    ...
    for (std::int32_t step = 0; step <= 21; ++step) {
        const bool compute = step == 0 || step == 19 || step == 20 || step == 21;
...
int main() {
    ...
    test_runtime_values_reach_controller_contract();
    test_thor_performance_profile_scope();
    test_runtime_parser_rejects_invalid_values();
    test_easycache_exact_and_reuse_paths();
    test_late_cfg_exact_windows_cadence_and_prediction();
    test_prediction_fails_closed_to_actual();
