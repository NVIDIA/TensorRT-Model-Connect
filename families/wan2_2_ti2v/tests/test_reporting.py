...
from families.wan2_2_ti2v.tests import reporting
...
def test_paired_previews_keep_matched_indices_and_pixels(tmp_path: Path):
    ...
    for index, values in enumerate(frames):
        ...
        paths.append(path)
    actual = {"frames": [str(path) for path in paths]}
    expected = {"frame_paths": paths}
    recorder = e2e_evidence.Evidence(tmp_path / "evidence", family="wan2_2_ti2v", case="preview-probe",
