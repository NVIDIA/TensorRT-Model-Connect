...
pytest.importorskip("tensorrt", reason="Mixtral routing contracts require TensorRT")

from families.mixtral import model  # noqa: E402
...
def test_fp16_mixtral_quantizes_router_logits_before_fp32_routing(monkeypatch) -> None:
    ...
    monkeypatch.setattr(model, "trt", fake_trt)
    ...
    monkeypatch.setattr(model.graph_ops, "add_matmul_rhs_constant", fake_matmul)
    monkeypatch.setattr(
        model,
        "_add_swiglu_expert",
    ...
    network = _Network()
    result = model._add_mixtral_moe_block(
        network=network,
        inp=_Tensor("hidden_states", "fp16"),
        weights={
            "layer.0.router": np.array([[0.1, -0.2], [0.3, -0.4]], dtype=np.float32),
            **{
    ...
    softmax_input = next(event[1] for event in network.events if event[0] == "softmax")
    topk_input = next(event[1] for event in network.events if event[0] == "topk")
    router_cast = next(
        event for event in network.events if event[0] == "cast" and event[1].name == "router_logits"
    )
    ...
    products = [
        event for event in network.events if event[0] == "elementwise" and event[3] == "prod"
    ]
    assert products
