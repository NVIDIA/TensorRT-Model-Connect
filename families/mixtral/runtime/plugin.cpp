...
#include "families/mixtral/runtime/chat_templates.h"
#include "families/mixtral/runtime/distributed_runtime.h"
#include "families/mixtral/runtime/kv_cache.h"
#include "families/mixtral/runtime/pipeline.h"
#include "families/mixtral/runtime/plugin_helpers.h"
#include "families/mixtral/runtime/tensor_names.h"
...
T require_value(const nlohmann::json& json, const char* name) {
    if (!json.contains(name))
        throw std::runtime_error(std::string("mixtral runtime.json missing '") + name + "'");
    try {
        return json.at(name).get<T>();
    } catch (const nlohmann::json::exception&) {
        throw std::runtime_error(std::string("mixtral runtime.json has invalid '") + name + "'");
    }
}

std::int32_t require_eos_token(const nlohmann::json& json) {
    if (!json.contains("eos_token_id"))
        throw std::runtime_error("mixtral runtime.json missing 'eos_token_id'");
    ...
    throw std::runtime_error("mixtral runtime.json has invalid 'eos_token_id'");
}
...
RuntimeConfig parse_runtime_config(const BundleReader& bundle) {
    ...
    try {
        ...
        throw std::runtime_error("mixtral invalid runtime.json: " + std::string(error.what()));
    }
    if (!json.is_object())
        throw std::runtime_error("mixtral runtime.json must be an object");
    if (json.size() != 14)
        throw std::runtime_error("mixtral runtime.json has an unexpected field set");
    ...
    if (config.hidden_size <= 0 || config.num_layers <= 0 || config.num_heads <= 0 ||
        ...
        throw std::runtime_error("mixtral runtime.json contains invalid dimensions");
    }
    if (config.num_key_value_heads % config.tensor_parallel_size != 0)
        throw std::runtime_error("mixtral KV heads must be divisible by tensor parallel size");
    if (config.precision != "fp16" && config.precision != "bf16" && config.precision != "fp32") {
        throw std::runtime_error("mixtral runtime.json contains invalid precision");
    }
    if (config.decoder_engine_layout != "single" && config.decoder_engine_layout != "split" &&
        config.decoder_engine_layout != "dual_profile") {
        throw std::runtime_error("mixtral runtime.json contains invalid decoder_engine_layout");
    }
    if (config.decoder_engine_layout != "dual_profile" && config.tensor_parallel_size != 1)
        throw std::runtime_error("mixtral non-dual-profile engines require tensor_parallel_size=1");
    const std::string expected_mode =
        config.tensor_parallel_size > 1 ? "tensor_parallel" : "single";
    if (config.tensor_parallel_mode != expected_mode)
        throw std::runtime_error("mixtral runtime.json has inconsistent tensor_parallel_mode");
    return config;
}
...
MixtralKvCacheNames make_kv_names(std::int32_t num_layers) {
    MixtralKvCacheNames names;
    for (std::int32_t layer = 0; layer < num_layers; ++layer) {
        names.cache_k.push_back(mixtral_layer_tensor_name("cache_k", layer));
        names.cache_v.push_back(mixtral_layer_tensor_name("cache_v", layer));
        names.present_k.push_back(mixtral_layer_tensor_name("present_k", layer));
        names.present_v.push_back(mixtral_layer_tensor_name("present_v", layer));
    }
    return names;
}
...
DecoderModules load_modules(const FamilyContext& context, const RuntimeConfig& config) {
    ...
    if (config.decoder_engine_layout == "single") {
        modules.decode = load_engine(
            context.backend, require_section(context.reader, "engine.plan"), "mixtral decoder");
        return modules;
    }
    if (config.decoder_engine_layout == "split") {
        ...
        modules.prefill = load_engine(
            context.backend, require_section(context.reader, "prefill.plan"), "mixtral prefill");
        return modules;
    }
    ...
    if (dual.decode == nullptr || !dual.decode->ok() || dual.prefill == nullptr ||
        !dual.prefill->ok()) {
        throw std::runtime_error("mixtral dual-profile engine did not create both contexts");
    }
...
ITask* create(const FamilyContext& context) {
    ...
    auto state = std::make_unique<MixtralKvCache>(config.num_layers, config.max_cache_length,
                                                  kv_dim, stream, cache_dtype(config.precision),
                                                  make_kv_names(config.num_layers));
    if (!state->ok())
        throw std::runtime_error("mixtral failed to create KV cache");

    MixtralTextGenConfig text_config;
    ...
    text_config.chat_template_format =
        mixtral_detect_chat_template_format(chat_template(context.reader));
    text_config.prefill_max_length = config.max_cache_length;
    text_config.num_layers = config.num_layers;

    return new MixtralTextGenerationPipeline(
...
TRTMC_DEFINE_FAMILY_PLUGIN_V1("mixtral")

extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    if (context.kv_cache_size_bytes != 0)
        throw std::invalid_argument("mixtral does not support --kv-cache-size");
    return trtmc::mixtral::create(context);
}
