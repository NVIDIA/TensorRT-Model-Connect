...
def test_runtime_does_not_require_unused_bundle_metadata() -> None:
    runtime = Path(__file__).resolve().parents[1] / "runtime"
    source = (runtime / "diffusion_helpers.cpp").read_text(encoding="utf-8")
    header = (runtime / "ltx_video_diffusion_types.h").read_text(encoding="utf-8")

    for field in ("freq_dim", "num_vae_caches", "vae_model_id"):
