...
"""LTX-Video checkpoint, component, scheduler, and RoPE contracts."""
...
pytest.importorskip("tensorrt", reason="TensorRT is required for LTX-Video builder tests")
safetensors_numpy = pytest.importorskip(
    "safetensors.numpy",
    reason="safetensors is required for LTX-Video checkpoint tests",
)

from families.ltx_video import model  # noqa: E402
from families.ltx_video.config import ModelConfig  # noqa: E402
from families.ltx_video.ltx_dit_builder import (  # noqa: E402
    _make_ltx_rotate_half_matrix,
    make_ltx_rope_tables,
)
...
def test_load_weights_requires_ltx_diffusers_model_index(tmp_path: Path) -> None:
    ...
    (model_dir / "vae" / "config.json").write_text(
        json.dumps({"scaling_factor": 1.0}),
        encoding="utf-8",
    )
    safetensors_numpy.save_file(
        {
            "latents_mean": np.asarray([0.1, 0.2], dtype=np.float32),
            "latents_std": np.asarray([0.9, 1.1], dtype=np.float32),
        },
        str(model_dir / "vae" / "diffusion_pytorch_model.safetensors"),
    )
    ...
    config = _cfg()
    weights = model._LTXVideoModel().load_weights(str(model_dir), config)
    ...
    assert config.raw["_scheduler_config"]["base_image_seq_len"] == 1024

    bad_dir = tmp_path / "bad"
    bad_dir.mkdir()
    with pytest.raises(ValueError, match="Expected diffusers format"):
        model._LTXVideoModel().load_weights(str(bad_dir), _cfg())
    ...
    with pytest.raises(ValueError, match="Expected LTX pipeline"):
        model._LTXVideoModel().load_weights(str(non_ltx), _cfg())


def test_build_components_calls_native_subbuilders(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setitem(
        sys.modules,
        "families.ltx_video.t5_encoder_builder",
        _module(
            "families.ltx_video.t5_encoder_builder",
    ...
    monkeypatch.setattr(model, "_compile_ltx_denoiser_engine", compile_denoiser)
    monkeypatch.setattr(model, "_compile_ltx_vae_decoder_engine", compile_vae)
    ...
    out = model._LTXVideoModel().build_components(
        "/model",
        _cfg(
            video_height=256,
            video_width=384,
...
def test_get_diffusion_config_uses_ltx_scheduler_fields() -> None:
    ...
    out = model._LTXVideoModel().get_diffusion_config(config)
...
def test_ltx_rope_tables_use_diffusers_frequency_triplet_order() -> None:
    ...
    token = 5
    coords = np.array([0.0, 32.0 / 2048.0, 2.0 * 32.0 / 2048.0], dtype=np.float32)
