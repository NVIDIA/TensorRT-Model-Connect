...
"""Direct build, native-runtime, and official-reference E2E for dpr."""
...
import pytest
import numpy as np
from families.dpr.cli import BuildRequest, build_bundle
...
def _build(model_dir: Path, bundle: Path, manifest: dict) -> None:
    ...
    if any(manifest.get(name) is not None for name in ("image_height", "image_width", "video_num_frames")):
        raise ValueError("unsupported image or video build dimensions")
    if int(manifest.get("max_batch_size", 1)) != 1:
        raise ValueError("unsupported build batch size")
    build_bundle(
        BuildRequest(
            model_dir=model_dir,
            task=manifest['task'],
...
def _official_reference(model_dir: Path, manifest: dict, case: dict, tmp_path: Path):
    ...
    tokenizer = DPRContextEncoderTokenizerFast.from_pretrained(model_dir, trust_remote_code=True)
    encoded = tokenizer(_case_text(case), return_tensors="pt")
    wrapper = DPRContextEncoder.from_pretrained(
        model_dir, trust_remote_code=True, torch_dtype=_torch_dtype(case["reference_precision"])
    ).to(device)
    model = wrapper.ctx_encoder.bert_model.eval()
    encoded = {key: value.to(device) for key, value in encoded.items()}
    with torch.no_grad():
        outputs = model(**encoded)
    assert task == "encoding"
    return {"values": outputs.last_hidden_state[0, 0].float().cpu().numpy()}
...
def test_dpr_cases_keep_the_encoder_only_cls_contract() -> None:
