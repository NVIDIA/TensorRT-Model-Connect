...
from families.dpr import cli, support
...
def test_cli_defaults_match_support_and_publish_a_bundle(monkeypatch, tmp_path):
    ...
    def select_backend(backend):
        assert backend == "trt"
        monkeypatch.setitem(sys.modules, "families.dpr.model", SimpleNamespace(build=build_model))

    monkeypatch.setattr(cli, "select_backend", select_backend)
    output = tmp_path / "model.bundle"
    declaration = support.describe(ModelMetadata({"model_type": "dpr"}, {}))
    assert declaration is not None
    assert family_cli.main(["dpr", "build", str(tmp_path), "-o", str(output)]) == 0
    assert len(calls) == 1 and type(calls[0]) is cli.BuildRequest
    assert calls[0].task == declaration.default_task == "embedding"
...
def test_failed_owner_preserves_existing_output(monkeypatch, tmp_path):
    ...
    monkeypatch.setitem(sys.modules, "families.dpr.model", SimpleNamespace(build=fail))
...
def test_legacy_unsupported_inputs_are_still_rejected(tmp_path, changes):
    legacy = LegacyRequest(tmp_path, tmp_path / "out", "dpr", "embedding", "fp32")
...
def test_invalid_input_is_rejected_before_model_loading(monkeypatch, tmp_path):
    ...
    with pytest.raises(SystemExit):
        family_cli.main(
            ["dpr", "build", str(tmp_path), "-o", str(tmp_path / "out"), "--video-num-frames", "2"]
        )
...
def test_help_is_lazy_and_owned(monkeypatch, capsys):
    ...
    with pytest.raises(SystemExit) as error:
        family_cli.main(["dpr", "build", "--help"])
