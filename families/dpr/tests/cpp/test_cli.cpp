...
class FakeDpr final : public trtmc::IEncoding, public trtmc::IEmbedding, public trtmc::IReranking {
  public:
...
void dpr_contract(const fs::path& root) {
    const auto path = root / "dpr.bundle";
    write_bundle(path, "dpr");
    ...
    check(encoded.first == 0 && Json::parse(encoded.second.output).at("values") == Json({3.0, 1.0}),
          "DPR owner converts the encoding request and preserves result fields");
    check(loaded_runtime == "/installed/runtime",
          "DPR owner uses the supplied installed runtime root");
    ...
    check(embedded.first == 0 &&
          ...
          "DPR owner selects the embedding interface");
    check(loaded_runtime == "/override/runtime", "runtime_root override remains owned by DPR");
...
std::unique_ptr<ITask> load_task(const BundleReader&, const std::string& runtime_root,
    ...
    return std::make_unique<FakeDpr>();
}
} // namespace trtmc

int main() {
    const auto root = fs::temp_directory_path() / ("trtmc-dpr-cli-" + std::to_string(getpid()));
