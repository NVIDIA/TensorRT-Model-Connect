...
#include "families/dpr/runtime/distributed_runtime.h"
#include "families/dpr/runtime/pipeline.h"
#include "families/dpr/runtime/plugin_helpers.h"
...
nlohmann::json require_config(const BundleReader& bundle) {
    ...
    if (!config.is_object())
        throw std::runtime_error("DPR runtime.json must be an object");
    return config;
}
...
std::int32_t require_tensor_parallel_size(const nlohmann::json& config) {
    if (!config.contains("tensor_parallel_size") ||
        ...
        throw std::runtime_error("DPR runtime.json requires tensor_parallel_size");
    }
    const auto value = config.at("tensor_parallel_size").get<std::int64_t>();
    if (value <= 0 || value > std::numeric_limits<std::int32_t>::max())
        throw std::runtime_error("DPR tensor_parallel_size is invalid");
...
std::string require_task(const BundleInfo& info) {
    ...
    throw std::runtime_error("DPR does not implement task: " + info.task);
}

} // namespace
} // namespace trtmc::dpr_factory

TRTMC_DEFINE_FAMILY_PLUGIN_V1("dpr")

extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    if (context.kv_cache_size_bytes != 0)
        throw std::invalid_argument("dpr does not support --kv-cache-size");
    const auto config = trtmc::dpr_factory::require_config(context.reader);
    const auto tp_size = trtmc::dpr_factory::require_tensor_parallel_size(config);
    const auto group = trtmc::dpr::initialize_tensor_parallel_group(tp_size);
    const std::string section =
        tp_size == 1 ? "engine.plan" : "engine.rank" + std::to_string(group.rank) + ".plan";
    const auto& plan = trtmc::dpr_factory::require_section(context.reader, section.c_str());
    ...
    if (!tokenizer)
        throw std::runtime_error("DPR bundle does not contain its required tokenizer");
    const auto task = trtmc::dpr_factory::require_task(context.reader.info());
