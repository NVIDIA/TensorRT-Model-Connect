...
Interface& require_interface(trtmc::ITask& task) {
    ...
    if (result == nullptr)
        throw std::invalid_argument("DPR bundle does not implement the requested interface");
    return *result;
}
...
nlohmann::json execute(const std::string& handler, const nlohmann::json& values,
    ...
    if (handler != "encode" && handler != "embed" && handler != "rerank")
        throw std::invalid_argument("unknown DPR CLI handler: " + handler);
    // Check identity before loading: a DPR command must not execute another
    ...
    if (reader.info().family != "dpr")
        throw std::invalid_argument("DPR CLI requires a dpr bundle");
    ...
    if (handler == "rerank") {
        ...
        if (!std::isfinite(score))
            throw std::runtime_error("DPR reranking returned a non-finite score");
        return {{"score", score}};
    }
    ...
    for (const auto value : result.data) {
        if (!std::isfinite(value))
            throw std::runtime_error("DPR embedding returned a non-finite value");
    }
