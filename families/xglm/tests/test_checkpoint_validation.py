...
def test_loader_rejects_complete_malformed_embedding_shape(optimize, shape):
    ...
    env = _load_source(
        ...
        optimize,
        WeightDict=dict,
        _open_torch_checkpoint=lambda _: [],
    ...
    with pytest.raises(ValueError, match="Embedding shape") as error:
        env["_XGLMModel"]().load_weights("unused", config)
...
def test_valid_embedding_mapping_is_preserved(optimize):
    ...
    env = _load_source(
        ["_XGLMModel", "_make_sinusoidal_position_embedding"],
        optimize,
        WeightDict=dict,
        _open_torch_checkpoint=lambda _: [],
    ...
    weights = env["_XGLMModel"]().load_weights("unused", config)
