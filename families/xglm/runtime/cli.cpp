...
trtmc::TextGenerationConfig generation_config(const Json& values) {
    ...
    if (config.temperature < 0.0F || config.top_p < 0.0F || config.top_p > 1.0F ||
        ...
        throw std::invalid_argument("invalid xglm sampling configuration");
    return config;
}

Json execute(const std::string& handler, const Json& values, const char* default_runtime_root) {
    if (handler != "generate")
        throw std::invalid_argument("unknown xglm CLI handler: " + handler);
    const trtmc::BundleReader reader(values.at("bundle").get<std::string>());
    if (reader.info().family != "xglm")
        throw std::invalid_argument("xglm CLI requires a xglm bundle");
    ...
    if (generation == nullptr)
        throw std::invalid_argument("xglm bundle does not implement text generation");
    ...
    if (!std::isfinite(result.setup_ms) || !std::isfinite(result.prefill_ms) ||
        ...
        throw std::runtime_error("xglm returned non-finite timing values");
