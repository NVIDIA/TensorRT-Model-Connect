...
#include "families/xglm/runtime/pipeline.h"

#include "families/xglm/runtime/chat_templates.h"
#include "families/xglm/runtime/kv_cache.h"
#include "families/xglm/runtime/tensor_names.h"
...
XglmTextGenerationPipeline::XglmTextGenerationPipeline(std::unique_ptr<ITrtModule> decoder,
                                                       std::unique_ptr<XglmInferenceState> state,
                                                       XglmTextGenConfig config,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("XglmTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("XglmTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
TextResult XglmTextGenerationPipeline::run(const internal::TextContinuationRequest& request,
                                           internal::ConfigView supplied) {
    const auto cfg = xglm::parse_text_config(supplied);
    ...
    if (const auto* prompt = std::get_if<std::string_view>(&request.prefix)) {
        ...
        input_ids = xglm::copy_token_prefix(std::get<Span<const std::int32_t>>(request.prefix));
    }
...
void require_prefill_kv_pointers(ITrtModule& prefill, const XglmTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(xglm_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(xglm_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "XglmTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const XglmTextGenConfig& cfg, int32_t sq,
                                      XglmInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("XglmTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error("XglmTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("XglmTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error("XglmTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<XglmKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("XglmTextGenerationPipeline: prefill requires XglmKvCache");
}

} // namespace

void XglmTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error("XglmTextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(lt.numel()) < vocab)
        throw std::runtime_error(
            "XglmTextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
XglmTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
XglmTextGenerationPipeline::TimedGenResult XglmTextGenerationPipeline::generate_from_ids(
    ...
    if (mode != "ar")
        throw std::runtime_error("XglmTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_xglm_sampler(params);
...
int32_t XglmTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const XglmSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& XglmTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
