...
"""timm MobileNetV2 image-classification family plugin.

Supports timm MobileNetV2 classifiers stored in HF Hub format. The initial target
is:
...
activation is uniform ReLU6, so only the per-stage stride comes from an
architecture table. MobileNetV2 has no squeeze-excite gate; the gate is still
...
_BN_EPS = 1e-5
_ARCHITECTURE = "mobilenetv2_100"

# Per-stage stride for the 7-stage MobileNetV2 layout; the stride applies to the
...
def _resolve_config(raw: dict) -> dict:
    ...
    if not isinstance(pcfg, dict):
        raise ValueError("timm MobileNetV2 config requires pretrained_cfg")
    ...
    if missing:
        raise ValueError(f"timm MobileNetV2 config is missing required fields: {missing}")
    input_size = pcfg["input_size"]
    if not isinstance(input_size, list) or len(input_size) != 3 or int(input_size[0]) != 3:
        raise ValueError("timm MobileNetV2 pretrained_cfg.input_size must be [3, height, width]")
    image_h, image_w = int(input_size[1]), int(input_size[2])
    if not isinstance(pcfg["mean"], list) or not isinstance(pcfg["std"], list):
        raise ValueError("timm MobileNetV2 mean/std must be lists")
    ...
    if image_h <= 0 or image_w <= 0 or num_classes <= 0:
        raise ValueError("timm MobileNetV2 image dimensions and num_classes must be positive")
    if len(mean) != 3 or len(std) != 3 or any(value == 0.0 for value in std):
        raise ValueError("timm MobileNetV2 mean/std must contain three channels with non-zero std")
    if not 0.0 < crop_pct <= 1.0 or pcfg.get("crop_mode", "center") != "center":
        raise ValueError("timm MobileNetV2 requires a center crop with crop_pct in (0, 1]")
    if interpolation not in {"bilinear", "bicubic"}:
        raise ValueError("timm MobileNetV2 supports only bilinear or bicubic interpolation")
    return {
...
def _discover_layout(readers) -> dict:
    ...
    if strides is None:
        raise ValueError(f"No MobileNetV2 stride schedule for {len(stages)} stages")
    ...
    for stage in stages:
        ...
        for index in indices:
            ...
            if kind != expected_kind:
                raise ValueError(f"MobileNetV2 stage {stage} requires {expected_kind}, found {kind}")
            if present != required:
                raise ValueError(f"Unexpected MobileNetV2 block keys at stage {stage}: {present}")
            blocks.append(
                {
...
class _TimmMobilenetv2Model:
    def load_weights(
        ...
        *,
        ...
        cfg.update(layout)
        raw["_timm_mobilenetv2_config"] = cfg
    ...
    def build_engine(
        ...
        *,
        ...
        if precision == "fp16":
            ...
            raise ValueError(f"Unsupported timm_mobilenetv2 precision: {precision}")

        cfg = config.raw.get("_timm_mobilenetv2_config")
        if cfg is None:
        ...
        if image_h % total_stride != 0 or image_w % total_stride != 0:
            raise ValueError(
                f"timm_mobilenetv2 input {image_h}x{image_w} must be divisible by {total_stride}"
            )

        if verbose:
            print(
                "[trtmc build] timm_mobilenetv2: "
        ...
        hidden = graph_ops.add_relu6(network, hidden)
        ...
        # MobileNetV2 runs the head convolution on the feature map and pools after,
        ...
        if plan is None:
            raise RuntimeError("TensorRT timm_mobilenetv2 engine build failed")
        return bytes(plan)
    ...
    def get_bundle_config_overrides(self, config: ModelConfig) -> dict:
        ...
        if not isinstance(vocabulary_id, str):
            raise ValueError("timm MobileNetV2 vocabulary_id must be a string")
        if not isinstance(labels, list) or (
            ...
            raise ValueError("timm MobileNetV2 label_names must name every class in order")
        return {
...
def build(request: "BuildRequest", writer: "BundleWriter") -> None:
    """Build one timm MobileNetV2 image-classification bundle."""
    if request.dynamic_kv_cache:
        raise NotImplementedError("timm_mobilenetv2 does not support dynamic_kv_cache")

    if request.image_height is not None:
        raise NotImplementedError("timm_mobilenetv2 does not support image_height")
    if request.image_width is not None:
        raise NotImplementedError("timm_mobilenetv2 does not support image_width")
    if request.video_num_frames is not None:
        raise NotImplementedError("timm_mobilenetv2 does not support video_num_frames")
    if request.max_batch_size != 1:
        raise NotImplementedError("timm_mobilenetv2 does not support max_batch_size")
    if request.tensor_parallel_size != 1:
        raise NotImplementedError("timm_mobilenetv2 does not support tensor parallelism")
    if request.context_parallel_size != 1:
        raise NotImplementedError("timm_mobilenetv2 does not support context parallelism")
    if request.task != "image_to_class_scores":
        raise ValueError("timm_mobilenetv2 supports only task=image_to_class_scores")
    if request.quantization not in {None, "none"}:
        raise NotImplementedError("timm_mobilenetv2 does not support quantization")
    if request.fp32_layers:
        raise NotImplementedError("timm_mobilenetv2 does not support mixed-precision layers")
    if request.max_sequence_length not in {None, 1}:
        raise NotImplementedError("timm_mobilenetv2 supports only max_sequence_length=1")

    model_dir = Path(request.model_dir)
    config = ModelConfig.from_dir(model_dir)
    if config.architecture != _ARCHITECTURE:
        raise ValueError(f"timm MobileNetV2 does not support architecture={config.architecture!r}")
    precision = str(request.precision).lower()
    model = _TimmMobilenetv2Model()
    ...
    writer.set_header(family="timm_mobilenetv2", task=request.task, backend=request.backend)
