...
MobileNetV2 is an inverted-residual stack with uniform ReLU and no squeeze-excite
gate, so it needs no ops beyond the plain convolutional set: grouped depthwise
convolution, folded batch norm, ReLU, pooling, and a linear head.
"""
...
def add_relu6(network, x):
    """ReLU6: min(max(x, 0), 6), the activation used by MobileNetV2."""
