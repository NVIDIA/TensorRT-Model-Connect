...
try:
    ...
    from families.timm_mobilenetv2.config import ModelConfig
    from families.timm_mobilenetv2.model import _TimmMobilenetv2Model, build as build_family
...
def _write_tiny_mobilenetv2(tmp_path: Path, *, stages: int = 7, with_se: bool = False) -> None:
...
def test_model_config_uses_timm_architecture_when_model_type_absent(tmp_path: Path):
    ...
    assert cfg.architecture == "mobilenetv2_100"
...
def test_layout_rejects_an_unconsumed_block_tensor(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="Unexpected MobileNetV2 block keys"):
        model.load_weights(str(tmp_path), ModelConfig.from_dir(tmp_path), precision="fp32")
...
def test_build_rejects_unqualified_semobilenetv2_variant(tmp_path: Path):
    _write_tiny_mobilenetv2(tmp_path)
    config_path = tmp_path / "config.json"
    config = json.loads(config_path.read_text(encoding="utf-8"))
    config["architecture"] = "semobilenetv2_100"
    config_path.write_text(json.dumps(config), encoding="utf-8")
    ...
    with pytest.raises(ValueError, match="architecture='semobilenetv2_100'"):
...
def test_build_exports_semantic_task_and_complete_class_metadata(tmp_path: Path, monkeypatch):
    _write_tiny_mobilenetv2(tmp_path)
    monkeypatch.setattr(_TimmMobilenetv2Model, "build_engine", lambda *args, **kwargs: b"plan")
    sections = {}
