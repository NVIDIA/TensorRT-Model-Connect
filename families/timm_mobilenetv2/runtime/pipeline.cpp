...
const Tensor& require_logits(const TensorMap& outputs) {
    ...
    if (found->second.numel() <= 0)
        throw std::runtime_error("timm MobileNetV2 engine returned empty logits");
    return found->second;
}
...
ClassificationResult TimmMobilenetv2ImageClassificationPipeline::classify(const float* pixels,
                                                                          int32_t height,
                                                                          int32_t width) {
    auto pixel_values =
        preprocess_timm_mobilenetv2_image(pixels, height, width, preprocess_config_);
