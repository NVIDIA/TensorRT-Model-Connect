    ...
    *,
    ...
    scores = graph.add(
        ...
        graph.cast(
            graph.reshape(
                mask_bias,
                (1, windows, 1, ATOM_WINDOW_QUERIES, ATOM_WINDOW_KEYS),
            ),
            scores.dtype,
        ),
    )
...
def reference_atom_features(
    ...
    *,
    ...
    displacement = graph.sub(
        graph.reshape(
            q_pos,
            (1, padded_atoms // ATOM_WINDOW_QUERIES, ATOM_WINDOW_QUERIES, 1, 3),
        ),
        graph.reshape(
            k_pos,
            (1, padded_atoms // ATOM_WINDOW_QUERIES, 1, ATOM_WINDOW_KEYS, 3),
        ),
    )
    pair_mask = atom_pair_mask(
        graph,
    ...
    same_uid = graph.equal(
        graph.reshape(
            q_uid,
            (1, padded_atoms // ATOM_WINDOW_QUERIES, ATOM_WINDOW_QUERIES, 1, 1),
        ),
        graph.reshape(
            k_uid,
            (1, padded_atoms // ATOM_WINDOW_QUERIES, 1, ATOM_WINDOW_KEYS, 1),
        ),
    )
    valid = graph.mul(
        graph.reshape(
            ...
            (
                1,
                padded_atoms // ATOM_WINDOW_QUERIES,
                ATOM_WINDOW_QUERIES,
                ATOM_WINDOW_KEYS,
                1,
            ),
