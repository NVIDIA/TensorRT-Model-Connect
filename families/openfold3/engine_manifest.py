...
from .atom_attention_builder import ATOM_WINDOW_QUERIES
from .contracts import INITIAL_FP16_PROFILE


PAIRFORMER_SEGMENT_SIZE: Final = 6
DIFFUSION_SEGMENT_SIZE: Final = 6
PAIRFORMER_BLOCKS: Final = 48
DIFFUSION_TOKEN_BLOCKS: Final = 24
...
PAIRFORMER_ENGINE_SPECS: Final = tuple(
    EngineSpec(
        "pairformer",
        f"openfold3_pairformer_{start:02d}_{start + PAIRFORMER_SEGMENT_SIZE:02d}_plan",
        ("s", "z", "token_mask"),
        ("s_out", "z_out"),
    )
...
def graph_manifest_json(
    *,
    ...
    if (
        not INITIAL_FP16_PROFILE.min_tokens <= token_count <= INITIAL_FP16_PROFILE.max_tokens
        or atom_count <= 0
        or padded_atom_count < atom_count
        or padded_atom_count % ATOM_WINDOW_QUERIES != 0
    ):
