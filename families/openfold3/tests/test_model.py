...
from families.openfold3.engine_manifest import (
    ALL_ENGINE_SPECS,
    graph_manifest_json,
)
from families.openfold3.feature_bundle import (
    FEATURE_NAMES,
...
from families.openfold3.graph_ops import (
    accumulation_dtype,
...
from families.openfold3.model_config import (
    CHECKPOINT,
    COMPONENTS,
    FEATURES,
    QUERY,
...
from families.openfold3.random_samples import (
    deserialize_random_samples,
    serialize_pinned_random_samples,
    serialize_random_arrays,
)
from families.openfold3.support import describe
...
def test_pinned_provenance_is_immutable_and_complete() -> None:
    ...
    assert pin.chemical_components.size_bytes == 63_393_643
...
def test_mixed_fp16_is_default_and_bf16_remains_supported() -> None:
    ...
    assert model.DEFAULT_PRECISION == "fp16"
    ...
    model._require_precision("bf16")
    with pytest.raises(ValueError, match="supports mixed precision"):
        model._require_precision("fp32")


def test_family_support_owns_the_mixed_fp16_default() -> None:
    files = tuple(
        str(path) for path in (CHECKPOINT, COMPONENTS, QUERY, FEATURES, STRUCTURE_METADATA)
    )
    support = describe(ModelMetadata(config={}, model_index={}, files=files))
...
def test_sampling_controls_are_rejected_before_cuda_is_loaded() -> None:
    with pytest.raises(ValueError, match="exactly 200"):
        serialize_pinned_random_samples(
            atom_mask=np.ones((1, 32), dtype=np.float32), sampling_steps=199
        )


def test_prepared_package_rejects_symlinked_artifacts(tmp_path: Path) -> None:
    paths = (CHECKPOINT, COMPONENTS, QUERY, FEATURES, STRUCTURE_METADATA)
    for relative in paths:
        (tmp_path / relative).write_bytes(b"owned")
    (tmp_path / COMPONENTS).unlink()
    (tmp_path / COMPONENTS).symlink_to(tmp_path / CHECKPOINT)
    assert resolve_package_root(tmp_path) is None
...
def test_pinned_ubiquitin_build_inputs_have_expected_identity() -> None:
    ...
    assert features_path.stat().st_size == 4_386_540
    assert structure_path.stat().st_size == 93_641
