...
void OpenFold3Pipeline::validateEngineSet() {
    static_assert(kPairformerSegments > 0);
    static_assert(kTokenSegments > 0);
...
StructureConfidence OpenFold3Pipeline::runConfidence(const std::vector<float>& coordinates) {
    ...
    for (float& value : result.plddt)
        value *= 100.0F;
    result.average_plddt = std::accumulate(result.plddt.begin(), result.plddt.end(), 0.0F) /
...
std::string OpenFold3Pipeline::resultMetadata(const StructureConfidence& confidence) const {
...
StructurePredictionResult OpenFold3Pipeline::predict_structure(const std::string& input) {
    ...
    result.metadata_json = resultMetadata(confidence);
    return result;
}
