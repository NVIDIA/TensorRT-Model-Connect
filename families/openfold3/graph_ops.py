...
"""OpenFold3-owned strongly typed TensorRT graph primitives."""
...
def low_precision_dtype(trt: Any, precision: str):
    """Resolve the qualified mixed-precision compute type."""

    if precision == "fp16":
        return trt.float16
    if precision == "bf16":
        dtype = getattr(trt, "bfloat16", None)
        if dtype is not None:
            return dtype
    raise ValueError(f"unsupported OpenFold3 mixed precision: {precision}")


def accumulation_dtype(trt: Any, precision: str):
    """Keep pair and MSA contractions numerically stable."""

    low_precision_dtype(trt, precision)
    return trt.float32


def stable_attention_dtype(trt: Any):
    """Keep sequence and atom diffusion attention numerically stable."""

    return trt.float32


def triangle_attention_dtype(trt: Any, precision: str):
    """Use BF16 triangle attention while retaining stable mixed FP16."""

    # Upstream leaves triangle attention in its BF16 autocast region but
    # explicitly promotes sequence, atom, and diffusion attention.  FP16 needs
    # the additional promotion because the upstream FP16 rollout is non-finite.
    if precision == "fp16":
        return trt.float32
    return low_precision_dtype(trt, precision)


def diffusion_compute_dtype(trt: Any, precision: str):
    """Resolve learned-projection compute inside the FP32 diffusion state."""
...
class Graph:
    ...
    def __init__(
        ...
        network: Any,
        trt: Any,
        weights: MutableMapping[Any, Any],
        *,
        precision: str = "fp16",
    ):
        ...
        self.low_precision_dtype = low_precision_dtype(trt, precision)
        self.accumulation_dtype = accumulation_dtype(trt, precision)
        self.stable_attention_dtype = stable_attention_dtype(trt)
        self.triangle_attention_dtype = triangle_attention_dtype(trt, precision)
        if not isinstance(weights, MutableMapping):
            raise TypeError("OpenFold3 TensorRT weights must provide mutable lifetime storage")
    ...
    def weight(self, name: str) -> np.ndarray:
        try:
            ...
            raise ValueError(f"OpenFold3 checkpoint tensor is missing: {name}") from error
    ...
    def linear(self, tensor: Any, prefix: str):
        ...
        if int(tensor.shape[-1]) != in_features:
            raise ValueError(
                f"OpenFold3 linear input mismatch for {prefix}: "
