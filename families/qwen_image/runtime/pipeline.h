...
class QwenImagePipeline final : public IImageGeneration,
  ...
  public:
    ...
    //
    // attention_mask contains one 1/0 entry per text row. It is expanded to
    // the joint text+image key mask expected by the denoiser engine.
    ...
    // before the loop). joint_attention_mask is the additive [text,image] key
    // mask, also prepared once before the loop.
