...
std::vector<float> make_joint_attention_mask(const std::vector<int32_t>& text_mask,
                                             int max_text_tokens, int image_tokens) {
    if (text_mask.size() != static_cast<std::size_t>(max_text_tokens)) {
        throw std::runtime_error(
            "QwenImagePipeline: attention_mask size does not match max_text_tokens");
    }
    std::vector<float> out(static_cast<std::size_t>(max_text_tokens + image_tokens), 0.0F);
    for (int token = 0; token < max_text_tokens; ++token) {
        if (text_mask[static_cast<std::size_t>(token)] == 0) {
            out[static_cast<std::size_t>(token)] = kAdditiveMaskPad;
        }
    }
    return out;
}
...
std::vector<float>
stack_joint_attention_masks(const std::vector<QwenImagePipeline::EncodedPrompt>& encoded,
                            int max_text_tokens, int image_tokens) {
    const std::size_t per_sample = static_cast<std::size_t>(max_text_tokens + image_tokens);
    std::vector<float> out(encoded.size() * per_sample, 0.0F);
    for (std::size_t batch = 0; batch < encoded.size(); ++batch) {
        const auto mask =
            make_joint_attention_mask(encoded[batch].attention_mask, max_text_tokens, image_tokens);
        float* sample = out.data() + batch * per_sample;
        std::copy(mask.begin(), mask.end(), sample);
    }
    return out;
}
...
void dit_forward_batched(ITrtModule& engine, float* latents_batch, float* hidden_batch,
                         float* attention_mask_batch, float* timestep_buf,
                         const BatchedChunkShape& s, std::vector<float>& out_buf,
    ...
    inputs["attention_mask"] =
        Tensor{attention_mask_batch,
               {static_cast<int64_t>(s.B), 1, 1, static_cast<int64_t>(s.max_text + s.n_img)},
               DType::kFloat32};
...
std::vector<ImageResult> QwenImagePipeline::generate_image_batch_chunk(
    ...
    std::vector<float> pos_mask_batch =
        stack_joint_attention_masks(pos_encoded, s.max_text, s.n_img);
    std::vector<float> neg_mask_batch =
        stack_joint_attention_masks(neg_encoded, s.max_text, s.n_img);
    ...
    for (int step = 0; step < num_steps; ++step) {
        ...
        dit_forward_batched(*denoiser_engine_, latents_packed_batch.data(), pos_hidden_batch.data(),
                            pos_mask_batch.data(), timestep_buf.data(), s, noise_pos_buf,
                            "positive");

        const float* noise_for_scheduler = noise_pos_buf.data();
        if (do_cfg) {
            dit_forward_batched(*denoiser_engine_, latents_packed_batch.data(),
                                neg_hidden_batch.data(), neg_mask_batch.data(), timestep_buf.data(),
                                s, noise_neg_buf, "negative");
...
QwenImagePipeline::run_denoiser_once(const std::vector<float>& latents_packed, float normalized_t,
    ...
    const int image_tokens =
        static_cast<int>(latents_packed.size() / static_cast<std::size_t>(in_channels));
    const auto joint_attention_mask =
        make_joint_attention_mask(attention_mask, max_text_tokens, image_tokens);
    std::vector<float> result;
    run_denoiser_into(latents_packed, normalized_t, hidden_states, joint_attention_mask, result);
    return result;
}
...
void QwenImagePipeline::run_denoiser_into(const std::vector<float>& latents_packed,
                                          float normalized_t,
                                          const std::vector<float>& hidden_states,
                                          const std::vector<float>& joint_attention_mask,
    ...
    if (joint_attention_mask.size() !=
        static_cast<std::size_t>(max_text_tokens) + static_cast<std::size_t>(n_img)) {
        throw std::runtime_error(
            "QwenImagePipeline::run_denoiser_into: joint attention mask has wrong size");
    }
    ...
    inputs["attention_mask"] = Tensor{const_cast<float*>(joint_attention_mask.data()),
                                      {1, 1, 1, n_img + max_text_tokens},
                                      DType::kFloat32};
...
std::vector<float> QwenImagePipeline::denoise_loop_with_cfg(
    ...
    const int total_image_tokens =
        static_cast<int>(total_numel / static_cast<std::size_t>(in_channels));
    const auto pos_joint_attention_mask = make_joint_attention_mask(
        pos.attention_mask, config_.denoiser.max_text_tokens, total_image_tokens);
    std::vector<float> neg_joint_attention_mask;
    if (do_cfg) {
        neg_joint_attention_mask = make_joint_attention_mask(
            neg.attention_mask, config_.denoiser.max_text_tokens, total_image_tokens);
    }
    ...
    for (int step = 0; step < num_steps; ++step) {
        ...
        run_denoiser_into(*model_latents, norm_t, pos.hidden_states, pos_joint_attention_mask,
                          noise_pos_buf);
        const float* noise_for_scheduler = noise_pos_buf.data();
        if (do_cfg) {
            run_denoiser_into(*model_latents, norm_t, neg.hidden_states, neg_joint_attention_mask,
                              noise_neg_buf);
