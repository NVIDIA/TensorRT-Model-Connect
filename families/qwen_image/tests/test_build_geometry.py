...
from families.qwen_image.config import ModelConfig
from families.qwen_image.model import _apply_static_image_geometry
...
def test_build_flags_drive_dit_vae_and_bundle_geometry() -> None:
    ...
    assert _apply_static_image_geometry(config, bundle) == (512, 768, 64, 96, 32, 48)
    assert bundle["image"]["default_height"] == 512
    assert bundle["image"]["default_width"] == 768


def test_build_geometry_defaults_to_bundle_size() -> None:
    config = ModelConfig.create_tiny("qwen_image")

    assert _apply_static_image_geometry(config, _bundle_config()) == (
        1024,
        1024,
        128,
        128,
...
def test_build_geometry_rejects_unbuildable_shapes(
    height: int,
    width: int,
    message: str,
) -> None:
