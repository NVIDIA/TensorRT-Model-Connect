...
"""Run the official Transformers InternVL reference for qualification."""
...
def _prepare(reference: Reference, image_path: str, prompt: str) -> dict[str, Any]:
    ...
    if (
        ...
        or prompt not in rendered
        or "<IMG_CONTEXT>" not in rendered
    ):
        raise ValueError("InternVL chat template lost the image or text prompt")
...
def _performance(arguments: argparse.Namespace) -> int:
    ...
    if arguments.family != "internvl" or arguments.operation != "generate":
        raise ValueError("InternVL reference only supports internvl generate")
    if arguments.mode != "pytorch-eager":
        raise ValueError("InternVL reference only supports pytorch-eager")
