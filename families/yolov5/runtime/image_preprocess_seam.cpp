...
std::vector<float> preprocess_yolov5_image(const float* pixels, std::int32_t height,
                                           std::int32_t width, const Yolo11PreprocessConfig& config,
                                           Yolo11Letterbox& letterbox) {
    if (pixels == nullptr || height <= 0 || width <= 0)
        throw std::invalid_argument("YOLOv5 preprocessing needs a non-empty image");
    if (config.input_image_h <= 0 || config.input_image_w <= 0)
        throw std::invalid_argument("YOLOv5 input size must be positive");
