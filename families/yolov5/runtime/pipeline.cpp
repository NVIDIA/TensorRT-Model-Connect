...
// Greedy non-maximum suppression, per class. YOLOv5's head reports one
...
std::vector<DetectionBox> suppress_yolov5_boxes(std::vector<DetectionBox> boxes,
...
ObjectDetectionResult Yolo11ObjectDetectionPipeline::detect(const float* pixels, int32_t height,
    ...
    auto values = preprocess_yolov5_image(pixels, height, width, preprocess_config_, letterbox);
    Tensor input;
    ...
    if (boxes == nullptr || scores == nullptr || classes == nullptr)
        throw std::runtime_error("YOLOv5 engine did not return boxes, scores and classes");
    if (boxes->dtype != DType::kFloat32 || scores->dtype != DType::kFloat32)
        throw std::runtime_error("YOLOv5 boxes and scores must be float32");
    if (classes->dtype != DType::kInt32)
        throw std::runtime_error("YOLOv5 classes must be int32");
    ...
    if (static_cast<std::size_t>(boxes->numel()) != count * 4U ||
        ...
        throw std::runtime_error("YOLOv5 detection outputs disagree on their length");
    ...
    result.boxes = suppress_yolov5_boxes(std::move(candidates), iou_threshold_,
