...
// The defaults ultralytics predicts with.
constexpr float kScoreThreshold = 0.25F;
constexpr float kIouThreshold = 0.7F;
constexpr std::int32_t kMaxDetections = 300;
...
Yolo11PreprocessConfig parse_config(const std::vector<char>& data) {
    const auto json = nlohmann::json::parse(data.begin(), data.end());
    Yolo11PreprocessConfig config;
    ...
    if (config.input_image_h <= 0 || config.input_image_w <= 0 || config.pad_value < 0.0F ||
        ...
        throw std::runtime_error("YOLOv5 runtime.json does not match its contract");
    }
    return config;
}
...
std::unique_ptr<ITrtModule> load_engine(IBackend& backend, const std::vector<char>& plan) {
    ...
    if (!engine || !engine->ok())
        throw std::runtime_error("YOLOv5 engine failed to load");
    return engine;
}
...
TRTMC_DEFINE_FAMILY_PLUGIN_V1("yolov5")

extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    if (context.kv_cache_size_bytes != 0)
        throw std::invalid_argument("yolov5 does not support --kv-cache-size");
    const auto config_data = trtmc::yolov5::require_section(context.reader, "runtime.json");
    const auto plan = trtmc::yolov5::require_section(context.reader, "engine.plan");
    auto config = trtmc::yolov5::parse_config(config_data);
    auto engine = trtmc::yolov5::load_engine(context.backend, plan);
    return new trtmc::Yolo11ObjectDetectionPipeline(
        std::move(engine), std::move(config), trtmc::yolov5::kScoreThreshold,
        trtmc::yolov5::kIouThreshold, trtmc::yolov5::kMaxDetections);
}
