...
from families.magpie_tts.tests import test_e2e as e2e
from families.magpie_tts.tests.reporting import record_audio_views
...
def test_listening_views_keep_real_rates_before_comparison_failure(monkeypatch, tmp_path: Path):
    ...
    sf.write(reference_audio, samples, 22050, subtype="FLOAT")
    expected["audio"] = str(reference_audio)
    expected["sample_rate"] = 999  # The listening caption must use the WAV header.
