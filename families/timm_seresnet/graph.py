...
"""Small TensorRT graph vocabulary owned by timm SEResNet."""
...
def convolution(
    ...
    *,
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a SEResNet convolution")
...
def relu(network, tensor):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a SEResNet ReLU")
    return layer.get_output(0)
...
def add(network, left, right):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a SEResNet residual add")
    return layer.get_output(0)
...
def squeeze_excite(
    ...
    *,
    ...
    if pooled is None:
        raise RuntimeError("TensorRT rejected the SEResNet squeeze pooling")
    ...
    if activation is None:
        raise RuntimeError("TensorRT rejected the SEResNet excitation sigmoid")
    scaled = network.add_elementwise(
        tensor, activation.get_output(0), trt.ElementWiseOperation.PROD
    )
    if scaled is None:
        raise RuntimeError("TensorRT rejected the SEResNet excitation product")
...
def max_pool(network, tensor, *, kernel: int, stride: int, padding: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected the SEResNet stem pooling")
    layer.stride_nd = (stride, stride)
    layer.padding_nd = (padding, padding)
    return layer.get_output(0)
...
def global_average_pool(network, tensor, height: int, width: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected SEResNet global average pooling")
    layer.stride_nd = (1, 1)
    return layer.get_output(0)
...
def classifier(
    ...
    *,
    ...
    if flattened is None:
        raise RuntimeError("TensorRT rejected the SEResNet classifier reshape")
    ...
    if matrix_layer is None:
        raise RuntimeError("TensorRT rejected the SEResNet classifier weights")
    ...
    if product is None:
        raise RuntimeError("TensorRT rejected the SEResNet classifier matmul")
    ...
    if bias_layer is None:
        raise RuntimeError("TensorRT rejected the SEResNet classifier bias")
    ...
    if output is None:
        raise RuntimeError("TensorRT rejected the SEResNet classifier output")
