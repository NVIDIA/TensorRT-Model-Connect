...
TimmSEResNetPreprocessConfig parse_config(const nlohmann::json& json) {
    ...
    if (config.input_image_h <= 0 || config.input_image_w <= 0 || config.crop_pct <= 0.0F ||
        ...
        throw std::runtime_error("timm SE-ResNet runtime.json does not match its runtime contract");
    }
    return config;
}
...
std::unique_ptr<ITrtModule> load_engine(IBackend& backend, const std::vector<char>& plan) {
    ...
    if (!engine || !engine->ok())
        throw std::runtime_error("timm SE-ResNet engine failed to load");
    return engine;
}
...
extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    ...
    auto config = trtmc::timm_seresnet::parse_config(metadata);
    auto engine = trtmc::timm_seresnet::load_engine(context.backend, plan);
    return new trtmc::TimmSEResNetImageClassificationPipeline(
