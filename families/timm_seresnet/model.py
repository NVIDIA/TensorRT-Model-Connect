...
"""Plain TensorRT build entrypoint for timm SE-ResNet classifiers.

Depth comes from the checkpoint rather than a table: each stage's block count
is read from its `layer<n>.<index>` keys, and whether the block is a bottleneck
or a basic block is decided by whether a third convolution exists. The grouped
(SE-ResNeXt) case falls out of the 3x3's own weight shape.

This family only builds checkpoints that carry a squeeze-excitation gate; a
plain ResNet is a different family and is rejected rather than silently built
without its gate.
"""
...
def _read_config(model_dir: Path) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"SEResNet model config is missing: {path}")
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise ValueError("SEResNet config.json must contain one object")
    ...
    if not isinstance(identity, str) or not identity.lower().startswith("seresnet"):
        raise ValueError(f"unsupported timm SE-ResNet model identity: {identity!r}")
    return value
...
def _preprocess_config(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(input_size, int):
        ...
        raise ValueError("SEResNet pretrained input_size must be [3, height, width]")
    mean = source.get("mean", [0.485, 0.456, 0.406])
    std = source.get("std", [0.229, 0.224, 0.225])
    if not isinstance(mean, list) or len(mean) != 3:
        raise ValueError("SEResNet image mean must contain three values")
    if not isinstance(std, list) or len(std) != 3:
        raise ValueError("SEResNet image std must contain three values")
    result = {
    ...
    if (
        ...
        raise ValueError("SEResNet preprocessing or classifier config is invalid")
    return result
...
def _layout(checkpoint: Checkpoint) -> dict[str, Any]:
    ...
    for stage in _STAGES:
        ...
        if not indices:
            raise ValueError(f"SEResNet checkpoint has no blocks for stage {stage}")
        if sorted(indices) != list(range(len(indices))):
            raise ValueError(f"SEResNet stage {stage} block indices are not contiguous")
        depths.append(len(indices))
    if not any(name.startswith("layer1.0.se.") for name in names):
        raise ValueError(
            "SEResNet checkpoint has no squeeze-excitation gate; a plain ResNet "
...
def _fold_norm(
    ...
    if not (gamma.shape == beta.shape == mean.shape == variance.shape):
        raise ValueError(f"SEResNet norm {norm} has mismatched parameter shapes")
    if gamma.shape != (weight.shape[0],):
        raise ValueError(f"SEResNet norm {norm} does not match convolution {conv}")
    if np.any(variance + _BATCH_NORM_EPSILON <= 0.0):
        raise ValueError(f"SEResNet norm {norm} has a non-positive running variance")
...
def _weights(
    ...
    if result["fc.weight"].ndim != 2 or result["fc.bias"].shape != (result["fc.weight"].shape[0],):
        raise ValueError("SEResNet classifier weights have incompatible shapes")
    return result
...
def _block(network, tensor, weights, prefix: str, dtype, *, stride: int, bottleneck: bool):
    ...
    if f"{prefix}.downsample.weight" in weights:
        identity = graph.convolution(
            network, identity, weights[f"{prefix}.downsample.weight"],
            weights[f"{prefix}.downsample.bias"], stride=stride, dtype=dtype,
        )
...
def _build_engine(
    ...
    if precision == "fp16":
        ...
        raise ValueError(f"unsupported timm SEResNet precision: {precision}")
    ...
    if weights["fc.weight"].shape[0] != config["num_classes"]:
        raise ValueError("SEResNet classifier dimensions do not match config.json")
    ...
    if height % total_stride or width % total_stride:
        raise ValueError(f"SEResNet input {height}x{width} must be divisible by {total_stride}")
    if verbose:
        print(
            "[trtmc build] timm_seresnet: "
    ...
    if pixels is None:
        raise RuntimeError("TensorRT rejected the SEResNet input")
    hidden = pixels
    if hidden.dtype != tensor_dtype:
        cast = network.add_cast(hidden, tensor_dtype)
        if cast is None:
            raise RuntimeError("TensorRT rejected the SEResNet input cast")
        hidden = cast.get_output(0)

    stem = weights["stem.weight"]
    hidden = graph.convolution(
        network, hidden, stem, weights["stem.bias"],
        stride=2, padding=int(stem.shape[2]) // 2, dtype=numpy_dtype,
    )
    ...
    if logits.dtype != trt.float32:
        ...
        if cast is None:
            raise RuntimeError("TensorRT rejected the SEResNet output cast")
        logits = cast.get_output(0)
    logits.name = "logits"
    network.mark_output(logits)
    plan = builder.build_serialized_network(network, builder_config)
    if plan is None:
        raise RuntimeError("TensorRT timm SEResNet engine build failed")
...
def build(request: "BuildRequest", writer: "BundleWriter") -> None:
    """Build one timm SE-ResNet image-classification bundle."""
    ...
    if not isinstance(vocabulary_id, str):
        raise ValueError("timm SE-ResNet vocabulary_id must be a string")
    if not isinstance(labels, list) or (labels and (
        len(labels) != runtime["num_classes"]
        or any(not isinstance(label, str) or not label for label in labels)
    )):
        raise ValueError("timm SE-ResNet label_names must name every class")
    writer.set_header(family="timm_seresnet", task=request.task, backend=request.backend)
