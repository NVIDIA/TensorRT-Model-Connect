...
from families.timm_seresnet import model
...
def test_builder_task_and_metadata(tmp_path, monkeypatch, metadata, invalid):
    ...
    request = BuildRequest(model_dir=tmp_path, output_path=tmp_path / "model.bundle",
                           family="timm_seresnet", task="image_to_class_scores", precision="fp32")
    if invalid:
        ...
        return
    model.build(request, writer)
    assert headers == [{"family": "timm_seresnet", "task": "image_to_class_scores", "backend": "trt"}]
