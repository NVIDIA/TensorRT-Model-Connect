...
"""A failed audio comparison must not discard its waveform evidence."""
...
from families.bark.tests import test_e2e as e2e
from tools import e2e_evidence
from families.bark.tests.reporting import record_audio_views


def test_audio_artifact_is_copied_before_comparison_failure(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(e2e, "_native", lambda *args: {"audio": str(audio), "sample_rate": 24000})
    monkeypatch.setattr(
        e2e, "_official_reference", lambda *args: {"samples": np.ones(200), "sample_rate": 24000}
    )
    monkeypatch.setattr(e2e, "_thresholds", lambda case: {"audio_duration_rel_diff": 0.1})

    def fail(*args):
        raise AssertionError("audio parity failed")

    monkeypatch.setattr(e2e, "_assert_contract", fail)
    recorder = e2e_evidence.Evidence(
        tmp_path / "evidence", family="bark", case=name, source_revision="a" * 40, roots=(tmp_path,)
    )
    token = e2e_evidence._ACTIVE.set(recorder)
    try:
        with pytest.raises(AssertionError, match="audio parity failed"):
            e2e.test_official_checkpoint_e2e(name, tmp_path)
    finally:
        e2e_evidence._ACTIVE.reset(token)
    artifact = recorder.directory / recorder.data["native"]["audio"]["artifact"]
    assert artifact.read_bytes() == payload
    assert recorder.data["reference"]["samples"]["shape"] == [200]
