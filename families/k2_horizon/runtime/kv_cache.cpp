...
void validate_int32_scalar_input(ITrtModule& module, const std::string& name) {
...
void K2HorizonKvCache::validate_engine_contract(ITrtModule& module) const {
...
void K2HorizonKvCache::bind_cache_aliases(ITrtModule& module) {
    for (int32_t layer = 0; layer < num_layers_; ++layer) {
        const auto index = static_cast<std::size_t>(layer);
...
void K2HorizonKvCache::bind_to(ITrtModule& module) {
    if (!ok())
