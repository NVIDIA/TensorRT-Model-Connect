...
"""Test-only fixed-step debug runner for native K2-Horizon bundles."""
...
except ImportError:  # pragma: no cover - TensorRT-free source jobs
    trt = None
...
def _require_runtime() -> None:
    if trt is None:
        raise ImportError("tensorrt is required for the K2-Horizon debug runner")
    if cudart is None:
        raise ImportError("cuda-python is required for the K2-Horizon debug runner")
...
def _tensor_nbytes(engine, name: str) -> int:
    ...
    if any(value <= 0 for value in shape):
        raise ValueError(f"K2-Horizon debug tensor {name!r} must have a static shape")
...
def load_engine_from_bundle(
    ...
    with open(bundle_path, "rb") as handle:
        ...
        plan = handle.read(int(section["length"]))
    return plan, header
...
def load_section_from_bundle(bundle_path: str, section_name: str) -> bytes | None:
    with open(bundle_path, "rb") as handle:
        ...
        if not isinstance(section, dict):
            return None
        handle.seek(16 + header_length + int(section["offset"]))
        return handle.read(int(section["length"]))


def load_config_from_bundle(bundle_path: str) -> dict:
    data = load_section_from_bundle(bundle_path, "runtime.json")
    return json.loads(data.decode("utf-8")) if data is not None else {}


class K2HorizonTrtRunner:
    """One-token native-KV runner for the family-owned dense engine."""
    ...
    def __init__(
        ...
        self._device_logits: int | None = None
    ...
    def _initialize(
        ...
        max_cache_length: int,
        num_layers: int,
        distributed_communicator: object | None,
    ) -> None:
        _require_runtime()
        self.max_cache_length = int(max_cache_length)
        self.num_layers = int(num_layers)
        if self.max_cache_length <= 0:
            raise ValueError("K2-Horizon debug max_cache_length must be positive")

        logger = trt.Logger(trt.Logger.WARNING)
        runtime = trt.Runtime(logger)
        self.engine = runtime.deserialize_cuda_engine(engine_plan)
        if self.engine is None:
            raise RuntimeError("Failed to deserialize K2-Horizon TensorRT engine")
        self.context = self.engine.create_execution_context()
        if self.context is None:
            raise RuntimeError("Failed to create K2-Horizon TensorRT execution context")
        ...
        self._cache_bytes = _tensor_nbytes(self.engine, "cache_k_0") if self.num_layers else 0
        ...
        err, device_logits = cudart.cudaMalloc(self._host_logits.nbytes)
        _check_cuda(err)
        self._device_logits = device_logits

        state_prefixes = ("present_k_", "present_v_")
    ...
    def step(self, token_id: int) -> dict[str, np.ndarray]:
        if self.cache_length >= self.max_cache_length:
            raise RuntimeError("K2-Horizon sequence exceeds the fixed native KV capacity")
        ...
        if not self.context.execute_async_v3(self.stream):
            raise RuntimeError("K2-Horizon TensorRT execution failed")
...
def runner_from_bundle(
    *,
    ...
    if engine_plan is None:
        engine_plan, _header = load_engine_from_bundle(bundle_path)
    if config.get("model_type") != "k2_horizon":
        raise ValueError("K2-Horizon debug runner requires model_type='k2_horizon'")
    if config.get("native_kv_cache") is not True:
        raise ValueError("K2-Horizon debug runner requires native KV")
    return K2HorizonTrtRunner(
        engine_plan=engine_plan,
        max_cache_length=int(config.get("max_cache_length", 0)),
