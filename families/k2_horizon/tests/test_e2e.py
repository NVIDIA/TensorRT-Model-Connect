...
def _run_native(
    ...
    try:
        ...
        raise RuntimeError(f"K2-Horizon native generation timed out; full stderr: {log}") from error
    if tp_size == 1:
...
def _hf_reference(
    ...
    actual_ids: list[int],
    torch,
) -> tuple[list[int], str, float | None, str, np.ndarray]:
    ...
    with torch.inference_mode():
        if _is_sampling(case):
            ...
            reference_ids = generated.sequences[0, inputs["input_ids"].shape[1] :].tolist()
            reference_text = tokenizer.decode(reference_ids, skip_special_tokens=True).strip()
            reference_logits = np.stack(
                [score[0].detach().float().cpu().numpy() for score in generated.scores],
                axis=0,
            )
...
def _native_logits(
    ...
    from .debug_runner import load_config_from_bundle, load_engine_from_bundle
    from .debug_runner import runner_from_bundle
    ...
    encoded = _render_prompt(tokenizer, prompt, case)
    input_ids = [int(token) for token in encoded["input_ids"][0].tolist()]
    config = load_config_from_bundle(str(bundle))
    engine_plan, _header = load_engine_from_bundle(str(bundle))
    ...
    try:
        outputs = runner.generate(input_ids, int(case["max_new_tokens"]))
    finally:
        runner.close()
    start = max(len(input_ids) - 1, 0)
    selected = outputs[start : start + int(case["max_new_tokens"])]
    return np.stack(
        [np.asarray(output["logits"], dtype=np.float32).reshape(-1) for output in selected],
        axis=0,
    )
