...
"""Time VGG inference and host logits, not classification/reporting."""
...
def _summary(scores):
    ...
    if scores.ndim != 2 or scores.shape[0] != 1 or scores.shape[1] == 0:
        raise ValueError("VGG must return one nonempty class-score vector")
    if not np.isfinite(scores).all():
        raise ValueError("VGG returned nonfinite class scores")
    return {
...
def run(arguments):
    ...
    if (arguments.family != "timm_vgg" or arguments.operation != "classify"
            or arguments.selected_task != "image_to_class_scores"):
        raise ValueError("reference requires timm_vgg image_to_class_scores/classify")
