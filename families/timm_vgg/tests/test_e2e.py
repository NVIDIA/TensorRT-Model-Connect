...
"""Direct build, native-runtime, and official-reference E2E for timm_vgg."""
...
def _assert_sdk_consumers(
    ...
    for language in ("c", "cpp"):
        consumer = native_build / f"test_timm_vgg_sdk_{language}"
...
def test_official_checkpoint_e2e(case_name: str, tmp_path: Path) -> None:
    ...
    with evidence_stage("sdk"):
        _assert_sdk_consumers(runtime_root, bundle, case, config, int(np.argmax(expected["logits"])), tmp_path)
