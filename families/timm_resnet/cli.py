...
"""Family-owned timm_resnet commands with lazy builder imports."""
...
class BuildRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if str(self.precision).lower() not in {"fp16", "fp32"}:
            raise ValueError("timm_resnet supports only fp16 and fp32 precision")
...
def coerce_request(request: object) -> BuildRequest:
    ...
    if unknown := set(vars(request)) - allowed:
        raise ValueError(f"unknown timm_resnet build inputs: {sorted(unknown)}")
    return BuildRequest(
