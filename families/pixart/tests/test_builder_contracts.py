...
"""PixArt family-owned weight, component, and metadata contracts."""
...
pytest.importorskip("tensorrt", reason="TensorRT is required for PixArt builder tests")

from families.pixart import model as pixart_mod  # noqa: E402
from families.pixart.config import ModelConfig  # noqa: E402
...
def test_load_weights_success_and_missing_model_index(tmp_path) -> None:
    ...
    weights = pixart_mod._PixArtModel().load_weights(str(model_dir), _cfg())
    ...
    with pytest.raises(ValueError, match="Expected diffusers format"):
        pixart_mod._PixArtModel().load_weights(str(bad_dir), _cfg())
...
def test_build_components_uses_transformer_and_t5_configs(
    ...
    monkeypatch.setitem(
        sys.modules,
        "families.pixart.t5_encoder_builder",
        _module(
            "families.pixart.t5_encoder_builder",
    ...
    monkeypatch.setitem(
        sys.modules,
        "families.pixart.standard_dit_builder",
        _module(
            "families.pixart.standard_dit_builder",
    ...
    monkeypatch.setitem(
        sys.modules,
        "families.pixart.vae_2d_builder",
        _module(
            "families.pixart.vae_2d_builder",
    ...
    out = pixart_mod._PixArtModel().build_components(
        str(model_dir),
        config,
        weights,
        precision="fp16",
...
def test_get_diffusion_config_uses_transformer_overrides() -> None:
    ...
    dc = pixart_mod._PixArtModel().get_diffusion_config(cfg)
...
def test_load_pixart_dit_weights_maps_optional_biases(
    ...
    cm = importlib.import_module("families.pixart.checkpoint_mapper")
...
def test_serialize_preprocessor_weights_flattens_patch_conv() -> None:
    ...
    dit_weights = {
        ...
        "adaln_single.emb.timestep_embedder.linear_1.weight": np.arange(
            6, dtype=np.float32
        ).reshape(2, 3),
