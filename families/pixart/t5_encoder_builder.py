...
def build_t5_encoder_engine(
    ...
    *,
    ...
    if precision == "fp16":
        ...
        raise ValueError(f"Unsupported T5 precision {precision!r}; expected fp32 or fp16")
    ...
    for layer_idx in range(num_layers):
        ...
        normed = graph_ops.add_rms_norm(
            network, hidden, d_model, norm1_gamma, eps_t, dtype=work_np_dtype
        )
        ...
        q = graph_ops.add_matmul_rhs_constant(
            network, normed, d_model, attention_size, w_q, dtype=work_np_dtype
        )
        k = graph_ops.add_matmul_rhs_constant(
            network, normed, d_model, attention_size, w_k, dtype=work_np_dtype
        )
        v = graph_ops.add_matmul_rhs_constant(
            network, normed, d_model, attention_size, w_v, dtype=work_np_dtype
        )
        ...
        attn_out = graph_ops.add_matmul_rhs_constant(
            network, context_flat, attention_size, d_model, w_o, dtype=work_np_dtype
        )
        ...
        ffn_normed = graph_ops.add_rms_norm(
            network, hidden, d_model, norm2_gamma, eps_t, dtype=work_np_dtype
        )
        ...
        fc1 = graph_ops.add_matmul_rhs_constant(
            network, ffn_normed, d_model, d_ff, w_fc1, dtype=work_np_dtype
        )
        fc1_gate = graph_ops.add_matmul_rhs_constant(
            network, ffn_normed, d_model, d_ff, w_fc1_gate, dtype=work_np_dtype
        )
        ...
        ffn_out = graph_ops.add_matmul_rhs_constant(
            network, gated.get_output(0), d_ff, d_model, w_fc2, dtype=work_np_dtype
        )
