...
void load_preprocessor_weights(const nlohmann::json& index_json, const char* blob,
                               std::size_t blob_size, PixArtPreprocessorWeights& w) {
...
PixArtPreprocessorWeights parse_preprocessor_weights(const std::vector<char>& data) {
    PixArtPreprocessorWeights w;
    const char* blob = nullptr;
    std::size_t blob_size = 0;
    const auto index_json =
        pixart_preprocessor_weights::extract_preprocessor_index(data, blob, blob_size);
    load_preprocessor_weights(index_json, blob, blob_size, w);
    finalize_preprocessor_weights(w);

    std::cerr << "[pixart] Preprocessor weights loaded: " << (w.valid ? "OK" : "INCOMPLETE")
...
PixArtDiffusionConfig make_diffusion_config(const std::string& json) {
    ...
    dc.tokenizer_special_suffix_ids = optional_int_array(document, "tokenizer_special_suffix_ids");
    dc.pos_embed_base_size = require_int(document, "pos_embed_base_size");
    dc.pos_embed_interpolation_scale = require_number(document, "pos_embed_interpolation_scale");
    return dc;
}
