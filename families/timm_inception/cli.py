...
"""Family-owned timm_inception commands; importing this module does not load TensorRT."""
...
class BuildRequest:
    ...
    def __post_init__(self) -> None:
        if self.task != "image_to_class_scores":
            raise ValueError("timm_inception supports only task=image_to_class_scores")
        if str(self.precision).lower() not in {"fp16", "fp32"}:
            raise ValueError("timm_inception supports only fp16 and fp32 precision")
...
def coerce_request(request: object) -> BuildRequest:
    ...
    if unknown := set(vars(request)) - allowed:
        raise ValueError(f"unknown timm_inception build inputs: {sorted(unknown)}")
