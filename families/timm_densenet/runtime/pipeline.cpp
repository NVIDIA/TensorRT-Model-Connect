...
const Tensor& require_logits(const TensorMap& outputs) {
    ...
    if (found->second.data == nullptr || found->second.dtype != DType::kFloat32 ||
        ...
        throw std::runtime_error("timm DenseNet engine must return nonempty float32 logits");
    return found->second;
}
...
TimmDensenetImageClassificationPipeline::TimmDensenetImageClassificationPipeline(
    ...
    if (num_classes_ <= 0 ||
        ...
        throw std::runtime_error("timm DenseNet class metadata does not match its output size");
}

internal::LabelScoresResult
TimmDensenetImageClassificationPipeline::run(const internal::ImageToClassScoresRequest& request,
                                             internal::ConfigView config) {
    if (!config.empty())
        throw internal::ConfigError("timm DenseNet has no runtime configuration");
    ...
    if (image.format != internal::ImageFormat::Float32 || image.channels != 3 ||
        ...
        throw std::invalid_argument("timm DenseNet requires contiguous float32 RGB input");
    auto pixel_values = preprocess_timm_densenet_image(
    ...
    if (n != static_cast<std::size_t>(num_classes_))
        throw std::runtime_error("timm DenseNet logits do not match its configured class count");
