...
"""Tests for the timm DenseNet image-classification family model."""
...
try:
    ...
    from families.timm_densenet.config import ModelConfig
    from families.timm_densenet.model import _TimmDensenetModel
    from families.timm_densenet.support import describe
...
def test_support_rejects_unrelated_model_types(model_type: str) -> None:
    metadata = ModelMetadata(config={"model_type": model_type}, model_index={})
...
def _write_tiny_densenet(tmp_path: Path, *, block_layers: tuple[int, ...] = (2, 3, 2, 2)) -> None:
...
def test_layout_counts_layers_per_dense_block(tmp_path: Path):
    ...
    model.load_weights(str(tmp_path), cfg)
