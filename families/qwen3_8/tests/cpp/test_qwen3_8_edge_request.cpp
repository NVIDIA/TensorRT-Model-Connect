...
int main() {
    ...
    const auto request = trtmc::qwen3_8::edge_llm::make_request("Hello", config, 128);
    ...
    const auto raw = trtmc::qwen3_8::edge_llm::make_request("raw", config, 32);
    if (raw.maxGenerateLength != 32 || raw.temperature != 0 || raw.applyChatTemplate)
        return 1;
    config.seed = 42;
    try {
        trtmc::qwen3_8::edge_llm::make_request("unsupported", config, 32);
        return 1;
