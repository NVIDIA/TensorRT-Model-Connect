...
bool is_eos_token(const std::vector<int32_t>& eos_token_ids, int32_t token_id) {
    return std::find(eos_token_ids.begin(), eos_token_ids.end(), token_id) != eos_token_ids.end();
}

Qwen38SampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                      const std::vector<int32_t>& eos_token_ids) {
    Qwen38SampleResult result;
    ...
    result.logprob = *best;
    result.is_eos = is_eos_token(eos_token_ids, result.token_id);
    return result;
}
...
bool greedy_equivalent(const Qwen38SamplingParams& params) {
...
class Qwen38GreedySampler final : public Qwen38ISampler {
  public:
    Qwen38SampleResult sample(const float* logits, int32_t vocab_size,
                              const Qwen38SamplingParams& params) override {
        if (vocab_size <= 0 || logits == nullptr) {
            Qwen38SampleResult result;
            result.token_id = 0;
            result.is_eos = is_eos_token(params.eos_token_ids, 0);
            return result;
        }
        return argmax_over_logits(logits, vocab_size, params.eos_token_ids);
    }

    Qwen38LogitsLocation logits_location() const override { return Qwen38LogitsLocation::HOST; }
    const char* sampler_type() const override { return "qwen38_greedy"; }
};

class Qwen38TopKSampler final : public Qwen38ISampler {
  public:
    explicit Qwen38TopKSampler(uint64_t initial_seed)
    ...
    Qwen38SampleResult sample(const float* logits, int32_t vocab_size,
        ...
        for (int32_t i = 0; i < dist.keep; ++i) {
            ...
            if (u < cumulative) {
                ...
                result.is_eos = is_eos_token(params.eos_token_ids, result.token_id);
                return result;
            }
        }
    ...
    Qwen38LogitsLocation logits_location() const override { return Qwen38LogitsLocation::HOST; }
    const char* sampler_type() const override { return "qwen38_top_k"; }
...
Qwen38SamplingParams qwen38_sampling_params_from_config(const TextGenerationConfig& cfg,
                                                        const std::vector<int32_t>& default_eos) {
    Qwen38SamplingParams p;
    ...
    p.seed = cfg.seed;
    p.eos_token_ids = cfg.eos_token_id >= 0 ? std::vector<int32_t>{cfg.eos_token_id} : default_eos;
    return p;
}
...
std::unique_ptr<Qwen38ISampler> create_qwen38_sampler(const Qwen38SamplingParams& params) {
    ...
    if (params.top_k <= 1 && top_p >= 1.0F - kSamplingEpsilon && min_p <= 0.0F && params.seed < 0) {
        return std::make_unique<Qwen38GreedySampler>();
    }

    const uint64_t seed = (params.seed >= 0) ? static_cast<uint64_t>(params.seed) : 42ULL;
    return std::make_unique<Qwen38TopKSampler>(seed);
}
