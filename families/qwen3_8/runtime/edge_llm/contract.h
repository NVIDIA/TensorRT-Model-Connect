...
/// DSpark proposal length excludes the one additional verification token.
inline bool valid_execution_variant(const std::string& variant, int block_size) {
    return variant == "dspark" && block_size == 7;
}
