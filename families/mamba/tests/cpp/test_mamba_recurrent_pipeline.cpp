...
// Intent:         RecurrentPipeline with Mamba and RWKV recurrent state
// Preconditions:  TRT + CUDA GPU available, mock engines
// Postconditions: Pipeline generates tokens with correct recurrent state management
// =============================================================================

// =============================================================================
// Test suite: RecurrentPipeline (Mamba, RWKV)
// =============================================================================
//
// Tests the RecurrentPipeline with mock engines and recurrent state.
// =============================================================================
...
static void test_mamba_pipeline() {
    ...
    std::vector<trtmc::MambaRecurrentState::TensorSpec> specs = {
        {"conv_state", {12}},
        {"ssm_state", {32}},
    };
    auto rs = std::make_unique<trtmc::MambaRecurrentState>(1, specs, stream);
    ...
    trtmc::RecurrentPipeline pipeline(std::move(module), std::move(rs), cfg, stream,
                                      "MambaPipeline");
    check(std::string(pipeline.task()) == trtmc::ITextGeneration::kTask, "mamba pipeline task");
...
static void test_rwkv_pipeline() {
    ...
    auto rs = std::make_unique<trtmc::MambaRecurrentState>(2, specs, stream);
    ...
    trtmc::RecurrentPipeline pipeline(std::move(module), std::move(rs), cfg, stream,
                                      "RwkvPipeline");
    check(std::string(pipeline.task()) == trtmc::ITextGeneration::kTask, "rwkv pipeline task");
...
static void test_argmax_recurrent() {
    ...
    trtmc::MambaSamplingParams params;
    auto sampler = trtmc::create_mamba_sampler(params);
    check(sampler->sample(v.data(), static_cast<int32_t>(v.size()), params).token_id == 1,
          "argmax = 1");
}
