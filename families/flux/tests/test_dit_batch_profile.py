...
try:
    from families.flux import flux_dit_builder
except (ImportError, ModuleNotFoundError):
    pytest.skip("Flux builder tests require TensorRT", allow_module_level=True)
...
def _install(monkeypatch):
    ...
    from families.flux import parallel as eb
...
def test_fp16_precision_casts_inputs_and_matmul_constants(monkeypatch):
    ...
    try:
        ...
        assert casts == [(flux_dit_builder.trt.float32, flux_dit_builder.trt.float16)]
...
def test_fp16_adaln_keeps_modulation_in_fp32(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _network, shape, _values, dtype=np.float32: _FakeTensor(
            "constant",
            (flux_dit_builder.trt.float16 if dtype == np.float16 else flux_dit_builder.trt.float32),
            shape,
        ),
    )
...
def test_fp16_block_updates_accumulate_in_fp32_residual():
    ...
    class _TypedNetwork:
        ...
        def add_elementwise(self, left, right, operation):
            ...
            return _RecordingLayer(_FakeTensor("residual_sum", left.dtype, left.shape))
    ...
    try:
        ...
        assert sums == [(fp32, fp32, flux_dit_builder.trt.ElementWiseOperation.SUM)]
    finally:
        flux_dit_builder._configure_compute_precision("fp32")
...
def test_fp16_block_gate_runs_before_overflow_in_fp32():
    ...
    class _TypedNetwork:
        ...
        def add_elementwise(self, left, right, operation):
            ...
            return _RecordingLayer(_FakeTensor("gated_update", left.dtype, left.shape))
    ...
    try:
        ...
        assert products == [(fp32, fp32, flux_dit_builder.trt.ElementWiseOperation.PROD)]
    finally:
        flux_dit_builder._configure_compute_precision("fp32")
