...
def _import_with_fake_trt(module_name: str, fake_trt: types.SimpleNamespace):
    """Import one Flux family module while TensorRT is mocked."""
    for mod_name in (
        module_name,
        "families.flux.graph_ops",
        "families.flux.graph_blocks",
    ):
...
def _make_clip_weights(hidden: int, intermediate: int, vocab: int, seq_len: int, num_layers: int):
    w: dict[str, np.ndarray] = {
        ...
        "text_model.embeddings.position_embedding.weight": np.zeros(
            (seq_len, hidden), dtype=np.float32
        ),
...
def _make_t5_weights(
    ...
    for i in range(num_layers):
        p = f"encoder.block.{i}"
        for proj in ("q", "k", "v", "o"):
            w[f"{p}.layer.0.SelfAttention.{proj}.weight"] = np.zeros(
                (d_model, attn), dtype=np.float32
            )
...
@pytest.mark.unit
def test_build_clip_encoder_engine_success_uses_fake_builder_and_marks_outputs(
    ...
    mod = _import_with_fake_trt("families.flux.clip_encoder_builder", fake_trt)
...
@pytest.mark.unit
def test_build_clip_encoder_engine_raises_when_builder_returns_none(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_load_clip_weights_transposes_projection_matrices_and_keeps_biases() -> None:
    ...
    tensors: dict[str, np.ndarray] = {
        "text_model.embeddings.token_embedding.weight": np.arange(32, dtype=np.float32).reshape(
            8, 4
        ),
        "text_model.embeddings.position_embedding.weight": np.arange(12, dtype=np.float32).reshape(
            3, 4
        ),
    ...
    for layer in range(2):
        ...
        for proj in ("q_proj", "k_proj", "v_proj", "out_proj"):
            tensors[f"{p}.self_attn.{proj}.weight"] = np.arange(16, dtype=np.float32).reshape(
                4, 4
            ) + (10 * layer)
    ...
    cm = importlib.import_module("families.flux.checkpoint_mapper")

    with (
        patch.object(cm, "_open_safetensors", lambda _path: tensors),
        patch.object(cm, "_load_tensor", lambda readers, name: readers[name]),
    ):
...
@pytest.mark.unit
def test_build_t5_encoder_engine_success_exercises_relative_bias_fallback(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    """
    fake_trt = _make_fake_trt()
    mod = _import_with_fake_trt("families.flux.t5_encoder_builder", fake_trt)
    ...
    monkeypatch.setattr(mod.graph_ops, "add_constant", _fake_add_constant)
    monkeypatch.setattr(
        mod.graph_ops, "make_t5_relative_position_bias", lambda *_a, **_k: bucket_indices
    )
...
@pytest.mark.unit
def test_build_t5_encoder_engine_raises_when_builder_returns_none(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        mod.graph_ops,
        "make_t5_relative_position_bias",
        lambda *_a, **_k: np.zeros((2, 2), dtype=np.int32),
    )
    monkeypatch.setattr(mod.graph_ops, "add_rms_norm", _fake_tensor_fn("rms"))
    monkeypatch.setattr(mod.graph_ops, "add_matmul_rhs_constant", _fake_tensor_fn("mm"))
    monkeypatch.setattr(mod.graph_ops, "add_gelu_new", _fake_tensor_fn("gelu"))
    ...
    with pytest.raises(RuntimeError, match="T5 encoder"):
        mod.build_t5_encoder_engine(
            _make_t5_weights(
                d_model=4, num_heads=2, d_ff=6, num_layers=1, vocab_size=8, num_buckets=4
            ),
            d_model=4,
            num_heads=2,
            d_kv=2,
...
def test_load_t5_weights_transposes_and_bias_fallback() -> None:
    ...
    checkpoint_mapper = importlib.import_module("families.flux.checkpoint_mapper")

    def load(precision: str = "fp32"):
        with (
            patch.object(checkpoint_mapper, "_open_safetensors", lambda _path: tensors),
            patch.object(checkpoint_mapper, "_load_tensor", lambda readers, name: readers[name]),
            patch.object(checkpoint_mapper, "_has_tensor", lambda readers, name: name in readers),
            patch.object(
                checkpoint_mapper,
                "_target_np_dtype",
    ...
    assert fp16_weights["encoder.block.0.layer.0.SelfAttention.q.weight"].dtype == np.float16
