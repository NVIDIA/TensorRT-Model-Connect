...
def _assert_parity(actual, expected, manifest: dict, case: dict, thresholds: dict) -> None:
    ...
    if task == "image_generation_batch":
        assert len(actual_images) == int(case["inputs"]["expected_batch_size"])
        pairwise_mae = float(np.mean(np.abs(actual_images[0] - actual_images[1])))
        assert pairwise_mae >= float(thresholds["batch_min_pairwise_pixel_mae"])
