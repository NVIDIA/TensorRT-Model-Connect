...
"""Gemma checkpoint-gated activation contracts."""
...
def test_gemma_model_forwards_checkpoint_activation(monkeypatch) -> None:
    module = importlib.import_module("families.gemma.model")
    ...
    result = module._GemmaModel().build_engine(
        _config(),
    ...
    def add_activation(self, tensor, activation):
        ...
        if activation == self._trt.ActivationType.GELU_TANH:
            result = (
                ...
                * (
                    1.0
                    + np.tanh(math.sqrt(2.0 / math.pi) * (tensor + 0.044715 * np.power(tensor, 3)))
                )
            )
...
def test_gemma_dual_profile_mlp_uses_checkpoint_gelu(monkeypatch) -> None:
    module = importlib.import_module("families.gemma.dual_profile_decoder_builder")
    ...
    expected = (
        0.5
        * gate
        * (1.0 + np.tanh(math.sqrt(2.0 / math.pi) * (gate + 0.044715 * np.power(gate, 3))))
        * up
    )
...
def test_gemma_rejects_missing_checkpoint_activation() -> None:
    ...
    with pytest.raises(
        ...
        module._GemmaModel().build_engine(config, {}, 23)
