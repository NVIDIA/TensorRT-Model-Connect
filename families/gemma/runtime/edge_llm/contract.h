...
inline void validate_generation(const TextGenerationConfig& c, bool dspark = false) {
    ...
    if (c.min_p != 0 || c.seed != -1 || c.eos_token_id != -1 || c.repetition_penalty != 1 ||
        ...
        throw std::invalid_argument("Requested generation controls are unsupported by Gemma4 Edge");
}
...
inline void validate_capacity(int prompt_tokens, int input_limit, int capacity,
    ...
    if (prompt_tokens <= 0 || prompt_tokens > input_limit || generated_tokens <= 0 ||
        ...
        throw std::invalid_argument("Gemma4 Edge prompt and generation exceed bundle capacity");
}
