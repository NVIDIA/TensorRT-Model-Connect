...
"""gemma build inputs and strict compatibility for existing Python callers."""
...
class BuildRequest:
    """gemma-owned inputs; unsupported legacy controls are read-only defaults."""
...
def coerce_request(request: object) -> BuildRequest:
    ...
    if unknown := set(vars(request)) - names - set(unsupported):
        raise ValueError(f"unknown gemma build inputs: {sorted(unknown)}")
