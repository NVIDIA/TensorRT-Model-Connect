...
#include "families/gpt_neox/runtime/pipeline.h"

#include "families/gpt_neox/runtime/chat_templates.h"
#include "families/gpt_neox/runtime/kv_cache.h"
#include "families/gpt_neox/runtime/tensor_names.h"
...
GptNeoxTextGenerationPipeline::GptNeoxTextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<GptNeoxInferenceState> state,
    GptNeoxTextGenConfig config, std::shared_ptr<ITokenizer> tokenizer,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("GptNeoxTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("GptNeoxTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
void require_prefill_kv_pointers(ITrtModule& prefill, const GptNeoxTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(gpt_neox_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(gpt_neox_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "GptNeoxTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const GptNeoxTextGenConfig& cfg,
                                      int32_t sq, GptNeoxInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("GptNeoxTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "GptNeoxTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("GptNeoxTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "GptNeoxTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<GptNeoxKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("GptNeoxTextGenerationPipeline: prefill requires GptNeoxKvCache");
}
...
std::string
GptNeoxTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
GptNeoxTextGenerationPipeline::TimedGenResult GptNeoxTextGenerationPipeline::generate_from_ids(
    ...
    const GptNeoxSamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("GptNeoxTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_gpt_neox_sampler(params);
...
int32_t GptNeoxTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const GptNeoxSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& GptNeoxTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
