...
"""gpt_neox command handlers and model-owned build inputs."""
...
class BuildRequest:
    ...
    def __post_init__(self) -> None:
        if self.task not in {"text_generation"}:
            raise ValueError("unsupported gpt_neox task")
        if str(self.precision).lower() not in {"fp16", "bf16", "fp32"}:
            raise ValueError("unsupported gpt_neox precision")
...
def coerce_request(request: object) -> BuildRequest:
    ...
    if unknown := set(vars(request)) - set(fields) - legacy:
        raise ValueError(f"unknown gpt_neox build inputs: {sorted(unknown)}")
