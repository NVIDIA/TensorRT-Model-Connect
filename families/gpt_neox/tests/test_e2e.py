...
from families.gpt_neox.cli import BuildRequest, build_bundle
...
def test_config_preserves_zero_token_ids() -> None:
    from families.gpt_neox.config import ModelConfig

    config = ModelConfig.from_json('{"bos_token_id":0,"eos_token_id":0,"pad_token_id":null}')

    assert (config.bos_token_id, config.eos_token_id, config.pad_token_id) == (0, 0, -1)
...
def _text_threshold(thresholds: dict[str, float]) -> float:
    _evidence_threshold = float(thresholds.get("contract_ned_threshold", 0.25))
...
def _assert_correctness(
    ...
    token_threshold = thresholds.get("contract_token_agreement_rate")
    if token_threshold is not None:
        assert reference_ids
        common = min(len(actual_ids), len(reference_ids))
        agreement = sum(actual_ids[index] == reference_ids[index] for index in range(common)) / max(
            len(actual_ids), len(reference_ids)
        )
        assert agreement >= float(token_threshold)
        if float(token_threshold) == 1.0:
