...
def _linear(network, hidden, weights, name: str):
    ...
    if prescale is not None:
        ...
        prescale = op.cast(network, prescale, hidden.dtype)
...
def build_multimodal_text_encoder_engine(
    ...
    *,
    ...
    unexpected = sorted(set(weights) - expected_keys - optional_keys)
    ...
    for name in optional_keys & set(weights):
        ...
        if value.shape != (width,) or not np.isfinite(value).all():
            raise ValueError(f"Invalid MiniMax-H3 AWQ input scale: {name}")
