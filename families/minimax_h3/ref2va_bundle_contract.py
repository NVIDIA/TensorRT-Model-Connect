...
from .quantized_checkpoint import QuantizedCheckpointIdentity, QUANTIZED_REF2VA_CHECKPOINT_IDENTITY
...
def ref2va_bundle_metadata(
    transformer_ref: TransformerRefIdentity | QuantizedCheckpointIdentity,
    profile: Ref2VADenoiserProfile = Ref2VADenoiserProfile(),
    *,
    first_block_cache: bool = False,
    ...
    if isinstance(transformer_ref, QuantizedCheckpointIdentity):
        if transformer_ref.bundle_metadata() != QUANTIZED_REF2VA_CHECKPOINT_IDENTITY.bundle_metadata():
            raise ValueError("MiniMax-H3 Ref2VA requires the distinct full Comfy INT8 checkpoint")
    elif not isinstance(transformer_ref, TransformerRefIdentity):
