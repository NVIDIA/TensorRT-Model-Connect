...
void test_pipeline_destruction_flushes_runtime_cache() {
    ...
    trtmc::MiniMaxH3ModuleLoader unused_loader =
        [](const std::string&, cudaStream_t, const std::vector<trtmc::ModuleExternalBinding>&,
...
trtmc::MiniMaxH3ModuleLoader make_unused_pipeline_loader() {
    return [](const std::string&, cudaStream_t, const std::vector<trtmc::ModuleExternalBinding>&,
              int32_t) -> std::unique_ptr<trtmc::ITrtModule> {
        throw std::runtime_error("test loader must not be invoked");
    };
}
...
void test_pipeline_rejects_initial_latents_before_plan_loading() {
    ...
    try {
        ...
        rejected =
            std::string(error.what()).find("does not accept initial_latents") != std::string::npos;
    }
    if (!rejected)
