...
void test_denoiser_optimization_profile_selection() {
    ...
    check(trtmc::select_minimax_h3_denoiser_profile(2, 537, five_seconds) == 0,
          "H3 exact reference five-second request selects the static profile");
    ...
    const auto two_keyframe_max_canvas = trtmc::make_minimax_h3_fl2va_geometry(max_canvas, 2);
    check(trtmc::select_minimax_h3_denoiser_profile(3, 2641, two_keyframe_max_canvas) == 1,
