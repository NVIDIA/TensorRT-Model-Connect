...
void test_denoiser_profile_selection_keeps_public_fallback() {
    ...
    require(select_ref2va_denoiser_profile(2, 14985, 414, 1) == 0,
            "Ref2VA compact five-second audio-only target missed the supported profile");
    ...
    require(rejects([] { (void)select_ref2va_denoiser_profile(0, 28224, 754, 2571); }) &&
                ...
                rejects([] { (void)select_ref2va_denoiser_profile(2, 14984, 754, 2571); }),
...
void test_strict_plan_abi_and_fake_end_to_end() {
    for (const auto kind : {trtmc::minimax_h3::Ref2vaPlanKind::kDenoiser,
                            trtmc::minimax_h3::Ref2vaPlanKind::kDenoiserHead,
                            trtmc::minimax_h3::Ref2vaPlanKind::kDenoiserTail,
                            trtmc::minimax_h3::Ref2vaPlanKind::kDenoiserFinish}) {
        auto compact = kind == trtmc::minimax_h3::Ref2vaPlanKind::kDenoiser
                           ? make_denoiser_module() : make_cached_denoiser_module(kind);
        for (auto& entry : compact.tensors) {
            auto& minimum = entry.second.minimum;
            if (!minimum.empty() && minimum[0] == 18870) minimum[0] = 14985;
            if (!minimum.empty() && minimum[0] == 19285) minimum[0] = 15400;
        }
        trtmc::minimax_h3::validate_ref2va_plan(compact, kind);
        const char* packed = kind == trtmc::minimax_h3::Ref2vaPlanKind::kDenoiser
                                 ? "adaln_indices"
                                 : kind == trtmc::minimax_h3::Ref2vaPlanKind::kDenoiserHead
                                       ? "previous_head_residual" : "head_hidden";
        compact.tensors.at(packed).minimum[0] = 15401;
        require(rejects([&] { trtmc::minimax_h3::validate_ref2va_plan(compact, kind); }),
                "Ref2VA compact plan accepted an inconsistent packed minimum");
    }
