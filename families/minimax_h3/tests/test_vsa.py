...
def test_vsa_geometry_matches_the_fasth3_64_token_contract() -> None:
    ...
    assert geometry.num_video_tiles == 660
    assert geometry.num_tiles == 676
    assert geometry.topk_video_tiles == 66
    assert geometry.padded_rows == 676 * 64
...
def test_vsa_video_tiles_follow_fastvideo_t_h_w_order() -> None:
    ...
    expected = [
        video_start + t * 24 * 42 + h * 42 + w
        for t in range(4)
        for h in range(4)
        for w in range(4)
    ]
    assert geometry.gather_indices[first_video_slot : first_video_slot + 64].tolist() == expected
    # The final video tile covers 1x4x2 because T=37, H=24, W=42.
    assert geometry.variable_block_sizes[-1] == 8
...
def test_vsa_workspace_covers_fp32_routing_compression_and_block_map() -> None:
    ...
    assert vsa_workspace_bytes(geometry, profile) == 263_023_488
