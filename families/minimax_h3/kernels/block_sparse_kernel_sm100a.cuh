...
__device__ __forceinline__ void tcgen05_mma_ws_f16_ss_1sm_lead(uint32_t lead, uint32_t tmem_d,
    ...
    asm volatile("{\n\t"
                 ...
                 "}\n" ::"r"(lead),
                 "r"(tmem_d), "l"(desc_a), "l"(desc_b), "r"(enable_input_d ? 1u : 0u), "r"(idesc));
}
__device__ __forceinline__ void tcgen05_mma_ws_f16_ts_1sm_lead(uint32_t lead, uint32_t tmem_d,
    ...
    asm volatile("{\n\t"
                 ...
                 "@q tcgen05.mma.ws.cta_group::1.kind::f16 [%1], [%2], %3, %5, p, 0;\n\t"
                 "}\n" ::"r"(lead),
                 "r"(tmem_d), "r"(tmem_a), "l"(desc_b), "r"(enable_input_d ? 1u : 0u), "r"(idesc));
}
...
__device__ __forceinline__ WorkItem decode_workitem(int workitem_id, int num_heads, int num_blocks,
                                                    int packed_mtiles_per_seq,
                                                    unsigned long long magic0,
                                                    unsigned long long magic1,
                                                    unsigned long long magic2, const int* q2k_num) {
    WorkItem it;
...
template <int S_LD_COLS = 32, bool FULL_NAMED_BAR = false, bool EX2_EMU = false,
          bool SPLIT_P = false, bool SOFTMAX_THROTTLE = false, bool USE_CLC = true,
          bool Q_RASTER = true, bool MHA = true, int RESCALE_THRESHOLD = 8,

          bool BHSD = false>
__global__ void __cluster_dims__(1, 1, 1) __launch_bounds__(N_WARPS * 32, 1)
    fmha_context_bf16_gen_kernel(
        const __grid_constant__ CUtensorMap tmap_q, const __grid_constant__ CUtensorMap tmap_k,
        const __grid_constant__ CUtensorMap tmap_v_t, const __grid_constant__ CUtensorMap tmap_v,
        const __grid_constant__ CUtensorMap tmap_o, int seqlen, int num_heads, float scale_log2,
        int num_samples, int num_blocks, int packed_mtiles_per_seq, int max_kv,
        ...
        const int* __restrict__ variable_block_sizes, float* __restrict__ lse_out) {
...
#pragma unroll
    ...
    if (warp_id == W_LOAD) {
        ...
        while (true) {
            const WorkItem it =
                decode_workitem<Q_RASTER>(workitem_id, num_heads, num_blocks, packed_mtiles_per_seq,
            ...
            auto load_k_oneslot = [&](int mtile_idx, int ktile_idx, int s) {
                ...
                mbarrier_wait_parity_suspend(smem_ptr_u32(&empty_bar[kv_stage]),
                                             kv_empty_ph.get_phase());
                kv_empty_ph.advance();
...
#pragma unroll
                for (int blk = 0; blk < BLOCKS_PER_KTILE; ++blk)
                    kv_tok[blk] =
                        k_start +
                        get_kv_block_id(mtile_idx, ktile_idx * BLOCKS_PER_KTILE + blk) * BLOCK;
                if (elect_one_sync()) {
                    mbarrier_arrive_expect_tx(smem_ptr_u32(&full_bar[kv_stage]),
                                              KV_RING_SLOT_BYTES);
                    if constexpr (BLK128) {

                        if constexpr (BHSD)
                            tma_load_4d(smem_ptr_u32((sKV + kv_stage * KV_RING_SLOT_BYTES)),
                                        &tmap_k, smem_ptr_u32(&full_bar[kv_stage]), 0,
                                        kv_tok[0] - k_start, 0, it.sample * num_heads + it.head);
                        else
                            tma_load_3d(smem_ptr_u32((sKV + kv_stage * KV_RING_SLOT_BYTES)),
                                        &tmap_k, smem_ptr_u32(&full_bar[kv_stage]), 0, kv_tok[0],
                                        it.head * K_SUBTILES);
                    } else {
#pragma unroll
                        for (int blk = 0; blk < BLOCKS_PER_KTILE; ++blk)
                            if constexpr (BHSD)
                                tma_load_4d(smem_ptr_u32((sKV + kv_stage * KV_RING_SLOT_BYTES) +
                                            ...
                                            &tmap_k, smem_ptr_u32(&full_bar[kv_stage]), 0,
                                            kv_tok[blk] - k_start, s,
                                            it.sample * num_heads + it.head);
                            else
                                tma_load_3d(smem_ptr_u32((sKV + kv_stage * KV_RING_SLOT_BYTES) +
                                                         blk * BLOCK * SUB_COLS_BYTES),
                                            &tmap_k, smem_ptr_u32(&full_bar[kv_stage]), 0,
                                            kv_tok[blk], it.head * K_SUBTILES + s);
                    }
                }
            };
            auto load_v_oneslot = [&](int mtile_idx, int ktile_idx, int p) {
                ...
                if constexpr (BLK128) {
                    ...
                    if (elect_one_sync()) {
                        ...
                        if constexpr (BHSD)
                            tma_load_4d(smem_ptr_u32((sKV + kv_stage * KV_RING_SLOT_BYTES)),
                                        &tmap_v, smem_ptr_u32(&full_bar[kv_stage]), 0,
                                        kv_tok0 - k_start, 0, it.sample * num_heads + it.head);
                        else
                            tma_load_3d(smem_ptr_u32((sKV + kv_stage * KV_RING_SLOT_BYTES)),
                                        &tmap_v, smem_ptr_u32(&full_bar[kv_stage]), 0, kv_tok0,
                                        it.head * K_SUBTILES);
                    }
                } else {
                    int kv_tok[2];
#pragma unroll
                    for (int h = 0; h < 2; ++h)
                        kv_tok[h] =
                            k_start +
                            get_kv_block_id(mtile_idx, ktile_idx * BLOCKS_PER_KTILE + 2 * h + p) *
                                BLOCK;
                    if (elect_one_sync()) {
                        mbarrier_arrive_expect_tx(smem_ptr_u32(&full_bar[kv_stage]),
                                                  KV_RING_SLOT_BYTES);
...
#pragma unroll
                            for (int s2 = 0; s2 < K_SUBTILES; ++s2) {
                                ...
                                if constexpr (BHSD)
                                    tma_load_4d(dst, &tmap_v, smem_ptr_u32(&full_bar[kv_stage]), 0,
                                                kv_tok[h] - k_start, s2,
                                                it.sample * num_heads + it.head);
                                else
                                    tma_load_3d(dst, &tmap_v, smem_ptr_u32(&full_bar[kv_stage]), 0,
                                                kv_tok[h], it.head * K_SUBTILES + s2);
                            }
                        }
                    }
                }
...
#pragma unroll
            for (int m = 0; m < M_TILES_PER_CTA; ++m) {
                ...
                if (elect_one_sync()) {
                    ...
                    tma_load_4d(smem_ptr_u32(sQ[m]), &tmap_q, smem_ptr_u32(&full_bar_q[m]), 0,
            ...
            if constexpr (USE_CLC) {
                ...
                if (workitem_id >= total_workitems)
                    break;
            }
        }
        ...
        const uint32_t idesc_pv =
            make_idesc_bf16_f32(M_TILE, BLK128 ? HEAD_DIM : 2 * HEAD_DIM, false, true);
...
#pragma unroll
                    for (int ki = 0; ki < K_ATOMS_PER_TILE; ++ki) {
                        ...
                        if constexpr (BLK128)
                            tcgen05_mma_f16_ss_lead(lead, s_tmem_addr, da + 2 * ki, db + 2 * ki,
                                                    idesc_qk, enable_d);
                        else
                            tcgen05_mma_ws_f16_ss_1sm_lead(lead, s_tmem_addr, da + 2 * ki,
                                                           db + 2 * ki, idesc_qk, enable_d);
                    }
...
#pragma unroll
                    for (int ki = 0; ki < K_ATOMS_PER_TILE; ++ki) {
                        ...
                        if constexpr (SPLIT_P)
                            if (a == SPLIT_P_ATOM) {
                                mbarrier_wait_parity(smem_ptr_u32(&full_bar_p_last[i]),
                                                     spo_ph.get_phase());
                            }
                        const bool accumulate = (!first_ktile) || (a != 0);
                        if constexpr (BLK128) {
                            tcgen05_mma_f16_ts_1sm_lead(lead, o_tmem_addr,
                                                        s_tmem_addr + (uint32_t)(a * 8),
                                                        desc_bv.u64, idesc_pv, accumulate);
                            desc_add_lo(desc_bv, PV_DESC_STEP);
                        } else {
                            tcgen05_mma_ws_f16_ts_1sm_lead(lead, o_tmem_addr,
                                                           s_tmem_addr + (uint32_t)(a * 8),
                                                           desc_bv.u64, idesc_pv, accumulate);
                            desc_add_lo(desc_bv, PV_DESC_STEP);
                        }
...
#pragma unroll
            for (int m = 0; m < M_TILES_PER_CTA; ++m) {
                mbarrier_wait_parity_suspend(smem_ptr_u32(&full_bar_o_epi[m]),
                                             full_o_ph.get_phase());
...
#pragma unroll
            for (int i = 0; i < M_TILES_PER_CTA; ++i) {
                if constexpr (FULL_NAMED_BAR)
                    ...
                    mbarrier_wait_parity_suspend(smem_ptr_u32(&full_bar_alpha[i]),
                                                 alpha_ph.get_phase());
                mbarrier_arrive(smem_ptr_u32(&empty_bar_alpha_and_l[i]));
            }
            if constexpr (!FULL_NAMED_BAR)
...
#pragma unroll
                for (int i = 0; i < M_TILES_PER_CTA; ++i) {
                    ...
                    if (!skip) {
                        ...
                        const uint32_t o_tmem_addr = tmem_base +
                                                     (uint32_t)(2 * S_COLS + i * O_COLS) +
...
#pragma unroll
            for (int i = 0; i < M_TILES_PER_CTA; ++i) {
                mbarrier_wait_parity_suspend(smem_ptr_u32(&full_bar_o_acc[i]),
                                             o_acc_ph.get_phase());
                if constexpr (FULL_NAMED_BAR)
                    full_bar_wait(i, corr_warp_id);
                else
                    mbarrier_wait_parity_suspend(smem_ptr_u32(&full_bar_l[i]),
                                                 o_acc_ph.get_phase());
                ...
                if constexpr (BLK128) {
                    ...
                    const float l_par =
                        alpha_and_l_smem[(i * STAT_REGIONS + 1) * STATS + (corr_tid ^ 64)];
                    const float m_par =
                        alpha_and_l_smem[(i * STAT_REGIONS + 2) * STATS + (corr_tid ^ 64)];
                    mbarrier_arrive(smem_ptr_u32(&empty_bar_alpha_and_l[i]));
                    ...
                    if (lse_out != nullptr && (corr_tid >> 6) == 0) {
                        ...
                        const int q_row =
                            (i == 0 ? it.mtile0 : it.mtile1) * BLOCK + (corr_tid & 63);
...
#pragma unroll
                        for (int vv = 0; vv < 2; ++vv) {
                            ...
                            *reinterpret_cast<uint4*>(
                                &so_sub[corr_row * SUB_COLS_BF16 + (v ^ (corr_row & 7)) * 8]) =
                                packed;
                        }
                    }
                } else {
...
#pragma unroll
                            for (int vv = 0; vv < 2; ++vv) {
                                ...
                                o2[vv * 4 + 0] = fadd2(
                                    o2[vv * 4 + 0],
                                    __bfloat1622float2(*reinterpret_cast<__nv_bfloat162*>(&h1.x)));
                                o2[vv * 4 + 1] = fadd2(
                                    o2[vv * 4 + 1],
                                    __bfloat1622float2(*reinterpret_cast<__nv_bfloat162*>(&h1.y)));
                                o2[vv * 4 + 2] = fadd2(
                                    o2[vv * 4 + 2],
                                    __bfloat1622float2(*reinterpret_cast<__nv_bfloat162*>(&h1.z)));
                                o2[vv * 4 + 3] = fadd2(
                                    o2[vv * 4 + 3],
                                    __bfloat1622float2(*reinterpret_cast<__nv_bfloat162*>(&h1.w)));
                                uint4 packed;
                                packed.x = cvt_f32x2_to_bf16x2(o2[vv * 4 + 0].x, o2[vv * 4 + 0].y);
                                packed.y = cvt_f32x2_to_bf16x2(o2[vv * 4 + 1].x, o2[vv * 4 + 1].y);
                                packed.z = cvt_f32x2_to_bf16x2(o2[vv * 4 + 2].x, o2[vv * 4 + 2].y);
        ...
        const uint32_t alpha_slot_u32 =
            smem_ptr_u32(&alpha_and_l_smem[(m_tile * STAT_REGIONS + 0) * STATS + sm_tid]);
        ...
        while (true) {
            ...
            auto get_vbs_thresholds = [&](int k, int gqb_mt, int half, int& t0, int& t1) {
                if (k >= thr_window + 32) {
                    ...
                    if constexpr (BLK128) {
                        thr_cache0 =
                            (kk < cnt) ? variable_block_sizes[q2k_idx[gqb_mt * max_kv + kk]] : 0;
                    } else {
                        const int b0 = kk * BLOCKS_PER_KTILE + 2 * half;
                        thr_cache0 =
                            (b0 < cnt) ? variable_block_sizes[q2k_idx[gqb_mt * max_kv + b0]] : 0;
                        thr_cache1 = (b0 + 1 < cnt)
                                         ? variable_block_sizes[q2k_idx[gqb_mt * max_kv + b0 + 1]]
                                         : 0;
                    }
                }
            ...
            auto softmax_step = [&](auto is_first_c, int k) {
                ...
                mbarrier_wait_parity_suspend(smem_ptr_u32(&full_bar_spo[m_tile]),
                                             spo_ph.get_phase());
...
#pragma unroll
                for (int c0 = 0; c0 < S_COLS; c0 += S_LD_COLS) {
                    ...
                    if constexpr (S_LD_COLS == 32)
                        tcgen05_ld_32x32b_x32(taddr,
                                              *reinterpret_cast<uint32_t (*)[32]>(&s_regs[c0]));
                    else if constexpr (S_LD_COLS == 64)
                        tcgen05_ld_32x32b_x64(taddr,
                                              *reinterpret_cast<uint32_t (*)[64]>(&s_regs[c0]));
                    else if constexpr (S_LD_COLS == 128)
                        tcgen05_ld_32x32b_x128(taddr,
                                               *reinterpret_cast<uint32_t (*)[128]>(&s_regs[c0]));
                }
                ...
                if constexpr (BLK128) {
                    if (vbs_thr0_ < S_COLS)
                        mask_s_row_r2p<false, S_COLS>(scores, 0, 0, vbs_thr0_);
                } else {
                    if (vbs_thr0_ < 64)
                        mask_s_row_r2p<false, 64>(scores, 0, 0, vbs_thr0_);
                    if (vbs_thr1_ < 64)
                        mask_s_row_r2p<false, 64>(scores + 64, 0, 0, vbs_thr1_);
                }
...
#pragma unroll
                ...
                [[maybe_unused]] float2 lt2_live =
                    make_float2(IS_FIRST ? 0.0f : l_run * alpha, 0.0f);
#pragma unroll
                for (int c = 0; c < S_COLS / 2; ++c) {
                    ...
                    if constexpr (EX2_EMU) {
                        ...
                        const bool use_hw =
                            (kk % EX2_FREQ < EX2_FREQ - EX2_RES) || (jj >= EX2_FRG_CNT - 1);
                        scores2[c] = use_hw
                                         ? make_float2(ex2_approx_f32(a2.x), ex2_approx_f32(a2.y))
                                         : ex2_emu_f32x2(a2.x, a2.y);
                    } else {
                        scores2[c] = make_float2(ex2_approx_f32(a2.x), ex2_approx_f32(a2.y));
                    }
                    if constexpr (!DEFER_ROWSUM)
                        lt2_live = fadd2(lt2_live, scores2[c]);
                    p_regs[c] = cvt_f32x2_to_bf16x2(scores2[c].x, scores2[c].y);
                }
                const uint32_t p_tmem_addr = s_tmem_addr;

                if constexpr (SPLIT_P) {
                    tcgen05_st_32x32b_x32(p_tmem_addr,
                                          *reinterpret_cast<uint32_t (*)[32]>(&p_regs[0]));
                    tcgen05_st_32x32b_x16(p_tmem_addr + 32,
                                          *reinterpret_cast<uint32_t (*)[16]>(&p_regs[32]));
                    tcgen05_wait_st();
                    tcgen05_fence_before_thread_sync();
                    mbarrier_arrive(smem_ptr_u32(&empty_bar_spo[m_tile]));
                    tcgen05_st_32x32b_x16(
                        p_tmem_addr + SPLIT_P_COL,
                    ...
                    tcgen05_st_32x32b_x32(p_tmem_addr,
                                          *reinterpret_cast<uint32_t (*)[32]>(&p_regs[0]));
                    tcgen05_st_32x32b_x32(p_tmem_addr + 32,
                                          *reinterpret_cast<uint32_t (*)[32]>(&p_regs[32]));
                    tcgen05_wait_st();
                    tcgen05_fence_before_thread_sync();
                    mbarrier_arrive(smem_ptr_u32(&empty_bar_spo[m_tile]));
                }
