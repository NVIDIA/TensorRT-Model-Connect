...
constexpr int32_t kFlFiveSecondVideoRows = kMaxFiveSecondTargetVideoRows + kMaxConditionVideoRows;
...
int32_t select_minimax_h3_denoiser_profile(int32_t optimization_profile_count, int32_t text_rows,
                                           const MiniMaxH3Geometry& geometry) {
    ...
    const bool specialized_five_second_request =
        ...
        geometry.video_rows == kFiveSecondVideoRows && geometry.audio_rows == kFiveSecondAudioRows;
    if (specialized_five_second_request)
        return 0;
    if (optimization_profile_count == 2)
        return 1;
    const bool five_second_fl_profile = geometry.output_frames == kDefaultOutputFrames &&
                                        geometry.audio_rows == kFiveSecondAudioRows &&
                                        geometry.video_rows >= kMinVideoRows &&
                                        geometry.video_rows <= kFlFiveSecondVideoRows &&
                                        text_rows >= kMinTextRows && text_rows <= kMaxTextRows;
...
struct MiniMaxH3Pipeline::ResidentState {
    ...
    bool prepare_denoiser(const MiniMaxH3ModuleLoader& loader, cudaStream_t stream,
                          int32_t optimization_profile_count, const MiniMaxH3Geometry& geometry);
    DenoiserStats
...
void MiniMaxH3Pipeline::ResidentState::load_first_block_cache_denoiser(
    ...
    const int64_t profile_sequence_rows =
        exact_five_second_profile
            ? kFiveSecondPackedRows
            : (fl_five_second_profile ? kFlFiveSecondPackedRows : kMaxSequenceRows);
    const int64_t profile_video_rows =
        exact_five_second_profile
            ? kFiveSecondVideoRows
            : (fl_five_second_profile ? kFlFiveSecondVideoRows : kMaxVideoRows);
    const int64_t profile_audio_rows = (exact_five_second_profile || fl_five_second_profile)
                                           ? kFiveSecondAudioRows
                                           : kMaxAudioRows;
...
VideoResult MiniMaxH3Pipeline::generate_video_request_impl(const VideoGenerationRequest& request,
    ...
    try {
        ...
        ImageGenerationConfig resolved_config = request.config;
        ...
        const ImageGenerationConfig& cfg = resolved_config;
