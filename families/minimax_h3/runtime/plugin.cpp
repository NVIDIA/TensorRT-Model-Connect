...
minimax_h3::SuperResolutionConfig load_super_resolution_config(const nlohmann::json& config) {
    ...
    result.enabled = true;
    result.mode = found->value("mode", std::string{});
    if (result.mode != "explicit")
        throw std::runtime_error(
            "MiniMax-H3 legacy or unsupported SR mode; rebuild with super_resolution=true");
    if (found->value("source_shape", nlohmann::json{}) != nlohmann::json::array({480, 864}) ||
        found->value("target_shape", nlohmann::json{}) != nlohmann::json::array({720, 1296}) ||
        found->value("batch_profile", nlohmann::json{}) != nlohmann::json::array({1, 4, 8}))
        throw std::runtime_error("MiniMax-H3 SR metadata has an incompatible shape or batch profile");
...
MiniMaxH3DenoiserConfig load_denoiser_config(const nlohmann::json& config) {
    ...
    result.optimization_profile_layout = parse_minimax_h3_denoiser_profile_layout(
        config.value("denoiser_profile_layout", std::string{}), result.optimization_profile_count);
    return result;
}
