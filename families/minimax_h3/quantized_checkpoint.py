...
"""Strict loader for the public full MiniMax-H3 INT8 ConvRot transformers.
...
T2VA/FL2VA and Ref2VA share this tensor layout but use distinct checkpoint
values. The explicit workflow selects the pinned source; it never substitutes
the FL2VA checkpoint for Ref2VA or accepts the pruned AdaLN variant.
...
file size is checked without rereading the 34 GB payload before every build.
"""
...
REF2VA_CHECKPOINT_FILENAME = "diffusion_models/minimax_h3_ref2va_int8_convrot.safetensors"
REF2VA_CHECKPOINT_BYTES = 34_038_894_550
...
QUANTIZED_REF2VA_CHECKPOINT_IDENTITY = replace(
    QUANTIZED_CHECKPOINT_IDENTITY,
    filename=REF2VA_CHECKPOINT_FILENAME,
    size_bytes=REF2VA_CHECKPOINT_BYTES,
)


def _checkpoint_identity(workflow: str) -> QuantizedCheckpointIdentity:
    if workflow == "fl2va":
        return QUANTIZED_CHECKPOINT_IDENTITY
    if workflow == "ref2va":
        return QUANTIZED_REF2VA_CHECKPOINT_IDENTITY
    raise ValueError("MiniMax-H3 quantized checkpoint workflow must be 'fl2va' or 'ref2va'")
...
def _physical_contract() -> tuple[dict[str, _TensorSpec], dict[str, int]]:
    ...
    if len(specs) != 1_035 or len(groups) != 250:
        raise RuntimeError("MiniMax-H3 quantized checkpoint contract is incomplete")
    return specs, groups
...
def _authenticate_quantized_source(path: Path, *, workflow: str = "fl2va") -> None:
    """Validate the pinned HF cache/local-dir layout without rereading 34 GB."""

    identity = _checkpoint_identity(workflow)
    ...
    if source_root.parent.name == "snapshots":
        ...
        if source_root.name != identity.revision or repository_root.name != _HF_CACHE_REPOSITORY:
            raise ValueError(
    ...
    try:
        ...
        raise ValueError(
            "Cannot validate the MiniMax-H3 quantized checkpoint source. "
            "Download it with `hf download Comfy-Org/MiniMax-H3 --revision "
            f'{identity.revision} --include "{identity.filename}" --local-dir <directory>` '
            "so Hugging Face writes pinned download metadata."
        ) from error
    if not metadata or metadata[0] != identity.revision:
        raise ValueError(
...
def validate_quantized_transformer_checkpoint(
    ...
    *,
    ...
    """Validate the pinned full INT8 ConvRot checkpoint for the selected workflow.
    ...
    embedded quantization marker are required.
    """
    ...
    expected_name = Path(identity.filename).name
    ...
    if size != identity.size_bytes:
        raise ValueError(
            ...
            f"expected={identity.size_bytes}, actual={size}"
        )
    source_identity = _source_file_identity(path)
    _authenticate_quantized_source(path, workflow=workflow)
...
def _make_quantized_weight(
    ...
    *,
    ...
    if transform == "swap_swiglu":
        ...
        raise RuntimeError(f"MiniMax-H3 quantized transform is invalid: {transform}")
...
def _make_qkv_values(
    ...
    if group_size is None:
        raise RuntimeError("MiniMax-H3 QKV quantization group is missing")
    ...
    *,
    ...
    for logical_name in requested:
        ...
        if base in _QUANT_GROUPS:
            ...
            raise RuntimeError(f"MiniMax-H3 logical transform is invalid: {transform}")
    return result
