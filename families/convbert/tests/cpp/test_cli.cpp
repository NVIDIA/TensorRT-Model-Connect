...
class FakeConvconvbert final : public trtmc::IEncoding,
...
void convbert_contract(const fs::path& root) {
    const auto path = root / "convbert.bundle";
    write_bundle(path, "convbert");
    ...
    check(encoded.first == 0 && Json::parse(encoded.second.output).at("values") == Json({3.0, 1.0}),
          "CONVBERT owner converts the encoding request and preserves result fields");
    check(loaded_runtime == "/installed/runtime",
          "CONVBERT owner uses the supplied installed runtime root");
    ...
    check(embedded.first == 0 &&
          ...
          "CONVBERT owner selects the embedding interface");
    check(loaded_runtime == "/override/runtime", "runtime_root override remains owned by CONVBERT");
...
std::unique_ptr<ITask> load_task(const BundleReader&, const std::string& runtime_root,
    ...
    return std::make_unique<FakeConvconvbert>();
}
} // namespace trtmc

int main() {
    const auto root =
        fs::temp_directory_path() / ("trtmc-convbert-cli-" + std::to_string(getpid()));
    fs::create_directories(root);
    try {
        convbert_contract(root);
