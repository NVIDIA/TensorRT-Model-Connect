...
@pytest.mark.gpu
@pytest.mark.parametrize("precision", ["fp32", "fp16", "bf16"])
def test_prefill_uses_sanitized_current_kv_and_persists_prefix(precision):
    trt = pytest.importorskip("tensorrt")
    torch = pytest.importorskip("torch")
    if not torch.cuda.is_available():
        pytest.skip("native prefill graph execution requires a CUDA device")
    ...
    write_indices = network.add_input("write_indices", trt.int32, (-1,))
    active = network.add_input("active_mask", trt.bool, (-1, 1, 1, capacity))
    mask = network.add_input("attention_mask", trt.bool, (-1, 1, -1, -1))
    update_shape = network.add_shape(k_update).get_output(0)
    length = network.add_slice(update_shape, (2,), (1,), (1,)).get_output(0)
    key, value = add_cached_kv(
        trt, network, k_update, v_update, capacity, write_indices, active, "0", weights,
        attention_length=length, attention_from_updates=True,
    )
    # This path deliberately exercises native padded writes, not packed row selection.
    updates = [network.get_layer(index) for index in range(network.num_layers)
               if network.get_layer(index).name in {"cache.0.k.update", "cache.0.v.update"}]
    assert len(updates) == 2
    for layer in updates:
        assert layer.type == trt.LayerType.KV_CACHE_UPDATE
        # get_layer returns ILayer; expose the verified native layer's typed API.
        layer.__class__ = trt.IKVCacheUpdateLayer
        assert layer.update_form == trt.AttentionIOForm.PADDED_BHND
    for name, tensor in (("attention_k", key), ("attention_v", value)):
        tensor.name = name
    ...
    scores = network.add_elementwise(
        scores, network.add_cast(mask, dtype).get_output(0), trt.ElementWiseOperation.PROD,
    ).get_output(0)
    hstu = network.add_matrix_multiply(
        scores, trt.MatrixOperation.NONE, value, trt.MatrixOperation.NONE,
    ).get_output(0)
    ...
    profile = builder.create_optimization_profile()
    for name in ("query", "k_update", "v_update"):
        profile.set_shape(name, (1, heads, 1, width), (2, heads, 4, width), (2, heads, 4, width))
    profile.set_shape("write_indices", (1,), (2,), (2,))
    profile.set_shape("active_mask", (1, 1, 1, capacity), (2, 1, 1, capacity), (2, 1, 1, capacity))
    profile.set_shape("attention_mask", (1, 1, 1, 1), (2, 1, 4, 4), (2, 1, 4, 4))
    for name in ("cache_0_k", "cache_0_v"):
        profile.set_shape(name, (1, heads, capacity, width), (2, heads, capacity, width), (2, heads, capacity, width))
    config = builder.create_builder_config()
    config.builder_optimization_level = 0
    ...
    config.add_optimization_profile(profile)
    serialized = builder.build_serialized_network(network, config)
    assert serialized is not None, "native direct-current-KV prefill graph must build"
    runtime = trt.Runtime(logger)
    engine = runtime.deserialize_cuda_engine(serialized)
    assert engine is not None
    context = engine.create_execution_context()
    ...
    generator = torch.Generator(device="cuda").manual_seed(937)

    def random(shape):
        return torch.randn(shape, generator=generator, device="cuda", dtype=torch_dtype) / 4

    # A finite sentinel distinguishes untouched storage from both valid writes
    # and NaN padding written by PADDED_BHND. Only the valid prefix is persistent.
    caches = [torch.full((2, heads, capacity, width), -7.0, device="cuda", dtype=torch_dtype)
              for _ in range(2)]
    expected_caches = [cache.clone() for cache in caches]
    tolerances = {"fp32": (1e-5, 1e-6), "fp16": (5e-3, 1e-3), "bf16": (3e-2, 5e-3)}
    rtol, atol = tolerances[precision]

    for lengths_list in ([4, 2], [1, 3]):
        query_count = max(lengths_list)
        q = random((2, heads, query_count, width))
        new_k, new_v = [random((2, heads, query_count, width)) for _ in range(2)]
        for updates in (new_k, new_v):
            for sample, valid_length in enumerate(lengths_list):
                updates[sample, :, valid_length:] = float("nan")
        lengths = torch.tensor(lengths_list, device="cuda", dtype=torch.int32)
        active_rows = torch.arange(capacity, device="cuda")[None, :] < lengths[:, None]
        valid_rows = active_rows[:, :query_count]
        positions = torch.arange(query_count, device="cuda")
        allowed = positions[None, None, None, :] <= positions[None, None, :, None]
        allowed = allowed & valid_rows[:, None, None, :] & valid_rows[:, None, :, None]
        inputs = {
            "query": q, "k_update": new_k, "v_update": new_v,
            "write_indices": torch.zeros((2,), device="cuda", dtype=torch.int32),
            "active_mask": active_rows[:, None, None, :].contiguous(),
            "attention_mask": allowed.contiguous(),
            "cache_0_k": caches[0], "cache_0_v": caches[1],
        }
        # Padded writes replace all query_count rows, including poisoned padding;
        # attention must use a separately sanitized view of the current tensors.
        for cache, update in zip(expected_caches, (new_k, new_v)):
            cache[:, :, :query_count] = update
        for name, tensor in inputs.items():
            assert context.set_input_shape(name, tuple(tensor.shape))
        ...
        assert context.set_tensor_address("present_0_v", caches[1].data_ptr())
        outputs = {}
        for name in ("attention_k", "attention_v", "sdpa", "hstu"):
            outputs[name] = torch.empty(tuple(context.get_tensor_shape(name)), device="cuda", dtype=torch_dtype)
            assert context.set_tensor_address(name, outputs[name].data_ptr())
        assert context.execute_async_v3(torch.cuda.current_stream().cuda_stream)
        torch.cuda.synchronize()
        for actual, expected, update in zip(caches, expected_caches, (new_k, new_v)):
            torch.testing.assert_close(actual, expected, rtol=0, atol=0, equal_nan=True)
            for sample, valid_length in enumerate(lengths_list):
                torch.testing.assert_close(actual[sample, :, :valid_length],
                                           update[sample, :, :valid_length], rtol=0, atol=0)
        safe_k, safe_v = [torch.where(valid_rows[:, None, :, None], update, 0)
                          for update in (new_k, new_v)]
        torch.testing.assert_close(outputs["attention_k"], safe_k, rtol=0, atol=0)
        torch.testing.assert_close(outputs["attention_v"], safe_v, rtol=0, atol=0)
        expected_sdpa = torch.nn.functional.scaled_dot_product_attention(
            q, safe_k, safe_v, attn_mask=allowed, scale=0.5,
        )
        expected_hstu = (torch.nn.functional.silu(q @ safe_k.transpose(-1, -2)) * allowed) @ safe_v
        for name in ("sdpa", "hstu"):
