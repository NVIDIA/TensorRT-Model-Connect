...
void validate_dense_config(const RuntimeConfig& config) {
    if (config.enable_history_cache || config.mode != "ranking" || !config.is_causal ||
        config.target_group_size != 1 || config.scaling_seqlen != 1024 || config.time_buckets != 0)
        throw std::invalid_argument(
            "hstu native dense attention requires uncached causal ranking/group1/scale1024");
    for (const auto& table : config.embedding_tables)
        if (table.role == "context")
            throw std::invalid_argument(
                "hstu native dense attention does not admit context tables");
}

void validate_dense_attention(ITrtModule& engine, const RuntimeConfig& config) {
    validate_dense_config(config);
    validate_tensor(engine, "attention_metadata", true, DType::kInt32, 3, 8);
    if (engine.tensor_shape("attention_metadata")[0] != 5 || config.max_sequence_length > 1024 ||
        engine.has_input("cache_0_pages") || engine.has_input("page_update_rows"))
        throw std::invalid_argument("hstu uncached dense attention input contract mismatch");
}

void validate_mask_attention(ITrtModule& engine) {
    const auto* mask_name = attention_input_name(engine);
    const auto mask_dtype = engine.tensor_dtype(mask_name);
    if (prepared_attention(engine) && mask_dtype != DType::kFloat32 &&
        mask_dtype != DType::kFloat16 && mask_dtype != DType::kBFloat16)
        throw std::invalid_argument("hstu attention weights require FP32, FP16, or BF16");
    validate_tensor(engine, mask_name, true,
                    prepared_attention(engine) ? mask_dtype : DType::kFloat32, 4);
    if (engine.has_input("scaling_seqlen") || !prepared_attention(engine))
        validate_tensor(engine, "scaling_seqlen", true, DType::kFloat32, 1);
}
...
struct Inputs {
    ...
    AttentionMask mask;
    std::vector<std::int32_t> attention_metadata;
    float scaling{1.0F};
};
...
AttentionMask prepare_mask(const ITrtModule& engine, std::size_t batch, std::size_t length,
                           float scaling, bool dense) {
    if (dense)
        return {};
    const bool weighted = prepared_attention(engine);
    return make_attention_mask(batch, length, attention_key_width(engine, length),
                               engine.tensor_dtype(attention_input_name(engine)),
                               weighted ? 1.0F / scaling : 1.0F,
                               engine.has_input("attention_weights_transposed"));
}
...
Inputs prepare_inputs(const RecommendationRequest& request, const std::vector<Sequence>& sequences,
                      ...
                      const ITrtModule& engine) {
    const auto batch = sequences.size();
    Inputs inputs;
    const bool dense = engine.has_input("attention_metadata");
    if (dense)
        inputs.attention_metadata = make_dense_attention_metadata(sequences, length);
    ...
    inputs.mask = prepare_mask(engine, batch, length, inputs.scaling, dense);
    for (std::size_t sample = 0; sample < batch; ++sample) {
        ...
        if (!dense)
            fill_attention_mask(inputs.mask, sample, sequence, config);
    }
    return inputs;
}
...
TensorMap input_tensors(Inputs& data, const RuntimeConfig& config, std::int64_t batch,
                        std::int64_t length, std::int64_t candidates, const ITrtModule& engine) {
    TensorMap inputs = {
        {"token_ids", {data.tokens.data(), {batch, length}, DType::kInt32}},
    };
    if (engine.has_input("attention_metadata"))
        inputs["attention_metadata"] = {
            data.attention_metadata.data(), {5, batch + 1, 8}, DType::kInt32};
    else
        inputs[attention_input_name(engine)] = data.mask.tensor();
    if (engine.has_input("scaling_seqlen"))
        inputs["scaling_seqlen"] = {&data.scaling, {1}, DType::kFloat32};
    if (config.position_buckets > 0)
...
Pipeline::Pipeline(std::unique_ptr<ITrtModule> engine, RuntimeConfig config)
    ...
    if (engine_->has_input("attention_metadata")) {
        validate_dense_attention(*engine_, config_);
    } else {
        validate_mask_attention(*engine_);
    }
...
RecommendationResult Pipeline::recommend(const RecommendationRequest& request) {
    ...
    auto data = prepare_inputs(request, sequences, config_, length, candidates, *engine_);
    ...
    auto inputs =
        input_tensors(data, config_, batch, width, static_cast<std::int64_t>(candidates), *engine_);
    ...
    const auto* logits = config_.mode == "ranking"
                             ? output_data(outputs, "logits", batch, logit_rows, config_.output_dim)
                             : nullptr;
    const auto* items =
    ...
    for (std::size_t sample = 0; sample < sequences.size(); ++sample) {
        ...
        result.sequences.push_back(collect(
            ...
            logits == nullptr ? nullptr : logits + sample * logit_rows * config_.output_dim,
