...
TimmRes2NetPreprocessConfig parse_config(const nlohmann::json& json) {
    ...
    if (config.input_image_h <= 0 || config.input_image_w <= 0 || config.crop_pct <= 0.0F ||
        ...
        throw std::runtime_error("timm Res2Net runtime.json does not match its runtime contract");
    }
    return config;
}
...
extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    ...
    auto config = trtmc::timm_res2net::parse_config(metadata);
    auto engine = trtmc::timm_res2net::load_engine(context.backend, plan);
    return new trtmc::TimmRes2NetImageClassificationPipeline(
