...
TimmRes2NetPreprocessConfig parse_config(const std::vector<char>& data) {
    const auto json = nlohmann::json::parse(data.begin(), data.end());
    TimmRes2NetPreprocessConfig config;
    ...
    if (config.input_image_h <= 0 || config.input_image_w <= 0 || config.crop_pct <= 0.0F ||
        ...
        throw std::runtime_error("timm Res2Net runtime.json does not match its contract");
    }
    return config;
}
...
std::unique_ptr<ITrtModule> load_engine(IBackend& backend, const std::vector<char>& plan) {
    ...
    if (!engine || !engine->ok())
        throw std::runtime_error("timm Res2Net engine failed to load");
    return engine;
}
...
TRTMC_DEFINE_FAMILY_PLUGIN_V1("timm_res2net")

extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    if (context.kv_cache_size_bytes != 0)
        throw std::invalid_argument("timm_res2net does not support --kv-cache-size");
    const auto config_data = trtmc::timm_res2net::require_section(context.reader, "runtime.json");
    const auto plan = trtmc::timm_res2net::require_section(context.reader, "engine.plan");
    auto config = trtmc::timm_res2net::parse_config(config_data);
    auto engine = trtmc::timm_res2net::load_engine(context.backend, plan);
    return new trtmc::TimmRes2NetImageClassificationPipeline(std::move(engine), std::move(config));
}
