...
stbir_filter resize_filter(const std::string& interpolation) {
    ...
    throw std::invalid_argument("Unsupported timm Res2Net interpolation: " + interpolation);
}

void validate_config(const TimmRes2NetPreprocessConfig& config) {
    if (config.input_image_h <= 0 || config.input_image_w <= 0)
        throw std::invalid_argument("timm Res2Net input dimensions must be positive");
    if (config.crop_pct <= 0.0F || config.crop_pct > 1.0F)
        throw std::invalid_argument("timm Res2Net crop_pct must be in (0, 1]");
    if (config.image_mean.size() != 3 || config.image_std.size() != 3)
        throw std::invalid_argument("timm Res2Net image mean/std must contain three channels");
    for (const float value : config.image_std) {
        if (value == 0.0F)
            throw std::invalid_argument("timm Res2Net image std must be non-zero");
    }
}
...
TimmRes2NetResizeShape
compute_timm_res2net_resize_shape(int32_t image_height, int32_t image_width,
                                  const TimmRes2NetPreprocessConfig& config) {
    if (image_height <= 0 || image_width <= 0)
        throw std::invalid_argument("timm Res2Net source dimensions must be positive");
...
std::vector<float> preprocess_timm_res2net_image(const float* image_pixels, int32_t image_height,
    ...
    if (image_pixels == nullptr || image_height <= 0 || image_width <= 0)
        throw std::invalid_argument("timm Res2Net source image must be non-empty");
    validate_config(config);
    const auto shape = compute_timm_res2net_resize_shape(image_height, image_width, config);
    ...
    if (stbir_resize(
        ...
        throw std::runtime_error("Failed to resize timm Res2Net input image");
    }
