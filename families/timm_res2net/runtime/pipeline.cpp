...
const Tensor& require_logits(const TensorMap& outputs) {
    for (const auto& [name, tensor] : outputs) {
        ...
        if (tensor.data == nullptr || tensor.dtype != DType::kFloat32 || tensor.numel() <= 0)
            throw std::runtime_error("timm Res2Net engine must return nonempty float32 logits");
        return tensor;
    }
    throw std::runtime_error("timm Res2Net engine did not return logits");
}

} // namespace

TimmRes2NetImageClassificationPipeline::TimmRes2NetImageClassificationPipeline(
    ...
    if (!model_ || !model_->ok())
        throw std::runtime_error("TimmRes2NetImageClassificationPipeline: invalid model");
    if (num_classes_ <= 0 ||
...
TimmRes2NetImageClassificationPipeline::run(const internal::ImageToClassScoresRequest& request,
                                            internal::ConfigView config) {
    if (!config.empty())
        throw internal::ConfigError("timm Res2Net has no runtime configuration");
    ...
    if (image.format != internal::ImageFormat::Float32 || image.channels != 3 ||
        ...
        throw std::invalid_argument("timm Res2Net requires contiguous float32 RGB input");
    auto pixel_values = preprocess_timm_res2net_image(
    ...
    if (n != static_cast<std::size_t>(num_classes_))
        throw std::runtime_error("timm Res2Net logits do not match its configured class count");
