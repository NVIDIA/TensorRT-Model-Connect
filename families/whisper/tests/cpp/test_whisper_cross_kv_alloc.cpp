...
// Exercises the ownership the Whisper pipeline uses for its cross-attention K/V
// buffers, against CPU CUDA stubs so each allocation can be failed in turn
// without a GPU. WhisperPipeline itself needs live TensorRT modules to build, so
...
// The allocation loop from WhisperPipeline's constructor, over the same type.
void allocate_cross_kv(std::vector<trtmc::whisper::DeviceBuffer>& keys,
                       std::vector<trtmc::whisper::DeviceBuffer>& values, int32_t layers,
                       std::size_t bytes) {
...
int main() {
    ...
    std::printf("whisper cross-attention allocation-failure checks passed\n");
    return 0;
}
