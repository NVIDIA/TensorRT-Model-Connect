...
def _logits_trace_binary() -> tuple[Path, Path]:
    ...
    assert value, "selected Qwen MoE logits trace requires TRTMC_NATIVE_BUILD_DIR"
    build_dir = Path(value)
    assert build_dir.is_dir(), "selected Qwen MoE logits trace requires TRTMC_NATIVE_BUILD_DIR"
    subprocess.run(
        [
            "cmake",
            "--build",
            ...
            "8",
            "--target",
            "qwen_moe_logits_trace",
            "trtmc_backend_trt",
        ],
        check=True,
        timeout=600,
    )
    binary = build_dir / "families" / _FAMILY / "qwen_moe_logits_trace"
    assert binary.is_file()
    assert (build_dir / "libtrtmc_backend_trt.so").is_file()
    return binary, build_dir


def _native_logits_trace(bundle: Path, prompt: str, case: dict, tmp_path: Path) -> np.ndarray:
    binary, runtime_root = _logits_trace_binary()
    prefix = tmp_path / "qwen-moe-logits"
    environment = dict(os.environ)
...
def _hf_reference(
    ...
    actual_ids: list[int],
    torch,
) -> tuple[list[int], str, float | None, str, np.ndarray | None]:
...
def test_e2e(case_name: str, request, tmp_path: Path) -> None:
    ...
    if case_name in _LOGIT_ORACLES:
        with evidence_stage("native"):
            payload["logits_trace"] = _native_logits_trace(bundle, prompt, case, tmp_path)
        record_evidence("native", payload)
