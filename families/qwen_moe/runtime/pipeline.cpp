...
#include "families/qwen_moe/runtime/kv_cache.h"
#include "families/qwen_moe/runtime/tensor_names.h"
...
QwenMoeTextGenerationPipeline::QwenMoeTextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<QwenMoeInferenceState> state,
    QwenMoeTextGenConfig config, std::shared_ptr<ITokenizer> tokenizer,
    ...
    if (prefill_ && !prefill_->ok())
        throw std::runtime_error("QwenMoeTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("QwenMoeTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
void require_prefill_kv_pointers(ITrtModule& prefill, const QwenMoeTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(qwen_moe_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(qwen_moe_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "QwenMoeTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const QwenMoeTextGenConfig& cfg,
                                      int32_t sq, QwenMoeInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("QwenMoeTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "QwenMoeTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("QwenMoeTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "QwenMoeTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<QwenMoeKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("QwenMoeTextGenerationPipeline: prefill requires QwenMoeKvCache");
}

} // namespace

void QwenMoeTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error(
            "QwenMoeTextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(lt.numel()) < vocab)
        throw std::runtime_error(
            "QwenMoeTextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
QwenMoeTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
QwenMoeTextGenerationPipeline::TimedGenResult QwenMoeTextGenerationPipeline::generate_from_ids(
    ...
    const QwenMoeSamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("QwenMoeTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_qwen_moe_sampler(params);
...
int32_t QwenMoeTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const QwenMoeSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& QwenMoeTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
...
QwenMoeTextGenerationPipeline::LogitsTrace
QwenMoeTextGenerationPipeline::trace_logits(const std::string& prompt,
                                            const TextGenerationConfig& cfg) {
    const auto input_ids = encode_prompt(*tokenizer_, config_, prompt, cfg);
    if (input_ids.empty())
        throw std::invalid_argument("Qwen MoE logits trace requires a non-empty prompt");
    const int32_t max_new_tokens = std::max(cfg.max_new_tokens, 0);
    if (input_ids.size() + static_cast<std::size_t>(max_new_tokens) >
        static_cast<std::size_t>(state_->max_length())) {
        throw std::invalid_argument("Qwen MoE logits trace exceeds its fixed cache capacity");
    }
    const auto params = qwen_moe_sampling_params_from_config(cfg, config_.id_eos);
    auto sampler = create_qwen_moe_sampler(params);
    sampler->reset();
    reset_generation_context();
    state_->set_prompt_length(static_cast<int32_t>(input_ids.size()));
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        ...
        if (result.is_eos || result.token_id == params.eos_token_id || step + 1 >= max_new_tokens) {
            break;
        }
        run_step(result.token_id, logits);
        trace.rows.push_back(logits);
