...
#include "families/timm_dpn/runtime/pipeline.h"
...
trtmc::TimmDpnPreprocessConfig preprocessing() {
...
void test_binding_and_complete_owned_logits() {
    trtmc::internal::LabelScoresResult result;
    {
        ...
        trtmc::TimmDpnImageClassificationPipeline model(std::move(module), preprocessing(), 5, "",
                                                        {});
...
void test_class_identity_metadata() {
    ...
    for (const auto& labels : {partial, std::vector<std::string>(5)}) {
        rejects<std::runtime_error>(
            [&] {
                trtmc::TimmDpnImageClassificationPipeline model(std::make_unique<RecordingModule>(),
                                                                preprocessing(), 5, "", labels);
            },
