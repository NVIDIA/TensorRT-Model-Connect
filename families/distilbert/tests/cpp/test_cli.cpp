...
class FakeDistildistilbert final : public trtmc::IEncoding,
...
void distilbert_contract(const fs::path& root) {
    const auto path = root / "distilbert.bundle";
    write_bundle(path, "distilbert");
    ...
    check(encoded.first == 0 && Json::parse(encoded.second.output).at("values") == Json({3.0, 1.0}),
          "DISTILBERT owner converts the encoding request and preserves result fields");
    check(loaded_runtime == "/installed/runtime",
          "DISTILBERT owner uses the supplied installed runtime root");
    ...
    check(embedded.first == 0 &&
          ...
          "DISTILBERT owner selects the embedding interface");
    check(loaded_runtime == "/override/runtime",
          "runtime_root override remains owned by DISTILBERT");
...
std::unique_ptr<ITask> load_task(const BundleReader&, const std::string& runtime_root,
    ...
    return std::make_unique<FakeDistildistilbert>();
}
} // namespace trtmc

int main() {
    const auto root =
        fs::temp_directory_path() / ("trtmc-distilbert-cli-" + std::to_string(getpid()));
    fs::create_directories(root);
    try {
        distilbert_contract(root);
