...
from families.distilbert import cli, support
...
def test_cli_defaults_match_support_and_publish_a_bundle(monkeypatch, tmp_path):
    ...
    def build_model(request, writer):
        ...
        writer.set_header(family="distilbert", task=request.task, backend=request.backend)
        writer.add_bytes("engine.plan", b"PLAN")

    def select_backend(backend):
        assert backend == "trt"
        monkeypatch.setitem(
            sys.modules, "families.distilbert.model", SimpleNamespace(build=build_model)
        )

    monkeypatch.setattr(cli, "select_backend", select_backend)
    output = tmp_path / "model.bundle"
    declaration = support.describe(ModelMetadata({"model_type": "distilbert"}, {}))
    assert declaration is not None
    assert family_cli.main(["distilbert", "build", str(tmp_path), "-o", str(output)]) == 0
    assert len(calls) == 1 and type(calls[0]) is cli.BuildRequest
    assert calls[0].task == declaration.default_task == "encoding"
...
def test_failed_owner_preserves_existing_output(monkeypatch, tmp_path):
    ...
    monkeypatch.setitem(sys.modules, "families.distilbert.model", SimpleNamespace(build=fail))
...
def test_legacy_unsupported_inputs_are_still_rejected(tmp_path, changes):
    legacy = LegacyRequest(tmp_path, tmp_path / "out", "distilbert", "encoding", "fp32")
...
def test_help_is_lazy_and_owned(monkeypatch, capsys):
    ...
    def guarded(name, *args, **kwargs):
        assert name not in {
            "families.distilbert.cli",
            "families.distilbert.model",
            "tensorrt",
            "tensorrt_rtx",
            "huggingface_hub",
        }
    ...
    with pytest.raises(SystemExit) as error:
        family_cli.main(["distilbert", "build", "--help"])
