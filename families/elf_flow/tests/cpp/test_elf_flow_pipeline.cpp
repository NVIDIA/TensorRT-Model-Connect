...
class FakeTokenizer final : public trtmc::ITokenizer {
  public:
    ...
    std::string decode(const std::vector<int32_t>& ids) const override {
        std::string output;
        for (int32_t id : ids) {
            output += token_for_id(id);
        }
        return output;
    }

    int32_t id_for_token(std::string_view token) const override {
        return token == "</s>" ? eos_id : -1;
    }
...
class FakeTextEncoderModule final : public trtmc::ITrtModule {
  public:
    ...
    trtmc::DType tensor_dtype(const std::string& name) const override {
        return name == "input_ids" ? trtmc::DType::kInt32 : trtmc::DType::kFloat32;
    }
    ...
    bool input_is_dynamic(const std::string& /*name*/) const override { return false; }
...
class FakeElfModule final : public trtmc::ITrtModule {
  public:
    bool ok() const override { return true; }
  ...
  private:
    trtmc::TensorMap forward_denoise(const trtmc::TensorMap& inputs, const float* latent,
            ...
            if (cond_call && check_first_cond_uses_initial_latent &&
                ...
                for (int32_t row = 1; row < kMaxLength && matches_initial; ++row) {
                    for (int32_t col = 0; col < kTextDim; ++col) {
                        const auto latent_index = row * kInputDim + col;
                        const auto expected_index = row * kTextDim + col;
                        matches_initial = matches_initial &&
                                          close_enough(latent[latent_index],
                                                       expected_initial_latent[expected_index]);
                    }
                }
        ...
        if (check_first_denoise_latent && denoise_calls == 1) {
            ...
            check(matches, "explicit SDE noise controls first denoise latent");
        }
...
void test_generate_runs_upstream_shape_contract() {
    ...
    config.initial_latents = {0.1F, 0.2F, 0.3F, 0.4F, 0.5F, 0.6F};

    const auto result = pipeline.generate("", config);
...
void test_conditional_generate_restores_prefix_and_strips_decoded_condition() {
    ...
    config.sde_gamma = 0.0F;
    config.initial_latents = {0.1F, 0.2F, 0.3F, 0.4F, 0.5F, 0.6F};
    config.condition_latents = {9.0F, 8.0F, 0.0F, 0.0F, 0.0F, 0.0F};
    config.condition_mask = {1.0F, 0.0F, 0.0F};
...
void test_conditional_defaults_match_upstream_sampling_config() {
    ...
    config.condition_latents = {9.0F, 8.0F, 0.0F, 0.0F, 0.0F, 0.0F};
    config.condition_mask = {1.0F, 0.0F, 0.0F};
    module_ptr->expected_initial_latent = config.initial_latents;
...
void test_explicit_sampling_steps_and_sde_noise_drive_upstream_inputs() {
    auto module = std::make_unique<FakeElfModule>();
    module->require_zero_denoise_timestep = false;
    ...
    config.initial_latents = {0.1F, 0.2F, 0.3F, 0.4F, 0.5F, 0.6F};
    config.sampling_steps = {0.0F, 0.5F, 1.0F};
    config.sde_noises = {2.0F, 2.0F, 2.0F, 2.0F, 2.0F, 2.0F};

    const auto result = pipeline.generate("", config);
    check(result.text == "ABC", "explicit sampling inputs still decode text");
    check(module_ptr->denoise_timesteps.size() == 2,
          "explicit sampling runs one SDE and final ODE step");
    check(close_enough(module_ptr->denoise_timesteps[0], 0.0F),
          "explicit sampling first timestep uses supplied start");
    check(close_enough(module_ptr->denoise_timesteps[1], 0.5F),
          "explicit sampling final ODE uses supplied penultimate timestep");
}
...
void test_prompt_condition_uses_bundled_t5_encoder() {
    ...
    const auto result = pipeline.generate("Quelle", config);
...
void test_prompt_condition_requires_bundled_t5_encoder() {
    ...
    bool threw = false;
    try {
        (void)pipeline.generate("Quelle", config);
    } catch (const std::runtime_error& error) {
        threw = std::string(error.what()).find("text_encoder.plan") != std::string::npos;
    }
...
void test_condition_api_requires_latents_and_mask() {
    {
        ...
        bool threw = false;
        try {
            (void)pipeline.generate("ignored", config);
        } catch (const std::runtime_error& error) {
            threw = std::string(error.what()).find("condition_latents") != std::string::npos;
        }
    ...
    }
    {
        ...
        try {
            ...
            threw = std::string(error.what()).find("condition_mask") != std::string::npos;
        }
...
int main() {
    ...
    test_explicit_sampling_steps_and_sde_noise_drive_upstream_inputs();
