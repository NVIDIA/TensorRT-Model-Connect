...
#include "families/olmo/runtime/pipeline.h"

#include "families/olmo/runtime/chat_templates.h"
#include "families/olmo/runtime/kv_cache.h"
#include "families/olmo/runtime/tensor_names.h"
...
OlmoTextGenerationPipeline::OlmoTextGenerationPipeline(std::unique_ptr<ITrtModule> decoder,
                                                       std::unique_ptr<OlmoInferenceState> state,
                                                       OlmoTextGenConfig config,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("OlmoTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("OlmoTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
TextResult OlmoTextGenerationPipeline::run(const internal::TextContinuationRequest& request,
                                           internal::ConfigView supplied) {
    const auto cfg = olmo::parse_text_config(supplied);
    ...
    if (const auto* prompt = std::get_if<std::string_view>(&request.prefix)) {
        ...
        input_ids = olmo::copy_token_prefix(std::get<Span<const std::int32_t>>(request.prefix));
    }
...
void require_prefill_kv_pointers(ITrtModule& prefill, const OlmoTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(olmo_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(olmo_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "OlmoTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const OlmoTextGenConfig& cfg, int32_t sq,
                                      OlmoInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("OlmoTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error("OlmoTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("OlmoTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error("OlmoTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<OlmoKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("OlmoTextGenerationPipeline: prefill requires OlmoKvCache");
}

} // namespace

void OlmoTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
...
std::string
OlmoTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
OlmoTextGenerationPipeline::TimedGenResult OlmoTextGenerationPipeline::generate_from_ids(
    ...
    if (mode != "ar")
        throw std::runtime_error("OlmoTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_olmo_sampler(params);
...
int32_t OlmoTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const OlmoSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& OlmoTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
