...
def build(request: "BuildRequest", writer: "BundleWriter") -> None:
    ...
    if request.dynamic_kv_cache:
        raise NotImplementedError("olmo does not support dynamic_kv_cache")
    ...
    if request.task != "text_continuation":
        raise ValueError("olmo supports only task=text_continuation")
    ...
    writer.set_header(family="olmo", task=request.task, backend=request.backend)
    if parallel.enabled:
