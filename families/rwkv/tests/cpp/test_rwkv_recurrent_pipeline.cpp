...
static void test_mamba_pipeline() {
    ...
    std::vector<trtmc::RwkvRecurrentState::TensorSpec> specs = {
        {"conv_state", {12}},
        {"ssm_state", {32}},
    };
    auto rs = std::make_unique<trtmc::RwkvRecurrentState>(1, specs, stream);
...
static void test_rwkv_pipeline() {
    ...
    auto rs = std::make_unique<trtmc::RwkvRecurrentState>(2, specs, stream);
...
static void test_argmax_recurrent() {
    ...
    trtmc::RwkvSamplingParams params;
    auto sampler = trtmc::create_rwkv_sampler(params);
