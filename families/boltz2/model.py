...
"""Build a reproducible native Boltz-2 multi-plan bundle."""
...
from .request_preparation import load_profile_features
...
class Boltz2Plugin:
    ...
    def load_weights(
        ...
        *,
        ...
        features = load_profile_features(root / PROCESSED, root / MOLS)
    ...
    def build_extra_engines(
        ...
        **_kwargs: Any,
        ...
        random_samples = serialize_profile_random_samples(atom_count=atom_count)
        sections: dict[str, bytes] = {}
        with tempfile.TemporaryDirectory(prefix="trtmc-boltz2-plans-") as directory:
...
def build(request: Any, writer: Any) -> None:
    ...
    sections = family.build_extra_engines(
        None, weights, 0, precision=request.precision, verbose=request.verbose
    )
    writer.set_header(family="boltz2", task=request.task, backend=request.backend)
    writer.add_bytes("engine.plan", primary)
    for name, payload in sections.items():
        writer.add_bytes(name, payload)
    writer.add_json("runtime.json", family.get_bundle_config_overrides(None))
