...
StructurePredictionConfig resolveRandomControls(const StructurePredictionConfig& requested,
                                                const RandomSamples& random, bool has_affinity) {
    ...
    if (has_affinity && result.affinity_sampling_steps == 0)
        result.affinity_sampling_steps = random.affinity.sampling_steps;
    if (has_affinity && result.affinity_diffusion_samples == 0)
        result.affinity_diffusion_samples = random.affinity.sample_count;
    return result;
}
...
void validateStructureControls(const StructurePredictionConfig& cfg, const RandomSamples& random) {
    ...
    if (!std::isfinite(cfg.step_scale) || cfg.step_scale < 0.5F || cfg.step_scale > 5.0F)
        throw std::invalid_argument("Boltz-2 step scale must be finite and in [0.5, 5.0]");
}

void validateAffinityControls(const StructurePredictionConfig& cfg, const RandomSamples& random) {
    if (cfg.affinity_sampling_steps != random.affinity.sampling_steps)
        throw std::invalid_argument(
            "Boltz-2 affinity sampling steps differ from the prepared request random stream");
    if (cfg.affinity_diffusion_samples != random.affinity.sample_count)
        throw std::invalid_argument(
            "Boltz-2 affinity sample count differs from the prepared request random stream");
}
...
float correctedAffinityValue(float value, const std::string& metadata_json) {
    const auto metadata = nlohmann::json::parse(metadata_json);
    if (!metadata.contains("affinity_mw") || !metadata["affinity_mw"].is_number())
        throw std::invalid_argument(
            "Boltz-2 molecular-weight correction requires affinity molecular weight metadata");
    const float molecular_weight = metadata["affinity_mw"].get<float>();
    if (!std::isfinite(molecular_weight) || molecular_weight <= 0.0F)
        throw std::invalid_argument(
            "Boltz-2 affinity molecular weight must be positive and finite");
    return 1.03525938F * value - 0.59992683F * std::pow(molecular_weight, 0.3F) + 2.83288489F;
}
...
int32_t representativeAtom(const float* token_to_rep_atom, int token, int atom_count) {
    const auto offset = static_cast<std::size_t>(token) * atom_count;
    for (int atom = 0; atom < atom_count; ++atom)
        if (token_to_rep_atom[offset + static_cast<std::size_t>(atom)] != 0.0F)
            return atom;
    return -1;
}

std::optional<std::array<int32_t, 3>> ligandFrame(const std::vector<Vec3>& points, int token,
                                                  const float* token_mask, const int32_t* asym_id,
                                                  const int32_t* mol_type,
                                                  const float* token_to_rep_atom, int token_count,
                                                  int atom_count) {
    // Upstream confidence rebuilds nonpolymer frames from every predicted pose.
    constexpr int32_t kNonPolymer = 3;
    if (mol_type[token] != kNonPolymer)
        return std::nullopt;
    const int32_t center = representativeAtom(token_to_rep_atom, token, atom_count);
    if (center < 0)
        return std::nullopt;
    std::vector<std::pair<float, int32_t>> neighbors;
    for (int candidate = 0; candidate < token_count; ++candidate) {
        if (token_mask[candidate] == 0.0F || asym_id[candidate] != asym_id[token])
            continue;
        const int32_t atom = representativeAtom(token_to_rep_atom, candidate, atom_count);
        if (atom < 0)
            continue;
        float squared_distance = 0.0F;
        for (int axis = 0; axis < 3; ++axis) {
            const float difference = points[center][axis] - points[atom][axis];
            squared_distance += difference * difference;
        }
        neighbors.emplace_back(squared_distance, atom);
    }
    if (neighbors.size() < 3)
        return std::nullopt;
    std::sort(neighbors.begin(), neighbors.end());
    return std::array<int32_t, 3>{neighbors[1].second, neighbors[0].second, neighbors[2].second};
}

std::vector<float> confidenceFrameMask(const std::vector<float>& coordinates, const int32_t* frames,
                                       const float* token_mask, const int32_t* asym_id,
                                       const int32_t* mol_type, const float* token_to_rep_atom,
                                       int token_count, int atom_count) {
    ...
    for (int token = 0; token < token_count; ++token) {
        std::array<int32_t, 3> frame{frames[token * 3], frames[token * 3 + 1],
                                     frames[token * 3 + 2]};
        if (const auto dynamic = ligandFrame(points, token, token_mask, asym_id, mol_type,
                                             token_to_rep_atom, token_count, atom_count))
            frame = *dynamic;
...
void Boltz2Pipeline::runTrunk(int recycling_steps, bool affinity_input, bool apply_templates) {
    bindInputEmbedding(affinity_input);
    if (apply_templates && use_templates_)
        bindTemplatePath();
    else
        bindPointer(*engines_.msa, "z", *engines_.trunk_init, "z");
...
StructureConfidence Boltz2Pipeline::runConfidence(const std::vector<float>& coordinates) {
    ...
    const auto frame_mask = confidenceFrameMask(coordinates, frames, token_mask, asym_id, mol_type,
                                                token_to_rep_atom, token_count_, atom_count_);
...
AffinityPrediction Boltz2Pipeline::predictAffinity(const StructurePredictionConfig& cfg) {
    const auto* token_mask = reinterpret_cast<const float*>(feature("token_pad_mask").data.data());
    ...
    try {
        ...
        for (int sample = 0; sample < cfg.affinity_diffusion_samples; ++sample) {
            auto candidate = sampleCoordinates(artifacts_.random_samples.affinity,
                                               cfg.affinity_sampling_steps, sample, cfg.step_scale);
        ...
        auto result = runAffinity(best_coordinates);
        if (cfg.affinity_mw_correction)
            result.value = correctedAffinityValue(result.value, artifacts_.structure_metadata_json);
        if (!method_feature.copy_from_host(original_method))
            throw std::runtime_error("Boltz-2 failed to restore method conditioning");
        return result;
    } catch (...) {
        static_cast<void>(method_feature.copy_from_host(original_method));
        throw;
    }
}
...
std::string Boltz2Pipeline::resultMetadata(const StructurePredictionConfig& cfg,
    ...
    if (affinity.has_value()) {
        ...
        metadata["affinity_sampling_steps"] = cfg.affinity_sampling_steps;
        metadata["affinity_diffusion_samples"] = cfg.affinity_diffusion_samples;
        metadata["affinity_mw_correction"] = cfg.affinity_mw_correction;
...
Boltz2Pipeline::predict_structure(const StructurePredictionRequest& request) {
    ...
    const auto cfg =
        resolveRandomControls(request.config, artifacts_.random_samples, has_affinity_);
    validateStructureControls(cfg, artifacts_.random_samples);
    validateOutputFormat(cfg.output_format);
    if (has_affinity_)
        validateAffinityControls(cfg, artifacts_.random_samples);
    engines_.input->forward_device_async({});
    ...
    for (int sample = 0; sample < cfg.diffusion_samples; ++sample) {
        auto coordinates = sampleCoordinates(artifacts_.random_samples.structure,
                                             cfg.sampling_steps, sample, cfg.step_scale);
