...
constexpr uint32_t kExpectedTensorCount = 43;
...
class Reader {
  public:
    Reader(const void* data, std::size_t size)
        ...
        if (data == nullptr && size != 0)
            throw std::invalid_argument("Boltz-2 feature section has null storage");
    }
  ...
  private:
    void requireBytes(std::size_t size) const {
        if (size > remaining_)
            throw std::invalid_argument("truncated Boltz-2 feature section");
    }
...
DType decodeDtype(uint8_t code) {
    ...
    case 3:
        return DType::kInt8;
    default:
        throw std::invalid_argument("unsupported Boltz-2 feature dtype code");
    }
}
...
std::size_t checkedElementCount(const std::vector<int64_t>& shape) {
    ...
    for (const int64_t dimension : shape) {
        if (dimension <= 0)
            throw std::invalid_argument("Boltz-2 feature dimensions must be positive");
        const auto value = static_cast<std::size_t>(dimension);
        if (count > std::numeric_limits<std::size_t>::max() / value)
            throw std::invalid_argument("Boltz-2 feature shape overflows host size");
        count *= value;
    }
    return count;
}
...
ParsedFeature readFeature(Reader& source) {
    ...
    if (name.empty())
        throw std::invalid_argument("Boltz-2 feature name must not be empty");
    const auto dtype = decodeDtype(source.littleEndian<uint8_t>());
    const auto rank = source.littleEndian<uint8_t>();
    if (rank == 0 || rank > 8)
        throw std::invalid_argument("Boltz-2 feature rank must be in [1, 8]");
    ...
    if (payload_size_u64 > std::numeric_limits<std::size_t>::max())
        throw std::invalid_argument("Boltz-2 feature payload is too large");
    ...
    if (element_count > std::numeric_limits<std::size_t>::max() / dtype_size(dtype))
        throw std::invalid_argument("Boltz-2 feature payload size overflows host size");
    if (payload_size != element_count * dtype_size(dtype))
        throw std::invalid_argument("Boltz-2 feature payload size does not match its shape");
...
FeatureBundle FeatureBundle::parse(const void* data, std::size_t size) {
    ...
    if (source.string(kMagic.size()) != std::string(kMagic.data(), kMagic.size()))
        throw std::invalid_argument("invalid Boltz-2 feature section magic");
    ...
    if (count != kExpectedTensorCount)
        throw std::invalid_argument("Boltz-2 feature section must contain exactly 43 tensors");
    ...
    for (uint32_t index = 0; index < count; ++index) {
        ...
        if (!result.tensors_.emplace(parsed.name, std::move(parsed.tensor)).second)
            throw std::invalid_argument("duplicate Boltz-2 feature tensor: " + parsed.name);
    }
    if (source.remaining() != 0)
        throw std::invalid_argument("Boltz-2 feature section has trailing bytes");
    return result;
}
...
const FeatureTensor& FeatureBundle::require(std::string_view name) const {
    ...
    if (found == tensors_.end())
        throw std::invalid_argument("Boltz-2 feature section is missing tensor: " +
                                    std::string(name));
    return found->second;
}
