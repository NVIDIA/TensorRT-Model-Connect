...
class FakeDedebertaa final : public trtmc::IEncoding,
...
void deberta_contract(const fs::path& root) {
    const auto path = root / "deberta.bundle";
    write_bundle(path, "deberta");
    ...
    check(encoded.first == 0 && Json::parse(encoded.second.output).at("values") == Json({3.0, 1.0}),
          "DEBERTA owner converts the encoding request and preserves result fields");
    check(loaded_runtime == "/installed/runtime",
          "DEBERTA owner uses the supplied installed runtime root");
    ...
    check(embedded.first == 0 &&
          ...
          "DEBERTA owner selects the embedding interface");
    check(loaded_runtime == "/override/runtime", "runtime_root override remains owned by DEBERTA");
...
std::unique_ptr<ITask> load_task(const BundleReader&, const std::string& runtime_root,
    ...
    return std::make_unique<FakeDedebertaa>();
}
} // namespace trtmc

int main() {
    const auto root = fs::temp_directory_path() / ("trtmc-deberta-cli-" + std::to_string(getpid()));
    fs::create_directories(root);
    try {
        deberta_contract(root);
