...
def test_deberta_owns_its_tensor_parallel_builder() -> None:
    ...
    assert "build_tp_deberta_encoder_engine" in functions
    assert "shard_deberta_weights" in functions
    assert "shard_standard_decoder_weights" not in functions
    assert ".checkpoint_mapper" not in parallel
    assert "pos_q_proj" in parallel
    assert (
    ...
    assert "from .parallel import build_tp_deberta_encoder_engine" in model
