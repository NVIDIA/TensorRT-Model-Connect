...
"""Direct build, native runtime, and timm reference proof for ConvNeXt."""
...
from families.timm_convnext.cli import BuildRequest, build_bundle


FAMILY = "timm_convnext"
...
def pytest_generate_tests(metafunc) -> None:
    ...
    if not selected:
        names = [
            pytest.param(
                name,
                marks=pytest.mark.skip(reason="real ConvNeXt E2E requires explicit selection"),
                id=name,
            )
            for name in names
        ]
...
def _required_path(value: str | None, label: str) -> Path:
    assert value, f"selected ConvNeXt E2E requires {label}"
    path = Path(value)
    assert path.exists(), f"selected ConvNeXt E2E {label} does not exist: {path}"
    return path


def _model_dir(manifest: dict) -> Path:
    explicit = os.environ.get("TRTMC_TIMM_TIMM_CONVNEXT_MODEL_DIR")
    if explicit:
        return _required_path(explicit, "TRTMC_TIMM_TIMM_CONVNEXT_MODEL_DIR")
...
def _asset(case: dict) -> Path:
    ...
    assert path.is_file(), f"selected ConvNeXt E2E image is missing: {path}"
    record_evidence("inputs", {"asset": path})
    return path


def _assert_parity(actual, expected, model_config: dict) -> None:
...
def _assert_sdk_consumers(
    ...
    for language in ("c", "cpp"):
        consumer = native_build / f"test_timm_convnext_sdk_{language}"
...
def test_official_checkpoint_e2e(case_name: str, tmp_path: Path) -> None:
    ...
    with evidence_stage("compare"):
        _assert_parity(actual, int(np.argmax(expected)), config)
