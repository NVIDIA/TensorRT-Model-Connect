...
TimmConvNeXtPreprocessConfig parse_config(const std::vector<char>& data) {
    const auto json = nlohmann::json::parse(data.begin(), data.end());
    TimmConvNeXtPreprocessConfig config;
    ...
    if (config.input_image_h <= 0 || config.input_image_w <= 0 || config.crop_pct <= 0.0F ||
        ...
        throw std::runtime_error("timm ConvNeXt runtime.json does not match its contract");
    }
    return config;
}
...
std::unique_ptr<ITrtModule> load_engine(IBackend& backend, const std::vector<char>& plan) {
    ...
    if (!engine || !engine->ok())
        throw std::runtime_error("timm ConvNeXt engine failed to load");
    return engine;
}
...
extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    ...
    const auto config_data = trtmc::timm_convnext::require_section(context.reader, "runtime.json");
    const auto plan = trtmc::timm_convnext::require_section(context.reader, "engine.plan");
    auto config = trtmc::timm_convnext::parse_config(config_data);
    auto engine = trtmc::timm_convnext::load_engine(context.backend, plan);
    return new trtmc::TimmConvNeXtImageClassificationPipeline(std::move(engine), std::move(config));
}
