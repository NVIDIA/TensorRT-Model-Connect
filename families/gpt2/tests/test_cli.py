...
from families.gpt2 import cli, support
...
def test_cli_defaults_match_support_and_publish_a_bundle(monkeypatch, tmp_path):
    ...
    def select_backend(backend):
        assert backend == "trt"
        monkeypatch.setitem(sys.modules, "families.gpt2.model", SimpleNamespace(build=build_model))

    monkeypatch.setattr(cli, "select_backend", select_backend)
    output = tmp_path / "model.bundle"
    declaration = support.describe(ModelMetadata({"model_type": "gpt2"}, {}))
    assert declaration is not None
    assert family_cli.main(["gpt2", "build", str(tmp_path), "-o", str(output)]) == 0
...
def test_failed_owner_preserves_existing_output(monkeypatch, tmp_path):
    ...
    monkeypatch.setitem(sys.modules, "families.gpt2.model", SimpleNamespace(build=fail))
...
def test_legacy_unsupported_inputs_are_still_rejected(tmp_path, changes):
    legacy = LegacyRequest(tmp_path, tmp_path / "out", "gpt2", "text_generation", "fp32")
...
def test_invalid_input_is_rejected_before_model_loading(monkeypatch, tmp_path):
    ...
    with pytest.raises(SystemExit):
        family_cli.main(
            ["gpt2", "build", str(tmp_path), "-o", str(tmp_path / "out"), "--video-num-frames", "2"]
        )
...
def test_help_is_lazy_and_owned(monkeypatch, capsys):
    ...
    with pytest.raises(SystemExit) as error:
        family_cli.main(["gpt2", "build", "--help"])
