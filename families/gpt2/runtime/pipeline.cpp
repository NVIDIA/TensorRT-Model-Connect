...
#include "families/gpt2/runtime/pipeline.h"

#include "families/gpt2/runtime/chat_templates.h"
#include "families/gpt2/runtime/kv_cache.h"
#include "families/gpt2/runtime/tensor_names.h"
...
Gpt2TextGenerationPipeline::Gpt2TextGenerationPipeline(std::unique_ptr<ITrtModule> decoder,
                                                       std::unique_ptr<Gpt2InferenceState> state,
                                                       Gpt2TextGenConfig config,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("Gpt2TextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("Gpt2TextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
TextResult Gpt2TextGenerationPipeline::run(const internal::TextContinuationRequest& request,
                                           internal::ConfigView supplied) {
    const auto cfg = gpt2::parse_text_config(supplied);
    ...
    if (const auto* prompt = std::get_if<std::string_view>(&request.prefix)) {
        ...
        input_ids = gpt2::copy_token_prefix(std::get<Span<const std::int32_t>>(request.prefix));
    }
...
void require_prefill_kv_pointers(ITrtModule& prefill, const Gpt2TextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(gpt2_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(gpt2_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "Gpt2TextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const Gpt2TextGenConfig& cfg, int32_t sq,
                                      Gpt2InferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("Gpt2TextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error("Gpt2TextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("Gpt2TextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error("Gpt2TextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<Gpt2KvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("Gpt2TextGenerationPipeline: prefill requires Gpt2KvCache");
}

} // namespace

void Gpt2TextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error("Gpt2TextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(lt.numel()) < vocab)
        throw std::runtime_error(
            "Gpt2TextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
Gpt2TextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
Gpt2TextGenerationPipeline::TimedGenResult Gpt2TextGenerationPipeline::generate_from_ids(
    ...
    if (mode != "ar")
        throw std::runtime_error("Gpt2TextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_gpt2_sampler(params);
...
int32_t Gpt2TextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const Gpt2SampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& Gpt2TextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
