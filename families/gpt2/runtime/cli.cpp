...
trtmc::TextGenerationConfig generation_config(const Json& values) {
    ...
    if (config.temperature < 0.0F || config.top_p < 0.0F || config.top_p > 1.0F ||
        ...
        throw std::invalid_argument("invalid gpt2 sampling configuration");
    return config;
}

Json execute(const std::string& handler, const Json& values, const char* default_runtime_root) {
    if (handler != "generate")
        throw std::invalid_argument("unknown gpt2 CLI handler: " + handler);
    const trtmc::BundleReader reader(values.at("bundle").get<std::string>());
    if (reader.info().family != "gpt2")
        throw std::invalid_argument("gpt2 CLI requires a gpt2 bundle");
    ...
    if (generation == nullptr)
        throw std::invalid_argument("gpt2 bundle does not implement text generation");
    ...
    if (!std::isfinite(result.setup_ms) || !std::isfinite(result.prefill_ms) ||
        ...
        throw std::runtime_error("gpt2 returned non-finite timing values");
