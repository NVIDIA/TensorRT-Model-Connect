...
#include "families/gpt2/runtime/distributed_runtime.h"
...
T require_value(const nlohmann::json& json, const char* name) {
    if (!json.contains(name))
        throw std::runtime_error(std::string("gpt2 runtime config missing '") + name + "'");
    try {
        return json.at(name).get<T>();
    } catch (const nlohmann::json::exception&) {
        throw std::runtime_error(std::string("gpt2 runtime config has invalid '") + name + "'");
    }
}
...
RuntimeConfig parse_runtime_config(const BundleReader& bundle) {
    ...
    try {
        ...
        throw std::runtime_error("gpt2 invalid runtime.json: " + std::string(error.what()));
    }
    if (!json.is_object())
        throw std::runtime_error("gpt2 runtime.json must be an object");
    if (json.size() != 14)
        throw std::runtime_error("gpt2 runtime.json has an unexpected field set");
    ...
    if (config.hidden_size <= 0 || config.num_layers <= 0 || config.num_heads <= 0 ||
        ...
        config.tensor_parallel_size <= 0 || config.num_key_value_heads <= 0 ||
        config.head_dim <= 0 || config.num_key_value_heads * config.head_dim > config.hidden_size) {
        throw std::runtime_error("gpt2 runtime.json contains invalid dimensions");
    }
    if (config.precision != "fp16" && config.precision != "bf16" && config.precision != "fp32") {
        throw std::runtime_error("gpt2 runtime.json contains invalid precision");
    }
    if (config.decoder_engine_layout != "split" && config.decoder_engine_layout != "dual_profile") {
        throw std::runtime_error("gpt2 runtime.json contains invalid decoder_engine_layout");
    }
...
Gpt2KvCacheNames make_kv_names(std::int32_t num_layers) {
    ...
    for (std::int32_t layer = 0; layer < num_layers; ++layer) {
        names.cache_k.push_back(gpt2_layer_tensor_name("cache_k", layer));
        names.cache_v.push_back(gpt2_layer_tensor_name("cache_v", layer));
        names.present_k.push_back(gpt2_layer_tensor_name("present_k", layer));
        names.present_v.push_back(gpt2_layer_tensor_name("present_v", layer));
    }
    return names;
}
...
DecoderModules load_modules(const FamilyContext& context, const RuntimeConfig& config) {
    ...
    if (config.decoder_engine_layout == "split") {
        modules.decode = load_engine(
            context.backend, require_section(context.reader, "engine.plan"), "gpt2 decoder");
        modules.prefill = load_engine(
            context.backend, require_section(context.reader, "prefill.plan"), "gpt2 prefill");
        return modules;
    }
    ...
    if (dual.decode == nullptr || !dual.decode->ok() || dual.prefill == nullptr ||
        !dual.prefill->ok()) {
        throw std::runtime_error("gpt2 dual-profile engine did not create both contexts");
    }
...
ITask* create(const FamilyContext& context) {
    ...
    if (!state->ok())
        throw std::runtime_error("gpt2 failed to create KV cache");

    Gpt2TextGenConfig text_config;
    ...
    text_config.chat_template_format =
        gpt2_detect_chat_template_format(chat_template(context.reader));
    ...
    return new Gpt2TextGenerationPipeline(
        std::move(modules.decode), std::move(state), std::move(text_config), std::move(tokenizer),
        std::move(modules.prefill), std::move(modules.distributed_owner));
}

} // namespace trtmc::gpt2

TRTMC_DEFINE_FAMILY_PLUGIN_V1("gpt2")
