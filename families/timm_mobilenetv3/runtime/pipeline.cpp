...
#include "families/timm_mobilenetv3/runtime/pipeline.h"
...
const Tensor& require_logits(const TensorMap& outputs) {
    ...
    if (found == outputs.end())
        throw std::runtime_error("timm MobileNetV3 engine did not return logits");
    if (found->second.data == nullptr || found->second.dtype != DType::kFloat32 ||
        found->second.numel() <= 0)
        throw std::runtime_error("timm MobileNetV3 engine must return nonempty float32 logits");
    return found->second;
}

} // namespace

TimmMobilenetv3ImageClassificationPipeline::TimmMobilenetv3ImageClassificationPipeline(
    ...
    if (!model_ || !model_->ok())
        throw std::runtime_error("TimmMobilenetv3ImageClassificationPipeline: invalid model");
    if (num_classes_ <= 0 ||
        (!labels_.empty() && labels_.size() != static_cast<std::size_t>(num_classes_)))
        throw std::runtime_error("timm MobileNetV3 class metadata does not match its output size");
}

internal::LabelScoresResult
TimmMobilenetv3ImageClassificationPipeline::run(const internal::ImageToClassScoresRequest& request,
                                                internal::ConfigView config) {
    if (!config.empty())
        throw internal::ConfigError("timm MobileNetV3 has no runtime configuration");
    ...
    if (image.format != internal::ImageFormat::Float32 || image.channels != 3 ||
        ...
        throw std::invalid_argument("timm MobileNetV3 requires contiguous float32 RGB input");
    auto pixel_values = preprocess_timm_mobilenetv3_image(
    ...
    if (n != static_cast<std::size_t>(num_classes_))
        throw std::runtime_error("timm MobileNetV3 logits do not match its configured class count");
