...
Json classify(const Json& values, const char* default_runtime_root) {
    ...
    if (reader.info().family != "timm_mobilenetv3")
        throw std::invalid_argument("timm_mobilenetv3 CLI requires its own family bundle");
...
extern "C" int trtmc_family_cli_v1(const char* handler, const char* values_json,
    ...
    try {
        if (std::string(handler) != "classify")
            throw std::invalid_argument("unknown timm_mobilenetv3 CLI handler");
