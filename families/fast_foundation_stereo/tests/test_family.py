...
@pytest.fixture
def family_builder(monkeypatch):
    module_name = "families.fast_foundation_stereo.builder"
    package = importlib.import_module("families.fast_foundation_stereo")
    previous_module = sys.modules.pop(module_name, None)
    previous_attribute = package.__dict__.pop("builder", None)
    monkeypatch.setitem(sys.modules, "tensorrt", SimpleNamespace())
    module = importlib.import_module(module_name)
    yield module
    sys.modules.pop(module_name, None)
    package.__dict__.pop("builder", None)
    if previous_module is not None:
        sys.modules[module_name] = previous_module
    if previous_attribute is not None:
        package.builder = previous_attribute


def test_native_builder_rejects_unsupported_precision(family_builder) -> None:
    assert family_builder._validate_precision("fp16") is True
    with pytest.raises(ValueError, match="supports precision='fp16' only"):
        family_builder._validate_precision("fp32")


def test_native_builder_uses_family_owned_feature_and_post_profiles(family_builder) -> None:
    class Config:
    ...
    assert family_builder._serialize_network(
        trt,
        fake_builder,
        object(),
...
def test_post_engine_consumes_only_feature_engine_outputs(family_builder) -> None:
    assert family_builder.POST_INPUT_NAMES == family_builder.FEATURE_OUTPUT_NAMES
    assert "gwc_volume" not in family_builder.POST_INPUT_NAMES


def test_native_basic_conv_preserves_distilled_boolean_activation_semantics() -> None:
    from families.fast_foundation_stereo.native_graph import NativeGraph
...
def test_feature_att_8_folds_exact_three_bn_and_preserves_relu_gate_and_checkpoint() -> None:
    from families.fast_foundation_stereo.native_graph import NativeGraph
    from families.fast_foundation_stereo.native_post import (
...
def test_cost_aggregation_uses_fixed_default_winner_routes(monkeypatch) -> None:
    from families.fast_foundation_stereo import native_post
...
def test_spatial_attention_uses_native_two_output_reduction_plugin(monkeypatch) -> None:
    ...
    from families.fast_foundation_stereo import native_plugin_builder
...
def test_raft_gru_stacks_update_and_reset_convolutions() -> None:
    from families.fast_foundation_stereo.native_post import _raft_gru
