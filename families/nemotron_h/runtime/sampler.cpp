...
#include "families/nemotron_h/runtime/sampler.h"
...
NemotronHSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
    ...
    result.logprob = *best;
    result.is_eos = std::find(eos_token_ids.begin(), eos_token_ids.end(), result.token_id) !=
                    eos_token_ids.end();
    return result;
}
...
bool greedy_equivalent(const NemotronHSamplingParams& params) {
...
class NemotronHGreedySampler final : public NemotronHISampler {
  public:
    NemotronHSampleResult sample(const float* logits, int32_t vocab_size,
                                 const NemotronHSamplingParams& params) override {
        if (vocab_size <= 0 || logits == nullptr) {
            NemotronHSampleResult result;
            result.token_id = 0;
            result.is_eos = std::find(params.eos_token_ids.begin(), params.eos_token_ids.end(),
                                      0) != params.eos_token_ids.end();
            return result;
        }
...
class NemotronHTopKSampler final : public NemotronHISampler {
  public:
    explicit NemotronHTopKSampler(uint64_t initial_seed)
    ...
    NemotronHSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        for (int32_t i = 0; i < dist.keep; ++i) {
            ...
            if (u < cumulative) {
                ...
                result.is_eos = std::find(params.eos_token_ids.begin(), params.eos_token_ids.end(),
                                          result.token_id) != params.eos_token_ids.end();
                return result;
            }
        }
...
create_nemotron_h_sampler(const NemotronHSamplingParams& params) {
    const float top_p = sanitized_top_p(params.top_p);
    const float min_p = sanitized_min_p(params.min_p);
    if (params.top_k <= 1 && top_p >= 1.0F - kSamplingEpsilon && min_p <= 0.0F && params.seed < 0) {
        return std::make_unique<NemotronHGreedySampler>();
    }

    const uint64_t seed = (params.seed >= 0) ? static_cast<uint64_t>(params.seed) : 42ULL;
    return std::make_unique<NemotronHTopKSampler>(seed);
}
