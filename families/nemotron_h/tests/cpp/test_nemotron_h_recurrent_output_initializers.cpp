...
void test_every_declared_eos_token_stops_generation() {
    trtmc::NemotronHSamplingParams params;
    params.eos_token_ids = {2, 11, 12};
    auto sampler = trtmc::create_nemotron_h_sampler(params);
    std::vector<float> logits(16, -1.0F);
    logits[11] = 1.0F;
    check(sampler->sample(logits.data(), static_cast<int32_t>(logits.size()), params).is_eos,
          "nemotron-h stops on a secondary EOS token");
    logits[11] = -1.0F;
    logits[7] = 1.0F;
    check(!sampler->sample(logits.data(), static_cast<int32_t>(logits.size()), params).is_eos,
          "nemotron-h continues on a non-EOS token");
}
...
int main() {
    ...
    test_every_declared_eos_token_stops_generation();
