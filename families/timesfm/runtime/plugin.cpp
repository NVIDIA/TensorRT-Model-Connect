...
RuntimeConfig parse_runtime_config(const std::vector<char>& data) {
    ...
    if (!json.is_object())
        throw std::runtime_error("TimesFM runtime.json must be an object");
...
TRTMC_DEFINE_FAMILY_PLUGIN_V1("timesfm")

extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    if (context.kv_cache_size_bytes != 0)
        throw std::invalid_argument("timesfm does not support --kv-cache-size");
