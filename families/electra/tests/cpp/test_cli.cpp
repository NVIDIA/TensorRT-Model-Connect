...
class FakeElectra final : public trtmc::IEncoding,
...
void electra_contract(const fs::path& root) {
    const auto path = root / "electra.bundle";
    write_bundle(path, "electra");
    ...
    check(encoded.first == 0 && Json::parse(encoded.second.output).at("values") == Json({3.0, 1.0}),
          "ELECTRA owner converts the encoding request and preserves result fields");
    check(loaded_runtime == "/installed/runtime",
          "ELECTRA owner uses the supplied installed runtime root");
    ...
    check(embedded.first == 0 &&
          ...
          "ELECTRA owner selects the embedding interface");
    check(loaded_runtime == "/override/runtime", "runtime_root override remains owned by ELECTRA");
...
std::unique_ptr<ITask> load_task(const BundleReader&, const std::string& runtime_root,
    ...
    return std::make_unique<FakeElectra>();
}
} // namespace trtmc

int main() {
    const auto root = fs::temp_directory_path() / ("trtmc-electra-cli-" + std::to_string(getpid()));
    fs::create_directories(root);
    try {
        electra_contract(root);
