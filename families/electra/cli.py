...
"""electra command handlers and model-owned build inputs."""
...
class BuildRequest:
    ...
    def __post_init__(self) -> None:
        if self.task not in {"encoding", "embedding", "reranking"}:
            raise ValueError("unsupported electra task")
        if str(self.precision).lower() not in {"fp16", "bf16", "fp32"}:
            raise ValueError("unsupported electra precision")
...
def _reject_legacy_options(request: object) -> None:
    if request.dynamic_kv_cache:
        raise NotImplementedError("electra does not support dynamic_kv_cache")
...
def coerce_request(request: object) -> BuildRequest:
    ...
    if unknown := set(vars(request)) - set(fields) - legacy:
        raise ValueError(f"unknown electra build inputs: {sorted(unknown)}")
