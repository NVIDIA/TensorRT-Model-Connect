...
```sh
trtmc build timm/dm_nfnet_f0.dm_in1k -o model.bundle
...
```sh
cmake --build build --target test_timm_nfnet_task_contract test_timm_nfnet_image_preprocess
ctest --test-dir build --output-on-failure -R '^timm_nfnet_(task_contract|image_preprocess)$'
```
...
```sh
build/test_timm_nfnet_sdk_c model.bundle build image.rgb.f32 480 640
build/test_timm_nfnet_sdk_cpp model.bundle build image.rgb.f32 480 640
```
...
`tests/performance.yaml` takes over the existing `timm_nfnet.classify`
