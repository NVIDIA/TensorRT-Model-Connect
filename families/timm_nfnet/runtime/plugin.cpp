...
TimmNfnetPreprocessConfig parse_config(const std::vector<char>& data) {
    ...
    config.crop_mode = json.value("crop_mode", std::string{"center"});
...
std::unique_ptr<ITrtModule> load_engine(IBackend& backend, const std::vector<char>& plan) {
    ...
    if (!engine || !engine->ok())
        throw std::runtime_error("timm NFNet engine failed to load");
    return engine;
}
...
TRTMC_DEFINE_FAMILY_PLUGIN_V1("timm_nfnet")
