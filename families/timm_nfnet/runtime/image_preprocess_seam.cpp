...
stbir_filter resize_filter(const std::string& interpolation) {
    ...
    throw std::invalid_argument("Unsupported timm NFNet interpolation: " + interpolation);
}

void validate_config(const TimmNfnetPreprocessConfig& config) {
    if (config.input_image_h <= 0 || config.input_image_w <= 0)
        throw std::invalid_argument("timm NFNet input dimensions must be positive");
    if (config.crop_pct <= 0.0F || config.crop_pct > 1.0F)
        throw std::invalid_argument("timm NFNet crop_pct must be in (0, 1]");
    if (config.image_mean.size() != 3 || config.image_std.size() != 3)
        throw std::invalid_argument("timm NFNet image mean/std must contain three channels");
    for (const float value : config.image_std) {
        if (value == 0.0F)
            throw std::invalid_argument("timm NFNet image std must be non-zero");
    }
    if (config.crop_mode != "center" && config.crop_mode != "squash")
        throw std::invalid_argument("Unsupported timm NFNet crop_mode: " + config.crop_mode);
}
...
TimmNfnetResizeShape compute_timm_nfnet_resize_shape(int32_t image_height, int32_t image_width,
    ...
    if (image_height <= 0 || image_width <= 0)
        throw std::invalid_argument("timm NFNet source dimensions must be positive");
    validate_config(config);
    if (config.crop_mode == "squash") {
        // Both axes go to the same length, so the aspect ratio is not kept.
        // timm resizes to floor(size / crop_pct) and then centre crops.
        return {
            static_cast<int32_t>(
                std::floor(static_cast<float>(config.input_image_h) / config.crop_pct)),
            static_cast<int32_t>(
                std::floor(static_cast<float>(config.input_image_w) / config.crop_pct)),
        };
    }
...
std::vector<float> preprocess_timm_nfnet_image(const float* image_pixels, int32_t image_height,
    ...
    if (image_pixels == nullptr || image_height <= 0 || image_width <= 0)
        throw std::invalid_argument("timm NFNet source image must be non-empty");
    validate_config(config);
    const auto shape = compute_timm_nfnet_resize_shape(image_height, image_width, config);
    ...
    if (stbir_resize(
        ...
        throw std::runtime_error("Failed to resize timm NFNet input image");
    }
