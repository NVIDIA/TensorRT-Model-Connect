...
Interface& require_interface(trtmc::ITask& task) {
    ...
    if (result == nullptr)
        throw std::invalid_argument("FNET bundle does not implement the requested interface");
    return *result;
}
...
nlohmann::json execute(const std::string& handler, const nlohmann::json& values,
    ...
    if (handler != "encode" && handler != "embed" && handler != "rerank")
        throw std::invalid_argument("unknown FNET CLI handler: " + handler);
    // Check identity before loading: a FNET command must not execute another
    ...
    if (reader.info().family != "fnet")
        throw std::invalid_argument("FNET CLI requires a fnet bundle");
    ...
    if (handler == "rerank") {
        ...
        if (!std::isfinite(score))
            throw std::runtime_error("FNET reranking returned a non-finite score");
        return {{"score", score}};
    }
    ...
    for (const auto value : result.data) {
        if (!std::isfinite(value))
            throw std::runtime_error("FNET embedding returned a non-finite value");
    }
