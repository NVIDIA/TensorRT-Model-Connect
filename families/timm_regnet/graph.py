...
"""Small TensorRT graph vocabulary owned by timm RegNet."""
...
def convolution(
    ...
    *,
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a RegNet convolution")
...
def relu(network, tensor):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a RegNet ReLU")
    return layer.get_output(0)
...
def add(network, left, right):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a RegNet residual add")
    return layer.get_output(0)
...
def squeeze_excite(
    ...
    *,
    ...
    if pooled is None:
        raise RuntimeError("TensorRT rejected the RegNet squeeze pooling")
    ...
    if activation is None:
        raise RuntimeError("TensorRT rejected the RegNet excitation sigmoid")
    scaled = network.add_elementwise(
        tensor, activation.get_output(0), trt.ElementWiseOperation.PROD
    )
    if scaled is None:
        raise RuntimeError("TensorRT rejected the RegNet excitation product")
...
def global_average_pool(network, tensor, height: int, width: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected RegNet global average pooling")
    layer.stride_nd = (1, 1)
    return layer.get_output(0)
...
def classifier(
    ...
    *,
    ...
    if flattened is None:
        raise RuntimeError("TensorRT rejected the RegNet classifier reshape")
    ...
    if matrix_layer is None:
        raise RuntimeError("TensorRT rejected the RegNet classifier weights")
    ...
    if product is None:
        raise RuntimeError("TensorRT rejected the RegNet classifier matmul")
    ...
    if bias_layer is None:
        raise RuntimeError("TensorRT rejected the RegNet classifier bias")
    ...
    if output is None:
        raise RuntimeError("TensorRT rejected the RegNet classifier output")
