...
"""Family-owned timm_regnet commands; importing this module does not load TensorRT."""
...
class BuildRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if str(self.precision).lower() not in {"fp16", "fp32"}:
            raise ValueError("timm_regnet supports only fp16 and fp32 precision")
...
def coerce_request(request: object) -> BuildRequest:
    ...
    if unknown := set(vars(request)) - allowed:
        raise ValueError(f"unknown timm_regnet build inputs: {sorted(unknown)}")
