...
pytest.importorskip("tensorrt", reason="Qwen-VL precision contracts require TensorRT")

from families.qwen_vl import graph_blocks, graph_ops  # noqa: E402
from families.qwen_vl.config import get_rope_scaling  # noqa: E402
from families.qwen_vl.qwen_vl_vision_builder import (  # noqa: E402
...
class _Network:
    ...
    def add_activation(self, tensor: _Tensor, operation: object) -> _Layer:
        self.operations.append(("activation", tensor.name, tensor.dtype, operation))
    ...
    def add_elementwise(self, lhs: _Tensor, rhs: _Tensor, operation: object) -> _Layer:
        self.operations.append(("elementwise", lhs.name, lhs.dtype, rhs.name, rhs.dtype, operation))
...
def test_qwen3_position_interpolation_matches_bf16_operation_order() -> None:
    ...
    actual = _interpolate_qwen3_position_bf16(embeddings, positions, weights)
...
def test_bf16_swiglu_uses_fp32_silu_then_bf16_up_product(monkeypatch) -> None:
    network = _Network()
    ...
    monkeypatch.setattr(graph_blocks, "_make_matmul_fn", fake_make_matmul_fn)
    ...
    assert network.operations[0] == ("cast", "gate", trt.bfloat16, trt.float32)
...
def test_bf16_silu_uses_fp32_internal_compute() -> None:
    ...
    result = graph_ops.add_silu(network, _Tensor("input", trt.bfloat16))
...
def test_bf16_gelu_uses_fp32_internal_compute(monkeypatch) -> None:
    ...
    result = graph_ops.add_gelu_new(network, _Tensor("input", trt.bfloat16), dtype=np.float16)

    assert result.dtype == trt.bfloat16
    assert network.operations[0] == ("cast", "input", trt.bfloat16, trt.float32)
    assert network.operations[-1] == (
        "cast",
        "elementwise",
        trt.float32,
...
def test_bf16_vision_rope_computes_in_fp32_then_publishes_bf16() -> None:
    ...
    rotary = next(operation for operation in network.operations if operation[0] == "rotary")
    assert rotary == (
        "rotary",
        trt.float32,
        trt.float32,
