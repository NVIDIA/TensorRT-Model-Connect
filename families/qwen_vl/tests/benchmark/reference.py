...
"""Run the official Transformers Qwen-VL reference for qualification."""
...
def _prepare(reference: Reference, image_path: str, prompt: str) -> dict[str, Any]:
    ...
    rendered = reference.processor.apply_chat_template(
        [
            {
                "role": "user",
                "content": [
    ...
    if not isinstance(rendered, str) or not rendered.strip():
        raise ValueError("Qwen-VL chat template produced an empty prompt")
    if not any(marker in rendered for marker in ("<|image_pad|>", "<|vision_start|>", "<image>")):
        raise ValueError("Qwen-VL chat template lost the image placeholder")
    encoded = reference.processor(text=rendered, images=image, padding=True, return_tensors="pt")
    return {
        name: value.to(
            device="cuda",
...
def _infer(reference: Reference, inputs: Mapping[str, Any], max_new_tokens: int) -> dict[str, Any]:
    import torch

    with torch.inference_mode():
        generated = reference.model.generate(
            **inputs,
    ...
    tokens = sequence[input_length:] if sequence.shape[0] > input_length else sequence
    token_ids = [int(token) for token in tokens.detach().cpu().tolist()]
    return {
        "text": reference.processor.decode(tokens, skip_special_tokens=True),
        "token_ids": token_ids,
...
def _accuracy(arguments: argparse.Namespace) -> int:
    ...
    prompt = str(controls.get("prompt", ""))
    max_new_tokens = int(controls.get("max_new_tokens", 10))
    results = []
        ...
        results.append(
            {
                ...
                **_infer(reference, inputs, max_new_tokens),
            }
        )
...
def _performance(arguments: argparse.Namespace) -> int:
    ...
    if arguments.family != "qwen_vl" or arguments.operation != "generate":
        raise ValueError("Qwen-VL reference only supports qwen_vl generate")
    if arguments.mode != "pytorch-eager":
        raise ValueError("Qwen-VL reference only supports pytorch-eager")
    ...
    inputs = _prepare(reference, str(request["image_path"]), str(request.get("prompt", "")))
    max_new_tokens = int(request.get("max_new_tokens", 10))
    ...
    for _ in range(arguments.iterations):
        ...
        output_summary = _infer(reference, inputs, max_new_tokens)
        torch.cuda.synchronize()
