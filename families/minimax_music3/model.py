...
def build(request: "BuildRequest", writer: "BundleWriter") -> None:
    """Build one family-owned MiniMax-Music3 audio-generation bundle."""
    if request.task != "audio_generation":
        raise ValueError("minimax_music3 supports only task=audio_generation")
    if request.precision != "bf16":
        raise ValueError("MiniMax-Music3 requires precision=bf16")
    if request.backend != "trt":
        raise NotImplementedError("MiniMax-Music3 supports only backend=trt")
    if request.tensor_parallel_size != 1 or request.context_parallel_size != 1:
        raise NotImplementedError("MiniMax-Music3 requires single-GPU parallelism")
    if request.max_batch_size != 1:
        raise NotImplementedError("MiniMax-Music3 requires max_batch_size=1")
    if request.quantization not in {None, "none"}:
        raise NotImplementedError("MiniMax-Music3 does not support quantization")
    ...
    if any(
        ...
        raise NotImplementedError("MiniMax-Music3 does not accept image or video build shapes")

    model = _MiniMaxMusic3Model()
    weights = model.load_weights(str(request.model_dir), None)
    cache_length = int(request.max_sequence_length or 9216)
    writer.set_header(family="minimax_music3", task=request.task, backend=request.backend)
    writer.add_bytes(
        "dit.plan",
        model.build_engine(
            None,
    ...
    for old_name, data in extras.items():
        writer.add_bytes(old_name.removesuffix("_plan") + ".plan", data)
    tokenizer = model.tokenizer_json_bundle_override(request.model_dir)
    if tokenizer is None:
        raise FileNotFoundError("MiniMax-Music3 checkpoint is missing tokenizer/tokenizer.json")
    writer.add_bytes("tokenizer.json", tokenizer)
    writer.add_json("runtime.json", model.get_bundle_config_overrides(None))
