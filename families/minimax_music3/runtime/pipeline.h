...
struct MinimaxMusic3Config {
    ...
    int64_t seed{-1};
    // The checkpoint's family-owned sampling defaults.
    int32_t top_k{50};
...
struct MusicGenerationConfig {
    ...
    float temperature{1.0F};
    int32_t seed{-1};
    int32_t num_steps{30};
};

class MinimaxMusic3TextToMusicPipeline final : public IAudioGeneration {
  public:
  ...
  private:
    ...
    std::vector<int32_t> tokenize_prompt(const std::string& caption,
    ...
    std::vector<float> collect_frame_states(const std::vector<int32_t>& prompt_ids, int32_t frames,
                                            const MusicGenerationConfig& cfg, int32_t& emitted);
    ...
    MusicGenerationConfig sampling_config(const MusicGenerationConfig& cfg) const;
    ...
    void sample_residual_codes(const DepthStep& step, const MusicGenerationConfig& cfg,
                               uint64_t& rng_state);
