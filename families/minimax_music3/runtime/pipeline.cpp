...
#include "families/minimax_music3/runtime/prompt_format.h"
...
int32_t sample(const float* logits, int32_t count, const MusicGenerationConfig& cfg,
               std::mt19937_64& rng) {
...
int32_t sample_audio_code(const float* logits, int32_t vocab, const MusicGenerationConfig& cfg,
                          std::mt19937_64& rng) {
...
std::vector<int32_t>
MinimaxMusic3TextToMusicPipeline::tokenize_prompt(const std::string& caption,
    ...
    const auto assembled = minimax_music3::assemble_prompt(caption, lyrics);
...
std::vector<float> MinimaxMusic3TextToMusicPipeline::collect_frame_states(
    const std::vector<int32_t>& prompt_ids, int32_t frames, const MusicGenerationConfig& cfg,
    int32_t& emitted) {
...
AudioResult MinimaxMusic3TextToMusicPipeline::generate_audio(const std::string& prompt,
    ...
    const auto prompt_ids = tokenize_prompt(cfg.description, prompt);
    const int32_t requested = std::min(
        config_.max_frames, cfg.max_new_tokens > 0 ? cfg.max_new_tokens : config_.max_audio_frames);
    const MusicGenerationConfig generation{config_.top_k, config_.temperature, cfg.seed,
                                           config_.default_inference_steps};

    int32_t emitted = 0;
    const auto frame_hidden = collect_frame_states(prompt_ids, requested, generation, emitted);
    if (emitted <= 0)
    ...
    const int32_t steps = generation.num_steps;
    ...
    for (std::size_t window = 0; window < starts.size(); ++window) {
        ...
        const auto latents =
            denoise_window(condition, latent_length, steps,
                           static_cast<uint64_t>(generation.seed) + window, previous);
...
void MinimaxMusic3TextToMusicPipeline::sample_residual_codes(const DepthStep& step,
                                                             const MusicGenerationConfig& cfg,
                                                             uint64_t& rng_state) {
...
// The checkpoint's draw defaults are stored in the family bundle.
MusicGenerationConfig
MinimaxMusic3TextToMusicPipeline::sampling_config(const MusicGenerationConfig& cfg) const {
    MusicGenerationConfig draw = cfg;
    if (draw.top_k == 1)
...
MinimaxMusic3TextToMusicPipeline::generate_codes(const std::vector<int32_t>& prompt_ids,
                                                 int32_t frames, const MusicGenerationConfig& cfg,
    ...
    const MusicGenerationConfig draw = sampling_config(cfg);
