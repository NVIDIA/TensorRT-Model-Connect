...
class FakeAlalbert final : public trtmc::IEncoding,
...
void albert_contract(const fs::path& root) {
    const auto path = root / "albert.bundle";
    write_bundle(path, "albert");
    ...
    check(encoded.first == 0 && Json::parse(encoded.second.output).at("values") == Json({3.0, 1.0}),
          "ALBERT owner converts the encoding request and preserves result fields");
    check(loaded_runtime == "/installed/runtime",
          "ALBERT owner uses the supplied installed runtime root");
    ...
    check(embedded.first == 0 &&
          ...
          "ALBERT owner selects the embedding interface");
    check(loaded_runtime == "/override/runtime", "runtime_root override remains owned by ALBERT");
...
std::unique_ptr<ITask> load_task(const BundleReader&, const std::string& runtime_root,
    ...
    return std::make_unique<FakeAlalbert>();
}
} // namespace trtmc

int main() {
    const auto root = fs::temp_directory_path() / ("trtmc-albert-cli-" + std::to_string(getpid()));
    fs::create_directories(root);
    try {
        albert_contract(root);
