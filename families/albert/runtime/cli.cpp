...
Interface& require_interface(trtmc::ITask& task) {
    ...
    if (result == nullptr)
        throw std::invalid_argument("ALBERT bundle does not implement the requested interface");
    return *result;
}
...
nlohmann::json execute(const std::string& handler, const nlohmann::json& values,
    ...
    if (handler != "encode" && handler != "embed" && handler != "rerank")
        throw std::invalid_argument("unknown ALBERT CLI handler: " + handler);
    // Check identity before loading: a ALBERT command must not execute another
    ...
    if (reader.info().family != "albert")
        throw std::invalid_argument("ALBERT CLI requires a albert bundle");
    ...
    if (handler == "rerank") {
        ...
        if (!std::isfinite(score))
            throw std::runtime_error("ALBERT reranking returned a non-finite score");
        return {{"score", score}};
    }
    ...
    for (const auto value : result.data) {
        if (!std::isfinite(value))
            throw std::runtime_error("ALBERT embedding returned a non-finite value");
    }
