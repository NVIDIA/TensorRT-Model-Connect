...
// Intent:         LancePipeline text-only generation with mock engines; constructor
//                 validation (null decoder/cache); LanceConfig sync from
//                 LancePreprocessConfig; zero max_tokens early exit; string-based
...
// =============================================================================
// Test suite: LancePipeline — vision-language generation
// =============================================================================

#include "core/runtime/primitives/trt_common.h"
#include "core/runtime/tensorrt/trt_module_impl.h"
#include "families/lance/runtime/kv_cache.h"
#include "families/lance/runtime/pipeline.h"
#include "families/lance/runtime/tokenizer.h"
...
static trtmc::TrtUniquePtr<nvinfer1::ICudaEngine> build_mock_decoder() {
    ...
    auto* mask =
        n->addInput("attention_mask", nvinfer1::DataType::kFLOAT, nvinfer1::Dims{2, {1, 9}});
    auto* cache_k = n->addInput("cache_k_0", nvinfer1::DataType::kFLOAT, nvinfer1::Dims{2, {8, 4}});
    auto* cache_v = n->addInput("cache_v_0", nvinfer1::DataType::kFLOAT, nvinfer1::Dims{2, {8, 4}});
    ...
    auto* token_out = n->addIdentity(*tok)->getOutput(0);
    token_out->setName("_t");
    n->markOutput(*token_out);
    auto* mask_out = n->addIdentity(*mask)->getOutput(0);
    mask_out->setName("_m");
    n->markOutput(*mask_out);
    auto* present_k = n->addSlice(*cache_k, nvinfer1::Dims{2, {0, 0}}, nvinfer1::Dims{2, {1, 4}},
                                  nvinfer1::Dims{2, {1, 1}})
                          ->getOutput(0);
    present_k->setName("present_k_0");
    n->markOutput(*present_k);
    auto* present_v = n->addSlice(*cache_v, nvinfer1::Dims{2, {0, 0}}, nvinfer1::Dims{2, {1, 4}},
                                  nvinfer1::Dims{2, {1, 1}})
                          ->getOutput(0);
    present_v->setName("present_v_0");
    n->markOutput(*present_v);
...
static void test_vl_text_only() {
    ...
    trtmc::LancePipeline pipeline(std::move(decoder), nullptr, std::move(cache), cfg, vl_pp,
                                  stream);
    check(std::string(pipeline.task()) == trtmc::IVisionLanguageGeneration::kTask,
          "vl task contract");
...
static void test_vl_validates_decoder() {
    ...
    bool threw = false;
    try {
        trtmc::LancePipeline p(nullptr, nullptr, std::move(cache), cfg, vl_pp, stream);
...
static void test_vl_validates_cache() {
    ...
    bool threw = false;
    try {
        trtmc::LancePipeline p(std::move(decoder), nullptr, nullptr, cfg, vl_pp, stream);
    } catch (const std::exception&) {
        threw = true;
    }
...
static void test_vl_config_sync() {
    // LancePreprocessConfig has image_token_id=1 and vision_output_dim=64,
    // LanceConfig has image_token_id=-1 and vision_output_dim=0.
    ...
    // Construction succeeded; the current abstract task contract is preserved.
    check(std::string(pipeline.task()) == trtmc::IVisionLanguageGeneration::kTask,
          "config_sync: task correct");
...
static void test_vl_generate_with_image_no_encoder() {
    ...
    trtmc::LancePipeline pipeline(std::move(decoder), nullptr, std::move(cache), cfg, vl_pp, stream,
                                  tokenizer);
...
static void test_vl_generate_with_vision_encoder() {
    ...
    //   convert_float_to_decoded (lines 64-79), lance_preprocess_decoded_image with simple_chw,
...
static trtmc::TrtUniquePtr<nvinfer1::ICudaEngine> build_mock_decoder_with_embed() {
    ...
    auto* mask =
        n->addInput("attention_mask", nvinfer1::DataType::kFLOAT, nvinfer1::Dims{2, {1, 9}});
    auto* ue =
        n->addInput("use_input_embed", nvinfer1::DataType::kFLOAT, nvinfer1::Dims{2, {1, 1}});
    auto* emb = n->addInput("input_embed", nvinfer1::DataType::kFLOAT, nvinfer1::Dims{2, {1, 4}});
    auto* cache_k = n->addInput("cache_k_0", nvinfer1::DataType::kFLOAT, nvinfer1::Dims{2, {8, 4}});
    auto* cache_v = n->addInput("cache_v_0", nvinfer1::DataType::kFLOAT, nvinfer1::Dims{2, {8, 4}});
    ...
    for (auto [input, name] : {std::pair{tok, "_t"}, std::pair{mask, "_m"}, std::pair{ue, "_ue"},
                               std::pair{emb, "_e"}}) {
        auto* output = n->addIdentity(*input)->getOutput(0);
        output->setName(name);
        n->markOutput(*output);
    }
    auto* present_k = n->addSlice(*cache_k, nvinfer1::Dims{2, {0, 0}}, nvinfer1::Dims{2, {1, 4}},
...
static void test_vl_generate_with_embed_decoder() {
    // Covers LancePipeline::run_text_step_with_embed() full body (lines 285-361).
...
static void test_vl_sequence_prefill_uses_one_text_launch() {
    ...
    auto tokenizer = std::make_shared<VLFixedTokenizer>();
    trtmc::LancePipeline pipeline(std::move(decoder), std::move(vision), std::move(cache), cfg,
                                  vl_pp, stream, tokenizer, "", std::move(prefill));
