...
class FakeMpnet final : public trtmc::IEncoding,
...
void mpnet_contract(const fs::path& root) {
    const auto path = root / "mpnet.bundle";
    write_bundle(path, "mpnet");
    ...
    check(encoded.first == 0 && Json::parse(encoded.second.output).at("values") == Json({3.0, 1.0}),
          "MPNET owner converts the encoding request and preserves result fields");
    check(loaded_runtime == "/installed/runtime",
          "MPNET owner uses the supplied installed runtime root");
    ...
    check(embedded.first == 0 &&
          ...
          "MPNET owner selects the embedding interface");
    check(loaded_runtime == "/override/runtime", "runtime_root override remains owned by MPNET");
...
std::unique_ptr<ITask> load_task(const BundleReader&, const std::string& runtime_root,
    ...
    return std::make_unique<FakeMpnet>();
}
} // namespace trtmc

int main() {
    const auto root = fs::temp_directory_path() / ("trtmc-mpnet-cli-" + std::to_string(getpid()));
