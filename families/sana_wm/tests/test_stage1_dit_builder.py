...
from families.sana_wm.checkpoint_mapper import WeightDict  # noqa: E402
from families.sana_wm import stage1_dit_builder  # noqa: E402
...
def test_load_sana_wm_stage1_dit_weights_uses_trt_layouts(tmp_path) -> None:
    ...
    np.testing.assert_allclose(
        ...
        linear.T.astype(ml_dtypes.bfloat16),
    )
    assert weights["y_embedder.y_proj.fc1.weight"].dtype == ml_dtypes.bfloat16
    assert weights["x_embedder.proj.weight"].shape == conv.shape
    assert weights["x_embedder.proj.weight"].dtype == ml_dtypes.bfloat16
...
def test_stage1_mlp_silu_uses_fp32_roundtrip() -> None:
    ml_dtypes = pytest.importorskip("ml_dtypes")
    network = _FakeNetwork()
...
def test_add_rmsnorm_uses_runtime_rounded_weight_values() -> None:
    ...
    np.testing.assert_array_equal(
        ...
        weight.astype(ml_dtypes.bfloat16).astype(np.float32).reshape((1, 4)),
    )
...
class _FakeNetwork:
    def __init__(self, flags: int = 0) -> None:
        ...
        self.paddings: list[_FakePadding] = []
    ...
    def add_padding_nd(self, inp, *, pre_padding, post_padding):  # noqa: ANN001
        layer = _FakePadding(inp, tuple(pre_padding), tuple(post_padding))
...
class _FakePadding:
    def __init__(self, inp, pre_padding, post_padding) -> None:  # noqa: ANN001
        self.inp = inp
        self.pre_padding = pre_padding
        self.post_padding = post_padding
        self.output = _FakeTensor("padding", dtype=getattr(inp, "dtype", None))
...
class _FakePluginRegistry:
    def __init__(self) -> None:
        self.creators: dict[str, _FakePluginCreator] = {}

    def get_creator(self, name: str, version: str, namespace: str):
        assert version == "1"
        assert namespace == ""
        return self.creators.setdefault(name, _FakePluginCreator())
...
class _FakeTrtWithPlugin(_FakeTrt):
    ...
    _plugin_registry = _FakePluginRegistry()

    @classmethod
    def get_plugin_registry(cls) -> _FakePluginRegistry:
...
def test_define_sana_wm_stage1_inputs_names_shapes_and_dtypes() -> None:
    ...
    assert network.inputs == [
        ("x", "bfloat16", (2, 128, 41, 22, 40)),
        ("timestep", "float32", (2, 1, 41)),
        ("y", "bfloat16", (2, 1, 300, 2304)),
        ("mask", "int32", (2, 300)),
        ("camera_conditions", "bfloat16", (2, 41, 20)),
        ("raymats", "float32", (2, 41 * 22 * 40, 4, 4)),
        ("raymats_inv", "float32", (2, 41 * 22 * 40, 4, 4)),
        ("chunk_plucker", "bfloat16", (2, 48, 41, 22, 40)),
    ]
...
def test_lower_sana_wm_stage1_frontend_embeds_latents_and_plucker() -> None:
    ...
    assert network.convolutions == []
    assert [layer.plugin.name for layer in network.plugins] == [
        "sana_wm_torch_conv2d",
        "sana_wm_torch_conv2d",
    ]
    assert len(network.shuffles) == 6
    assert network.shuffles[0].first_transpose.values == [0, 2, 1, 3, 4]
    assert network.shuffles[2].reshape_dims == (2, 41 * 22 * 40, 2240)
    assert network.shuffles[5].reshape_dims == (2, 41 * 22 * 40, 2240)
...
def test_lower_sana_wm_stage1_conditioning_embeds_timestep_and_text() -> None:
    ...
    assert network.matrix_multiply == []
    assert [layer.plugin.name for layer in network.plugins] == [
        "sana_wm_timestep_embed",
        "sana_wm_caption_embed",
    ]
    ...
    assert [(layer.dtype, layer.inp.name) for layer in network.casts] == [
        ("bfloat16", "timestep.frequency_embedding"),
    ]
    assert [layer.op for layer in network.unary] == ["cos", "sin"]
    assert [layer.shape for layer in network.constants] == [
        (1, 1, 1, 128),
        (2304, 2240),
        (2240,),
        (2240, 2240),
...
def test_lower_sana_wm_stage1_block_preamble_builds_adaln_and_qkv() -> None:
    ...
    frontend = stage1_dit_builder.SanaWmStage1Frontend(
        x_tokens=_FakeTensor("x_tokens", dtype=_FakeTrtWithBf16Plugin.bfloat16),
        token_count=41 * 22 * 40,
        hidden_size=2240,
        plucker_tokens=_FakeTensor("plucker_tokens", dtype=_FakeTrtWithBf16Plugin.bfloat16),
    )
    conditioning = stage1_dit_builder.SanaWmStage1Conditioning(
        t=_FakeTensor("t_embedder.output", dtype=_FakeTrtWithBf16Plugin.bfloat16),
        t0=_FakeTensor("t_block.output", dtype=_FakeTrtWithBf16Plugin.bfloat16),
        y=_FakeTensor("y_embedder.output", dtype=_FakeTrtWithBf16Plugin.bfloat16),
    ...
    assert [layer.activation_type for layer in network.activations] == ["relu", "relu"]
    assert [layer.plugin.name for layer in network.plugins] == [
        "sana_wm_layer_norm",
        "sana_wm_t2i_modulate",
        "sana_wm_short_conv",
        "sana_wm_gate_proj",
        "sana_wm_frame_mean",
        "sana_wm_gate_proj",
    ...
    assert len(network.matrix_multiply) == 1
...
def test_lower_sana_wm_stage1_gdn_core_uses_raw_fused_plugin(monkeypatch) -> None:
    ...
    np.testing.assert_array_equal(
        ...
        q_norm_weight.astype(ml_dtypes.bfloat16).astype(np.float32),
    )
    np.testing.assert_array_equal(
        network.constants[1].weights.value,
        k_norm_weight.astype(ml_dtypes.bfloat16).astype(np.float32),
    )
...
def test_lower_sana_wm_stage1_camera_uses_combined_plugin(monkeypatch) -> None:
    ml_dtypes = pytest.importorskip("ml_dtypes")
    creator = _FakePluginCreator()
    monkeypatch.setattr(
        stage1_dit_builder,
...
def test_lower_sana_wm_stage1_camera_preamble_reaches_ucpe_inputs() -> None:
    ...
    assert network.convolutions == []
    assert [layer.plugin.name for layer in network.plugins] == ["sana_wm_short_conv"]
    assert [layer.op for layer in network.reductions] == ["avg", "avg"]
...
def test_lower_sana_wm_stage1_camera_ucpe_stabilizes_and_discounts_beta() -> None:
    ...
    camera = stage1_dit_builder.SanaWmStage1CameraPreamble(
        q=_FakeTensor("q_cam", dtype=_FakeTrtWithBf16Plugin.bfloat16),
        k=_FakeTensor("k_cam", dtype=_FakeTrtWithBf16Plugin.bfloat16),
        v=_FakeTensor("v_cam", dtype=_FakeTrtWithBf16Plugin.bfloat16),
        num_heads=2,
        head_dim=8,
    )
    ...
    ucpe = stage1_dit_builder.lower_sana_wm_stage1_camera_ucpe(
        ...
        preamble,
        {
            "raymats": _FakeTensor("raymats", dtype=_FakeTrtWithBf16Plugin.float32),
            "raymats_inv": _FakeTensor("raymats_inv", dtype=_FakeTrtWithBf16Plugin.float32),
        },
        shape,
        _raw_sana_wm_config(),
        trt_module=_FakeTrtWithBf16Plugin,
    ...
    assert any(layer.reshape_dims == (1, 1, 4, 2, 1) for layer in network.shuffles)
...
def test_apply_ucpe_block_uses_registered_plugin_by_default(monkeypatch) -> None:
    ...
    out = stage1_dit_builder._apply_ucpe_block_diagonal_to_bhnd(
        network,
        _FakeTensor("feats", dtype=_FakeTrtWithBf16Plugin.bfloat16),
        _FakeTensor("raymats", dtype=_FakeTrtWithBf16Plugin.float32),
        _FakeTensor("cos", dtype=_FakeTrtWithBf16Plugin.float32),
        _FakeTensor("sin", dtype=_FakeTrtWithBf16Plugin.float32),
        shape,
        num_heads=2,
        head_dim=8,
        inverse_rope=True,
...
def test_lower_sana_wm_stage1_camera_softmax_uses_hf_padded_scale() -> None:
    ...
    out = stage1_dit_builder._lower_softmax_attention_bhnd(
        network,
        _FakeTensor("q", dtype=_FakeTrtWithBf16Plugin.bfloat16),
        _FakeTensor("k", dtype=_FakeTrtWithBf16Plugin.bfloat16),
        _FakeTensor("v", dtype=_FakeTrtWithBf16Plugin.bfloat16),
        head_dim=112,
        scale_head_dim=128,
        trt_module=_FakeTrtWithBf16Plugin,
        dtype=ml_dtypes.bfloat16,
    ...
    scales = [
        float(np.asarray(value, dtype=np.float32).reshape(-1)[0])
        for value in stage1_dit_builder._BF16_WEIGHT_REFS
        if np.asarray(value).shape == (1, 1, 1, 1)
    ]
    assert out.name == "blocks.3.attn.cam_softmax.out_bhnd"
    assert len(network.attentions) == 1
    assert len(network.matrix_multiply) == 0
    assert network.attentions[0].q is network.paddings[0].get_output(0)
    assert network.paddings[0].post_padding == (0, 16)
    assert np.isclose(scales[-1], float(ml_dtypes.bfloat16(128**-0.5)))
    assert not np.isclose(scales[-1], float(ml_dtypes.bfloat16(112**-0.5)))
...
def test_lower_sana_wm_stage1_block_post_attention_lowers_cross_plucker_and_mlp() -> None:
    ...
    assert network.softmax == []
    assert network.convolutions == []
    assert network.matrix_multiply == []
    assert [layer.plugin.name for layer in network.plugins] == [
        "sana_wm_frame_gate",
        "sana_wm_gate_proj",
        "sana_wm_cross_attention",
        "sana_wm_layer_norm",
        "sana_wm_t2i_modulate",
        "sana_wm_glumbconvtemp",
        "sana_wm_frame_gate",
    ]
...
def test_lower_sana_wm_stage1_final_layer_matches_upstream_frame_aware_path() -> None:
    ...
    assert network.reductions == []
    assert network.unary == []
    assert [layer.plugin.name for layer in network.plugins] == [
        "sana_wm_layer_norm",
        "sana_wm_t2i_modulate",
    ]
...
def test_build_sana_wm_stage1_dit_engine_starts_direct_trt_network(monkeypatch) -> None:
    ...
    monkeypatch.setattr(stage1_dit_builder, "trt", _FakeTrtWithBf16Plugin)
    stale_ref = np.zeros((1,), dtype=np.float32)
    stage1_dit_builder._BF16_WEIGHT_REFS.append(stale_ref)
    ...
    assert all(item is not stale_ref for item in stage1_dit_builder._BF16_WEIGHT_REFS)
    assert builder.config.builder_optimization_level == 1
    assert builder.config.pool_limits == [("workspace", 64 << 30)]
    assert builder.network is not None
    assert builder.network.flags == 1
    assert builder.network.inputs[0] == ("x", "bfloat16", (2, 128, 41, 22, 40))
    assert builder.network.convolutions == []
    assert len(builder.network.plugins) == 33
    assert [layer.plugin.name for layer in builder.network.plugins[:4]] == [
        "sana_wm_torch_conv2d",
        "sana_wm_torch_conv2d",
        "sana_wm_timestep_embed",
        "sana_wm_caption_embed",
    ]
    assert [layer.plugin.name for layer in builder.network.plugins[-2:]] == [
        "sana_wm_layer_norm",
        "sana_wm_t2i_modulate",
    ]
    assert len(builder.network.matrix_multiply) == 9
    assert len(builder.network.slices) == 21
    assert builder.network.inputs[6] == ("raymats_inv", "float32", (2, 41 * 22 * 40, 4, 4))
    assert builder.network.softmax == []
...
def test_build_sana_wm_stage1_dit_engine_lowers_hybrid_softmax_block(monkeypatch) -> None:
    ...
    assert builder.network is not None
    assert builder.network.softmax == []
    assert builder.network.attentions == []
    assert [layer.plugin.name for layer in builder.network.plugins].count(
        "sana_wm_softmax_attention"
    ) == 2
    assert len(builder.network.plugins) == 24
    assert len(builder.network.matrix_multiply) == 6
