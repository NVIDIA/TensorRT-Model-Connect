...
#include "families/eagle_vlm/runtime/distributed_runtime.h"
...
TRTMC_DEFINE_FAMILY_PLUGIN_V1("eagle_vlm")

extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    if (context.kv_cache_size_bytes != 0)
        throw std::invalid_argument("eagle_vlm does not support --kv-cache-size");
    ...
    const auto group = trtmc::eagle_vlm::initialize_tensor_parallel_group(tp_size);
