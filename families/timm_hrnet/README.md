...
```sh
trtmc build timm/hrnet_w18.ms_aug_in1k -o model.bundle
...
```sh
cmake --build build --target test_timm_hrnet_task_contract test_timm_hrnet_image_preprocess
ctest --test-dir build --output-on-failure -R '^timm_hrnet_(task_contract|image_preprocess)$'
```
...
```sh
build/test_timm_hrnet_sdk_c model.bundle build image.rgb.f32 480 640
build/test_timm_hrnet_sdk_cpp model.bundle build image.rgb.f32 480 640
```
...
`tests/performance.yaml` takes over the existing `timm_hrnet.classify`
