...
#include "families/codegen/runtime/pipeline.h"

#include "families/codegen/runtime/chat_templates.h"
#include "families/codegen/runtime/kv_cache.h"
#include "families/codegen/runtime/tensor_names.h"
...
CodegenTextGenerationPipeline::CodegenTextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<CodegenInferenceState> state,
    CodegenTextGenConfig config, std::shared_ptr<ITokenizer> tokenizer,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("CodegenTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("CodegenTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
TextResult CodegenTextGenerationPipeline::run(const internal::TextContinuationRequest& request,
                                              internal::ConfigView supplied) {
    const auto cfg = codegen::parse_text_config(supplied);
    ...
    if (const auto* prompt = std::get_if<std::string_view>(&request.prefix)) {
        ...
        input_ids = codegen::copy_token_prefix(std::get<Span<const std::int32_t>>(request.prefix));
    }
...
void require_prefill_kv_pointers(ITrtModule& prefill, const CodegenTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(codegen_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(codegen_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "CodegenTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const CodegenTextGenConfig& cfg,
                                      int32_t sq, CodegenInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("CodegenTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "CodegenTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("CodegenTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "CodegenTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<CodegenKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("CodegenTextGenerationPipeline: prefill requires CodegenKvCache");
}

} // namespace

void CodegenTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error(
            "CodegenTextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(lt.numel()) < vocab)
        throw std::runtime_error(
            "CodegenTextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
CodegenTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
CodegenTextGenerationPipeline::TimedGenResult CodegenTextGenerationPipeline::generate_from_ids(
    ...
    const CodegenSamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("CodegenTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_codegen_sampler(params);
...
int32_t CodegenTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const CodegenSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& CodegenTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
