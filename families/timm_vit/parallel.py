...
"""timm ViT-owned tensor-parallel image-classification builder."""
...
class ParallelConfig:
    ...
    def validate(self) -> None:
        ...
        if self.rank < -1 or self.rank >= self.tp_size:
            raise ValueError("timm ViT tensor-parallel rank is outside the requested world")
...
def add_all_reduce_sum(network, tensor, tp_size: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT failed to create timm ViT ALL_REDUCE")
...
def _validate_timm_vit_tp(config: "ModelConfig", parallel: "ParallelConfig") -> None:
    ...
    vit_cfg = config.raw["_timm_vit_config"]
