...
"""CPU contracts for the timm_vit command boundary."""
...
from families.timm_vit import cli
...
def test_ignored_legacy_settings_do_not_become_new_cli_options(tmp_path):
    ...
    assert cli.coerce_request(replace(legacy, max_sequence_length=7)) == request
    with pytest.raises(ValueError, match="positive integer"):
        cli.coerce_request(SimpleNamespace(**{**vars(legacy), "max_sequence_length": -1}))
...
def test_tensor_parallel_build_is_family_owned(monkeypatch, tmp_path):
    calls = []
    monkeypatch.setattr(cli, "build_bundle", lambda request, output: calls.append(request))
    assert (
        family_cli.main(
            [
                FAMILY,
    ...
    with pytest.raises(ValueError, match="one of"):
        cli.BuildRequest(tmp_path, tensor_parallel_size=3)
