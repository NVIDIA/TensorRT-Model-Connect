...
"""Direct build, native-runtime, and official-reference E2E for timm_vit."""
...
import pytest
import numpy as np
from families.timm_vit.cli import BuildRequest, build_bundle
...
def _assert_sdk_consumers(
    ...
    for language in ("c", "cpp"):
        consumer = native_build / f"test_timm_vit_sdk_{language}"
...
def test_official_checkpoint_e2e(case_name: str, tmp_path: Path) -> None:
    ...
    with evidence_stage("compare"):
        ...
        assert actual["labels"] == model_config.get("label_names", [])
        if int(manifest["tensor_parallel_size"]) == 1:
            _assert_sdk_consumers(runtime_root, bundle, case, model_config, int(np.argmax(expected["logits"])), tmp_path)
