...
"""Family-owned timm_vit commands with lazy builder imports."""
...
class BuildRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if str(self.precision).lower() not in {"fp16", "fp32"}:
            raise ValueError("timm_vit supports only fp16 and fp32 precision")
        ...
        if type(self.tensor_parallel_size) is not int or self.tensor_parallel_size not in {
            ...
            raise ValueError("timm ViT tensor_parallel_size must be one of 1, 2, 4, 8")
...
def coerce_request(request: object) -> BuildRequest:
    ...
    if unknown := set(vars(request)) - allowed:
        raise ValueError(f"unknown timm_vit build inputs: {sorted(unknown)}")
    return BuildRequest(
        request.model_dir,
        request.task,
        request.precision,
        request.backend,
        request.verbose,
        tensor_parallel_size=_positive_int(request.tensor_parallel_size, "tensor_parallel_size"),
    )
