...
ClassificationResult TimmInceptionResNetImageClassificationPipeline::classify(const float* pixels,
                                                                              int32_t height,
                                                                              int32_t width) {
    auto values = preprocess_timm_inception_resnet_image(pixels, height, width, preprocess_config_);
    Tensor input;
    ...
    if (logits == nullptr || logits->numel() <= 0)
        throw std::runtime_error("timm InceptionResNet engine returned no logits");
    if (logits->dtype != DType::kFloat32)
        throw std::runtime_error("timm InceptionResNet logits must be float32");
