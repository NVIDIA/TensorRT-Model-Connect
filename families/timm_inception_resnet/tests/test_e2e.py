...
"""Direct build, native runtime, and timm reference proof for InceptionResNet."""
...
def pytest_generate_tests(metafunc) -> None:
    ...
    if not selected:
        names = [
            pytest.param(
                name,
                marks=pytest.mark.skip(reason="real InceptionResNet E2E requires explicit selection"),
                id=name,
            )
            for name in names
        ]
...
def _required_path(value: str | None, label: str) -> Path:
    assert value, f"selected InceptionResNet E2E requires {label}"
    path = Path(value)
    assert path.exists(), f"selected InceptionResNet E2E {label} does not exist: {path}"
    return path


def _model_dir(manifest: dict) -> Path:
    explicit = os.environ.get("TRTMC_TIMM_TIMM_INCEPTION_RESNET_MODEL_DIR")
    if explicit:
        return _required_path(explicit, "TRTMC_TIMM_TIMM_INCEPTION_RESNET_MODEL_DIR")
...
def _asset(case: dict) -> Path:
    ...
    assert path.is_file(), f"selected InceptionResNet E2E image is missing: {path}"
...
def _assert_sdk_consumers(
    ...
    for language in ("c", "cpp"):
        consumer = native_build / f"test_timm_inception_resnet_sdk_{language}"
