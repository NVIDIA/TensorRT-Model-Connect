...
"""Time Inception-ResNet-v2 inference and host logits, not classification/reporting."""
...
def _load_reference(arguments, request):
    ...
    model = timm.create_model(
        ...
        pretrained_cfg=config["pretrained_cfg"], num_classes=int(config["num_classes"]),
...
def _summary(scores):
    ...
    if scores.ndim != 2 or scores.shape[0] != 1 or scores.shape[1] == 0:
        raise ValueError("Inception-ResNet-v2 must return one nonempty class-score vector")
    if not np.isfinite(scores).all():
        raise ValueError("Inception-ResNet-v2 returned nonfinite class scores")
    return {
...
def run(arguments):
    ...
    if (arguments.family != "timm_inception_resnet" or arguments.operation != "classify"
            or arguments.selected_task != "image_to_class_scores"):
        raise ValueError("reference requires timm_inception_resnet image_to_class_scores/classify")
