...
"""Small TensorRT graph vocabulary owned by timm InceptionResNet."""
...
def convolution(
    ...
    *,
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a InceptionResNet convolution")
...
def relu(network, tensor):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a InceptionResNet ReLU")
    return layer.get_output(0)
...
def max_pool(network, tensor, *, kernel: int, stride: int, padding: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected an InceptionResNet max pool")
    layer.stride_nd = (stride, stride)
    layer.padding_nd = (padding, padding)
    return layer.get_output(0)
...
def average_pool(network, tensor, *, kernel: int, stride: int, padding: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected an InceptionResNet average pool")
    layer.stride_nd = (stride, stride)
    layer.padding_nd = (padding, padding)
...
def concatenate(network, tensors):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected an InceptionResNet concatenation")
    layer.axis = 1
    return layer.get_output(0)


def scale(network, tensor, factor: float, *, dtype: np.dtype):
    """Multiply a whole tensor by one constant."""
    layer = network.add_scale(
        tensor,
        trt.ScaleMode.UNIFORM,
        shift=trt.Weights(np.array([0.0], dtype=dtype)),
        scale=trt.Weights(np.array([factor], dtype=dtype)),
    )
    if layer is None:
        raise RuntimeError("TensorRT rejected an InceptionResNet residual scale")
    return layer.get_output(0)
...
def add(network, left, right):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected an InceptionResNet residual add")
    return layer.get_output(0)
...
def global_average_pool(network, tensor, height: int, width: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected InceptionResNet global average pooling")
    layer.stride_nd = (1, 1)
    return layer.get_output(0)
...
def classifier(
    ...
    *,
    ...
    if flattened is None:
        raise RuntimeError("TensorRT rejected the InceptionResNet classifier reshape")
    ...
    if matrix_layer is None:
        raise RuntimeError("TensorRT rejected the InceptionResNet classifier weights")
    ...
    if product is None:
        raise RuntimeError("TensorRT rejected the InceptionResNet classifier matmul")
    ...
    if bias_layer is None:
        raise RuntimeError("TensorRT rejected the InceptionResNet classifier bias")
    ...
    if output is None:
        raise RuntimeError("TensorRT rejected the InceptionResNet classifier output")
