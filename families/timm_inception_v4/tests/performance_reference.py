...
"""Time Inception-v4 inference and host logits, not classification/reporting."""
...
def _summary(scores):
    ...
    if scores.ndim != 2 or scores.shape[0] != 1 or scores.shape[1] == 0:
        raise ValueError("Inception-v4 must return one nonempty class-score vector")
    if not np.isfinite(scores).all():
        raise ValueError("Inception-v4 returned nonfinite class scores")
    return {
...
def run(arguments):
    ...
    if (arguments.family != "timm_inception_v4" or arguments.operation != "classify"
            or arguments.selected_task != "image_to_class_scores"):
        raise ValueError("reference requires timm_inception_v4 image_to_class_scores/classify")
