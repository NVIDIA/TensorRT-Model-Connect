...
"""Direct build, native runtime, and timm reference proof for InceptionV4."""
...
from families.timm_inception_v4.cli import BuildRequest, build_bundle


FAMILY = "timm_inception_v4"
...
def pytest_generate_tests(metafunc) -> None:
    ...
    if not selected:
        names = [
            pytest.param(
                name,
                marks=pytest.mark.skip(reason="real InceptionV4 E2E requires explicit selection"),
                id=name,
            )
            for name in names
        ]
...
def _required_path(value: str | None, label: str) -> Path:
    assert value, f"selected InceptionV4 E2E requires {label}"
    path = Path(value)
    assert path.exists(), f"selected InceptionV4 E2E {label} does not exist: {path}"
    return path


def _model_dir(manifest: dict) -> Path:
    explicit = os.environ.get("TRTMC_TIMM_TIMM_INCEPTION_V4_MODEL_DIR")
    if explicit:
        return _required_path(explicit, "TRTMC_TIMM_TIMM_INCEPTION_V4_MODEL_DIR")
...
def _asset(case: dict) -> Path:
    ...
    assert path.is_file(), f"selected InceptionV4 E2E image is missing: {path}"
    record_evidence("inputs", {"asset": path})
    return path


def _assert_class_metadata(actual: dict, config: dict) -> None:
    assert actual["score_kind"] == "logit"
    assert actual["scores"] == actual["logits"]
    assert len(actual["scores"]) == int(config["num_classes"])
    assert all(math.isfinite(value) for value in actual["scores"])
    assert actual["top_score"] == max(actual["scores"])
    assert actual["vocabulary_id"] == config.get("vocabulary_id", "")
    assert actual["labels"] == config.get("label_names", [])
...
def _assert_sdk_consumers(
    ...
    for language in ("c", "cpp"):
        consumer = native_build / f"test_timm_inception_v4_sdk_{language}"
...
def test_official_checkpoint_e2e(case_name: str, tmp_path: Path) -> None:
    ...
    with evidence_stage("compare"):
        ...
        _assert_class_metadata(actual, config)
    with evidence_stage("sdk"):
        _assert_sdk_consumers(runtime_root, bundle, case, config, int(np.argmax(expected)), tmp_path)
