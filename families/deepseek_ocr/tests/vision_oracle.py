...
def _runtime_config(bundle: Path) -> dict:
    return json.loads(_bundle_section(bundle, "runtime.json"))


def native_vision_features(bundle: Path, image) -> np.ndarray:
    pixels = _native_pixels(image, _runtime_config(bundle))
    return _execute_vision_plan(_bundle_section(bundle, "vision.plan"), {"pixel_values": pixels})


def official_vision_features(model, image, bundle: Path) -> np.ndarray:
    import torch

    pixels = torch.from_numpy(_native_pixels(image, _runtime_config(bundle))).to(
        device="cuda", dtype=model.dtype
    )
    core = model.model
    with torch.no_grad():
        features = core.sam_model(pixels)
        features = core.qwen2_model(features)
        features = core.projector(features).reshape(-1, core.view_seperator.shape[-1])
        features = torch.cat((features, core.view_seperator.reshape(1, -1)), dim=0)
