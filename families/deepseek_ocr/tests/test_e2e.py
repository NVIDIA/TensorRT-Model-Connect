...
def _official_reference(model_dir: Path, bundle: Path, manifest: dict, case: dict, tmp_path: Path):
    ...
    with tempfile.TemporaryDirectory(prefix="deepseek_ocr_hf_") as output_dir:
        text = model.infer(
            tokenizer,
            prompt=prompt,
            image_file=str(_asset(case["test_image"])),
...
def test_full_hf_route_requires_substrings_and_full_text_parity() -> None:
    required = "\n".join(_REQUIRED_OCR_SUBSTRINGS["deepseek-ocr"])
    thresholds = {"normalized_text_edit_distance": 0.5}
    _assert_parity(
        {"text": required},
        {"text": required},
    ...
    with pytest.raises(AssertionError):
        _assert_parity(
            {"text": required.replace("Vision: SAM ViT-B + Qwen2 encoder", "")},
            {"text": required},
            {"task": "vision_language_generation"},
            {"name": "deepseek-ocr"},
            thresholds,
    ...
    with pytest.raises(AssertionError):
        _assert_parity(
            {"text": required + " unrelated" * len(required)},
            {"text": required},
            {"task": "vision_language_generation"},
            {"name": "deepseek-ocr"},
            thresholds,
