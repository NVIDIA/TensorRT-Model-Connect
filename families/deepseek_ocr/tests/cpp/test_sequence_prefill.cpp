...
#include "families/deepseek_ocr/runtime/kv_cache.h"
#include "families/deepseek_ocr/runtime/pipeline.h"
#include "families/deepseek_ocr/runtime/tokenizer.h"
...
struct CountingTextStats {
    std::int32_t calls{0};
    std::unordered_map<std::string, std::vector<std::int64_t>> shapes;
    std::unordered_map<std::string, std::vector<std::int64_t>> bound_shapes;
...
class CountingTextModule final : public trtmc::ITrtModule {
  public:
    ...
    std::int32_t profile_idx() const override { return prefill_ ? 0 : 1; }
    ...
    std::int32_t optimization_profile_count() const override { return prefill_ ? 2 : 1; }
    ...
    void bind_external(const std::string& name, void*,
...
class FixedTokenizer final : public trtmc::ITokenizer {
  public:
    std::vector<std::int32_t> encode(const std::string&) const override { return {1, 2, 3}; }
    std::string decode(const std::vector<std::int32_t>&) const override { return "out"; }
...
void test_sequence_prefill_uses_one_text_launch(cudaStream_t stream) {
    ...
    trtmc::DeepseekOcrConfig config;
    config.vocab_size = 4;
    config.id_eos = 2;
    config.image_token_id = 1;
    config.vision_output_dim = 4;
    config.num_layers = 1;
    config.prefill_max_length = 8;

    trtmc::DeepseekOcrPreprocessConfig preprocess;
    ...
    trtmc::DeepseekOcrPipeline pipeline(std::move(decoder), std::move(vision), std::move(cache),
                                        config, preprocess, stream, tokenizer, "",
                                        std::move(prefill));

    float pixels[2 * 2 * 3] = {0.5F, 0.5F, 0.5F, 0.4F, 0.4F, 0.4F,
                               0.3F, 0.3F, 0.3F, 0.2F, 0.2F, 0.2F};
    trtmc::TextGenerationConfig generation;
    generation.max_new_tokens = 1;
    generation.eos_token_id = 2;
    const auto result = pipeline.generate("test", pixels, 2, 2, generation);

    check(result.token_ids == std::vector<std::int32_t>{2},
    ...
    check(prefill_stats->shapes["token_id"] == std::vector<std::int64_t>{3},
          "sequence prefill: token shape");
    check(prefill_stats->shapes["input_embed"] == std::vector<std::int64_t>({3, 4}),
          "sequence prefill: input embed shape");
    check(prefill_stats->shapes["use_input_embed"] == std::vector<std::int64_t>({3, 1}),
          "sequence prefill: embed selector shape");
    check(prefill_stats->shapes["deepstack_embed_0"] == std::vector<std::int64_t>({3, 4}),
          "sequence prefill: deepstack embed shape");
    check(prefill_stats->shapes["deepstack_active"] == std::vector<std::int64_t>({3, 1}),
    ...
    check(prefill_stats->bound_shapes["cache_k_0"] == std::vector<std::int64_t>({8, 4}),
          "sequence prefill: dynamic K cache binds at full profile length");
    check(prefill_stats->bound_shapes["cache_v_0"] == std::vector<std::int64_t>({8, 4}),
...
int main() {
    ...
    if (cudaStreamCreate(&stream) != cudaSuccess)
        return 77;
    {
        test_sequence_prefill_uses_one_text_launch(stream);
    }
