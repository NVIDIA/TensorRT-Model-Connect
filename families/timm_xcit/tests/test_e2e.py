...
"""Direct build, native runtime, and timm reference proof for XCiT."""
...
from families.timm_xcit.cli import BuildRequest, build_bundle
...
def pytest_generate_tests(metafunc) -> None:
    ...
    if not selected:
        names = [
            pytest.param(
                name,
                marks=pytest.mark.skip(reason="real XCiT E2E requires explicit selection"),
                id=name,
            )
            for name in names
        ]
...
def _required_path(value: str | None, label: str) -> Path:
    assert value, f"selected XCiT E2E requires {label}"
    path = Path(value)
    assert path.exists(), f"selected XCiT E2E {label} does not exist: {path}"
    return path


def _model_dir(manifest: dict) -> Path:
    explicit = os.environ.get("TRTMC_TIMM_TIMM_XCIT_MODEL_DIR")
    if explicit:
        return _required_path(explicit, "TRTMC_TIMM_TIMM_XCIT_MODEL_DIR")
