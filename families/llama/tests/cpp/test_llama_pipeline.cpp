...
class FixedLlamaModule final : public trtmc::ITrtModule {
  public:
    FixedLlamaModule(cudaStream_t stream, bool prefill, std::vector<std::int32_t> output_tokens,
...
trtmc::LlamaTextGenConfig make_config() {
    trtmc::LlamaTextGenConfig config;
    config.vocab_size = 4;
    config.id_bos = 0;
    config.id_eos = 2;
...
std::unique_ptr<trtmc::LlamaTextGenerationPipeline>
make_pipeline(cudaStream_t stream, trtmc::LlamaTextGenConfig config,
    ...
    auto decoder = std::make_unique<FixedLlamaModule>(stream, false, std::move(decode_tokens), 16);
    auto prefill = std::make_unique<FixedLlamaModule>(stream, true, std::move(prefill_tokens), 16);
    auto cache = std::make_unique<trtmc::LlamaKvCache>(1, 16, 4, stream);
    return std::make_unique<trtmc::LlamaTextGenerationPipeline>(
...
void test_argmax() {
    trtmc::LlamaSamplingParams params;
    auto sampler = trtmc::create_llama_sampler(params);
...
void test_kv_reset_is_logical_and_masks_stale_rows() {
    StreamFixture fixture;
    FixedLlamaModule decoder(fixture.stream, false, {2}, 8);
    trtmc::LlamaKvCache cache(1, 8, 4, fixture.stream);
...
int main() {
    ...
    if (failures != 0)
        std::cerr << failures << " Llama pipeline test(s) failed\n";
    return failures;
}
