...
"""Llama 3.1 RoPE scaling contract."""

from __future__ import annotations

import numpy as np

from families.llama import graph_ops
...
def _inverse_frequencies(head_dim: int, rope_theta: float) -> np.ndarray:
    inverse = 1.0 / (rope_theta ** (np.arange(0, head_dim, 2, dtype=np.float64) / head_dim))
...
def test_half_dim_table_matches_llama3_reference_formula() -> None:
    positions, head_dim, rope_theta = 32, 128, 500000.0
    angles = np.outer(
        np.arange(positions, dtype=np.float64),
        _inverse_frequencies(head_dim, rope_theta),
    )
