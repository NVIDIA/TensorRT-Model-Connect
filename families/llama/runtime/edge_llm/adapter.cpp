...
#include "families/llama/runtime/edge_llm/request.h"
...
void check_cuda(cudaError_t result) {
    if (result != cudaSuccess)
        throw std::runtime_error(std::string("Llama3 Edge CUDA error: ") +
...
void validate_target(const nlohmann::json& target) {
    utsname host{};
    if (uname(&host) != 0)
        throw std::runtime_error("Cannot identify Llama3 Edge runtime host");
    ...
    if (target.at("os") != "linux" || target.at("os_version") != os_version ||
        ...
        throw std::runtime_error(
            "Llama3 Edge bundle requires its build GPU and CUDA/TensorRT stack");
}
...
std::unique_ptr<void, CloseLibrary> load_plugin() {
    ...
    if (!dladdr(reinterpret_cast<void*>(&create), &location) || !location.dli_fname)
        throw std::runtime_error("Cannot locate Llama3 family library");
    const auto path =
    ...
    if (!initialize || !initialize(static_cast<nvinfer1::ILogger*>(&trt_edgellm::gLogger), ""))
        throw std::runtime_error("Cannot initialize Llama3 Edge plugin");
    return plugin;
}
...
/// Read tokenizer policy using the same bundle byte parser as other family metadata.
std::string bundle_raw_prefix(const BundleReader& bundle) {
    const auto bytes = bundle.read_section("edge_llm/engine/tokenizer_config.json");
    const auto tokenizer = bundle.read_section("edge_llm/engine/tokenizer.json");
    return raw_prompt_prefix(nlohmann::json::parse(bytes.begin(), bytes.end()),
                             nlohmann::json::parse(tokenizer.begin(), tokenizer.end()));
}

/// Use the pinned runtime's complete speculative implementation, not a family decoder.
std::unique_ptr<trt_edgellm::rt::LLMInferenceRuntime>
make_runtime(const Artifacts& artifacts, const nlohmann::json& marker, cudaStream_t stream) {
    using Runtime = trt_edgellm::rt::LLMInferenceRuntime;
    if (marker.value("execution_variant", "autoregressive") == "eagle3") {
        trt_edgellm::rt::SpecDecodeDraftingConfig drafting{};
        drafting.draftingTopK = 10;
        drafting.draftingStep = 6;
        drafting.verifySize = 60;
    ...
    return std::make_unique<Runtime>(artifacts.engine(), "",
...
class EdgeTask final : public ITextGeneration {
  public:
    ...
    TextResult generate(const std::string& prompt, const TextGenerationConfig& config) override {
        auto request = make_request(prompt, config, default_max_new_tokens(), raw_prefix_);
        std::lock_guard<std::mutex> lock(mutex_);
        const auto counts = runtime_->countPromptTokens(request);
        if (counts.size() != 1)
            throw std::runtime_error("Llama3 Edge returned invalid prompt counts");
        ...
        if (!runtime_->handleRequest(request, response, stream_.get()) ||
            ...
            throw std::runtime_error("Llama3 Edge generation failed");
        if (response.finishReasons.size() != 1 ||
            ...
            throw std::runtime_error("Llama3 Edge generation did not complete successfully");
...
ITask* create(const BundleReader& bundle) {
    ...
    if (marker.at("version") != 1 || marker.at("edge_revision") != kRevision ||
        ...
        throw std::runtime_error("Invalid Llama3 Edge bundle contract");
