...
"""llama build inputs and strict compatibility for existing Python callers."""
...
class BuildRequest:
    """llama-owned inputs; unsupported legacy controls are read-only defaults."""
...
def coerce_request(request: object) -> BuildRequest:
    ...
    for name, default in unsupported.items():
        ...
        if name == "quantization" and value == "none":
            continue
        if value != default:
            raise NotImplementedError(f"llama does not support {name}")
    names = {field.name for field in fields(BuildRequest)}
    if unknown := set(vars(request)) - names - set(unsupported):
        raise ValueError(f"unknown llama build inputs: {sorted(unknown)}")
