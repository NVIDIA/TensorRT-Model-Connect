...
//
// Composes: ITrtModule (decoder) + GptOssKvCache + ITokenizer for this runtime
...
#include "families/gpt_oss/runtime/inference_state.h"
#include "families/gpt_oss/runtime/sampler.h"
#include "families/gpt_oss/runtime/tokenizer.h"
...
class GptOssTextGenerationPipeline final : public ITextGeneration {
  public:
    GptOssTextGenerationPipeline(std::unique_ptr<ITrtModule> decoder,
                                 std::unique_ptr<GptOssInferenceState> state,
                                 GptOssTextGenConfig config, std::shared_ptr<ITokenizer> tokenizer,
