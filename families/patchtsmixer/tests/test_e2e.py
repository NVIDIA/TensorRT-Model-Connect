...
"""Direct build, native-runtime, and official-reference E2E for patchtsmixer."""
...
def _reference_model(model_dir: Path):
    ...
    model = transformers.PatchTSMixerForPrediction(config)
    model.load_state_dict(load_file(model_dir / "model.safetensors"), strict=True)
    model = model.to(dtype=torch.float32).eval()
    channels = int(getattr(config, "num_input_channels", 1))
    context = int(getattr(config, "context_length", 1))
    return model, channels, context
...
def _official_reference(model_dir: Path, manifest: dict, case: dict, tmp_path: Path):
    ...
    inputs = case["inputs"]
    model, channels, context = _reference_model(model_dir)
    raw = list(inputs["past_values"])
    values = np.zeros(context * channels, dtype=np.float32)
    values[-min(len(raw), values.size) :] = raw[-values.size :]
    mask = np.zeros_like(values)
    mask[-min(len(raw), values.size) :] = 1.0
    kwargs = {
        "past_values": torch.tensor(values).reshape(1, context, channels),
        "return_dict": True,
    }
    kwargs["observed_mask"] = torch.tensor(mask).reshape(1, context, channels)
    kwargs["return_loss"] = False
    with torch.no_grad():
        output = model(**kwargs).prediction_outputs
...
def test_official_checkpoint_e2e(case_name: str, tmp_path: Path) -> None:
    ...
    from families.patchtsmixer.tests.etth1 import CASE as ETTH1_CASE

    if case_name == ETTH1_CASE:
        from families.patchtsmixer.tests.etth1 import GATES, windows
