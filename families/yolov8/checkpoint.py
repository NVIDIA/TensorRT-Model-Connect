...
"""Exact NumPy access to an Ultralytics YOLOv8 `.pt` checkpoint.

Ultralytics publishes YOLOv8 as a pickled torch archive rather than
...
class Checkpoint:
    ...
    def open(cls, model_dir: Path) -> "Checkpoint":
        ...
        if not path.is_file():
            raise FileNotFoundError(f"YOLOv8 model directory has no {cls.ARCHIVE}: {model_dir}")
    ...
    def from_archive(cls, path: Path) -> "Checkpoint":
        ...
        if model is None:
            raise ValueError(f"YOLOv8 archive has no model entry: {path}")
        names = getattr(model, "names", None)
        if not isinstance(names, dict) or not names:
            raise ValueError(f"YOLOv8 archive has no class names: {path}")
    ...
    def tensor(self, name: str) -> np.ndarray:
        ...
        if value is None:
            raise KeyError(f"YOLOv8 checkpoint tensor not found: {name}")
        return value
