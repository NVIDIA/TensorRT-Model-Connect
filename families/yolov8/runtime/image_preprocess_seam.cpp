...
std::vector<float> preprocess_yolov8_image(const float* pixels, std::int32_t height,
                                           std::int32_t width, const Yolov8PreprocessConfig& config,
                                           Yolov8Letterbox& letterbox) {
    if (pixels == nullptr || height <= 0 || width <= 0)
        throw std::invalid_argument("YOLOv8 preprocessing needs a non-empty image");
    if (config.input_image_h <= 0 || config.input_image_w <= 0)
        throw std::invalid_argument("YOLOv8 input size must be positive");
    ...
    // The image is pasted at an integer offset, so the reported padding has to
    // be that same integer. Halving in floating point first reports a half
    // pixel whenever the total padding is odd, and the pipeline then subtracts
    // that half pixel from every box. Found by WenJing95 in YOLOv10 (#1270).
