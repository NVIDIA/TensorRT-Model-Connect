...
#include "families/yolov8/runtime/pipeline.h"
...
// Greedy non-maximum suppression, per class. YOLOv8's head reports one
...
Yolov8ObjectDetectionPipeline::Yolov8ObjectDetectionPipeline(
    std::unique_ptr<ITrtModule> model, Yolov8PreprocessConfig preprocess_config,
    ...
    if (!model_ || !model_->ok())
        throw std::runtime_error("Yolov8ObjectDetectionPipeline: invalid model");
}

ObjectDetectionResult Yolov8ObjectDetectionPipeline::detect(const float* pixels, int32_t height,
                                                            int32_t width) {
    Yolov8Letterbox letterbox;
    auto values = preprocess_yolov8_image(pixels, height, width, preprocess_config_, letterbox);
    Tensor input;
    ...
    if (boxes == nullptr || scores == nullptr || classes == nullptr)
        throw std::runtime_error("YOLOv8 engine did not return boxes, scores and classes");
    if (boxes->dtype != DType::kFloat32 || scores->dtype != DType::kFloat32)
        throw std::runtime_error("YOLOv8 boxes and scores must be float32");
    if (classes->dtype != DType::kInt32)
        throw std::runtime_error("YOLOv8 classes must be int32");
    ...
    if (static_cast<std::size_t>(boxes->numel()) != count * 4U ||
        ...
        throw std::runtime_error("YOLOv8 detection outputs disagree on their length");
    ...
    result.boxes = suppress_yolov8_boxes(std::move(candidates), iou_threshold_,
