...
Yolov8PreprocessConfig parse_config(const std::vector<char>& data) {
    const auto json = nlohmann::json::parse(data.begin(), data.end());
    Yolov8PreprocessConfig config;
    ...
    if (config.input_image_h <= 0 || config.input_image_w <= 0 || config.pad_value < 0.0F ||
        ...
        throw std::runtime_error("YOLOv8 runtime.json does not match its contract");
    }
    return config;
}
...
std::unique_ptr<ITrtModule> load_engine(IBackend& backend, const std::vector<char>& plan) {
    ...
    if (!engine || !engine->ok())
        throw std::runtime_error("YOLOv8 engine failed to load");
    return engine;
}
...
extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    ...
    const auto config_data = trtmc::yolov8::require_section(context.reader, "runtime.json");
    const auto plan = trtmc::yolov8::require_section(context.reader, "engine.plan");
    auto config = trtmc::yolov8::parse_config(config_data);
    auto engine = trtmc::yolov8::load_engine(context.backend, plan);
    return new trtmc::Yolov8ObjectDetectionPipeline(
        std::move(engine), std::move(config), trtmc::yolov8::kScoreThreshold,
        trtmc::yolov8::kIouThreshold, trtmc::yolov8::kMaxDetections);
}
