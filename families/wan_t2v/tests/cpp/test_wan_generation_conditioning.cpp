...
void test_wan_t5_token_framing_matches_hf() {
    ...
    check(legacy == std::vector<std::int32_t>({289, 3735, 1}),
    ...
    check(unframed == std::vector<std::int32_t>({289, 3735, 1}),
          "wan token framing appends HF EOS");
    const auto truncated =
        trtmc::diffusion::normalize_wan_t5_token_ids({2, 10, 11, 12, 13, 1}, 4, true);
    check(truncated == std::vector<std::int32_t>({10, 11, 12, 1}),
...
void test_text_conditioning_uses_both_prompt_and_null_prompt() {
    ...
    const bool ok = trtmc::diffusion::build_wan_text_conditioning(
        ...
        [&encoder_calls](const std::vector<std::int32_t>& ids, std::vector<float>& embeddings,
                         std::string&) {
            ++encoder_calls;
            embeddings.assign(ids.begin(), ids.end());
            return true;
        },
        [&projector_calls](const std::vector<float>& embeddings, std::int32_t seq_len,
...
void test_conditioning_inputs_skip_attention_mask_when_rope_enabled() {
    ...
    check(inputs.null_ids == std::vector<std::int32_t>({1, 0, 0, 0}),
...
void test_text_conditioning_propagates_encoder_failures() {
    ...
    const bool prompt_fail = trtmc::diffusion::build_wan_text_conditioning(
        {1, 2}, inputs, 2, error,
        [](const std::vector<std::int32_t>&, std::vector<float>&, std::string& message) {
            message = "prompt encode failed";
            return false;
        },
        [](const std::vector<float>&, std::int32_t, std::vector<float>&) {}, conditioning);
    ...
    const bool null_fail = trtmc::diffusion::build_wan_text_conditioning(
        ...
        [&encoder_calls](const std::vector<std::int32_t>& ids, std::vector<float>& embeddings,
            ...
            ++encoder_calls;
            if (encoder_calls == 2) {
                message = "null encode failed";
                return false;
            }
            embeddings.assign(ids.begin(), ids.end());
            return true;
        },
        [](const std::vector<float>& embeddings, std::int32_t seq_len,
           std::vector<float>& projected) {
            projected = embeddings;
            projected.push_back(static_cast<float>(seq_len));
        },
...
void test_text_conditioning_rejects_non_finite_embeddings() {
    ...
    const bool ok = trtmc::diffusion::build_wan_text_conditioning(
        {7, 8}, inputs, 2, error,
        [](const std::vector<std::int32_t>&, std::vector<float>& embeddings, std::string&) {
            embeddings = {1.0F, std::numeric_limits<float>::quiet_NaN()};
            return true;
        },
        [](const std::vector<float>& embeddings, std::int32_t, std::vector<float>& projected) {
            projected = embeddings;
        },
        conditioning);
...
void test_initial_latents_are_deterministic_by_seed() {
    const auto first = trtmc::diffusion::make_wan_initial_latents(6, 42U);
    const auto repeat = trtmc::diffusion::make_wan_initial_latents(6, 42U);
    const auto other = trtmc::diffusion::make_wan_initial_latents(6, 7U);
    check(first == repeat, "wan initial latents are deterministic for identical seeds");
    check(first != other, "wan initial latents change for different seeds");
    check(std::fabs(first[0]) > 0.0F, "wan initial latents contain gaussian samples");
}
