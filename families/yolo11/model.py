...
"""Plain TensorRT build entrypoint for YOLO11 detectors.

YOLO11 is end to end: the one-to-one head it uses at inference emits one box
...
# PSA fixes the size of an attention head rather than how many there are, so
# the head count follows from the attended width. Nothing records this.
_PSA_HEAD_DIM = 64

# Stage table for the YOLO11 topology: (index, kind, inputs).
...
# always carries its residual - that is what lets one table cover every YOLO11
...
def describe_checkpoint(checkpoint: Checkpoint) -> dict[str, Any]:
    ...
    if not checkpoint.class_names:
        raise ValueError("YOLO11 archive names no classes")
    if checkpoint.image_size % 32:
        raise ValueError(
            f"YOLO11 input {checkpoint.image_size} must be divisible by 32, "
...
def _preprocess_config(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if height <= 0 or width <= 0:
        raise ValueError("YOLO11 input size must be positive")
    if height % max(_STRIDES) or width % max(_STRIDES):
        raise ValueError(f"YOLO11 input {height}x{width} must divide {max(_STRIDES)}")
    return {
        "image_height": height,
        "image_width": width,
...
def _layout(checkpoint: Checkpoint) -> None:
    ...
    if not present:
        raise ValueError("YOLO11 checkpoint has no model.<index> tensors")
    ...
    if present != expected:
        ...
        raise ValueError(
            f"YOLO11 stage set does not match the expected topology; "
...
def _fold(checkpoint: Checkpoint, prefix: str, dtype: np.dtype) -> tuple[np.ndarray, np.ndarray]:
    ...
    Every convolution in YOLO11 is a Conv(conv, bn, SiLU); the statistics stay
    ...
    if not (gamma.shape == beta.shape == mean.shape == variance.shape):
        raise ValueError(f"YOLO11 norm {prefix}.bn has mismatched parameter shapes")
    if gamma.shape != (weight.shape[0],):
        raise ValueError(f"YOLO11 norm {prefix}.bn does not match its convolution")
    if np.any(variance + _BATCH_NORM_EPSILON <= 0.0):
        raise ValueError(f"YOLO11 norm {prefix}.bn has a non-positive running variance")
...
def _c3k(network, tensor, weights: _Weights, prefix: str, dtype, *, residual: bool):
    """A small CSP block: two entry convolutions, a bottleneck chain on one.

    YOLO11 nests these inside C3k2 at the deeper stages. A C3k carries a third
    convolution where a bottleneck does not, which is how the two are told
    apart without a table.
    """
    left = _conv(network, tensor, weights, f"{prefix}.cv1", dtype, stride=1)
    right = _conv(network, tensor, weights, f"{prefix}.cv2", dtype, stride=1)
    index = 0
    while weights.exists(f"{prefix}.m.{index}.cv1.conv.weight"):
        ...
        index += 1
    if index == 0:
        raise ValueError(f"YOLO11 {prefix} has no inner bottlenecks")
    merged = graph.concatenate(network, [left, right])
    return _conv(network, merged, weights, f"{prefix}.cv3", dtype, stride=1)


def _c3k2(network, tensor, weights: _Weights, prefix: str, dtype, *, residual: bool):
    ...
    sees `2 + len(blocks)` groups of channels. Each inner block is either a
    bottleneck or a C3k; a C3k has its own cv3, which a bottleneck never does,
    so the choice is read from the checkpoint rather than tabulated.
    """
    ...
    while True:
        ...
        if not weights.exists(f"{leaf}.cv1.conv.weight"):
            break
        if weights.exists(f"{leaf}.cv3.conv.weight"):
            parts.append(_c3k(network, parts[-1], weights, leaf, dtype, residual=True))
        else:
            parts.append(_bottleneck(network, parts[-1], weights, leaf, dtype, residual=residual))
        index += 1
    if index == 0:
        raise ValueError(f"YOLO11 {prefix} has no inner blocks")
...
def _attention(network, tensor, weights: _Weights, prefix: str, dtype, *, heads: int):
    """Spatial self-attention over the flattened map, plus a positional
    ...
    spatial = graph.convolution(
        ...
        position,
        position_bias,
        padding=int(position.shape[2]) // 2,
        groups=channels,
        dtype=dtype,
    )
...
def _psa_block(network, tensor, weights: _Weights, prefix: str, dtype, *, heads: int):
    """One attention and one feed forward, each added back."""
    tensor = graph.add(
        network,
        tensor,
        _attention(network, tensor, weights, f"{prefix}.attn", dtype, heads=heads),
    )
    feed = _conv(network, tensor, weights, f"{prefix}.ffn.0", dtype, stride=1)
    weight, bias = weights.conv(f"{prefix}.ffn.1")
    # The second feed-forward convolution carries no activation.
    feed = graph.convolution(network, feed, weight, bias, dtype=dtype)
    return graph.add(network, tensor, feed)


def _c2psa(network, tensor, weights: _Weights, prefix: str, dtype):
    """Attention over half the channels, the other half passed through.

    The head count follows from the attended width: PSA fixes the size of a
    head rather than how many there are. Dividing by the wrong number still
    builds and still produces plausible output.
    """
    entry = _conv(network, tensor, weights, f"{prefix}.cv1", dtype, stride=1)
    half = int(entry.shape[1]) // 2
    ...
    heads = max(1, half // _PSA_HEAD_DIM)
    index = 0
    while weights.exists(f"{prefix}.m.{index}.attn.qkv.conv.weight"):
        attended = _psa_block(network, attended, weights, f"{prefix}.m.{index}", dtype, heads=heads)
        index += 1
    if index == 0:
        raise ValueError(f"YOLO11 {prefix} has no attention blocks")
...
def _backbone(network, pixels, weights: _Weights, dtype):
    ...
    for index, kind, sources in _STAGES:
        ...
        if kind == "conv":
            ...
            tensor = _c2psa(network, tensor, weights, prefix, dtype)
        elif kind in _RESIDUAL:
            tensor = _c3k2(network, tensor, weights, prefix, dtype, residual=_RESIDUAL[kind])
        elif kind == "sppf":
            ...
            raise ValueError(f"YOLO11 stage {index} has an unknown kind {kind!r}")
...
_STRIDED_CONVS = frozenset({0, 1, 3, 5, 7, 17, 20})
...
# Every YOLO11 C3k2 adds its input back, in the neck as well as the backbone.
# YOLOv8 drops it in the neck, and the channel shapes match either way, so this
# was checked against the reference rather than carried over.
_RESIDUAL = {"c3k2": True}
...
def _detect(network, sources, weights: _Weights, dtype, *, config, reg_max: int):
    ...
    # YOLO11's head is not end to end: it emits one prediction per anchor and
...
def _build_engine(
    ...
    if precision == "fp16":
        ...
        raise ValueError(f"unsupported YOLO11 precision: {precision}")
    ...
    if pixels is None:
        raise RuntimeError("TensorRT rejected the YOLO11 input")
    hidden = pixels
    if hidden.dtype != tensor_dtype:
        cast = network.add_cast(hidden, tensor_dtype)
        if cast is None:
            raise RuntimeError("TensorRT rejected the YOLO11 input cast")
    ...
    for tensor, name in ((boxes, "boxes"), (scores, "scores")):
        if tensor.dtype != trt.float32:
            ...
            if cast is None:
                raise RuntimeError(f"TensorRT rejected the YOLO11 {name} cast")
    ...
    if classes.dtype != trt.int32:
        ...
        if cast is None:
            raise RuntimeError("TensorRT rejected the YOLO11 classes cast")
    ...
    if plan is None:
        raise RuntimeError("TensorRT YOLO11 engine build failed")
...
def build(request: "BuildRequest", writer: "BundleWriter") -> None:
    """Build one YOLO11 object-detection bundle."""
    if request.dynamic_kv_cache:
        raise NotImplementedError("yolo11 does not support dynamic_kv_cache")
    if request.image_height is not None:
        raise NotImplementedError("yolo11 does not support image_height")
    if request.image_width is not None:
        raise NotImplementedError("yolo11 does not support image_width")
    if request.video_num_frames is not None:
        raise NotImplementedError("yolo11 does not support video_num_frames")
    if request.max_batch_size != 1:
        raise NotImplementedError("yolo11 does not support max_batch_size")
    ...
    if request.fp32_layers:
        raise NotImplementedError("yolo11 does not support mixed-precision layers")
    ...
    writer.set_header(family="yolo11", task=request.task, backend=request.backend)
