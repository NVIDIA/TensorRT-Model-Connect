...
"""Small TensorRT graph vocabulary owned by YOLO11."""
...
def convolution(
    ...
    *,
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 convolution")
...
def silu(network, tensor):
    """x * sigmoid(x), the activation every YOLO11 convolution uses."""
    gate = network.add_activation(tensor, trt.ActivationType.SIGMOID)
    if gate is None:
        raise RuntimeError("TensorRT rejected a YOLO11 SiLU sigmoid")
    product = network.add_elementwise(tensor, gate.get_output(0), trt.ElementWiseOperation.PROD)
    if product is None:
        raise RuntimeError("TensorRT rejected a YOLO11 SiLU product")
...
def add(network, left, right):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 add")
    return layer.get_output(0)
...
def concatenate(network, tensors, *, axis: int = 1):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 concatenation")
    layer.axis = axis
    return layer.get_output(0)
...
def slice_axis(network, tensor, *, axis: int, start: int, count: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 slice")
    return layer.get_output(0)
...
def max_pool(network, tensor, *, kernel: int, stride: int, padding: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 max pool")
    layer.stride_nd = (stride, stride)
    layer.padding_nd = (padding, padding)
    return layer.get_output(0)
...
def nearest_upsample(network, tensor, factor: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 upsample")
...
def reshape(network, tensor, shape: tuple[int, ...]):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 reshape")
...
def permute(network, tensor, permutation: tuple[int, ...]):
    layer = network.add_shuffle(tensor)
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 permutation")
...
def matmul(network, left, right, *, transpose_left=False, transpose_right=False):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 matmul")
    return layer.get_output(0)
...
def softmax(network, tensor, axis: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 softmax")
    layer.axes = 1 << axis
    return layer.get_output(0)
...
def sigmoid(network, tensor):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 sigmoid")
    return layer.get_output(0)
...
def constant(network, values: np.ndarray, *, dtype: np.dtype, like=None):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 constant")
    output = layer.get_output(0)
    if like is None or output.dtype == like.dtype:
        return output
    cast = network.add_cast(output, like.dtype)
    if cast is None:
        raise RuntimeError("TensorRT rejected a YOLO11 constant cast")
...
def subtract(network, left, right):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 subtraction")
    return layer.get_output(0)
...
def reduce_max(network, tensor, axis: int, *, keep_dims: bool):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 max reduction")
    return layer.get_output(0)
...
def top_k(network, tensor, *, k: int, axis: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 top-k")
...
def gather(network, tensor, indices, *, axis: int):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 gather")
    return layer.get_output(0)
...
def multiply(network, left, right):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 product")
    return layer.get_output(0)
...
def floor_divide(network, left, right):
    ...
    if layer is None:
        raise RuntimeError("TensorRT rejected a YOLO11 floor division")
    return layer.get_output(0)
