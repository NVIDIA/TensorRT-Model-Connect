...
"""Run the official Ultralytics YOLO11 archive for qualification."""
...
def _load_model(model_dir: Path, precision: str) -> Any:
    ...
    if not archive.is_file():
        raise FileNotFoundError(f"YOLO11 model directory has no {ARCHIVE}: {model_dir}")
    blob = torch.load(str(archive), map_location="cpu", weights_only=False)
    model = blob.get("model") if isinstance(blob, dict) else None
    if model is None:
        raise ValueError(f"YOLO11 archive has no model entry: {archive}")
...
def _detect(
    ...
    return {
        ...
        "class_ids": [int(row[5]) for row in rows],
    }
...
def _performance(arguments: argparse.Namespace) -> int:
    ...
    if arguments.family != "yolo11" or arguments.operation != "detect":
        raise ValueError("YOLO11 reference only supports yolo11 detect")
    if arguments.mode != "pytorch-eager":
        raise ValueError("YOLO11 reference only supports pytorch-eager")
