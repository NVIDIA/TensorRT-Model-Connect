...
void test_letterbox_keeps_aspect_and_centres_the_image() {
    ...
    const auto values = trtmc::preprocess_yolo11_image(pixels.data(), 2, 4, config, letterbox);
...
void test_preprocess_reads_interleaved_and_writes_planar() {
    ...
    trtmc::Yolo11Letterbox letterbox;
    const auto values = trtmc::preprocess_yolo11_image(pixels.data(), 4, 4, config, letterbox);
...
void test_letterbox_reports_a_mapping_that_inverts() {
    ...
    trtmc::Yolo11Letterbox letterbox;
    (void)trtmc::preprocess_yolo11_image(pixels.data(), 5, 9, config, letterbox);
...
void test_preprocess_rejects_an_empty_image() {
    ...
    try {
        trtmc::Yolo11PreprocessConfig config;
        trtmc::Yolo11Letterbox letterbox;
        (void)trtmc::preprocess_yolo11_image(nullptr, 0, 0, config, letterbox);
    } catch (const std::invalid_argument&) {
        threw = true;
    }
    check(threw, "YOLO11 rejects an empty image");
}

void test_suppression_keeps_the_strongest_and_drops_its_overlaps() {
    // YOLO11's head reports one prediction per anchor and leaves the overlaps
    ...
    const auto kept = trtmc::suppress_yolo11_boxes(candidates, 0.7F, 300U);
...
void test_suppression_honours_the_detection_cap() {
    ...
    const auto kept = trtmc::suppress_yolo11_boxes(candidates, 0.7F, 4U);
...
void test_reported_padding_matches_where_the_image_is_placed() {
    ...
    trtmc::Yolo11Letterbox letterbox;
    const auto values = trtmc::preprocess_yolo11_image(pixels.data(), 5, 8, config, letterbox);
...
int main() {
    ...
    if (g_failures != 0) {
        std::cerr << g_failures << " YOLO11 preprocess test(s) failed\n";
        return 1;
    }
    return 0;
}
