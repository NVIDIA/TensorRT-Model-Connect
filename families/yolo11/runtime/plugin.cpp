...
Yolo11PreprocessConfig parse_config(const std::vector<char>& data) {
    ...
    if (config.input_image_h <= 0 || config.input_image_w <= 0 || config.pad_value < 0.0F ||
        ...
        throw std::runtime_error("YOLO11 runtime.json does not match its contract");
    }
    return config;
}
...
std::unique_ptr<ITrtModule> load_engine(IBackend& backend, const std::vector<char>& plan) {
    ...
    if (!engine || !engine->ok())
        throw std::runtime_error("YOLO11 engine failed to load");
    return engine;
}
...
TRTMC_DEFINE_FAMILY_PLUGIN_V1("yolo11")

extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    if (context.kv_cache_size_bytes != 0)
        throw std::invalid_argument("yolo11 does not support --kv-cache-size");
    const auto config_data = trtmc::yolo11::require_section(context.reader, "runtime.json");
    const auto plan = trtmc::yolo11::require_section(context.reader, "engine.plan");
    auto config = trtmc::yolo11::parse_config(config_data);
    auto engine = trtmc::yolo11::load_engine(context.backend, plan);
    return new trtmc::Yolo11ObjectDetectionPipeline(
        std::move(engine), std::move(config), trtmc::yolo11::kScoreThreshold,
        trtmc::yolo11::kIouThreshold, trtmc::yolo11::kMaxDetections);
}
