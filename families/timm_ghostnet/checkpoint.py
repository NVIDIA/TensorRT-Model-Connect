...
"""Exact NumPy safetensors access for timm GhostNet checkpoints."""
...
class Checkpoint:
    ...
    def open(cls, model_dir: Path) -> "Checkpoint":
        ...
        if not index_path.is_file():
            raise FileNotFoundError(f"GhostNet checkpoint has no model safetensors: {model_dir}")
        payload = json.loads(index_path.read_text(encoding="utf-8"))
        weight_map = payload.get("weight_map") if isinstance(payload, dict) else None
        if not isinstance(weight_map, dict) or not weight_map:
            raise ValueError("GhostNet safetensors index has no weight_map")
        if not all(
            isinstance(name, str) and name and isinstance(shard, str) and shard
            for name, shard in weight_map.items()
        ):
            raise ValueError("GhostNet safetensors index contains invalid entries")
        shard_names = sorted(set(weight_map.values()))
        for name in shard_names:
            path = PurePosixPath(name)
            if path.is_absolute() or len(path.parts) != 1 or path.name != name:
                raise ValueError("GhostNet safetensors shard names must be direct relative files")
        readers = {
    ...
    def tensor(self, name: str) -> np.ndarray:
        ...
        if reader is None:
            raise KeyError(f"GhostNet checkpoint tensor not found: {name}")
