...
Layer 2 in the three-layer builder stack:
    ...
    graph_blocks.py     Layer 2: Composable blocks (weight-aware)  <- THIS FILE
        |
    family builders     Layer 3: Full engine assembly
...
def make_matmul_fn(network, dtype):
    """Create the family-owned unquantized matmul callable.
...
def apply_norm(
    ...
    if norm_type != "rmsnorm":
        raise ValueError("qwen3_omni supports only RMSNorm")
...
def add_swiglu_mlp(
    ...
    *,
    ...
    swish = graph_ops.add_activation(network, gate, "silu", dtype=dtype)
    gated = network.add_elementwise(swish, up, trt.ElementWiseOperation.PROD)

    mlp_out = matmul(
        gated.get_output(0), mlp_size, hidden_size, weights[f"{prefix}.w_down"], f"{prefix}.w_down"
    )
    return mlp_out
