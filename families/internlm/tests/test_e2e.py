...
def _render_prompt(tokenizer, prompt: str, case: dict):
    if not case.get("use_chat_template", False):
        return tokenizer(prompt, return_tensors="pt", return_token_type_ids=False)
...
def _hf_reference(
    ...
    from families.internlm.tokenizer_json import ensure_tokenizer_json
    ...
    tokenizer = PreTrainedTokenizerFast(
        tokenizer_file=str(ensure_tokenizer_json(model_dir)),
