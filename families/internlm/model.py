    ...
    def load_weights(
        ...
        for layer_idx in range(num_layers):
            ...
            if total_qkv != expected_qkv:
                raise ValueError(
                    f"Layer {layer_idx} wqkv rows {total_qkv} != expected {expected_qkv} (q={q_dim}, kv={kv_dim})"
                )
...
def build(request: "BuildRequest", writer: "BundleWriter") -> None:
    ...
    if request.dynamic_kv_cache:
        raise NotImplementedError("internlm does not support dynamic_kv_cache")
    ...
    writer.set_header(family="internlm", task=request.task, backend=request.backend)
    if parallel.enabled:
