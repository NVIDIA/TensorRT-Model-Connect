...
from families.gpt_neo import cli, support
...
def test_cli_defaults_match_support_and_publish_a_bundle(monkeypatch, tmp_path):
    ...
    def select_backend(backend):
        assert backend == "trt"
        monkeypatch.setitem(
            sys.modules, "families.gpt_neo.model", SimpleNamespace(build=build_model)
        )

    monkeypatch.setattr(cli, "select_backend", select_backend)
    output = tmp_path / "model.bundle"
    declaration = support.describe(ModelMetadata({"model_type": "gpt_neo"}, {}))
    assert declaration is not None
    assert family_cli.main(["gpt_neo", "build", str(tmp_path), "-o", str(output)]) == 0
...
def test_failed_owner_preserves_existing_output(monkeypatch, tmp_path):
    ...
    monkeypatch.setitem(sys.modules, "families.gpt_neo.model", SimpleNamespace(build=fail))
...
def test_legacy_unsupported_inputs_are_still_rejected(tmp_path, changes):
    legacy = LegacyRequest(tmp_path, tmp_path / "out", "gpt_neo", "text_generation", "fp32")
...
def test_help_is_lazy_and_owned(monkeypatch, capsys):
    ...
    def guarded(name, *args, **kwargs):
        assert name not in {
            "families.gpt_neo.cli",
            "families.gpt_neo.model",
            "tensorrt",
            "tensorrt_rtx",
            "huggingface_hub",
        }
    ...
    with pytest.raises(SystemExit) as error:
        family_cli.main(["gpt_neo", "build", "--help"])
