...
```sh
trtmc build EleutherAI/gpt-neo-125m -o model.bundle
...
```sh
cmake --build build --target trtmc trtmc_backend_trt trtmc_model_gpt_neo test_gpt_neo_task_config
ctest --test-dir build --output-on-failure -R '^gpt_neo_task_config$'
build/test_gpt_neo_sdk_c model.bundle build text "Hello" max_new_tokens=20
build/test_gpt_neo_sdk_cpp model.bundle build text "Hello" max_new_tokens=20
```
