...
#include "families/gpt_neo/runtime/pipeline.h"

#include "families/gpt_neo/runtime/chat_templates.h"
#include "families/gpt_neo/runtime/kv_cache.h"
#include "families/gpt_neo/runtime/tensor_names.h"
...
GptNeoTextGenerationPipeline::GptNeoTextGenerationPipeline(
    std::unique_ptr<ITrtModule> decoder, std::unique_ptr<GptNeoInferenceState> state,
    GptNeoTextGenConfig config, std::shared_ptr<ITokenizer> tokenizer,
    ...
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error("GptNeoTextGenerationPipeline: invalid prefill module");
    if (!tokenizer_)
        throw std::runtime_error("GptNeoTextGenerationPipeline: invalid tokenizer");
    if (!state_ || !state_->ok()) {
...
TextResult GptNeoTextGenerationPipeline::run(const internal::TextContinuationRequest& request,
                                             internal::ConfigView supplied) {
    const auto cfg = gpt_neo::parse_text_config(supplied);
...
void require_prefill_kv_pointers(ITrtModule& prefill, const GptNeoTextGenConfig& cfg,
    ...
    for (int32_t layer = 0; layer < cfg.num_layers; ++layer) {
        ...
        pk[index] = prefill.device_ptr(gpt_neo_expand_layer_name(cfg.present_k_pattern, layer));
        pv[index] = prefill.device_ptr(gpt_neo_expand_layer_name(cfg.present_v_pattern, layer));
        if (pk[index] == nullptr || pv[index] == nullptr) {
            throw std::runtime_error(
                "GptNeoTextGenerationPipeline: prefill module is missing K/V output for layer " +
                std::to_string(layer));
        }
    }
}

void require_batched_prefill_contract(ITrtModule& prefill, const GptNeoTextGenConfig& cfg,
                                      int32_t sq, GptNeoInferenceState* state) {
    if (!prefill.ok())
        throw std::runtime_error("GptNeoTextGenerationPipeline: invalid prefill module");
    if (sq <= 0)
        throw std::runtime_error(
            "GptNeoTextGenerationPipeline: prefill requires a non-empty prompt");
    if (cfg.prefill_max_length <= 0 || cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        throw std::runtime_error("GptNeoTextGenerationPipeline: invalid prefill configuration");
    if (sq > cfg.prefill_max_length)
        throw std::runtime_error(
            "GptNeoTextGenerationPipeline: prompt exceeds the prefill profile");
    auto* kv = dynamic_cast<GptNeoKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("GptNeoTextGenerationPipeline: prefill requires GptNeoKvCache");
}

} // namespace

void GptNeoTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (logits_it == outputs.end())
        throw std::runtime_error(
            "GptNeoTextGenerationPipeline: prefill module has no logits output");
    ...
    if (static_cast<std::size_t>(lt.numel()) < vocab)
        throw std::runtime_error(
            "GptNeoTextGenerationPipeline: prefill logits are smaller than vocabulary");
    logits.resize(vocab);
...
std::string
GptNeoTextGenerationPipeline::resolve_generation_mode(const TextGenerationConfig& cfg) const {
...
GptNeoTextGenerationPipeline::TimedGenResult GptNeoTextGenerationPipeline::generate_from_ids(
    ...
    const GptNeoSamplingParams& params, const TextGenerationConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "ar")
        throw std::runtime_error("GptNeoTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");
    auto active_sampler = create_gpt_neo_sampler(params);
...
int32_t GptNeoTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        const GptNeoSampleResult result = sampler->sample(logits.data(), vocab_size, params);
...
ITrtModule& GptNeoTextGenerationPipeline::bind_decoder_for_step() {
    if (!state_bound_) {
