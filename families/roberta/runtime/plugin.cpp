...
#include "families/roberta/runtime/distributed_runtime.h"
#include "families/roberta/runtime/pipeline.h"
#include "families/roberta/runtime/plugin_helpers.h"
...
nlohmann::json require_config(const BundleReader& bundle) {
    ...
    if (!config.is_object())
        throw std::runtime_error("RoBERTa runtime.json must be an object");
    return config;
}
...
std::int32_t require_tensor_parallel_size(const nlohmann::json& config) {
    if (!config.contains("tensor_parallel_size") ||
        ...
        throw std::runtime_error("RoBERTa runtime.json requires tensor_parallel_size");
    }
    const auto value = config.at("tensor_parallel_size").get<std::int64_t>();
    if (value <= 0 || value > std::numeric_limits<std::int32_t>::max())
        throw std::runtime_error("RoBERTa tensor_parallel_size is invalid");
...
std::string require_task(const BundleInfo& info) {
    ...
    throw std::runtime_error("RoBERTa does not implement task: " + info.task);
}
...
extern "C" trtmc::ITask* trtmc_create_family(const trtmc::FamilyContext& context) {
    ...
    const auto config = trtmc::roberta_factory::require_config(context.reader);
    const auto tp_size = trtmc::roberta_factory::require_tensor_parallel_size(config);
    const auto group = trtmc::roberta::initialize_tensor_parallel_group(tp_size);
    const std::string section =
        tp_size == 1 ? "engine.plan" : "engine.rank" + std::to_string(group.rank) + ".plan";
    const auto& plan = trtmc::roberta_factory::require_section(context.reader, section.c_str());
    ...
    if (!tokenizer)
        throw std::runtime_error("RoBERTa bundle does not contain its required tokenizer");
    const auto task = trtmc::roberta_factory::require_task(context.reader.info());
