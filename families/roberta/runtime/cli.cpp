...
Interface& require_interface(trtmc::ITask& task) {
    ...
    if (result == nullptr)
        throw std::invalid_argument("ROBERTA bundle does not implement the requested interface");
    return *result;
}
...
nlohmann::json execute(const std::string& handler, const nlohmann::json& values,
    ...
    if (handler != "encode" && handler != "embed" && handler != "rerank")
        throw std::invalid_argument("unknown ROBERTA CLI handler: " + handler);
    // Check identity before loading: a ROBERTA command must not execute another
    ...
    if (reader.info().family != "roberta")
        throw std::invalid_argument("ROBERTA CLI requires a roberta bundle");
    ...
    if (handler == "rerank") {
        ...
        if (!std::isfinite(score))
            throw std::runtime_error("ROBERTA reranking returned a non-finite score");
        return {{"score", score}};
    }
    ...
    for (const auto value : result.data) {
        if (!std::isfinite(value))
            throw std::runtime_error("ROBERTA embedding returned a non-finite value");
    }
