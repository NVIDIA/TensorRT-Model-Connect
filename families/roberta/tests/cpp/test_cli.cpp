...
class FakeRorobertaa final : public trtmc::IEncoding,
...
void roberta_contract(const fs::path& root) {
    const auto path = root / "roberta.bundle";
    write_bundle(path, "roberta");
    ...
    check(encoded.first == 0 && Json::parse(encoded.second.output).at("values") == Json({3.0, 1.0}),
          "ROBERTA owner converts the encoding request and preserves result fields");
    check(loaded_runtime == "/installed/runtime",
          "ROBERTA owner uses the supplied installed runtime root");
    ...
    check(embedded.first == 0 &&
          ...
          "ROBERTA owner selects the embedding interface");
    check(loaded_runtime == "/override/runtime", "runtime_root override remains owned by ROBERTA");
...
std::unique_ptr<ITask> load_task(const BundleReader&, const std::string& runtime_root,
    ...
    return std::make_unique<FakeRorobertaa>();
}
} // namespace trtmc

int main() {
    const auto root = fs::temp_directory_path() / ("trtmc-roberta-cli-" + std::to_string(getpid()));
    fs::create_directories(root);
    try {
        roberta_contract(root);
