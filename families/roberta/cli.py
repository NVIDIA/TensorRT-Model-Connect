...
"""roberta command handlers and model-owned build inputs."""
...
class BuildRequest:
    ...
    def __post_init__(self) -> None:
        if self.task not in {"encoding", "embedding", "reranking"}:
            raise ValueError("unsupported roberta task")
        if str(self.precision).lower() not in {"fp16", "bf16", "fp32"}:
            raise ValueError("unsupported roberta precision")
...
def coerce_request(request: object) -> BuildRequest:
    ...
    if unknown := set(vars(request)) - set(fields) - legacy:
        raise ValueError(f"unknown roberta build inputs: {sorted(unknown)}")
