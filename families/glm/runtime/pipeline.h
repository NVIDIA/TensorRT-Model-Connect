...
//
// Composes: ITrtModule (decoder) + GlmKvCache + ITokenizer for this runtime
...
#include "families/glm/runtime/inference_state.h"
#include "families/glm/runtime/sampler.h"
#include "families/glm/runtime/tokenizer.h"
...
class GlmTextGenerationPipeline final : public ITextGeneration {
  public:
    GlmTextGenerationPipeline(std::unique_ptr<ITrtModule> decoder,
                              std::unique_ptr<GlmInferenceState> state, GlmTextGenConfig config,
  ...
  private:
    ...
    std::unique_ptr<GlmInferenceState> state_;
