...
// Exercises the ownership the T5 pipeline uses for its cross-attention K/V
// buffers, against CPU CUDA stubs so each allocation can be failed in turn
// without a GPU. T5Pipeline itself needs live TensorRT modules to build, so
...
// The allocation loop from T5Pipeline's constructor, over the same type.
void allocate_cross_kv(std::vector<trtmc::t5::DeviceBuffer>& keys,
                       std::vector<trtmc::t5::DeviceBuffer>& values, int32_t layers,
                       std::size_t bytes) {
