...
std::unique_ptr<ITrtModule> load_engine(IBackend& backend, const std::vector<char>& plan,
    ...
    if (engine == nullptr || !engine->ok())
        throw std::runtime_error(std::string("m2m_100 failed to load ") + label);
...
std::shared_ptr<ITokenizer> create_tokenizer(const BundleReader& bundle) {
    ...
    auto tokenizer = CreateSentencePieceBpeTokenizer(data.data(), data.size(), true);
    if (tokenizer == nullptr)
        throw std::runtime_error("m2m_100 SentencePiece-BPE tokenizer construction failed");
