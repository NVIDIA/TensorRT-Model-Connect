...
from families.m2m_100 import graph_ops, model
...
def test_process_logger_is_reused_and_retained(monkeypatch) -> None:
    ...
    monkeypatch.setattr(model, "trt", SimpleNamespace(Logger=_FakeLogger))
    monkeypatch.setattr(model, "_PROCESS_LOGGERS", {})

    first = model._get_process_logger(verbose=False)
    logger_ref = weakref.ref(first)
    del first
    gc.collect()

    second = model._get_process_logger(verbose=True)
    third = model._get_process_logger(verbose=False)
