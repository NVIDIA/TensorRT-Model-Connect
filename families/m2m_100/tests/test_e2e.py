...
"""Direct build, native-runtime, and official-reference E2E for m2m_100."""
...
def _visible_generated_ids(ids: list[int], decoder_start, eos) -> list[int]:
    result = list(ids)
    if decoder_start is not None and result and result[0] == int(decoder_start):
        result.pop(0)
    stop_tokens = {
        int(value) for value in (eos if isinstance(eos, list) else [eos]) if value is not None
    }
    if result and result[-1] in stop_tokens:
        result.pop()
    return result
...
def test_visible_generated_ids_exclude_decoder_control_tokens() -> None:
    assert _visible_generated_ids([2, 256057, 1034, 248075, 2], 2, 2) == [
        256057,
        1034,
        248075,
    ]
