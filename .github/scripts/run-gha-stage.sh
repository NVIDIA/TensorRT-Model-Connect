...
docker run --rm \
  ...
  -e TRT_PACKAGE_DIR \
  -e TRTMC_TRT_PACKAGE_DIR \
  -e TRTMC_EDGE_LLM_ARTIFACT_DIR \
  -e TRTMC_EDGE_LLM_MODEL_DIR \
  -e TRTMC_EDGE_LLM_ENGINE_DIR \
  -e TRTMC_EDGE_LLM_TARGET \
  -e TRTMC_EDGE_LLM_MAX_CACHE_LENGTH \
  -e TRTMC_EDGE_LLM_PRECISION \
  -e TRTMC_EDGE_LLM_BUILD_DIR \
  -e TRTMC_EDGE_LLM_PROVIDER_BUILD_DIR \
  -e TRTMC_EDGE_LLM_BUILD_JOBS \
  -e TRTMC_EDGE_LLM_CMAKE_ARGS \
  -e TRTMC_PROVIDER_CMAKE_ARGS \
  -e TRTMC_EDGE_LLM_EXPORT_TOOL \
  -e TRTMC_EDGE_LLM_PROMPT \
  -e TRTMC_EDGE_LLM_MAX_NEW_TOKENS \
  -e TRTMC_EDGE_LLM_BENCHMARK_ITERS \
  -e TRTMC_EDGE_LLM_WARMUP_ITERS \
  -e HF_TOKEN \
