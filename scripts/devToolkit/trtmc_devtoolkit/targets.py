...
class DockerEnvironment:
    ...
    def prepare(
        ...
        *,
        ...
        image = target.image or (
            f"trtmc-devtoolkit:{plan.cohort.id}-{plan.architecture}-"
            f"{plan.image_fingerprint[:12]}"
        )
    ...
    def _ensure_container(
        ...
        storage_root = (
            Path(target.storage_root).resolve()
            if target.storage_root
            else cache_root / "storage"
        )
    ...
    def _verify_container(self, plan: PreparationPlan, container: str, sm: str) -> None:
        ...
        observed_sm = command_output(
            ...
            cwd=self.repository,
        ).splitlines()[0].replace(".", "").strip()
        if observed_sm != sm:
            raise DevToolkitError(
                f"Container sees SM {observed_sm}; host selected SM {sm}"
            )
    ...
    def _build_install(
        ...
        environment = {
            ...
            "TRTMC_PACKAGE_WHEEL_ARCH": plan.cohort.architectures[
                plan.architecture
            ].wheel_platform,
    ...
    def prepare(
        ...
        *,
        ...
        if not python.is_file():
            ...
            self.runner.run(
                [target.python, "-m", "venv", "--system-site-packages", venv],
                cwd=self.repository,
            )
        environment = {
        ...
        handle = EnvironmentHandle(
            ...
            trtmc=str(trtmc),
            python=str(python),
            activate_command=f". {venv / 'bin' / 'activate'}",
        ...
        if plan.request.mode == "development":
            ...
            self.runner.run(
                [
                    ...
                    "-DCMAKE_BUILD_TYPE=Release",
                    f"-DCMAKE_CUDA_ARCHITECTURES={sm}-real",
                    f"-DTRTMC_TRT_INCLUDE_DIR={contract.tensorrt_include_dir}",
                    f"-DTRTMC_TRT_LIBRARY={contract.tensorrt_library_dir}/libnvinfer.so",
                    "-DTRTMC_BUILD_BACKEND_TRT=ON",
                    "-DTRTMC_BUILD_BACKEND_RTX=OFF",
                    "-DTRTMC_BUILD_TESTS=OFF",
                    "-DTRTMC_BUILD_BENCHMARKS=OFF",
