...
WORKER_PROMPT = textwrap.dedent("""\
    ...
    ## Step 1: Establish the model-owned capsule
    `scripts/new_family.py` is only a preliminary Python bootstrap. If the
    family package does not exist, you may run it inside the container:
    ```
    ...
    ```
    If the family package already exists, skip that command. In either case,
    complete all three ownership descriptors and their implementations:
    ...
    The bootstrap does not create the runtime DSO, model-local E2E plugins, or
    ...
    {{
        ...
        "family": "{family_name}",
        "runtime_strategy": "{family_name}_decoder_kv_cache",
        ...
        "testcases": [
            {{
                "name": "{family_name}",
                "trace_id": "IT-E2E-{family_name}-01",
    ...
    listed under `test_manifests`, and add the matching Python/runtime
    `MODEL.toml` descriptors. Adjust both strategies and the testcase contract
    for non-decoder models. The runtime strategy must match the Python plugin,
    ...
    git add python/tensorrt_model_connect/families/{family_name}
    git add src/runtime/models/{family_name}
    git add tests/e2e/models/{family_name}
    git add tests/runtime_strategy_matrix.yaml
    ```
...
def launch_agent(
    ...
    print(
        f"  Started agent {agent_id} (pid={proc.pid}, family={task['family_name']}, log={log_path})"
    )
...
def wait_for_batch(
    ...
    while procs and (time.time() - start) < timeout:
        for agent_id, (proc, task) in list(procs.items()):
            ...
            if ret is not None:
                ...
                status_file = (
                    f"{WORKSPACE_ROOT}/{agent_id}/tensorrt-model-connect/.autopilot_status.json"
                )
                ...
                if ret == 0 and status.get("status") == "PASS":
                    print(
                        f"  \u2713 {agent_id} ({family}): PASSED "
                        f"— branch {status.get('branch', '?')}"
                    )
                elif status.get("status") == "SKIP":
                    print(
                        f"  - {agent_id} ({family}): SKIPPED "
                        f"— {status.get('reason', 'needs C++ plugin')}"
                    )
                else:
                    print(f"  \u2717 {agent_id} ({family}): FAILED (exit={ret})")
    ...
    for agent_id, (proc, task) in procs.items():
        ...
        results[agent_id] = {"family": task["family_name"], "status": "TIMEOUT"}
...
def dispatch(
    ...
    for batch_num in range(total_batches):
        ...
        print(f"\nBatch {batch_num + 1}/{total_batches} ({len(batch)} tasks):")
        for i, task in enumerate(batch):
            agent = agent_ids[i]
            print(f"  {agent}: {task['model_type']:<20} ({task['hf_id']})")
        ...
        for i, task in enumerate(batch):
            agent = agent_ids[i]
            proc = launch_agent(agent, task, dry_run=is_dry_run, optimize=optimize)
            if proc:
        ...
        # Wait for batch
        print(f"\nWaiting for batch {batch_num + 1} (timeout={timeout}s)...")
    ...
    # Save full results
    results_path = os.path.join(os.environ.get("TMPDIR", "/tmp"), "autopilot_results.json")
...
def main():
    ...
    parser.add_argument(
        "--agent-ids", nargs="+", default=None, help="Specific agent IDs (default: agent-1..N)"
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--optimize", action="store_true", help="After validation, run precision optimization"
    )
    parser.add_argument("--offset", type=int, default=0, help="Skip first N tasks (for resuming)")
    ...
    for aid in agent_ids:
        ...
        if not ws.is_dir() and args.mode != "dry-run":
            ...
            print(
                f"  Bootstrap it: ./scripts/bootstrap_workspace.sh "
                f"--id {aid} --branch main --detach",
                file=sys.stderr,
            )

    dispatch(tasks, agent_ids, mode=args.mode, timeout=args.timeout, optimize=args.optimize)
