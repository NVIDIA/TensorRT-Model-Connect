...
class SanaWmQkRopePlugin : public nvinfer1::IPluginV2DynamicExt {
  public:
    SanaWmQkRopePlugin(int32_t frames, int32_t spatial, int32_t heads, int32_t head_dim,
                       float norm_eps, std::vector<float> q_norm_weight,
                       std::vector<float> k_norm_weight, bool torch_rms = false);
    SanaWmQkRopePlugin(const void* data, size_t length);
    ~SanaWmQkRopePlugin() override = default;
    ...
    SanaWmQkRopePlugin* clone() const noexcept override;
    ...
    static constexpr const char* kPLUGIN_NAME = "SanaWmQkRope";
