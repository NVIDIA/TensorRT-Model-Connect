...
class SanaWmQkRopeCreator : public nvinfer1::IPluginCreator {
  public:
    SanaWmQkRopeCreator() {
        ...
        fields_.push_back({"torch_rms", nullptr, nvinfer1::PluginFieldType::kINT32, 1});
    ...
    char const* getPluginName() const noexcept override {
        return SanaWmQkRopePlugin::kPLUGIN_NAME;
    }

    char const* getPluginVersion() const noexcept override {
        return SanaWmQkRopePlugin::kPLUGIN_VERSION;
    }
    ...
    nvinfer1::IPluginV2* createPlugin(char const*, nvinfer1::PluginFieldCollection const* fc)
        ...
        if (fc != nullptr) {
            for (int32_t i = 0; i < fc->nbFields; ++i) {
                ...
                } else if (std::strcmp(f.name, "torch_rms") == 0 && f.data != nullptr) {
                    torch_rms = *static_cast<const int32_t*>(f.data) != 0;
        ...
        return new SanaWmQkRopePlugin(frames, spatial, heads, head_dim, norm_eps,
    ...
    nvinfer1::IPluginV2* deserializePlugin(char const*, void const* data, size_t length)
        noexcept override {
        return new SanaWmQkRopePlugin(data, length);
    }
...
static nvinfer1::PluginRegistrar<trtmc::SanaWmQkRopeCreator> pluginRegistrarSanaWmQkRope{};

extern "C" void sana_wm_qk_rope_plugin_force_link() {}
