...
void durable_flush(const fs::path& path) {
#if defined(_WIN32)
...
#else
    // POSIX rename retains the existing best-effort durability contract.
    (void)path;
#endif
}
...
void atomic_replace(const fs::path& temporary, const fs::path& target) {
    ...
    if (error) {
        throw std::system_error(error,
                                "failed to atomically replace RTX runtime cache " + target.string());
    }
#endif
}
