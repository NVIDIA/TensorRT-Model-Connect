...
#include "runtime/models/qwen/triattention_kernels.h"
#endif
...
QwenTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return QwenTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return QwenTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(QwenTriAttentionScoreAggregation value) {
    switch (value) {
    case QwenTriAttentionScoreAggregation::kMean:
        return "mean";
    case QwenTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

QwenTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return QwenTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return QwenTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, QwenKvCacheNames& names) {
...
// override helpers that patched QwenTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     QwenTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(QwenTriAttentionConfig& cfg,
...
void fill_debug_from_registry(QwenTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(QwenTriAttentionConfig& cfg, const std::string& config_json,
...
static void validate_triattention_config(const QwenTriAttentionConfig& cfg,
                                         int32_t max_cache_length) {
    if (cfg.kv_budget < 1)
...
QwenTriAttentionConfig
qwen_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                      const ::trtmc::config::ConfigBundle* runtime_config) {
    QwenTriAttentionConfig cfg;
...
void fill_stats_header(QwenTriAttentionStats& stats, const json& root, int32_t num_attention_heads,
...
void fill_stats_inv_freq(QwenTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(QwenTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(QwenTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(QwenTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(QwenTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(QwenTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
void init_layer_stats_empty(QwenTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(QwenTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(QwenTriAttentionStats& stats, const std::vector<int32_t>& group_counts,
                           int32_t half_dim) {
    bool any_stats = false;
    for (int32_t layer = 0; layer < stats.num_layers; ++layer) {
        auto& layer_stats = stats.layer_stats[static_cast<std::size_t>(layer)];
...
void parse_sparse_stats(QwenTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
QwenTriAttentionStats qwen_parse_triattention_stats_json(const std::string& stats_json,
                                                         int32_t num_attention_heads,
                                                         int32_t num_key_value_heads,
                                                         int32_t num_layers) {
    QwenTriAttentionStats stats;
...
void QwenTriAttentionKvCache::validate_shapes() {
...
void QwenTriAttentionKvCache::normalize_sampled_heads() {
...
void QwenTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void QwenTriAttentionKvCache::allocate_layer_tensors() {
...
QwenTriAttentionKvCache::QwenTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                 int32_t max_length, int32_t kv_dim,
                                                 cudaStream_t stream, QwenTriAttentionConfig config,
                                                 QwenTriAttentionStats stats, DType cache_dtype,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void QwenTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void QwenTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void QwenTriAttentionKvCache::initialize_gpu_state() {
...
bool QwenTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool QwenTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool QwenTriAttentionKvCache::can_use_gpu_selection() const {
...
void QwenTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t QwenTriAttentionKvCache::preferred_cache_rows() const {
...
void QwenTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void QwenTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float>
QwenTriAttentionKvCache::copy_cache_rows_to_host(const DeviceTensor& tensor, int32_t rows,
                                                 QwenTriAttentionCompactionProfile* profile) const {
...
void QwenTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t QwenTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> QwenTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t> QwenTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
QwenTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                             QwenTriAttentionCompactionProfile* profile) {
...
int32_t QwenTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t QwenTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> QwenTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void QwenTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, QwenTriAttentionCompactionProfile* profile) const {
...
bool QwenTriAttentionKvCache::layer_stats_shapes_valid(const QwenTriAttentionHeadStats& layer_stats,
...
void QwenTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                            int32_t half_dim, float& k_rot_real,
                                            float& k_rot_imag) const {
    if (stats_.rope_style == QwenTriAttentionRopeStyle::kInterleaved) {
...
float QwenTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == QwenTriAttentionScoreAggregation::kMax)
...
float QwenTriAttentionKvCache::score_one_row(
    const float* row_ptr, const QwenTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void QwenTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const QwenTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void QwenTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void QwenTriAttentionKvCache::accumulate_layer_fallback(
...
void QwenTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == QwenTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void QwenTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
QwenTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void QwenTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == QwenTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void QwenTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> QwenTriAttentionKvCache::build_keep_indices_per_head(
...
bool QwenTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, QwenTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> QwenTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, QwenTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void QwenTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                     int32_t total_tokens) const {
...
void QwenTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void QwenTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
QwenTriAttentionKvCache::GpuLayerResult QwenTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = qwen_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == QwenTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == QwenTriAttentionScoreAggregation::kMax,
...
bool QwenTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool QwenTriAttentionKvCache::upload_gpu_trig_phases(int32_t num_offsets, int32_t half_dim,
                                                     QwenTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void QwenTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> QwenTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool QwenTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> QwenTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, QwenTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void QwenTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void QwenTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool QwenTriAttentionKvCache::gpu_compaction_upload_keep(int32_t keep_count,
...
bool QwenTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                    std::size_t row_bytes) {
    const bool ok_k = qwen_triattention_compact_rows_gpu(
    ...
    const bool ok_v = qwen_triattention_compact_rows_gpu(
...
bool QwenTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
                                                   const std::vector<int32_t>& keep_indices,
...
void QwenTriAttentionKvCache::compact_layer_on_host(int32_t layer,
                                                    const std::vector<int32_t>& keep_indices,
                                                    int32_t keep_count, std::size_t row_bytes,
...
void QwenTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    QwenTriAttentionCompactionProfile profile;
    QwenTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void QwenTriAttentionKvCache::finalize_repack_profile(
    QwenTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
QwenTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
QwenTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                   int32_t keep_count) const {
...
int32_t QwenTriAttentionKvCache::count_prefix_positions(
...
void QwenTriAttentionKvCache::log_compact_debug(
...
void QwenTriAttentionKvCache::log_compact_profile(
    const QwenTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool QwenTriAttentionKvCache::should_emit_keep_dump(
    const QwenTriAttentionCompactionProfile* profile_ptr) const {
...
void QwenTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "QwenTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void QwenTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void QwenTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void QwenTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool QwenTriAttentionKvCache::ok() const {
