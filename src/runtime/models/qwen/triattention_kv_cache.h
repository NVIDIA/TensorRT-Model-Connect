...
enum class QwenTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class QwenTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct QwenTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    QwenTriAttentionScoreAggregation score_aggregation{QwenTriAttentionScoreAggregation::kMean};
    QwenTriAttentionScoreAggregation per_layer_aggregation{QwenTriAttentionScoreAggregation::kMean};
...
struct QwenTriAttentionHeadStats {
...
struct QwenTriAttentionStats {
    int32_t head_dim{0};
    QwenTriAttentionRopeStyle rope_style{QwenTriAttentionRopeStyle::kHalf};
    ...
    std::vector<QwenTriAttentionHeadStats> layer_stats;
};

struct QwenTriAttentionCompactionProfile {
...
// Build a QwenTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class QwenTriAttentionKvCache : public QwenInferenceState {
  public:
    QwenTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                            int32_t kv_dim, cudaStream_t stream, QwenTriAttentionConfig config,
                            QwenTriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(QwenTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const QwenTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const QwenTriAttentionCompactionProfile* profile_ptr) const;
    ...
    std::vector<int32_t> select_keep_indices(int32_t keep_budget,
                                             QwenTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             QwenTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const QwenTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const QwenTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const QwenTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    QwenTriAttentionConfig config_;
    QwenTriAttentionStats stats_;
