...
// QwenTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/qwen/argmax_kernel.h"
#include "runtime/models/qwen/sparse_multinomial_kernel.h"
...
class TorchCudaMultinomialSampler final : public QwenISampler {
  public:
    ...
    QwenSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        qwen_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const QwenTorchMultinomialExecutionPolicy policy =
                qwen_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public QwenISampler {
  public:
    ...
    QwenSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        qwen_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    QwenLogitsLocation logits_location() const override { return QwenLogitsLocation::DEVICE; }
...
create_qwen_sampler(const QwenSamplingParams& params,
                    [[maybe_unused]] const QwenSamplerFactoryOptions& options) {
...
std::unique_ptr<QwenISampler> create_qwen_sampler(const QwenSamplingParams& params) {
    return create_qwen_sampler(params, QwenSamplerFactoryOptions{});
}
