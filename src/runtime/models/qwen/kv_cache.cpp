...
QwenKvCache::QwenKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim,
    ...
    if (runtime_state_) {
        if (runtime_state_->capacity_tokens() != static_cast<std::uint64_t>(max_length_))
            throw std::invalid_argument("Qwen runtime KV capacity does not match max_length");
        reset();
        return;
    }
...
QwenKvCache::~QwenKvCache() = default;

// Masked score constant is model-local.
static constexpr float kMaskedScore = -1.0e4F;

void QwenKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t QwenKvCache::preferred_cache_rows() const {
    if (runtime_state_) {
        ...
        if (bound > static_cast<std::uint64_t>(std::numeric_limits<int32_t>::max())) {
            throw std::overflow_error("Qwen runtime KV profile bound exceeds int32");
        }
...
void QwenKvCache::rebind_cache_rows(int32_t cache_rows) {
...
std::vector<int64_t> QwenKvCache::mask_shape_for_engine(int32_t mask_width,
...
void QwenKvCache::prepare_step(TensorMap& inputs, int32_t seq_len) {
    ...
    if (runtime_state_) {
        if (bound_module_ == nullptr)
            throw std::logic_error("Qwen runtime KV state is not bound to an execution role");
        if (!bound_module_->has_input(names_.history_length)) {
            throw std::runtime_error("Qwen segmented runtime graph is missing history_length");
        }
        if (position_ > max_length_ - seq_len)
            throw std::runtime_error("Qwen runtime KV invocation exceeds its physical capacity");
    ...
    if (!dynamic_binding_enabled_ && !names_.cache_k.empty()) {
        ...
        if (declared_shape.size() < 2 || declared_shape[0] != max_length_ ||
            ...
            throw std::runtime_error(
                "QwenKvCache::bind_cache_inputs: static prefill cache shape does not match "
...
void QwenKvCache::validate_runtime_staging_pointers(
    const std::vector<const void*>& present_k, const std::vector<const void*>& present_v) const {
    if (!runtime_state_)
        throw std::logic_error("Qwen runtime staging validation requires contiguous_runtime_v1");
    if (static_cast<int32_t>(present_k.size()) != num_layers_ ||
        static_cast<int32_t>(present_v.size()) != num_layers_) {
        throw std::runtime_error("Qwen runtime staging pointer count does not match layer count");
    }
    for (int32_t layer = 0; layer < num_layers_; ++layer) {
        ...
        if (present_k[index] !=
            ...
            throw std::runtime_error("Qwen runtime present K/V is not bound to exact-Sq staging");
        }
    }
}

void QwenKvCache::commit_runtime_rows(int32_t seq_len) {
    if (!runtime_state_)
        throw std::logic_error("Qwen runtime commit requires contiguous_runtime_v1");
    if (seq_len <= 0 || seq_len != pending_tokens_) {
        throw std::logic_error("Qwen runtime KV commit does not match the prepared invocation");
    }
    if (position_ > max_length_ - seq_len)
        throw std::runtime_error("Qwen runtime KV commit exceeds capacity");
...
void QwenKvCache::write_prefill_kv(const std::vector<const void*>& prefill_k,
    ...
    if (runtime_state_) {
        ...
        if (position_ != 0)
            throw std::logic_error("Qwen runtime initial prefill commit requires empty history");
...
int32_t QwenKvCache::prefill_chunk_limit() const {
    if (!runtime_state_)
        return max_length_;
    return static_cast<int32_t>(runtime_state_->layout().prefill_chunk_limit);
}

std::uint64_t QwenKvCache::runtime_kv_capacity_tokens() const {
    return runtime_state_ ? runtime_state_->capacity_tokens() : 0;
}

void QwenKvCache::finalize_runtime_memory() {
    if (runtime_state_)
        runtime_state_->synchronize_commits();
}

void QwenKvCache::sample_runtime_memory_high_water() noexcept {
    if (runtime_state_)
        runtime_state_->sample_request_completion_device_memory();
}

std::string QwenKvCache::runtime_memory_receipt_json() const {
