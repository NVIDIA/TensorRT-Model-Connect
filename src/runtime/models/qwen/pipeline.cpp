...
RuntimeMemoryQualificationResultV1 QwenTextGenerationPipeline::qualify_runtime_memory(
    const RuntimeMemoryQualificationRequestV1& request) {
    if (!state_->runtime_owned_kv()) {
        throw std::runtime_error(
            "QwenTextGenerationPipeline: qualification requires a runtime-memory bundle");
    }
    ...
    if (request.input_ids.empty()) {
        throw admission_error(
            "QwenTextGenerationPipeline: qualification input_ids must not be empty");
    }
    for (std::size_t index = 0; index < request.input_ids.size(); ++index) {
        ...
        if (token < 0 || token >= config_.vocab_size) {
            throw admission_error("QwenTextGenerationPipeline: token ID at index " +
    ...
    try {
        validate_sequence_admission_with_runtime_memory(
            ...
            config_.runtime_sequence_admission, "QwenTextGenerationPipeline");
    ...
    try {
        ...
        if (logits.size() != static_cast<std::size_t>(config_.vocab_size)) {
            throw std::runtime_error(
                "QwenTextGenerationPipeline: qualification prefill returned an unexpected "
                "logit count");
        }
        ...
        for (int32_t step = 0; step < request.max_new_tokens; ++step) {
            ...
            if (logits.size() != static_cast<std::size_t>(config_.vocab_size)) {
                throw std::runtime_error(
                    "QwenTextGenerationPipeline: qualification decode returned an unexpected "
                    "logit count");
            }
            result.step_logits.push_back(logits);
...
bool QwenTextGenerationPipeline::run_prefill_runtime_chunks(const std::vector<int32_t>& input_ids,
                                                            std::vector<float>& logits,
                                                            bool gpu_sampling) {
    if (!state_->runtime_owned_kv())
        return false;
    if (prefill_ == nullptr)
        throw std::runtime_error(
            "QwenTextGenerationPipeline: runtime-memory bundle is missing its prefill role");

    const int32_t chunk_limit = state_->prefill_chunk_limit();
    if (chunk_limit <= 0)
        throw std::runtime_error("QwenTextGenerationPipeline: invalid runtime prefill chunk limit");
    if (config_.prefill_max_length > 0 && chunk_limit > config_.prefill_max_length) {
        throw std::runtime_error(
            "QwenTextGenerationPipeline: runtime prefill chunk exceeds engine profile");
    }
    ...
    while (offset < input_ids.size()) {
        ...
        if (last && !gpu_sampling) {
            ...
            if (found == outputs.end())
                throw std::runtime_error(
                    "QwenTextGenerationPipeline: prefill role did not return logits");
            const auto vocab = static_cast<std::size_t>(config_.vocab_size);
            if (static_cast<std::size_t>(found->second.numel()) < vocab)
                throw std::runtime_error(
                    "QwenTextGenerationPipeline: prefill logits are truncated");
            logits.resize(vocab);
    ...
    if (launches != expected)
        throw std::logic_error("QwenTextGenerationPipeline: incorrect prefill launch count");
...
void QwenTextGenerationPipeline::run_prefill(const std::vector<int32_t>& input_ids,
    ...
    if (!config_.kv_cache_compaction &&
        ...
        throw std::runtime_error(
            "QwenTextGenerationPipeline: prompt length exceeds runtime KV cache capacity");
    }
...
void QwenTextGenerationPipeline::run_prefill_block(const std::vector<int32_t>& input_ids,
    ...
    if (append_kv && kv.position() + sq > kv.max_length()) {
        throw std::runtime_error(
            "QwenTextGenerationPipeline: prefill block exceeds runtime KV cache capacity");
    }
...
std::uint64_t
QwenTextGenerationPipeline::qualification_bound_tokens(std::uint64_t history_tokens) const {
    const auto selected = state_->runtime_kv_bound_tokens(history_tokens);
    if (selected == 0)
        throw std::logic_error(
            "QwenTextGenerationPipeline: runtime state did not report a T bound");
    return selected;
}

RuntimeMemoryQualificationExecutionAttemptBaseline
QwenTextGenerationPipeline::qualification_execution_attempt_baseline() const {
...
void QwenTextGenerationPipeline::append_qualification_invocation(
    ...
    if (ledger == nullptr) {
        throw std::logic_error(
            "QwenTextGenerationPipeline: runtime backend has no transfer ledger");
    }
    ...
    if (commit_after.device_to_device_bytes < commit_before.device_to_device_bytes ||
        ...
        throw std::logic_error("QwenTextGenerationPipeline: runtime commit counters regressed");
    }
...
QwenTextGenerationPipeline::qualification_transfer_snapshot(const TrtModule& module) const {
    if (active_qualification_ == nullptr)
        return {};
    const auto* ledger = dynamic_cast<const IRuntimeMemoryTransferLedgerV1*>(&module);
    if (ledger == nullptr) {
...
RuntimeKvCommitSnapshot QwenTextGenerationPipeline::qualification_commit_snapshot() const {
    if (active_qualification_ == nullptr)
        return {};
    const auto* cache = dynamic_cast<const QwenKvCache*>(state_.get());
    if (cache == nullptr || !cache->runtime_owned_kv()) {
        throw std::logic_error("QwenTextGenerationPipeline: runtime qualification has no "
