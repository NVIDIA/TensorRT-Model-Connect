...
//
// Composes: TrtModule (decoder) + QwenKvCache + ITokenizer for this runtime
...
class QwenTextGenerationPipeline final : public IPipeline {
  public:
    ...
    QwenTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                               ...
                               std::unique_ptr<QwenISampler> sampler = nullptr,
                               std::shared_ptr<void> distributed_owner = nullptr);
    QwenTextGenerationPipeline(std::vector<DecoderContext> decoders,
                               std::unique_ptr<QwenInferenceState> state, QwenTextGenConfig config,
                               cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer = nullptr,
                               std::string model_id_str = "",
                               std::unique_ptr<QwenISampler> sampler = nullptr,
  ...
  private:
    ...
    std::unique_ptr<QwenISampler> sampler_;
    ...
    TimedGenResult generate_from_ids(const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
                                     const QwenSamplingParams& params, const GenerateConfig& cfg);
    ...
    QwenKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(QwenKvCache& kv, TrtModule& prefill, int32_t sq);
