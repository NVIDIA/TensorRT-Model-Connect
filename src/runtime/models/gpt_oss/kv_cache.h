...
// GptOssKvCache: autoregressive KV cache state manager.
...
class GptOssKvCache : public GptOssInferenceState {
  public:
    ...
    GptOssKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, GptOssKvCacheNames names = {});

    // --- GptOssInferenceState overrides ---
    void reset() override;
    ...
    // --- GptOssKvCache-specific methods (not on the interface) ---
    ...
    // — too big for GptOssKvCache's single-row present buffer. The caller reads
