...
// GptOssInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   GptOssKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — GptOssKvCache + family-owned recurrent state composed
...
class GptOssInferenceState {
  public:
    virtual ~GptOssInferenceState() = default;
    ...
    // GptOssKvCache -> true. Family-owned recurrent state -> false.
