...
#include "runtime/models/gpt_oss/pipeline.h"

#include "runtime/models/gpt_oss/chat_templates.h"
#include "runtime/models/gpt_oss/kv_cache.h"
#include "runtime/models/gpt_oss/tensor_names.h"
...
std::vector<GptOssTextGenerationPipeline::DecoderContext>
single_decoder_context(std::unique_ptr<TrtModule> decoder) {
    std::vector<GptOssTextGenerationPipeline::DecoderContext> decoders;
    decoders.push_back(GptOssTextGenerationPipeline::DecoderContext{0, std::move(decoder)});
    return decoders;
}
...
bool greedy_text_diffusion_params(const GptOssSamplingParams& params) {
...
GptOssTextGenerationPipeline::GptOssTextGenerationPipeline(
    std::unique_ptr<TrtModule> decoder, std::unique_ptr<GptOssInferenceState> state,
    GptOssTextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<GptOssISampler> sampler,
    std::shared_ptr<void> distributed_owner)
    : GptOssTextGenerationPipeline(single_decoder_context(std::move(decoder)), std::move(state),
...
GptOssTextGenerationPipeline::GptOssTextGenerationPipeline(
    std::vector<DecoderContext> decoders, std::unique_ptr<GptOssInferenceState> state,
    GptOssTextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<GptOssISampler> sampler,
    ...
    if (decoders_.empty()) {
        throw std::runtime_error("GptOssTextGenerationPipeline: no decoder modules");
    }
    for (const auto& decoder_ctx : decoders_) {
        if (!decoder_ctx.module || !decoder_ctx.module->ok()) {
            throw std::runtime_error("GptOssTextGenerationPipeline: invalid decoder module");
        }
    }
    if (!state_ || !state_->ok()) {
        throw std::runtime_error("GptOssTextGenerationPipeline: invalid inference state");
    }
...
static std::vector<int32_t> encode_prompt(const ITokenizer& tokenizer,
                                          const GptOssTextGenConfig& config,
    ...
    if (cfg.use_chat_template && !config.chat_template_format.empty()) {
        effective =
            gpt_oss_apply_chat_template(config.chat_template_format, prompt, cfg.enable_thinking);
        templated = true;
    }
...
TextResult GptOssTextGenerationPipeline::generate(const std::string& prompt,
                                                  const GenerateConfig& cfg) {
    if (!tokenizer_) {
        throw std::runtime_error("GptOssTextGenerationPipeline: no tokenizer configured");
    }
...
GptOssTextGenerationPipeline::generate_ids(const std::vector<int32_t>& input_ids,
                                           const GenerateConfig& cfg) {
...
std::unique_ptr<GptOssISampler>
GptOssTextGenerationPipeline::make_step_sampler(const GptOssSamplingParams& params) {
    ...
    if (prefer_gpu_greedy_ && greedy_params) {
        if (auto gpu = create_gpt_oss_gpu_greedy_sampler(stream_))
            return gpu;
    }
    return create_gpt_oss_sampler(params);
}
...
namespace {
bool gather_prefill_kv_pointers(TrtModule& prefill, const GptOssTextGenConfig& cfg,
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        pk[li] = prefill.device_ptr(gpt_oss_expand_layer_name(cfg.present_k_pattern, i));
        pv[li] = prefill.device_ptr(gpt_oss_expand_layer_name(cfg.present_v_pattern, i));
...
bool batched_prefill_supported(const TrtModule* prefill, const GptOssTextGenConfig& cfg, int32_t sq,
                               GptOssInferenceState* state) {
    ...
    if (cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        return false;
    return dynamic_cast<GptOssKvCache*>(state) != nullptr;
}
} // namespace

bool GptOssTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
                                                       std::vector<float>& logits) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    if (!batched_prefill_supported(prefill_.get(), config_, sq, state_.get()))
        return false;
    auto* kv = static_cast<GptOssKvCache*>(state_.get());
...
void GptOssTextGenerationPipeline::prime_decoder_after_batched_prefill(
...
void GptOssTextGenerationPipeline::run_prefill(const std::vector<int32_t>& input_ids,
...
TrtModule& GptOssTextGenerationPipeline::require_block_prefill(int32_t sq,
                                                               TrtModule* prefill_override) {
    TrtModule* prefill = prefill_override != nullptr ? prefill_override : prefill_.get();
    if (prefill == nullptr)
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: block generation requires prefill module");
    if (sq <= 0)
        throw std::runtime_error("GptOssTextGenerationPipeline: empty block");
    if (config_.prefill_max_length > 0 && sq > config_.prefill_max_length) {
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: block length exceeds prefill profile");
    }
    return *prefill;
}

GptOssKvCache& GptOssTextGenerationPipeline::require_block_kv_cache() {
    auto* kv = dynamic_cast<GptOssKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: block generation requires GptOssKvCache");
    return *kv;
}

void GptOssTextGenerationPipeline::copy_block_logits(const TensorMap& outputs,
                                                     std::vector<float>& logits) const {
    auto logits_it = outputs.find(config_.logits_output_name);
    if (logits_it == outputs.end())
        throw std::runtime_error("GptOssTextGenerationPipeline: prefill module has no '" +
...
void GptOssTextGenerationPipeline::append_prefill_kv(GptOssKvCache& kv, TrtModule& prefill,
                                                     int32_t sq) {
    std::vector<const void*> pk, pv;
    if (!gather_prefill_kv_pointers(prefill, config_, pk, pv)) {
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: prefill module is missing present_k/present_v outputs");
    }
    kv.append_prefill_kv(pk, pv, sq);
}

void GptOssTextGenerationPipeline::run_prefill_block(const std::vector<int32_t>& input_ids,
                                                     bool bidirectional, bool append_kv,
                                                     std::vector<float>& logits,
                                                     TrtModule* prefill_override) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    TrtModule& prefill = require_block_prefill(sq, prefill_override);
    GptOssKvCache& kv = require_block_kv_cache();
...
std::string GptOssTextGenerationPipeline::resolve_generation_mode(const GenerateConfig& cfg) const {
...
int32_t GptOssTextGenerationPipeline::resolve_text_diffusion_block_length(
    ...
    if (!config_.supports_text_diffusion || config_.mask_token_id < 0)
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: bundle does not support text diffusion");
    ...
    if (require_divisible && max_new_tokens % block_len != 0) {
        throw std::runtime_error("GptOssTextGenerationPipeline: diffusion mode requires "
                                 "max_new_tokens % block_length == 0");
    }
    return block_len;
}

int32_t GptOssTextGenerationPipeline::seed_next_token_from_prefill(
    ...
    if (static_cast<int32_t>(logits.size()) < vocab)
        throw std::runtime_error("GptOssTextGenerationPipeline: missing prefill logits");
    return argmax_with_confidence(logits.data() + logits.size() - static_cast<std::size_t>(vocab),
                                  vocab, 0)
        .token_id;
}

void GptOssTextGenerationPipeline::fill_diffusion_block(std::vector<int32_t>& block,
                                                        std::vector<float>& logits,
    ...
    for (int32_t step = 0; step < block_len && has_mask_token(block, config_.mask_token_id);
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "GptOssTextGenerationPipeline: diffusion engine must output full block logits");
        }
...
int32_t GptOssTextGenerationPipeline::verify_diffusion_block(const std::vector<int32_t>& block,
                                                             std::vector<float>& logits,
                                                             int32_t block_len, int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: diffusion engine must output full verify logits");
    }
...
bool GptOssTextGenerationPipeline::append_tokens_until_eos(
    const std::vector<int32_t>& tokens, std::vector<int32_t>& output,
    const GptOssSamplingParams& params) const {
...
void GptOssTextGenerationPipeline::fill_linear_spec_block(std::vector<int32_t>& block,
                                                          std::vector<float>& logits,
                                                          int32_t block_len, int32_t vocab,
    ...
    while (has_mask_token(block, config_.mask_token_id)) {
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "GptOssTextGenerationPipeline: linear_spec engine must output full block logits");
        }
        const auto preds = masked_predictions(logits, block, config_.mask_token_id, vocab);
...
std::vector<int32_t>
GptOssTextGenerationPipeline::verify_linear_spec_block(const std::vector<int32_t>& block,
                                                       std::vector<float>& logits,
                                                       int32_t block_len, int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: linear_spec engine must output full verify logits");
    }
...
int32_t
GptOssTextGenerationPipeline::count_linear_spec_accepts(const std::vector<int32_t>& ar_tokens,
                                                        const std::vector<int32_t>& block) {
    if (ar_tokens.empty())
        return 0;
    if (block.size() < 2)
...
bool GptOssTextGenerationPipeline::append_linear_spec_tokens(
    const std::vector<int32_t>& ar_tokens, int32_t emit_count, std::vector<int32_t>& output,
    int32_t& generated, const GptOssSamplingParams& params) const {
...
GptOssTextGenerationPipeline::TimedGenResult GptOssTextGenerationPipeline::generate_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
    const GptOssSamplingParams& params, const GenerateConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "auto" && mode != "ar")
        throw std::runtime_error("GptOssTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");

    GptOssISampler* active_sampler = sampler_.get();
    std::unique_ptr<GptOssISampler> local_sampler;
    if (!active_sampler) {
    ...
    std::vector<float> logits;
    const bool gpu_sampling = (active_sampler->logits_location() == GptOssLogitsLocation::DEVICE);
...
GptOssTextGenerationPipeline::TimedGenResult
GptOssTextGenerationPipeline::generate_diffusion_from_ids(const std::vector<int32_t>& input_ids,
                                                          int32_t max_new_tokens,
                                                          const GptOssSamplingParams& params,
                                                          const GenerateConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: diffusion mode currently supports greedy temperature=0 "
            "generation");
    }
    const int32_t block_len =
...
GptOssTextGenerationPipeline::TimedGenResult
GptOssTextGenerationPipeline::generate_linear_spec_from_ids(const std::vector<int32_t>& input_ids,
                                                            int32_t max_new_tokens,
                                                            const GptOssSamplingParams& params,
                                                            const GenerateConfig& cfg,
                                                            bool use_lora_draft) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error("GptOssTextGenerationPipeline: linear_spec mode currently "
                                 "supports greedy temperature=0 "
                                 "generation");
    }
    if (use_lora_draft && linear_spec_lora_prefill_ == nullptr) {
        throw std::runtime_error("GptOssTextGenerationPipeline: linear_spec_lora mode requires a "
    ...
    if (kv == nullptr)
        throw std::runtime_error(
            "GptOssTextGenerationPipeline: linear_spec requires GptOssKvCache");
...
bool GptOssTextGenerationPipeline::should_stop_on_answer(const std::vector<int32_t>& output,
...
void GptOssTextGenerationPipeline::log_decode_summary(int32_t steps, double ms) const {
...
int32_t GptOssTextGenerationPipeline::run_decode_loop(
    GptOssISampler* sampler, const GptOssSamplingParams& params, std::vector<int32_t>& output,
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        ...
        const GptOssSampleResult result = sampler->sample(sample_ptr, vocab_size, params);
...
int32_t GptOssTextGenerationPipeline::select_decoder_index(int32_t desired_rows) const {
    if (decoders_.size() == 1)
        return 0;
...
TrtModule& GptOssTextGenerationPipeline::bind_decoder_for_step() {
...
void GptOssTextGenerationPipeline::run_step(int32_t token_id, std::vector<float>& logits) {
    TensorMap inputs;
    ...
    if (it == outputs.end()) {
        throw std::runtime_error("GptOssTextGenerationPipeline: no '" + logits_output_name_ +
                                 "' output");
    }
...
void GptOssTextGenerationPipeline::run_step_device(int32_t token_id) {
    TensorMap inputs;
...
int32_t GptOssTextGenerationPipeline::argmax(const std::vector<float>& logits) {
    if (logits.empty())
        return 0;
