...
#include "runtime/models/gpt_oss/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/gpt_oss/triattention_kernels.h"
#endif
...
GptOssTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return GptOssTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return GptOssTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(GptOssTriAttentionScoreAggregation value) {
    switch (value) {
    case GptOssTriAttentionScoreAggregation::kMean:
        return "mean";
    case GptOssTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

GptOssTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return GptOssTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return GptOssTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, GptOssKvCacheNames& names) {
...
// override helpers that patched GptOssTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     GptOssTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(GptOssTriAttentionConfig& cfg,
...
void fill_debug_from_registry(GptOssTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(GptOssTriAttentionConfig& cfg,
...
static void validate_triattention_config(const GptOssTriAttentionConfig& cfg,
...
void fill_stats_header(GptOssTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(GptOssTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(GptOssTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(GptOssTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(GptOssTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(GptOssTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(GptOssTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(GptOssTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(GptOssTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(GptOssTriAttentionStats& stats, const std::vector<int32_t>& group_counts,
                           int32_t half_dim) {
    bool any_stats = false;
    for (int32_t layer = 0; layer < stats.num_layers; ++layer) {
        auto& layer_stats = stats.layer_stats[static_cast<std::size_t>(layer)];
...
void parse_sparse_stats(GptOssTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
void GptOssTriAttentionKvCache::validate_shapes() {
...
void GptOssTriAttentionKvCache::normalize_sampled_heads() {
...
void GptOssTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void GptOssTriAttentionKvCache::allocate_layer_tensors() {
...
GptOssTriAttentionKvCache::GptOssTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                     int32_t max_length, int32_t kv_dim,
                                                     cudaStream_t stream,
                                                     GptOssTriAttentionConfig config,
                                                     GptOssTriAttentionStats stats,
                                                     DType cache_dtype, GptOssKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void GptOssTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void GptOssTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void GptOssTriAttentionKvCache::initialize_gpu_state() {
...
bool GptOssTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool GptOssTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool GptOssTriAttentionKvCache::can_use_gpu_selection() const {
...
void GptOssTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t GptOssTriAttentionKvCache::preferred_cache_rows() const {
...
void GptOssTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void GptOssTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> GptOssTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows, GptOssTriAttentionCompactionProfile* profile) const {
...
void GptOssTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t GptOssTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> GptOssTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
GptOssTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
GptOssTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
...
int32_t GptOssTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t GptOssTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> GptOssTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void GptOssTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, GptOssTriAttentionCompactionProfile* profile) const {
...
bool GptOssTriAttentionKvCache::layer_stats_shapes_valid(
    const GptOssTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void GptOssTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                              int32_t half_dim, float& k_rot_real,
                                              float& k_rot_imag) const {
    if (stats_.rope_style == GptOssTriAttentionRopeStyle::kInterleaved) {
...
float GptOssTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == GptOssTriAttentionScoreAggregation::kMax)
...
float GptOssTriAttentionKvCache::score_one_row(
    const float* row_ptr, const GptOssTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void GptOssTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const GptOssTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void GptOssTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void GptOssTriAttentionKvCache::accumulate_layer_fallback(
...
void GptOssTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == GptOssTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void GptOssTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
GptOssTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void GptOssTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == GptOssTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void GptOssTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> GptOssTriAttentionKvCache::build_keep_indices_per_head(
...
bool GptOssTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, GptOssTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> GptOssTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, GptOssTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void GptOssTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                       int32_t total_tokens) const {
...
void GptOssTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void GptOssTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
GptOssTriAttentionKvCache::GpuLayerResult
GptOssTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = gpt_oss_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == GptOssTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == GptOssTriAttentionScoreAggregation::kMax,
...
bool GptOssTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool GptOssTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, GptOssTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void GptOssTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> GptOssTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool GptOssTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> GptOssTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, GptOssTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void GptOssTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void GptOssTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool GptOssTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool GptOssTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                      std::size_t row_bytes) {
    const bool ok_k = gpt_oss_triattention_compact_rows_gpu(
    ...
    const bool ok_v = gpt_oss_triattention_compact_rows_gpu(
...
bool GptOssTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void GptOssTriAttentionKvCache::compact_layer_on_host(
...
void GptOssTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    GptOssTriAttentionCompactionProfile profile;
    GptOssTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void GptOssTriAttentionKvCache::finalize_repack_profile(
    GptOssTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
GptOssTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
GptOssTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                     int32_t keep_count) const {
...
int32_t GptOssTriAttentionKvCache::count_prefix_positions(
...
void GptOssTriAttentionKvCache::log_compact_debug(
...
void GptOssTriAttentionKvCache::log_compact_profile(
    const GptOssTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool GptOssTriAttentionKvCache::should_emit_keep_dump(
    const GptOssTriAttentionCompactionProfile* profile_ptr) const {
...
void GptOssTriAttentionKvCache::emit_keep_dump_json(
...
void GptOssTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "GptOssTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void GptOssTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void GptOssTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void GptOssTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool GptOssTriAttentionKvCache::ok() const {
