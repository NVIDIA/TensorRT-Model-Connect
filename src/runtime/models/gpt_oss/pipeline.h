...
//
// Composes: TrtModule (decoder) + GptOssKvCache + ITokenizer for this runtime
...
#include "runtime/models/gpt_oss/inference_state.h"
...
class GptOssTextGenerationPipeline final : public IPipeline {
  public:
    ...
    GptOssTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                 std::unique_ptr<GptOssInferenceState> state,
                                 GptOssTextGenConfig config, cudaStream_t stream,
                                 std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                 std::string model_id_str = "",
                                 std::unique_ptr<GptOssISampler> sampler = nullptr,
                                 std::shared_ptr<void> distributed_owner = nullptr);
    GptOssTextGenerationPipeline(std::vector<DecoderContext> decoders,
                                 std::unique_ptr<GptOssInferenceState> state,
                                 GptOssTextGenConfig config, cudaStream_t stream,
                                 std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                 std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "GptOssTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<GptOssInferenceState> state_;
    GptOssTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<GptOssISampler> sampler_;
    ...
    GptOssKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(GptOssKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(GptOssISampler* sampler, const GptOssSamplingParams& params,
    ...
    std::unique_ptr<GptOssISampler> make_step_sampler(const GptOssSamplingParams& params);
