...
// GptOssISampler: token selection abstraction for autoregressive generation.
//
...
//
// GptOssLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// gpt_oss-owned sampler interface.
class GptOssISampler {
  public:
    virtual ~GptOssISampler() = default;
    ...
    virtual GptOssSampleResult sample(const float* logits, int32_t vocab_size,
                                      const GptOssSamplingParams& params) = 0;
    ...
    virtual GptOssLogitsLocation logits_location() const = 0;
...
/// Build GptOssSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from GptOssSamplingParams.
...
std::unique_ptr<GptOssISampler> create_gpt_oss_sampler(const GptOssSamplingParams& params,
                                                       const GptOssSamplerFactoryOptions& options);
