...
// GptOssISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// GptOssTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/gpt_oss/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/gpt_oss/argmax_kernel.h"
#include "runtime/models/gpt_oss/sparse_multinomial_kernel.h"
...
static GptOssSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                             int32_t eos_token_id) {
    GptOssSampleResult result;
...
static bool greedy_equivalent(const GptOssSamplingParams& params) {
...
class GreedySampler final : public GptOssISampler {
  public:
    GptOssSampleResult sample(const float* logits, int32_t vocab_size,
                              const GptOssSamplingParams& params) override {
    ...
    GptOssLogitsLocation logits_location() const override { return GptOssLogitsLocation::HOST; }
...
class TopKSampler final : public GptOssISampler {
  public:
...
class TorchCudaMultinomialSampler final : public GptOssISampler {
  public:
    ...
    GptOssSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        gpt_oss_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const GptOssTorchMultinomialExecutionPolicy policy =
                gpt_oss_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public GptOssISampler {
  public:
    ...
    GptOssSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        gpt_oss_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    GptOssLogitsLocation logits_location() const override { return GptOssLogitsLocation::DEVICE; }
...
GptOssSamplingParams gpt_oss_sampling_params_from_config(const GenerateConfig& cfg,
                                                         int32_t default_eos) {
    GptOssSamplingParams p;
...
create_gpt_oss_sampler(const GptOssSamplingParams& params,
                       [[maybe_unused]] const GptOssSamplerFactoryOptions& options) {
...
std::unique_ptr<GptOssISampler> create_gpt_oss_sampler(const GptOssSamplingParams& params) {
    return create_gpt_oss_sampler(params, GptOssSamplerFactoryOptions{});
}

std::unique_ptr<GptOssISampler> create_gpt_oss_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
