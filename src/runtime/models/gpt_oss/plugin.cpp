...
resolve_kv_cache_runtime_sizing(const PipelineContext& ctx, const TrtModule& module,
                                const GptOssKvCacheNames& kv_names, DType cache_dtype,
                                const GptOssTriAttentionConfig& tri_cfg, int32_t kv_dim) {
...
class GptOssDecoderPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<GptOssTextGenerationPipeline>(
  ...
  private:
    ...
    load_split_prefill_module(const PipelineContext& ctx, cudaStream_t stream, const IoMap& io,
                              const GptOssKvCacheNames& kv_names, int32_t& prefill_profile_idx,
    ...
    static void build_kv_names(const PipelineContext& ctx, const IoMap& io,
        ...
        for (int32_t i = 0; i < ctx.config.num_layers; ++i) {
            kv_names.cache_k.push_back(gpt_oss_expand_layer_name(io.cache_k_pattern, i));
            kv_names.cache_v.push_back(gpt_oss_expand_layer_name(io.cache_v_pattern, i));
            kv_names.present_k.push_back(gpt_oss_expand_layer_name(io.present_k_pattern, i));
            kv_names.present_v.push_back(gpt_oss_expand_layer_name(io.present_v_pattern, i));
        }
    }
    ...
    static std::vector<GptOssTextGenerationPipeline::DecoderContext>
    build_decoder_contexts(BackendProfileModules profile_modules, int32_t runtime_rows,
        ...
        for (const auto& profile : profile_roles.decode_profiles) {
            ...
            decoders.push_back(GptOssTextGenerationPipeline::DecoderContext{
    ...
    static std::unique_ptr<GptOssInferenceState>
    build_inference_state(const PipelineContext& ctx, const KvCacheRuntimeSizing& sizing,
                          GptOssTriAttentionConfig& tri_cfg, DType cache_dtype, int32_t kv_dim,
                          GptOssKvCacheNames& kv_names, cudaStream_t stream) {
        std::unique_ptr<GptOssInferenceState> state;
        if (tri_cfg.enabled) {
            ...
            GptOssTriAttentionStats tri_stats = gpt_oss_parse_triattention_stats_json(
                stats_json, ctx.config.num_heads, ctx.config.num_kv_heads, ctx.config.num_layers);
            state = std::make_unique<GptOssTriAttentionKvCache>(
            ...
            state =
                std::make_unique<GptOssKvCache>(ctx.config.num_layers, sizing.runtime_rows, kv_dim,
                                                stream, cache_dtype, std::move(kv_names));
        }
        if (!state->ok())
            throw std::runtime_error("Failed to create GptOssKvCache");
        return state;
    }
    ...
    static void
    populate_text_gen_config(const PipelineContext& ctx, GptOssTextGenConfig& tgc, const IoMap& io,
                             const GptOssTextGenerationPipeline::DecoderContext& first_dec,
    ...
    static void apply_chat_template_format(const BundleFile& bundle, GptOssTextGenConfig& tgc) {
        std::string chat_tpl;
        ...
        tgc.chat_template_format = gpt_oss_detect_chat_template_format(chat_tpl);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_gpt_oss_plugin, GptOssDecoderPlugin,
                                       "gpt_oss_decoder_moe");
