...
// DeepseekV2TextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/deepseek_v2/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/deepseek_v2/argmax_kernel.h"
#include "runtime/models/deepseek_v2/sparse_multinomial_kernel.h"
...
class GreedySampler final : public DeepseekV2ISampler {
  public:
    ...
    DeepseekV2LogitsLocation logits_location() const override {
        return DeepseekV2LogitsLocation::HOST;
    }
...
class TorchCudaMultinomialSampler final : public DeepseekV2ISampler {
  public:
    ...
    DeepseekV2SampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        deepseek_v2_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const DeepseekV2TorchMultinomialExecutionPolicy policy =
                deepseek_v2_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public DeepseekV2ISampler {
  public:
    ...
    DeepseekV2SampleResult sample(const float* logits, int32_t vocab_size,
        ...
        deepseek_v2_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    DeepseekV2LogitsLocation logits_location() const override {
        return DeepseekV2LogitsLocation::DEVICE;
    }
...
DeepseekV2SamplingParams deepseek_v2_sampling_params_from_config(const GenerateConfig& cfg,
                                                                 int32_t default_eos) {
...
create_deepseek_v2_sampler(const DeepseekV2SamplingParams& params,
                           [[maybe_unused]] const DeepseekV2SamplerFactoryOptions& options) {
...
std::unique_ptr<DeepseekV2ISampler>
create_deepseek_v2_sampler(const DeepseekV2SamplingParams& params) {
    return create_deepseek_v2_sampler(params, DeepseekV2SamplerFactoryOptions{});
}

std::unique_ptr<DeepseekV2ISampler> create_deepseek_v2_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
