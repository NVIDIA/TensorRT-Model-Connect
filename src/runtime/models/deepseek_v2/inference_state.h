...
// DeepseekV2InferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   DeepseekV2KvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — DeepseekV2KvCache + family-owned recurrent state composed
...
class DeepseekV2InferenceState {
  public:
    virtual ~DeepseekV2InferenceState() = default;
    ...
    // DeepseekV2KvCache -> true. Family-owned recurrent state -> false.
