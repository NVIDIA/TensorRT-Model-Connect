#include "runtime/models/deepseek_v2/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/deepseek_v2/triattention_kernels.h"
#endif
...
DeepseekV2TriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return DeepseekV2TriAttentionScoreAggregation::kMean;
    if (value == "max")
        return DeepseekV2TriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(DeepseekV2TriAttentionScoreAggregation value) {
    switch (value) {
    case DeepseekV2TriAttentionScoreAggregation::kMean:
        return "mean";
    case DeepseekV2TriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

DeepseekV2TriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return DeepseekV2TriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return DeepseekV2TriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, DeepseekV2KvCacheNames& names) {
...
// override helpers that patched DeepseekV2TriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     DeepseekV2TriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(DeepseekV2TriAttentionConfig& cfg,
...
void fill_debug_from_registry(DeepseekV2TriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(DeepseekV2TriAttentionConfig& cfg,
...
static void validate_triattention_config(const DeepseekV2TriAttentionConfig& cfg,
...
DeepseekV2TriAttentionConfig
deepseek_v2_parse_triattention_bundle_config(const std::string& config_json,
                                             int32_t max_cache_length,
                                             const ::trtmc::config::ConfigBundle* runtime_config) {
    DeepseekV2TriAttentionConfig cfg;
...
void fill_stats_header(DeepseekV2TriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(DeepseekV2TriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(DeepseekV2TriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(DeepseekV2TriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(DeepseekV2TriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(DeepseekV2TriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(DeepseekV2TriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(DeepseekV2TriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(DeepseekV2TriAttentionStats& stats, const json& raw_stats,
...
bool finalize_sparse_stats(DeepseekV2TriAttentionStats& stats,
...
void parse_sparse_stats(DeepseekV2TriAttentionStats& stats, const json& root, int32_t half_dim) {
...
DeepseekV2TriAttentionStats deepseek_v2_parse_triattention_stats_json(const std::string& stats_json,
                                                                      int32_t num_attention_heads,
                                                                      int32_t num_key_value_heads,
                                                                      int32_t num_layers) {
    DeepseekV2TriAttentionStats stats;
...
void DeepseekV2TriAttentionKvCache::validate_shapes() {
...
void DeepseekV2TriAttentionKvCache::normalize_sampled_heads() {
...
void DeepseekV2TriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void DeepseekV2TriAttentionKvCache::allocate_layer_tensors() {
...
DeepseekV2TriAttentionKvCache::DeepseekV2TriAttentionKvCache(
    int32_t num_layers, int32_t num_kv_heads, int32_t max_length, int32_t kv_dim,
    cudaStream_t stream, DeepseekV2TriAttentionConfig config, DeepseekV2TriAttentionStats stats,
    DType cache_dtype, DeepseekV2KvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void DeepseekV2TriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void DeepseekV2TriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void DeepseekV2TriAttentionKvCache::initialize_gpu_state() {
...
bool DeepseekV2TriAttentionKvCache::core_selection_buffers_ready() const {
...
bool DeepseekV2TriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool DeepseekV2TriAttentionKvCache::can_use_gpu_selection() const {
...
void DeepseekV2TriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t DeepseekV2TriAttentionKvCache::preferred_cache_rows() const {
...
void DeepseekV2TriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void DeepseekV2TriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> DeepseekV2TriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows,
    DeepseekV2TriAttentionCompactionProfile* profile) const {
...
void DeepseekV2TriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t DeepseekV2TriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> DeepseekV2TriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
DeepseekV2TriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t> DeepseekV2TriAttentionKvCache::select_keep_indices(
    int32_t keep_budget, DeepseekV2TriAttentionCompactionProfile* profile) {
...
int32_t DeepseekV2TriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t DeepseekV2TriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> DeepseekV2TriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void DeepseekV2TriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, DeepseekV2TriAttentionCompactionProfile* profile) const {
...
bool DeepseekV2TriAttentionKvCache::layer_stats_shapes_valid(
    const DeepseekV2TriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void DeepseekV2TriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset,
                                                  int32_t d, int32_t half_dim, float& k_rot_real,
                                                  float& k_rot_imag) const {
    if (stats_.rope_style == DeepseekV2TriAttentionRopeStyle::kInterleaved) {
...
float DeepseekV2TriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == DeepseekV2TriAttentionScoreAggregation::kMax)
...
float DeepseekV2TriAttentionKvCache::score_one_row(
    const float* row_ptr, const DeepseekV2TriAttentionHeadStats& layer_stats,
...
void DeepseekV2TriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const DeepseekV2TriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void DeepseekV2TriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void DeepseekV2TriAttentionKvCache::accumulate_layer_fallback(
...
void DeepseekV2TriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max =
        config_.per_layer_aggregation == DeepseekV2TriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void DeepseekV2TriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
DeepseekV2TriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void DeepseekV2TriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
...
void DeepseekV2TriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> DeepseekV2TriAttentionKvCache::build_keep_indices_per_head(
...
bool DeepseekV2TriAttentionKvCache::process_layer_for_host_selection(
...
std::vector<int32_t> DeepseekV2TriAttentionKvCache::select_keep_indices_host(
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void DeepseekV2TriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                           int32_t total_tokens) const {
...
void DeepseekV2TriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void DeepseekV2TriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
DeepseekV2TriAttentionKvCache::GpuLayerResult
DeepseekV2TriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    std::vector<int32_t>& contributing_layers_by_cache_head,
    DeepseekV2TriAttentionCompactionProfile* profile) {
    ...
    const bool launched = deepseek_v2_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == DeepseekV2TriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == DeepseekV2TriAttentionScoreAggregation::kMax,
...
bool DeepseekV2TriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool DeepseekV2TriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, DeepseekV2TriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void DeepseekV2TriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> DeepseekV2TriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool DeepseekV2TriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> DeepseekV2TriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, DeepseekV2TriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void DeepseekV2TriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor,
                                                            int32_t rows, const std::string& path,
...
void DeepseekV2TriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool DeepseekV2TriAttentionKvCache::gpu_compaction_upload_keep(
...
bool DeepseekV2TriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                          std::size_t row_bytes) {
    const bool ok_k = deepseek_v2_triattention_compact_rows_gpu(
    ...
    const bool ok_v = deepseek_v2_triattention_compact_rows_gpu(
...
bool DeepseekV2TriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void DeepseekV2TriAttentionKvCache::compact_layer_on_host(
...
void DeepseekV2TriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    DeepseekV2TriAttentionCompactionProfile profile;
    DeepseekV2TriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void DeepseekV2TriAttentionKvCache::finalize_repack_profile(
    DeepseekV2TriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
DeepseekV2TriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
DeepseekV2TriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                         int32_t keep_count) const {
...
int32_t DeepseekV2TriAttentionKvCache::count_prefix_positions(
...
void DeepseekV2TriAttentionKvCache::log_compact_debug(
...
void DeepseekV2TriAttentionKvCache::log_compact_profile(
    const DeepseekV2TriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool DeepseekV2TriAttentionKvCache::should_emit_keep_dump(
    const DeepseekV2TriAttentionCompactionProfile* profile_ptr) const {
...
void DeepseekV2TriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "DeepseekV2TriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void DeepseekV2TriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void DeepseekV2TriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void DeepseekV2TriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool DeepseekV2TriAttentionKvCache::ok() const {
