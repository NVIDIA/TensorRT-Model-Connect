...
#include "runtime/models/deepseek_v2/inference_state.h"
...
enum class DeepseekV2TriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class DeepseekV2TriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct DeepseekV2TriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    DeepseekV2TriAttentionScoreAggregation score_aggregation{
        DeepseekV2TriAttentionScoreAggregation::kMean};
    DeepseekV2TriAttentionScoreAggregation per_layer_aggregation{
        DeepseekV2TriAttentionScoreAggregation::kMean};
...
struct DeepseekV2TriAttentionHeadStats {
...
struct DeepseekV2TriAttentionStats {
    int32_t head_dim{0};
    DeepseekV2TriAttentionRopeStyle rope_style{DeepseekV2TriAttentionRopeStyle::kHalf};
    ...
    std::vector<DeepseekV2TriAttentionHeadStats> layer_stats;
};

struct DeepseekV2TriAttentionCompactionProfile {
...
// Build a DeepseekV2TriAttentionConfig from the bundle's JSON plus (optionally) the
...
class DeepseekV2TriAttentionKvCache : public DeepseekV2InferenceState {
  public:
    DeepseekV2TriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                                  int32_t kv_dim, cudaStream_t stream,
                                  DeepseekV2TriAttentionConfig config,
                                  DeepseekV2TriAttentionStats stats,
                                  DType cache_dtype = DType::kFloat32,
                                  DeepseekV2KvCacheNames names = {});
  ...
  private:
    ...
    void finalize_repack_profile(DeepseekV2TriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const DeepseekV2TriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const DeepseekV2TriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        DeepseekV2TriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             DeepseekV2TriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const DeepseekV2TriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const DeepseekV2TriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const DeepseekV2TriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    DeepseekV2KvCacheNames names_;
    DeepseekV2TriAttentionConfig config_;
    DeepseekV2TriAttentionStats stats_;
