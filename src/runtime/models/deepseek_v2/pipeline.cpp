...
#include "runtime/models/deepseek_v2/pipeline.h"

#include "runtime/models/deepseek_v2/chat_templates.h"
#include "runtime/models/deepseek_v2/kv_cache.h"
#include "runtime/models/deepseek_v2/tensor_names.h"
...
std::vector<DeepseekV2TextGenerationPipeline::DecoderContext>
single_decoder_context(std::unique_ptr<TrtModule> decoder) {
    std::vector<DeepseekV2TextGenerationPipeline::DecoderContext> decoders;
    decoders.push_back(DeepseekV2TextGenerationPipeline::DecoderContext{0, std::move(decoder)});
    return decoders;
}
...
bool greedy_text_diffusion_params(const DeepseekV2SamplingParams& params) {
...
DeepseekV2TextGenerationPipeline::DeepseekV2TextGenerationPipeline(
    std::unique_ptr<TrtModule> decoder, std::unique_ptr<DeepseekV2InferenceState> state,
    DeepseekV2TextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<DeepseekV2ISampler> sampler,
    std::shared_ptr<void> distributed_owner)
    : DeepseekV2TextGenerationPipeline(single_decoder_context(std::move(decoder)), std::move(state),
...
DeepseekV2TextGenerationPipeline::DeepseekV2TextGenerationPipeline(
    std::vector<DecoderContext> decoders, std::unique_ptr<DeepseekV2InferenceState> state,
    DeepseekV2TextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<DeepseekV2ISampler> sampler,
    ...
    if (decoders_.empty()) {
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: no decoder modules");
    }
    ...
    if (!state_ || !state_->ok()) {
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: invalid inference state");
    }
...
static std::vector<int32_t> encode_prompt(const ITokenizer& tokenizer,
                                          const DeepseekV2TextGenConfig& config,
    ...
    if (cfg.use_chat_template && !config.chat_template_format.empty()) {
        effective = deepseek_v2_apply_chat_template(config.chat_template_format, prompt,
                                                    cfg.enable_thinking);
        templated = true;
    }
...
TextResult DeepseekV2TextGenerationPipeline::generate(const std::string& prompt,
                                                      const GenerateConfig& cfg) {
    if (!tokenizer_) {
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: no tokenizer configured");
    }
...
DeepseekV2TextGenerationPipeline::generate_ids(const std::vector<int32_t>& input_ids,
                                               const GenerateConfig& cfg) {
...
DeepseekV2TextGenerationPipeline::make_step_sampler(const DeepseekV2SamplingParams& params) {
    ...
    if (prefer_gpu_greedy_ && greedy_params) {
        if (auto gpu = create_deepseek_v2_gpu_greedy_sampler(stream_))
            return gpu;
    }
    return create_deepseek_v2_sampler(params);
}
...
namespace {
bool gather_prefill_kv_pointers(TrtModule& prefill, const DeepseekV2TextGenConfig& cfg,
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        pk[li] = prefill.device_ptr(deepseek_v2_expand_layer_name(cfg.present_k_pattern, i));
        pv[li] = prefill.device_ptr(deepseek_v2_expand_layer_name(cfg.present_v_pattern, i));
...
bool batched_prefill_supported(const TrtModule* prefill, const DeepseekV2TextGenConfig& cfg,
                               int32_t sq, DeepseekV2InferenceState* state) {
    ...
    if (cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        return false;
    return dynamic_cast<DeepseekV2KvCache*>(state) != nullptr;
}
} // namespace

bool DeepseekV2TextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
                                                           std::vector<float>& logits) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    if (!batched_prefill_supported(prefill_.get(), config_, sq, state_.get()))
        return false;
    auto* kv = static_cast<DeepseekV2KvCache*>(state_.get());
...
void DeepseekV2TextGenerationPipeline::prime_decoder_after_batched_prefill(
...
TrtModule& DeepseekV2TextGenerationPipeline::require_block_prefill(int32_t sq,
                                                                   TrtModule* prefill_override) {
    TrtModule* prefill = prefill_override != nullptr ? prefill_override : prefill_.get();
    if (prefill == nullptr)
        throw std::runtime_error(
            "DeepseekV2TextGenerationPipeline: block generation requires prefill module");
    if (sq <= 0)
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: empty block");
    if (config_.prefill_max_length > 0 && sq > config_.prefill_max_length) {
        throw std::runtime_error(
            "DeepseekV2TextGenerationPipeline: block length exceeds prefill profile");
    }
    return *prefill;
}

DeepseekV2KvCache& DeepseekV2TextGenerationPipeline::require_block_kv_cache() {
    auto* kv = dynamic_cast<DeepseekV2KvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error(
            "DeepseekV2TextGenerationPipeline: block generation requires DeepseekV2KvCache");
    return *kv;
}

void DeepseekV2TextGenerationPipeline::copy_block_logits(const TensorMap& outputs,
                                                         std::vector<float>& logits) const {
    auto logits_it = outputs.find(config_.logits_output_name);
    if (logits_it == outputs.end())
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: prefill module has no '" +
...
void DeepseekV2TextGenerationPipeline::append_prefill_kv(DeepseekV2KvCache& kv, TrtModule& prefill,
                                                         int32_t sq) {
    std::vector<const void*> pk, pv;
    if (!gather_prefill_kv_pointers(prefill, config_, pk, pv)) {
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: prefill module is missing "
                                 "present_k/present_v outputs");
    }
    kv.append_prefill_kv(pk, pv, sq);
}

void DeepseekV2TextGenerationPipeline::run_prefill_block(const std::vector<int32_t>& input_ids,
                                                         bool bidirectional, bool append_kv,
                                                         std::vector<float>& logits,
                                                         TrtModule* prefill_override) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    TrtModule& prefill = require_block_prefill(sq, prefill_override);
    DeepseekV2KvCache& kv = require_block_kv_cache();
...
std::string
DeepseekV2TextGenerationPipeline::resolve_generation_mode(const GenerateConfig& cfg) const {
...
int32_t DeepseekV2TextGenerationPipeline::resolve_text_diffusion_block_length(
    ...
    if (!config_.supports_text_diffusion || config_.mask_token_id < 0)
        throw std::runtime_error(
            "DeepseekV2TextGenerationPipeline: bundle does not support text diffusion");
    ...
    if (require_divisible && max_new_tokens % block_len != 0) {
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: diffusion mode requires "
                                 "max_new_tokens % block_length == 0");
    }
    return block_len;
}

int32_t DeepseekV2TextGenerationPipeline::seed_next_token_from_prefill(
    ...
    if (static_cast<int32_t>(logits.size()) < vocab)
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: missing prefill logits");
    return argmax_with_confidence(logits.data() + logits.size() - static_cast<std::size_t>(vocab),
                                  vocab, 0)
        .token_id;
}

void DeepseekV2TextGenerationPipeline::fill_diffusion_block(std::vector<int32_t>& block,
                                                            std::vector<float>& logits,
    ...
    for (int32_t step = 0; step < block_len && has_mask_token(block, config_.mask_token_id);
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "DeepseekV2TextGenerationPipeline: diffusion engine must output full block logits");
        }
...
int32_t DeepseekV2TextGenerationPipeline::verify_diffusion_block(const std::vector<int32_t>& block,
                                                                 std::vector<float>& logits,
                                                                 int32_t block_len, int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "DeepseekV2TextGenerationPipeline: diffusion engine must output full verify logits");
    }
...
bool DeepseekV2TextGenerationPipeline::append_tokens_until_eos(
    const std::vector<int32_t>& tokens, std::vector<int32_t>& output,
    const DeepseekV2SamplingParams& params) const {
...
void DeepseekV2TextGenerationPipeline::fill_linear_spec_block(
    ...
    while (has_mask_token(block, config_.mask_token_id)) {
        ...
        run_prefill_block(block, /*bidirectional=*/true, /*append_kv=*/false, logits,
                          draft_prefill);
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error("DeepseekV2TextGenerationPipeline: linear_spec engine must "
...
std::vector<int32_t>
DeepseekV2TextGenerationPipeline::verify_linear_spec_block(const std::vector<int32_t>& block,
                                                           std::vector<float>& logits,
                                                           int32_t block_len, int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "DeepseekV2TextGenerationPipeline: linear_spec engine must output full verify logits");
    }
...
int32_t
DeepseekV2TextGenerationPipeline::count_linear_spec_accepts(const std::vector<int32_t>& ar_tokens,
                                                            const std::vector<int32_t>& block) {
    if (ar_tokens.empty())
        return 0;
    if (block.size() < 2)
...
bool DeepseekV2TextGenerationPipeline::append_linear_spec_tokens(
    const std::vector<int32_t>& ar_tokens, int32_t emit_count, std::vector<int32_t>& output,
    int32_t& generated, const DeepseekV2SamplingParams& params) const {
...
DeepseekV2TextGenerationPipeline::TimedGenResult
DeepseekV2TextGenerationPipeline::generate_from_ids(const std::vector<int32_t>& input_ids,
    ...
    if (mode != "auto" && mode != "ar")
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");

    DeepseekV2ISampler* active_sampler = sampler_.get();
    std::unique_ptr<DeepseekV2ISampler> local_sampler;
    if (!active_sampler) {
    ...
    const bool gpu_sampling =
        (active_sampler->logits_location() == DeepseekV2LogitsLocation::DEVICE);
...
DeepseekV2TextGenerationPipeline::TimedGenResult
DeepseekV2TextGenerationPipeline::generate_diffusion_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
    const DeepseekV2SamplingParams& params, const GenerateConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: diffusion mode currently "
...
DeepseekV2TextGenerationPipeline::TimedGenResult
DeepseekV2TextGenerationPipeline::generate_linear_spec_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
    const DeepseekV2SamplingParams& params, const GenerateConfig& cfg, bool use_lora_draft) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: linear_spec mode currently "
                                 "supports greedy temperature=0 "
                                 "generation");
    }
    if (use_lora_draft && linear_spec_lora_prefill_ == nullptr) {
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: linear_spec_lora mode requires "
    ...
    if (kv == nullptr)
        throw std::runtime_error(
            "DeepseekV2TextGenerationPipeline: linear_spec requires DeepseekV2KvCache");
...
bool DeepseekV2TextGenerationPipeline::should_stop_on_answer(const std::vector<int32_t>& output,
...
void DeepseekV2TextGenerationPipeline::log_decode_summary(int32_t steps, double ms) const {
...
int32_t DeepseekV2TextGenerationPipeline::run_decode_loop(
    DeepseekV2ISampler* sampler, const DeepseekV2SamplingParams& params,
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        ...
        const DeepseekV2SampleResult result = sampler->sample(sample_ptr, vocab_size, params);
...
int32_t DeepseekV2TextGenerationPipeline::select_decoder_index(int32_t desired_rows) const {
    if (decoders_.size() == 1)
        return 0;
...
TrtModule& DeepseekV2TextGenerationPipeline::bind_decoder_for_step() {
...
void DeepseekV2TextGenerationPipeline::run_step(int32_t token_id, std::vector<float>& logits) {
    TensorMap inputs;
    ...
    if (it == outputs.end()) {
        throw std::runtime_error("DeepseekV2TextGenerationPipeline: no '" + logits_output_name_ +
                                 "' output");
    }
...
void DeepseekV2TextGenerationPipeline::run_step_device(int32_t token_id) {
    TensorMap inputs;
...
int32_t DeepseekV2TextGenerationPipeline::argmax(const std::vector<float>& logits) {
    if (logits.empty())
        return 0;
