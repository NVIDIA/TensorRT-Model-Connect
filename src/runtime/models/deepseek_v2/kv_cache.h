...
// DeepseekV2KvCache: autoregressive KV cache state manager.
...
class DeepseekV2KvCache : public DeepseekV2InferenceState {
  public:
    ...
    DeepseekV2KvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                      DType cache_dtype = DType::kFloat32, DeepseekV2KvCacheNames names = {});

    // --- DeepseekV2InferenceState overrides ---
    void reset() override;
    ...
    // --- DeepseekV2KvCache-specific methods (not on the interface) ---
    ...
    // — too big for DeepseekV2KvCache's single-row present buffer. The caller reads
