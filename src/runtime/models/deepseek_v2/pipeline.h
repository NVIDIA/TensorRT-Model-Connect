...
//
// Composes: TrtModule (decoder) + DeepseekV2KvCache + ITokenizer for this runtime
...
class DeepseekV2KvCache;

struct DeepseekV2TextGenConfig {
...
class DeepseekV2TextGenerationPipeline final : public IPipeline {
  public:
    ...
    DeepseekV2TextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                     std::unique_ptr<DeepseekV2InferenceState> state,
                                     DeepseekV2TextGenConfig config, cudaStream_t stream,
                                     std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                     std::string model_id_str = "",
                                     std::unique_ptr<DeepseekV2ISampler> sampler = nullptr,
                                     std::shared_ptr<void> distributed_owner = nullptr);
    DeepseekV2TextGenerationPipeline(std::vector<DecoderContext> decoders,
                                     std::unique_ptr<DeepseekV2InferenceState> state,
                                     DeepseekV2TextGenConfig config, cudaStream_t stream,
                                     std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                     std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "DeepseekV2TextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<DeepseekV2InferenceState> state_;
    DeepseekV2TextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<DeepseekV2ISampler> sampler_;
    ...
    DeepseekV2KvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(DeepseekV2KvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(DeepseekV2ISampler* sampler, const DeepseekV2SamplingParams& params,
    ...
    std::unique_ptr<DeepseekV2ISampler> make_step_sampler(const DeepseekV2SamplingParams& params);
