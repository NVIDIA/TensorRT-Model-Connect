...
// SegformerInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   SegformerKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — SegformerKvCache + family-owned recurrent state composed
...
class SegformerInferenceState {
  public:
    virtual ~SegformerInferenceState() = default;
    ...
    // SegformerKvCache -> true. Family-owned recurrent state -> false.
