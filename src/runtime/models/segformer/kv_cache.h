...
// SegformerKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/segformer/inference_state.h"
...
class SegformerKvCache : public SegformerInferenceState {
  public:
    ...
    SegformerKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                     DType cache_dtype = DType::kFloat32, SegformerKvCacheNames names = {});

    // --- SegformerInferenceState overrides ---
    void reset() override;
    ...
    // --- SegformerKvCache-specific methods (not on the interface) ---
    ...
    // — too big for SegformerKvCache's single-row present buffer. The caller reads
