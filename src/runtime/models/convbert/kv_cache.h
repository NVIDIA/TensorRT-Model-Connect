...
// ConvbertKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/convbert/inference_state.h"
...
class ConvbertKvCache : public ConvbertInferenceState {
  public:
    ...
    ConvbertKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                    DType cache_dtype = DType::kFloat32, ConvbertKvCacheNames names = {});

    // --- ConvbertInferenceState overrides ---
    void reset() override;
    ...
    // --- ConvbertKvCache-specific methods (not on the interface) ---
    ...
    // — too big for ConvbertKvCache's single-row present buffer. The caller reads
