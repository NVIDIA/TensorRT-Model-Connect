...
// ConvbertInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   ConvbertKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — ConvbertKvCache + family-owned recurrent state composed
...
class ConvbertInferenceState {
  public:
    virtual ~ConvbertInferenceState() = default;
    ...
    // ConvbertKvCache -> true. Family-owned recurrent state -> false.
