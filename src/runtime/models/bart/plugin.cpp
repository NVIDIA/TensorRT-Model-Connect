...
#include "runtime/models/bart/decode_runtime.h"
...
class BartPipeline final : public IPipeline {
  public:
    BartPipeline(std::unique_ptr<TrtModule> encoder, std::unique_ptr<TrtModule> decoder,
    ...
    const char* pipeline_type() const override { return "BartPipeline"; }
...
class BartPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        auto enc_loaded = load_trt_module_from_plan(
            ctx.backend, find_section(ctx.bundle, "vision_engine_plan"), "bart encoder", opts);
        ...
        auto dec_loaded = load_trt_module_from_plan(
            ctx.backend, find_section(ctx.bundle, decoder_section), "bart decoder", decoder_opts);
        ...
        auto state = std::make_unique<BartKvCache>(dl, max_cache, kv_dim, stream);
        if (!state->ok())
            throw std::runtime_error("Failed to create BartKvCache for BART decoder");
        auto tok = create_tokenizer_from_bundle(ctx.bundle);
        return std::make_unique<BartPipeline>(
...
REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_bart_plugin, BartPlugin,
                                       "bart_seq2seq_encoder_decoder");
