...
// BartKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/bart/inference_state.h"
...
class BartKvCache : public BartInferenceState {
  public:
    ...
    BartKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                DType cache_dtype = DType::kFloat32, BartKvCacheNames names = {});

    // --- BartInferenceState overrides ---
    void reset() override;
    ...
    // --- BartKvCache-specific methods (not on the interface) ---

    // DEPRECATED: Use prepare_step() instead.
    // Kept for backward compatibility with tests that call this directly.
    void bart_build_attention_mask(std::vector<float>& mask) const;
    ...
    // — too big for BartKvCache's single-row present buffer. The caller reads
