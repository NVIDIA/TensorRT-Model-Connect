...
// BartInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   BartKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — BartKvCache + family-owned recurrent state composed
...
class BartInferenceState {
  public:
    virtual ~BartInferenceState() = default;
    ...
    // BartKvCache -> true. Family-owned recurrent state -> false.
