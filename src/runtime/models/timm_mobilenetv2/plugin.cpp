...
// TimmMobilenetv2Plugin: handles timm MobileNetV2 image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_mobilenetv2/pipeline.h"
...
TimmMobilenetv2PreprocessConfig make_timm_mobilenetv2_preprocess_config(const std::string& json) {
    TimmMobilenetv2PreprocessConfig cfg;
...
class TimmMobilenetv2Plugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmMobilenetv2ImageClassificationPipeline>(
            std::move(loaded.module), make_timm_mobilenetv2_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_mobilenetv2_plugin, TimmMobilenetv2Plugin,
                                       "timm_mobilenetv2_image_classification");
