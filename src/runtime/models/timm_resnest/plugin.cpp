...
// TimmResnestPlugin: handles timm ResNeSt image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_resnest/pipeline.h"
...
TimmResnestPreprocessConfig make_timm_resnest_preprocess_config(const std::string& json) {
    TimmResnestPreprocessConfig cfg;
...
class TimmResnestPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmResnestImageClassificationPipeline>(
            std::move(loaded.module), make_timm_resnest_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_resnest_plugin, TimmResnestPlugin,
                                       "timm_resnest_image_classification");
