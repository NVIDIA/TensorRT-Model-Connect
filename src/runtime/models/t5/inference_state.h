...
// T5InferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   T5KvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — T5KvCache + family-owned recurrent state composed
...
class T5InferenceState {
  public:
    virtual ~T5InferenceState() = default;
    ...
    // T5KvCache -> true. Family-owned recurrent state -> false.
