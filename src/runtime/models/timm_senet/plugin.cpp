...
// TimmSenetPlugin: handles timm SENet image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_senet/pipeline.h"
...
TimmSenetPreprocessConfig make_timm_senet_preprocess_config(const std::string& json) {
    TimmSenetPreprocessConfig cfg;
...
class TimmSenetPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmSenetImageClassificationPipeline>(
            std::move(loaded.module), make_timm_senet_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_senet_plugin, TimmSenetPlugin,
                                       "timm_senet_image_classification");
