...
stbir_filter resolve_timm_senet_resize_filter(const std::string& interpolation) {
    if (interpolation == "bilinear")
        return STBIR_FILTER_TRIANGLE;
    if (interpolation == "bicubic")
        return STBIR_FILTER_CATMULLROM;
    throw std::invalid_argument("Unsupported timm SENet interpolation: " + interpolation);
}

void validate_timm_senet_preprocess_config(const TimmSenetPreprocessConfig& config) {
    if (config.input_image_h <= 0 || config.input_image_w <= 0) {
        throw std::invalid_argument("timm SENet input dimensions must be positive");
    }
    if (config.crop_pct <= 0.0F || config.crop_pct > 1.0F) {
        throw std::invalid_argument("timm SENet crop_pct must be in (0, 1]");
    }
    if (config.image_mean.size() != 3 || config.image_std.size() != 3) {
        throw std::invalid_argument("timm SENet image mean/std must contain three channels");
    }
    for (float value : config.image_std) {
        if (value == 0.0F)
            throw std::invalid_argument("timm SENet image std must be non-zero");
    }
}
...
TimmSenetResizeShape compute_timm_senet_resize_shape(int32_t image_height, int32_t image_width,
                                                     const TimmSenetPreprocessConfig& config) {
    if (image_height <= 0 || image_width <= 0) {
        throw std::invalid_argument("timm SENet source dimensions must be positive");
    }
    validate_timm_senet_preprocess_config(config);
...
std::vector<float> preprocess_timm_senet_image(const float* image_pixels, int32_t image_height,
                                               int32_t image_width,
                                               const TimmSenetPreprocessConfig& config) {
    if (image_pixels == nullptr || image_height <= 0 || image_width <= 0) {
        throw std::invalid_argument("timm SENet source image must be non-empty");
    }
    validate_timm_senet_preprocess_config(config);

    const auto resize_shape = compute_timm_senet_resize_shape(image_height, image_width, config);
    ...
    if (stbir_resize(image_pixels, image_width, image_height,
                     ...
                     resolve_timm_senet_resize_filter(config.interpolation)) == nullptr) {
        throw std::runtime_error("Failed to resize timm SENet input image");
    }
