...
class Lfm2MoeByteLevelDecoderTokenizer final : public ITokenizer {
  public:
    explicit Lfm2MoeByteLevelDecoderTokenizer(std::unique_ptr<ITokenizer> inner)
    ...
    std::string decode(const std::vector<int32_t>& ids) const override {
        return lfm2_moe_decode_gpt2_byte_level(inner_->decode(ids));
    }
...
bool lfm2_moe_uses_sequence_byte_level_decoder(const char* tokenizer_json, std::size_t size) {
...
std::string lfm2_moe_decode_gpt2_byte_level(std::string_view encoded) {
