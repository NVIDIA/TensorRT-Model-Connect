...
void validate_lfm2_moe_native_contract(const PipelineContext& ctx) {
...
Lfm2MoeGeometry parse_lfm2_moe_geometry(const PipelineContext& ctx) {
    Lfm2MoeGeometry geometry;
...
void validate_lfm2_moe_attention_geometry(const BaseConfig& config) {
...
Lfm2MoeTextGenConfig create_lfm2_moe_generation_config(const PipelineContext& ctx) {
    Lfm2MoeTextGenConfig generation;
    ...
    apply_lfm2_moe_chat_template_format(ctx.bundle, generation);
    return generation;
}

} // namespace

class Lfm2MoePlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        validate_lfm2_moe_native_contract(ctx);
        const auto geometry = parse_lfm2_moe_geometry(ctx);
        validate_lfm2_moe_attention_geometry(ctx.config);

        ModuleCreateOptions options;
        options.runtime_cache_path = ctx.runtime_cache_path.c_str();
        options.cuda_graphs = ctx.cuda_graphs;
        auto decoder =
            load_lfm2_moe_module(ctx.backend, find_section(ctx.bundle, "engine_plan"), options);
        const cudaStream_t stream = decoder->stream();
        const DType state_dtype = lfm2_moe_state_dtype(ctx.config.precision);

        auto state = create_lfm2_moe_state(ctx, geometry, stream, state_dtype);
        auto generation = create_lfm2_moe_generation_config(ctx);
        auto tokenizer = create_lfm2_moe_tokenizer(ctx.bundle);
        return std::make_unique<Lfm2MoeTextGenerationPipeline>(
            std::move(decoder), std::move(state), std::move(generation), stream,
            std::move(tokenizer), ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_lfm2_moe_plugin, Lfm2MoePlugin,
                                       "lfm2_moe_hybrid_conv_attention");
