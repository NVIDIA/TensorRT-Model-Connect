...
void validate_geometry(int32_t num_layers, int32_t max_length, int32_t num_kv_heads,
    ...
    if (num_layers <= 0 || max_length <= 0 || num_kv_heads <= 0 || head_dim <= 0 ||
        ...
        throw std::invalid_argument("Lfm2MoeKvCache requires positive, representable geometry");
    }
}

void populate_default_names(Lfm2MoeKvCacheNames& names, int32_t num_layers) {
...
void validate_name_counts(const Lfm2MoeKvCacheNames& names, std::size_t expected) {
    if (names.cache_k.size() != expected || names.cache_v.size() != expected ||
        ...
        throw std::invalid_argument(
            "Lfm2MoeKvCache tensor-name count does not match attention layers");
    }
}
...
void Lfm2MoeKvCache::validate_contract(TrtModule& module) const {
...
void Lfm2MoeKvCache::bind_to(TrtModule& module) {
...
void Lfm2MoeKvCache::prepare_step(TensorMap& inputs, int32_t seq_len) {
    if (seq_len <= 0)
...
std::size_t Lfm2MoeKvCache::device_memory_bytes() const {
    std::size_t total = 0;
