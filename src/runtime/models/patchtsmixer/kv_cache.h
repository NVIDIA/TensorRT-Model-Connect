...
// PatchtsmixerKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/patchtsmixer/inference_state.h"
...
struct PatchtsmixerKvCacheNames {
...
class PatchtsmixerKvCache : public PatchtsmixerInferenceState {
  public:
    ...
    PatchtsmixerKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                        DType cache_dtype = DType::kFloat32, PatchtsmixerKvCacheNames names = {});

    // --- PatchtsmixerInferenceState overrides ---
    void reset() override;
    ...
    // --- PatchtsmixerKvCache-specific methods (not on the interface) ---
    ...
    // — too big for PatchtsmixerKvCache's single-row present buffer. The caller reads
  ...
  private:
    ...
    PatchtsmixerKvCacheNames names_;
