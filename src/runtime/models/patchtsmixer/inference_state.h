...
// PatchtsmixerInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   PatchtsmixerKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — PatchtsmixerKvCache + family-owned recurrent state composed
...
class PatchtsmixerInferenceState {
  public:
    virtual ~PatchtsmixerInferenceState() = default;
    ...
    // PatchtsmixerKvCache -> true. Family-owned recurrent state -> false.
