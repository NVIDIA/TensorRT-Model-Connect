...
//
// Composes: TrtModule (decoder) + GemmaKvCache + ITokenizer for this runtime
...
class GemmaTextGenerationPipeline final : public IPipeline {
  public:
    ...
    GemmaTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                std::unique_ptr<GemmaInferenceState> state,
                                GemmaTextGenConfig config, cudaStream_t stream,
                                std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                std::string model_id_str = "",
                                std::unique_ptr<GemmaISampler> sampler = nullptr,
                                std::shared_ptr<void> distributed_owner = nullptr);
    GemmaTextGenerationPipeline(std::vector<DecoderContext> decoders,
                                std::unique_ptr<GemmaInferenceState> state,
                                GemmaTextGenConfig config, cudaStream_t stream,
                                std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "GemmaTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<GemmaInferenceState> state_;
    GemmaTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<GemmaISampler> sampler_;
    ...
    GemmaKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(GemmaKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(GemmaISampler* sampler, const GemmaSamplingParams& params,
    ...
    std::unique_ptr<GemmaISampler> make_step_sampler(const GemmaSamplingParams& params);
