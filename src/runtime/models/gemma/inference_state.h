...
// GemmaInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   GemmaKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — GemmaKvCache + family-owned recurrent state composed
...
class GemmaInferenceState {
  public:
    virtual ~GemmaInferenceState() = default;
    ...
    // GemmaKvCache -> true. Family-owned recurrent state -> false.
