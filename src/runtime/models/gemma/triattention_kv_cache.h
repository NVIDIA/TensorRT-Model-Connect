...
#include "runtime/models/gemma/inference_state.h"
...
enum class GemmaTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class GemmaTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct GemmaTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    GemmaTriAttentionScoreAggregation score_aggregation{GemmaTriAttentionScoreAggregation::kMean};
    GemmaTriAttentionScoreAggregation per_layer_aggregation{
        GemmaTriAttentionScoreAggregation::kMean};
...
struct GemmaTriAttentionHeadStats {
...
struct GemmaTriAttentionStats {
    int32_t head_dim{0};
    GemmaTriAttentionRopeStyle rope_style{GemmaTriAttentionRopeStyle::kHalf};
    ...
    std::vector<GemmaTriAttentionHeadStats> layer_stats;
};

struct GemmaTriAttentionCompactionProfile {
...
// Build a GemmaTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class GemmaTriAttentionKvCache : public GemmaInferenceState {
  public:
    GemmaTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                             int32_t kv_dim, cudaStream_t stream, GemmaTriAttentionConfig config,
                             GemmaTriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(GemmaTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const GemmaTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const GemmaTriAttentionCompactionProfile* profile_ptr) const;
    ...
    std::vector<int32_t> select_keep_indices(int32_t keep_budget,
                                             GemmaTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             GemmaTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                GemmaTriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const GemmaTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const GemmaTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const GemmaTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    GemmaKvCacheNames names_;
    GemmaTriAttentionConfig config_;
    GemmaTriAttentionStats stats_;
