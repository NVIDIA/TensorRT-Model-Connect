...
// GemmaKvCache: autoregressive KV cache state manager.
...
class GemmaKvCache : public GemmaInferenceState {
  public:
    ...
    GemmaKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                 DType cache_dtype = DType::kFloat32, GemmaKvCacheNames names = {});

    // --- GemmaInferenceState overrides ---
    void reset() override;
    ...
    // --- GemmaKvCache-specific methods (not on the interface) ---
    ...
    // — too big for GemmaKvCache's single-row present buffer. The caller reads
