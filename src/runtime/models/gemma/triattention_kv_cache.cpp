...
#include "runtime/models/gemma/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/gemma/triattention_kernels.h"
#endif
...
GemmaTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return GemmaTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return GemmaTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(GemmaTriAttentionScoreAggregation value) {
    switch (value) {
    case GemmaTriAttentionScoreAggregation::kMean:
        return "mean";
    case GemmaTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

GemmaTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return GemmaTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return GemmaTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, GemmaKvCacheNames& names) {
...
// override helpers that patched GemmaTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     GemmaTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(GemmaTriAttentionConfig& cfg,
...
void fill_debug_from_registry(GemmaTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(GemmaTriAttentionConfig& cfg, const std::string& config_json,
...
static void validate_triattention_config(const GemmaTriAttentionConfig& cfg,
                                         int32_t max_cache_length) {
    if (cfg.kv_budget < 1)
...
GemmaTriAttentionConfig
gemma_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                       const ::trtmc::config::ConfigBundle* runtime_config) {
    GemmaTriAttentionConfig cfg;
...
void fill_stats_header(GemmaTriAttentionStats& stats, const json& root, int32_t num_attention_heads,
...
void fill_stats_inv_freq(GemmaTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(GemmaTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(GemmaTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(GemmaTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(GemmaTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(GemmaTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(GemmaTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(GemmaTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(GemmaTriAttentionStats& stats, const std::vector<int32_t>& group_counts,
                           int32_t half_dim) {
    bool any_stats = false;
    for (int32_t layer = 0; layer < stats.num_layers; ++layer) {
        auto& layer_stats = stats.layer_stats[static_cast<std::size_t>(layer)];
...
void parse_sparse_stats(GemmaTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
GemmaTriAttentionStats gemma_parse_triattention_stats_json(const std::string& stats_json,
                                                           int32_t num_attention_heads,
                                                           int32_t num_key_value_heads,
                                                           int32_t num_layers) {
    GemmaTriAttentionStats stats;
...
void GemmaTriAttentionKvCache::validate_shapes() {
...
void GemmaTriAttentionKvCache::normalize_sampled_heads() {
...
void GemmaTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void GemmaTriAttentionKvCache::allocate_layer_tensors() {
...
GemmaTriAttentionKvCache::GemmaTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                   int32_t max_length, int32_t kv_dim,
                                                   cudaStream_t stream,
                                                   GemmaTriAttentionConfig config,
                                                   GemmaTriAttentionStats stats, DType cache_dtype,
                                                   GemmaKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void GemmaTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void GemmaTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void GemmaTriAttentionKvCache::initialize_gpu_state() {
...
bool GemmaTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool GemmaTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool GemmaTriAttentionKvCache::can_use_gpu_selection() const {
...
void GemmaTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t GemmaTriAttentionKvCache::preferred_cache_rows() const {
...
void GemmaTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void GemmaTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> GemmaTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows, GemmaTriAttentionCompactionProfile* profile) const {
...
void GemmaTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t GemmaTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> GemmaTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t> GemmaTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
GemmaTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                              GemmaTriAttentionCompactionProfile* profile) {
...
int32_t GemmaTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t GemmaTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> GemmaTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void GemmaTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, GemmaTriAttentionCompactionProfile* profile) const {
...
bool GemmaTriAttentionKvCache::layer_stats_shapes_valid(
    const GemmaTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void GemmaTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                             int32_t half_dim, float& k_rot_real,
                                             float& k_rot_imag) const {
    if (stats_.rope_style == GemmaTriAttentionRopeStyle::kInterleaved) {
...
float GemmaTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == GemmaTriAttentionScoreAggregation::kMax)
...
float GemmaTriAttentionKvCache::score_one_row(
    const float* row_ptr, const GemmaTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void GemmaTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const GemmaTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void GemmaTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void GemmaTriAttentionKvCache::accumulate_layer_fallback(
...
void GemmaTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == GemmaTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void GemmaTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
GemmaTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void GemmaTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == GemmaTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void GemmaTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> GemmaTriAttentionKvCache::build_keep_indices_per_head(
...
bool GemmaTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, GemmaTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> GemmaTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, GemmaTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void GemmaTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                      int32_t total_tokens) const {
...
void GemmaTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void GemmaTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
GemmaTriAttentionKvCache::GpuLayerResult GemmaTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = gemma_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == GemmaTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == GemmaTriAttentionScoreAggregation::kMax,
...
bool GemmaTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool GemmaTriAttentionKvCache::upload_gpu_trig_phases(int32_t num_offsets, int32_t half_dim,
                                                      GemmaTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void GemmaTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> GemmaTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool GemmaTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> GemmaTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, GemmaTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void GemmaTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void GemmaTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool GemmaTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool GemmaTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                     std::size_t row_bytes) {
    const bool ok_k = gemma_triattention_compact_rows_gpu(
    ...
    const bool ok_v = gemma_triattention_compact_rows_gpu(
...
bool GemmaTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void GemmaTriAttentionKvCache::compact_layer_on_host(
...
void GemmaTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    GemmaTriAttentionCompactionProfile profile;
    GemmaTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void GemmaTriAttentionKvCache::finalize_repack_profile(
    GemmaTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
GemmaTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
GemmaTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                    int32_t keep_count) const {
...
int32_t GemmaTriAttentionKvCache::count_prefix_positions(
...
void GemmaTriAttentionKvCache::log_compact_debug(
...
void GemmaTriAttentionKvCache::log_compact_profile(
    const GemmaTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool GemmaTriAttentionKvCache::should_emit_keep_dump(
    const GemmaTriAttentionCompactionProfile* profile_ptr) const {
...
void GemmaTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "GemmaTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void GemmaTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void GemmaTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void GemmaTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool GemmaTriAttentionKvCache::ok() const {
