...
// GemmaISampler: token selection abstraction for autoregressive generation.
//
...
//
// GemmaLogitsLocation tells the pipeline whether to D2H transfer logits before
...
class GemmaISampler {
  public:
    ...
    virtual GemmaLogitsLocation logits_location() const = 0;
...
/// Build GemmaSamplingParams from GenerateConfig fields.
...
std::unique_ptr<GemmaISampler> create_gemma_sampler(const GemmaSamplingParams& params,
                                                    const GemmaSamplerFactoryOptions& options);
