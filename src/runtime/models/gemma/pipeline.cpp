...
#include "runtime/models/gemma/chat_templates.h"
#include "runtime/models/gemma/kv_cache.h"
#include "runtime/models/gemma/tensor_names.h"
...
std::vector<GemmaTextGenerationPipeline::DecoderContext>
single_decoder_context(std::unique_ptr<TrtModule> decoder) {
    std::vector<GemmaTextGenerationPipeline::DecoderContext> decoders;
    decoders.push_back(GemmaTextGenerationPipeline::DecoderContext{0, std::move(decoder)});
    return decoders;
}
...
bool greedy_text_diffusion_params(const GemmaSamplingParams& params) {
...
GemmaTextGenerationPipeline::GemmaTextGenerationPipeline(
    std::unique_ptr<TrtModule> decoder, std::unique_ptr<GemmaInferenceState> state,
    GemmaTextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<GemmaISampler> sampler,
    std::shared_ptr<void> distributed_owner)
    : GemmaTextGenerationPipeline(single_decoder_context(std::move(decoder)), std::move(state),
...
GemmaTextGenerationPipeline::GemmaTextGenerationPipeline(
    std::vector<DecoderContext> decoders, std::unique_ptr<GemmaInferenceState> state,
    GemmaTextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<GemmaISampler> sampler,
...
TextResult GemmaTextGenerationPipeline::generate(const std::string& prompt,
    ...
    if (!tokenizer_) {
        throw std::runtime_error("GemmaTextGenerationPipeline: no tokenizer configured");
    }
...
GemmaTextGenerationPipeline::make_step_sampler(const GemmaSamplingParams& params) {
    ...
    if (prefer_gpu_greedy_ && greedy_params) {
        if (auto gpu = create_gemma_gpu_greedy_sampler(stream_))
            return gpu;
    }
    return create_gemma_sampler(params);
}
...
namespace {
bool gather_prefill_kv_pointers(TrtModule& prefill, const GemmaTextGenConfig& cfg,
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        pk[li] = prefill.device_ptr(gemma_expand_layer_name(cfg.present_k_pattern, i));
        pv[li] = prefill.device_ptr(gemma_expand_layer_name(cfg.present_v_pattern, i));
...
bool batched_prefill_supported(const TrtModule* prefill, const GemmaTextGenConfig& cfg, int32_t sq,
                               GemmaInferenceState* state) {
    ...
    if (cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        return false;
    return dynamic_cast<GemmaKvCache*>(state) != nullptr;
}
} // namespace

bool GemmaTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
...
TrtModule& GemmaTextGenerationPipeline::require_block_prefill(int32_t sq,
                                                              TrtModule* prefill_override) {
    TrtModule* prefill = prefill_override != nullptr ? prefill_override : prefill_.get();
    if (prefill == nullptr)
        throw std::runtime_error(
            "GemmaTextGenerationPipeline: block generation requires prefill module");
    if (sq <= 0)
        throw std::runtime_error("GemmaTextGenerationPipeline: empty block");
    if (config_.prefill_max_length > 0 && sq > config_.prefill_max_length) {
        throw std::runtime_error(
            "GemmaTextGenerationPipeline: block length exceeds prefill profile");
    }
    return *prefill;
}

GemmaKvCache& GemmaTextGenerationPipeline::require_block_kv_cache() {
    auto* kv = dynamic_cast<GemmaKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error(
            "GemmaTextGenerationPipeline: block generation requires GemmaKvCache");
    return *kv;
}

void GemmaTextGenerationPipeline::copy_block_logits(const TensorMap& outputs,
                                                    std::vector<float>& logits) const {
    auto logits_it = outputs.find(config_.logits_output_name);
    if (logits_it == outputs.end())
        throw std::runtime_error("GemmaTextGenerationPipeline: prefill module has no '" +
...
void GemmaTextGenerationPipeline::append_prefill_kv(GemmaKvCache& kv, TrtModule& prefill,
                                                    int32_t sq) {
    std::vector<const void*> pk, pv;
    if (!gather_prefill_kv_pointers(prefill, config_, pk, pv)) {
        throw std::runtime_error(
            "GemmaTextGenerationPipeline: prefill module is missing present_k/present_v outputs");
    }
    kv.append_prefill_kv(pk, pv, sq);
}

void GemmaTextGenerationPipeline::run_prefill_block(const std::vector<int32_t>& input_ids,
                                                    bool bidirectional, bool append_kv,
                                                    std::vector<float>& logits,
                                                    TrtModule* prefill_override) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    TrtModule& prefill = require_block_prefill(sq, prefill_override);
    GemmaKvCache& kv = require_block_kv_cache();
...
std::string GemmaTextGenerationPipeline::resolve_generation_mode(const GenerateConfig& cfg) const {
...
int32_t GemmaTextGenerationPipeline::resolve_text_diffusion_block_length(
    ...
    if (!config_.supports_text_diffusion || config_.mask_token_id < 0)
        throw std::runtime_error(
            "GemmaTextGenerationPipeline: bundle does not support text diffusion");
    ...
    if (require_divisible && max_new_tokens % block_len != 0) {
        throw std::runtime_error("GemmaTextGenerationPipeline: diffusion mode requires "
                                 "max_new_tokens % block_length == 0");
    }
    return block_len;
}

int32_t GemmaTextGenerationPipeline::seed_next_token_from_prefill(
    ...
    if (static_cast<int32_t>(logits.size()) < vocab)
        throw std::runtime_error("GemmaTextGenerationPipeline: missing prefill logits");
    return argmax_with_confidence(logits.data() + logits.size() - static_cast<std::size_t>(vocab),
                                  vocab, 0)
        .token_id;
}

void GemmaTextGenerationPipeline::fill_diffusion_block(std::vector<int32_t>& block,
                                                       std::vector<float>& logits,
    ...
    for (int32_t step = 0; step < block_len && has_mask_token(block, config_.mask_token_id);
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "GemmaTextGenerationPipeline: diffusion engine must output full block logits");
        }
...
int32_t GemmaTextGenerationPipeline::verify_diffusion_block(const std::vector<int32_t>& block,
                                                            std::vector<float>& logits,
                                                            int32_t block_len, int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "GemmaTextGenerationPipeline: diffusion engine must output full verify logits");
    }
...
bool GemmaTextGenerationPipeline::append_tokens_until_eos(const std::vector<int32_t>& tokens,
...
void GemmaTextGenerationPipeline::fill_linear_spec_block(std::vector<int32_t>& block,
                                                         std::vector<float>& logits,
                                                         int32_t block_len, int32_t vocab,
    ...
    while (has_mask_token(block, config_.mask_token_id)) {
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "GemmaTextGenerationPipeline: linear_spec engine must output full block logits");
        }
        const auto preds = masked_predictions(logits, block, config_.mask_token_id, vocab);
...
std::vector<int32_t>
GemmaTextGenerationPipeline::verify_linear_spec_block(const std::vector<int32_t>& block,
                                                      std::vector<float>& logits, int32_t block_len,
                                                      int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "GemmaTextGenerationPipeline: linear_spec engine must output full verify logits");
    }
...
int32_t
GemmaTextGenerationPipeline::count_linear_spec_accepts(const std::vector<int32_t>& ar_tokens,
                                                       const std::vector<int32_t>& block) {
    if (ar_tokens.empty())
        return 0;
    if (block.size() < 2)
...
bool GemmaTextGenerationPipeline::append_linear_spec_tokens(
    const std::vector<int32_t>& ar_tokens, int32_t emit_count, std::vector<int32_t>& output,
    int32_t& generated, const GemmaSamplingParams& params) const {
...
GemmaTextGenerationPipeline::TimedGenResult GemmaTextGenerationPipeline::generate_from_ids(
    ...
    const GemmaSamplingParams& params, const GenerateConfig& cfg) {
    ...
    GemmaISampler* active_sampler = sampler_.get();
    std::unique_ptr<GemmaISampler> local_sampler;
    if (!active_sampler) {
    ...
    std::vector<float> logits;
    const bool gpu_sampling = (active_sampler->logits_location() == GemmaLogitsLocation::DEVICE);
...
GemmaTextGenerationPipeline::TimedGenResult
GemmaTextGenerationPipeline::generate_diffusion_from_ids(const std::vector<int32_t>& input_ids,
    ...
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error(
            "GemmaTextGenerationPipeline: diffusion mode currently supports greedy temperature=0 "
            "generation");
    }
    const int32_t block_len =
...
GemmaTextGenerationPipeline::TimedGenResult
GemmaTextGenerationPipeline::generate_linear_spec_from_ids(const std::vector<int32_t>& input_ids,
                                                           int32_t max_new_tokens,
                                                           const GemmaSamplingParams& params,
                                                           const GenerateConfig& cfg,
                                                           bool use_lora_draft) {
    ...
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error(
            "GemmaTextGenerationPipeline: linear_spec mode currently supports greedy temperature=0 "
            "generation");
    }
    if (use_lora_draft && linear_spec_lora_prefill_ == nullptr) {
        throw std::runtime_error("GemmaTextGenerationPipeline: linear_spec_lora mode requires a "
    ...
    auto* kv = dynamic_cast<GemmaKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error("GemmaTextGenerationPipeline: linear_spec requires GemmaKvCache");
...
void GemmaTextGenerationPipeline::log_decode_summary(int32_t steps, double ms) const {
...
int32_t GemmaTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        ...
        const GemmaSampleResult result = sampler->sample(sample_ptr, vocab_size, params);
...
int32_t GemmaTextGenerationPipeline::select_decoder_index(int32_t desired_rows) const {
    if (decoders_.size() == 1)
        return 0;
...
TrtModule& GemmaTextGenerationPipeline::bind_decoder_for_step() {
...
void GemmaTextGenerationPipeline::run_step_device(int32_t token_id) {
    TensorMap inputs;
...
int32_t GemmaTextGenerationPipeline::argmax(const std::vector<float>& logits) {
    if (logits.empty())
        return 0;
