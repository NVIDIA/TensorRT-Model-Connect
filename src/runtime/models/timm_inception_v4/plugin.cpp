...
// TimmInceptionV4Plugin: handles timm Inception-v4 image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_inception_v4/pipeline.h"
...
TimmInceptionV4PreprocessConfig make_timm_inception_v4_preprocess_config(const std::string& json) {
    TimmInceptionV4PreprocessConfig cfg;
...
class TimmInceptionV4Plugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmInceptionV4ImageClassificationPipeline>(
            std::move(loaded.module), make_timm_inception_v4_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_inception_v4_plugin, TimmInceptionV4Plugin,
                                       "timm_inception_v4_image_classification");
