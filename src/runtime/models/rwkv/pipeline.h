...
// RecurrentPipeline: RWKV-owned recurrent text pipeline.
// Uses RwkvInferenceState for recurrent state ownership.

#include "runtime/models/rwkv/inference_state.h"
#include "runtime/models/rwkv/sampler.h"
...
class RecurrentPipeline final : public IPipeline {
  public:
    RecurrentPipeline(std::unique_ptr<TrtModule> decoder, std::unique_ptr<RwkvInferenceState> state,
                      RecurrentGenConfig config, cudaStream_t stream, const char* name,
                      std::shared_ptr<ITokenizer> tokenizer = nullptr,
                      std::string model_id_str = "",
                      std::unique_ptr<RwkvISampler> sampler = nullptr);
  ...
  private:
    ...
    std::unique_ptr<RwkvInferenceState> state_;
    RecurrentGenConfig config_;
    cudaStream_t stream_;
    const char* name_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<RwkvISampler> sampler_;
