...
// RwkvInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   RwkvKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — RwkvKvCache + family-owned recurrent state composed
...
class RwkvInferenceState {
  public:
    virtual ~RwkvInferenceState() = default;
    ...
    // RwkvKvCache -> true. Family-owned recurrent state -> false.
