...
#include "runtime/models/rwkv/chat_templates.h"
...
static std::vector<int32_t> encode_prompt(const ITokenizer& tokenizer,
    ...
    if (cfg.use_chat_template && !config.chat_template_format.empty()) {
        effective =
            rwkv_apply_chat_template(config.chat_template_format, prompt, cfg.enable_thinking);
        templated = true;
    }
...
TextResult RecurrentPipeline::generate(const std::string& prompt, const GenerateConfig& cfg) {
    ...
    auto sp = rwkv_sampling_params_from_config(cfg, eos);
...
std::vector<int32_t> RecurrentPipeline::generate_from_ids(const std::vector<int32_t>& input_ids,
    ...
    RwkvISampler* active_sampler = sampler_.get();
    std::unique_ptr<RwkvISampler> local_sampler;
    if (!active_sampler) {
        local_sampler = create_rwkv_sampler(params);
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        RwkvSampleResult result = active_sampler->sample(logits.data(), vocab_size, params);
