...
// RwkvKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/rwkv/inference_state.h"
...
class RwkvKvCache : public RwkvInferenceState {
  public:
    ...
    RwkvKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                DType cache_dtype = DType::kFloat32, RwkvKvCacheNames names = {});

    // --- RwkvInferenceState overrides ---
    void reset() override;
    ...
    // --- RwkvKvCache-specific methods (not on the interface) ---
    ...
    // — too big for RwkvKvCache's single-row present buffer. The caller reads
