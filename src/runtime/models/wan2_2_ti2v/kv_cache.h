...
// Wan22Ti2vKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/wan2_2_ti2v/inference_state.h"
...
class Wan22Ti2vKvCache : public Wan22Ti2vInferenceState {
  public:
    ...
    Wan22Ti2vKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                     DType cache_dtype = DType::kFloat32, Wan22Ti2vKvCacheNames names = {});

    // --- Wan22Ti2vInferenceState overrides ---
    void reset() override;
    ...
    // --- Wan22Ti2vKvCache-specific methods (not on the interface) ---
    ...
    // — too big for Wan22Ti2vKvCache's single-row present buffer. The caller reads
