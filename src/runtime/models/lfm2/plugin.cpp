...
class Lfm2Plugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        const int32_t num_attention_layers =
            extract_json_int(ctx.config_json, "num_attention_layers", 0);
        const int32_t num_conv_layers = extract_json_int(ctx.config_json, "num_conv_layers", 0);
        const int32_t conv_cache_length = extract_json_int(ctx.config_json, "conv_L_cache", 0);
        const int32_t conv_state_dim =
            extract_json_int(ctx.config_json, "conv_dim", ctx.config.hidden_size);
        if (num_attention_layers <= 0 || num_conv_layers <= 0 ||
            num_attention_layers + num_conv_layers != ctx.config.num_layers) {
            throw std::runtime_error("LFM2 bundle has inconsistent hybrid layer counts");
        }
        if (conv_cache_length <= 0 || conv_state_dim != ctx.config.hidden_size) {
        ...
        auto kv = std::make_unique<Lfm2KvCache>(num_attention_layers, ctx.config.max_cache_length,
                                                ctx.config.num_kv_heads, ctx.config.head_dim,
                                                stream, state_dtype,
                                                lfm2_kv_names(ctx.config, num_attention_layers));
        auto conv = std::make_unique<Lfm2ConvState>(num_conv_layers, conv_state_dim,
                                                    conv_cache_length, stream, state_dtype);
