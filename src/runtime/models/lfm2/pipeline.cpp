...
Lfm2TextGenerationPipeline::Lfm2TextGenerationPipeline(
    std::unique_ptr<TrtModule> decoder, std::unique_ptr<Lfm2InferenceState> state,
    Lfm2TextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id, std::unique_ptr<Lfm2ISampler> sampler)
...
std::vector<int32_t> Lfm2TextGenerationPipeline::encode_prompt(const std::string& prompt,
...
Lfm2TextGenerationPipeline::generate_from_ids(const std::vector<int32_t>& input_ids,
    ...
    Lfm2ISampler* active_sampler = sampler_.get();
    std::unique_ptr<Lfm2ISampler> local_sampler;
    if (active_sampler == nullptr) {
        local_sampler = create_lfm2_sampler(params);
