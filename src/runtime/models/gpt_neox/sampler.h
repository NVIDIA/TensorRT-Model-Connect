...
// GptNeoxISampler: token selection abstraction for autoregressive generation.
//
...
//
// GptNeoxLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// gpt_neox-owned sampler interface.
class GptNeoxISampler {
  public:
    virtual ~GptNeoxISampler() = default;
    ...
    virtual GptNeoxSampleResult sample(const float* logits, int32_t vocab_size,
                                       const GptNeoxSamplingParams& params) = 0;
    ...
    virtual GptNeoxLogitsLocation logits_location() const = 0;
...
/// Build GptNeoxSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from GptNeoxSamplingParams.
...
create_gpt_neox_sampler(const GptNeoxSamplingParams& params,
                        const GptNeoxSamplerFactoryOptions& options);
