...
enum class GptNeoxTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class GptNeoxTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct GptNeoxTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    GptNeoxTriAttentionScoreAggregation score_aggregation{
        GptNeoxTriAttentionScoreAggregation::kMean};
    GptNeoxTriAttentionScoreAggregation per_layer_aggregation{
        GptNeoxTriAttentionScoreAggregation::kMean};
...
struct GptNeoxTriAttentionHeadStats {
...
struct GptNeoxTriAttentionStats {
    int32_t head_dim{0};
    GptNeoxTriAttentionRopeStyle rope_style{GptNeoxTriAttentionRopeStyle::kHalf};
    ...
    std::vector<GptNeoxTriAttentionHeadStats> layer_stats;
};

struct GptNeoxTriAttentionCompactionProfile {
...
// Build a GptNeoxTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class GptNeoxTriAttentionKvCache : public GptNeoxInferenceState {
  public:
    GptNeoxTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                               int32_t kv_dim, cudaStream_t stream,
                               GptNeoxTriAttentionConfig config, GptNeoxTriAttentionStats stats,
                               DType cache_dtype = DType::kFloat32, GptNeoxKvCacheNames names = {});
  ...
  private:
    ...
    void finalize_repack_profile(GptNeoxTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const GptNeoxTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const GptNeoxTriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        GptNeoxTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             GptNeoxTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                GptNeoxTriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const GptNeoxTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const GptNeoxTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const GptNeoxTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    GptNeoxTriAttentionConfig config_;
    GptNeoxTriAttentionStats stats_;
