...
//
// Composes: TrtModule (decoder) + GptNeoxKvCache + ITokenizer for this runtime
...
class GptNeoxKvCache;

struct GptNeoxTextGenConfig {
...
class GptNeoxTextGenerationPipeline final : public IPipeline {
  public:
    ...
    GptNeoxTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                  std::unique_ptr<GptNeoxInferenceState> state,
                                  GptNeoxTextGenConfig config, cudaStream_t stream,
                                  std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                  std::string model_id_str = "",
                                  std::unique_ptr<GptNeoxISampler> sampler = nullptr,
                                  std::shared_ptr<void> distributed_owner = nullptr);
    GptNeoxTextGenerationPipeline(std::vector<DecoderContext> decoders,
                                  std::unique_ptr<GptNeoxInferenceState> state,
                                  GptNeoxTextGenConfig config, cudaStream_t stream,
                                  std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                  std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "GptNeoxTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<GptNeoxInferenceState> state_;
    GptNeoxTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<GptNeoxISampler> sampler_;
    ...
    GptNeoxKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(GptNeoxKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(GptNeoxISampler* sampler, const GptNeoxSamplingParams& params,
    ...
    std::unique_ptr<GptNeoxISampler> make_step_sampler(const GptNeoxSamplingParams& params);
