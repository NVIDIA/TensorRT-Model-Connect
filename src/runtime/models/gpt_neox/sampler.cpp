// GptNeoxISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// GptNeoxTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/gpt_neox/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/gpt_neox/argmax_kernel.h"
#include "runtime/models/gpt_neox/sparse_multinomial_kernel.h"
...
static GptNeoxSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                              int32_t eos_token_id) {
    GptNeoxSampleResult result;
...
static bool greedy_equivalent(const GptNeoxSamplingParams& params) {
...
class GreedySampler final : public GptNeoxISampler {
  public:
    GptNeoxSampleResult sample(const float* logits, int32_t vocab_size,
                               const GptNeoxSamplingParams& params) override {
    ...
    GptNeoxLogitsLocation logits_location() const override { return GptNeoxLogitsLocation::HOST; }
...
class TopKSampler final : public GptNeoxISampler {
  public:
...
class TorchCudaMultinomialSampler final : public GptNeoxISampler {
  public:
    ...
    GptNeoxSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        gpt_neox_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const GptNeoxTorchMultinomialExecutionPolicy policy =
                gpt_neox_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public GptNeoxISampler {
  public:
    ...
    GptNeoxSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        gpt_neox_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    GptNeoxLogitsLocation logits_location() const override { return GptNeoxLogitsLocation::DEVICE; }
...
GptNeoxSamplingParams gpt_neox_sampling_params_from_config(const GenerateConfig& cfg,
                                                           int32_t default_eos) {
    GptNeoxSamplingParams p;
...
create_gpt_neox_sampler(const GptNeoxSamplingParams& params,
                        [[maybe_unused]] const GptNeoxSamplerFactoryOptions& options) {
...
std::unique_ptr<GptNeoxISampler> create_gpt_neox_sampler(const GptNeoxSamplingParams& params) {
    return create_gpt_neox_sampler(params, GptNeoxSamplerFactoryOptions{});
}

std::unique_ptr<GptNeoxISampler> create_gpt_neox_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
