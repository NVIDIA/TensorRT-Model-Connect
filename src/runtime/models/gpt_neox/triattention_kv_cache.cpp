...
#include "runtime/models/gpt_neox/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/gpt_neox/triattention_kernels.h"
#endif
...
GptNeoxTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return GptNeoxTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return GptNeoxTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(GptNeoxTriAttentionScoreAggregation value) {
    switch (value) {
    case GptNeoxTriAttentionScoreAggregation::kMean:
        return "mean";
    case GptNeoxTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

GptNeoxTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return GptNeoxTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return GptNeoxTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, GptNeoxKvCacheNames& names) {
...
// override helpers that patched GptNeoxTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     GptNeoxTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(GptNeoxTriAttentionConfig& cfg,
...
void fill_debug_from_registry(GptNeoxTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(GptNeoxTriAttentionConfig& cfg,
...
static void validate_triattention_config(const GptNeoxTriAttentionConfig& cfg,
...
GptNeoxTriAttentionConfig
gpt_neox_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                          const ::trtmc::config::ConfigBundle* runtime_config) {
    GptNeoxTriAttentionConfig cfg;
...
void fill_stats_header(GptNeoxTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(GptNeoxTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(GptNeoxTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(GptNeoxTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(GptNeoxTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(GptNeoxTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(GptNeoxTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(GptNeoxTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(GptNeoxTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(GptNeoxTriAttentionStats& stats,
...
void parse_sparse_stats(GptNeoxTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
GptNeoxTriAttentionStats gpt_neox_parse_triattention_stats_json(const std::string& stats_json,
                                                                int32_t num_attention_heads,
                                                                int32_t num_key_value_heads,
                                                                int32_t num_layers) {
    GptNeoxTriAttentionStats stats;
...
void GptNeoxTriAttentionKvCache::validate_shapes() {
...
void GptNeoxTriAttentionKvCache::normalize_sampled_heads() {
...
void GptNeoxTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void GptNeoxTriAttentionKvCache::allocate_layer_tensors() {
...
GptNeoxTriAttentionKvCache::GptNeoxTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                       int32_t max_length, int32_t kv_dim,
                                                       cudaStream_t stream,
                                                       GptNeoxTriAttentionConfig config,
                                                       GptNeoxTriAttentionStats stats,
                                                       DType cache_dtype, GptNeoxKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void GptNeoxTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void GptNeoxTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void GptNeoxTriAttentionKvCache::initialize_gpu_state() {
...
bool GptNeoxTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool GptNeoxTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool GptNeoxTriAttentionKvCache::can_use_gpu_selection() const {
...
void GptNeoxTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t GptNeoxTriAttentionKvCache::preferred_cache_rows() const {
...
void GptNeoxTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void GptNeoxTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> GptNeoxTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows, GptNeoxTriAttentionCompactionProfile* profile) const {
...
void GptNeoxTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t GptNeoxTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> GptNeoxTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
GptNeoxTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
GptNeoxTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                                GptNeoxTriAttentionCompactionProfile* profile) {
...
int32_t GptNeoxTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t GptNeoxTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> GptNeoxTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void GptNeoxTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, GptNeoxTriAttentionCompactionProfile* profile) const {
...
bool GptNeoxTriAttentionKvCache::layer_stats_shapes_valid(
    const GptNeoxTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void GptNeoxTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                               int32_t half_dim, float& k_rot_real,
                                               float& k_rot_imag) const {
    if (stats_.rope_style == GptNeoxTriAttentionRopeStyle::kInterleaved) {
...
float GptNeoxTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == GptNeoxTriAttentionScoreAggregation::kMax)
...
float GptNeoxTriAttentionKvCache::score_one_row(
    const float* row_ptr, const GptNeoxTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void GptNeoxTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const GptNeoxTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void GptNeoxTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void GptNeoxTriAttentionKvCache::accumulate_layer_fallback(
...
void GptNeoxTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == GptNeoxTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void GptNeoxTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
GptNeoxTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void GptNeoxTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == GptNeoxTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void GptNeoxTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> GptNeoxTriAttentionKvCache::build_keep_indices_per_head(
...
bool GptNeoxTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, GptNeoxTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> GptNeoxTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, GptNeoxTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void GptNeoxTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                        int32_t total_tokens) const {
...
void GptNeoxTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void GptNeoxTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
GptNeoxTriAttentionKvCache::GpuLayerResult
GptNeoxTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = gpt_neox_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == GptNeoxTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == GptNeoxTriAttentionScoreAggregation::kMax,
...
bool GptNeoxTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool GptNeoxTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, GptNeoxTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void GptNeoxTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> GptNeoxTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool GptNeoxTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> GptNeoxTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, GptNeoxTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void GptNeoxTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void GptNeoxTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool GptNeoxTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool GptNeoxTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                       std::size_t row_bytes) {
    const bool ok_k = gpt_neox_triattention_compact_rows_gpu(
    ...
    const bool ok_v = gpt_neox_triattention_compact_rows_gpu(
...
bool GptNeoxTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void GptNeoxTriAttentionKvCache::compact_layer_on_host(
...
void GptNeoxTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    GptNeoxTriAttentionCompactionProfile profile;
    GptNeoxTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void GptNeoxTriAttentionKvCache::finalize_repack_profile(
    GptNeoxTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
GptNeoxTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
GptNeoxTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                      int32_t keep_count) const {
...
int32_t GptNeoxTriAttentionKvCache::count_prefix_positions(
...
void GptNeoxTriAttentionKvCache::log_compact_debug(
...
void GptNeoxTriAttentionKvCache::log_compact_profile(
    const GptNeoxTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool GptNeoxTriAttentionKvCache::should_emit_keep_dump(
    const GptNeoxTriAttentionCompactionProfile* profile_ptr) const {
...
void GptNeoxTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "GptNeoxTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void GptNeoxTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void GptNeoxTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void GptNeoxTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool GptNeoxTriAttentionKvCache::ok() const {
