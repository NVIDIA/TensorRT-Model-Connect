...
GptNeoxTextGenerationPipeline::GptNeoxTextGenerationPipeline(
    ...
    if (decoders_.size() != 1 || !decoders_.front().module || !decoders_.front().module->ok())
        throw std::runtime_error(
            "GptNeoxTextGenerationPipeline requires exactly one native decode module");
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error(
            "GptNeoxTextGenerationPipeline requires a native split prefill module");
    if (!state_ || !state_->ok()) {
...
namespace {
void gather_prefill_kv_pointers(TrtModule& prefill, const GptNeoxTextGenConfig& cfg,
                                std::vector<const void*>& pk, std::vector<const void*>& pv) {
    if (cfg.num_layers <= 0)
        throw std::runtime_error("GPT-NeoX native prefill has no layer geometry");
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        if (pk[li] == nullptr || pv[li] == nullptr) {
            throw std::runtime_error(
                "GPT-NeoX native prefill is missing an aliased present_k/present_v output");
        }
    }
}

void validate_generation_capacity(const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
                                  GptNeoxInferenceState* state) {
    const auto* kv = dynamic_cast<const GptNeoxKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("GPT-NeoX native runtime requires GptNeoxKvCache");
    ...
    if (input_ids.size() > capacity ||
        ...
        throw std::runtime_error(
            "GPT-NeoX requested prompt and generation exceed the model's fixed KV cache capacity");
    }
}
} // namespace

void GptNeoxTextGenerationPipeline::run_prefill_chunk(const int32_t* token_ids, int32_t chunk_size,
                                                      GptNeoxKvCache& kv,
    ...
    if (logits_it == outputs.end()) {
        throw std::runtime_error(
            "GptNeoxTextGenerationPipeline: prefill module has no logits output");
    }
    ...
    if (static_cast<std::size_t>(logits_tensor.numel()) < vocab) {
        throw std::runtime_error(
            "GptNeoxTextGenerationPipeline: prefill logits are smaller than vocabulary");
    }
    logits.resize(vocab);
    ...
    if (retain_device_logits) {
        ...
        if (device_logits == nullptr) {
            throw std::runtime_error(
                "GptNeoxTextGenerationPipeline: prefill logits have no device buffer");
        }
...
void GptNeoxTextGenerationPipeline::log_batched_prefill(int32_t token_count, int32_t chunk_count,
...
void GptNeoxTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
                                                        std::vector<float>& logits,
                                                        bool retain_device_logits) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    if (sq <= 0)
        throw std::invalid_argument("GPT-NeoX native prefill requires at least one token");
    auto* kv = dynamic_cast<GptNeoxKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error("GPT-NeoX native prefill requires GptNeoxKvCache");
    if (config_.prefill_max_length <= 0)
        throw std::runtime_error("GPT-NeoX native prefill engine has no valid profile capacity");

    kv->bind_cache_inputs(*prefill_);
    if (sq > kv->max_length())
        throw std::runtime_error("GPT-NeoX sequence exceeds the fixed KV cache capacity");
...
void GptNeoxTextGenerationPipeline::run_prefill(const std::vector<int32_t>& input_ids,
    ...
    if (step_trace_enabled()) {
        if (gpu_sampling) {
            throw std::runtime_error(
                "GPT-NeoX text trace requires host logits and cannot use GPU-only sampling");
        }
...
void GptNeoxTextGenerationPipeline::run_step(int32_t token_id, std::vector<float>& logits,
    ...
    state_->advance();
    maybe_append_step_trace(phase, position_before, token_id, 0, rows_before,
