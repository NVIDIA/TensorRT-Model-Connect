...
// GptNeoxInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   GptNeoxKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — GptNeoxKvCache + family-owned recurrent state composed
...
class GptNeoxInferenceState {
  public:
    virtual ~GptNeoxInferenceState() = default;
    ...
    // GptNeoxKvCache -> true. Family-owned recurrent state -> false.
