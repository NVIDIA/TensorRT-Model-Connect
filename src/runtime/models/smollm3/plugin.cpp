...
#include "runtime/models/smollm3/tensor_names.h"
#include "runtime/models/smollm3/triattention_kv_cache.h"
...
void validate_native_kv_marker(const std::string& config_json, bool engine_uses_native_kv) {
    ...
    if (!declares_native_kv || !engine_uses_native_kv ||
        ...
        throw std::runtime_error("SmolLM3 native KV metadata does not match the engine contract");
    }
}

std::uint64_t checked_multiply(std::uint64_t lhs, std::uint64_t rhs) {
    if (lhs != 0 && rhs > std::numeric_limits<std::uint64_t>::max() / lhs)
        throw std::overflow_error("SmolLM3 native KV byte accounting overflow");
    return lhs * rhs;
}
...
void admit_native_kv_allocation(const PipelineContext& ctx, bool native_kv,
    ...
    if (status != cudaSuccess) {
        throw std::runtime_error(std::string("SmolLM3 native KV CUDA memory query failed: ") +
    ...
    if (sizing.cache_bytes > available) {
        throw std::runtime_error(
            "SmolLM3 native KV cache admission failed before allocation: capacity=" +
...
void reject_native_kv_size_override(const PipelineContext& ctx) {
    if (ctx.kv_cache_size_bytes != 0) {
        throw std::invalid_argument(
            "SmolLM3 native TensorRT KV cache allocates the model's complete fixed "
...
KvCacheRuntimeSizing resolve_kv_cache_runtime_sizing(
    ...
    if (ctx.config.num_layers <= 0 || kv_dim <= 0 || bundle_max_rows <= 0)
        throw std::runtime_error("SmolLM3 KV geometry must be positive");
...
void validate_native_kv_runtime(const PipelineContext& ctx, const TrtModule& module,
    ...
    if (cache_dtype != DType::kBFloat16 || tri_cfg.enabled || tp_config.enabled) {
        throw std::runtime_error(
            "SmolLM3 native KV requires BF16, single-GPU, non-TriAttention runtime");
    }
    ...
    if (module.tensor_shape(kv_names.cache_k.front()) != expected_shape) {
        throw std::runtime_error(
            "SmolLM3 native KV requires cache shape [1,num_kv_heads,capacity,128]");
    }
}
  ...
  private:
    ...
    static void build_kv_names(const PipelineContext& ctx, const IoMap& io,
        ...
        for (int32_t i = 0; i < ctx.config.num_layers; ++i) {
            kv_names.cache_k.push_back(smollm3_expand_layer_name(io.cache_k_pattern, i));
            kv_names.cache_v.push_back(smollm3_expand_layer_name(io.cache_v_pattern, i));
            kv_names.present_k.push_back(smollm3_expand_layer_name(io.present_k_pattern, i));
            kv_names.present_v.push_back(smollm3_expand_layer_name(io.present_v_pattern, i));
        }
    }
    ...
    static void apply_chat_template_format(const BundleFile& bundle, SmolLM3TextGenConfig& tgc) {
        ...
        tgc.chat_template_format = smollm3_detect_chat_template_format(chat_tpl);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_smollm3_plugin, DecoderPlugin,
