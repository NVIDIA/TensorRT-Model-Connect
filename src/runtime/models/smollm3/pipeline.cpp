...
SmolLM3TextGenerationPipeline::make_step_sampler(const SmolLM3SamplingParams& params) {
    ...
    if (prefer_gpu_greedy_ && greedy_params) {
        if (auto gpu = create_smollm3_gpu_greedy_sampler(stream_))
            return gpu;
    }
    return create_smollm3_sampler(params);
}
...
bool gather_prefill_kv_pointers(TrtModule& prefill, const SmolLM3TextGenConfig& cfg,
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        pk[li] = prefill.device_ptr(smollm3_expand_layer_name(cfg.present_k_pattern, i));
        pv[li] = prefill.device_ptr(smollm3_expand_layer_name(cfg.present_v_pattern, i));
...
int32_t resolve_prefill_chunk_limit(const SmolLM3KvCache& kv, const SmolLM3TextGenConfig& cfg,
    ...
    if (cfg.prefill_max_length <= 0)
        throw std::runtime_error("SmolLM3 native KV prefill engine has no valid profile capacity");
...
SmolLM3TextGenerationPipeline::generate_linear_spec_from_ids(const std::vector<int32_t>& input_ids,
    ...
    if (smollm3_is_eos_token(params, next_token)) {
        return TimedGenResult{std::move(output),
