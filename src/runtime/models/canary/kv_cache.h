...
// CanaryKvCache: autoregressive KV cache state manager.
...
class CanaryKvCache : public CanaryInferenceState {
  public:
    ...
    // --- CanaryKvCache-specific methods (not on the interface) ---

    // DEPRECATED: Use prepare_step() instead.
    // Kept for backward compatibility with tests that call this directly.
    void canary_build_attention_mask(std::vector<float>& mask) const;
