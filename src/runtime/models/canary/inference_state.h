...
// CanaryInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   CanaryKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — CanaryKvCache + family-owned recurrent state composed
...
class CanaryInferenceState {
  public:
    virtual ~CanaryInferenceState() = default;
    ...
    // CanaryKvCache -> true. Family-owned recurrent state -> false.
