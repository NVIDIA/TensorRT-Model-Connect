...
void CanaryKvCache::canary_build_attention_mask(std::vector<float>& mask) const {
...
int32_t CanaryKvCache::preferred_cache_rows() const {
...
std::vector<int64_t> CanaryKvCache::mask_shape_for_engine(int32_t mask_width,
...
void CanaryKvCache::bind_to(TrtModule& module) {
...
void CanaryKvCache::bind_cache_inputs(TrtModule& module) {
    has_position_input_ = module.has_input(names_.position_id);
    for (int32_t i = 0; i < num_layers_; ++i) {
        auto li = static_cast<std::size_t>(i);
        module.bind_external(names_.cache_k[li], cache_k_[li].data());
