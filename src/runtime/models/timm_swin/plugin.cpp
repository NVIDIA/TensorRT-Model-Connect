...
// TimmSwinPlugin: handles timm Swin Transformer image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_swin/pipeline.h"
...
TimmSwinPreprocessConfig make_timm_swin_preprocess_config(const std::string& json) {
    TimmSwinPreprocessConfig cfg;
...
class TimmSwinPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmSwinImageClassificationPipeline>(
            std::move(loaded.module), make_timm_swin_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_swin_plugin, TimmSwinPlugin,
                                       "timm_swin_image_classification");
