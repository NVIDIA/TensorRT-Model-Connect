...
// TimesfmInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   TimesfmKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — TimesfmKvCache + family-owned recurrent state composed
...
class TimesfmInferenceState {
  public:
    virtual ~TimesfmInferenceState() = default;
    ...
    // TimesfmKvCache -> true. Family-owned recurrent state -> false.
