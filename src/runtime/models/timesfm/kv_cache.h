...
// TimesfmKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/timesfm/inference_state.h"
...
class TimesfmKvCache : public TimesfmInferenceState {
  public:
    ...
    TimesfmKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                   DType cache_dtype = DType::kFloat32, TimesfmKvCacheNames names = {});

    // --- TimesfmInferenceState overrides ---
    void reset() override;
    ...
    // --- TimesfmKvCache-specific methods (not on the interface) ---
    ...
    // — too big for TimesfmKvCache's single-row present buffer. The caller reads
