// NemotronISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// NemotronTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/nemotron/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/nemotron/argmax_kernel.h"
#include "runtime/models/nemotron/sparse_multinomial_kernel.h"
...
static NemotronSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                               int32_t eos_token_id) {
    NemotronSampleResult result;
...
static bool greedy_equivalent(const NemotronSamplingParams& params) {
...
class GreedySampler final : public NemotronISampler {
  public:
    NemotronSampleResult sample(const float* logits, int32_t vocab_size,
                                const NemotronSamplingParams& params) override {
    ...
    NemotronLogitsLocation logits_location() const override { return NemotronLogitsLocation::HOST; }
...
class TopKSampler final : public NemotronISampler {
  public:
...
class TorchCudaMultinomialSampler final : public NemotronISampler {
  public:
    ...
    NemotronSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        nemotron_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const NemotronTorchMultinomialExecutionPolicy policy =
                nemotron_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public NemotronISampler {
  public:
    ...
    NemotronSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        nemotron_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    NemotronLogitsLocation logits_location() const override {
        return NemotronLogitsLocation::DEVICE;
    }
...
NemotronSamplingParams nemotron_sampling_params_from_config(const GenerateConfig& cfg,
                                                            int32_t default_eos) {
    NemotronSamplingParams p;
...
create_nemotron_sampler(const NemotronSamplingParams& params,
                        [[maybe_unused]] const NemotronSamplerFactoryOptions& options) {
...
std::unique_ptr<NemotronISampler> create_nemotron_sampler(const NemotronSamplingParams& params) {
    return create_nemotron_sampler(params, NemotronSamplerFactoryOptions{});
}

std::unique_ptr<NemotronISampler> create_nemotron_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
