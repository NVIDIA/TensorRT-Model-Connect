...
// NemotronKvCache: autoregressive KV cache state manager.
...
class NemotronKvCache : public NemotronInferenceState {
  public:
    ...
    NemotronKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                    DType cache_dtype = DType::kFloat32, NemotronKvCacheNames names = {});

    // --- NemotronInferenceState overrides ---
    void reset() override;
    ...
    // --- NemotronKvCache-specific methods (not on the interface) ---
    ...
    // — too big for NemotronKvCache's single-row present buffer. The caller reads
