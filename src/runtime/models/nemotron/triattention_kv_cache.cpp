...
#include "runtime/models/nemotron/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/nemotron/triattention_kernels.h"
#endif
...
NemotronTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return NemotronTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return NemotronTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(NemotronTriAttentionScoreAggregation value) {
    switch (value) {
    case NemotronTriAttentionScoreAggregation::kMean:
        return "mean";
    case NemotronTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

NemotronTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return NemotronTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return NemotronTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, NemotronKvCacheNames& names) {
...
// override helpers that patched NemotronTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     NemotronTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(NemotronTriAttentionConfig& cfg,
...
void fill_debug_from_registry(NemotronTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(NemotronTriAttentionConfig& cfg,
...
static void validate_triattention_config(const NemotronTriAttentionConfig& cfg,
...
NemotronTriAttentionConfig
nemotron_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                          const ::trtmc::config::ConfigBundle* runtime_config) {
    NemotronTriAttentionConfig cfg;
...
void fill_stats_header(NemotronTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(NemotronTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(NemotronTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(NemotronTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(NemotronTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(NemotronTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(NemotronTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(NemotronTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(NemotronTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(NemotronTriAttentionStats& stats,
...
void parse_sparse_stats(NemotronTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
NemotronTriAttentionStats nemotron_parse_triattention_stats_json(const std::string& stats_json,
                                                                 int32_t num_attention_heads,
                                                                 int32_t num_key_value_heads,
                                                                 int32_t num_layers) {
    NemotronTriAttentionStats stats;
...
void NemotronTriAttentionKvCache::validate_shapes() {
...
void NemotronTriAttentionKvCache::normalize_sampled_heads() {
...
void NemotronTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void NemotronTriAttentionKvCache::allocate_layer_tensors() {
...
NemotronTriAttentionKvCache::NemotronTriAttentionKvCache(
    int32_t num_layers, int32_t num_kv_heads, int32_t max_length, int32_t kv_dim,
    cudaStream_t stream, NemotronTriAttentionConfig config, NemotronTriAttentionStats stats,
    DType cache_dtype, NemotronKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void NemotronTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void NemotronTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void NemotronTriAttentionKvCache::initialize_gpu_state() {
...
bool NemotronTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool NemotronTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool NemotronTriAttentionKvCache::can_use_gpu_selection() const {
...
void NemotronTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t NemotronTriAttentionKvCache::preferred_cache_rows() const {
...
void NemotronTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void NemotronTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> NemotronTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows,
    NemotronTriAttentionCompactionProfile* profile) const {
...
void NemotronTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t NemotronTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> NemotronTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
NemotronTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
NemotronTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                                 NemotronTriAttentionCompactionProfile* profile) {
...
int32_t NemotronTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t NemotronTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> NemotronTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void NemotronTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, NemotronTriAttentionCompactionProfile* profile) const {
...
bool NemotronTriAttentionKvCache::layer_stats_shapes_valid(
    const NemotronTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void NemotronTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset,
                                                int32_t d, int32_t half_dim, float& k_rot_real,
                                                float& k_rot_imag) const {
    if (stats_.rope_style == NemotronTriAttentionRopeStyle::kInterleaved) {
...
float NemotronTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == NemotronTriAttentionScoreAggregation::kMax)
...
float NemotronTriAttentionKvCache::score_one_row(
    const float* row_ptr, const NemotronTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void NemotronTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const NemotronTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void NemotronTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void NemotronTriAttentionKvCache::accumulate_layer_fallback(
...
void NemotronTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max =
        config_.per_layer_aggregation == NemotronTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void NemotronTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
NemotronTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void NemotronTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
...
void NemotronTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> NemotronTriAttentionKvCache::build_keep_indices_per_head(
...
bool NemotronTriAttentionKvCache::process_layer_for_host_selection(
...
std::vector<int32_t> NemotronTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, NemotronTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void NemotronTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                         int32_t total_tokens) const {
...
void NemotronTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void NemotronTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
NemotronTriAttentionKvCache::GpuLayerResult
NemotronTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = nemotron_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == NemotronTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == NemotronTriAttentionScoreAggregation::kMax,
...
bool NemotronTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool NemotronTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, NemotronTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void NemotronTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> NemotronTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool NemotronTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> NemotronTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, NemotronTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void NemotronTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void NemotronTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool NemotronTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool NemotronTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                        std::size_t row_bytes) {
    const bool ok_k = nemotron_triattention_compact_rows_gpu(
    ...
    const bool ok_v = nemotron_triattention_compact_rows_gpu(
...
bool NemotronTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void NemotronTriAttentionKvCache::compact_layer_on_host(
...
void NemotronTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    NemotronTriAttentionCompactionProfile profile;
    NemotronTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void NemotronTriAttentionKvCache::finalize_repack_profile(
    NemotronTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
NemotronTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
NemotronTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                       int32_t keep_count) const {
...
int32_t NemotronTriAttentionKvCache::count_prefix_positions(
...
void NemotronTriAttentionKvCache::log_compact_debug(
...
void NemotronTriAttentionKvCache::log_compact_profile(
    const NemotronTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool NemotronTriAttentionKvCache::should_emit_keep_dump(
    const NemotronTriAttentionCompactionProfile* profile_ptr) const {
...
void NemotronTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "NemotronTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void NemotronTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void NemotronTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void NemotronTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool NemotronTriAttentionKvCache::ok() const {
