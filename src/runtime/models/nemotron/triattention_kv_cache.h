...
#include "runtime/models/nemotron/inference_state.h"
...
enum class NemotronTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class NemotronTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct NemotronTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    NemotronTriAttentionScoreAggregation score_aggregation{
        NemotronTriAttentionScoreAggregation::kMean};
    NemotronTriAttentionScoreAggregation per_layer_aggregation{
        NemotronTriAttentionScoreAggregation::kMean};
...
struct NemotronTriAttentionHeadStats {
...
struct NemotronTriAttentionStats {
    int32_t head_dim{0};
    NemotronTriAttentionRopeStyle rope_style{NemotronTriAttentionRopeStyle::kHalf};
    ...
    std::vector<NemotronTriAttentionHeadStats> layer_stats;
};

struct NemotronTriAttentionCompactionProfile {
...
// Build a NemotronTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class NemotronTriAttentionKvCache : public NemotronInferenceState {
  public:
    NemotronTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                                int32_t kv_dim, cudaStream_t stream,
                                NemotronTriAttentionConfig config, NemotronTriAttentionStats stats,
  ...
  private:
    ...
    void finalize_repack_profile(NemotronTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const NemotronTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const NemotronTriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        NemotronTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             NemotronTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const NemotronTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const NemotronTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const NemotronTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    NemotronKvCacheNames names_;
    NemotronTriAttentionConfig config_;
    NemotronTriAttentionStats stats_;
