...
//
// Composes: TrtModule (decoder) + NemotronKvCache + ITokenizer for this runtime
...
class NemotronTextGenerationPipeline final : public IPipeline {
  public:
    ...
    NemotronTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                   std::unique_ptr<NemotronInferenceState> state,
                                   NemotronTextGenConfig config, cudaStream_t stream,
                                   std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                   std::string model_id_str = "",
                                   std::unique_ptr<NemotronISampler> sampler = nullptr,
                                   std::shared_ptr<void> distributed_owner = nullptr);
    NemotronTextGenerationPipeline(std::vector<DecoderContext> decoders,
                                   std::unique_ptr<NemotronInferenceState> state,
                                   NemotronTextGenConfig config, cudaStream_t stream,
                                   std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                   std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "NemotronTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<NemotronInferenceState> state_;
    NemotronTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<NemotronISampler> sampler_;
    ...
    NemotronKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(NemotronKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(NemotronISampler* sampler, const NemotronSamplingParams& params,
    ...
    std::unique_ptr<NemotronISampler> make_step_sampler(const NemotronSamplingParams& params);
