...
// NemotronISampler: token selection abstraction for autoregressive generation.
//
...
//
// NemotronLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// nemotron-owned sampler interface.
class NemotronISampler {
  public:
    virtual ~NemotronISampler() = default;
    ...
    virtual NemotronSampleResult sample(const float* logits, int32_t vocab_size,
                                        const NemotronSamplingParams& params) = 0;
    ...
    virtual NemotronLogitsLocation logits_location() const = 0;
...
/// Build NemotronSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from NemotronSamplingParams.
...
create_nemotron_sampler(const NemotronSamplingParams& params,
                        const NemotronSamplerFactoryOptions& options);
