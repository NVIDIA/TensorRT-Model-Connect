...
// TimmVggPlugin: handles timm VGG image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_vgg/pipeline.h"
...
TimmVggPreprocessConfig make_timm_vgg_preprocess_config(const std::string& json) {
    TimmVggPreprocessConfig cfg;
...
class TimmVggPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmVggImageClassificationPipeline>(
            std::move(loaded.module), make_timm_vgg_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_vgg_plugin, TimmVggPlugin,
