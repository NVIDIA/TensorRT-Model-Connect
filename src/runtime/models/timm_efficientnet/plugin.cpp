...
// TimmEfficientnetPlugin: handles timm EfficientNet image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_efficientnet/pipeline.h"
...
TimmEfficientnetPreprocessConfig make_timm_efficientnet_preprocess_config(const std::string& json) {
    TimmEfficientnetPreprocessConfig cfg;
...
class TimmEfficientnetPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmEfficientnetImageClassificationPipeline>(
            std::move(loaded.module), make_timm_efficientnet_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_efficientnet_plugin, TimmEfficientnetPlugin,
