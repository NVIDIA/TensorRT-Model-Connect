...
// SanaWmInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   SanaWmKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — SanaWmKvCache + family-owned recurrent state composed
...
class SanaWmInferenceState {
  public:
    virtual ~SanaWmInferenceState() = default;
    ...
    // SanaWmKvCache -> true. Family-owned recurrent state -> false.
