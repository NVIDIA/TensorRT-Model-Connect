...
// SanaWmKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/sana_wm/inference_state.h"
...
class SanaWmKvCache : public SanaWmInferenceState {
  public:
    ...
    SanaWmKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, SanaWmKvCacheNames names = {});

    // --- SanaWmInferenceState overrides ---
    void reset() override;
    ...
    // --- SanaWmKvCache-specific methods (not on the interface) ---
    ...
    // — too big for SanaWmKvCache's single-row present buffer. The caller reads
