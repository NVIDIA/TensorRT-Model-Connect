...
#include "runtime/models/qwen_moe/inference_state.h"
...
enum class QwenMoeTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class QwenMoeTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct QwenMoeTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    QwenMoeTriAttentionScoreAggregation score_aggregation{
        QwenMoeTriAttentionScoreAggregation::kMean};
    QwenMoeTriAttentionScoreAggregation per_layer_aggregation{
        QwenMoeTriAttentionScoreAggregation::kMean};
...
struct QwenMoeTriAttentionHeadStats {
...
struct QwenMoeTriAttentionStats {
    int32_t head_dim{0};
    QwenMoeTriAttentionRopeStyle rope_style{QwenMoeTriAttentionRopeStyle::kHalf};
    ...
    std::vector<QwenMoeTriAttentionHeadStats> layer_stats;
};

struct QwenMoeTriAttentionCompactionProfile {
...
// Build a QwenMoeTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class QwenMoeTriAttentionKvCache : public QwenMoeInferenceState {
  public:
    QwenMoeTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                               int32_t kv_dim, cudaStream_t stream,
                               QwenMoeTriAttentionConfig config, QwenMoeTriAttentionStats stats,
  ...
  private:
    ...
    void finalize_repack_profile(QwenMoeTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const QwenMoeTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const QwenMoeTriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        QwenMoeTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             QwenMoeTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                QwenMoeTriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const QwenMoeTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const QwenMoeTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const QwenMoeTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    QwenMoeTriAttentionConfig config_;
    QwenMoeTriAttentionStats stats_;
