...
#include "runtime/models/qwen_moe/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/qwen_moe/triattention_kernels.h"
#endif
...
QwenMoeTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return QwenMoeTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return QwenMoeTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(QwenMoeTriAttentionScoreAggregation value) {
    switch (value) {
    case QwenMoeTriAttentionScoreAggregation::kMean:
        return "mean";
    case QwenMoeTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

QwenMoeTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return QwenMoeTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return QwenMoeTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, QwenMoeKvCacheNames& names) {
...
// override helpers that patched QwenMoeTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     QwenMoeTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(QwenMoeTriAttentionConfig& cfg,
...
void fill_debug_from_registry(QwenMoeTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(QwenMoeTriAttentionConfig& cfg,
...
static void validate_triattention_config(const QwenMoeTriAttentionConfig& cfg,
...
QwenMoeTriAttentionConfig
qwen_moe_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                          const ::trtmc::config::ConfigBundle* runtime_config) {
    QwenMoeTriAttentionConfig cfg;
...
void fill_stats_header(QwenMoeTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(QwenMoeTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(QwenMoeTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(QwenMoeTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(QwenMoeTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(QwenMoeTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(QwenMoeTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(QwenMoeTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(QwenMoeTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(QwenMoeTriAttentionStats& stats,
...
void parse_sparse_stats(QwenMoeTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
QwenMoeTriAttentionStats qwen_moe_parse_triattention_stats_json(const std::string& stats_json,
                                                                int32_t num_attention_heads,
                                                                int32_t num_key_value_heads,
                                                                int32_t num_layers) {
    QwenMoeTriAttentionStats stats;
...
void QwenMoeTriAttentionKvCache::validate_shapes() {
...
void QwenMoeTriAttentionKvCache::normalize_sampled_heads() {
...
void QwenMoeTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void QwenMoeTriAttentionKvCache::allocate_layer_tensors() {
...
QwenMoeTriAttentionKvCache::QwenMoeTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                       int32_t max_length, int32_t kv_dim,
                                                       cudaStream_t stream,
                                                       QwenMoeTriAttentionConfig config,
                                                       QwenMoeTriAttentionStats stats,
                                                       DType cache_dtype, QwenMoeKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void QwenMoeTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void QwenMoeTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void QwenMoeTriAttentionKvCache::initialize_gpu_state() {
...
bool QwenMoeTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool QwenMoeTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool QwenMoeTriAttentionKvCache::can_use_gpu_selection() const {
...
void QwenMoeTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t QwenMoeTriAttentionKvCache::preferred_cache_rows() const {
...
void QwenMoeTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void QwenMoeTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> QwenMoeTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows, QwenMoeTriAttentionCompactionProfile* profile) const {
...
void QwenMoeTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t QwenMoeTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> QwenMoeTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
QwenMoeTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
QwenMoeTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                                QwenMoeTriAttentionCompactionProfile* profile) {
...
int32_t QwenMoeTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t QwenMoeTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> QwenMoeTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void QwenMoeTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, QwenMoeTriAttentionCompactionProfile* profile) const {
...
bool QwenMoeTriAttentionKvCache::layer_stats_shapes_valid(
    const QwenMoeTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void QwenMoeTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                               int32_t half_dim, float& k_rot_real,
                                               float& k_rot_imag) const {
    if (stats_.rope_style == QwenMoeTriAttentionRopeStyle::kInterleaved) {
...
float QwenMoeTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == QwenMoeTriAttentionScoreAggregation::kMax)
...
float QwenMoeTriAttentionKvCache::score_one_row(
    const float* row_ptr, const QwenMoeTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void QwenMoeTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const QwenMoeTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void QwenMoeTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void QwenMoeTriAttentionKvCache::accumulate_layer_fallback(
...
void QwenMoeTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == QwenMoeTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void QwenMoeTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
QwenMoeTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void QwenMoeTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == QwenMoeTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void QwenMoeTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> QwenMoeTriAttentionKvCache::build_keep_indices_per_head(
...
bool QwenMoeTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, QwenMoeTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> QwenMoeTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, QwenMoeTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void QwenMoeTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                        int32_t total_tokens) const {
...
void QwenMoeTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void QwenMoeTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
QwenMoeTriAttentionKvCache::GpuLayerResult
QwenMoeTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = qwen_moe_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == QwenMoeTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == QwenMoeTriAttentionScoreAggregation::kMax,
...
bool QwenMoeTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool QwenMoeTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, QwenMoeTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void QwenMoeTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> QwenMoeTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool QwenMoeTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> QwenMoeTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, QwenMoeTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void QwenMoeTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void QwenMoeTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool QwenMoeTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool QwenMoeTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                       std::size_t row_bytes) {
    const bool ok_k = qwen_moe_triattention_compact_rows_gpu(
    ...
    const bool ok_v = qwen_moe_triattention_compact_rows_gpu(
...
bool QwenMoeTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void QwenMoeTriAttentionKvCache::compact_layer_on_host(
...
void QwenMoeTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    QwenMoeTriAttentionCompactionProfile profile;
    QwenMoeTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void QwenMoeTriAttentionKvCache::finalize_repack_profile(
    QwenMoeTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
QwenMoeTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
QwenMoeTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                      int32_t keep_count) const {
...
int32_t QwenMoeTriAttentionKvCache::count_prefix_positions(
...
void QwenMoeTriAttentionKvCache::log_compact_debug(
...
void QwenMoeTriAttentionKvCache::log_compact_profile(
    const QwenMoeTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool QwenMoeTriAttentionKvCache::should_emit_keep_dump(
    const QwenMoeTriAttentionCompactionProfile* profile_ptr) const {
...
void QwenMoeTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "QwenMoeTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void QwenMoeTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void QwenMoeTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void QwenMoeTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool QwenMoeTriAttentionKvCache::ok() const {
