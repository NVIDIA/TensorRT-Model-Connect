...
//
// Composes: TrtModule (decoder) + QwenMoeKvCache + ITokenizer for this runtime
...
class QwenMoeKvCache;

struct QwenMoeTextGenConfig {
...
class QwenMoeTextGenerationPipeline final : public IPipeline {
  public:
    ...
    QwenMoeTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                  std::unique_ptr<QwenMoeInferenceState> state,
                                  QwenMoeTextGenConfig config, cudaStream_t stream,
                                  std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                  std::string model_id_str = "",
                                  std::unique_ptr<QwenMoeISampler> sampler = nullptr,
                                  std::shared_ptr<void> distributed_owner = nullptr);
    QwenMoeTextGenerationPipeline(std::vector<DecoderContext> decoders,
                                  std::unique_ptr<QwenMoeInferenceState> state,
                                  QwenMoeTextGenConfig config, cudaStream_t stream,
                                  std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                  std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "QwenMoeTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<QwenMoeInferenceState> state_;
    QwenMoeTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<QwenMoeISampler> sampler_;
    ...
    QwenMoeKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(QwenMoeKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(QwenMoeISampler* sampler, const QwenMoeSamplingParams& params,
    ...
    std::unique_ptr<QwenMoeISampler> make_step_sampler(const QwenMoeSamplingParams& params);
