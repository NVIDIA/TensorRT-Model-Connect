// QwenMoeISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// QwenMoeTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/qwen_moe/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/qwen_moe/argmax_kernel.h"
#include "runtime/models/qwen_moe/sparse_multinomial_kernel.h"
...
static QwenMoeSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                              int32_t eos_token_id) {
    QwenMoeSampleResult result;
...
static bool greedy_equivalent(const QwenMoeSamplingParams& params) {
...
class GreedySampler final : public QwenMoeISampler {
  public:
    QwenMoeSampleResult sample(const float* logits, int32_t vocab_size,
                               const QwenMoeSamplingParams& params) override {
    ...
    QwenMoeLogitsLocation logits_location() const override { return QwenMoeLogitsLocation::HOST; }
...
class TopKSampler final : public QwenMoeISampler {
  public:
...
class TorchCudaMultinomialSampler final : public QwenMoeISampler {
  public:
    ...
    QwenMoeSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        qwen_moe_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const QwenMoeTorchMultinomialExecutionPolicy policy =
                qwen_moe_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public QwenMoeISampler {
  public:
    ...
    QwenMoeSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        qwen_moe_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    QwenMoeLogitsLocation logits_location() const override { return QwenMoeLogitsLocation::DEVICE; }
...
QwenMoeSamplingParams qwen_moe_sampling_params_from_config(const GenerateConfig& cfg,
                                                           int32_t default_eos) {
    QwenMoeSamplingParams p;
...
create_qwen_moe_sampler(const QwenMoeSamplingParams& params,
                        [[maybe_unused]] const QwenMoeSamplerFactoryOptions& options) {
...
std::unique_ptr<QwenMoeISampler> create_qwen_moe_sampler(const QwenMoeSamplingParams& params) {
    return create_qwen_moe_sampler(params, QwenMoeSamplerFactoryOptions{});
}

std::unique_ptr<QwenMoeISampler> create_qwen_moe_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
