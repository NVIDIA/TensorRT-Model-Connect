...
// QwenMoeISampler: token selection abstraction for autoregressive generation.
//
...
//
// QwenMoeLogitsLocation tells the pipeline whether to D2H transfer logits before
...
class QwenMoeISampler {
  public:
    ...
    virtual QwenMoeLogitsLocation logits_location() const = 0;
...
/// Build QwenMoeSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from QwenMoeSamplingParams.
...
create_qwen_moe_sampler(const QwenMoeSamplingParams& params,
                        const QwenMoeSamplerFactoryOptions& options);
