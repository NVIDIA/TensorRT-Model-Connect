...
// QwenMoeDecoderPlugin: Qwen-MoE-owned decoder runtime.
...
resolve_kv_cache_runtime_sizing(const PipelineContext& ctx, const TrtModule& module,
                                const QwenMoeKvCacheNames& kv_names, DType cache_dtype,
                                const QwenMoeTriAttentionConfig& tri_cfg, int32_t kv_dim) {
...
class QwenMoeDecoderPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        QwenMoeKvCacheNames kv_names;
        build_kv_names(ctx, io, kv_names);

        const DType cache_dtype = cache_dtype_from_precision(ctx.config.precision);
        QwenMoeTriAttentionConfig tri_cfg = qwen_moe_parse_triattention_bundle_config(
        ...
        return std::make_unique<QwenMoeTextGenerationPipeline>(
  ...
  private:
    ...
    load_split_prefill_module(const PipelineContext& ctx, cudaStream_t stream, const IoMap& io,
                              const QwenMoeKvCacheNames& kv_names, int32_t& prefill_profile_idx,
    ...
    static void build_kv_names(const PipelineContext& ctx, const IoMap& io,
        ...
        for (int32_t i = 0; i < ctx.config.num_layers; ++i) {
            kv_names.cache_k.push_back(qwen_moe_expand_layer_name(io.cache_k_pattern, i));
            kv_names.cache_v.push_back(qwen_moe_expand_layer_name(io.cache_v_pattern, i));
            kv_names.present_k.push_back(qwen_moe_expand_layer_name(io.present_k_pattern, i));
            kv_names.present_v.push_back(qwen_moe_expand_layer_name(io.present_v_pattern, i));
        }
    }
    ...
    static std::vector<QwenMoeTextGenerationPipeline::DecoderContext>
    build_decoder_contexts(BackendProfileModules profile_modules, int32_t runtime_rows,
        ...
        for (const auto& profile : profile_roles.decode_profiles) {
            ...
            decoders.push_back(QwenMoeTextGenerationPipeline::DecoderContext{
    ...
    static std::unique_ptr<QwenMoeInferenceState>
    build_inference_state(const PipelineContext& ctx, const KvCacheRuntimeSizing& sizing,
                          QwenMoeTriAttentionConfig& tri_cfg, DType cache_dtype, int32_t kv_dim,
                          QwenMoeKvCacheNames& kv_names, cudaStream_t stream) {
        std::unique_ptr<QwenMoeInferenceState> state;
        if (tri_cfg.enabled) {
            ...
            QwenMoeTriAttentionStats tri_stats = qwen_moe_parse_triattention_stats_json(
                stats_json, ctx.config.num_heads, ctx.config.num_kv_heads, ctx.config.num_layers);
            state = std::make_unique<QwenMoeTriAttentionKvCache>(
            ...
            state =
                std::make_unique<QwenMoeKvCache>(ctx.config.num_layers, sizing.runtime_rows, kv_dim,
                                                 stream, cache_dtype, std::move(kv_names));
        }
        if (!state->ok())
            throw std::runtime_error("Failed to create QwenMoeKvCache");
        return state;
    }

    static void log_kv_cache_sizing(const PipelineContext& ctx, const KvCacheRuntimeSizing& sizing,
                                    QwenMoeInferenceState* state) {
    ...
    static void
    populate_text_gen_config(const PipelineContext& ctx, QwenMoeTextGenConfig& tgc, const IoMap& io,
                             const QwenMoeTextGenerationPipeline::DecoderContext& first_dec,
    ...
    static void apply_chat_template_format(const BundleFile& bundle, QwenMoeTextGenConfig& tgc) {
        std::string chat_tpl;
        ...
        tgc.chat_template_format = qwen_moe_detect_chat_template_format(chat_tpl);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_qwen_moe_plugin, QwenMoeDecoderPlugin,
                                       "qwen_moe_decoder_moe");
