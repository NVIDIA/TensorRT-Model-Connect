...
// InternvlInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   InternvlKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — InternvlKvCache + family-owned recurrent state composed
...
class InternvlInferenceState {
  public:
    virtual ~InternvlInferenceState() = default;
    ...
    // InternvlKvCache -> true. Family-owned recurrent state -> false.
