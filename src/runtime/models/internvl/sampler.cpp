// InternVlISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// InternVlPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/internvl/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/internvl/argmax_kernel.h"
#include "runtime/models/internvl/sparse_multinomial_kernel.h"
...
static InternVlSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                               int32_t eos_token_id) {
    InternVlSampleResult result;
...
static bool greedy_equivalent(const InternVlSamplingParams& params) {
...
class GreedySampler final : public InternVlISampler {
  public:
    InternVlSampleResult sample(const float* logits, int32_t vocab_size,
                                const InternVlSamplingParams& params) override {
    ...
    InternVlLogitsLocation logits_location() const override { return InternVlLogitsLocation::HOST; }
...
class TopKSampler final : public InternVlISampler {
  public:
...
class TorchCudaMultinomialSampler final : public InternVlISampler {
  public:
    ...
    InternVlSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        internvl_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const InternvlTorchMultinomialExecutionPolicy policy =
                internvl_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public InternVlISampler {
  public:
    ...
    InternVlSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        internvl_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    InternVlLogitsLocation logits_location() const override {
        return InternVlLogitsLocation::DEVICE;
    }
...
InternVlSamplingParams internvl_sampling_params_from_config(const GenerateConfig& cfg,
                                                            int32_t default_eos) {
    InternVlSamplingParams p;
...
std::unique_ptr<InternVlISampler>
create_internvl_sampler(const InternVlSamplingParams& params,
                        [[maybe_unused]] const InternVlSamplerFactoryOptions& options) {
...
std::unique_ptr<InternVlISampler> create_internvl_sampler(const InternVlSamplingParams& params) {
    return create_internvl_sampler(params, InternVlSamplerFactoryOptions{});
}

std::unique_ptr<InternVlISampler> create_internvl_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
