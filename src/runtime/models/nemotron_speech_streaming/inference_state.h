...
// NemotronSpeechStreamingInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   NemotronSpeechStreamingKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — NemotronSpeechStreamingKvCache + family-owned recurrent state
//   composed (future: RingKvCache, PagedKvCache, MlaCache, SlidingWindowCache)
...
class NemotronSpeechStreamingInferenceState {
  public:
    virtual ~NemotronSpeechStreamingInferenceState() = default;
    ...
    // NemotronSpeechStreamingKvCache -> true. Family-owned recurrent state -> false.
