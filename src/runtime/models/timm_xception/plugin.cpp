...
// TimmXceptionPlugin: handles timm Xception image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_xception/pipeline.h"
...
TimmXceptionPreprocessConfig make_timm_xception_preprocess_config(const std::string& json) {
    TimmXceptionPreprocessConfig cfg;
...
class TimmXceptionPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmXceptionImageClassificationPipeline>(
            std::move(loaded.module), make_timm_xception_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_xception_plugin, TimmXceptionPlugin,
                                       "timm_xception_image_classification");
