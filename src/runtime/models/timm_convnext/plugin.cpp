...
// TimmConvnextPlugin: handles timm ConvNeXt image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_convnext/pipeline.h"
...
TimmConvnextPreprocessConfig make_timm_convnext_preprocess_config(const std::string& json) {
    TimmConvnextPreprocessConfig cfg;
...
class TimmConvnextPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmConvnextImageClassificationPipeline>(
            std::move(loaded.module), make_timm_convnext_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_convnext_plugin, TimmConvnextPlugin,
                                       "timm_convnext_image_classification");
