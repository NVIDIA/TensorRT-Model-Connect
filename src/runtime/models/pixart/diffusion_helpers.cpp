...
void load_preprocessor_weights(const std::string& index_json, const char* blob,
    ...
    pixart_preprocessor_weights::load_with_fallback(index_json, blob, blob_size,
                                                    "patch_embedding.weight", "x_embedder.weight",
                                                    w.patch_embed_weight);
    pixart_preprocessor_weights::load_with_fallback(
...
PixArtPreprocessorWeights parse_preprocessor_weights(const std::vector<char>& data) {
    ...
    if (!pixart_preprocessor_weights::extract_preprocessor_index(data, index_json, blob,
                                                                 blob_size)) {
        return w;
    }
