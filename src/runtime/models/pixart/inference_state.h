...
// PixartInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   PixartKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — PixartKvCache + family-owned recurrent state composed
...
class PixartInferenceState {
  public:
    virtual ~PixartInferenceState() = default;
    ...
    // PixartKvCache -> true. Family-owned recurrent state -> false.
