...
// PixartKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/pixart/inference_state.h"
...
class PixartKvCache : public PixartInferenceState {
  public:
    ...
    PixartKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, PixartKvCacheNames names = {});

    // --- PixartInferenceState overrides ---
    void reset() override;
    ...
    // --- PixartKvCache-specific methods (not on the interface) ---
    ...
    // — too big for PixartKvCache's single-row present buffer. The caller reads
