...
//
// Composes: TrtModule (decoder) + FalconKvCache + ITokenizer for this runtime
...
class FalconTextGenerationPipeline final : public IPipeline {
  public:
    ...
    FalconTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                 std::unique_ptr<FalconInferenceState> state,
                                 FalconTextGenConfig config, cudaStream_t stream,
                                 std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                 std::string model_id_str = "",
                                 std::unique_ptr<FalconISampler> sampler = nullptr,
                                 std::shared_ptr<void> distributed_owner = nullptr);
    FalconTextGenerationPipeline(std::vector<DecoderContext> decoders,
                                 std::unique_ptr<FalconInferenceState> state,
                                 FalconTextGenConfig config, cudaStream_t stream,
                                 std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                 std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "FalconTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<FalconInferenceState> state_;
    FalconTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<FalconISampler> sampler_;
    ...
    FalconKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(FalconKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(FalconISampler* sampler, const FalconSamplingParams& params,
    ...
    std::unique_ptr<FalconISampler> make_step_sampler(const FalconSamplingParams& params);
