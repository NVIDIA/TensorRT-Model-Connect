...
// FalconInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   FalconKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — FalconKvCache + family-owned recurrent state composed
...
class FalconInferenceState {
  public:
    virtual ~FalconInferenceState() = default;
    ...
    // FalconKvCache -> true. Family-owned recurrent state -> false.
