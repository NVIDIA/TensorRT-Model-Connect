...
// FalconISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// FalconTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/falcon/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/falcon/argmax_kernel.h"
#include "runtime/models/falcon/sparse_multinomial_kernel.h"
...
static FalconSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                             int32_t eos_token_id) {
    FalconSampleResult result;
...
static bool greedy_equivalent(const FalconSamplingParams& params) {
...
class GreedySampler final : public FalconISampler {
  public:
    FalconSampleResult sample(const float* logits, int32_t vocab_size,
                              const FalconSamplingParams& params) override {
    ...
    FalconLogitsLocation logits_location() const override { return FalconLogitsLocation::HOST; }
...
class TopKSampler final : public FalconISampler {
  public:
...
class TorchCudaMultinomialSampler final : public FalconISampler {
  public:
    ...
    FalconSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        falcon_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const FalconTorchMultinomialExecutionPolicy policy =
                falcon_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public FalconISampler {
  public:
    ...
    FalconSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        falcon_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    FalconLogitsLocation logits_location() const override { return FalconLogitsLocation::DEVICE; }
...
FalconSamplingParams falcon_sampling_params_from_config(const GenerateConfig& cfg,
                                                        int32_t default_eos) {
    FalconSamplingParams p;
...
create_falcon_sampler(const FalconSamplingParams& params,
                      [[maybe_unused]] const FalconSamplerFactoryOptions& options) {
...
std::unique_ptr<FalconISampler> create_falcon_sampler(const FalconSamplingParams& params) {
    return create_falcon_sampler(params, FalconSamplerFactoryOptions{});
}

std::unique_ptr<FalconISampler> create_falcon_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
