#include "runtime/models/falcon/triattention_kv_cache.h"
...
FalconTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return FalconTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return FalconTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(FalconTriAttentionScoreAggregation value) {
    switch (value) {
    case FalconTriAttentionScoreAggregation::kMean:
        return "mean";
    case FalconTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

FalconTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return FalconTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return FalconTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, FalconKvCacheNames& names) {
...
// override helpers that patched FalconTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     FalconTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(FalconTriAttentionConfig& cfg,
...
void fill_debug_from_registry(FalconTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(FalconTriAttentionConfig& cfg,
...
static void validate_triattention_config(const FalconTriAttentionConfig& cfg,
...
FalconTriAttentionConfig
falcon_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                        const ::trtmc::config::ConfigBundle* runtime_config) {
    FalconTriAttentionConfig cfg;
...
void fill_stats_header(FalconTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(FalconTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(FalconTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(FalconTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(FalconTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(FalconTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(FalconTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(FalconTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(FalconTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(FalconTriAttentionStats& stats, const std::vector<int32_t>& group_counts,
                           int32_t half_dim) {
    bool any_stats = false;
    for (int32_t layer = 0; layer < stats.num_layers; ++layer) {
        auto& layer_stats = stats.layer_stats[static_cast<std::size_t>(layer)];
...
void parse_sparse_stats(FalconTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
FalconTriAttentionStats falcon_parse_triattention_stats_json(const std::string& stats_json,
                                                             int32_t num_attention_heads,
                                                             int32_t num_key_value_heads,
                                                             int32_t num_layers) {
    FalconTriAttentionStats stats;
...
void FalconTriAttentionKvCache::validate_shapes() {
...
void FalconTriAttentionKvCache::normalize_sampled_heads() {
...
void FalconTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void FalconTriAttentionKvCache::allocate_layer_tensors() {
...
FalconTriAttentionKvCache::FalconTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                     int32_t max_length, int32_t kv_dim,
                                                     cudaStream_t stream,
                                                     FalconTriAttentionConfig config,
                                                     FalconTriAttentionStats stats,
                                                     DType cache_dtype, FalconKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void FalconTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void FalconTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void FalconTriAttentionKvCache::initialize_gpu_state() {
...
bool FalconTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool FalconTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool FalconTriAttentionKvCache::can_use_gpu_selection() const {
...
void FalconTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t FalconTriAttentionKvCache::preferred_cache_rows() const {
...
void FalconTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void FalconTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> FalconTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows, FalconTriAttentionCompactionProfile* profile) const {
...
void FalconTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t FalconTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> FalconTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
FalconTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
FalconTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                               FalconTriAttentionCompactionProfile* profile) {
...
int32_t FalconTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t FalconTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> FalconTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void FalconTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, FalconTriAttentionCompactionProfile* profile) const {
...
bool FalconTriAttentionKvCache::layer_stats_shapes_valid(
    const FalconTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void FalconTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                              int32_t half_dim, float& k_rot_real,
                                              float& k_rot_imag) const {
    if (stats_.rope_style == FalconTriAttentionRopeStyle::kInterleaved) {
...
float FalconTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == FalconTriAttentionScoreAggregation::kMax)
...
float FalconTriAttentionKvCache::score_one_row(
    const float* row_ptr, const FalconTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void FalconTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const FalconTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void FalconTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void FalconTriAttentionKvCache::accumulate_layer_fallback(
...
void FalconTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == FalconTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void FalconTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
FalconTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void FalconTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == FalconTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void FalconTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> FalconTriAttentionKvCache::build_keep_indices_per_head(
...
bool FalconTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, FalconTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> FalconTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, FalconTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void FalconTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                       int32_t total_tokens) const {
...
void FalconTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void FalconTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
FalconTriAttentionKvCache::GpuLayerResult
FalconTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = falcon_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == FalconTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == FalconTriAttentionScoreAggregation::kMax,
...
bool FalconTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool FalconTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, FalconTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void FalconTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> FalconTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool FalconTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> FalconTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, FalconTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void FalconTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void FalconTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool FalconTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool FalconTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                      std::size_t row_bytes) {
    const bool ok_k = falcon_triattention_compact_rows_gpu(
    ...
    const bool ok_v = falcon_triattention_compact_rows_gpu(
...
bool FalconTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void FalconTriAttentionKvCache::compact_layer_on_host(
...
void FalconTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    FalconTriAttentionCompactionProfile profile;
    FalconTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void FalconTriAttentionKvCache::finalize_repack_profile(
    FalconTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
FalconTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
FalconTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                     int32_t keep_count) const {
...
int32_t FalconTriAttentionKvCache::count_prefix_positions(
...
void FalconTriAttentionKvCache::log_compact_debug(
...
void FalconTriAttentionKvCache::log_compact_profile(
    const FalconTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool FalconTriAttentionKvCache::should_emit_keep_dump(
    const FalconTriAttentionCompactionProfile* profile_ptr) const {
...
void FalconTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "FalconTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void FalconTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void FalconTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void FalconTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool FalconTriAttentionKvCache::ok() const {
