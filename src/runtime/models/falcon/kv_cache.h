...
// FalconKvCache: autoregressive KV cache state manager.
...
class FalconKvCache : public FalconInferenceState {
  public:
    ...
    FalconKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, FalconKvCacheNames names = {});

    // --- FalconInferenceState overrides ---
    void reset() override;
    ...
    // --- FalconKvCache-specific methods (not on the interface) ---
    ...
    // — too big for FalconKvCache's single-row present buffer. The caller reads
