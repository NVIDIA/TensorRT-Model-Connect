...
#include "runtime/models/falcon/inference_state.h"
...
enum class FalconTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class FalconTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct FalconTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    FalconTriAttentionScoreAggregation score_aggregation{FalconTriAttentionScoreAggregation::kMean};
    FalconTriAttentionScoreAggregation per_layer_aggregation{
        FalconTriAttentionScoreAggregation::kMean};
...
struct FalconTriAttentionHeadStats {
...
struct FalconTriAttentionStats {
    int32_t head_dim{0};
    FalconTriAttentionRopeStyle rope_style{FalconTriAttentionRopeStyle::kHalf};
    ...
    std::vector<FalconTriAttentionHeadStats> layer_stats;
};

struct FalconTriAttentionCompactionProfile {
...
// Build a FalconTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class FalconTriAttentionKvCache : public FalconInferenceState {
  public:
    FalconTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                              int32_t kv_dim, cudaStream_t stream, FalconTriAttentionConfig config,
                              FalconTriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(FalconTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const FalconTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const FalconTriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        FalconTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             FalconTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                FalconTriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const FalconTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const FalconTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const FalconTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    FalconKvCacheNames names_;
    FalconTriAttentionConfig config_;
    FalconTriAttentionStats stats_;
