...
// DecoderPlugin: Nemotron Labs Diffusion-owned text diffusion runtime.
...
KvCacheRuntimeSizing resolve_kv_cache_runtime_sizing(
    const PipelineContext& ctx, const TrtModule& module,
    const NemotronLabsDiffusionKvCacheNames& kv_names, DType cache_dtype,
    const NemotronLabsDiffusionTriAttentionConfig& tri_cfg, int32_t kv_dim) {
...
class DecoderPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        NemotronLabsDiffusionKvCacheNames kv_names;
        build_kv_names(ctx, io, kv_names);

        const DType cache_dtype = cache_dtype_from_precision(ctx.config.precision);
        NemotronLabsDiffusionTriAttentionConfig tri_cfg =
            nemotron_labs_diffusion_parse_triattention_bundle_config(
        ...
        NemotronLabsDiffusionTextGenConfig tgc;
        ...
        return std::make_unique<NemotronLabsDiffusionTextGenerationPipeline>(
  ...
  private:
    ...
    load_split_prefill_module(const PipelineContext& ctx, cudaStream_t stream, const IoMap& io,
                              const NemotronLabsDiffusionKvCacheNames& kv_names,
                              int32_t& prefill_profile_idx, int32_t& prefill_max_length,
                              std::string& prefill_log_label) {
    ...
    static void populate_nemotron_diffusion_config(const PipelineContext& ctx,
                                                   NemotronLabsDiffusionTextGenConfig& tgc) {
        if (ctx.config.runtime_strategy != "nemotron_labs_diffusion")
            return;
    ...
    static void build_kv_names(const PipelineContext& ctx, const IoMap& io,
        ...
        for (int32_t i = 0; i < ctx.config.num_layers; ++i) {
            kv_names.cache_k.push_back(
                nemotron_labs_diffusion_expand_layer_name(io.cache_k_pattern, i));
            kv_names.cache_v.push_back(
                nemotron_labs_diffusion_expand_layer_name(io.cache_v_pattern, i));
            kv_names.present_k.push_back(
                nemotron_labs_diffusion_expand_layer_name(io.present_k_pattern, i));
            kv_names.present_v.push_back(
                nemotron_labs_diffusion_expand_layer_name(io.present_v_pattern, i));
        }
    }
    ...
    static std::vector<NemotronLabsDiffusionTextGenerationPipeline::DecoderContext>
    build_decoder_contexts(BackendProfileModules profile_modules, int32_t runtime_rows,
        ...
        for (const auto& profile : profile_roles.decode_profiles) {
            ...
            decoders.push_back(NemotronLabsDiffusionTextGenerationPipeline::DecoderContext{
    ...
    static std::unique_ptr<NemotronLabsDiffusionInferenceState>
    build_inference_state(const PipelineContext& ctx, const KvCacheRuntimeSizing& sizing,
                          NemotronLabsDiffusionTriAttentionConfig& tri_cfg, DType cache_dtype,
                          int32_t kv_dim, NemotronLabsDiffusionKvCacheNames& kv_names,
                          cudaStream_t stream) {
        ...
        if (tri_cfg.enabled) {
            ...
            NemotronLabsDiffusionTriAttentionStats tri_stats =
                nemotron_labs_diffusion_parse_triattention_stats_json(
                    stats_json, ctx.config.num_heads, ctx.config.num_kv_heads,
                    ctx.config.num_layers);
            state = std::make_unique<NemotronLabsDiffusionTriAttentionKvCache>(
            ...
            state = std::make_unique<NemotronLabsDiffusionKvCache>(
                ctx.config.num_layers, sizing.runtime_rows, kv_dim, stream, cache_dtype,
                std::move(kv_names));
        }
        if (!state->ok())
            throw std::runtime_error("Failed to create NemotronLabsDiffusionKvCache");
        return state;
    }
    ...
    static void populate_text_gen_config(
        const PipelineContext& ctx, NemotronLabsDiffusionTextGenConfig& tgc, const IoMap& io,
        const NemotronLabsDiffusionTextGenerationPipeline::DecoderContext& first_dec,
    ...
    static void apply_chat_template_format(const BundleFile& bundle,
                                           NemotronLabsDiffusionTextGenConfig& tgc) {
        std::string chat_tpl;
        ...
        tgc.chat_template_format = nemotron_labs_diffusion_detect_chat_template_format(chat_tpl);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_nemotron_labs_diffusion_plugin, DecoderPlugin,
