...
// NemotronLabsDiffusionInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   NemotronLabsDiffusionKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — NemotronLabsDiffusionKvCache + family-owned recurrent state
...
class NemotronLabsDiffusionInferenceState {
  public:
    virtual ~NemotronLabsDiffusionInferenceState() = default;
    ...
    // NemotronLabsDiffusionKvCache -> true. Family-owned recurrent state -> false.
