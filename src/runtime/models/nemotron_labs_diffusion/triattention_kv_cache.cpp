...
#include "runtime/models/nemotron_labs_diffusion/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/nemotron_labs_diffusion/triattention_kernels.h"
#endif
...
NemotronLabsDiffusionTriAttentionScoreAggregation
parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return NemotronLabsDiffusionTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return NemotronLabsDiffusionTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(NemotronLabsDiffusionTriAttentionScoreAggregation value) {
    switch (value) {
    case NemotronLabsDiffusionTriAttentionScoreAggregation::kMean:
        return "mean";
    case NemotronLabsDiffusionTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

NemotronLabsDiffusionTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return NemotronLabsDiffusionTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return NemotronLabsDiffusionTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, NemotronLabsDiffusionKvCacheNames& names) {
...
// override helpers that patched NemotronLabsDiffusionTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     NemotronLabsDiffusionTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(NemotronLabsDiffusionTriAttentionConfig& cfg,
...
void fill_debug_from_registry(NemotronLabsDiffusionTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(NemotronLabsDiffusionTriAttentionConfig& cfg,
...
static void validate_triattention_config(const NemotronLabsDiffusionTriAttentionConfig& cfg,
...
NemotronLabsDiffusionTriAttentionConfig nemotron_labs_diffusion_parse_triattention_bundle_config(
    const std::string& config_json, int32_t max_cache_length,
    const ::trtmc::config::ConfigBundle* runtime_config) {
    NemotronLabsDiffusionTriAttentionConfig cfg;
...
void fill_stats_header(NemotronLabsDiffusionTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(NemotronLabsDiffusionTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(NemotronLabsDiffusionTriAttentionStats& stats,
                                     const json& sampled_root, int32_t head_upper_bound) {
...
void populate_sampled_heads(NemotronLabsDiffusionTriAttentionStats& stats, const json& root,
...
void copy_dense_head_row(NemotronLabsDiffusionTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(NemotronLabsDiffusionTriAttentionHeadStats& dst,
                                 const json& layer_node, int32_t& inferred_stats_heads,
                                 int32_t half_dim) {
...
bool try_parse_dense_layer_stats(NemotronLabsDiffusionTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(NemotronLabsDiffusionTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(NemotronLabsDiffusionTriAttentionStats& stats, const json& raw_stats,
                             int32_t layer, int32_t head, int32_t half_dim,
...
bool finalize_sparse_stats(NemotronLabsDiffusionTriAttentionStats& stats,
...
void parse_sparse_stats(NemotronLabsDiffusionTriAttentionStats& stats, const json& root,
                        int32_t half_dim) {
...
NemotronLabsDiffusionTriAttentionStats nemotron_labs_diffusion_parse_triattention_stats_json(
    const std::string& stats_json, int32_t num_attention_heads, int32_t num_key_value_heads,
    int32_t num_layers) {
    NemotronLabsDiffusionTriAttentionStats stats;
...
void NemotronLabsDiffusionTriAttentionKvCache::validate_shapes() {
...
void NemotronLabsDiffusionTriAttentionKvCache::normalize_sampled_heads() {
...
void NemotronLabsDiffusionTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void NemotronLabsDiffusionTriAttentionKvCache::allocate_layer_tensors() {
...
NemotronLabsDiffusionTriAttentionKvCache::NemotronLabsDiffusionTriAttentionKvCache(
    int32_t num_layers, int32_t num_kv_heads, int32_t max_length, int32_t kv_dim,
    cudaStream_t stream, NemotronLabsDiffusionTriAttentionConfig config,
    NemotronLabsDiffusionTriAttentionStats stats, DType cache_dtype,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void NemotronLabsDiffusionTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void NemotronLabsDiffusionTriAttentionKvCache::build_layer_gpu_stats(int32_t layer,
                                                                     int32_t half_dim) {
...
void NemotronLabsDiffusionTriAttentionKvCache::initialize_gpu_state() {
...
bool NemotronLabsDiffusionTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool NemotronLabsDiffusionTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool NemotronLabsDiffusionTriAttentionKvCache::can_use_gpu_selection() const {
...
void NemotronLabsDiffusionTriAttentionKvCache::build_attention_mask(
...
int32_t NemotronLabsDiffusionTriAttentionKvCache::preferred_cache_rows() const {
...
void NemotronLabsDiffusionTriAttentionKvCache::prepare_step(TensorMap& inputs,
                                                            int32_t /*seq_len*/) {
...
void NemotronLabsDiffusionTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> NemotronLabsDiffusionTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows,
    NemotronLabsDiffusionTriAttentionCompactionProfile* profile) const {
...
void NemotronLabsDiffusionTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t NemotronLabsDiffusionTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char>
NemotronLabsDiffusionTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
NemotronLabsDiffusionTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t> NemotronLabsDiffusionTriAttentionKvCache::select_keep_indices(
    int32_t keep_budget, NemotronLabsDiffusionTriAttentionCompactionProfile* profile) {
...
int32_t
NemotronLabsDiffusionTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t NemotronLabsDiffusionTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> NemotronLabsDiffusionTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void NemotronLabsDiffusionTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, NemotronLabsDiffusionTriAttentionCompactionProfile* profile) const {
...
bool NemotronLabsDiffusionTriAttentionKvCache::layer_stats_shapes_valid(
    const NemotronLabsDiffusionTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void NemotronLabsDiffusionTriAttentionKvCache::extract_k_rot(const float* row_ptr,
                                                             int32_t head_offset, int32_t d,
                                                             int32_t half_dim, float& k_rot_real,
                                                             float& k_rot_imag) const {
    if (stats_.rope_style == NemotronLabsDiffusionTriAttentionRopeStyle::kInterleaved) {
...
float NemotronLabsDiffusionTriAttentionKvCache::reduce_trig_sums(
    const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == NemotronLabsDiffusionTriAttentionScoreAggregation::kMax)
...
float NemotronLabsDiffusionTriAttentionKvCache::score_one_row(
    const float* row_ptr, const NemotronLabsDiffusionTriAttentionHeadStats& layer_stats,
...
void NemotronLabsDiffusionTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const NemotronLabsDiffusionTriAttentionHeadStats& layer_stats, int32_t score_head,
...
void NemotronLabsDiffusionTriAttentionKvCache::standardize_scores(
...
void NemotronLabsDiffusionTriAttentionKvCache::accumulate_layer_fallback(
...
void NemotronLabsDiffusionTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max =
        config_.per_layer_aggregation == NemotronLabsDiffusionTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void NemotronLabsDiffusionTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float> NemotronLabsDiffusionTriAttentionKvCache::compute_fallback_mean(
    const std::vector<float>& global_fallback_sum, int32_t global_fallback_count,
...
void NemotronLabsDiffusionTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
...
void NemotronLabsDiffusionTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> NemotronLabsDiffusionTriAttentionKvCache::build_keep_indices_per_head(
...
bool NemotronLabsDiffusionTriAttentionKvCache::process_layer_for_host_selection(
...
std::vector<int32_t> NemotronLabsDiffusionTriAttentionKvCache::select_keep_indices_host(
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void NemotronLabsDiffusionTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                                      int32_t total_tokens) const {
...
void NemotronLabsDiffusionTriAttentionKvCache::accumulate_flat_fallback(
    const float* rows, int32_t num_rows, int32_t total_tokens, std::vector<float>& fallback_sum,
...
void NemotronLabsDiffusionTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
NemotronLabsDiffusionTriAttentionKvCache::GpuLayerResult
NemotronLabsDiffusionTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    std::vector<int32_t>& contributing_layers_by_cache_head,
    NemotronLabsDiffusionTriAttentionCompactionProfile* profile) {
    ...
    const bool launched = nemotron_labs_diffusion_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == NemotronLabsDiffusionTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == NemotronLabsDiffusionTriAttentionScoreAggregation::kMax,
...
bool NemotronLabsDiffusionTriAttentionKvCache::upload_candidate_indices_identity(
...
bool NemotronLabsDiffusionTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim,
    NemotronLabsDiffusionTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void NemotronLabsDiffusionTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> NemotronLabsDiffusionTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool NemotronLabsDiffusionTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> NemotronLabsDiffusionTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates,
    NemotronLabsDiffusionTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
...
void NemotronLabsDiffusionTriAttentionKvCache::dump_cache_rows_to_file(
    const DeviceTensor& tensor, int32_t rows, const std::string& path,
...
void NemotronLabsDiffusionTriAttentionKvCache::maybe_dump_score_cache(
    int32_t rows, const char* k_pattern, const char* v_pattern,
    std::vector<std::string>& score_files, std::vector<std::string>& value_files) {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool NemotronLabsDiffusionTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool NemotronLabsDiffusionTriAttentionKvCache::gpu_compact_one_layer(int32_t layer,
                                                                     int32_t keep_count,
                                                                     std::size_t row_bytes) {
    const bool ok_k = nemotron_labs_diffusion_triattention_compact_rows_gpu(
    ...
    const bool ok_v = nemotron_labs_diffusion_triattention_compact_rows_gpu(
...
bool NemotronLabsDiffusionTriAttentionKvCache::compact_layer_on_gpu(
    int32_t layer, const std::vector<int32_t>& keep_indices, int32_t keep_count,
    std::size_t row_bytes, int64_t& repack_calls, std::size_t& repack_bytes) {
#ifdef TRTMC_HAS_CUDA_KERNELS
...
void NemotronLabsDiffusionTriAttentionKvCache::compact_layer_on_host(
    int32_t layer, const std::vector<int32_t>& keep_indices, int32_t keep_count,
...
void NemotronLabsDiffusionTriAttentionKvCache::compact_existing_cache(
    bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    NemotronLabsDiffusionTriAttentionCompactionProfile profile;
    NemotronLabsDiffusionTriAttentionCompactionProfile* profile_ptr =
        profile_enabled_ ? &profile : nullptr;
...
void NemotronLabsDiffusionTriAttentionKvCache::finalize_repack_profile(
    NemotronLabsDiffusionTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
NemotronLabsDiffusionTriAttentionKvCache::build_new_positions_by_head(
...
std::vector<int32_t> NemotronLabsDiffusionTriAttentionKvCache::collect_dropped_positions(
    const std::vector<int32_t>& keep_indices, int32_t keep_count) const {
...
int32_t NemotronLabsDiffusionTriAttentionKvCache::count_prefix_positions(
...
void NemotronLabsDiffusionTriAttentionKvCache::log_compact_debug(
...
void NemotronLabsDiffusionTriAttentionKvCache::log_compact_profile(
    const NemotronLabsDiffusionTriAttentionCompactionProfile* profile_ptr,
    int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool NemotronLabsDiffusionTriAttentionKvCache::should_emit_keep_dump(
    const NemotronLabsDiffusionTriAttentionCompactionProfile* profile_ptr) const {
...
void NemotronLabsDiffusionTriAttentionKvCache::emit_keep_dump_json(
...
void NemotronLabsDiffusionTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 &&
           "NemotronLabsDiffusionTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void NemotronLabsDiffusionTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void NemotronLabsDiffusionTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void NemotronLabsDiffusionTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool NemotronLabsDiffusionTriAttentionKvCache::ok() const {
