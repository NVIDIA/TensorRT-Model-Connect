...
#include "runtime/models/nemotron_labs_diffusion/inference_state.h"
...
enum class NemotronLabsDiffusionTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class NemotronLabsDiffusionTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct NemotronLabsDiffusionTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    NemotronLabsDiffusionTriAttentionScoreAggregation score_aggregation{
        NemotronLabsDiffusionTriAttentionScoreAggregation::kMean};
    NemotronLabsDiffusionTriAttentionScoreAggregation per_layer_aggregation{
        NemotronLabsDiffusionTriAttentionScoreAggregation::kMean};
...
struct NemotronLabsDiffusionTriAttentionHeadStats {
...
struct NemotronLabsDiffusionTriAttentionStats {
    int32_t head_dim{0};
    NemotronLabsDiffusionTriAttentionRopeStyle rope_style{
        NemotronLabsDiffusionTriAttentionRopeStyle::kHalf};
    ...
    std::vector<NemotronLabsDiffusionTriAttentionHeadStats> layer_stats;
};

struct NemotronLabsDiffusionTriAttentionCompactionProfile {
...
// Build a NemotronLabsDiffusionTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class NemotronLabsDiffusionTriAttentionKvCache : public NemotronLabsDiffusionInferenceState {
  public:
    NemotronLabsDiffusionTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                             int32_t max_length, int32_t kv_dim,
                                             cudaStream_t stream,
                                             NemotronLabsDiffusionTriAttentionConfig config,
  ...
  private:
    ...
    void finalize_repack_profile(NemotronLabsDiffusionTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const NemotronLabsDiffusionTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(
    ...
    select_keep_indices(int32_t keep_budget,
                        NemotronLabsDiffusionTriAttentionCompactionProfile* profile = nullptr);
    ...
    std::vector<int32_t> select_keep_indices_host(
        int32_t keep_budget, const std::vector<int32_t>& reserved,
        const std::vector<int32_t>& candidates,
        NemotronLabsDiffusionTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const NemotronLabsDiffusionTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr,
                        const NemotronLabsDiffusionTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const NemotronLabsDiffusionTriAttentionHeadStats& layer_stats,
                             int32_t score_head, int32_t cache_head, int32_t half_dim,
                             int32_t total_tokens, const std::vector<std::vector<float>>& cos_phase,
                             const std::vector<std::vector<float>>& sin_phase) const;
    ...
    build_keep_indices_per_head(const std::vector<std::vector<float>>& aggregated_scores,
                                ...
                                int32_t need) const;
    bool process_layer_for_host_selection(
        ...
        NemotronLabsDiffusionTriAttentionCompactionProfile* profile) const;
    std::vector<float> copy_cache_rows_to_host(
    ...
    NemotronLabsDiffusionKvCacheNames names_;
    NemotronLabsDiffusionTriAttentionConfig config_;
    NemotronLabsDiffusionTriAttentionStats stats_;
