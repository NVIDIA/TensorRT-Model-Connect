...
//
// Composes: TrtModule (decoder) + NemotronLabsDiffusionKvCache + ITokenizer for this runtime
...
class NemotronLabsDiffusionTextGenerationPipeline final : public IPipeline {
  public:
    ...
    NemotronLabsDiffusionTextGenerationPipeline(
        ...
        std::unique_ptr<NemotronLabsDiffusionISampler> sampler = nullptr,
    ...
    const char* pipeline_type() const override {
        return "NemotronLabsDiffusionTextGenerationPipeline";
    }
  ...
  private:
    ...
    std::unique_ptr<NemotronLabsDiffusionISampler> sampler_;
    ...
    void append_prefill_kv(NemotronLabsDiffusionKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    make_step_sampler(const NemotronLabsDiffusionSamplingParams& params);
