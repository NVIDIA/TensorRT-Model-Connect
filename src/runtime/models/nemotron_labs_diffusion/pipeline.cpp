...
#include "runtime/models/nemotron_labs_diffusion/pipeline.h"

#include "runtime/models/nemotron_labs_diffusion/chat_templates.h"
#include "runtime/models/nemotron_labs_diffusion/kv_cache.h"
#include "runtime/models/nemotron_labs_diffusion/tensor_names.h"
...
std::vector<NemotronLabsDiffusionTextGenerationPipeline::DecoderContext>
single_decoder_context(std::unique_ptr<TrtModule> decoder) {
    std::vector<NemotronLabsDiffusionTextGenerationPipeline::DecoderContext> decoders;
    decoders.push_back(
        NemotronLabsDiffusionTextGenerationPipeline::DecoderContext{0, std::move(decoder)});
    return decoders;
}
...
NemotronLabsDiffusionTextGenerationPipeline::NemotronLabsDiffusionTextGenerationPipeline(
    std::unique_ptr<TrtModule> decoder, std::unique_ptr<NemotronLabsDiffusionInferenceState> state,
    NemotronLabsDiffusionTextGenConfig config, cudaStream_t stream,
    std::shared_ptr<ITokenizer> tokenizer, std::string model_id_str,
    std::unique_ptr<NemotronLabsDiffusionISampler> sampler, std::shared_ptr<void> distributed_owner)
    : NemotronLabsDiffusionTextGenerationPipeline(
          single_decoder_context(std::move(decoder)), std::move(state), std::move(config), stream,
          std::move(tokenizer), std::move(model_id_str), std::move(sampler),
          /*prefill=*/nullptr, /*linear_spec_lora_prefill=*/nullptr, std::move(distributed_owner)) {
}

NemotronLabsDiffusionTextGenerationPipeline::NemotronLabsDiffusionTextGenerationPipeline(
    std::vector<DecoderContext> decoders,
    std::unique_ptr<NemotronLabsDiffusionInferenceState> state,
    NemotronLabsDiffusionTextGenConfig config, cudaStream_t stream,
    std::shared_ptr<ITokenizer> tokenizer, std::string model_id_str,
    std::unique_ptr<NemotronLabsDiffusionISampler> sampler, std::unique_ptr<TrtModule> prefill,
    ...
    if (decoders_.empty()) {
        throw std::runtime_error("NemotronLabsDiffusionTextGenerationPipeline: no decoder modules");
    }
...
TextResult NemotronLabsDiffusionTextGenerationPipeline::generate(const std::string& prompt,
    ...
    if (!tokenizer_) {
        throw std::runtime_error(
            "NemotronLabsDiffusionTextGenerationPipeline: no tokenizer configured");
    }
...
std::unique_ptr<NemotronLabsDiffusionISampler>
NemotronLabsDiffusionTextGenerationPipeline::make_step_sampler(
    ...
    if (prefer_gpu_greedy_ && greedy_params) {
        if (auto gpu = create_nemotron_labs_diffusion_gpu_greedy_sampler(stream_))
            return gpu;
    }
    return create_nemotron_labs_diffusion_sampler(params);
}
...
namespace {
bool gather_prefill_kv_pointers(TrtModule& prefill, const NemotronLabsDiffusionTextGenConfig& cfg,
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        pk[li] =
            prefill.device_ptr(nemotron_labs_diffusion_expand_layer_name(cfg.present_k_pattern, i));
        pv[li] =
            prefill.device_ptr(nemotron_labs_diffusion_expand_layer_name(cfg.present_v_pattern, i));
...
bool batched_prefill_supported(const TrtModule* prefill,
    ...
    if (cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        return false;
    return dynamic_cast<NemotronLabsDiffusionKvCache*>(state) != nullptr;
}
} // namespace

bool NemotronLabsDiffusionTextGenerationPipeline::run_prefill_batched(
...
void NemotronLabsDiffusionTextGenerationPipeline::append_prefill_kv(
    NemotronLabsDiffusionKvCache& kv, TrtModule& prefill, int32_t sq) {
    std::vector<const void*> pk, pv;
    if (!gather_prefill_kv_pointers(prefill, config_, pk, pv)) {
        throw std::runtime_error("NemotronLabsDiffusionTextGenerationPipeline: prefill module is "
                                 "missing present_k/present_v outputs");
    }
...
void NemotronLabsDiffusionTextGenerationPipeline::run_prefill_block(
    ...
    std::vector<float>& logits, TrtModule* prefill_override) {
    const auto sq = static_cast<int32_t>(input_ids.size());
...
NemotronLabsDiffusionTextGenerationPipeline::generate_from_ids(
    ...
    const NemotronLabsDiffusionSamplingParams& params, const GenerateConfig& cfg) {
    ...
    NemotronLabsDiffusionISampler* active_sampler = sampler_.get();
    std::unique_ptr<NemotronLabsDiffusionISampler> local_sampler;
    if (!active_sampler) {
    ...
    const bool gpu_sampling =
        (active_sampler->logits_location() == NemotronLabsDiffusionLogitsLocation::DEVICE);
...
bool NemotronLabsDiffusionTextGenerationPipeline::should_stop_on_answer(
    const std::vector<int32_t>& output, int32_t prompt_token_count, const GenerateConfig& cfg,
...
void NemotronLabsDiffusionTextGenerationPipeline::log_decode_summary(int32_t steps,
                                                                     double ms) const {
...
int32_t NemotronLabsDiffusionTextGenerationPipeline::run_decode_loop(
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        ...
        const NemotronLabsDiffusionSampleResult result =
            sampler->sample(sample_ptr, vocab_size, params);
...
int32_t
NemotronLabsDiffusionTextGenerationPipeline::select_decoder_index(int32_t desired_rows) const {
    if (decoders_.size() == 1)
        return 0;
...
TrtModule& NemotronLabsDiffusionTextGenerationPipeline::bind_decoder_for_step() {
...
void NemotronLabsDiffusionTextGenerationPipeline::run_step_device(int32_t token_id) {
    TensorMap inputs;
...
int32_t NemotronLabsDiffusionTextGenerationPipeline::argmax(const std::vector<float>& logits) {
    if (logits.empty())
        return 0;
