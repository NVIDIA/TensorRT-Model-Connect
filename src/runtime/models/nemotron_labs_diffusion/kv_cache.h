...
// NemotronLabsDiffusionKvCache: autoregressive KV cache state manager.
...
struct NemotronLabsDiffusionKvCacheNames {
...
class NemotronLabsDiffusionKvCache : public NemotronLabsDiffusionInferenceState {
  public:
    ...
    NemotronLabsDiffusionKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim,
                                 cudaStream_t stream, DType cache_dtype = DType::kFloat32,
                                 NemotronLabsDiffusionKvCacheNames names = {});

    // --- NemotronLabsDiffusionInferenceState overrides ---
    void reset() override;
    ...
    // --- NemotronLabsDiffusionKvCache-specific methods (not on the interface) ---
    ...
    // — too big for NemotronLabsDiffusionKvCache's single-row present buffer. The caller reads
