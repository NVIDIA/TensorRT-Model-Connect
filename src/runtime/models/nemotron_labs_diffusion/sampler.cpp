// NemotronLabsDiffusionISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// NemotronLabsDiffusionTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/nemotron_labs_diffusion/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/nemotron_labs_diffusion/argmax_kernel.h"
#include "runtime/models/nemotron_labs_diffusion/sparse_multinomial_kernel.h"
...
static NemotronLabsDiffusionSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                                            int32_t eos_token_id) {
    NemotronLabsDiffusionSampleResult result;
...
static bool greedy_equivalent(const NemotronLabsDiffusionSamplingParams& params) {
...
build_filtered_distribution(const float* logits, int32_t vocab_size,
...
class GreedySampler final : public NemotronLabsDiffusionISampler {
  public:
    NemotronLabsDiffusionSampleResult
    sample(const float* logits, int32_t vocab_size,
           const NemotronLabsDiffusionSamplingParams& params) override {
    ...
    NemotronLabsDiffusionLogitsLocation logits_location() const override {
        return NemotronLabsDiffusionLogitsLocation::HOST;
    }
...
class TopKSampler final : public NemotronLabsDiffusionISampler {
  public:
...
class TorchCudaMultinomialSampler final : public NemotronLabsDiffusionISampler {
  public:
    ...
    sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        nemotron_labs_diffusion_gpu_sparse_torch_multinomial_exact(
            d_indices_, d_probs_, dist.keep, initial_seed_, current_offset_, total_threads_,
            d_token_id_, stream_);
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const NemotronLabsDiffusionTorchMultinomialExecutionPolicy policy =
                nemotron_labs_diffusion_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public NemotronLabsDiffusionISampler {
  public:
    ...
    sample(const float* logits, int32_t vocab_size,
        ...
        nemotron_labs_diffusion_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    NemotronLabsDiffusionLogitsLocation logits_location() const override {
        return NemotronLabsDiffusionLogitsLocation::DEVICE;
    }
...
nemotron_labs_diffusion_sampling_params_from_config(const GenerateConfig& cfg,
                                                    int32_t default_eos) {
    NemotronLabsDiffusionSamplingParams p;
...
std::unique_ptr<NemotronLabsDiffusionISampler> create_nemotron_labs_diffusion_sampler(
    const NemotronLabsDiffusionSamplingParams& params,
    [[maybe_unused]] const NemotronLabsDiffusionSamplerFactoryOptions& options) {
...
create_nemotron_labs_diffusion_sampler(const NemotronLabsDiffusionSamplingParams& params) {
    return create_nemotron_labs_diffusion_sampler(params,
                                                  NemotronLabsDiffusionSamplerFactoryOptions{});
}

std::unique_ptr<NemotronLabsDiffusionISampler>
create_nemotron_labs_diffusion_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
