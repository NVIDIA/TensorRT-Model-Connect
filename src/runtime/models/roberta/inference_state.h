...
// RobertaInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   RobertaKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — RobertaKvCache + family-owned recurrent state composed
...
class RobertaInferenceState {
  public:
    virtual ~RobertaInferenceState() = default;
    ...
    // RobertaKvCache -> true. Family-owned recurrent state -> false.
