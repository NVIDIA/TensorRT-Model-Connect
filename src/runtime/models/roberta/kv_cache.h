...
// RobertaKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/roberta/inference_state.h"
...
class RobertaKvCache : public RobertaInferenceState {
  public:
    ...
    RobertaKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                   DType cache_dtype = DType::kFloat32, RobertaKvCacheNames names = {});

    // --- RobertaInferenceState overrides ---
    void reset() override;
    ...
    // --- RobertaKvCache-specific methods (not on the interface) ---
    ...
    // — too big for RobertaKvCache's single-row present buffer. The caller reads
