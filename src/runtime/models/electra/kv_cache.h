...
// ElectraKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/electra/inference_state.h"
...
class ElectraKvCache : public ElectraInferenceState {
  public:
    ...
    ElectraKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                   DType cache_dtype = DType::kFloat32, ElectraKvCacheNames names = {});

    // --- ElectraInferenceState overrides ---
    void reset() override;
    ...
    // --- ElectraKvCache-specific methods (not on the interface) ---
    ...
    // — too big for ElectraKvCache's single-row present buffer. The caller reads
