...
// ElectraInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   ElectraKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — ElectraKvCache + family-owned recurrent state composed
...
class ElectraInferenceState {
  public:
    virtual ~ElectraInferenceState() = default;
    ...
    // ElectraKvCache -> true. Family-owned recurrent state -> false.
