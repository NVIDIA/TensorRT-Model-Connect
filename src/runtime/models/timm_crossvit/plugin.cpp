...
// TimmCrossvitPlugin: handles timm CrossViT image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_crossvit/pipeline.h"
...
TimmCrossvitPreprocessConfig make_timm_crossvit_preprocess_config(const std::string& json) {
    TimmCrossvitPreprocessConfig cfg;
...
class TimmCrossvitPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmCrossvitImageClassificationPipeline>(
            std::move(loaded.module), make_timm_crossvit_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_crossvit_plugin, TimmCrossvitPlugin,
                                       "timm_crossvit_image_classification");
