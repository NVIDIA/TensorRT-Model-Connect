...
// CodegenKvCache: autoregressive KV cache state manager.
...
class CodegenKvCache : public CodegenInferenceState {
  public:
    ...
    CodegenKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                   DType cache_dtype = DType::kFloat32, CodegenKvCacheNames names = {});

    // --- CodegenInferenceState overrides ---
    void reset() override;
    ...
    // --- CodegenKvCache-specific methods (not on the interface) ---
    ...
    // — too big for CodegenKvCache's single-row present buffer. The caller reads
