#include "runtime/models/codegen/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/codegen/triattention_kernels.h"
#endif
...
CodegenTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return CodegenTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return CodegenTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(CodegenTriAttentionScoreAggregation value) {
    switch (value) {
    case CodegenTriAttentionScoreAggregation::kMean:
        return "mean";
    case CodegenTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

CodegenTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return CodegenTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return CodegenTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, CodegenKvCacheNames& names) {
...
// override helpers that patched CodegenTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     CodegenTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(CodegenTriAttentionConfig& cfg,
...
void fill_debug_from_registry(CodegenTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(CodegenTriAttentionConfig& cfg,
...
static void validate_triattention_config(const CodegenTriAttentionConfig& cfg,
...
CodegenTriAttentionConfig
codegen_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                         const ::trtmc::config::ConfigBundle* runtime_config) {
    CodegenTriAttentionConfig cfg;
...
void fill_stats_header(CodegenTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(CodegenTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(CodegenTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(CodegenTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(CodegenTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(CodegenTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(CodegenTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(CodegenTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(CodegenTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(CodegenTriAttentionStats& stats,
...
void parse_sparse_stats(CodegenTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
CodegenTriAttentionStats codegen_parse_triattention_stats_json(const std::string& stats_json,
                                                               int32_t num_attention_heads,
                                                               int32_t num_key_value_heads,
                                                               int32_t num_layers) {
    CodegenTriAttentionStats stats;
...
void CodegenTriAttentionKvCache::validate_shapes() {
...
void CodegenTriAttentionKvCache::normalize_sampled_heads() {
...
void CodegenTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void CodegenTriAttentionKvCache::allocate_layer_tensors() {
...
CodegenTriAttentionKvCache::CodegenTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                       int32_t max_length, int32_t kv_dim,
                                                       cudaStream_t stream,
                                                       CodegenTriAttentionConfig config,
                                                       CodegenTriAttentionStats stats,
                                                       DType cache_dtype, CodegenKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void CodegenTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void CodegenTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void CodegenTriAttentionKvCache::initialize_gpu_state() {
...
bool CodegenTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool CodegenTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool CodegenTriAttentionKvCache::can_use_gpu_selection() const {
...
void CodegenTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t CodegenTriAttentionKvCache::preferred_cache_rows() const {
...
void CodegenTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void CodegenTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> CodegenTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows, CodegenTriAttentionCompactionProfile* profile) const {
...
void CodegenTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t CodegenTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> CodegenTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
CodegenTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
CodegenTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                                CodegenTriAttentionCompactionProfile* profile) {
...
int32_t CodegenTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t CodegenTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> CodegenTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void CodegenTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, CodegenTriAttentionCompactionProfile* profile) const {
...
bool CodegenTriAttentionKvCache::layer_stats_shapes_valid(
    const CodegenTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void CodegenTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                               int32_t half_dim, float& k_rot_real,
                                               float& k_rot_imag) const {
    if (stats_.rope_style == CodegenTriAttentionRopeStyle::kInterleaved) {
...
float CodegenTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == CodegenTriAttentionScoreAggregation::kMax)
...
float CodegenTriAttentionKvCache::score_one_row(
    const float* row_ptr, const CodegenTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void CodegenTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const CodegenTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void CodegenTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void CodegenTriAttentionKvCache::accumulate_layer_fallback(
...
void CodegenTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == CodegenTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void CodegenTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
CodegenTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void CodegenTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == CodegenTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void CodegenTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> CodegenTriAttentionKvCache::build_keep_indices_per_head(
...
bool CodegenTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, CodegenTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> CodegenTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, CodegenTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void CodegenTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                        int32_t total_tokens) const {
...
void CodegenTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void CodegenTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
CodegenTriAttentionKvCache::GpuLayerResult
CodegenTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = codegen_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == CodegenTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == CodegenTriAttentionScoreAggregation::kMax,
...
bool CodegenTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool CodegenTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, CodegenTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void CodegenTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> CodegenTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool CodegenTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> CodegenTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, CodegenTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void CodegenTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void CodegenTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool CodegenTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool CodegenTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                       std::size_t row_bytes) {
    const bool ok_k = codegen_triattention_compact_rows_gpu(
    ...
    const bool ok_v = codegen_triattention_compact_rows_gpu(
...
bool CodegenTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void CodegenTriAttentionKvCache::compact_layer_on_host(
...
void CodegenTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    CodegenTriAttentionCompactionProfile profile;
    CodegenTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void CodegenTriAttentionKvCache::finalize_repack_profile(
    CodegenTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
CodegenTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
CodegenTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                      int32_t keep_count) const {
...
int32_t CodegenTriAttentionKvCache::count_prefix_positions(
...
void CodegenTriAttentionKvCache::log_compact_debug(
...
void CodegenTriAttentionKvCache::log_compact_profile(
    const CodegenTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool CodegenTriAttentionKvCache::should_emit_keep_dump(
    const CodegenTriAttentionCompactionProfile* profile_ptr) const {
...
void CodegenTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "CodegenTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void CodegenTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void CodegenTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void CodegenTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool CodegenTriAttentionKvCache::ok() const {
