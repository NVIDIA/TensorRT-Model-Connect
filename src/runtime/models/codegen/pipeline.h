...
//
// Composes: TrtModule (decoder) + CodegenKvCache + ITokenizer for this runtime
...
class CodegenKvCache;

struct CodegenTextGenConfig {
...
class CodegenTextGenerationPipeline final : public IPipeline {
  public:
    ...
    CodegenTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                  std::unique_ptr<CodegenInferenceState> state,
                                  CodegenTextGenConfig config, cudaStream_t stream,
                                  std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                  std::string model_id_str = "",
                                  std::unique_ptr<CodegenISampler> sampler = nullptr,
                                  std::shared_ptr<void> distributed_owner = nullptr);
    CodegenTextGenerationPipeline(std::vector<DecoderContext> decoders,
                                  std::unique_ptr<CodegenInferenceState> state,
                                  CodegenTextGenConfig config, cudaStream_t stream,
                                  std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                  std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "CodegenTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<CodegenInferenceState> state_;
    CodegenTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<CodegenISampler> sampler_;
    ...
    CodegenKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(CodegenKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(CodegenISampler* sampler, const CodegenSamplingParams& params,
    ...
    std::unique_ptr<CodegenISampler> make_step_sampler(const CodegenSamplingParams& params);
