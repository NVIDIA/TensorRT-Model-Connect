...
// CodegenISampler: token selection abstraction for autoregressive generation.
//
...
//
// CodegenLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// codegen-owned sampler interface.
class CodegenISampler {
  public:
    virtual ~CodegenISampler() = default;
    ...
    virtual CodegenSampleResult sample(const float* logits, int32_t vocab_size,
                                       const CodegenSamplingParams& params) = 0;
    ...
    virtual CodegenLogitsLocation logits_location() const = 0;
...
/// Build CodegenSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from CodegenSamplingParams.
...
create_codegen_sampler(const CodegenSamplingParams& params,
                       const CodegenSamplerFactoryOptions& options);
