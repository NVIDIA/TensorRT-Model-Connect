...
#include "runtime/models/codegen/inference_state.h"
...
enum class CodegenTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class CodegenTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct CodegenTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    CodegenTriAttentionScoreAggregation score_aggregation{
        CodegenTriAttentionScoreAggregation::kMean};
    CodegenTriAttentionScoreAggregation per_layer_aggregation{
        CodegenTriAttentionScoreAggregation::kMean};
...
struct CodegenTriAttentionHeadStats {
...
struct CodegenTriAttentionStats {
    int32_t head_dim{0};
    CodegenTriAttentionRopeStyle rope_style{CodegenTriAttentionRopeStyle::kHalf};
    ...
    std::vector<CodegenTriAttentionHeadStats> layer_stats;
};

struct CodegenTriAttentionCompactionProfile {
...
// Build a CodegenTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class CodegenTriAttentionKvCache : public CodegenInferenceState {
  public:
    CodegenTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                               int32_t kv_dim, cudaStream_t stream,
                               CodegenTriAttentionConfig config, CodegenTriAttentionStats stats,
                               DType cache_dtype = DType::kFloat32, CodegenKvCacheNames names = {});
  ...
  private:
    ...
    void finalize_repack_profile(CodegenTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const CodegenTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const CodegenTriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        CodegenTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             CodegenTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                CodegenTriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const CodegenTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const CodegenTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const CodegenTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    CodegenKvCacheNames names_;
    CodegenTriAttentionConfig config_;
    CodegenTriAttentionStats stats_;
