...
// CodegenInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   CodegenKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — CodegenKvCache + family-owned recurrent state composed
...
class CodegenInferenceState {
  public:
    virtual ~CodegenInferenceState() = default;
    ...
    // CodegenKvCache -> true. Family-owned recurrent state -> false.
