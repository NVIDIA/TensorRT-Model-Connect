...
// CodegenISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// CodegenTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/codegen/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/codegen/argmax_kernel.h"
#include "runtime/models/codegen/sparse_multinomial_kernel.h"
...
static CodegenSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                              int32_t eos_token_id) {
    CodegenSampleResult result;
...
static bool greedy_equivalent(const CodegenSamplingParams& params) {
...
class GreedySampler final : public CodegenISampler {
  public:
    CodegenSampleResult sample(const float* logits, int32_t vocab_size,
                               const CodegenSamplingParams& params) override {
    ...
    CodegenLogitsLocation logits_location() const override { return CodegenLogitsLocation::HOST; }
...
class TopKSampler final : public CodegenISampler {
  public:
...
class TorchCudaMultinomialSampler final : public CodegenISampler {
  public:
    ...
    CodegenSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        codegen_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const CodegenTorchMultinomialExecutionPolicy policy =
                codegen_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public CodegenISampler {
  public:
    ...
    CodegenSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        codegen_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    CodegenLogitsLocation logits_location() const override { return CodegenLogitsLocation::DEVICE; }
...
CodegenSamplingParams codegen_sampling_params_from_config(const GenerateConfig& cfg,
                                                          int32_t default_eos) {
    CodegenSamplingParams p;
...
create_codegen_sampler(const CodegenSamplingParams& params,
                       [[maybe_unused]] const CodegenSamplerFactoryOptions& options) {
...
std::unique_ptr<CodegenISampler> create_codegen_sampler(const CodegenSamplingParams& params) {
    return create_codegen_sampler(params, CodegenSamplerFactoryOptions{});
}

std::unique_ptr<CodegenISampler> create_codegen_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
