...
StablelmKvCache::StablelmKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim,
                                 cudaStream_t stream, DType cache_dtype, StablelmKvCacheNames names)
...
void StablelmKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t StablelmKvCache::preferred_cache_rows() const {
    if (!dynamic_binding_enabled_)
        return max_length_;
    return round_up_rows(std::max(position_, 1), kRuntimeBucketRows, max_length_);
}

void StablelmKvCache::rebind_cache_rows(int32_t cache_rows) {
...
std::vector<int64_t> StablelmKvCache::mask_shape_for_engine(int32_t mask_width,
...
void StablelmKvCache::write_position_input(TensorMap& inputs, int32_t seq_len) {
...
void StablelmKvCache::write_batched_mask(TensorMap& inputs, int32_t seq_len) {
...
void StablelmKvCache::write_bidirectional_mask(TensorMap& inputs, int32_t seq_len) {
...
void StablelmKvCache::write_decode_mask(TensorMap& inputs) {
    const int32_t valid = std::max(0, std::min(position_, max_length_));
...
void StablelmKvCache::prepare_step(TensorMap& inputs, int32_t seq_len) {
    if (seq_len <= 0)
        seq_len = 1;
...
void StablelmKvCache::write_prefill_kv(const std::vector<const void*>& prefill_k,
                                       const std::vector<const void*>& prefill_v, int32_t seq_len) {
    if (seq_len <= 0)
        return;
    if (seq_len > max_length_)
        throw std::runtime_error("StablelmKvCache::write_prefill_kv: seq_len exceeds max_length");
...
void StablelmKvCache::append_prefill_kv(const std::vector<const void*>& prefill_k,
                                        const std::vector<const void*>& prefill_v,
                                        int32_t seq_len) {
    if (seq_len <= 0)
        return;
    if (position_ + seq_len > max_length_)
        throw std::runtime_error("StablelmKvCache::append_prefill_kv: append exceeds max_length");
    if (static_cast<int32_t>(prefill_k.size()) != num_layers_ ||
        static_cast<int32_t>(prefill_v.size()) != num_layers_) {
        throw std::runtime_error(
...
void StablelmKvCache::set_position(int32_t position) {
    position_ = std::max(0, std::min(position, max_length_));
}

void StablelmKvCache::advance(int32_t n_tokens) {
