...
std::vector<StablelmTextGenerationPipeline::DecoderContext>
single_decoder_context(std::unique_ptr<TrtModule> decoder) {
    std::vector<StablelmTextGenerationPipeline::DecoderContext> decoders;
    decoders.push_back(StablelmTextGenerationPipeline::DecoderContext{0, std::move(decoder)});
    return decoders;
}
...
bool greedy_text_diffusion_params(const StablelmSamplingParams& params) {
...
StablelmTextGenerationPipeline::StablelmTextGenerationPipeline(
    std::unique_ptr<TrtModule> decoder, std::unique_ptr<StablelmInferenceState> state,
    StablelmTextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<StablelmISampler> sampler,
    std::shared_ptr<void> distributed_owner)
    : StablelmTextGenerationPipeline(single_decoder_context(std::move(decoder)), std::move(state),
...
StablelmTextGenerationPipeline::StablelmTextGenerationPipeline(
    std::vector<DecoderContext> decoders, std::unique_ptr<StablelmInferenceState> state,
    StablelmTextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<StablelmISampler> sampler,
    ...
    if (decoders_.empty()) {
        throw std::runtime_error("StablelmTextGenerationPipeline: no decoder modules");
    }
    for (const auto& decoder_ctx : decoders_) {
        if (!decoder_ctx.module || !decoder_ctx.module->ok()) {
            throw std::runtime_error("StablelmTextGenerationPipeline: invalid decoder module");
        }
    }
    if (!state_ || !state_->ok()) {
        throw std::runtime_error("StablelmTextGenerationPipeline: invalid inference state");
    }
...
static std::vector<int32_t> encode_prompt(const ITokenizer& tokenizer,
    ...
    if (cfg.use_chat_template && !config.chat_template_format.empty()) {
        effective =
            stablelm_apply_chat_template(config.chat_template_format, prompt, cfg.enable_thinking);
        templated = true;
    }
...
TextResult StablelmTextGenerationPipeline::generate(const std::string& prompt,
                                                    const GenerateConfig& cfg) {
    if (!tokenizer_) {
        throw std::runtime_error("StablelmTextGenerationPipeline: no tokenizer configured");
    }
...
StablelmTextGenerationPipeline::generate_ids(const std::vector<int32_t>& input_ids,
                                             const GenerateConfig& cfg) {
...
std::unique_ptr<StablelmISampler>
StablelmTextGenerationPipeline::make_step_sampler(const StablelmSamplingParams& params) {
    ...
    if (prefer_gpu_greedy_ && greedy_params) {
        if (auto gpu = create_stablelm_gpu_greedy_sampler(stream_))
            return gpu;
    }
    return create_stablelm_sampler(params);
}
...
namespace {
bool gather_prefill_kv_pointers(TrtModule& prefill, const StablelmTextGenConfig& cfg,
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        pk[li] = prefill.device_ptr(stablelm_expand_layer_name(cfg.present_k_pattern, i));
        pv[li] = prefill.device_ptr(stablelm_expand_layer_name(cfg.present_v_pattern, i));
...
bool batched_prefill_supported(const TrtModule* prefill, const StablelmTextGenConfig& cfg,
...
bool StablelmTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
    ...
    if (!batched_prefill_supported(prefill_.get(), config_, sq, state_.get()))
        return false;
    auto* kv = static_cast<StablelmKvCache*>(state_.get());
...
void StablelmTextGenerationPipeline::prime_decoder_after_batched_prefill(
...
TrtModule& StablelmTextGenerationPipeline::require_block_prefill(int32_t sq,
                                                                 TrtModule* prefill_override) {
    TrtModule* prefill = prefill_override != nullptr ? prefill_override : prefill_.get();
    if (prefill == nullptr)
        throw std::runtime_error(
            "StablelmTextGenerationPipeline: block generation requires prefill module");
    if (sq <= 0)
        throw std::runtime_error("StablelmTextGenerationPipeline: empty block");
    if (config_.prefill_max_length > 0 && sq > config_.prefill_max_length) {
        throw std::runtime_error(
            "StablelmTextGenerationPipeline: block length exceeds prefill profile");
    }
    return *prefill;
}

StablelmKvCache& StablelmTextGenerationPipeline::require_block_kv_cache() {
    auto* kv = dynamic_cast<StablelmKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error(
            "StablelmTextGenerationPipeline: block generation requires StablelmKvCache");
    return *kv;
}

void StablelmTextGenerationPipeline::copy_block_logits(const TensorMap& outputs,
                                                       std::vector<float>& logits) const {
    auto logits_it = outputs.find(config_.logits_output_name);
    if (logits_it == outputs.end())
        throw std::runtime_error("StablelmTextGenerationPipeline: prefill module has no '" +
...
void StablelmTextGenerationPipeline::append_prefill_kv(StablelmKvCache& kv, TrtModule& prefill,
                                                       int32_t sq) {
    std::vector<const void*> pk, pv;
    if (!gather_prefill_kv_pointers(prefill, config_, pk, pv)) {
        throw std::runtime_error("StablelmTextGenerationPipeline: prefill module is missing "
                                 "present_k/present_v outputs");
    }
    kv.append_prefill_kv(pk, pv, sq);
}

void StablelmTextGenerationPipeline::run_prefill_block(const std::vector<int32_t>& input_ids,
                                                       bool bidirectional, bool append_kv,
                                                       std::vector<float>& logits,
                                                       TrtModule* prefill_override) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    TrtModule& prefill = require_block_prefill(sq, prefill_override);
    StablelmKvCache& kv = require_block_kv_cache();
...
std::string
StablelmTextGenerationPipeline::resolve_generation_mode(const GenerateConfig& cfg) const {
...
int32_t StablelmTextGenerationPipeline::resolve_text_diffusion_block_length(
    ...
    if (!config_.supports_text_diffusion || config_.mask_token_id < 0)
        throw std::runtime_error(
            "StablelmTextGenerationPipeline: bundle does not support text diffusion");
    ...
    if (require_divisible && max_new_tokens % block_len != 0) {
        throw std::runtime_error("StablelmTextGenerationPipeline: diffusion mode requires "
                                 "max_new_tokens % block_length == 0");
    }
    return block_len;
}

int32_t StablelmTextGenerationPipeline::seed_next_token_from_prefill(
    ...
    if (static_cast<int32_t>(logits.size()) < vocab)
        throw std::runtime_error("StablelmTextGenerationPipeline: missing prefill logits");
    return argmax_with_confidence(logits.data() + logits.size() - static_cast<std::size_t>(vocab),
                                  vocab, 0)
        .token_id;
}

void StablelmTextGenerationPipeline::fill_diffusion_block(std::vector<int32_t>& block,
                                                          std::vector<float>& logits,
    ...
    for (int32_t step = 0; step < block_len && has_mask_token(block, config_.mask_token_id);
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "StablelmTextGenerationPipeline: diffusion engine must output full block logits");
        }
...
int32_t StablelmTextGenerationPipeline::verify_diffusion_block(const std::vector<int32_t>& block,
                                                               std::vector<float>& logits,
                                                               int32_t block_len, int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "StablelmTextGenerationPipeline: diffusion engine must output full verify logits");
    }
...
bool StablelmTextGenerationPipeline::append_tokens_until_eos(
    const std::vector<int32_t>& tokens, std::vector<int32_t>& output,
    const StablelmSamplingParams& params) const {
...
void StablelmTextGenerationPipeline::fill_linear_spec_block(std::vector<int32_t>& block,
                                                            std::vector<float>& logits,
                                                            int32_t block_len, int32_t vocab,
    ...
    while (has_mask_token(block, config_.mask_token_id)) {
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "StablelmTextGenerationPipeline: linear_spec engine must output full block logits");
        }
        const auto preds = masked_predictions(logits, block, config_.mask_token_id, vocab);
...
std::vector<int32_t>
StablelmTextGenerationPipeline::verify_linear_spec_block(const std::vector<int32_t>& block,
                                                         std::vector<float>& logits,
                                                         int32_t block_len, int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "StablelmTextGenerationPipeline: linear_spec engine must output full verify logits");
    }
...
int32_t
StablelmTextGenerationPipeline::count_linear_spec_accepts(const std::vector<int32_t>& ar_tokens,
                                                          const std::vector<int32_t>& block) {
    if (ar_tokens.empty())
        return 0;
    if (block.size() < 2)
...
bool StablelmTextGenerationPipeline::append_linear_spec_tokens(
    const std::vector<int32_t>& ar_tokens, int32_t emit_count, std::vector<int32_t>& output,
    int32_t& generated, const StablelmSamplingParams& params) const {
...
StablelmTextGenerationPipeline::TimedGenResult StablelmTextGenerationPipeline::generate_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
    const StablelmSamplingParams& params, const GenerateConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "auto" && mode != "ar")
        throw std::runtime_error("StablelmTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");

    StablelmISampler* active_sampler = sampler_.get();
    std::unique_ptr<StablelmISampler> local_sampler;
    if (!active_sampler) {
    ...
    std::vector<float> logits;
    const bool gpu_sampling = (active_sampler->logits_location() == StablelmLogitsLocation::DEVICE);
...
StablelmTextGenerationPipeline::TimedGenResult
StablelmTextGenerationPipeline::generate_diffusion_from_ids(const std::vector<int32_t>& input_ids,
                                                            int32_t max_new_tokens,
                                                            const StablelmSamplingParams& params,
                                                            const GenerateConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error("StablelmTextGenerationPipeline: diffusion mode currently "
...
StablelmTextGenerationPipeline::TimedGenResult
StablelmTextGenerationPipeline::generate_linear_spec_from_ids(const std::vector<int32_t>& input_ids,
                                                              int32_t max_new_tokens,
                                                              const StablelmSamplingParams& params,
                                                              const GenerateConfig& cfg,
                                                              bool use_lora_draft) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error("StablelmTextGenerationPipeline: linear_spec mode currently "
                                 "supports greedy temperature=0 "
                                 "generation");
    }
    if (use_lora_draft && linear_spec_lora_prefill_ == nullptr) {
        throw std::runtime_error("StablelmTextGenerationPipeline: linear_spec_lora mode requires a "
    ...
    if (kv == nullptr)
        throw std::runtime_error(
            "StablelmTextGenerationPipeline: linear_spec requires StablelmKvCache");
...
bool StablelmTextGenerationPipeline::should_stop_on_answer(const std::vector<int32_t>& output,
...
void StablelmTextGenerationPipeline::log_decode_summary(int32_t steps, double ms) const {
...
int32_t StablelmTextGenerationPipeline::run_decode_loop(
    StablelmISampler* sampler, const StablelmSamplingParams& params, std::vector<int32_t>& output,
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        ...
        const StablelmSampleResult result = sampler->sample(sample_ptr, vocab_size, params);
...
int32_t StablelmTextGenerationPipeline::select_decoder_index(int32_t desired_rows) const {
    if (decoders_.size() == 1)
        return 0;
...
TrtModule& StablelmTextGenerationPipeline::bind_decoder_for_step() {
...
void StablelmTextGenerationPipeline::run_step(int32_t token_id, std::vector<float>& logits) {
    TensorMap inputs;
    ...
    if (it == outputs.end()) {
        throw std::runtime_error("StablelmTextGenerationPipeline: no '" + logits_output_name_ +
                                 "' output");
    }
...
void StablelmTextGenerationPipeline::run_step_device(int32_t token_id) {
    TensorMap inputs;
...
int32_t StablelmTextGenerationPipeline::argmax(const std::vector<float>& logits) {
    if (logits.empty())
        return 0;
