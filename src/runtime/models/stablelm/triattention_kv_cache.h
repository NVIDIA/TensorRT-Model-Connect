...
enum class StablelmTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class StablelmTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct StablelmTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    StablelmTriAttentionScoreAggregation score_aggregation{
        StablelmTriAttentionScoreAggregation::kMean};
    StablelmTriAttentionScoreAggregation per_layer_aggregation{
        StablelmTriAttentionScoreAggregation::kMean};
...
struct StablelmTriAttentionHeadStats {
...
struct StablelmTriAttentionStats {
    int32_t head_dim{0};
    StablelmTriAttentionRopeStyle rope_style{StablelmTriAttentionRopeStyle::kHalf};
    ...
    std::vector<StablelmTriAttentionHeadStats> layer_stats;
};

struct StablelmTriAttentionCompactionProfile {
...
// Build a StablelmTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class StablelmTriAttentionKvCache : public StablelmInferenceState {
  public:
    StablelmTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                                int32_t kv_dim, cudaStream_t stream,
                                StablelmTriAttentionConfig config, StablelmTriAttentionStats stats,
  ...
  private:
    ...
    void finalize_repack_profile(StablelmTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const StablelmTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const StablelmTriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        StablelmTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             StablelmTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const StablelmTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const StablelmTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const StablelmTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    StablelmKvCacheNames names_;
    StablelmTriAttentionConfig config_;
    StablelmTriAttentionStats stats_;
