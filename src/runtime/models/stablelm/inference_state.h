...
// StablelmInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   StablelmKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — StablelmKvCache + family-owned recurrent state composed
...
class StablelmInferenceState {
  public:
    virtual ~StablelmInferenceState() = default;
    ...
    // StablelmKvCache -> true. Family-owned recurrent state -> false.
