...
//
// Composes: TrtModule (decoder) + StablelmKvCache + ITokenizer for this runtime
...
class StablelmKvCache;

struct StablelmTextGenConfig {
...
class StablelmTextGenerationPipeline final : public IPipeline {
  public:
    ...
    StablelmTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                   std::unique_ptr<StablelmInferenceState> state,
                                   StablelmTextGenConfig config, cudaStream_t stream,
                                   std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                   std::string model_id_str = "",
                                   std::unique_ptr<StablelmISampler> sampler = nullptr,
                                   std::shared_ptr<void> distributed_owner = nullptr);
    StablelmTextGenerationPipeline(std::vector<DecoderContext> decoders,
                                   std::unique_ptr<StablelmInferenceState> state,
                                   StablelmTextGenConfig config, cudaStream_t stream,
                                   std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                   std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "StablelmTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<StablelmInferenceState> state_;
    StablelmTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<StablelmISampler> sampler_;
    ...
    StablelmKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(StablelmKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(StablelmISampler* sampler, const StablelmSamplingParams& params,
    ...
    std::unique_ptr<StablelmISampler> make_step_sampler(const StablelmSamplingParams& params);
