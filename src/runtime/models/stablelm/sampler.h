...
// StablelmISampler: token selection abstraction for autoregressive generation.
//
...
//
// StablelmLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// stablelm-owned sampler interface.
class StablelmISampler {
  public:
    virtual ~StablelmISampler() = default;
    ...
    virtual StablelmSampleResult sample(const float* logits, int32_t vocab_size,
                                        const StablelmSamplingParams& params) = 0;
    ...
    virtual StablelmLogitsLocation logits_location() const = 0;
...
/// Build StablelmSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from StablelmSamplingParams.
...
create_stablelm_sampler(const StablelmSamplingParams& params,
                        const StablelmSamplerFactoryOptions& options);
