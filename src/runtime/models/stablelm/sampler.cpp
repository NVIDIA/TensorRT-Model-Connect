...
// StablelmISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// StablelmTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/stablelm/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/stablelm/argmax_kernel.h"
#include "runtime/models/stablelm/sparse_multinomial_kernel.h"
...
static StablelmSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                               int32_t eos_token_id) {
    StablelmSampleResult result;
...
static bool greedy_equivalent(const StablelmSamplingParams& params) {
...
class GreedySampler final : public StablelmISampler {
  public:
    StablelmSampleResult sample(const float* logits, int32_t vocab_size,
                                const StablelmSamplingParams& params) override {
    ...
    StablelmLogitsLocation logits_location() const override { return StablelmLogitsLocation::HOST; }
...
class TopKSampler final : public StablelmISampler {
  public:
...
class TorchCudaMultinomialSampler final : public StablelmISampler {
  public:
    ...
    StablelmSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        stablelm_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const StablelmTorchMultinomialExecutionPolicy policy =
                stablelm_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public StablelmISampler {
  public:
    ...
    StablelmSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        stablelm_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    StablelmLogitsLocation logits_location() const override {
        return StablelmLogitsLocation::DEVICE;
    }
...
StablelmSamplingParams stablelm_sampling_params_from_config(const GenerateConfig& cfg,
                                                            int32_t default_eos) {
    StablelmSamplingParams p;
...
create_stablelm_sampler(const StablelmSamplingParams& params,
                        [[maybe_unused]] const StablelmSamplerFactoryOptions& options) {
...
std::unique_ptr<StablelmISampler> create_stablelm_sampler(const StablelmSamplingParams& params) {
    return create_stablelm_sampler(params, StablelmSamplerFactoryOptions{});
}

std::unique_ptr<StablelmISampler> create_stablelm_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
