...
#include "runtime/models/stablelm/triattention_kernels.h"
#endif
...
StablelmTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return StablelmTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return StablelmTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(StablelmTriAttentionScoreAggregation value) {
    switch (value) {
    case StablelmTriAttentionScoreAggregation::kMean:
        return "mean";
    case StablelmTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

StablelmTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return StablelmTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return StablelmTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, StablelmKvCacheNames& names) {
...
// override helpers that patched StablelmTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     StablelmTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(StablelmTriAttentionConfig& cfg,
...
void fill_debug_from_registry(StablelmTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(StablelmTriAttentionConfig& cfg,
...
static void validate_triattention_config(const StablelmTriAttentionConfig& cfg,
...
StablelmTriAttentionConfig
stablelm_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                          const ::trtmc::config::ConfigBundle* runtime_config) {
    StablelmTriAttentionConfig cfg;
...
void fill_stats_header(StablelmTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(StablelmTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(StablelmTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(StablelmTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(StablelmTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(StablelmTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(StablelmTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(StablelmTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(StablelmTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(StablelmTriAttentionStats& stats,
...
void parse_sparse_stats(StablelmTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
StablelmTriAttentionStats stablelm_parse_triattention_stats_json(const std::string& stats_json,
                                                                 int32_t num_attention_heads,
                                                                 int32_t num_key_value_heads,
                                                                 int32_t num_layers) {
    StablelmTriAttentionStats stats;
...
void StablelmTriAttentionKvCache::validate_shapes() {
...
void StablelmTriAttentionKvCache::normalize_sampled_heads() {
...
void StablelmTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void StablelmTriAttentionKvCache::allocate_layer_tensors() {
...
StablelmTriAttentionKvCache::StablelmTriAttentionKvCache(
    int32_t num_layers, int32_t num_kv_heads, int32_t max_length, int32_t kv_dim,
    cudaStream_t stream, StablelmTriAttentionConfig config, StablelmTriAttentionStats stats,
    DType cache_dtype, StablelmKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void StablelmTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void StablelmTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void StablelmTriAttentionKvCache::initialize_gpu_state() {
...
bool StablelmTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool StablelmTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool StablelmTriAttentionKvCache::can_use_gpu_selection() const {
...
void StablelmTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t StablelmTriAttentionKvCache::preferred_cache_rows() const {
...
void StablelmTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void StablelmTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> StablelmTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows,
    StablelmTriAttentionCompactionProfile* profile) const {
...
void StablelmTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t StablelmTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> StablelmTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
StablelmTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
StablelmTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                                 StablelmTriAttentionCompactionProfile* profile) {
...
int32_t StablelmTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t StablelmTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> StablelmTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void StablelmTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, StablelmTriAttentionCompactionProfile* profile) const {
...
bool StablelmTriAttentionKvCache::layer_stats_shapes_valid(
    const StablelmTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void StablelmTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset,
                                                int32_t d, int32_t half_dim, float& k_rot_real,
                                                float& k_rot_imag) const {
    if (stats_.rope_style == StablelmTriAttentionRopeStyle::kInterleaved) {
...
float StablelmTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == StablelmTriAttentionScoreAggregation::kMax)
...
float StablelmTriAttentionKvCache::score_one_row(
    const float* row_ptr, const StablelmTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void StablelmTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const StablelmTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void StablelmTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void StablelmTriAttentionKvCache::accumulate_layer_fallback(
...
void StablelmTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max =
        config_.per_layer_aggregation == StablelmTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void StablelmTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
StablelmTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void StablelmTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
...
void StablelmTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> StablelmTriAttentionKvCache::build_keep_indices_per_head(
...
bool StablelmTriAttentionKvCache::process_layer_for_host_selection(
...
std::vector<int32_t> StablelmTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, StablelmTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void StablelmTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                         int32_t total_tokens) const {
...
void StablelmTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void StablelmTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
StablelmTriAttentionKvCache::GpuLayerResult
StablelmTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = stablelm_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == StablelmTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == StablelmTriAttentionScoreAggregation::kMax,
...
bool StablelmTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool StablelmTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, StablelmTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void StablelmTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> StablelmTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool StablelmTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> StablelmTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, StablelmTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void StablelmTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void StablelmTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool StablelmTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool StablelmTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                        std::size_t row_bytes) {
    const bool ok_k = stablelm_triattention_compact_rows_gpu(
    ...
    const bool ok_v = stablelm_triattention_compact_rows_gpu(
...
bool StablelmTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void StablelmTriAttentionKvCache::compact_layer_on_host(
...
void StablelmTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    StablelmTriAttentionCompactionProfile profile;
    StablelmTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void StablelmTriAttentionKvCache::finalize_repack_profile(
    StablelmTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
StablelmTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
StablelmTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                       int32_t keep_count) const {
...
int32_t StablelmTriAttentionKvCache::count_prefix_positions(
...
void StablelmTriAttentionKvCache::log_compact_debug(
...
void StablelmTriAttentionKvCache::log_compact_profile(
    const StablelmTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool StablelmTriAttentionKvCache::should_emit_keep_dump(
    const StablelmTriAttentionCompactionProfile* profile_ptr) const {
...
void StablelmTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "StablelmTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void StablelmTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void StablelmTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void StablelmTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool StablelmTriAttentionKvCache::ok() const {
