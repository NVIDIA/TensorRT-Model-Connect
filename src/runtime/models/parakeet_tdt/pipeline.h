...
// TdtPipeline: native C++ greedy TDT speech-to-text pipeline.
...
class TdtPipeline final : public IPipeline {
  public:
    TdtPipeline(std::unique_ptr<TrtModule> encoder, std::unique_ptr<TrtModule> predictor,
    ...
    const char* pipeline_type() const override { return "TdtPipeline"; }

  private:
    friend class TdtTranscriptionStream;
