...
using TdtClock = std::chrono::steady_clock;

struct TdtStageTotals {
...
bool tdt_stage_timing_enabled() {
    const char* value = std::getenv("TRTMC_TDT_STAGE_TIMING");
    return value != nullptr && value[0] != '\0' && std::strcmp(value, "0") != 0;
}

double elapsed_ms(TdtClock::time_point start, TdtClock::time_point end) {
    return std::chrono::duration<double, std::milli>(end - start).count();
}

void report_tdt_stage_timing(bool enabled, const char* mode, const TdtStageTotals& stages,
    ...
    timing << "[trtmc.tdt_timing.json] {\"mode\":\"" << mode
...
bool tdt_stream_debug_enabled() {
    const char* v = std::getenv("TRTMC_TDT_STREAM_DEBUG");
    return v && std::string(v) != "0";
}

tdt::MelSpectrogramOptions make_tdt_mel_options(const TdtConfig& config) {
    tdt::MelSpectrogramOptions options;
    ...
    options.log_scale = tdt::MelLogScale::kNaturalLog;
    options.normalize_per_feature = true;
    return options;
}

void validate_tdt_module(const std::unique_ptr<TrtModule>& module, const char* name) {
    if (!module || !module->ok())
        throw std::runtime_error(std::string("TdtPipeline: invalid ") + name + " module");
}

void validate_tdt_core_config(const TdtConfig& config, const MelFilterbank& mel_fb) {
    if (mel_fb.data.empty())
        throw std::runtime_error("TdtPipeline: missing mel filterbank bundle section");
    ...
    if (config.blank_id < 0)
        throw std::runtime_error("TdtPipeline: invalid blank token id");
}
...
void validate_tdt_streaming_config(const TdtConfig& config, bool has_streaming_sections) {
    if (!has_streaming_sections)
        return;
    if (config.encoder_layers <= 0 || config.streaming_cache_left <= 0 ||
        config.streaming_time_cache <= 0)
        throw std::runtime_error("TdtPipeline: invalid TDT streaming dimensions in config");
}

void print_vector_stats(const char* label, const std::vector<float>& values) {
    if (values.empty()) {
        std::cerr << "[tdt.stream] " << label << " empty\n";
        return;
    }
    ...
    std::cerr << "[tdt.stream] " << label << " count=" << values.size() << " min=" << *min_it
              << " max=" << *max_it
...
class TdtTranscriptionStream final : public ITranscriptionStream {
  public:
    TdtTranscriptionStream(TdtPipeline& pipeline, TranscriptionStreamConfig cfg)
        : pipeline_(pipeline), cfg_(cfg),
          schedule_(make_tdt_streaming_schedule(
          ...
          transcribe_start_(TdtClock::now()),
          feature_state_(pipeline_.mel_fb_->data.data(), pipeline_.mel_fb_->n_freq_bins,
                         pipeline_.mel_fb_->n_mel_bins, make_tdt_mel_options(pipeline_.config_),
        ...
        const auto decode_start = TdtClock::now();
        pred_output_ = pipeline_.run_predictor(pipeline_.config_.blank_id, state_h_, state_c_);
        stages_.predictor_joint_and_transfer_ms += elapsed_ms(decode_start, TdtClock::now());
        reset_encoder_cache();
    }
    ...
    TranscriptionStreamResult accept_audio(const float* audio_samples, int32_t num_samples,
        ...
        if (num_samples < 0)
            throw std::runtime_error("TDT streaming transcription received negative sample count");
        if (num_samples > 0 && audio_samples == nullptr)
            throw std::runtime_error("TDT streaming transcription received null audio");
        if (finished_)
            throw std::runtime_error("TDT streaming transcription stream is already finished");

        if (num_samples > 0) {
            const auto frontend_start = TdtClock::now();
            feature_state_.accept_audio(audio_samples, num_samples);
            stages_.frontend_ms += elapsed_ms(frontend_start, TdtClock::now());
    ...
    TranscriptionStreamResult finish() override {
        ...
        report_tdt_stage_timing(
            tdt_stage_timing_enabled(), "streaming", stages_,
            elapsed_ms(transcribe_start_, TdtClock::now()), feature_stats.accepted_source_samples,
  ...
  private:
    ...
    TranscriptionStreamResult make_result(bool is_final) {
        const auto text_start = TdtClock::now();
        ...
        stages_.text_ms += elapsed_ms(text_start, TdtClock::now());
        return out;
    }
    ...
    void trace_streaming_chunk(const std::vector<float>& enc, std::size_t before_tokens,
        ...
        std::cerr << "[tdt.stream] chunk=" << chunk_index_ << " mel_start=" << next_mel_start_
    ...
    bool process_next_chunk(int32_t actual_mel_frames, bool final) {
        ...
        const auto encoder_start = TdtClock::now();
    ...
    TdtPipeline& pipeline_;
    TranscriptionStreamConfig cfg_;
    TdtStreamingSchedule schedule_;
    TdtClock::time_point transcribe_start_;
    tdt::IncrementalMelSpectrogram feature_state_;
    TdtStageTotals stages_;
...
TdtPipeline::TdtPipeline(std::unique_ptr<TrtModule> encoder, std::unique_ptr<TrtModule> predictor,
                         std::unique_ptr<TrtModule> joint, std::unique_ptr<TrtModule> prompt_kernel,
                         ...
                         std::string bundle_path, TdtConfig config, MelFilterbank mel_fb,
    ...
    module_options_.runtime_cache_path = runtime_cache_path_.c_str();
    validate_tdt_module(encoder_, "encoder");
    validate_tdt_module(predictor_, "predictor");
    validate_tdt_module(joint_, "joint");
    validate_tdt_core_config(config_, *mel_fb_);
    validate_tdt_streaming_config(
    ...
    if (config_.has_prompt_kernel) {
        if (!prompt_kernel_ || !prompt_kernel_->ok())
            throw std::runtime_error(
                "TdtPipeline: prompt_kernel module required when has_prompt_kernel=true");
        if (config_.num_prompts <= 0)
            throw std::runtime_error("TdtPipeline: invalid num_prompts for prompt_kernel variant");
    }
}

int32_t TdtPipeline::resolve_prompt_index(const std::string& tag) const {
    if (!config_.has_prompt_kernel)
        return 0;
    if (tag.empty())
        return 0;
...
std::vector<float> TdtPipeline::run_prompt_kernel(const float* encoder_frame) {
    if (!prompt_kernel_)
        throw std::runtime_error("TdtPipeline: run_prompt_kernel called without engine");
    ...
    if (it == outputs.end())
        throw std::runtime_error("TdtPipeline: prompt_kernel missing 'prompt_kernel_output'");
    if (it->second.numel() != config_.encoder_hidden_size)
        throw std::runtime_error("TdtPipeline: prompt_kernel output size does not match config");
...
TdtPipeline::~TdtPipeline() = default;

void TdtPipeline::setup_prompt_state(const std::string& language) {
    if (!config_.has_prompt_kernel) {
...
TdtPipeline::create_transcription_stream(const TranscriptionStreamConfig& cfg) {
    (void)cfg;
    throw std::runtime_error(
        "Parakeet TDT v3 only provides an offline encoder; streaming transcription is unsupported");
}

TextResult TdtPipeline::transcribe(const float* audio_data, int32_t num_samples,
                                   int32_t max_new_tokens, int32_t input_sample_rate) {
    const bool report_timing = tdt_stage_timing_enabled();
    const auto transcribe_start = TdtClock::now();
    TdtStageTotals stages;

    int32_t actual_mel_frames = 0;
    auto mel = extract_padded_mel(audio_data, num_samples, input_sample_rate, actual_mel_frames);
    const auto frontend_end = TdtClock::now();
    ...
    const auto encoder_end = TdtClock::now();
    ...
    decode_encoder_frames(encoder_output, encoder_frames, token_limit, pred_output, state_h,
                          state_c, emitted);
    const auto decode_end = TdtClock::now();
    ...
    const auto text_end = TdtClock::now();
    ...
    const int64_t resampled_samples =
        input_sample_rate > 0 && input_sample_rate != config_.sample_rate
            ...
            : num_samples;
    report_tdt_stage_timing(report_timing, "offline", stages,
...
TrtModule& TdtPipeline::streaming_encoder_for(int32_t right_context, bool first_step) {
    ...
    if (section_it == sections.end() || section_it->second.empty())
        throw std::runtime_error("TdtPipeline: missing streaming encoder plan for requested "
                                 "att_context_size");
    if (bundle_path_.empty())
        throw std::runtime_error(
            "TdtPipeline: missing bundle path for lazy streaming encoder load");

    const std::string label = std::string("tdt streaming encoder ") +
...
std::vector<float> TdtPipeline::run_streaming_encoder(
    ...
    const auto schedule =
        make_tdt_streaming_schedule(config_.att_context_left, right_context, config_.sample_rate,
...
std::vector<float> TdtPipeline::run_predictor(int32_t token_id, std::vector<float>& state_h,
    ...
    if (pred_it->second.numel() != config_.pred_hidden_size)
