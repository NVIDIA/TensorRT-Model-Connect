...
// AlbertKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/albert/inference_state.h"
...
class AlbertKvCache : public AlbertInferenceState {
  public:
    ...
    AlbertKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, AlbertKvCacheNames names = {});

    // --- AlbertInferenceState overrides ---
    void reset() override;
    ...
    // --- AlbertKvCache-specific methods (not on the interface) ---
    ...
    // — too big for AlbertKvCache's single-row present buffer. The caller reads
