...
// AlbertInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   AlbertKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — AlbertKvCache + family-owned recurrent state composed
...
class AlbertInferenceState {
  public:
    virtual ~AlbertInferenceState() = default;
    ...
    // AlbertKvCache -> true. Family-owned recurrent state -> false.
