...
// LtxVideoInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   LtxVideoKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — LtxVideoKvCache + family-owned recurrent state composed
...
class LtxVideoInferenceState {
  public:
    virtual ~LtxVideoInferenceState() = default;
    ...
    // LtxVideoKvCache -> true. Family-owned recurrent state -> false.
