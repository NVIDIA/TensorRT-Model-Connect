...
// LtxVideoKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/ltx_video/inference_state.h"
...
class LtxVideoKvCache : public LtxVideoInferenceState {
  public:
    ...
    LtxVideoKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                    DType cache_dtype = DType::kFloat32, LtxVideoKvCacheNames names = {});

    // --- LtxVideoInferenceState overrides ---
    void reset() override;
    ...
    // --- LtxVideoKvCache-specific methods (not on the interface) ---
    ...
    // — too big for LtxVideoKvCache's single-row present buffer. The caller reads
