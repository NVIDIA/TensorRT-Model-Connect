...
// PatchtstInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   PatchtstKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — PatchtstKvCache + family-owned recurrent state composed
...
class PatchtstInferenceState {
  public:
    virtual ~PatchtstInferenceState() = default;
    ...
    // PatchtstKvCache -> true. Family-owned recurrent state -> false.
