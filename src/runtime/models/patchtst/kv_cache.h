...
// PatchtstKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/patchtst/inference_state.h"
...
class PatchtstKvCache : public PatchtstInferenceState {
  public:
    ...
    PatchtstKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                    DType cache_dtype = DType::kFloat32, PatchtstKvCacheNames names = {});

    // --- PatchtstInferenceState overrides ---
    void reset() override;
    ...
    // --- PatchtstKvCache-specific methods (not on the interface) ---
    ...
    // — too big for PatchtstKvCache's single-row present buffer. The caller reads
