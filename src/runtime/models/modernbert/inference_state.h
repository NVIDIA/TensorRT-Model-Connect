...
// ModernbertInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   ModernbertKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — ModernbertKvCache + family-owned recurrent state composed
...
class ModernbertInferenceState {
  public:
    virtual ~ModernbertInferenceState() = default;
    ...
    // ModernbertKvCache -> true. Family-owned recurrent state -> false.
