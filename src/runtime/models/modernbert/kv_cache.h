...
// ModernbertKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/modernbert/inference_state.h"
...
class ModernbertKvCache : public ModernbertInferenceState {
  public:
    ...
    ModernbertKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                      DType cache_dtype = DType::kFloat32, ModernbertKvCacheNames names = {});

    // --- ModernbertInferenceState overrides ---
    void reset() override;
    ...
    // --- ModernbertKvCache-specific methods (not on the interface) ---
    ...
    // — too big for ModernbertKvCache's single-row present buffer. The caller reads
