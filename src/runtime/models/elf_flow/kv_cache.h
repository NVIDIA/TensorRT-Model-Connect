...
// ElfFlowKvCache: autoregressive KV cache state manager.
...
class ElfFlowKvCache : public ElfFlowInferenceState {
  public:
    ...
    ElfFlowKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                   DType cache_dtype = DType::kFloat32, ElfFlowKvCacheNames names = {});

    // --- ElfFlowInferenceState overrides ---
    void reset() override;
    ...
    // --- ElfFlowKvCache-specific methods (not on the interface) ---
    ...
    // — too big for ElfFlowKvCache's single-row present buffer. The caller reads
