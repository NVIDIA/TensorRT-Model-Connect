...
// ElfFlowInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   ElfFlowKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — ElfFlowKvCache + family-owned recurrent state composed
...
class ElfFlowInferenceState {
  public:
    virtual ~ElfFlowInferenceState() = default;
    ...
    // ElfFlowKvCache -> true. Family-owned recurrent state -> false.
