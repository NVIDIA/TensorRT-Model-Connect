...
// TimmInceptionResnetPlugin: handles timm Inception-ResNet image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_inception_resnet/pipeline.h"
...
TimmInceptionResnetPreprocessConfig
make_timm_inception_resnet_preprocess_config(const std::string& json) {
    TimmInceptionResnetPreprocessConfig cfg;
...
class TimmInceptionResnetPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmInceptionResnetImageClassificationPipeline>(
            std::move(loaded.module), make_timm_inception_resnet_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_inception_resnet_plugin,
                                       TimmInceptionResnetPlugin,
