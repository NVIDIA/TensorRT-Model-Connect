...
// XlnetKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/xlnet/inference_state.h"
...
class XlnetKvCache : public XlnetInferenceState {
  public:
    ...
    XlnetKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                 DType cache_dtype = DType::kFloat32, XlnetKvCacheNames names = {});

    // --- XlnetInferenceState overrides ---
    void reset() override;
    ...
    // --- XlnetKvCache-specific methods (not on the interface) ---
    ...
    // — too big for XlnetKvCache's single-row present buffer. The caller reads
