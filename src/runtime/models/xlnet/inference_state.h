...
// XlnetInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   XlnetKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — XlnetKvCache + family-owned recurrent state composed
...
class XlnetInferenceState {
  public:
    virtual ~XlnetInferenceState() = default;
    ...
    // XlnetKvCache -> true. Family-owned recurrent state -> false.
