...
__global__ void normalizationQualificationKernel(const std::uint8_t* values,
                                                 float* normalized_values,
                                                 const float* normalization_table) {
    const auto linear = static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (linear >= kSam2Rgb8NormalizationTableElements)
        return;
    const auto channel = linear / kSam2Rgb8ValueCount;
    normalized_values[linear] = normalization_table[channel * kSam2Rgb8ValueCount + values[linear]];
}
...
cudaError_t enqueueSam2Rgb8NormalizationQualification(const std::uint8_t* values,
    ...
    if (values == nullptr || normalized_values == nullptr || normalization_table == nullptr ||
        stream == nullptr) {
        return cudaErrorInvalidValue;
    }
    normalizationQualificationKernel<<<blockCount(kSam2Rgb8NormalizationTableElements), kThreads,
                                       0U, stream>>>(values, normalized_values,
                                                     normalization_table);
