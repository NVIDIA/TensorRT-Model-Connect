...
class PinnedAllocation final {
  public:
    explicit PinnedAllocation(std::size_t bytes) : bytes_(bytes) {
        if (bytes_ == 0U)
            throw std::invalid_argument("SAM2 pinned allocation size must be positive");
        checkCuda(cudaHostAlloc(&data_, bytes_, cudaHostAllocDefault), "pinned host allocation");
    }
...
class Event final {
  public:
    explicit Event(std::int32_t device) : device_(device) {
        ScopedDevice selected(device_);
        checkCuda(cudaEventCreateWithFlags(&event_, cudaEventDisableTiming), "CUDA event creation");
    }
...
bool sameOwner(const std::shared_ptr<const void>& left,
    ...
    return left != nullptr && right != nullptr && !left.owner_before(right) &&
           !right.owner_before(left);
}

using CuMemGetAddressRange = CUresult(CUDAAPI*)(CUdeviceptr*, std::size_t*, CUdeviceptr);

CuMemGetAddressRange loadCuMemGetAddressRange() noexcept {
    static const CuMemGetAddressRange function = [] {
        void* const driver = dlopen("libcuda.so.1", RTLD_NOW | RTLD_LOCAL);
        if (driver == nullptr)
            return static_cast<CuMemGetAddressRange>(nullptr);
        void* symbol = dlsym(driver, "cuMemGetAddressRange_v2");
        if (symbol == nullptr)
            symbol = dlsym(driver, "cuMemGetAddressRange");
        CuMemGetAddressRange result = nullptr;
...
struct WorkspaceRunOwner final {};
...
struct Sam2DeviceWorkspace::Impl final {
    explicit Impl(cudaStream_t requested_stream)
        : stream(requireStream(requested_stream)), device(queryStreamDevice(stream)),
          ...
          status_staging(sizeof(std::uint32_t)), bbox_ready(device), tracker_ready(device),
          bbox_offsets(bboxOffsets()) {
    ...
    std::shared_ptr<WorkspaceRunOwner> active_run_owner;
};

std::shared_ptr<Sam2DeviceWorkspace> Sam2DeviceWorkspace::create(cudaStream_t stream) {
    return std::shared_ptr<Sam2DeviceWorkspace>(new Sam2DeviceWorkspace(stream));
}
...
cudaStream_t Sam2DeviceWorkspace::stream() const noexcept {
...
void Sam2DeviceWorkspace::beginRun() {
    ScopedDevice selected(implementation_->device);
    implementation_->active_run_owner = std::make_shared<WorkspaceRunOwner>();
...
void Sam2DeviceWorkspace::enqueueBboxDownload(const ITrtModule& image) {
    ...
    checkCuda(cudaEventRecord(implementation_->bbox_ready.get(), implementation_->stream),
              "bbox completion event record");
}

Sam2BBoxRawOutputs Sam2DeviceWorkspace::waitForBbox() {
    ScopedDevice selected(implementation_->device);
    checkCuda(cudaEventSynchronize(implementation_->bbox_ready.get()),
              "bbox completion event wait");
    ...
    for (std::size_t index = 0; index < views.size(); ++index) {
        views[index] = {staging + implementation_->bbox_offsets[index], Sam2BBoxDataType::kBFloat16,
                        bboxShape(kBboxMaps[index]),
                        tensorBytes(kBboxMaps[index]) / sizeof(std::uint16_t)};
    }
...
void Sam2DeviceWorkspace::finishTrackerStage(const char* stage) {
    ...
    checkCuda(cudaEventRecord(implementation_->tracker_ready.get(), implementation_->stream),
              label + " completion event record");
    checkCuda(cudaEventSynchronize(implementation_->tracker_ready.get()),
...
void Sam2DeviceWorkspace::drain() {
    ScopedDevice selected(implementation_->device);
    checkCuda(cudaStreamSynchronize(implementation_->stream), "device stream drain");
}
...
void Sam2DeviceWorkspace::invalidateRun() noexcept {
    implementation_->active_run_owner.reset();
}

bool Sam2DeviceWorkspace::isDeviceSpan(const void* pointer, std::size_t bytes) const noexcept {
    ...
    try {
        ...
        if (attributes.type != cudaMemoryTypeDevice ||
            attributes.device != implementation_->device) {
            return false;
        }

        const auto get_address_range = loadCuMemGetAddressRange();
        if (get_address_range == nullptr)
            return false;
        const auto address = reinterpret_cast<CUdeviceptr>(pointer);
        CUdeviceptr allocation_base = 0U;
        std::size_t allocation_bytes = 0U;
        if (get_address_range(&allocation_base, &allocation_bytes, address) != CUDA_SUCCESS ||
            address < allocation_base) {
            return false;
        }
        const auto offset = static_cast<std::size_t>(address - allocation_base);
...
const void* Sam2DeviceWorkspace::maskPointer(std::size_t frame_index) const noexcept {
    if (frame_index >= static_cast<std::size_t>(kFrameCount))
        return nullptr;
...
Sam2VideoMaskBuffer Sam2DeviceWorkspace::makeMaskBuffer(std::size_t frame_index) {
    const void* pointer = maskPointer(frame_index);
    if (pointer == nullptr)
        throw std::out_of_range("SAM2 mask frame index is out of range");
    auto run_owner = implementation_->active_run_owner;
    if (run_owner == nullptr)
        throw std::logic_error("SAM2 device mask requires an active run provenance");
    auto workspace = shared_from_this();
    std::shared_ptr<const void> provenance = run_owner;
    std::shared_ptr<const void> owner(run_owner, pointer);
    return Sam2VideoMaskBuffer::cuda_device_binary(
        pointer, kMaskBytes, implementation_->device, std::move(owner),
        [workspace = std::move(workspace), run_owner = std::move(run_owner),
         provenance = std::move(provenance), frame_index] {
            (void)run_owner;
            return workspace->materializeMask(frame_index, provenance);
        });
}

bool Sam2DeviceWorkspace::ownsMask(const Sam2VideoMaskBuffer& mask,
                                   std::size_t frame_index) const noexcept {
    const auto& run_owner = implementation_->active_run_owner;
    std::shared_ptr<const void> provenance = run_owner;
    return mask.memory_kind() == TRTMC_SAM2_VIDEO_MASK_MEMORY_CUDA_DEVICE &&
           mask.data() == maskPointer(frame_index) && mask.byte_count() == kMaskBytes &&
           mask.device_ordinal() == implementation_->device && mask.binary_by_construction() &&
           mask.has_owner() && mask.can_materialize_host() && mask.shares_device_owner(provenance);
}

std::vector<std::uint8_t>
Sam2DeviceWorkspace::materializeMask(std::size_t frame_index,
                                     const std::shared_ptr<const void>& provenance) const {
    const auto& active_run_owner = implementation_->active_run_owner;
    std::shared_ptr<const void> active_provenance = active_run_owner;
    if (!sameOwner(active_provenance, provenance))
        throw std::logic_error("SAM2 device mask provenance is no longer active");
