...
// TimmDensenetPlugin: handles timm DenseNet image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_densenet/pipeline.h"
...
TimmDensenetPreprocessConfig make_timm_densenet_preprocess_config(const std::string& json) {
    TimmDensenetPreprocessConfig cfg;
...
class TimmDensenetPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmDensenetImageClassificationPipeline>(
            std::move(loaded.module), make_timm_densenet_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_densenet_plugin, TimmDensenetPlugin,
