...
//
// Composes: TrtModule (decoder) + Starcoder2KvCache + ITokenizer for this runtime
...
class Starcoder2KvCache;

struct Starcoder2TextGenConfig {
...
class Starcoder2TextGenerationPipeline final : public IPipeline {
  public:
    ...
    Starcoder2TextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                     std::unique_ptr<Starcoder2InferenceState> state,
                                     Starcoder2TextGenConfig config, cudaStream_t stream,
                                     std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                     std::string model_id_str = "",
                                     std::unique_ptr<Starcoder2ISampler> sampler = nullptr,
                                     std::shared_ptr<void> distributed_owner = nullptr);
    Starcoder2TextGenerationPipeline(std::vector<DecoderContext> decoders,
                                     std::unique_ptr<Starcoder2InferenceState> state,
                                     Starcoder2TextGenConfig config, cudaStream_t stream,
                                     std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                     std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "Starcoder2TextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<Starcoder2InferenceState> state_;
    Starcoder2TextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<Starcoder2ISampler> sampler_;
    ...
    Starcoder2KvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(Starcoder2KvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(Starcoder2ISampler* sampler, const Starcoder2SamplingParams& params,
    ...
    std::unique_ptr<Starcoder2ISampler> make_step_sampler(const Starcoder2SamplingParams& params);
