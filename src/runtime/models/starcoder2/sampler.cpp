// Starcoder2ISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// Starcoder2TextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/starcoder2/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/starcoder2/argmax_kernel.h"
#include "runtime/models/starcoder2/sparse_multinomial_kernel.h"
...
static Starcoder2SampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                                 int32_t eos_token_id) {
    Starcoder2SampleResult result;
...
static bool greedy_equivalent(const Starcoder2SamplingParams& params) {
...
class GreedySampler final : public Starcoder2ISampler {
  public:
    Starcoder2SampleResult sample(const float* logits, int32_t vocab_size,
                                  const Starcoder2SamplingParams& params) override {
    ...
    Starcoder2LogitsLocation logits_location() const override {
        return Starcoder2LogitsLocation::HOST;
    }
...
class TopKSampler final : public Starcoder2ISampler {
  public:
...
class TorchCudaMultinomialSampler final : public Starcoder2ISampler {
  public:
    ...
    Starcoder2SampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        starcoder2_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const Starcoder2TorchMultinomialExecutionPolicy policy =
                starcoder2_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public Starcoder2ISampler {
  public:
    ...
    Starcoder2SampleResult sample(const float* logits, int32_t vocab_size,
        ...
        starcoder2_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    Starcoder2LogitsLocation logits_location() const override {
        return Starcoder2LogitsLocation::DEVICE;
    }
...
Starcoder2SamplingParams starcoder2_sampling_params_from_config(const GenerateConfig& cfg,
                                                                int32_t default_eos) {
    Starcoder2SamplingParams p;
...
create_starcoder2_sampler(const Starcoder2SamplingParams& params,
                          [[maybe_unused]] const Starcoder2SamplerFactoryOptions& options) {
...
std::unique_ptr<Starcoder2ISampler>
create_starcoder2_sampler(const Starcoder2SamplingParams& params) {
    return create_starcoder2_sampler(params, Starcoder2SamplerFactoryOptions{});
}

std::unique_ptr<Starcoder2ISampler> create_starcoder2_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
