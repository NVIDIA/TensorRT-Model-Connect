...
#include "runtime/models/starcoder2/inference_state.h"
...
enum class Starcoder2TriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class Starcoder2TriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct Starcoder2TriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    Starcoder2TriAttentionScoreAggregation score_aggregation{
        Starcoder2TriAttentionScoreAggregation::kMean};
    Starcoder2TriAttentionScoreAggregation per_layer_aggregation{
        Starcoder2TriAttentionScoreAggregation::kMean};
...
struct Starcoder2TriAttentionHeadStats {
...
struct Starcoder2TriAttentionStats {
    int32_t head_dim{0};
    Starcoder2TriAttentionRopeStyle rope_style{Starcoder2TriAttentionRopeStyle::kHalf};
    ...
    std::vector<Starcoder2TriAttentionHeadStats> layer_stats;
};

struct Starcoder2TriAttentionCompactionProfile {
...
// Build a Starcoder2TriAttentionConfig from the bundle's JSON plus (optionally) the
...
class Starcoder2TriAttentionKvCache : public Starcoder2InferenceState {
  public:
    Starcoder2TriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                                  int32_t kv_dim, cudaStream_t stream,
                                  Starcoder2TriAttentionConfig config,
  ...
  private:
    ...
    void finalize_repack_profile(Starcoder2TriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const Starcoder2TriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const Starcoder2TriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        Starcoder2TriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             Starcoder2TriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const Starcoder2TriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const Starcoder2TriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const Starcoder2TriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    Starcoder2KvCacheNames names_;
    Starcoder2TriAttentionConfig config_;
    Starcoder2TriAttentionStats stats_;
