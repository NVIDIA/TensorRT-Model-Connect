...
// Starcoder2InferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   Starcoder2KvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — Starcoder2KvCache + family-owned recurrent state composed
...
class Starcoder2InferenceState {
  public:
    virtual ~Starcoder2InferenceState() = default;
    ...
    // Starcoder2KvCache -> true. Family-owned recurrent state -> false.
