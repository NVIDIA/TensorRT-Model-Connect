...
// Starcoder2KvCache: autoregressive KV cache state manager.
...
class Starcoder2KvCache : public Starcoder2InferenceState {
  public:
    ...
    Starcoder2KvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                      DType cache_dtype = DType::kFloat32, Starcoder2KvCacheNames names = {});

    // --- Starcoder2InferenceState overrides ---
    void reset() override;
    ...
    // --- Starcoder2KvCache-specific methods (not on the interface) ---
    ...
    // — too big for Starcoder2KvCache's single-row present buffer. The caller reads
