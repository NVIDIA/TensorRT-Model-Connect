...
#include "runtime/models/xglm/inference_state.h"
...
enum class XglmTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class XglmTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct XglmTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    XglmTriAttentionScoreAggregation score_aggregation{XglmTriAttentionScoreAggregation::kMean};
    XglmTriAttentionScoreAggregation per_layer_aggregation{XglmTriAttentionScoreAggregation::kMean};
...
struct XglmTriAttentionHeadStats {
...
struct XglmTriAttentionStats {
    int32_t head_dim{0};
    XglmTriAttentionRopeStyle rope_style{XglmTriAttentionRopeStyle::kHalf};
    ...
    std::vector<XglmTriAttentionHeadStats> layer_stats;
};

struct XglmTriAttentionCompactionProfile {
...
// Build a XglmTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class XglmTriAttentionKvCache : public XglmInferenceState {
  public:
    XglmTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                            int32_t kv_dim, cudaStream_t stream, XglmTriAttentionConfig config,
                            XglmTriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(XglmTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const XglmTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const XglmTriAttentionCompactionProfile* profile_ptr) const;
    ...
    std::vector<int32_t> select_keep_indices(int32_t keep_budget,
                                             XglmTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             XglmTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const XglmTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const XglmTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const XglmTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    XglmKvCacheNames names_;
    XglmTriAttentionConfig config_;
    XglmTriAttentionStats stats_;
