...
// XglmISampler: token selection abstraction for autoregressive generation.
//
...
//
// XglmLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// xglm-owned sampler interface.
class XglmISampler {
  public:
    virtual ~XglmISampler() = default;
    ...
    virtual XglmSampleResult sample(const float* logits, int32_t vocab_size,
                                    const XglmSamplingParams& params) = 0;
    ...
    virtual XglmLogitsLocation logits_location() const = 0;
...
/// Build XglmSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from XglmSamplingParams.
...
std::unique_ptr<XglmISampler> create_xglm_sampler(const XglmSamplingParams& params,
                                                  const XglmSamplerFactoryOptions& options);
