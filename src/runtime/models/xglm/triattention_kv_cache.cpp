...
#include "runtime/models/xglm/triattention_kernels.h"
#endif
...
XglmTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return XglmTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return XglmTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(XglmTriAttentionScoreAggregation value) {
    switch (value) {
    case XglmTriAttentionScoreAggregation::kMean:
        return "mean";
    case XglmTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

XglmTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return XglmTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return XglmTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, XglmKvCacheNames& names) {
...
// override helpers that patched XglmTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     XglmTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(XglmTriAttentionConfig& cfg,
...
void fill_debug_from_registry(XglmTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(XglmTriAttentionConfig& cfg, const std::string& config_json,
...
static void validate_triattention_config(const XglmTriAttentionConfig& cfg,
                                         int32_t max_cache_length) {
    if (cfg.kv_budget < 1)
...
XglmTriAttentionConfig
xglm_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                      const ::trtmc::config::ConfigBundle* runtime_config) {
    XglmTriAttentionConfig cfg;
...
void fill_stats_header(XglmTriAttentionStats& stats, const json& root, int32_t num_attention_heads,
...
void fill_stats_inv_freq(XglmTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(XglmTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(XglmTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(XglmTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(XglmTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(XglmTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
void init_layer_stats_empty(XglmTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(XglmTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(XglmTriAttentionStats& stats, const std::vector<int32_t>& group_counts,
                           int32_t half_dim) {
    bool any_stats = false;
    for (int32_t layer = 0; layer < stats.num_layers; ++layer) {
        auto& layer_stats = stats.layer_stats[static_cast<std::size_t>(layer)];
...
void parse_sparse_stats(XglmTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
XglmTriAttentionStats xglm_parse_triattention_stats_json(const std::string& stats_json,
                                                         int32_t num_attention_heads,
                                                         int32_t num_key_value_heads,
                                                         int32_t num_layers) {
    XglmTriAttentionStats stats;
...
void XglmTriAttentionKvCache::validate_shapes() {
...
void XglmTriAttentionKvCache::normalize_sampled_heads() {
...
void XglmTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void XglmTriAttentionKvCache::allocate_layer_tensors() {
...
XglmTriAttentionKvCache::XglmTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                 int32_t max_length, int32_t kv_dim,
                                                 cudaStream_t stream, XglmTriAttentionConfig config,
                                                 XglmTriAttentionStats stats, DType cache_dtype,
                                                 XglmKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void XglmTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void XglmTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void XglmTriAttentionKvCache::initialize_gpu_state() {
...
bool XglmTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool XglmTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool XglmTriAttentionKvCache::can_use_gpu_selection() const {
...
void XglmTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t XglmTriAttentionKvCache::preferred_cache_rows() const {
...
void XglmTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void XglmTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float>
XglmTriAttentionKvCache::copy_cache_rows_to_host(const DeviceTensor& tensor, int32_t rows,
                                                 XglmTriAttentionCompactionProfile* profile) const {
...
void XglmTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t XglmTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> XglmTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t> XglmTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
XglmTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                             XglmTriAttentionCompactionProfile* profile) {
...
int32_t XglmTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t XglmTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> XglmTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void XglmTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, XglmTriAttentionCompactionProfile* profile) const {
...
bool XglmTriAttentionKvCache::layer_stats_shapes_valid(const XglmTriAttentionHeadStats& layer_stats,
...
void XglmTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                            int32_t half_dim, float& k_rot_real,
                                            float& k_rot_imag) const {
    if (stats_.rope_style == XglmTriAttentionRopeStyle::kInterleaved) {
...
float XglmTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == XglmTriAttentionScoreAggregation::kMax)
...
float XglmTriAttentionKvCache::score_one_row(
    const float* row_ptr, const XglmTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void XglmTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const XglmTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void XglmTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void XglmTriAttentionKvCache::accumulate_layer_fallback(
...
void XglmTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == XglmTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void XglmTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
XglmTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void XglmTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == XglmTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void XglmTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> XglmTriAttentionKvCache::build_keep_indices_per_head(
...
bool XglmTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, XglmTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> XglmTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, XglmTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void XglmTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                     int32_t total_tokens) const {
...
void XglmTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void XglmTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
XglmTriAttentionKvCache::GpuLayerResult XglmTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = xglm_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == XglmTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == XglmTriAttentionScoreAggregation::kMax,
...
bool XglmTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool XglmTriAttentionKvCache::upload_gpu_trig_phases(int32_t num_offsets, int32_t half_dim,
                                                     XglmTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void XglmTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> XglmTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool XglmTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> XglmTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, XglmTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void XglmTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void XglmTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool XglmTriAttentionKvCache::gpu_compaction_upload_keep(int32_t keep_count,
...
bool XglmTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                    std::size_t row_bytes) {
    const bool ok_k = xglm_triattention_compact_rows_gpu(
    ...
    const bool ok_v = xglm_triattention_compact_rows_gpu(
...
bool XglmTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
                                                   const std::vector<int32_t>& keep_indices,
...
void XglmTriAttentionKvCache::compact_layer_on_host(int32_t layer,
                                                    const std::vector<int32_t>& keep_indices,
                                                    int32_t keep_count, std::size_t row_bytes,
...
void XglmTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    XglmTriAttentionCompactionProfile profile;
    XglmTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void XglmTriAttentionKvCache::finalize_repack_profile(
    XglmTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
XglmTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
XglmTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                   int32_t keep_count) const {
...
int32_t XglmTriAttentionKvCache::count_prefix_positions(
...
void XglmTriAttentionKvCache::log_compact_debug(
...
void XglmTriAttentionKvCache::log_compact_profile(
    const XglmTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool XglmTriAttentionKvCache::should_emit_keep_dump(
    const XglmTriAttentionCompactionProfile* profile_ptr) const {
...
void XglmTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "XglmTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void XglmTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void XglmTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void XglmTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool XglmTriAttentionKvCache::ok() const {
