...
//
// Composes: TrtModule (decoder) + XglmKvCache + ITokenizer for this runtime
...
class XglmTextGenerationPipeline final : public IPipeline {
  public:
    ...
    XglmTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                               ...
                               std::unique_ptr<XglmISampler> sampler = nullptr,
                               std::shared_ptr<void> distributed_owner = nullptr);
    XglmTextGenerationPipeline(std::vector<DecoderContext> decoders,
                               std::unique_ptr<XglmInferenceState> state, XglmTextGenConfig config,
                               cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer = nullptr,
                               std::string model_id_str = "",
                               std::unique_ptr<XglmISampler> sampler = nullptr,
    ...
    const char* pipeline_type() const override { return "XglmTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<XglmInferenceState> state_;
    XglmTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<XglmISampler> sampler_;
    ...
    TimedGenResult generate_from_ids(const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
                                     const XglmSamplingParams& params, const GenerateConfig& cfg);
    ...
    XglmKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(XglmKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(XglmISampler* sampler, const XglmSamplingParams& params,
    ...
    std::unique_ptr<XglmISampler> make_step_sampler(const XglmSamplingParams& params);
