...
// XglmKvCache: autoregressive KV cache state manager.
...
class XglmKvCache : public XglmInferenceState {
  public:
    ...
    XglmKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                DType cache_dtype = DType::kFloat32, XglmKvCacheNames names = {});

    // --- XglmInferenceState overrides ---
    void reset() override;
    ...
    // --- XglmKvCache-specific methods (not on the interface) ---
    ...
    // — too big for XglmKvCache's single-row present buffer. The caller reads
