// XglmISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// XglmTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/xglm/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/xglm/argmax_kernel.h"
#include "runtime/models/xglm/sparse_multinomial_kernel.h"
...
static XglmSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                           int32_t eos_token_id) {
    XglmSampleResult result;
...
static bool greedy_equivalent(const XglmSamplingParams& params) {
...
class GreedySampler final : public XglmISampler {
  public:
    XglmSampleResult sample(const float* logits, int32_t vocab_size,
                            const XglmSamplingParams& params) override {
    ...
    XglmLogitsLocation logits_location() const override { return XglmLogitsLocation::HOST; }
...
class TopKSampler final : public XglmISampler {
  public:
...
class TorchCudaMultinomialSampler final : public XglmISampler {
  public:
    ...
    XglmSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        xglm_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const XglmTorchMultinomialExecutionPolicy policy =
                xglm_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public XglmISampler {
  public:
    ...
    XglmSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        xglm_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    XglmLogitsLocation logits_location() const override { return XglmLogitsLocation::DEVICE; }
...
XglmSamplingParams xglm_sampling_params_from_config(const GenerateConfig& cfg,
                                                    int32_t default_eos) {
    XglmSamplingParams p;
...
create_xglm_sampler(const XglmSamplingParams& params,
                    [[maybe_unused]] const XglmSamplerFactoryOptions& options) {
...
std::unique_ptr<XglmISampler> create_xglm_sampler(const XglmSamplingParams& params) {
    return create_xglm_sampler(params, XglmSamplerFactoryOptions{});
}

std::unique_ptr<XglmISampler> create_xglm_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
