...
// TimmSeresnetPlugin: handles timm SE-ResNet image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_seresnet/pipeline.h"
...
TimmSeresnetPreprocessConfig make_timm_seresnet_preprocess_config(const std::string& json) {
    TimmSeresnetPreprocessConfig cfg;
...
class TimmSeresnetPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmSeresnetImageClassificationPipeline>(
            std::move(loaded.module), make_timm_seresnet_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_seresnet_plugin, TimmSeresnetPlugin,
