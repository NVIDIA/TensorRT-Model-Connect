// LanceISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// LancePipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/lance/argmax_kernel.h"
#include "runtime/models/lance/sparse_multinomial_kernel.h"
...
static LanceSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                            int32_t eos_token_id) {
    LanceSampleResult result;
...
static bool greedy_equivalent(const LanceSamplingParams& params) {
...
class GreedySampler final : public LanceISampler {
  public:
    LanceSampleResult sample(const float* logits, int32_t vocab_size,
                             const LanceSamplingParams& params) override {
    ...
    LanceLogitsLocation logits_location() const override { return LanceLogitsLocation::HOST; }
...
class TopKSampler final : public LanceISampler {
  public:
...
class TorchCudaMultinomialSampler final : public LanceISampler {
  public:
    ...
    LanceSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        lance_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const LanceTorchMultinomialExecutionPolicy policy =
                lance_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public LanceISampler {
  public:
    ...
    LanceSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        lance_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    LanceLogitsLocation logits_location() const override { return LanceLogitsLocation::DEVICE; }
...
LanceSamplingParams lance_sampling_params_from_config(const GenerateConfig& cfg,
                                                      int32_t default_eos) {
    LanceSamplingParams p;
...
std::unique_ptr<LanceISampler>
create_lance_sampler(const LanceSamplingParams& params,
                     [[maybe_unused]] const LanceSamplerFactoryOptions& options) {
...
std::unique_ptr<LanceISampler> create_lance_sampler(const LanceSamplingParams& params) {
    return create_lance_sampler(params, LanceSamplerFactoryOptions{});
}

std::unique_ptr<LanceISampler> create_lance_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
