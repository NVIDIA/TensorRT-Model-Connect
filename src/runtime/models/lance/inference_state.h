...
// LanceInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   LanceKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — LanceKvCache + family-owned recurrent state composed
...
class LanceInferenceState {
  public:
    virtual ~LanceInferenceState() = default;
    ...
    // LanceKvCache -> true. Family-owned recurrent state -> false.
