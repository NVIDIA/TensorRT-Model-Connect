...
// LanceKvCache: autoregressive KV cache state manager.
...
class LanceKvCache : public LanceInferenceState {
  public:
    ...
    LanceKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                 DType cache_dtype = DType::kFloat32, LanceKvCacheNames names = {});

    // --- LanceInferenceState overrides ---
    void reset() override;
    ...
    // --- LanceKvCache-specific methods (not on the interface) ---
    ...
    // — too big for LanceKvCache's single-row present buffer. The caller reads
