...
// FnetKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/fnet/inference_state.h"
...
class FnetKvCache : public FnetInferenceState {
  public:
    ...
    FnetKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                DType cache_dtype = DType::kFloat32, FnetKvCacheNames names = {});

    // --- FnetInferenceState overrides ---
    void reset() override;
    ...
    // --- FnetKvCache-specific methods (not on the interface) ---
    ...
    // — too big for FnetKvCache's single-row present buffer. The caller reads
