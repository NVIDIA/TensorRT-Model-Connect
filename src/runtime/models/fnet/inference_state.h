...
// FnetInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   FnetKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — FnetKvCache + family-owned recurrent state composed
...
class FnetInferenceState {
  public:
    virtual ~FnetInferenceState() = default;
    ...
    // FnetKvCache -> true. Family-owned recurrent state -> false.
