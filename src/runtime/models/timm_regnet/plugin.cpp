...
// TimmRegnetPlugin: handles timm RegNet image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_regnet/pipeline.h"
...
TimmRegnetPreprocessConfig make_timm_regnet_preprocess_config(const std::string& json) {
    TimmRegnetPreprocessConfig cfg;
...
class TimmRegnetPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmRegnetImageClassificationPipeline>(
            std::move(loaded.module), make_timm_regnet_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_regnet_plugin, TimmRegnetPlugin,
                                       "timm_regnet_image_classification");
