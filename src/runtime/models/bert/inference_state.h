...
// BertInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   BertKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — BertKvCache + family-owned recurrent state composed
...
class BertInferenceState {
  public:
    virtual ~BertInferenceState() = default;
    ...
    // BertKvCache -> true. Family-owned recurrent state -> false.
