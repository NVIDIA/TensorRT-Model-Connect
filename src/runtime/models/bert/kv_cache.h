...
// BertKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/bert/inference_state.h"
...
class BertKvCache : public BertInferenceState {
  public:
    ...
    BertKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                DType cache_dtype = DType::kFloat32, BertKvCacheNames names = {});

    // --- BertInferenceState overrides ---
    void reset() override;
    ...
    // --- BertKvCache-specific methods (not on the interface) ---
    ...
    // — too big for BertKvCache's single-row present buffer. The caller reads
