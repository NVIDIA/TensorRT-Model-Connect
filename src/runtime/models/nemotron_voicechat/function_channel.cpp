...
FunctionTool parse_tool(const json& entry) {
    ...
    if (const auto* acknowledgements = find_ack_messages(entry, body))
        tool.ack_messages = parse_ascii_messages(*acknowledgements, "ack_messages", false);
    return tool;
}
...
FunctionToolCatalog FunctionToolCatalog::from_json(std::string_view tools_json) {
    ...
    if (!root.is_array())
        throw std::invalid_argument("tools must be a JSON array");
...
FunctionOnHoldMessages FunctionOnHoldMessages::from_json(std::string_view messages_json) {
    ...
    FunctionOnHoldMessages messages;
    for (const auto& item : root.items()) {
        ...
        messages.messages_.emplace(item.key(),
                                   parse_ascii_messages(item.value(), "on-hold message", true));
    }
    return messages;
}

const std::vector<std::string>& FunctionOnHoldMessages::lookup(std::string_view tool_name) const {
    static const std::vector<std::string> empty;
    auto match = messages_.find(tool_name);
    if (match == messages_.end())
        match = messages_.find("default");
    return match == messages_.end() ? empty : match->second;
}
...
ForcedFunctionTokens build_tool_response_tokens(std::string_view result_json,
    ...
    ForcedFunctionTokens forced;
    forced.text = "<TOOL_RESPONSE>" + payload.dump() + "</TOOL_RESPONSE>";
    forced.token_ids = encode_text(forced.text);
    if (forced.token_ids.empty())
