// Olmo2ISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// Olmo2TextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/olmo2/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/olmo2/argmax_kernel.h"
#include "runtime/models/olmo2/sparse_multinomial_kernel.h"
...
static Olmo2SampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                            int32_t eos_token_id) {
    Olmo2SampleResult result;
...
static bool greedy_equivalent(const Olmo2SamplingParams& params) {
...
class GreedySampler final : public Olmo2ISampler {
  public:
    Olmo2SampleResult sample(const float* logits, int32_t vocab_size,
                             const Olmo2SamplingParams& params) override {
    ...
    Olmo2LogitsLocation logits_location() const override { return Olmo2LogitsLocation::HOST; }
...
class TopKSampler final : public Olmo2ISampler {
  public:
...
class TorchCudaMultinomialSampler final : public Olmo2ISampler {
  public:
    ...
    Olmo2SampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        olmo2_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const Olmo2TorchMultinomialExecutionPolicy policy =
                olmo2_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public Olmo2ISampler {
  public:
    ...
    Olmo2SampleResult sample(const float* logits, int32_t vocab_size,
        ...
        olmo2_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    Olmo2LogitsLocation logits_location() const override { return Olmo2LogitsLocation::DEVICE; }
...
Olmo2SamplingParams olmo2_sampling_params_from_config(const GenerateConfig& cfg,
                                                      int32_t default_eos) {
    Olmo2SamplingParams p;
...
create_olmo2_sampler(const Olmo2SamplingParams& params,
                     [[maybe_unused]] const Olmo2SamplerFactoryOptions& options) {
...
std::unique_ptr<Olmo2ISampler> create_olmo2_sampler(const Olmo2SamplingParams& params) {
    return create_olmo2_sampler(params, Olmo2SamplerFactoryOptions{});
}

std::unique_ptr<Olmo2ISampler> create_olmo2_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
