...
// Olmo2KvCache: autoregressive KV cache state manager.
...
class Olmo2KvCache : public Olmo2InferenceState {
  public:
    ...
    Olmo2KvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                 DType cache_dtype = DType::kFloat32, Olmo2KvCacheNames names = {});

    // --- Olmo2InferenceState overrides ---
    void reset() override;
    ...
    // --- Olmo2KvCache-specific methods (not on the interface) ---
    ...
    // — too big for Olmo2KvCache's single-row present buffer. The caller reads
