...
#include "runtime/models/olmo2/inference_state.h"
...
enum class Olmo2TriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class Olmo2TriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct Olmo2TriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    Olmo2TriAttentionScoreAggregation score_aggregation{Olmo2TriAttentionScoreAggregation::kMean};
    Olmo2TriAttentionScoreAggregation per_layer_aggregation{
        Olmo2TriAttentionScoreAggregation::kMean};
...
struct Olmo2TriAttentionHeadStats {
...
struct Olmo2TriAttentionStats {
    int32_t head_dim{0};
    Olmo2TriAttentionRopeStyle rope_style{Olmo2TriAttentionRopeStyle::kHalf};
    ...
    std::vector<Olmo2TriAttentionHeadStats> layer_stats;
};

struct Olmo2TriAttentionCompactionProfile {
...
// Build a Olmo2TriAttentionConfig from the bundle's JSON plus (optionally) the
...
class Olmo2TriAttentionKvCache : public Olmo2InferenceState {
  public:
    Olmo2TriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                             int32_t kv_dim, cudaStream_t stream, Olmo2TriAttentionConfig config,
                             Olmo2TriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(Olmo2TriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const Olmo2TriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const Olmo2TriAttentionCompactionProfile* profile_ptr) const;
    ...
    std::vector<int32_t> select_keep_indices(int32_t keep_budget,
                                             Olmo2TriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             Olmo2TriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                Olmo2TriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const Olmo2TriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const Olmo2TriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const Olmo2TriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    Olmo2KvCacheNames names_;
    Olmo2TriAttentionConfig config_;
    Olmo2TriAttentionStats stats_;
