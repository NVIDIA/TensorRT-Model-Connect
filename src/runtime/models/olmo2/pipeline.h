...
//
// Composes: TrtModule (decoder) + Olmo2KvCache + ITokenizer for this runtime
...
class Olmo2TextGenerationPipeline final : public IPipeline {
  public:
    ...
    Olmo2TextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                std::unique_ptr<Olmo2InferenceState> state,
                                Olmo2TextGenConfig config, cudaStream_t stream,
                                std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                std::string model_id_str = "",
                                std::unique_ptr<Olmo2ISampler> sampler = nullptr,
                                std::shared_ptr<void> distributed_owner = nullptr);
    Olmo2TextGenerationPipeline(std::vector<DecoderContext> decoders,
                                std::unique_ptr<Olmo2InferenceState> state,
                                Olmo2TextGenConfig config, cudaStream_t stream,
                                std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "Olmo2TextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<Olmo2InferenceState> state_;
    Olmo2TextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<Olmo2ISampler> sampler_;
    ...
    Olmo2KvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(Olmo2KvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(Olmo2ISampler* sampler, const Olmo2SamplingParams& params,
    ...
    std::unique_ptr<Olmo2ISampler> make_step_sampler(const Olmo2SamplingParams& params);
