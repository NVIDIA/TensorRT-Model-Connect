...
// ZImageKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/z_image/inference_state.h"
...
class ZImageKvCache : public ZImageInferenceState {
  public:
    ...
    ZImageKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, ZImageKvCacheNames names = {});

    // --- ZImageInferenceState overrides ---
    void reset() override;
    ...
    // --- ZImageKvCache-specific methods (not on the interface) ---
    ...
    // — too big for ZImageKvCache's single-row present buffer. The caller reads
