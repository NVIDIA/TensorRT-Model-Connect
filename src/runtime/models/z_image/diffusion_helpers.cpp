...
void load_preprocessor_weights(const std::string& index_json, const char* blob,
                               std::size_t blob_size, ZImageCommonPreprocessorWeights& w) {
    z_image_preprocessor_weights::load_with_fallback(index_json, blob, blob_size,
                                                     "patch_embedding.weight", "x_embedder.weight",
                                                     w.patch_embed_weight);
    z_image_preprocessor_weights::load_with_fallback(
...
void finalize_preprocessor_weights(ZImageCommonPreprocessorWeights& w) {
...
ZImageCommonPreprocessorWeights parse_preprocessor_weights(const std::vector<char>& data) {
    ZImageCommonPreprocessorWeights w;
    ...
    std::cerr << "[z-image] Common preprocessor weights loaded: " << (w.valid ? "OK" : "INCOMPLETE")
