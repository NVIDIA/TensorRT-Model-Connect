...
namespace trtmc {
namespace z_image_preprocessor_weights {
...
inline bool extract_preprocessor_index(const std::vector<char>& data, std::string& index_json,
    ...
    if (data.size() < 4) {
        std::cerr << "[z-image] preprocessor_weights section too small\n";
        return false;
    }
    ...
    if (4 + index_len > data.size()) {
        std::cerr << "[z-image] preprocessor_weights index length overflow\n";
        return false;
    }
