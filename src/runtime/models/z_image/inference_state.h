...
// ZImageInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   ZImageKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — ZImageKvCache + family-owned recurrent state composed
...
class ZImageInferenceState {
  public:
    virtual ~ZImageInferenceState() = default;
    ...
    // ZImageKvCache -> true. Family-owned recurrent state -> false.
