...
// Qwen3OmniKvCache: autoregressive KV cache state manager.
...
class Qwen3OmniKvCache : public Qwen3OmniInferenceState {
  public:
    ...
    Qwen3OmniKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                     DType cache_dtype = DType::kFloat32, Qwen3OmniKvCacheNames names = {});

    // --- Qwen3OmniInferenceState overrides ---
    void reset() override;
    ...
    // --- Qwen3OmniKvCache-specific methods (not on the interface) ---
    ...
    // — too big for Qwen3OmniKvCache's single-row present buffer. The caller reads
