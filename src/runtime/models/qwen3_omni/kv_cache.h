...
// Runtime-owned, fixed-capacity TensorRT native KV cache for the Omni Thinker.
...
class Qwen3OmniKvCache : public Qwen3OmniInferenceState {
  public:
    Qwen3OmniKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                     DType cache_dtype = DType::kBFloat16, Qwen3OmniKvCacheNames names = {});
  ...
  private:
    void validate_native_contract(TrtModule& module) const;
    ...
    int32_t kv_dim_{0};
    int32_t position_{0};
    std::vector<int32_t> position_buf_;
