...
// DebertaKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/deberta/inference_state.h"
...
class DebertaKvCache : public DebertaInferenceState {
  public:
    ...
    DebertaKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                   DType cache_dtype = DType::kFloat32, DebertaKvCacheNames names = {});

    // --- DebertaInferenceState overrides ---
    void reset() override;
    ...
    // --- DebertaKvCache-specific methods (not on the interface) ---
    ...
    // — too big for DebertaKvCache's single-row present buffer. The caller reads
