...
// DebertaInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   DebertaKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — DebertaKvCache + family-owned recurrent state composed
...
class DebertaInferenceState {
  public:
    virtual ~DebertaInferenceState() = default;
    ...
    // DebertaKvCache -> true. Family-owned recurrent state -> false.
