...
DualProfileModules load_text_modules(const PipelineContext& ctx, TextModuleRuntime& runtime,
    ...
    if (decode_loaded.prefill)
        throw std::runtime_error("LocateAnything native KV decode engine must be single-profile");
    ...
    if (prefill_loaded.prefill)
        throw std::runtime_error("LocateAnything native KV prefill engine must be single-profile");
...
bool engine_uses_native_kv_updates(const TrtModule& module,
                                   const LocateanythingKvCacheNames& names) {
    ...
    if (has_write != has_lengths)
        throw std::runtime_error("LocateAnything native KV engine must expose both scalar inputs");
    return has_write;
}
...
void validate_native_engine(const PipelineContext& ctx, const TrtModule& module,
    ...
    if (!engine_uses_native_kv_updates(module, names))
        throw std::runtime_error("LocateAnything legacy KV engines are no longer supported");
    if (cache_dtype != DType::kBFloat16)
        throw std::runtime_error("LocateAnything native KV runtime requires BF16");
    if (names.cache_k.empty())
        throw std::runtime_error("LocateAnything native KV engine has no cache inputs");
    ...
    if (shape != expected)
        throw std::runtime_error(
            "LocateAnything native KV cache shape does not match rank-local full capacity");
}

void validate_native_bundle(const PipelineContext& ctx, const TrtModule& decode,
                            const TrtModule& prefill, const LocateanythingKvCacheNames& names,
                            const TensorParallelRuntimeConfig& tp_config, DType cache_dtype) {
    if (!extract_json_bool(ctx.config_json, "native_kv_cache", false) ||
        ...
        throw std::runtime_error("LocateAnything bundle is missing native KV contract metadata");
    ...
    if (ctx.kv_cache_size_bytes != 0)
        throw std::invalid_argument(
            "LocateAnything native KV allocates the model's complete fixed capacity; "
            "kv_cache_size_bytes is not supported");
}

std::uint64_t checked_multiply(std::uint64_t lhs, std::uint64_t rhs) {
    if (lhs != 0 && rhs > std::numeric_limits<std::uint64_t>::max() / lhs)
        throw std::overflow_error("LocateAnything native KV byte accounting overflow");
    return lhs * rhs;
}
...
void admit_native_kv_allocation(const PipelineContext& ctx, int32_t local_kv_dim,
    ...
    if (status != cudaSuccess)
        throw std::runtime_error(std::string("LocateAnything CUDA memory query failed: ") +
                                 cudaGetErrorString(status));
    constexpr std::uint64_t kTwoGiB = 2ULL << 30;
    const auto free = static_cast<std::uint64_t>(free_bytes);
    const auto reserve = std::max(kTwoGiB, free / 10);
    const auto available = free > reserve ? free - reserve : 0;
    if (bytes > available)
        throw std::runtime_error(
            "LocateAnything native KV admission failed before allocation: required=" +
...
int32_t prefill_profile_capacity(const TrtModule& module) {
    ...
    if (value <= 0)
        throw std::runtime_error(
            "LocateAnything native prefill engine has invalid profile capacity");
    return value;
}
...
class VLPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        if (!state->ok())
            throw std::runtime_error("Failed to allocate LocateAnything native KV cache");
