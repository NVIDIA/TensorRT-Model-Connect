...
// LocateanythingKvCache: autoregressive KV cache state manager.
...
struct LocateanythingKvCacheNames {
...
class LocateanythingKvCache : public LocateanythingInferenceState {
  public:
    ...
    LocateanythingKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim,
                          cudaStream_t stream, DType cache_dtype = DType::kFloat32,
                          LocateanythingKvCacheNames names = {});

    // --- LocateanythingInferenceState overrides ---
    void reset() override;
    ...
    // --- LocateanythingKvCache-specific methods (not on the interface) ---
    ...
    // — too big for LocateanythingKvCache's single-row present buffer. The caller reads
  ...
  private:
    ...
    LocateanythingKvCacheNames names_;
