...
// LocateAnythingISampler: token selection abstraction for autoregressive generation.
//
...
//
// LocateAnythingLogitsLocation tells the pipeline whether to D2H transfer logits before
...
struct LocateAnythingSamplerFactoryOptions {
    bool prefer_torch_cuda_multinomial{true};
};

/// Where the sampler expects logits to live.
enum class LocateAnythingLogitsLocation {
...
class LocateAnythingISampler {
  public:
    ...
    virtual LocateAnythingLogitsLocation logits_location() const = 0;
...
/// Build LocateAnythingSamplingParams from GenerateConfig fields.
...
std::unique_ptr<LocateAnythingISampler>
create_locateanything_sampler(const LocateAnythingSamplingParams& params,
                              const LocateAnythingSamplerFactoryOptions& options);
