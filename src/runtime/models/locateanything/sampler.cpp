// LocateAnythingISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// LocateAnythingPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/locateanything/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/locateanything/argmax_kernel.h"
#include "runtime/models/locateanything/sparse_multinomial_kernel.h"
...
static LocateAnythingSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
...
static bool greedy_equivalent(const LocateAnythingSamplingParams& params) {
...
class GreedySampler final : public LocateAnythingISampler {
  public:
    LocateAnythingSampleResult sample(const float* logits, int32_t vocab_size,
                                      const LocateAnythingSamplingParams& params) override {
    ...
    LocateAnythingLogitsLocation logits_location() const override {
        return LocateAnythingLogitsLocation::HOST;
    }
...
class TopKSampler final : public LocateAnythingISampler {
  public:
...
class TorchCudaMultinomialSampler final : public LocateAnythingISampler {
  public:
    ...
    LocateAnythingSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        locateanything_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const LocateanythingTorchMultinomialExecutionPolicy policy =
                locateanything_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public LocateAnythingISampler {
  public:
    ...
    LocateAnythingSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        locateanything_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    LocateAnythingLogitsLocation logits_location() const override {
        return LocateAnythingLogitsLocation::DEVICE;
    }
...
LocateAnythingSamplingParams locateanything_sampling_params_from_config(const GenerateConfig& cfg,
                                                                        int32_t default_eos) {
    LocateAnythingSamplingParams p;
...
create_locateanything_sampler(const LocateAnythingSamplingParams& params,
                              [[maybe_unused]] const LocateAnythingSamplerFactoryOptions& options) {
...
std::unique_ptr<LocateAnythingISampler>
create_locateanything_sampler(const LocateAnythingSamplingParams& params) {
    return create_locateanything_sampler(params, LocateAnythingSamplerFactoryOptions{});
}

std::unique_ptr<LocateAnythingISampler> create_locateanything_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
