...
void validate_native_scalar_input(TrtModule& module, const std::string& name) {
    if (!module.has_input(name) || module.tensor_dtype(name) != DType::kInt32 ||
        ...
        throw std::runtime_error("LocateAnything native KV input '" + name + "' must be int32 [1]");
    }
}
...
void validate_native_cache_pair(TrtModule& module, const std::string& cache_name,
    ...
    if (!module.has_input(cache_name) || !module.has_output(present_name)) {
        throw std::runtime_error("LocateAnything native KV engine is missing cache/present pair '" +
    ...
    if (!valid_native_cache_shape(cache_shape, max_length, kv_dim) ||
        ...
        throw std::runtime_error(
            "LocateAnything native KV cache/present tensors must share [1,Hkv,capacity,D]");
    }
    if (module.tensor_dtype(cache_name) != cache_dtype ||
        module.tensor_dtype(present_name) != cache_dtype) {
        throw std::runtime_error(
            "LocateAnything native KV cache dtype does not match model precision");
    }
}
...
void populate_default_cache_names(LocateanythingKvCacheNames& names, int32_t num_layers) {
...
void validate_cache_name_count(const LocateanythingKvCacheNames& names, int32_t num_layers) {
    ...
    if (names.cache_k.size() != expected || names.cache_v.size() != expected ||
        ...
        throw std::invalid_argument("LocateanythingKvCache per-layer tensor name count mismatch");
    }
}
...
LocateanythingKvCache::LocateanythingKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim,
    ...
    if (num_layers <= 0 || max_length <= 0 || kv_dim <= 0)
        throw std::invalid_argument("LocateanythingKvCache dimensions must be positive");
...
void LocateanythingKvCache::validate_native_kv_contract(TrtModule& module) const {
    ...
    if (module.has_input("attention_mask")) {
        throw std::runtime_error(
            "LocateAnything native KV engine must use key_value_lengths, not attention_mask");
    }
...
void LocateanythingKvCache::bind_native_cache(TrtModule& module) {
    for (int32_t layer = 0; layer < num_layers_; ++layer) {
        const auto index = static_cast<std::size_t>(layer);
        ...
        if (module.device_ptr(names_.cache_k[index]) != cache_k_[index].data() ||
            ...
            throw std::runtime_error(
                "LocateAnything native KV engine did not preserve cache/present aliasing");
        }
    }
}

void LocateanythingKvCache::bind_to(TrtModule& module) {
    has_position_input_ = module.has_input(names_.position_id);
    validate_native_kv_contract(module);
    bind_native_cache(module);
}

void LocateanythingKvCache::bind_cache_inputs(TrtModule& module) {
    bind_to(module);
}

void LocateanythingKvCache::write_position_input(TensorMap& inputs, int32_t seq_len) {
...
void LocateanythingKvCache::write_native_kv_inputs(TensorMap& inputs, int32_t seq_len) {
    if (seq_len <= 0)
        throw std::invalid_argument("LocateAnything native KV step length must be positive");
    if (seq_len > max_length_ - position_)
        throw std::runtime_error("LocateAnything sequence exceeds fixed KV capacity");
...
void LocateanythingKvCache::prepare_step(TensorMap& inputs, int32_t seq_len) {
    if (seq_len <= 0)
        throw std::invalid_argument("LocateAnything native KV step length must be positive");
...
void LocateanythingKvCache::validate_native_aliases(
    ...
    if (present_k.size() != static_cast<std::size_t>(num_layers_) ||
        ...
        throw std::runtime_error("LocateAnything native KV per-layer pointer count mismatch");
    }
    for (int32_t layer = 0; layer < num_layers_; ++layer) {
        const auto index = static_cast<std::size_t>(layer);
        if (present_k[index] != cache_k_[index].data() ||
            ...
            throw std::runtime_error(
                "LocateAnything native prefill present tensors must alias the KV cache");
        }
    }
}

void LocateanythingKvCache::append_prefill_kv(const std::vector<const void*>& prefill_k,
                                              const std::vector<const void*>& prefill_v,
                                              int32_t seq_len) {
    if (seq_len <= 0)
        throw std::invalid_argument("LocateAnything native prefill length must be positive");
    if (seq_len > max_length_ - position_)
        throw std::runtime_error("LocateAnything native prefill exceeds fixed KV capacity");
    validate_native_aliases(prefill_k, prefill_v);
    position_ += seq_len;
}

void LocateanythingKvCache::advance(int32_t n_tokens) {
    if (n_tokens <= 0)
        throw std::invalid_argument("LocateAnything native KV advance must be positive");
...
bool LocateanythingKvCache::ok() const {
