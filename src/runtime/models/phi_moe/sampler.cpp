...
// PhiMoeISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// PhiMoeTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/phi_moe/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/phi_moe/argmax_kernel.h"
#include "runtime/models/phi_moe/sparse_multinomial_kernel.h"
...
static PhiMoeSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                             int32_t eos_token_id) {
    PhiMoeSampleResult result;
...
static bool greedy_equivalent(const PhiMoeSamplingParams& params) {
...
class GreedySampler final : public PhiMoeISampler {
  public:
    PhiMoeSampleResult sample(const float* logits, int32_t vocab_size,
                              const PhiMoeSamplingParams& params) override {
    ...
    PhiMoeLogitsLocation logits_location() const override { return PhiMoeLogitsLocation::HOST; }
...
class TopKSampler final : public PhiMoeISampler {
  public:
...
class TorchCudaMultinomialSampler final : public PhiMoeISampler {
  public:
    ...
    PhiMoeSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        phi_moe_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const PhiMoeTorchMultinomialExecutionPolicy policy =
                phi_moe_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public PhiMoeISampler {
  public:
    ...
    PhiMoeSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        phi_moe_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    PhiMoeLogitsLocation logits_location() const override { return PhiMoeLogitsLocation::DEVICE; }
...
PhiMoeSamplingParams phi_moe_sampling_params_from_config(const GenerateConfig& cfg,
                                                         int32_t default_eos) {
    PhiMoeSamplingParams p;
...
create_phi_moe_sampler(const PhiMoeSamplingParams& params,
                       [[maybe_unused]] const PhiMoeSamplerFactoryOptions& options) {
...
std::unique_ptr<PhiMoeISampler> create_phi_moe_sampler(const PhiMoeSamplingParams& params) {
    return create_phi_moe_sampler(params, PhiMoeSamplerFactoryOptions{});
}

std::unique_ptr<PhiMoeISampler> create_phi_moe_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
