...
// PhiMoeKvCache: autoregressive KV cache state manager.
...
class PhiMoeKvCache : public PhiMoeInferenceState {
  public:
    ...
    PhiMoeKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, PhiMoeKvCacheNames names = {});

    // --- PhiMoeInferenceState overrides ---
    void reset() override;
    ...
    // --- PhiMoeKvCache-specific methods (not on the interface) ---
    ...
    // — too big for PhiMoeKvCache's single-row present buffer. The caller reads
