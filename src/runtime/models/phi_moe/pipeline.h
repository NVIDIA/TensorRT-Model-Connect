...
//
// Composes: TrtModule (decoder) + PhiMoeKvCache + ITokenizer for this runtime
...
class PhiMoeTextGenerationPipeline final : public IPipeline {
  public:
    ...
    PhiMoeTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                 std::unique_ptr<PhiMoeInferenceState> state,
                                 PhiMoeTextGenConfig config, cudaStream_t stream,
                                 std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                 std::string model_id_str = "",
                                 std::unique_ptr<PhiMoeISampler> sampler = nullptr,
                                 std::shared_ptr<void> distributed_owner = nullptr);
    PhiMoeTextGenerationPipeline(std::vector<DecoderContext> decoders,
                                 std::unique_ptr<PhiMoeInferenceState> state,
                                 PhiMoeTextGenConfig config, cudaStream_t stream,
                                 std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                 std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "PhiMoeTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<PhiMoeInferenceState> state_;
    PhiMoeTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<PhiMoeISampler> sampler_;
    ...
    PhiMoeKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(PhiMoeKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(PhiMoeISampler* sampler, const PhiMoeSamplingParams& params,
    ...
    std::unique_ptr<PhiMoeISampler> make_step_sampler(const PhiMoeSamplingParams& params);
