...
#include "runtime/models/phi_moe/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/phi_moe/triattention_kernels.h"
#endif
...
PhiMoeTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return PhiMoeTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return PhiMoeTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(PhiMoeTriAttentionScoreAggregation value) {
    switch (value) {
    case PhiMoeTriAttentionScoreAggregation::kMean:
        return "mean";
    case PhiMoeTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

PhiMoeTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return PhiMoeTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return PhiMoeTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, PhiMoeKvCacheNames& names) {
...
// override helpers that patched PhiMoeTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     PhiMoeTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(PhiMoeTriAttentionConfig& cfg,
...
void fill_debug_from_registry(PhiMoeTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(PhiMoeTriAttentionConfig& cfg,
...
static void validate_triattention_config(const PhiMoeTriAttentionConfig& cfg,
...
PhiMoeTriAttentionConfig
phi_moe_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                         const ::trtmc::config::ConfigBundle* runtime_config) {
    PhiMoeTriAttentionConfig cfg;
...
void fill_stats_header(PhiMoeTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(PhiMoeTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(PhiMoeTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(PhiMoeTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(PhiMoeTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(PhiMoeTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(PhiMoeTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(PhiMoeTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(PhiMoeTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(PhiMoeTriAttentionStats& stats, const std::vector<int32_t>& group_counts,
                           int32_t half_dim) {
    bool any_stats = false;
    for (int32_t layer = 0; layer < stats.num_layers; ++layer) {
        auto& layer_stats = stats.layer_stats[static_cast<std::size_t>(layer)];
...
void parse_sparse_stats(PhiMoeTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
PhiMoeTriAttentionStats phi_moe_parse_triattention_stats_json(const std::string& stats_json,
                                                              int32_t num_attention_heads,
                                                              int32_t num_key_value_heads,
                                                              int32_t num_layers) {
    PhiMoeTriAttentionStats stats;
...
void PhiMoeTriAttentionKvCache::validate_shapes() {
...
void PhiMoeTriAttentionKvCache::normalize_sampled_heads() {
...
void PhiMoeTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void PhiMoeTriAttentionKvCache::allocate_layer_tensors() {
...
PhiMoeTriAttentionKvCache::PhiMoeTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                     int32_t max_length, int32_t kv_dim,
                                                     cudaStream_t stream,
                                                     PhiMoeTriAttentionConfig config,
                                                     PhiMoeTriAttentionStats stats,
                                                     DType cache_dtype, PhiMoeKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void PhiMoeTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void PhiMoeTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void PhiMoeTriAttentionKvCache::initialize_gpu_state() {
...
bool PhiMoeTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool PhiMoeTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool PhiMoeTriAttentionKvCache::can_use_gpu_selection() const {
...
void PhiMoeTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t PhiMoeTriAttentionKvCache::preferred_cache_rows() const {
...
void PhiMoeTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void PhiMoeTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> PhiMoeTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows, PhiMoeTriAttentionCompactionProfile* profile) const {
...
void PhiMoeTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t PhiMoeTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> PhiMoeTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
PhiMoeTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
PhiMoeTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                               PhiMoeTriAttentionCompactionProfile* profile) {
...
int32_t PhiMoeTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t PhiMoeTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> PhiMoeTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void PhiMoeTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, PhiMoeTriAttentionCompactionProfile* profile) const {
...
bool PhiMoeTriAttentionKvCache::layer_stats_shapes_valid(
    const PhiMoeTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void PhiMoeTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                              int32_t half_dim, float& k_rot_real,
                                              float& k_rot_imag) const {
    if (stats_.rope_style == PhiMoeTriAttentionRopeStyle::kInterleaved) {
...
float PhiMoeTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == PhiMoeTriAttentionScoreAggregation::kMax)
...
float PhiMoeTriAttentionKvCache::score_one_row(
    const float* row_ptr, const PhiMoeTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void PhiMoeTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const PhiMoeTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void PhiMoeTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void PhiMoeTriAttentionKvCache::accumulate_layer_fallback(
...
void PhiMoeTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == PhiMoeTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void PhiMoeTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
PhiMoeTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void PhiMoeTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == PhiMoeTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void PhiMoeTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> PhiMoeTriAttentionKvCache::build_keep_indices_per_head(
...
bool PhiMoeTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, PhiMoeTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> PhiMoeTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, PhiMoeTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void PhiMoeTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                       int32_t total_tokens) const {
...
void PhiMoeTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void PhiMoeTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
PhiMoeTriAttentionKvCache::GpuLayerResult
PhiMoeTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = phi_moe_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == PhiMoeTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == PhiMoeTriAttentionScoreAggregation::kMax,
...
bool PhiMoeTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool PhiMoeTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, PhiMoeTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void PhiMoeTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> PhiMoeTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool PhiMoeTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> PhiMoeTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, PhiMoeTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void PhiMoeTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void PhiMoeTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool PhiMoeTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool PhiMoeTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                      std::size_t row_bytes) {
    const bool ok_k = phi_moe_triattention_compact_rows_gpu(
    ...
    const bool ok_v = phi_moe_triattention_compact_rows_gpu(
...
bool PhiMoeTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void PhiMoeTriAttentionKvCache::compact_layer_on_host(
...
void PhiMoeTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    PhiMoeTriAttentionCompactionProfile profile;
    PhiMoeTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void PhiMoeTriAttentionKvCache::finalize_repack_profile(
    PhiMoeTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
PhiMoeTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
PhiMoeTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                     int32_t keep_count) const {
...
int32_t PhiMoeTriAttentionKvCache::count_prefix_positions(
...
void PhiMoeTriAttentionKvCache::log_compact_debug(
...
void PhiMoeTriAttentionKvCache::log_compact_profile(
    const PhiMoeTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool PhiMoeTriAttentionKvCache::should_emit_keep_dump(
    const PhiMoeTriAttentionCompactionProfile* profile_ptr) const {
...
void PhiMoeTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "PhiMoeTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void PhiMoeTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void PhiMoeTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void PhiMoeTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool PhiMoeTriAttentionKvCache::ok() const {
