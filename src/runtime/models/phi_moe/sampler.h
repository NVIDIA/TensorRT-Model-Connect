...
// PhiMoeISampler: token selection abstraction for autoregressive generation.
//
...
//
// PhiMoeLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// phi_moe-owned sampler interface.
class PhiMoeISampler {
  public:
    virtual ~PhiMoeISampler() = default;
    ...
    virtual PhiMoeSampleResult sample(const float* logits, int32_t vocab_size,
                                      const PhiMoeSamplingParams& params) = 0;
    ...
    virtual PhiMoeLogitsLocation logits_location() const = 0;
...
/// Build PhiMoeSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from PhiMoeSamplingParams.
...
std::unique_ptr<PhiMoeISampler> create_phi_moe_sampler(const PhiMoeSamplingParams& params,
                                                       const PhiMoeSamplerFactoryOptions& options);
