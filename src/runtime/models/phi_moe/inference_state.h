...
// PhiMoeInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   PhiMoeKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — PhiMoeKvCache + family-owned recurrent state composed
...
class PhiMoeInferenceState {
  public:
    virtual ~PhiMoeInferenceState() = default;
    ...
    // PhiMoeKvCache -> true. Family-owned recurrent state -> false.
