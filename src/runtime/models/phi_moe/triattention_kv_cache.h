...
#include "runtime/models/phi_moe/inference_state.h"
...
enum class PhiMoeTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class PhiMoeTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct PhiMoeTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    PhiMoeTriAttentionScoreAggregation score_aggregation{PhiMoeTriAttentionScoreAggregation::kMean};
    PhiMoeTriAttentionScoreAggregation per_layer_aggregation{
        PhiMoeTriAttentionScoreAggregation::kMean};
...
struct PhiMoeTriAttentionHeadStats {
...
struct PhiMoeTriAttentionStats {
    int32_t head_dim{0};
    PhiMoeTriAttentionRopeStyle rope_style{PhiMoeTriAttentionRopeStyle::kHalf};
    ...
    std::vector<PhiMoeTriAttentionHeadStats> layer_stats;
};

struct PhiMoeTriAttentionCompactionProfile {
...
// Build a PhiMoeTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class PhiMoeTriAttentionKvCache : public PhiMoeInferenceState {
  public:
    PhiMoeTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                              int32_t kv_dim, cudaStream_t stream, PhiMoeTriAttentionConfig config,
                              PhiMoeTriAttentionStats stats, DType cache_dtype = DType::kFloat32,
                              PhiMoeKvCacheNames names = {});
  ...
  private:
    ...
    void finalize_repack_profile(PhiMoeTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const PhiMoeTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const PhiMoeTriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        PhiMoeTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             PhiMoeTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                PhiMoeTriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const PhiMoeTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const PhiMoeTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const PhiMoeTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    PhiMoeKvCacheNames names_;
    PhiMoeTriAttentionConfig config_;
    PhiMoeTriAttentionStats stats_;
