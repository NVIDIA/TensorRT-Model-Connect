...
// WhisperInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   WhisperKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — WhisperKvCache + family-owned recurrent state composed
...
class WhisperInferenceState {
  public:
    virtual ~WhisperInferenceState() = default;
    ...
    // WhisperKvCache -> true. Family-owned recurrent state -> false.
