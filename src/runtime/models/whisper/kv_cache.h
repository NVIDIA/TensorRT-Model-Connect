...
// WhisperKvCache: autoregressive KV cache state manager.
...
class WhisperKvCache : public WhisperInferenceState {
  public:
    ...
    WhisperKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                   DType cache_dtype = DType::kFloat32, WhisperKvCacheNames names = {});

    // --- WhisperInferenceState overrides ---
    void reset() override;
    ...
    // --- WhisperKvCache-specific methods (not on the interface) ---

    // DEPRECATED: Use prepare_step() instead.
    // Kept for backward compatibility with tests that call this directly.
    void whisper_build_attention_mask(std::vector<float>& mask) const;
    ...
    // — too big for WhisperKvCache's single-row present buffer. The caller reads
