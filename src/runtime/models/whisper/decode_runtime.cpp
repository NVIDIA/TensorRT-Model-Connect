...
int32_t whisper_select_argmax_token(const std::vector<float>& logits) {
    if (logits.empty()) {
        return 0;
    }
...
int32_t whisper_sample_token_topk(const std::vector<float>& logits, float temperature,
    ...
    if (temperature < 1e-6F) {
        return whisper_select_argmax_token(logits);
    }
...
std::vector<int32_t> whisper_select_topk_tokens(const std::vector<float>& logits, int32_t k) {
    if (logits.empty() || k <= 0) {
        return {};
    }
...
std::vector<float> whisper_build_attention_mask(int32_t cache_length, int32_t max_cache_length,
    ...
    std::vector<float> mask(static_cast<std::size_t>(width), WhisperMaskedScore);
