...
// MambaKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/mamba/inference_state.h"
...
class MambaKvCache : public MambaInferenceState {
  public:
    ...
    MambaKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                 DType cache_dtype = DType::kFloat32, MambaKvCacheNames names = {});

    // --- MambaInferenceState overrides ---
    void reset() override;
    ...
    // --- MambaKvCache-specific methods (not on the interface) ---
    ...
    // — too big for MambaKvCache's single-row present buffer. The caller reads
