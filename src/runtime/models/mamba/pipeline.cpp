...
RecurrentPipeline::RecurrentPipeline(std::unique_ptr<TrtModule> decoder,
                                     ...
                                     std::unique_ptr<MambaISampler> sampler)
...
std::vector<int32_t> RecurrentPipeline::generate_from_ids(const std::vector<int32_t>& input_ids,
    ...
    MambaISampler* active_sampler = sampler_.get();
    std::unique_ptr<MambaISampler> local_sampler;
    if (!active_sampler) {
        local_sampler = create_mamba_sampler(params);
