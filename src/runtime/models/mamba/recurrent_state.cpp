...
MambaRecurrentState::MambaRecurrentState(int32_t num_layers, std::vector<TensorSpec> specs,
                                         cudaStream_t stream)
...
void MambaRecurrentState::bind_to(TrtModule& module) {
    for (std::size_t si = 0; si < specs_.size(); ++si) {
...
void MambaRecurrentState::prepare_step(TensorMap& /*inputs*/, int32_t /*seq_len*/) {}

void MambaRecurrentState::advance(int32_t n_tokens) {
    for (std::size_t si = 0; si < specs_.size(); ++si) {
        for (int32_t li = 0; li < num_layers_; ++li) {
            auto uli = static_cast<std::size_t>(li);
...
void MambaRecurrentState::reset() {
    position_ = 0;
    for (std::size_t si = 0; si < specs_.size(); ++si) {
        for (int32_t li = 0; li < num_layers_; ++li) {
            auto uli = static_cast<std::size_t>(li);
...
std::size_t MambaRecurrentState::device_memory_bytes() const {
    std::size_t total = 0;
    for (std::size_t si = 0; si < specs_.size(); ++si) {
...
bool MambaRecurrentState::ok() const {
    for (std::size_t si = 0; si < specs_.size(); ++si) {
