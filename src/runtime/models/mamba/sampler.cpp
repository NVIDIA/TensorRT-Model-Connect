#include "runtime/models/mamba/sampler.h"
...
MambaSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                     int32_t eos_token_id) {
    MambaSampleResult result;
...
bool greedy_equivalent(const MambaSamplingParams& params) {
...
class MambaGreedySampler final : public MambaISampler {
  public:
    MambaSampleResult sample(const float* logits, int32_t vocab_size,
                             const MambaSamplingParams& params) override {
    ...
    MambaLogitsLocation logits_location() const override { return MambaLogitsLocation::HOST; }
    const char* sampler_type() const override { return "mamba_greedy"; }
};

class MambaTopKSampler final : public MambaISampler {
  public:
    explicit MambaTopKSampler(uint64_t initial_seed)
    ...
    MambaLogitsLocation logits_location() const override { return MambaLogitsLocation::HOST; }
    const char* sampler_type() const override { return "mamba_top_k"; }
...
std::unique_ptr<MambaISampler> create_mamba_sampler(const MambaSamplingParams& params) {
    ...
    if (params.top_k <= 1 && top_p >= 1.0F - kSamplingEpsilon && min_p <= 0.0F && params.seed < 0) {
        return std::make_unique<MambaGreedySampler>();
    }

    const uint64_t seed = (params.seed >= 0) ? static_cast<uint64_t>(params.seed) : 42ULL;
    return std::make_unique<MambaTopKSampler>(seed);
}
