...
// MambaInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   MambaKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — MambaKvCache + family-owned recurrent state composed
...
class MambaInferenceState {
  public:
    virtual ~MambaInferenceState() = default;
    ...
    // MambaKvCache -> true. Family-owned recurrent state -> false.
