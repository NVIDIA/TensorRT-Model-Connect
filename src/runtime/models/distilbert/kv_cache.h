...
// DistilbertKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/distilbert/inference_state.h"
...
class DistilbertKvCache : public DistilbertInferenceState {
  public:
    ...
    DistilbertKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                      DType cache_dtype = DType::kFloat32, DistilbertKvCacheNames names = {});

    // --- DistilbertInferenceState overrides ---
    void reset() override;
    ...
    // --- DistilbertKvCache-specific methods (not on the interface) ---
    ...
    // — too big for DistilbertKvCache's single-row present buffer. The caller reads
