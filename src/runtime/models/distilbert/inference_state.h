...
// DistilbertInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   DistilbertKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — DistilbertKvCache + family-owned recurrent state composed
...
class DistilbertInferenceState {
  public:
    virtual ~DistilbertInferenceState() = default;
    ...
    // DistilbertKvCache -> true. Family-owned recurrent state -> false.
