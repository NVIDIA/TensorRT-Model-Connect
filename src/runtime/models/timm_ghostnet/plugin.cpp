...
// TimmGhostnetPlugin: handles timm GhostNet image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_ghostnet/pipeline.h"
...
TimmGhostnetPreprocessConfig make_timm_ghostnet_preprocess_config(const std::string& json) {
    TimmGhostnetPreprocessConfig cfg;
...
class TimmGhostnetPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmGhostnetImageClassificationPipeline>(
            std::move(loaded.module), make_timm_ghostnet_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_ghostnet_plugin, TimmGhostnetPlugin,
