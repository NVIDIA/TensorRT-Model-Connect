#include "runtime/models/wan/kv_cache.h"
...
WanKvCache::WanKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                       DType cache_dtype, WanKvCacheNames names)
...
void WanKvCache::build_attention_mask(std::vector<float>& mask) const {
...
std::vector<int64_t> WanKvCache::mask_shape_for_engine(int32_t mask_width,
...
void WanKvCache::write_position_input(TensorMap& inputs, int32_t seq_len) {
...
void WanKvCache::write_batched_mask(TensorMap& inputs, int32_t seq_len) {
...
void WanKvCache::write_bidirectional_mask(TensorMap& inputs, int32_t seq_len) {
...
void WanKvCache::write_decode_mask(TensorMap& inputs) {
    const int32_t valid = std::max(0, std::min(position_, max_length_));
...
void WanKvCache::prepare_step(TensorMap& inputs, int32_t seq_len) {
    if (seq_len <= 0)
        seq_len = 1;
...
void WanKvCache::prepare_bidirectional_step(TensorMap& inputs, int32_t seq_len) {
    if (seq_len <= 0)
        seq_len = 1;
    write_position_input(inputs, seq_len);
...
void WanKvCache::bind_to(TrtModule& module) {
...
void WanKvCache::bind_cache_inputs(TrtModule& module) {
    has_position_input_ = module.has_input(names_.position_id);
    for (int32_t i = 0; i < num_layers_; ++i) {
        auto li = static_cast<std::size_t>(i);
        module.bind_external(names_.cache_k[li], cache_k_[li].data());
...
void WanKvCache::write_prefill_kv(const std::vector<const void*>& prefill_k,
                                  const std::vector<const void*>& prefill_v, int32_t seq_len) {
    if (seq_len <= 0)
        return;
    if (seq_len > max_length_)
        throw std::runtime_error("WanKvCache::write_prefill_kv: seq_len exceeds max_length");
    if (static_cast<int32_t>(prefill_k.size()) != num_layers_ ||
        static_cast<int32_t>(prefill_v.size()) != num_layers_) {
        throw std::runtime_error("WanKvCache::write_prefill_kv: per-layer pointer count mismatch");
    }
...
void WanKvCache::append_prefill_kv(const std::vector<const void*>& prefill_k,
                                   const std::vector<const void*>& prefill_v, int32_t seq_len) {
    if (seq_len <= 0)
        return;
    if (position_ + seq_len > max_length_)
        throw std::runtime_error("WanKvCache::append_prefill_kv: append exceeds max_length");
    if (static_cast<int32_t>(prefill_k.size()) != num_layers_ ||
        static_cast<int32_t>(prefill_v.size()) != num_layers_) {
        throw std::runtime_error("WanKvCache::append_prefill_kv: per-layer pointer count mismatch");
    }
    const auto row_bytes = static_cast<std::size_t>(kv_dim_) * cache_element_size_;
    const auto block_bytes = static_cast<std::size_t>(seq_len) * row_bytes;
...
std::size_t WanKvCache::device_memory_bytes() const {
    std::size_t total = 0;
