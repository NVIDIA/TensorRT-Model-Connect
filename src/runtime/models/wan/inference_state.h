...
// WanInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   WanKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — WanKvCache + family-owned recurrent state composed
...
class WanInferenceState {
  public:
    virtual ~WanInferenceState() = default;
    ...
    // WanKvCache -> true. Family-owned recurrent state -> false.
