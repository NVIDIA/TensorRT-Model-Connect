...
// WanKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/wan/inference_state.h"
...
class WanKvCache : public WanInferenceState {
  public:
    ...
    WanKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
               DType cache_dtype = DType::kFloat32, WanKvCacheNames names = {});

    // --- WanInferenceState overrides ---
    void reset() override;
    ...
    // --- WanKvCache-specific methods (not on the interface) ---
    ...
    // — too big for WanKvCache's single-row present buffer. The caller reads
