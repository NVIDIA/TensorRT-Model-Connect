...
void load_preprocessor_weights(const std::string& index_json, const char* blob,
    ...
    wan_preprocessor_weights::load_with_fallback(index_json, blob, blob_size,
                                                 "patch_embedding.weight", "x_embedder.weight",
                                                 w.patch_embed_weight);
    wan_preprocessor_weights::load_with_fallback(
...
WanPreprocessorWeights parse_preprocessor_weights(const std::vector<char>& data) {
    ...
    if (!wan_preprocessor_weights::extract_preprocessor_index(data, index_json, blob, blob_size)) {
        return w;
    }
