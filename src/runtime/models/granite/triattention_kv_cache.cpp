...
#include "runtime/models/granite/triattention_kernels.h"
#endif
...
GraniteTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return GraniteTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return GraniteTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(GraniteTriAttentionScoreAggregation value) {
    switch (value) {
    case GraniteTriAttentionScoreAggregation::kMean:
        return "mean";
    case GraniteTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

GraniteTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return GraniteTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return GraniteTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, GraniteKvCacheNames& names) {
...
// override helpers that patched GraniteTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     GraniteTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(GraniteTriAttentionConfig& cfg,
...
void fill_debug_from_registry(GraniteTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(GraniteTriAttentionConfig& cfg,
...
static void validate_triattention_config(const GraniteTriAttentionConfig& cfg,
...
GraniteTriAttentionConfig
granite_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                         const ::trtmc::config::ConfigBundle* runtime_config) {
    GraniteTriAttentionConfig cfg;
...
void fill_stats_header(GraniteTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(GraniteTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(GraniteTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(GraniteTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(GraniteTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(GraniteTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(GraniteTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(GraniteTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(GraniteTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(GraniteTriAttentionStats& stats,
...
void parse_sparse_stats(GraniteTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
GraniteTriAttentionStats granite_parse_triattention_stats_json(const std::string& stats_json,
                                                               int32_t num_attention_heads,
                                                               int32_t num_key_value_heads,
                                                               int32_t num_layers) {
    GraniteTriAttentionStats stats;
...
void GraniteTriAttentionKvCache::validate_shapes() {
...
void GraniteTriAttentionKvCache::normalize_sampled_heads() {
...
void GraniteTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void GraniteTriAttentionKvCache::allocate_layer_tensors() {
...
GraniteTriAttentionKvCache::GraniteTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                       int32_t max_length, int32_t kv_dim,
                                                       cudaStream_t stream,
                                                       GraniteTriAttentionConfig config,
                                                       GraniteTriAttentionStats stats,
                                                       DType cache_dtype, GraniteKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void GraniteTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void GraniteTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void GraniteTriAttentionKvCache::initialize_gpu_state() {
...
bool GraniteTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool GraniteTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool GraniteTriAttentionKvCache::can_use_gpu_selection() const {
...
void GraniteTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t GraniteTriAttentionKvCache::preferred_cache_rows() const {
...
void GraniteTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void GraniteTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> GraniteTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows, GraniteTriAttentionCompactionProfile* profile) const {
...
void GraniteTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t GraniteTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> GraniteTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
GraniteTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
GraniteTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                                GraniteTriAttentionCompactionProfile* profile) {
...
int32_t GraniteTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t GraniteTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> GraniteTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void GraniteTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, GraniteTriAttentionCompactionProfile* profile) const {
...
bool GraniteTriAttentionKvCache::layer_stats_shapes_valid(
    const GraniteTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void GraniteTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                               int32_t half_dim, float& k_rot_real,
                                               float& k_rot_imag) const {
    if (stats_.rope_style == GraniteTriAttentionRopeStyle::kInterleaved) {
...
float GraniteTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == GraniteTriAttentionScoreAggregation::kMax)
...
float GraniteTriAttentionKvCache::score_one_row(
    const float* row_ptr, const GraniteTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void GraniteTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const GraniteTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void GraniteTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void GraniteTriAttentionKvCache::accumulate_layer_fallback(
...
void GraniteTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == GraniteTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void GraniteTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
GraniteTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void GraniteTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == GraniteTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void GraniteTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> GraniteTriAttentionKvCache::build_keep_indices_per_head(
...
bool GraniteTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, GraniteTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> GraniteTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, GraniteTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void GraniteTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                        int32_t total_tokens) const {
...
void GraniteTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void GraniteTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
GraniteTriAttentionKvCache::GpuLayerResult
GraniteTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = granite_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == GraniteTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == GraniteTriAttentionScoreAggregation::kMax,
...
bool GraniteTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool GraniteTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, GraniteTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void GraniteTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> GraniteTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool GraniteTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> GraniteTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, GraniteTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void GraniteTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void GraniteTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool GraniteTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool GraniteTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                       std::size_t row_bytes) {
    const bool ok_k = granite_triattention_compact_rows_gpu(
    ...
    const bool ok_v = granite_triattention_compact_rows_gpu(
...
bool GraniteTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void GraniteTriAttentionKvCache::compact_layer_on_host(
...
void GraniteTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    GraniteTriAttentionCompactionProfile profile;
    GraniteTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void GraniteTriAttentionKvCache::finalize_repack_profile(
    GraniteTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
GraniteTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
GraniteTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                      int32_t keep_count) const {
...
int32_t GraniteTriAttentionKvCache::count_prefix_positions(
...
void GraniteTriAttentionKvCache::log_compact_debug(
...
void GraniteTriAttentionKvCache::log_compact_profile(
    const GraniteTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool GraniteTriAttentionKvCache::should_emit_keep_dump(
    const GraniteTriAttentionCompactionProfile* profile_ptr) const {
...
void GraniteTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "GraniteTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void GraniteTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void GraniteTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void GraniteTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool GraniteTriAttentionKvCache::ok() const {
