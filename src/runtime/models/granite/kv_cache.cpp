...
void validate_native_scalar_input(TrtModule& module, const std::string& name) {
    if (!module.has_input(name) || module.tensor_dtype(name) != DType::kInt32 ||
        ...
        throw std::runtime_error("Granite native KV input '" + name + "' must be int32 [1]");
    }
}
...
void validate_native_cache_pair(TrtModule& module, const std::string& cache_name,
    ...
    if (!module.has_input(cache_name) || !module.has_output(present_name)) {
        throw std::runtime_error("Granite native KV engine is missing cache/present pair '" +
    ...
    if (!valid_native_cache_shape(cache_shape, max_length, kv_dim) ||
        ...
        throw std::runtime_error("Granite native KV cache/present tensors must share static "
                                 "[1,Hkv,max_length,D] shape");
    }
    if (module.tensor_dtype(cache_name) != cache_dtype ||
        module.tensor_dtype(present_name) != cache_dtype) {
        throw std::runtime_error("Granite native KV cache dtype does not match model precision");
    }
}
...
GraniteKvCacheNames default_cache_names(int32_t num_layers) {
...
void validate_cache_name_count(const GraniteKvCacheNames& names, std::size_t expected) {
    if (names.cache_k.size() != expected || names.cache_v.size() != expected ||
        names.present_k.size() != expected || names.present_v.size() != expected) {
        throw std::invalid_argument("Granite native KV per-layer tensor name count mismatch");
    }
}
...
GraniteKvCache::GraniteKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim,
                               cudaStream_t stream, DType cache_dtype, GraniteKvCacheNames names)
    ...
    if (num_layers <= 0 || max_length <= 0 || kv_dim <= 0)
        throw std::invalid_argument("Granite native KV geometry must be positive");
    if (cache_dtype != DType::kFloat16)
        throw std::invalid_argument("Granite native KV cache requires FP16 storage");
...
void GraniteKvCache::validate_native_kv_contract(TrtModule& module) const {
...
void GraniteKvCache::bind_native_cache(TrtModule& module) {
    for (int32_t layer = 0; layer < num_layers_; ++layer) {
        const auto index = static_cast<std::size_t>(layer);
        ...
        if (module.device_ptr(names_.cache_k[index]) != cache_k_[index].data() ||
            ...
            throw std::runtime_error(
                "Granite native KV engine did not preserve cache/present aliasing");
        }
    }
}

void GraniteKvCache::bind_to(TrtModule& module) {
    validate_native_kv_contract(module);
    has_position_input_ = module.has_input(names_.position_id);
    bind_native_cache(module);
}

void GraniteKvCache::bind_cache_inputs(TrtModule& module) {
    bind_to(module);
}

void GraniteKvCache::write_position_input(TensorMap& inputs, int32_t seq_len) {
...
void GraniteKvCache::write_native_kv_inputs(TensorMap& inputs, int32_t seq_len) {
    if (seq_len <= 0)
        throw std::invalid_argument("Granite native KV sequence length must be positive");
    if (position_ > max_length_ - seq_len)
        throw std::runtime_error("Granite sequence exceeds the model's KV cache capacity");
...
void GraniteKvCache::prepare_step(TensorMap& inputs, int32_t seq_len) {
    write_position_input(inputs, seq_len);
    write_native_kv_inputs(inputs, seq_len);
}

void GraniteKvCache::validate_native_aliases(const std::vector<const void*>& present_k,
                                             const std::vector<const void*>& present_v) const {
    if (present_k.size() != cache_k_.size() || present_v.size() != cache_v_.size())
        throw std::runtime_error("Granite native KV per-layer pointer count mismatch");
    for (std::size_t layer = 0; layer < cache_k_.size(); ++layer) {
        if (present_k[layer] != cache_k_[layer].data() ||
            ...
            throw std::runtime_error(
                "Granite native prefill present tensors must alias the KV cache");
        }
    }
}

void GraniteKvCache::append_prefill_kv(const std::vector<const void*>& prefill_k,
                                       const std::vector<const void*>& prefill_v, int32_t seq_len) {
    validate_native_aliases(prefill_k, prefill_v);
    advance(seq_len);
}

void GraniteKvCache::set_position(int32_t position) {
    if (position < 0 || position > max_length_)
        throw std::out_of_range("Granite native KV position is outside cache capacity");
    position_ = position;
}

void GraniteKvCache::advance(int32_t n_tokens) {
    if (n_tokens <= 0)
        throw std::invalid_argument("Granite native KV advance must be positive");
    if (position_ > max_length_ - n_tokens)
        throw std::runtime_error("Granite sequence exceeds the model's KV cache capacity");
    position_ += n_tokens;
}
