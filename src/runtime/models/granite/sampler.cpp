...
// GraniteISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// GraniteTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/granite/argmax_kernel.h"
#include "runtime/models/granite/sparse_multinomial_kernel.h"
...
static GraniteSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                              int32_t eos_token_id) {
    GraniteSampleResult result;
...
static bool greedy_equivalent(const GraniteSamplingParams& params) {
...
class GreedySampler final : public GraniteISampler {
  public:
    GraniteSampleResult sample(const float* logits, int32_t vocab_size,
                               const GraniteSamplingParams& params) override {
    ...
    GraniteLogitsLocation logits_location() const override { return GraniteLogitsLocation::HOST; }
...
class TopKSampler final : public GraniteISampler {
  public:
...
class TorchCudaMultinomialSampler final : public GraniteISampler {
  public:
    ...
    GraniteSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        granite_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const GraniteTorchMultinomialExecutionPolicy policy =
                granite_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public GraniteISampler {
  public:
    ...
    GraniteSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        granite_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    GraniteLogitsLocation logits_location() const override { return GraniteLogitsLocation::DEVICE; }
...
GraniteSamplingParams granite_sampling_params_from_config(const GenerateConfig& cfg,
                                                          int32_t default_eos) {
    GraniteSamplingParams p;
...
create_granite_sampler(const GraniteSamplingParams& params,
                       [[maybe_unused]] const GraniteSamplerFactoryOptions& options) {
...
std::unique_ptr<GraniteISampler> create_granite_sampler(const GraniteSamplingParams& params) {
    return create_granite_sampler(params, GraniteSamplerFactoryOptions{});
}

std::unique_ptr<GraniteISampler> create_granite_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
