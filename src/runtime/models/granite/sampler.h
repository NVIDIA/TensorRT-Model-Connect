...
// GraniteISampler: token selection abstraction for autoregressive generation.
//
...
//
// GraniteLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// granite-owned sampler interface.
class GraniteISampler {
  public:
    virtual ~GraniteISampler() = default;
    ...
    virtual GraniteSampleResult sample(const float* logits, int32_t vocab_size,
                                       const GraniteSamplingParams& params) = 0;
    ...
    virtual GraniteLogitsLocation logits_location() const = 0;
...
/// Build GraniteSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from GraniteSamplingParams.
...
create_granite_sampler(const GraniteSamplingParams& params,
                       const GraniteSamplerFactoryOptions& options);
