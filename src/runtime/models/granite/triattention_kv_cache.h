...
#include "runtime/models/granite/inference_state.h"
...
enum class GraniteTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class GraniteTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct GraniteTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    GraniteTriAttentionScoreAggregation score_aggregation{
        GraniteTriAttentionScoreAggregation::kMean};
    GraniteTriAttentionScoreAggregation per_layer_aggregation{
        GraniteTriAttentionScoreAggregation::kMean};
...
struct GraniteTriAttentionHeadStats {
...
struct GraniteTriAttentionStats {
    int32_t head_dim{0};
    GraniteTriAttentionRopeStyle rope_style{GraniteTriAttentionRopeStyle::kHalf};
    ...
    std::vector<GraniteTriAttentionHeadStats> layer_stats;
};

struct GraniteTriAttentionCompactionProfile {
...
// Build a GraniteTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class GraniteTriAttentionKvCache : public GraniteInferenceState {
  public:
    GraniteTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                               int32_t kv_dim, cudaStream_t stream,
                               GraniteTriAttentionConfig config, GraniteTriAttentionStats stats,
                               DType cache_dtype = DType::kFloat32, GraniteKvCacheNames names = {});
  ...
  private:
    ...
    void finalize_repack_profile(GraniteTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const GraniteTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const GraniteTriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        GraniteTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             GraniteTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                GraniteTriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const GraniteTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const GraniteTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const GraniteTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    GraniteKvCacheNames names_;
    GraniteTriAttentionConfig config_;
    GraniteTriAttentionStats stats_;
