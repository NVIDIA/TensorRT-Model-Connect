...
std::vector<int32_t> top_logit_indices(const std::vector<float>& logits, int32_t top_n) {
    ...
    if (top_n == static_cast<int32_t>(logits.size())) {
        const auto best = std::max_element(logits.begin(), logits.end()) - logits.begin();
        std::iter_swap(order.begin(), order.begin() + best);
        return order;
    }
    std::partial_sort(
...
void write_step_trace_line(std::ostream& out, int32_t position_before, int32_t token_id,
                           ...
                           int32_t top_n, const char* phase) {
    out << std::setprecision(std::numeric_limits<float>::max_digits10);
    out << "{\"phase\":\"" << phase << "\",\"position_before\":" << position_before
        << ",\"token_id\":" << token_id << ",\"decoder_idx\":" << decoder_idx
        << ",\"rows_before\":" << rows_before << ",\"rows_after\":" << rows_after
        << ",\"argmax_token\":" << order.front()
...
void maybe_append_step_trace(int32_t position_before, int32_t token_id, int32_t decoder_idx,
                             ...
                             const std::vector<float>& logits, const char* phase) {
...
GraniteTextGenerationPipeline::GraniteTextGenerationPipeline(
    std::vector<DecoderContext> decoders, std::unique_ptr<GraniteKvCache> state,
    ...
    std::unique_ptr<TrtModule> prefill, std::shared_ptr<void> distributed_owner)
    ...
    if (decoders_.size() != 1 || !decoders_.front().module || !decoders_.front().module->ok()) {
        throw std::runtime_error(
            "GraniteTextGenerationPipeline: native KV requires one decode module");
    }
    if (!state_ || !state_->ok()) {
    ...
    if (!config_.disable_cuda_graph) {
        decoders_.front().module->enable_cuda_graph();
    }
...
// Gather the aliased per-layer present_k/present_v device pointers from the
// required split prefill engine.
namespace {
void gather_prefill_kv_pointers(TrtModule& prefill, const GraniteTextGenConfig& cfg,
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        if (pk[li] == nullptr || pv[li] == nullptr) {
            throw std::runtime_error(
                "Granite native prefill engine is missing aliased present K/V outputs");
        }
    }
}

int32_t resolve_prefill_chunk_limit(const GraniteTextGenConfig& cfg) {
    if (cfg.prefill_max_length <= 0)
        throw std::runtime_error("Granite native KV prefill engine has no valid profile capacity");
...
void validate_generation_capacity(const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
    ...
    if (input_ids.size() > capacity ||
        ...
        throw std::runtime_error(
            "Granite requested prompt and generation exceed the model's fixed KV cache capacity");
    }
}
} // namespace

void GraniteTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
                                                        std::vector<float>& logits,
                                                        bool retain_device_logits) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    if (prefill_ == nullptr || sq <= 0 || config_.num_layers <= 0 || config_.vocab_size <= 0)
        throw std::runtime_error("Granite native KV requires a valid split prefill engine");
    auto* kv = state_.get();
    ...
    if (sq > kv->max_length()) {
        throw std::runtime_error("Granite sequence exceeds the model's fixed KV cache capacity");
    }

    std::vector<const void*> pk, pv;
    gather_prefill_kv_pointers(*prefill_, config_, pk, pv);
...
void GraniteTextGenerationPipeline::run_prefill_chunk(const int32_t* token_ids, int32_t chunk_size,
    ...
    if (logits_it == outputs.end()) {
        throw std::runtime_error(
            "GraniteTextGenerationPipeline: prefill module has no logits output");
    }
    ...
    if (static_cast<std::size_t>(logits_tensor.numel()) < vocab) {
        throw std::runtime_error(
            "GraniteTextGenerationPipeline: prefill logits are smaller than vocabulary");
    }
    ...
    if (retain_device_logits) {
        ...
        if (device_logits == nullptr) {
            throw std::runtime_error(
                "GraniteTextGenerationPipeline: prefill logits have no device buffer");
        }
...
void GraniteTextGenerationPipeline::log_batched_prefill(int32_t token_count, int32_t chunk_count,
...
void GraniteTextGenerationPipeline::run_prefill(const std::vector<int32_t>& input_ids,
    ...
    if (step_trace_config().enabled) {
        // Parity diagnostics intentionally use the native decode engine one
        // token at a time so every prompt position has a logit row. This does
        // not reintroduce the retired mask/present-copy KV implementation.
        trace_prefill_phase_ = true;
        for (const int32_t token_id : input_ids)
            run_step(token_id, logits);
        trace_prefill_phase_ = false;
        return;
    }
...
void GraniteTextGenerationPipeline::reset_generation_context() {
    ...
    state_bound_ = false;
    decoders_.front().module->reset_execution_context();
    if (prefill_)
...
void GraniteTextGenerationPipeline::run_step(int32_t token_id, std::vector<float>& logits) {
    ...
    state_->advance();
    maybe_append_step_trace(position_before, token_id, 0, position_before, state_->position(),
                            logits, trace_prefill_phase_ ? "prefill" : "decode");
}
