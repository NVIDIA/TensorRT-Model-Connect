...
// GraniteKvCache owns the full-capacity buffers consumed by TensorRT's native
// KV update layers. Cache inputs and present outputs alias the same storage;
// there is intentionally no legacy append/copy fallback.
...
class GraniteKvCache {
  public:
    GraniteKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                   DType cache_dtype = DType::kFloat16, GraniteKvCacheNames names = {});

    void reset();
    void bind_to(TrtModule& module);
    void prepare_step(TensorMap& inputs, int32_t seq_len = 1);
    void advance(int32_t n_tokens = 1);
