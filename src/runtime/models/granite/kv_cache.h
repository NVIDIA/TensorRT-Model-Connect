...
// GraniteKvCache: autoregressive KV cache state manager.
...
class GraniteKvCache : public GraniteInferenceState {
  public:
    ...
    GraniteKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                   DType cache_dtype = DType::kFloat32, GraniteKvCacheNames names = {});

    // --- GraniteInferenceState overrides ---
    void reset() override;
    ...
    // --- GraniteKvCache-specific methods (not on the interface) ---
    ...
    // — too big for GraniteKvCache's single-row present buffer. The caller reads
