...
// DprInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   DprKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — DprKvCache + family-owned recurrent state composed
...
class DprInferenceState {
  public:
    virtual ~DprInferenceState() = default;
    ...
    // DprKvCache -> true. Family-owned recurrent state -> false.
