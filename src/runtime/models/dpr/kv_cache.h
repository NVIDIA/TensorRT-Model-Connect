...
// DprKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/dpr/inference_state.h"
...
class DprKvCache : public DprInferenceState {
  public:
    ...
    DprKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
               DType cache_dtype = DType::kFloat32, DprKvCacheNames names = {});

    // --- DprInferenceState overrides ---
    void reset() override;
    ...
    // --- DprKvCache-specific methods (not on the interface) ---
    ...
    // — too big for DprKvCache's single-row present buffer. The caller reads
