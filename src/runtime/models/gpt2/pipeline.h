...
//
// Composes: TrtModule (decoder) + Gpt2KvCache + ITokenizer for this runtime
...
class Gpt2TextGenerationPipeline final : public IPipeline {
  public:
    ...
    Gpt2TextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                               std::unique_ptr<Gpt2InferenceState> state, Gpt2TextGenConfig config,
                               cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer = nullptr,
                               std::string model_id_str = "",
                               std::unique_ptr<Gpt2ISampler> sampler = nullptr,
                               std::shared_ptr<void> distributed_owner = nullptr);
    Gpt2TextGenerationPipeline(std::vector<DecoderContext> decoders,
                               std::unique_ptr<Gpt2InferenceState> state, Gpt2TextGenConfig config,
                               cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer = nullptr,
                               std::string model_id_str = "",
                               std::unique_ptr<Gpt2ISampler> sampler = nullptr,
    ...
    const char* pipeline_type() const override { return "Gpt2TextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<Gpt2InferenceState> state_;
    Gpt2TextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<Gpt2ISampler> sampler_;
    ...
    TimedGenResult generate_from_ids(const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
                                     const Gpt2SamplingParams& params, const GenerateConfig& cfg);
    ...
    Gpt2KvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(Gpt2KvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(Gpt2ISampler* sampler, const Gpt2SamplingParams& params,
    ...
    std::unique_ptr<Gpt2ISampler> make_step_sampler(const Gpt2SamplingParams& params);
