...
#include "runtime/models/gpt2/inference_state.h"
...
enum class Gpt2TriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class Gpt2TriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct Gpt2TriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    Gpt2TriAttentionScoreAggregation score_aggregation{Gpt2TriAttentionScoreAggregation::kMean};
    Gpt2TriAttentionScoreAggregation per_layer_aggregation{Gpt2TriAttentionScoreAggregation::kMean};
...
struct Gpt2TriAttentionHeadStats {
...
struct Gpt2TriAttentionStats {
    int32_t head_dim{0};
    Gpt2TriAttentionRopeStyle rope_style{Gpt2TriAttentionRopeStyle::kHalf};
    ...
    std::vector<Gpt2TriAttentionHeadStats> layer_stats;
};

struct Gpt2TriAttentionCompactionProfile {
...
// Build a Gpt2TriAttentionConfig from the bundle's JSON plus (optionally) the
...
class Gpt2TriAttentionKvCache : public Gpt2InferenceState {
  public:
    Gpt2TriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                            int32_t kv_dim, cudaStream_t stream, Gpt2TriAttentionConfig config,
                            Gpt2TriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(Gpt2TriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const Gpt2TriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const Gpt2TriAttentionCompactionProfile* profile_ptr) const;
    ...
    std::vector<int32_t> select_keep_indices(int32_t keep_budget,
                                             Gpt2TriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             Gpt2TriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const Gpt2TriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const Gpt2TriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const Gpt2TriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    Gpt2KvCacheNames names_;
    Gpt2TriAttentionConfig config_;
    Gpt2TriAttentionStats stats_;
