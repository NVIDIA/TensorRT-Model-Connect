...
// Gpt2KvCache: autoregressive KV cache state manager.
...
class Gpt2KvCache : public Gpt2InferenceState {
  public:
    ...
    Gpt2KvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                DType cache_dtype = DType::kFloat32, Gpt2KvCacheNames names = {});

    // --- Gpt2InferenceState overrides ---
    void reset() override;
    ...
    // --- Gpt2KvCache-specific methods (not on the interface) ---
    ...
    // — too big for Gpt2KvCache's single-row present buffer. The caller reads
