// Gpt2ISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// Gpt2TextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/gpt2/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/gpt2/argmax_kernel.h"
#include "runtime/models/gpt2/sparse_multinomial_kernel.h"
...
static Gpt2SampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                           int32_t eos_token_id) {
    Gpt2SampleResult result;
...
static bool greedy_equivalent(const Gpt2SamplingParams& params) {
...
class GreedySampler final : public Gpt2ISampler {
  public:
    Gpt2SampleResult sample(const float* logits, int32_t vocab_size,
                            const Gpt2SamplingParams& params) override {
    ...
    Gpt2LogitsLocation logits_location() const override { return Gpt2LogitsLocation::HOST; }
...
class TopKSampler final : public Gpt2ISampler {
  public:
...
class TorchCudaMultinomialSampler final : public Gpt2ISampler {
  public:
    ...
    Gpt2SampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        gpt2_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const Gpt2TorchMultinomialExecutionPolicy policy =
                gpt2_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public Gpt2ISampler {
  public:
    ...
    Gpt2SampleResult sample(const float* logits, int32_t vocab_size,
        ...
        gpt2_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    Gpt2LogitsLocation logits_location() const override { return Gpt2LogitsLocation::DEVICE; }
...
create_gpt2_sampler(const Gpt2SamplingParams& params,
                    [[maybe_unused]] const Gpt2SamplerFactoryOptions& options) {
...
std::unique_ptr<Gpt2ISampler> create_gpt2_sampler(const Gpt2SamplingParams& params) {
    return create_gpt2_sampler(params, Gpt2SamplerFactoryOptions{});
}
