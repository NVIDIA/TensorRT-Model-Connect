...
// FluxKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/flux/inference_state.h"
...
class FluxKvCache : public FluxInferenceState {
  public:
    ...
    FluxKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                DType cache_dtype = DType::kFloat32, FluxKvCacheNames names = {});

    // --- FluxInferenceState overrides ---
    void reset() override;
    ...
    // --- FluxKvCache-specific methods (not on the interface) ---
    ...
    // — too big for FluxKvCache's single-row present buffer. The caller reads
