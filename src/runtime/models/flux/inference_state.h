...
// FluxInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   FluxKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — FluxKvCache + family-owned recurrent state composed
...
class FluxInferenceState {
  public:
    virtual ~FluxInferenceState() = default;
    ...
    // FluxKvCache -> true. Family-owned recurrent state -> false.
