...
// M2m100KvCache: autoregressive KV cache state manager.
...
#include "runtime/models/m2m_100/inference_state.h"
...
class M2m100KvCache : public M2m100InferenceState {
  public:
    ...
    M2m100KvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, M2m100KvCacheNames names = {});

    // --- M2m100InferenceState overrides ---
    void reset() override;
    ...
    // --- M2m100KvCache-specific methods (not on the interface) ---

    // DEPRECATED: Use prepare_step() instead.
    // Kept for backward compatibility with tests that call this directly.
    void m2m_100_build_attention_mask(std::vector<float>& mask) const;
    ...
    // — too big for M2m100KvCache's single-row present buffer. The caller reads
