...
// TimmVitInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   TimmVitKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — TimmVitKvCache + family-owned recurrent state composed
...
class TimmVitInferenceState {
  public:
    virtual ~TimmVitInferenceState() = default;
    ...
    // TimmVitKvCache -> true. Family-owned recurrent state -> false.
