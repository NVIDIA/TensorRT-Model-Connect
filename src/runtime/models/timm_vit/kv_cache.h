...
// TimmVitKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/timm_vit/inference_state.h"
...
class TimmVitKvCache : public TimmVitInferenceState {
  public:
    ...
    TimmVitKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                   DType cache_dtype = DType::kFloat32, TimmVitKvCacheNames names = {});

    // --- TimmVitInferenceState overrides ---
    void reset() override;
    ...
    // --- TimmVitKvCache-specific methods (not on the interface) ---
    ...
    // — too big for TimmVitKvCache's single-row present buffer. The caller reads
