...
// Qwen38InferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   Qwen38KvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — Qwen38KvCache + family-owned recurrent state composed
...
class Qwen38InferenceState {
  public:
    virtual ~Qwen38InferenceState() = default;
    ...
    // Qwen38KvCache -> true. Family-owned recurrent state -> false.
