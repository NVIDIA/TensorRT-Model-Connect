...
// Qwen38KvCache: autoregressive KV cache state manager.
...
#include "runtime/models/qwen3_8/inference_state.h"
...
class Qwen38KvCache : public Qwen38InferenceState {
  public:
    ...
    Qwen38KvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, Qwen38KvCacheNames names = {});

    // --- Qwen38InferenceState overrides ---
    void reset() override;
    ...
    // --- Qwen38KvCache-specific methods (not on the interface) ---
    ...
    // — too big for Qwen38KvCache's single-row present buffer. The caller reads
