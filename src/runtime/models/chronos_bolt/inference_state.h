...
// ChronosBoltInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   ChronosBoltKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — ChronosBoltKvCache + family-owned recurrent state composed
...
class ChronosBoltInferenceState {
  public:
    virtual ~ChronosBoltInferenceState() = default;
    ...
    // ChronosBoltKvCache -> true. Family-owned recurrent state -> false.
