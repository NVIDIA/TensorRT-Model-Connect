...
// ChronosBoltKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/chronos_bolt/inference_state.h"
...
struct ChronosBoltKvCacheNames {
...
class ChronosBoltKvCache : public ChronosBoltInferenceState {
  public:
    ...
    ChronosBoltKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                       DType cache_dtype = DType::kFloat32, ChronosBoltKvCacheNames names = {});

    // --- ChronosBoltInferenceState overrides ---
    void reset() override;
    ...
    // --- ChronosBoltKvCache-specific methods (not on the interface) ---
    ...
    // — too big for ChronosBoltKvCache's single-row present buffer. The caller reads
  ...
  private:
    ...
    ChronosBoltKvCacheNames names_;
