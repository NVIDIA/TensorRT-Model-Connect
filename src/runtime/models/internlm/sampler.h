...
// InternlmISampler: token selection abstraction for autoregressive generation.
//
...
//
// InternlmLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// internlm-owned sampler interface.
class InternlmISampler {
  public:
    virtual ~InternlmISampler() = default;
    ...
    virtual InternlmSampleResult sample(const float* logits, int32_t vocab_size,
                                        const InternlmSamplingParams& params) = 0;
    ...
    virtual InternlmLogitsLocation logits_location() const = 0;
...
/// Build InternlmSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from InternlmSamplingParams.
...
create_internlm_sampler(const InternlmSamplingParams& params,
                        const InternlmSamplerFactoryOptions& options);
