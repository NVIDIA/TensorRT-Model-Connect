...
class InternlmTextGenerationPipeline final : public IPipeline {
  public:
    ...
    InternlmTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
  ...
  private:
    ...
    InternlmKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(InternlmKvCache& kv, TrtModule& prefill, int32_t sq);
