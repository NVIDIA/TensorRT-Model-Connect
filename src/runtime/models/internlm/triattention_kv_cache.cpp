...
#include "runtime/models/internlm/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/internlm/triattention_kernels.h"
#endif
...
InternlmTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return InternlmTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return InternlmTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(InternlmTriAttentionScoreAggregation value) {
    switch (value) {
    case InternlmTriAttentionScoreAggregation::kMean:
        return "mean";
    case InternlmTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

InternlmTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return InternlmTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return InternlmTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, InternlmKvCacheNames& names) {
...
// override helpers that patched InternlmTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     InternlmTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(InternlmTriAttentionConfig& cfg,
...
void fill_debug_from_registry(InternlmTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(InternlmTriAttentionConfig& cfg,
...
static void validate_triattention_config(const InternlmTriAttentionConfig& cfg,
...
InternlmTriAttentionConfig
internlm_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                          const ::trtmc::config::ConfigBundle* runtime_config) {
    InternlmTriAttentionConfig cfg;
...
void fill_stats_header(InternlmTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(InternlmTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(InternlmTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(InternlmTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(InternlmTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(InternlmTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(InternlmTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(InternlmTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(InternlmTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(InternlmTriAttentionStats& stats,
...
void parse_sparse_stats(InternlmTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
InternlmTriAttentionStats internlm_parse_triattention_stats_json(const std::string& stats_json,
                                                                 int32_t num_attention_heads,
                                                                 int32_t num_key_value_heads,
                                                                 int32_t num_layers) {
    InternlmTriAttentionStats stats;
...
void InternlmTriAttentionKvCache::validate_shapes() {
...
void InternlmTriAttentionKvCache::normalize_sampled_heads() {
...
void InternlmTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void InternlmTriAttentionKvCache::allocate_layer_tensors() {
...
InternlmTriAttentionKvCache::InternlmTriAttentionKvCache(
    int32_t num_layers, int32_t num_kv_heads, int32_t max_length, int32_t kv_dim,
    cudaStream_t stream, InternlmTriAttentionConfig config, InternlmTriAttentionStats stats,
    DType cache_dtype, InternlmKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void InternlmTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void InternlmTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void InternlmTriAttentionKvCache::initialize_gpu_state() {
...
bool InternlmTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool InternlmTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool InternlmTriAttentionKvCache::can_use_gpu_selection() const {
...
void InternlmTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t InternlmTriAttentionKvCache::preferred_cache_rows() const {
...
void InternlmTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void InternlmTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> InternlmTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows,
    InternlmTriAttentionCompactionProfile* profile) const {
...
void InternlmTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t InternlmTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> InternlmTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
InternlmTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
InternlmTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                                 InternlmTriAttentionCompactionProfile* profile) {
...
int32_t InternlmTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t InternlmTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> InternlmTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void InternlmTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, InternlmTriAttentionCompactionProfile* profile) const {
...
bool InternlmTriAttentionKvCache::layer_stats_shapes_valid(
    const InternlmTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void InternlmTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset,
                                                int32_t d, int32_t half_dim, float& k_rot_real,
                                                float& k_rot_imag) const {
    if (stats_.rope_style == InternlmTriAttentionRopeStyle::kInterleaved) {
...
float InternlmTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == InternlmTriAttentionScoreAggregation::kMax)
...
float InternlmTriAttentionKvCache::score_one_row(
    const float* row_ptr, const InternlmTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void InternlmTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const InternlmTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void InternlmTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void InternlmTriAttentionKvCache::accumulate_layer_fallback(
...
void InternlmTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max =
        config_.per_layer_aggregation == InternlmTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void InternlmTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
InternlmTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void InternlmTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
...
void InternlmTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> InternlmTriAttentionKvCache::build_keep_indices_per_head(
...
bool InternlmTriAttentionKvCache::process_layer_for_host_selection(
...
std::vector<int32_t> InternlmTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, InternlmTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void InternlmTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                         int32_t total_tokens) const {
...
void InternlmTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void InternlmTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
InternlmTriAttentionKvCache::GpuLayerResult
InternlmTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = internlm_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == InternlmTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == InternlmTriAttentionScoreAggregation::kMax,
...
bool InternlmTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool InternlmTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, InternlmTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void InternlmTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> InternlmTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool InternlmTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> InternlmTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, InternlmTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void InternlmTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void InternlmTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool InternlmTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool InternlmTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                        std::size_t row_bytes) {
    const bool ok_k = internlm_triattention_compact_rows_gpu(
    ...
    const bool ok_v = internlm_triattention_compact_rows_gpu(
...
bool InternlmTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void InternlmTriAttentionKvCache::compact_layer_on_host(
...
void InternlmTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    InternlmTriAttentionCompactionProfile profile;
    InternlmTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void InternlmTriAttentionKvCache::finalize_repack_profile(
    InternlmTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
InternlmTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
InternlmTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                       int32_t keep_count) const {
...
int32_t InternlmTriAttentionKvCache::count_prefix_positions(
...
void InternlmTriAttentionKvCache::log_compact_debug(
...
void InternlmTriAttentionKvCache::log_compact_profile(
    const InternlmTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool InternlmTriAttentionKvCache::should_emit_keep_dump(
    const InternlmTriAttentionCompactionProfile* profile_ptr) const {
...
void InternlmTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "InternlmTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void InternlmTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void InternlmTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void InternlmTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool InternlmTriAttentionKvCache::ok() const {
