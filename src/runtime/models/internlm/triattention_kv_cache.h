...
#include "runtime/models/internlm/inference_state.h"
...
enum class InternlmTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class InternlmTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct InternlmTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    InternlmTriAttentionScoreAggregation score_aggregation{
        InternlmTriAttentionScoreAggregation::kMean};
    InternlmTriAttentionScoreAggregation per_layer_aggregation{
        InternlmTriAttentionScoreAggregation::kMean};
...
struct InternlmTriAttentionHeadStats {
...
struct InternlmTriAttentionStats {
    int32_t head_dim{0};
    InternlmTriAttentionRopeStyle rope_style{InternlmTriAttentionRopeStyle::kHalf};
    ...
    std::vector<InternlmTriAttentionHeadStats> layer_stats;
};

struct InternlmTriAttentionCompactionProfile {
...
// Build a InternlmTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class InternlmTriAttentionKvCache : public InternlmInferenceState {
  public:
    InternlmTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                                int32_t kv_dim, cudaStream_t stream,
                                InternlmTriAttentionConfig config, InternlmTriAttentionStats stats,
  ...
  private:
    ...
    void finalize_repack_profile(InternlmTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const InternlmTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const InternlmTriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        InternlmTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             InternlmTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const InternlmTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const InternlmTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const InternlmTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    InternlmKvCacheNames names_;
    InternlmTriAttentionConfig config_;
    InternlmTriAttentionStats stats_;
