...
int32_t cache_row_width(const TrtModule& module, const std::string& tensor_name) {
    ...
    if (shape.size() != 4)
        throw std::runtime_error("InternLM native KV tensor '" + tensor_name +
                                 "' must have rank 4");
    ...
    if (heads <= 0 || head_dim <= 0 || heads > std::numeric_limits<int32_t>::max() / head_dim) {
        throw std::runtime_error("Unable to infer InternLM native KV row width from '" +
                                 tensor_name + "'");
    }
...
std::uint64_t checked_multiply(std::uint64_t lhs, std::uint64_t rhs) {
    if (lhs != 0 && rhs > std::numeric_limits<std::uint64_t>::max() / lhs)
        throw std::overflow_error("InternLM native KV byte accounting overflow");
    return lhs * rhs;
}
...
KvCacheRuntimeSizing resolve_kv_cache_sizing(const PipelineContext& ctx, DType cache_dtype,
    ...
    if (ctx.kv_cache_size_bytes != 0) {
        throw std::invalid_argument(
            "InternLM native TensorRT KV cache always allocates the model's full capacity; "
            "kv_cache_size_bytes is unsupported");
    }
    if (ctx.config.num_layers <= 0 || ctx.config.max_cache_length <= 0 || kv_dim <= 0)
        throw std::runtime_error("InternLM native KV geometry must be positive");
...
void admit_kv_allocation(const PipelineContext& ctx, const KvCacheRuntimeSizing& sizing) {
    ...
    if (status != cudaSuccess) {
        throw std::runtime_error(std::string("InternLM native KV CUDA memory query failed: ") +
    ...
    if (sizing.cache_bytes > available) {
        throw std::runtime_error(
            "InternLM native KV cache admission failed before allocation: capacity=" +
...
void validate_native_bundle_metadata(const PipelineContext& ctx, DType cache_dtype) {
    if (!extract_json_bool(ctx.config_json, "native_kv_cache", false) ||
        ...
        throw std::runtime_error(
            "InternLM runtime only accepts native KV contract version 1 bundles");
    }
    if (extract_json_string(ctx.config_json, "decoder_engine_layout", "") != "split")
        throw std::runtime_error(
            "InternLM native KV runtime requires split prefill/decode engines");
    if (cache_dtype != DType::kBFloat16)
        throw std::runtime_error("InternLM native KV runtime requires BF16");
    if (extract_json_string(ctx.config_json, "tensor_parallel_mode", "single") != "single" ||
        ...
        throw std::runtime_error("InternLM native KV runtime supports single-GPU bundles only");
    }
}

void validate_native_module_contract(const PipelineContext& ctx, const TrtModule& module,
                                     const InternlmKvCacheNames& names) {
    if (!module.has_input(names.cache_write_indices) ||
        !module.has_input(names.key_value_lengths)) {
        throw std::runtime_error(
            "InternLM native KV engine is missing cache_write_indices/key_value_lengths");
    }
    ...
    if (names.cache_k.empty() || module.tensor_shape(names.cache_k.front()) != expected_shape) {
        throw std::runtime_error(
            "InternLM native KV requires cache shape [1,num_kv_heads,capacity,128]");
    }
}

InternlmEngineRole parse_engine_role(const char* engine_role) {
    const std::string role(engine_role);
    if (role == "decode")
        return InternlmEngineRole::kDecode;
    if (role == "prefill")
        return InternlmEngineRole::kPrefill;
    throw std::invalid_argument("Unknown InternLM native engine role '" + role + "'");
}

int32_t validate_native_bundle(const PipelineContext& ctx, const TrtModule& module,
                               const InternlmKvCacheNames& names, DType cache_dtype,
                               const char* engine_role) {
    validate_native_bundle_metadata(ctx, cache_dtype);
    validate_native_module_contract(ctx, module, names);
    const auto native_role = parse_engine_role(engine_role);
    return validate_internlm_native_sequence_profile(module, ctx.config.io_map.token_id,
                                                     names.position_id, ctx.config.max_cache_length,
                                                     native_role);
}
...
std::unique_ptr<TrtModule> load_native_module(const PipelineContext& ctx,
    ...
    if (plan == nullptr || plan->empty())
        throw std::runtime_error("InternLM native KV bundle is missing " + section_name);
    ...
    if (modules.modules.size() != 1 || modules.modules.front().profile_idx != 0 ||
        !modules.modules.front().module || !modules.modules.front().module->ok() ||
        modules.modules.front().module->profile_idx() != 0) {
        throw std::runtime_error("Failed to load InternLM native KV " + section_name);
    }
    modules.modules.front().module->set_timing_label(section_name);
    return std::move(modules.modules.front().module);
}

void build_kv_names(const PipelineContext& ctx, InternlmKvCacheNames& names) {
    const auto& io = ctx.config.io_map;
    names.position_id = io.position_id;
    for (int32_t layer = 0; layer < ctx.config.num_layers; ++layer) {
        names.cache_k.push_back(internlm_expand_layer_name(io.cache_k_pattern, layer));
        names.cache_v.push_back(internlm_expand_layer_name(io.cache_v_pattern, layer));
        names.present_k.push_back(internlm_expand_layer_name(io.present_k_pattern, layer));
        names.present_v.push_back(internlm_expand_layer_name(io.present_v_pattern, layer));
    }
}
...
void populate_text_config(const PipelineContext& ctx, InternlmTextGenConfig& config,
                          int32_t prefill_max_length) {
    const auto& io = ctx.config.io_map;
    ...
    config.chat_template_format = internlm_detect_chat_template_format(chat_template);
}
...
class DecoderPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        (void)validate_native_bundle(ctx, *decoder, names, cache_dtype, "decode");
        ...
        auto state = std::make_unique<InternlmKvCache>(ctx.config.num_layers, sizing.rows, kv_dim,
                                                       stream, cache_dtype, std::move(names));
        if (!state->ok())
            throw std::runtime_error("Failed to allocate InternLM native KV cache");
        std::cerr << "[trtmc] KV cache rows=" << sizing.rows
                  << " (row=" << format_bytes(sizing.row_bytes)
                  << ", cache=" << format_bytes(sizing.cache_bytes) << ")\n";

        InternlmTextGenConfig config;
        populate_text_config(ctx, config, prefill_max_length);
        std::vector<InternlmTextGenerationPipeline::DecoderContext> decoders;
        decoders.push_back(InternlmTextGenerationPipeline::DecoderContext{std::move(decoder)});
        return std::make_unique<InternlmTextGenerationPipeline>(
...
REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_internlm_plugin, DecoderPlugin,
                                       "internlm_decoder_kv_cache");
