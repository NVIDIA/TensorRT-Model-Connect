...
// InternlmInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   InternlmKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — InternlmKvCache + family-owned recurrent state composed
...
class InternlmInferenceState {
  public:
    virtual ~InternlmInferenceState() = default;
    ...
    // InternlmKvCache -> true. Family-owned recurrent state -> false.
