...
void validate_native_scalar_input(TrtModule& module, const std::string& name) {
    if (!module.has_input(name) || module.tensor_dtype(name) != DType::kInt32 ||
        ...
        throw std::runtime_error("InternLM native KV input '" + name + "' must be int32 [1]");
    }
}
...
void validate_native_cache_pair(TrtModule& module, const std::string& cache_name,
    ...
    if (!module.has_input(cache_name) || !module.has_output(present_name)) {
        throw std::runtime_error("InternLM native KV engine is missing cache/present pair '" +
    ...
    if (!valid_native_cache_shape(cache_shape, max_length, kv_dim) ||
        ...
        throw std::runtime_error("InternLM native KV cache/present tensors must share static "
                                 "[1,Hkv,max_length,D] shape");
    }
    if (module.tensor_dtype(cache_name) != cache_dtype ||
        module.tensor_dtype(present_name) != cache_dtype) {
        throw std::runtime_error("InternLM native KV cache dtype does not match model precision");
    }
}
...
InternlmKvCacheNames default_cache_names(int32_t num_layers) {
...
void validate_cache_name_count(const InternlmKvCacheNames& names, std::size_t expected) {
    if (names.cache_k.size() != expected || names.cache_v.size() != expected ||
        names.present_k.size() != expected || names.present_v.size() != expected) {
        throw std::invalid_argument("InternlmKvCache per-layer tensor name count mismatch");
    }
}
...
InternlmKvCache::InternlmKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim,
                                 cudaStream_t stream, DType cache_dtype, InternlmKvCacheNames names)
    ...
    if (num_layers <= 0 || max_length <= 0 || kv_dim <= 0)
        throw std::invalid_argument("InternlmKvCache geometry must be positive");
...
void InternlmKvCache::validate_native_kv_contract(TrtModule& module) const {
    ...
    if (!module.has_input(names_.position_id) ||
        ...
        throw std::runtime_error("InternLM native KV engine requires an int32 position_id input");
    }
...
void InternlmKvCache::bind_native_cache(TrtModule& module) {
    for (int32_t layer = 0; layer < num_layers_; ++layer) {
        const auto index = static_cast<std::size_t>(layer);
        ...
        if (module.device_ptr(names_.cache_k[index]) != cache_k_[index].data() ||
            ...
            throw std::runtime_error(
                "InternLM native KV engine did not preserve cache/present aliasing");
        }
    }
}

void InternlmKvCache::bind_to(TrtModule& module) {
    validate_native_kv_contract(module);
    bind_native_cache(module);
}

void InternlmKvCache::bind_cache_inputs(TrtModule& module) {
    validate_native_kv_contract(module);
    bind_native_cache(module);
}

void InternlmKvCache::write_position_input(TensorMap& inputs, int32_t seq_len) {
...
void InternlmKvCache::write_native_kv_inputs(TensorMap& inputs, int32_t seq_len) {
...
void InternlmKvCache::prepare_step(TensorMap& inputs, int32_t seq_len) {
    if (seq_len <= 0)
        throw std::invalid_argument("InternLM native KV step length must be positive");
    write_position_input(inputs, seq_len);
    write_native_kv_inputs(inputs, seq_len);
}

void InternlmKvCache::advance(int32_t n_tokens) {
    if (n_tokens <= 0 || n_tokens > max_length_ - position_)
        throw std::runtime_error("InternLM native KV advance exceeds cache capacity");
    position_ += n_tokens;
}
