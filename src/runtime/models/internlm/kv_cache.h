...
// InternlmKvCache: autoregressive KV cache state manager.
...
class InternlmKvCache : public InternlmInferenceState {
  public:
    ...
    InternlmKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                    DType cache_dtype = DType::kFloat32, InternlmKvCacheNames names = {});

    // --- InternlmInferenceState overrides ---
    void reset() override;
    ...
    // --- InternlmKvCache-specific methods (not on the interface) ---
    ...
    // — too big for InternlmKvCache's single-row present buffer. The caller reads
