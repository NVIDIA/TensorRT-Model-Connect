#include "runtime/models/opt/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/opt/triattention_kernels.h"
#endif
...
OptTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return OptTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return OptTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(OptTriAttentionScoreAggregation value) {
    switch (value) {
    case OptTriAttentionScoreAggregation::kMean:
        return "mean";
    case OptTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

OptTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return OptTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return OptTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, OptKvCacheNames& names) {
...
// override helpers that patched OptTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     OptTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(OptTriAttentionConfig& cfg,
...
void fill_debug_from_registry(OptTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(OptTriAttentionConfig& cfg, const std::string& config_json,
...
static void validate_triattention_config(const OptTriAttentionConfig& cfg,
                                         int32_t max_cache_length) {
    if (cfg.kv_budget < 1)
...
OptTriAttentionConfig
opt_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                     const ::trtmc::config::ConfigBundle* runtime_config) {
    OptTriAttentionConfig cfg;
...
void fill_stats_header(OptTriAttentionStats& stats, const json& root, int32_t num_attention_heads,
...
void fill_stats_inv_freq(OptTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(OptTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(OptTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(OptTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(OptTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(OptTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
void init_layer_stats_empty(OptTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(OptTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(OptTriAttentionStats& stats, const std::vector<int32_t>& group_counts,
                           int32_t half_dim) {
    bool any_stats = false;
    for (int32_t layer = 0; layer < stats.num_layers; ++layer) {
        auto& layer_stats = stats.layer_stats[static_cast<std::size_t>(layer)];
...
void parse_sparse_stats(OptTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
OptTriAttentionStats opt_parse_triattention_stats_json(const std::string& stats_json,
                                                       int32_t num_attention_heads,
                                                       int32_t num_key_value_heads,
                                                       int32_t num_layers) {
    OptTriAttentionStats stats;
...
void OptTriAttentionKvCache::validate_shapes() {
...
void OptTriAttentionKvCache::normalize_sampled_heads() {
...
void OptTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void OptTriAttentionKvCache::allocate_layer_tensors() {
...
OptTriAttentionKvCache::OptTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                               int32_t max_length, int32_t kv_dim,
                                               cudaStream_t stream, OptTriAttentionConfig config,
                                               OptTriAttentionStats stats, DType cache_dtype,
                                               OptKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void OptTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void OptTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void OptTriAttentionKvCache::initialize_gpu_state() {
...
bool OptTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool OptTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool OptTriAttentionKvCache::can_use_gpu_selection() const {
...
void OptTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t OptTriAttentionKvCache::preferred_cache_rows() const {
...
void OptTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void OptTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float>
OptTriAttentionKvCache::copy_cache_rows_to_host(const DeviceTensor& tensor, int32_t rows,
                                                OptTriAttentionCompactionProfile* profile) const {
...
void OptTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t OptTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> OptTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t> OptTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
OptTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                            OptTriAttentionCompactionProfile* profile) {
...
int32_t OptTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t OptTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> OptTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void OptTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, OptTriAttentionCompactionProfile* profile) const {
...
bool OptTriAttentionKvCache::layer_stats_shapes_valid(const OptTriAttentionHeadStats& layer_stats,
...
void OptTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                           int32_t half_dim, float& k_rot_real,
                                           float& k_rot_imag) const {
    if (stats_.rope_style == OptTriAttentionRopeStyle::kInterleaved) {
...
float OptTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == OptTriAttentionScoreAggregation::kMax)
...
float OptTriAttentionKvCache::score_one_row(
    const float* row_ptr, const OptTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void OptTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const OptTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void OptTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void OptTriAttentionKvCache::accumulate_layer_fallback(
...
void OptTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == OptTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void OptTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
OptTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void OptTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == OptTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void OptTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> OptTriAttentionKvCache::build_keep_indices_per_head(
...
bool OptTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, OptTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> OptTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, OptTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void OptTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                    int32_t total_tokens) const {
...
void OptTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void OptTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
OptTriAttentionKvCache::GpuLayerResult OptTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = opt_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == OptTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == OptTriAttentionScoreAggregation::kMax,
...
bool OptTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool OptTriAttentionKvCache::upload_gpu_trig_phases(int32_t num_offsets, int32_t half_dim,
                                                    OptTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void OptTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> OptTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool OptTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> OptTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, OptTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void OptTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void OptTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool OptTriAttentionKvCache::gpu_compaction_upload_keep(int32_t keep_count,
...
bool OptTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                   std::size_t row_bytes) {
    const bool ok_k = opt_triattention_compact_rows_gpu(
    ...
    const bool ok_v = opt_triattention_compact_rows_gpu(
...
bool OptTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
                                                  const std::vector<int32_t>& keep_indices,
...
void OptTriAttentionKvCache::compact_layer_on_host(int32_t layer,
                                                   const std::vector<int32_t>& keep_indices,
                                                   int32_t keep_count, std::size_t row_bytes,
...
void OptTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    OptTriAttentionCompactionProfile profile;
    OptTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void OptTriAttentionKvCache::finalize_repack_profile(OptTriAttentionCompactionProfile* profile_ptr,
...
OptTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
OptTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                  int32_t keep_count) const {
...
int32_t OptTriAttentionKvCache::count_prefix_positions(
...
void OptTriAttentionKvCache::log_compact_debug(const std::vector<int32_t>& representative_positions,
                                               const std::vector<int32_t>& keep_indices,
                                               int32_t keep_count) const {
    if (!config_.debug)
        return;
...
void OptTriAttentionKvCache::log_compact_profile(
    const OptTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool OptTriAttentionKvCache::should_emit_keep_dump(
    const OptTriAttentionCompactionProfile* profile_ptr) const {
...
void OptTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "OptTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void OptTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void OptTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void OptTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool OptTriAttentionKvCache::ok() const {
