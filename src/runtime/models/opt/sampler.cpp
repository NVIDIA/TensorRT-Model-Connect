// OptISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// OptTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/opt/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/opt/argmax_kernel.h"
#include "runtime/models/opt/sparse_multinomial_kernel.h"
...
static OptSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                          int32_t eos_token_id) {
    OptSampleResult result;
...
static bool greedy_equivalent(const OptSamplingParams& params) {
...
static FilteredDistribution build_filtered_distribution(const float* logits, int32_t vocab_size,
                                                        const OptSamplingParams& params) {
...
class GreedySampler final : public OptISampler {
  public:
    OptSampleResult sample(const float* logits, int32_t vocab_size,
                           const OptSamplingParams& params) override {
    ...
    OptLogitsLocation logits_location() const override { return OptLogitsLocation::HOST; }
...
class TopKSampler final : public OptISampler {
  public:
...
class TorchCudaMultinomialSampler final : public OptISampler {
  public:
    ...
    OptSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        opt_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const OptTorchMultinomialExecutionPolicy policy =
                opt_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public OptISampler {
  public:
    ...
    OptSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        opt_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    OptLogitsLocation logits_location() const override { return OptLogitsLocation::DEVICE; }
...
OptSamplingParams opt_sampling_params_from_config(const GenerateConfig& cfg, int32_t default_eos) {
    OptSamplingParams p;
...
create_opt_sampler(const OptSamplingParams& params,
                   [[maybe_unused]] const OptSamplerFactoryOptions& options) {
...
std::unique_ptr<OptISampler> create_opt_sampler(const OptSamplingParams& params) {
    return create_opt_sampler(params, OptSamplerFactoryOptions{});
}

std::unique_ptr<OptISampler> create_opt_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
