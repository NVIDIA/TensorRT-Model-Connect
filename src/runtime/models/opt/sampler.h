...
// OptISampler: token selection abstraction for autoregressive generation.
//
...
//
// OptLogitsLocation tells the pipeline whether to D2H transfer logits before
...
class OptISampler {
  public:
    ...
    virtual OptLogitsLocation logits_location() const = 0;
...
/// Build OptSamplingParams from GenerateConfig fields.
/// Forward-declared here; defined in sampler.cpp alongside the factory.
struct GenerateConfig; // defined in trtmc/pipeline.h

OptSamplingParams opt_sampling_params_from_config(const GenerateConfig& cfg,
                                                  int32_t default_eos = -1);

/// Factory: create sampler from OptSamplingParams.
...
std::unique_ptr<OptISampler> create_opt_sampler(const OptSamplingParams& params,
                                                const OptSamplerFactoryOptions& options);
