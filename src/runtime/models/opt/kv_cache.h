...
// OptKvCache: autoregressive KV cache state manager.
...
class OptKvCache : public OptInferenceState {
  public:
    ...
    OptKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
               DType cache_dtype = DType::kFloat32, OptKvCacheNames names = {});

    // --- OptInferenceState overrides ---
    void reset() override;
    ...
    // --- OptKvCache-specific methods (not on the interface) ---
    ...
    // — too big for OptKvCache's single-row present buffer. The caller reads
