...
class DecoderPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<OptTextGenerationPipeline>(
  ...
  private:
    ...
    load_split_prefill_module(const PipelineContext& ctx, cudaStream_t stream, const IoMap& io,
                              const OptKvCacheNames& kv_names, int32_t& prefill_profile_idx,
    ...
    static void build_kv_names(const PipelineContext& ctx, const IoMap& io,
        ...
        for (int32_t i = 0; i < ctx.config.num_layers; ++i) {
            kv_names.cache_k.push_back(opt_expand_layer_name(io.cache_k_pattern, i));
            kv_names.cache_v.push_back(opt_expand_layer_name(io.cache_v_pattern, i));
            kv_names.present_k.push_back(opt_expand_layer_name(io.present_k_pattern, i));
            kv_names.present_v.push_back(opt_expand_layer_name(io.present_v_pattern, i));
        }
    }
    ...
    static std::vector<OptTextGenerationPipeline::DecoderContext>
    build_decoder_contexts(BackendProfileModules profile_modules, int32_t runtime_rows,
        ...
        for (const auto& profile : profile_roles.decode_profiles) {
            ...
            decoders.push_back(OptTextGenerationPipeline::DecoderContext{profile.kv_rows,
    ...
    static std::unique_ptr<OptInferenceState>
    build_inference_state(const PipelineContext& ctx, const KvCacheRuntimeSizing& sizing,
                          OptTriAttentionConfig& tri_cfg, DType cache_dtype, int32_t kv_dim,
                          OptKvCacheNames& kv_names, cudaStream_t stream) {
        ...
        if (tri_cfg.enabled) {
            ...
            OptTriAttentionStats tri_stats = opt_parse_triattention_stats_json(
                stats_json, ctx.config.num_heads, ctx.config.num_kv_heads, ctx.config.num_layers);
            state = std::make_unique<OptTriAttentionKvCache>(
            ...
            state = std::make_unique<OptKvCache>(ctx.config.num_layers, sizing.runtime_rows, kv_dim,
                                                 stream, cache_dtype, std::move(kv_names));
        }
        if (!state->ok())
            throw std::runtime_error("Failed to create OptKvCache");
        return state;
    }
    ...
    static void populate_text_gen_config(const PipelineContext& ctx, OptTextGenConfig& tgc,
                                         const IoMap& io,
                                         const OptTextGenerationPipeline::DecoderContext& first_dec,
    ...
    static void apply_chat_template_format(const BundleFile& bundle, OptTextGenConfig& tgc) {
        std::string chat_tpl;
        ...
        tgc.chat_template_format = opt_detect_chat_template_format(chat_tpl);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_opt_plugin, DecoderPlugin, "opt_decoder_kv_cache");
