...
//
// Composes: TrtModule (decoder) + OptKvCache + ITokenizer for this runtime
...
class OptTextGenerationPipeline final : public IPipeline {
  public:
    ...
    OptTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                              ...
                              std::unique_ptr<OptISampler> sampler = nullptr,
                              std::shared_ptr<void> distributed_owner = nullptr);
    OptTextGenerationPipeline(std::vector<DecoderContext> decoders,
                              std::unique_ptr<OptInferenceState> state, OptTextGenConfig config,
                              cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer = nullptr,
                              std::string model_id_str = "",
                              std::unique_ptr<OptISampler> sampler = nullptr,
    ...
    const char* pipeline_type() const override { return "OptTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<OptInferenceState> state_;
    OptTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<OptISampler> sampler_;
    ...
    TimedGenResult generate_from_ids(const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
                                     const OptSamplingParams& params, const GenerateConfig& cfg);
    ...
    OptKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(OptKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(OptISampler* sampler, const OptSamplingParams& params,
    ...
    std::unique_ptr<OptISampler> make_step_sampler(const OptSamplingParams& params);
