...
#include "runtime/models/opt/inference_state.h"
...
enum class OptTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class OptTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct OptTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    OptTriAttentionScoreAggregation score_aggregation{OptTriAttentionScoreAggregation::kMean};
    OptTriAttentionScoreAggregation per_layer_aggregation{OptTriAttentionScoreAggregation::kMean};
...
struct OptTriAttentionHeadStats {
...
struct OptTriAttentionStats {
    int32_t head_dim{0};
    OptTriAttentionRopeStyle rope_style{OptTriAttentionRopeStyle::kHalf};
    ...
    std::vector<OptTriAttentionHeadStats> layer_stats;
};

struct OptTriAttentionCompactionProfile {
...
// Build a OptTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class OptTriAttentionKvCache : public OptInferenceState {
  public:
    OptTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                           int32_t kv_dim, cudaStream_t stream, OptTriAttentionConfig config,
                           OptTriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(OptTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const OptTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const OptTriAttentionCompactionProfile* profile_ptr) const;
    ...
    std::vector<int32_t> select_keep_indices(int32_t keep_budget,
                                             OptTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             OptTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const OptTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const OptTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const OptTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    OptKvCacheNames names_;
    OptTriAttentionConfig config_;
    OptTriAttentionStats stats_;
