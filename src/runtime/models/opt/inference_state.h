...
// OptInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   OptKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — OptKvCache + family-owned recurrent state composed
...
class OptInferenceState {
  public:
    virtual ~OptInferenceState() = default;
    ...
    // OptKvCache -> true. Family-owned recurrent state -> false.
