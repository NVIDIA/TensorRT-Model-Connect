...
void OptTextGenerationPipeline::append_prefill_kv(OptKvCache& kv, TrtModule& prefill, int32_t sq) {
    ...
    if (!gather_prefill_kv_pointers(prefill, config_, pk, pv)) {
        throw std::runtime_error(
            "OptTextGenerationPipeline: prefill module is missing present_k/present_v outputs");
    }
...
OptTextGenerationPipeline::TimedGenResult OptTextGenerationPipeline::generate_diffusion_from_ids(
    ...
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error(
            "OptTextGenerationPipeline: diffusion mode currently supports greedy temperature=0 "
            "generation");
    }
    const int32_t block_len =
...
OptTextGenerationPipeline::TimedGenResult OptTextGenerationPipeline::generate_linear_spec_from_ids(
    ...
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error(
            "OptTextGenerationPipeline: linear_spec mode currently supports greedy temperature=0 "
            "generation");
    }
    if (use_lora_draft && linear_spec_lora_prefill_ == nullptr) {
        throw std::runtime_error(
            "OptTextGenerationPipeline: linear_spec_lora mode requires a linear-spec LoRA engine");
    }
    const int32_t block_len =
