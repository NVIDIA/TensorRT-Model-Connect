...
#include "runtime/models/opt/pipeline.h"

#include "runtime/models/opt/chat_templates.h"
#include "runtime/models/opt/kv_cache.h"
#include "runtime/models/opt/tensor_names.h"
...
std::vector<OptTextGenerationPipeline::DecoderContext>
single_decoder_context(std::unique_ptr<TrtModule> decoder) {
    std::vector<OptTextGenerationPipeline::DecoderContext> decoders;
    decoders.push_back(OptTextGenerationPipeline::DecoderContext{0, std::move(decoder)});
    return decoders;
}
...
bool greedy_text_diffusion_params(const OptSamplingParams& params) {
...
OptTextGenerationPipeline::OptTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                                     std::unique_ptr<OptInferenceState> state,
                                                     OptTextGenConfig config, cudaStream_t stream,
                                                     std::shared_ptr<ITokenizer> tokenizer,
                                                     std::string model_id_str,
                                                     std::unique_ptr<OptISampler> sampler,
                                                     std::shared_ptr<void> distributed_owner)
    : OptTextGenerationPipeline(single_decoder_context(std::move(decoder)), std::move(state),
...
OptTextGenerationPipeline::OptTextGenerationPipeline(
    std::vector<DecoderContext> decoders, std::unique_ptr<OptInferenceState> state,
    OptTextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<OptISampler> sampler,
    ...
    if (decoders_.empty()) {
        throw std::runtime_error("OptTextGenerationPipeline: no decoder modules");
    }
    for (const auto& decoder_ctx : decoders_) {
        if (!decoder_ctx.module || !decoder_ctx.module->ok()) {
            throw std::runtime_error("OptTextGenerationPipeline: invalid decoder module");
        }
    }
    if (!state_ || !state_->ok()) {
        throw std::runtime_error("OptTextGenerationPipeline: invalid inference state");
    }
...
static std::vector<int32_t> encode_prompt(const ITokenizer& tokenizer,
                                          const OptTextGenConfig& config, const std::string& prompt,
                                          const GenerateConfig& cfg) {
    std::string effective = prompt;
    bool templated = false;
    if (cfg.use_chat_template && !config.chat_template_format.empty()) {
        effective =
            opt_apply_chat_template(config.chat_template_format, prompt, cfg.enable_thinking);
        templated = true;
    }
...
TextResult OptTextGenerationPipeline::generate(const std::string& prompt,
                                               const GenerateConfig& cfg) {
    if (!tokenizer_) {
        throw std::runtime_error("OptTextGenerationPipeline: no tokenizer configured");
    }
...
OptTextGenerationPipeline::generate_ids(const std::vector<int32_t>& input_ids,
                                        const GenerateConfig& cfg) {
...
std::unique_ptr<OptISampler>
OptTextGenerationPipeline::make_step_sampler(const OptSamplingParams& params) {
    ...
    if (prefer_gpu_greedy_ && greedy_params) {
        if (auto gpu = create_opt_gpu_greedy_sampler(stream_))
            return gpu;
    }
    return create_opt_sampler(params);
}
...
namespace {
bool gather_prefill_kv_pointers(TrtModule& prefill, const OptTextGenConfig& cfg,
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        pk[li] = prefill.device_ptr(opt_expand_layer_name(cfg.present_k_pattern, i));
        pv[li] = prefill.device_ptr(opt_expand_layer_name(cfg.present_v_pattern, i));
...
bool batched_prefill_supported(const TrtModule* prefill, const OptTextGenConfig& cfg, int32_t sq,
                               OptInferenceState* state) {
    if (prefill == nullptr || sq <= 0)
        return false;
    if (cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        return false;
    return dynamic_cast<OptKvCache*>(state) != nullptr;
}

void validate_generation_capacity(const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
                                  OptInferenceState* state, const TrtModule* prefill,
    ...
    const auto* kv = dynamic_cast<const OptKvCache*>(state);
    if (kv == nullptr)
        return;
    ...
    if (input_ids.size() > capacity ||
        ...
        throw std::runtime_error(
            "Opt requested prompt and generation exceed the model's fixed KV cache capacity");
    }
}

int32_t resolve_batched_prefill_chunk_limit(const OptKvCache& kv, const OptTextGenConfig& config,
                                            int32_t token_count) {
    ...
    if (config.prefill_max_length <= 0)
        throw std::runtime_error("Opt native KV prefill engine has no valid profile capacity");
    return config.prefill_max_length;
}
} // namespace

void OptTextGenerationPipeline::run_prefill_chunk(const int32_t* token_ids, int32_t chunk_size,
                                                  OptKvCache& kv,
    ...
    if (logits_it == outputs.end()) {
        throw std::runtime_error("OptTextGenerationPipeline: prefill module has no logits output");
    }
    ...
    if (static_cast<std::size_t>(logits_tensor.numel()) < vocab) {
        throw std::runtime_error(
            "OptTextGenerationPipeline: prefill logits are smaller than vocabulary");
    }
    logits.resize(vocab);
    ...
    if (retain_device_logits) {
        ...
        if (device_logits == nullptr) {
            throw std::runtime_error(
                "OptTextGenerationPipeline: prefill logits have no device buffer");
        }
...
void OptTextGenerationPipeline::log_batched_prefill(int32_t token_count, int32_t chunk_count,
...
bool OptTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
                                                    std::vector<float>& logits,
                                                    bool retain_device_logits) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    if (!batched_prefill_supported(prefill_.get(), config_, sq, state_.get()))
        return false;
    auto* kv = static_cast<OptKvCache*>(state_.get());
    ...
    if (sq > kv->max_length()) {
        throw std::runtime_error("Opt sequence exceeds the model's fixed KV cache capacity");
    }
...
void OptTextGenerationPipeline::prime_decoder_after_batched_prefill(
...
void OptTextGenerationPipeline::run_prefill(const std::vector<int32_t>& input_ids,
...
TrtModule& OptTextGenerationPipeline::require_block_prefill(int32_t sq,
                                                            TrtModule* prefill_override) {
    TrtModule* prefill = prefill_override != nullptr ? prefill_override : prefill_.get();
    if (prefill == nullptr)
        throw std::runtime_error(
            "OptTextGenerationPipeline: block generation requires prefill module");
    if (sq <= 0)
        throw std::runtime_error("OptTextGenerationPipeline: empty block");
    if (config_.prefill_max_length > 0 && sq > config_.prefill_max_length) {
        throw std::runtime_error("OptTextGenerationPipeline: block length exceeds prefill profile");
    }
    return *prefill;
}

OptKvCache& OptTextGenerationPipeline::require_block_kv_cache() {
    auto* kv = dynamic_cast<OptKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error("OptTextGenerationPipeline: block generation requires OptKvCache");
    return *kv;
}

void OptTextGenerationPipeline::copy_block_logits(const TensorMap& outputs,
                                                  std::vector<float>& logits) const {
    auto logits_it = outputs.find(config_.logits_output_name);
    if (logits_it == outputs.end())
        throw std::runtime_error("OptTextGenerationPipeline: prefill module has no '" +
...
void OptTextGenerationPipeline::append_prefill_kv(OptKvCache& kv, TrtModule& prefill, int32_t sq) {
    std::vector<const void*> pk, pv;
    if (!gather_prefill_kv_pointers(prefill, config_, pk, pv)) {
        throw std::runtime_error("OptTextGenerationPipeline: prefill module is missing "
                                 "present_k/present_v outputs");
    }
    kv.append_prefill_kv(pk, pv, sq);
}

void OptTextGenerationPipeline::run_prefill_block(const std::vector<int32_t>& input_ids,
                                                  bool bidirectional, bool append_kv,
                                                  std::vector<float>& logits,
                                                  TrtModule* prefill_override) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    TrtModule& prefill = require_block_prefill(sq, prefill_override);
    OptKvCache& kv = require_block_kv_cache();
...
std::string OptTextGenerationPipeline::resolve_generation_mode(const GenerateConfig& cfg) const {
...
int32_t OptTextGenerationPipeline::resolve_text_diffusion_block_length(
    ...
    if (!config_.supports_text_diffusion || config_.mask_token_id < 0)
        throw std::runtime_error(
            "OptTextGenerationPipeline: bundle does not support text diffusion");
    ...
    if (require_divisible && max_new_tokens % block_len != 0) {
        throw std::runtime_error("OptTextGenerationPipeline: diffusion mode requires "
...
int32_t
OptTextGenerationPipeline::seed_next_token_from_prefill(const std::vector<int32_t>& input_ids,
    ...
    if (static_cast<int32_t>(logits.size()) < vocab)
        throw std::runtime_error("OptTextGenerationPipeline: missing prefill logits");
    return argmax_with_confidence(logits.data() + logits.size() - static_cast<std::size_t>(vocab),
                                  vocab, 0)
        .token_id;
}

void OptTextGenerationPipeline::fill_diffusion_block(std::vector<int32_t>& block,
    ...
    for (int32_t step = 0; step < block_len && has_mask_token(block, config_.mask_token_id);
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "OptTextGenerationPipeline: diffusion engine must output full block logits");
        }
...
int32_t OptTextGenerationPipeline::verify_diffusion_block(const std::vector<int32_t>& block,
                                                          std::vector<float>& logits,
                                                          int32_t block_len, int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "OptTextGenerationPipeline: diffusion engine must output full verify logits");
    }
...
bool OptTextGenerationPipeline::append_tokens_until_eos(const std::vector<int32_t>& tokens,
                                                        std::vector<int32_t>& output,
                                                        const OptSamplingParams& params) const {
...
void OptTextGenerationPipeline::fill_linear_spec_block(std::vector<int32_t>& block,
                                                       std::vector<float>& logits,
                                                       int32_t block_len, int32_t vocab,
    ...
    while (has_mask_token(block, config_.mask_token_id)) {
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "OptTextGenerationPipeline: linear_spec engine must output full block logits");
        }
        const auto preds = masked_predictions(logits, block, config_.mask_token_id, vocab);
...
std::vector<int32_t>
OptTextGenerationPipeline::verify_linear_spec_block(const std::vector<int32_t>& block,
                                                    std::vector<float>& logits, int32_t block_len,
                                                    int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "OptTextGenerationPipeline: linear_spec engine must output full verify logits");
    }
...
int32_t OptTextGenerationPipeline::count_linear_spec_accepts(const std::vector<int32_t>& ar_tokens,
                                                             const std::vector<int32_t>& block) {
    if (ar_tokens.empty())
        return 0;
    if (block.size() < 2)
...
bool OptTextGenerationPipeline::append_linear_spec_tokens(const std::vector<int32_t>& ar_tokens,
                                                          int32_t emit_count,
                                                          std::vector<int32_t>& output,
                                                          int32_t& generated,
                                                          const OptSamplingParams& params) const {
...
OptTextGenerationPipeline::TimedGenResult OptTextGenerationPipeline::generate_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens, const OptSamplingParams& params,
    const GenerateConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "auto" && mode != "ar")
        throw std::runtime_error("OptTextGenerationPipeline: unsupported generation mode '" + mode +
                                 "'");

    OptISampler* active_sampler = sampler_.get();
    std::unique_ptr<OptISampler> local_sampler;
    if (!active_sampler) {
    ...
    std::vector<float> logits;
    const bool gpu_sampling = (active_sampler->logits_location() == OptLogitsLocation::DEVICE);
...
OptTextGenerationPipeline::TimedGenResult OptTextGenerationPipeline::generate_diffusion_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens, const OptSamplingParams& params,
    const GenerateConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error("OptTextGenerationPipeline: diffusion mode currently "
...
OptTextGenerationPipeline::TimedGenResult OptTextGenerationPipeline::generate_linear_spec_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens, const OptSamplingParams& params,
    const GenerateConfig& cfg, bool use_lora_draft) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error("OptTextGenerationPipeline: linear_spec mode currently "
                                 "supports greedy temperature=0 "
                                 "generation");
    }
    if (use_lora_draft && linear_spec_lora_prefill_ == nullptr) {
        throw std::runtime_error("OptTextGenerationPipeline: linear_spec_lora mode requires a "
    ...
    auto* kv = dynamic_cast<OptKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error("OptTextGenerationPipeline: linear_spec requires OptKvCache");
...
bool OptTextGenerationPipeline::should_stop_on_answer(const std::vector<int32_t>& output,
...
void OptTextGenerationPipeline::log_decode_summary(int32_t steps, double ms) const {
...
int32_t
OptTextGenerationPipeline::run_decode_loop(OptISampler* sampler, const OptSamplingParams& params,
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        ...
        const OptSampleResult result = sampler->sample(sample_ptr, vocab_size, params);
...
int32_t OptTextGenerationPipeline::select_decoder_index(int32_t desired_rows) const {
    if (decoders_.size() == 1)
        return 0;
...
TrtModule& OptTextGenerationPipeline::bind_decoder_for_step() {
...
void OptTextGenerationPipeline::run_step(int32_t token_id, std::vector<float>& logits) {
    TensorMap inputs;
    ...
    if (it == outputs.end()) {
        throw std::runtime_error("OptTextGenerationPipeline: no '" + logits_output_name_ +
                                 "' output");
    }
...
void OptTextGenerationPipeline::run_step_device(int32_t token_id) {
    TensorMap inputs;
...
int32_t OptTextGenerationPipeline::argmax(const std::vector<float>& logits) {
    if (logits.empty())
        return 0;
