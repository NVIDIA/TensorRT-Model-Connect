// BloomISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// BloomTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/bloom/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/bloom/argmax_kernel.h"
#include "runtime/models/bloom/sparse_multinomial_kernel.h"
...
static BloomSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                            int32_t eos_token_id) {
    BloomSampleResult result;
...
static bool greedy_equivalent(const BloomSamplingParams& params) {
...
class GreedySampler final : public BloomISampler {
  public:
    BloomSampleResult sample(const float* logits, int32_t vocab_size,
                             const BloomSamplingParams& params) override {
    ...
    BloomLogitsLocation logits_location() const override { return BloomLogitsLocation::HOST; }
...
class TopKSampler final : public BloomISampler {
  public:
...
class TorchCudaMultinomialSampler final : public BloomISampler {
  public:
    ...
    BloomSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        bloom_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const BloomTorchMultinomialExecutionPolicy policy =
                bloom_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public BloomISampler {
  public:
    ...
    BloomSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        bloom_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    BloomLogitsLocation logits_location() const override { return BloomLogitsLocation::DEVICE; }
...
BloomSamplingParams bloom_sampling_params_from_config(const GenerateConfig& cfg,
                                                      int32_t default_eos) {
    BloomSamplingParams p;
...
create_bloom_sampler(const BloomSamplingParams& params,
                     [[maybe_unused]] const BloomSamplerFactoryOptions& options) {
...
std::unique_ptr<BloomISampler> create_bloom_sampler(const BloomSamplingParams& params) {
    return create_bloom_sampler(params, BloomSamplerFactoryOptions{});
}

std::unique_ptr<BloomISampler> create_bloom_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
