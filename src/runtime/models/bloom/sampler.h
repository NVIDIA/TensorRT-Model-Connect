...
// BloomISampler: token selection abstraction for autoregressive generation.
//
...
//
// BloomLogitsLocation tells the pipeline whether to D2H transfer logits before
...
class BloomISampler {
  public:
    ...
    virtual BloomLogitsLocation logits_location() const = 0;
...
/// Build BloomSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from BloomSamplingParams.
...
std::unique_ptr<BloomISampler> create_bloom_sampler(const BloomSamplingParams& params,
                                                    const BloomSamplerFactoryOptions& options);
