...
#include "runtime/models/bloom/inference_state.h"
...
enum class BloomTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class BloomTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct BloomTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    BloomTriAttentionScoreAggregation score_aggregation{BloomTriAttentionScoreAggregation::kMean};
    BloomTriAttentionScoreAggregation per_layer_aggregation{
        BloomTriAttentionScoreAggregation::kMean};
...
struct BloomTriAttentionHeadStats {
...
struct BloomTriAttentionStats {
    int32_t head_dim{0};
    BloomTriAttentionRopeStyle rope_style{BloomTriAttentionRopeStyle::kHalf};
    ...
    std::vector<BloomTriAttentionHeadStats> layer_stats;
};

struct BloomTriAttentionCompactionProfile {
...
// Build a BloomTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class BloomTriAttentionKvCache : public BloomInferenceState {
  public:
    BloomTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                             int32_t kv_dim, cudaStream_t stream, BloomTriAttentionConfig config,
                             BloomTriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(BloomTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const BloomTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const BloomTriAttentionCompactionProfile* profile_ptr) const;
    ...
    std::vector<int32_t> select_keep_indices(int32_t keep_budget,
                                             BloomTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             BloomTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                BloomTriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const BloomTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const BloomTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const BloomTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    BloomKvCacheNames names_;
    BloomTriAttentionConfig config_;
    BloomTriAttentionStats stats_;
