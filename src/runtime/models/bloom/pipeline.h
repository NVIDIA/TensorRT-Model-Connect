...
//
// Composes: TrtModule (decoder) + BloomKvCache + ITokenizer for this runtime
...
class BloomTextGenerationPipeline final : public IPipeline {
  public:
    ...
    BloomTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                std::unique_ptr<BloomInferenceState> state,
                                BloomTextGenConfig config, cudaStream_t stream,
                                std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                std::string model_id_str = "",
                                std::unique_ptr<BloomISampler> sampler = nullptr,
                                std::shared_ptr<void> distributed_owner = nullptr);
    BloomTextGenerationPipeline(std::vector<DecoderContext> decoders,
                                std::unique_ptr<BloomInferenceState> state,
                                BloomTextGenConfig config, cudaStream_t stream,
                                std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "BloomTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<BloomInferenceState> state_;
    BloomTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<BloomISampler> sampler_;
    ...
    BloomKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(BloomKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(BloomISampler* sampler, const BloomSamplingParams& params,
    ...
    std::unique_ptr<BloomISampler> make_step_sampler(const BloomSamplingParams& params);
