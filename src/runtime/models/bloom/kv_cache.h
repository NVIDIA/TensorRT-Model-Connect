...
// BloomKvCache: autoregressive KV cache state manager.
...
class BloomKvCache : public BloomInferenceState {
  public:
    ...
    BloomKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                 DType cache_dtype = DType::kFloat32, BloomKvCacheNames names = {});

    // --- BloomInferenceState overrides ---
    void reset() override;
    ...
    // --- BloomKvCache-specific methods (not on the interface) ---
    ...
    // — too big for BloomKvCache's single-row present buffer. The caller reads
