...
// PersonaplexKvCache: autoregressive KV cache state manager.
...
struct PersonaplexKvCacheNames {
...
class PersonaplexKvCache : public PersonaplexInferenceState {
  public:
    ...
    PersonaplexKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                       DType cache_dtype = DType::kFloat32, PersonaplexKvCacheNames names = {});

    // --- PersonaplexInferenceState overrides ---
    void reset() override;
    ...
    // --- PersonaplexKvCache-specific methods (not on the interface) ---

    // DEPRECATED: Use prepare_step() instead.
    // Kept for backward compatibility with tests that call this directly.
    void personaplex_build_attention_mask(std::vector<float>& mask) const;
    ...
    // — too big for PersonaplexKvCache's single-row present buffer. The caller reads
  ...
  private:
    ...
    PersonaplexKvCacheNames names_;
