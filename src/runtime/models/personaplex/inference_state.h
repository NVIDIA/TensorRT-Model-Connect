...
// PersonaplexInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   PersonaplexKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — PersonaplexKvCache + family-owned recurrent state composed
...
class PersonaplexInferenceState {
  public:
    virtual ~PersonaplexInferenceState() = default;
    ...
    // PersonaplexKvCache -> true. Family-owned recurrent state -> false.
