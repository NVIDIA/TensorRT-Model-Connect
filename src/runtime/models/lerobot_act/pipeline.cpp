...
void validate_policy_module(const TrtModule* policy) {
    if (!policy || !policy->ok())
...
void validate_observation_shape(const RobotObservation& observation, int32_t image_height,
    ...
    if (observation.state_dim != state_dim)
        throw std::invalid_argument(
...
LeRobotActPipeline::LeRobotActPipeline(std::unique_ptr<TrtModule> policy, int32_t image_height,
                                       int32_t image_width, int32_t image_channels,
                                       int32_t state_dim, int32_t action_dim, int32_t chunk_size,
    ...
    : policy_(std::move(policy)), image_height_(image_height), image_width_(image_width),
      ...
      action_max_(std::move(action_max)), model_id_(std::move(model_id)) {
...
void LeRobotActPipeline::validate_observation(const RobotObservation& observation) const {
    ...
    validate_image_values(observation.image_pixels, image_values);
    validate_state_values(observation.state, state_dim_);
}

bool LeRobotActPipeline::action_within_bounds(const float* action) const {
    if (!action)
        return false;
...
RobotActionChunk LeRobotActPipeline::predict_action_chunk(const RobotObservation& observation) {
    validate_observation(observation);

    Tensor image;
    image.data = const_cast<float*>(observation.image_pixels);
    image.shape = {1, image_height_, image_width_, image_channels_};
    image.dtype = DType::kFloat32;
    Tensor state;
    state.data = const_cast<float*>(observation.state);
...
RobotAction LeRobotActPipeline::act(const RobotObservation& observation) {
