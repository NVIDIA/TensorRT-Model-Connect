...
// PhiISampler: token selection abstraction for autoregressive generation.
//
...
//
// PhiLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// phi-owned sampler interface.
class PhiISampler {
  public:
    virtual ~PhiISampler() = default;
    ...
    virtual PhiSampleResult sample(const float* logits, int32_t vocab_size,
                                   const PhiSamplingParams& params) = 0;
    ...
    virtual PhiLogitsLocation logits_location() const = 0;
...
/// Build PhiSamplingParams from GenerateConfig fields.
/// Forward-declared here; defined in sampler.cpp alongside the factory.
struct GenerateConfig; // defined in trtmc/pipeline.h

PhiSamplingParams phi_sampling_params_from_config(const GenerateConfig& cfg,
                                                  int32_t default_eos = -1);

/// Factory: create sampler from PhiSamplingParams.
...
std::unique_ptr<PhiISampler> create_phi_sampler(const PhiSamplingParams& params,
                                                const PhiSamplerFactoryOptions& options);
