...
// PhiKvCache: autoregressive KV cache state manager.
...
class PhiKvCache : public PhiInferenceState {
  public:
    ...
    PhiKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
               DType cache_dtype = DType::kFloat32, PhiKvCacheNames names = {});

    // --- PhiInferenceState overrides ---
    void reset() override;
    ...
    // --- PhiKvCache-specific methods (not on the interface) ---
    ...
    // — too big for PhiKvCache's single-row present buffer. The caller reads
