...
#include "runtime/models/phi/inference_state.h"
...
enum class PhiTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class PhiTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct PhiTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    PhiTriAttentionScoreAggregation score_aggregation{PhiTriAttentionScoreAggregation::kMean};
    PhiTriAttentionScoreAggregation per_layer_aggregation{PhiTriAttentionScoreAggregation::kMean};
...
struct PhiTriAttentionHeadStats {
...
struct PhiTriAttentionStats {
    int32_t head_dim{0};
    PhiTriAttentionRopeStyle rope_style{PhiTriAttentionRopeStyle::kHalf};
    ...
    std::vector<PhiTriAttentionHeadStats> layer_stats;
};

struct PhiTriAttentionCompactionProfile {
...
// Build a PhiTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class PhiTriAttentionKvCache : public PhiInferenceState {
  public:
    PhiTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                           int32_t kv_dim, cudaStream_t stream, PhiTriAttentionConfig config,
                           PhiTriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(PhiTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const PhiTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const PhiTriAttentionCompactionProfile* profile_ptr) const;
    ...
    std::vector<int32_t> select_keep_indices(int32_t keep_budget,
                                             PhiTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             PhiTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const PhiTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const PhiTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const PhiTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    PhiKvCacheNames names_;
    PhiTriAttentionConfig config_;
    PhiTriAttentionStats stats_;
