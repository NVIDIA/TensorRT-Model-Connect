...
// PhiInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   PhiKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — PhiKvCache + family-owned recurrent state composed
...
class PhiInferenceState {
  public:
    virtual ~PhiInferenceState() = default;
    ...
    // PhiKvCache -> true. Family-owned recurrent state -> false.
