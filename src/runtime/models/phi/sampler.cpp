// PhiISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// PhiTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/phi/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/phi/argmax_kernel.h"
#include "runtime/models/phi/sparse_multinomial_kernel.h"
...
static PhiSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                          int32_t eos_token_id) {
    PhiSampleResult result;
...
static bool greedy_equivalent(const PhiSamplingParams& params) {
...
static FilteredDistribution build_filtered_distribution(const float* logits, int32_t vocab_size,
                                                        const PhiSamplingParams& params) {
...
class GreedySampler final : public PhiISampler {
  public:
    PhiSampleResult sample(const float* logits, int32_t vocab_size,
                           const PhiSamplingParams& params) override {
    ...
    PhiLogitsLocation logits_location() const override { return PhiLogitsLocation::HOST; }
...
class TopKSampler final : public PhiISampler {
  public:
...
class TorchCudaMultinomialSampler final : public PhiISampler {
  public:
    ...
    PhiSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        phi_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const PhiTorchMultinomialExecutionPolicy policy =
                phi_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public PhiISampler {
  public:
    ...
    PhiSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        phi_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    PhiLogitsLocation logits_location() const override { return PhiLogitsLocation::DEVICE; }
...
PhiSamplingParams phi_sampling_params_from_config(const GenerateConfig& cfg, int32_t default_eos) {
    PhiSamplingParams p;
...
create_phi_sampler(const PhiSamplingParams& params,
                   [[maybe_unused]] const PhiSamplerFactoryOptions& options) {
...
std::unique_ptr<PhiISampler> create_phi_sampler(const PhiSamplingParams& params) {
    return create_phi_sampler(params, PhiSamplerFactoryOptions{});
}

std::unique_ptr<PhiISampler> create_phi_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
