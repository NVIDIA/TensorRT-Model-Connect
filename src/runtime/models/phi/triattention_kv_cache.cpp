#include "runtime/models/phi/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/phi/triattention_kernels.h"
#endif
...
PhiTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return PhiTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return PhiTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(PhiTriAttentionScoreAggregation value) {
    switch (value) {
    case PhiTriAttentionScoreAggregation::kMean:
        return "mean";
    case PhiTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

PhiTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return PhiTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return PhiTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, PhiKvCacheNames& names) {
...
// override helpers that patched PhiTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     PhiTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(PhiTriAttentionConfig& cfg,
...
void fill_debug_from_registry(PhiTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(PhiTriAttentionConfig& cfg, const std::string& config_json,
...
static void validate_triattention_config(const PhiTriAttentionConfig& cfg,
                                         int32_t max_cache_length) {
    if (cfg.kv_budget < 1)
...
PhiTriAttentionConfig
phi_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                     const ::trtmc::config::ConfigBundle* runtime_config) {
    PhiTriAttentionConfig cfg;
...
void fill_stats_header(PhiTriAttentionStats& stats, const json& root, int32_t num_attention_heads,
...
void fill_stats_inv_freq(PhiTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(PhiTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(PhiTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(PhiTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(PhiTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(PhiTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
void init_layer_stats_empty(PhiTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(PhiTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(PhiTriAttentionStats& stats, const std::vector<int32_t>& group_counts,
                           int32_t half_dim) {
    bool any_stats = false;
    for (int32_t layer = 0; layer < stats.num_layers; ++layer) {
        auto& layer_stats = stats.layer_stats[static_cast<std::size_t>(layer)];
...
void parse_sparse_stats(PhiTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
PhiTriAttentionStats phi_parse_triattention_stats_json(const std::string& stats_json,
                                                       int32_t num_attention_heads,
                                                       int32_t num_key_value_heads,
                                                       int32_t num_layers) {
    PhiTriAttentionStats stats;
...
void PhiTriAttentionKvCache::validate_shapes() {
...
void PhiTriAttentionKvCache::normalize_sampled_heads() {
...
void PhiTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void PhiTriAttentionKvCache::allocate_layer_tensors() {
...
PhiTriAttentionKvCache::PhiTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                               int32_t max_length, int32_t kv_dim,
                                               cudaStream_t stream, PhiTriAttentionConfig config,
                                               PhiTriAttentionStats stats, DType cache_dtype,
                                               PhiKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void PhiTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void PhiTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void PhiTriAttentionKvCache::initialize_gpu_state() {
...
bool PhiTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool PhiTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool PhiTriAttentionKvCache::can_use_gpu_selection() const {
...
void PhiTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t PhiTriAttentionKvCache::preferred_cache_rows() const {
...
void PhiTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void PhiTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float>
PhiTriAttentionKvCache::copy_cache_rows_to_host(const DeviceTensor& tensor, int32_t rows,
                                                PhiTriAttentionCompactionProfile* profile) const {
...
void PhiTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t PhiTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> PhiTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t> PhiTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
PhiTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                            PhiTriAttentionCompactionProfile* profile) {
...
int32_t PhiTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t PhiTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> PhiTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void PhiTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, PhiTriAttentionCompactionProfile* profile) const {
...
bool PhiTriAttentionKvCache::layer_stats_shapes_valid(const PhiTriAttentionHeadStats& layer_stats,
...
void PhiTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                           int32_t half_dim, float& k_rot_real,
                                           float& k_rot_imag) const {
    if (stats_.rope_style == PhiTriAttentionRopeStyle::kInterleaved) {
...
float PhiTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == PhiTriAttentionScoreAggregation::kMax)
...
float PhiTriAttentionKvCache::score_one_row(
    const float* row_ptr, const PhiTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void PhiTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const PhiTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void PhiTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void PhiTriAttentionKvCache::accumulate_layer_fallback(
...
void PhiTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == PhiTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void PhiTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
PhiTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void PhiTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == PhiTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void PhiTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> PhiTriAttentionKvCache::build_keep_indices_per_head(
...
bool PhiTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, PhiTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> PhiTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, PhiTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void PhiTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                    int32_t total_tokens) const {
...
void PhiTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void PhiTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
PhiTriAttentionKvCache::GpuLayerResult PhiTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = phi_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == PhiTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == PhiTriAttentionScoreAggregation::kMax,
...
bool PhiTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool PhiTriAttentionKvCache::upload_gpu_trig_phases(int32_t num_offsets, int32_t half_dim,
                                                    PhiTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void PhiTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> PhiTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool PhiTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> PhiTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, PhiTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void PhiTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void PhiTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool PhiTriAttentionKvCache::gpu_compaction_upload_keep(int32_t keep_count,
...
bool PhiTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                   std::size_t row_bytes) {
    const bool ok_k = phi_triattention_compact_rows_gpu(
    ...
    const bool ok_v = phi_triattention_compact_rows_gpu(
...
bool PhiTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
                                                  const std::vector<int32_t>& keep_indices,
...
void PhiTriAttentionKvCache::compact_layer_on_host(int32_t layer,
                                                   const std::vector<int32_t>& keep_indices,
                                                   int32_t keep_count, std::size_t row_bytes,
...
void PhiTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    PhiTriAttentionCompactionProfile profile;
    PhiTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void PhiTriAttentionKvCache::finalize_repack_profile(PhiTriAttentionCompactionProfile* profile_ptr,
...
PhiTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
PhiTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                  int32_t keep_count) const {
...
int32_t PhiTriAttentionKvCache::count_prefix_positions(
...
void PhiTriAttentionKvCache::log_compact_debug(const std::vector<int32_t>& representative_positions,
                                               const std::vector<int32_t>& keep_indices,
                                               int32_t keep_count) const {
    if (!config_.debug)
        return;
...
void PhiTriAttentionKvCache::log_compact_profile(
    const PhiTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool PhiTriAttentionKvCache::should_emit_keep_dump(
    const PhiTriAttentionCompactionProfile* profile_ptr) const {
...
void PhiTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "PhiTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void PhiTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void PhiTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void PhiTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool PhiTriAttentionKvCache::ok() const {
