...
#include "runtime/models/phi/pipeline.h"

#include "runtime/models/phi/chat_templates.h"
#include "runtime/models/phi/kv_cache.h"
#include "runtime/models/phi/tensor_names.h"
...
std::vector<PhiTextGenerationPipeline::DecoderContext>
single_decoder_context(std::unique_ptr<TrtModule> decoder) {
    std::vector<PhiTextGenerationPipeline::DecoderContext> decoders;
    decoders.push_back(PhiTextGenerationPipeline::DecoderContext{0, std::move(decoder)});
    return decoders;
}
...
bool greedy_text_diffusion_params(const PhiSamplingParams& params) {
...
PhiTextGenerationPipeline::PhiTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                                     std::unique_ptr<PhiInferenceState> state,
                                                     PhiTextGenConfig config, cudaStream_t stream,
                                                     std::shared_ptr<ITokenizer> tokenizer,
                                                     std::string model_id_str,
                                                     std::unique_ptr<PhiISampler> sampler,
                                                     std::shared_ptr<void> distributed_owner)
    : PhiTextGenerationPipeline(single_decoder_context(std::move(decoder)), std::move(state),
...
PhiTextGenerationPipeline::PhiTextGenerationPipeline(
    std::vector<DecoderContext> decoders, std::unique_ptr<PhiInferenceState> state,
    PhiTextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<PhiISampler> sampler,
    ...
    if (decoders_.empty()) {
        throw std::runtime_error("PhiTextGenerationPipeline: no decoder modules");
    }
    for (const auto& decoder_ctx : decoders_) {
        if (!decoder_ctx.module || !decoder_ctx.module->ok()) {
            throw std::runtime_error("PhiTextGenerationPipeline: invalid decoder module");
        }
    }
    if (!state_ || !state_->ok()) {
        throw std::runtime_error("PhiTextGenerationPipeline: invalid inference state");
    }
...
static std::vector<int32_t> encode_prompt(const ITokenizer& tokenizer,
                                          const PhiTextGenConfig& config, const std::string& prompt,
                                          const GenerateConfig& cfg) {
    std::string effective = prompt;
    bool templated = false;
    if (cfg.use_chat_template && !config.chat_template_format.empty()) {
        effective =
            phi_apply_chat_template(config.chat_template_format, prompt, cfg.enable_thinking);
        templated = true;
    }
...
TextResult PhiTextGenerationPipeline::generate(const std::string& prompt,
                                               const GenerateConfig& cfg) {
    if (!tokenizer_) {
        throw std::runtime_error("PhiTextGenerationPipeline: no tokenizer configured");
    }
...
PhiTextGenerationPipeline::generate_ids(const std::vector<int32_t>& input_ids,
                                        const GenerateConfig& cfg) {
...
std::unique_ptr<PhiISampler>
PhiTextGenerationPipeline::make_step_sampler(const PhiSamplingParams& params) {
    ...
    if (prefer_gpu_greedy_ && greedy_params) {
        if (auto gpu = create_phi_gpu_greedy_sampler(stream_))
            return gpu;
    }
    return create_phi_sampler(params);
}
...
namespace {
bool gather_prefill_kv_pointers(TrtModule& prefill, const PhiTextGenConfig& cfg,
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        pk[li] = prefill.device_ptr(phi_expand_layer_name(cfg.present_k_pattern, i));
        pv[li] = prefill.device_ptr(phi_expand_layer_name(cfg.present_v_pattern, i));
...
bool batched_prefill_supported(const TrtModule* prefill, const PhiTextGenConfig& cfg, int32_t sq,
                               PhiInferenceState* state) {
    ...
    if (cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        return false;
    return dynamic_cast<PhiKvCache*>(state) != nullptr;
}
} // namespace

bool PhiTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
                                                    std::vector<float>& logits) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    if (!batched_prefill_supported(prefill_.get(), config_, sq, state_.get()))
        return false;
    auto* kv = static_cast<PhiKvCache*>(state_.get());
...
void PhiTextGenerationPipeline::prime_decoder_after_batched_prefill(
...
void PhiTextGenerationPipeline::run_prefill(const std::vector<int32_t>& input_ids,
...
TrtModule& PhiTextGenerationPipeline::require_block_prefill(int32_t sq,
                                                            TrtModule* prefill_override) {
    TrtModule* prefill = prefill_override != nullptr ? prefill_override : prefill_.get();
    if (prefill == nullptr)
        throw std::runtime_error(
            "PhiTextGenerationPipeline: block generation requires prefill module");
    if (sq <= 0)
        throw std::runtime_error("PhiTextGenerationPipeline: empty block");
    if (config_.prefill_max_length > 0 && sq > config_.prefill_max_length) {
        throw std::runtime_error("PhiTextGenerationPipeline: block length exceeds prefill profile");
    }
    return *prefill;
}

PhiKvCache& PhiTextGenerationPipeline::require_block_kv_cache() {
    auto* kv = dynamic_cast<PhiKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error("PhiTextGenerationPipeline: block generation requires PhiKvCache");
    return *kv;
}

void PhiTextGenerationPipeline::copy_block_logits(const TensorMap& outputs,
                                                  std::vector<float>& logits) const {
    auto logits_it = outputs.find(config_.logits_output_name);
    if (logits_it == outputs.end())
        throw std::runtime_error("PhiTextGenerationPipeline: prefill module has no '" +
...
void PhiTextGenerationPipeline::append_prefill_kv(PhiKvCache& kv, TrtModule& prefill, int32_t sq) {
    std::vector<const void*> pk, pv;
    if (!gather_prefill_kv_pointers(prefill, config_, pk, pv)) {
        throw std::runtime_error(
            "PhiTextGenerationPipeline: prefill module is missing present_k/present_v outputs");
    }
    kv.append_prefill_kv(pk, pv, sq);
}

void PhiTextGenerationPipeline::run_prefill_block(const std::vector<int32_t>& input_ids,
                                                  bool bidirectional, bool append_kv,
                                                  std::vector<float>& logits,
                                                  TrtModule* prefill_override) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    TrtModule& prefill = require_block_prefill(sq, prefill_override);
    PhiKvCache& kv = require_block_kv_cache();
...
std::string PhiTextGenerationPipeline::resolve_generation_mode(const GenerateConfig& cfg) const {
...
int32_t PhiTextGenerationPipeline::resolve_text_diffusion_block_length(
    ...
    if (!config_.supports_text_diffusion || config_.mask_token_id < 0)
        throw std::runtime_error(
            "PhiTextGenerationPipeline: bundle does not support text diffusion");
    ...
    if (require_divisible && max_new_tokens % block_len != 0) {
        throw std::runtime_error("PhiTextGenerationPipeline: diffusion mode requires "
...
int32_t
PhiTextGenerationPipeline::seed_next_token_from_prefill(const std::vector<int32_t>& input_ids,
    ...
    if (static_cast<int32_t>(logits.size()) < vocab)
        throw std::runtime_error("PhiTextGenerationPipeline: missing prefill logits");
    return argmax_with_confidence(logits.data() + logits.size() - static_cast<std::size_t>(vocab),
                                  vocab, 0)
        .token_id;
}

void PhiTextGenerationPipeline::fill_diffusion_block(std::vector<int32_t>& block,
    ...
    for (int32_t step = 0; step < block_len && has_mask_token(block, config_.mask_token_id);
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "PhiTextGenerationPipeline: diffusion engine must output full block logits");
        }
...
int32_t PhiTextGenerationPipeline::verify_diffusion_block(const std::vector<int32_t>& block,
                                                          std::vector<float>& logits,
                                                          int32_t block_len, int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "PhiTextGenerationPipeline: diffusion engine must output full verify logits");
    }
...
bool PhiTextGenerationPipeline::append_tokens_until_eos(const std::vector<int32_t>& tokens,
                                                        std::vector<int32_t>& output,
                                                        const PhiSamplingParams& params) const {
...
void PhiTextGenerationPipeline::fill_linear_spec_block(std::vector<int32_t>& block,
                                                       std::vector<float>& logits,
                                                       int32_t block_len, int32_t vocab,
    ...
    while (has_mask_token(block, config_.mask_token_id)) {
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "PhiTextGenerationPipeline: linear_spec engine must output full block logits");
        }
        const auto preds = masked_predictions(logits, block, config_.mask_token_id, vocab);
...
std::vector<int32_t>
PhiTextGenerationPipeline::verify_linear_spec_block(const std::vector<int32_t>& block,
                                                    std::vector<float>& logits, int32_t block_len,
                                                    int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "PhiTextGenerationPipeline: linear_spec engine must output full verify logits");
    }
...
int32_t PhiTextGenerationPipeline::count_linear_spec_accepts(const std::vector<int32_t>& ar_tokens,
                                                             const std::vector<int32_t>& block) {
    if (ar_tokens.empty())
        return 0;
    if (block.size() < 2)
...
bool PhiTextGenerationPipeline::append_linear_spec_tokens(const std::vector<int32_t>& ar_tokens,
                                                          int32_t emit_count,
                                                          std::vector<int32_t>& output,
                                                          int32_t& generated,
                                                          const PhiSamplingParams& params) const {
...
PhiTextGenerationPipeline::TimedGenResult PhiTextGenerationPipeline::generate_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens, const PhiSamplingParams& params,
    const GenerateConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "auto" && mode != "ar")
        throw std::runtime_error("PhiTextGenerationPipeline: unsupported generation mode '" + mode +
                                 "'");

    PhiISampler* active_sampler = sampler_.get();
    std::unique_ptr<PhiISampler> local_sampler;
    if (!active_sampler) {
    ...
    std::vector<float> logits;
    const bool gpu_sampling = (active_sampler->logits_location() == PhiLogitsLocation::DEVICE);
...
PhiTextGenerationPipeline::TimedGenResult PhiTextGenerationPipeline::generate_diffusion_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens, const PhiSamplingParams& params,
    const GenerateConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error(
            "PhiTextGenerationPipeline: diffusion mode currently supports greedy temperature=0 "
            "generation");
    }
    const int32_t block_len =
...
PhiTextGenerationPipeline::TimedGenResult PhiTextGenerationPipeline::generate_linear_spec_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens, const PhiSamplingParams& params,
    const GenerateConfig& cfg, bool use_lora_draft) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error(
            "PhiTextGenerationPipeline: linear_spec mode currently supports greedy temperature=0 "
            "generation");
    }
    if (use_lora_draft && linear_spec_lora_prefill_ == nullptr) {
        throw std::runtime_error(
            "PhiTextGenerationPipeline: linear_spec_lora mode requires a linear-spec LoRA engine");
    }
    const int32_t block_len =
    ...
    auto* kv = dynamic_cast<PhiKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error("PhiTextGenerationPipeline: linear_spec requires PhiKvCache");
...
bool PhiTextGenerationPipeline::should_stop_on_answer(const std::vector<int32_t>& output,
...
void PhiTextGenerationPipeline::log_decode_summary(int32_t steps, double ms) const {
...
int32_t
PhiTextGenerationPipeline::run_decode_loop(PhiISampler* sampler, const PhiSamplingParams& params,
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        ...
        const PhiSampleResult result = sampler->sample(sample_ptr, vocab_size, params);
...
int32_t PhiTextGenerationPipeline::select_decoder_index(int32_t desired_rows) const {
    if (decoders_.size() == 1)
        return 0;
...
TrtModule& PhiTextGenerationPipeline::bind_decoder_for_step() {
...
void PhiTextGenerationPipeline::run_step(int32_t token_id, std::vector<float>& logits) {
    TensorMap inputs;
    ...
    if (it == outputs.end()) {
        throw std::runtime_error("PhiTextGenerationPipeline: no '" + logits_output_name_ +
                                 "' output");
    }
...
void PhiTextGenerationPipeline::run_step_device(int32_t token_id) {
    TensorMap inputs;
...
int32_t PhiTextGenerationPipeline::argmax(const std::vector<float>& logits) {
    if (logits.empty())
        return 0;
