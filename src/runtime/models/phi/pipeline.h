...
//
// Composes: TrtModule (decoder) + PhiKvCache + ITokenizer for this runtime
...
class PhiTextGenerationPipeline final : public IPipeline {
  public:
    ...
    PhiTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                              std::unique_ptr<PhiInferenceState> state, PhiTextGenConfig config,
                              cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer = nullptr,
                              std::string model_id_str = "",
                              std::unique_ptr<PhiISampler> sampler = nullptr,
                              std::shared_ptr<void> distributed_owner = nullptr);
    PhiTextGenerationPipeline(std::vector<DecoderContext> decoders,
                              std::unique_ptr<PhiInferenceState> state, PhiTextGenConfig config,
                              cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer = nullptr,
                              std::string model_id_str = "",
                              std::unique_ptr<PhiISampler> sampler = nullptr,
    ...
    const char* pipeline_type() const override { return "PhiTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<PhiInferenceState> state_;
    PhiTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<PhiISampler> sampler_;
    ...
    TimedGenResult generate_from_ids(const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
                                     const PhiSamplingParams& params, const GenerateConfig& cfg);
    ...
    PhiKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(PhiKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(PhiISampler* sampler, const PhiSamplingParams& params,
    ...
    std::unique_ptr<PhiISampler> make_step_sampler(const PhiSamplingParams& params);
