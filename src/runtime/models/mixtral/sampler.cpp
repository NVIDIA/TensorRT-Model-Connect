// MixtralISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// MixtralTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/mixtral/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/mixtral/argmax_kernel.h"
#include "runtime/models/mixtral/sparse_multinomial_kernel.h"
...
static MixtralSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                              int32_t eos_token_id) {
    MixtralSampleResult result;
...
static bool greedy_equivalent(const MixtralSamplingParams& params) {
...
class GreedySampler final : public MixtralISampler {
  public:
    MixtralSampleResult sample(const float* logits, int32_t vocab_size,
                               const MixtralSamplingParams& params) override {
    ...
    MixtralLogitsLocation logits_location() const override { return MixtralLogitsLocation::HOST; }
...
class TopKSampler final : public MixtralISampler {
  public:
...
class TorchCudaMultinomialSampler final : public MixtralISampler {
  public:
    ...
    MixtralSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        mixtral_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const MixtralTorchMultinomialExecutionPolicy policy =
                mixtral_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public MixtralISampler {
  public:
    ...
    MixtralSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        mixtral_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    MixtralLogitsLocation logits_location() const override { return MixtralLogitsLocation::DEVICE; }
...
MixtralSamplingParams mixtral_sampling_params_from_config(const GenerateConfig& cfg,
                                                          int32_t default_eos) {
    MixtralSamplingParams p;
...
create_mixtral_sampler(const MixtralSamplingParams& params,
                       [[maybe_unused]] const MixtralSamplerFactoryOptions& options) {
...
std::unique_ptr<MixtralISampler> create_mixtral_sampler(const MixtralSamplingParams& params) {
    return create_mixtral_sampler(params, MixtralSamplerFactoryOptions{});
}

std::unique_ptr<MixtralISampler> create_mixtral_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
