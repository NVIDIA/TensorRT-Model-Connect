...
// MixtralISampler: token selection abstraction for autoregressive generation.
//
...
//
// MixtralLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// mixtral-owned sampler interface.
class MixtralISampler {
  public:
    virtual ~MixtralISampler() = default;
    ...
    virtual MixtralSampleResult sample(const float* logits, int32_t vocab_size,
                                       const MixtralSamplingParams& params) = 0;
    ...
    virtual MixtralLogitsLocation logits_location() const = 0;
...
/// Build MixtralSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from MixtralSamplingParams.
...
create_mixtral_sampler(const MixtralSamplingParams& params,
                       const MixtralSamplerFactoryOptions& options);
