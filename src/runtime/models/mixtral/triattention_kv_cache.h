...
#include "runtime/models/mixtral/inference_state.h"
...
enum class MixtralTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class MixtralTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct MixtralTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    MixtralTriAttentionScoreAggregation score_aggregation{
        MixtralTriAttentionScoreAggregation::kMean};
    MixtralTriAttentionScoreAggregation per_layer_aggregation{
        MixtralTriAttentionScoreAggregation::kMean};
...
struct MixtralTriAttentionHeadStats {
...
struct MixtralTriAttentionStats {
    int32_t head_dim{0};
    MixtralTriAttentionRopeStyle rope_style{MixtralTriAttentionRopeStyle::kHalf};
    ...
    std::vector<MixtralTriAttentionHeadStats> layer_stats;
};

struct MixtralTriAttentionCompactionProfile {
...
// Build a MixtralTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class MixtralTriAttentionKvCache : public MixtralInferenceState {
  public:
    MixtralTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                               int32_t kv_dim, cudaStream_t stream,
                               MixtralTriAttentionConfig config, MixtralTriAttentionStats stats,
                               DType cache_dtype = DType::kFloat32, MixtralKvCacheNames names = {});
  ...
  private:
    ...
    void finalize_repack_profile(MixtralTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const MixtralTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const MixtralTriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        MixtralTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             MixtralTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                MixtralTriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const MixtralTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const MixtralTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const MixtralTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    MixtralKvCacheNames names_;
    MixtralTriAttentionConfig config_;
    MixtralTriAttentionStats stats_;
