#include "runtime/models/mixtral/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/mixtral/triattention_kernels.h"
#endif
...
MixtralTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return MixtralTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return MixtralTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(MixtralTriAttentionScoreAggregation value) {
    switch (value) {
    case MixtralTriAttentionScoreAggregation::kMean:
        return "mean";
    case MixtralTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

MixtralTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return MixtralTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return MixtralTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, MixtralKvCacheNames& names) {
...
// override helpers that patched MixtralTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     MixtralTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(MixtralTriAttentionConfig& cfg,
...
void fill_debug_from_registry(MixtralTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(MixtralTriAttentionConfig& cfg,
...
static void validate_triattention_config(const MixtralTriAttentionConfig& cfg,
...
MixtralTriAttentionConfig
mixtral_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                         const ::trtmc::config::ConfigBundle* runtime_config) {
    MixtralTriAttentionConfig cfg;
...
void fill_stats_header(MixtralTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(MixtralTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(MixtralTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(MixtralTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(MixtralTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(MixtralTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(MixtralTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(MixtralTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(MixtralTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(MixtralTriAttentionStats& stats,
...
void parse_sparse_stats(MixtralTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
MixtralTriAttentionStats mixtral_parse_triattention_stats_json(const std::string& stats_json,
                                                               int32_t num_attention_heads,
                                                               int32_t num_key_value_heads,
                                                               int32_t num_layers) {
    MixtralTriAttentionStats stats;
...
void MixtralTriAttentionKvCache::validate_shapes() {
...
void MixtralTriAttentionKvCache::normalize_sampled_heads() {
...
void MixtralTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void MixtralTriAttentionKvCache::allocate_layer_tensors() {
...
MixtralTriAttentionKvCache::MixtralTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                       int32_t max_length, int32_t kv_dim,
                                                       cudaStream_t stream,
                                                       MixtralTriAttentionConfig config,
                                                       MixtralTriAttentionStats stats,
                                                       DType cache_dtype, MixtralKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void MixtralTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void MixtralTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void MixtralTriAttentionKvCache::initialize_gpu_state() {
...
bool MixtralTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool MixtralTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool MixtralTriAttentionKvCache::can_use_gpu_selection() const {
...
void MixtralTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t MixtralTriAttentionKvCache::preferred_cache_rows() const {
...
void MixtralTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void MixtralTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> MixtralTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows, MixtralTriAttentionCompactionProfile* profile) const {
...
void MixtralTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t MixtralTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> MixtralTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
MixtralTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
MixtralTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                                MixtralTriAttentionCompactionProfile* profile) {
...
int32_t MixtralTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t MixtralTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> MixtralTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void MixtralTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, MixtralTriAttentionCompactionProfile* profile) const {
...
bool MixtralTriAttentionKvCache::layer_stats_shapes_valid(
    const MixtralTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void MixtralTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                               int32_t half_dim, float& k_rot_real,
                                               float& k_rot_imag) const {
    if (stats_.rope_style == MixtralTriAttentionRopeStyle::kInterleaved) {
...
float MixtralTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == MixtralTriAttentionScoreAggregation::kMax)
...
float MixtralTriAttentionKvCache::score_one_row(
    const float* row_ptr, const MixtralTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void MixtralTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const MixtralTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void MixtralTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void MixtralTriAttentionKvCache::accumulate_layer_fallback(
...
void MixtralTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == MixtralTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void MixtralTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
MixtralTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void MixtralTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == MixtralTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void MixtralTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> MixtralTriAttentionKvCache::build_keep_indices_per_head(
...
bool MixtralTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, MixtralTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> MixtralTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, MixtralTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void MixtralTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                        int32_t total_tokens) const {
...
void MixtralTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void MixtralTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
MixtralTriAttentionKvCache::GpuLayerResult
MixtralTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = mixtral_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == MixtralTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == MixtralTriAttentionScoreAggregation::kMax,
...
bool MixtralTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool MixtralTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, MixtralTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void MixtralTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> MixtralTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool MixtralTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> MixtralTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, MixtralTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void MixtralTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void MixtralTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool MixtralTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool MixtralTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                       std::size_t row_bytes) {
    const bool ok_k = mixtral_triattention_compact_rows_gpu(
    ...
    const bool ok_v = mixtral_triattention_compact_rows_gpu(
...
bool MixtralTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void MixtralTriAttentionKvCache::compact_layer_on_host(
...
void MixtralTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    MixtralTriAttentionCompactionProfile profile;
    MixtralTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void MixtralTriAttentionKvCache::finalize_repack_profile(
    MixtralTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
MixtralTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
MixtralTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                      int32_t keep_count) const {
...
int32_t MixtralTriAttentionKvCache::count_prefix_positions(
...
void MixtralTriAttentionKvCache::log_compact_debug(
...
void MixtralTriAttentionKvCache::log_compact_profile(
    const MixtralTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool MixtralTriAttentionKvCache::should_emit_keep_dump(
    const MixtralTriAttentionCompactionProfile* profile_ptr) const {
...
void MixtralTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "MixtralTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void MixtralTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void MixtralTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void MixtralTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool MixtralTriAttentionKvCache::ok() const {
