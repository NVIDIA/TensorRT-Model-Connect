...
//
// Composes: TrtModule (decoder) + MixtralKvCache + ITokenizer for this runtime
...
class MixtralKvCache;

struct MixtralTextGenConfig {
...
class MixtralTextGenerationPipeline final : public IPipeline {
  public:
    ...
    MixtralTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                  std::unique_ptr<MixtralInferenceState> state,
                                  MixtralTextGenConfig config, cudaStream_t stream,
                                  std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                  std::string model_id_str = "",
                                  std::unique_ptr<MixtralISampler> sampler = nullptr,
                                  std::shared_ptr<void> distributed_owner = nullptr);
    MixtralTextGenerationPipeline(std::vector<DecoderContext> decoders,
                                  std::unique_ptr<MixtralInferenceState> state,
                                  MixtralTextGenConfig config, cudaStream_t stream,
                                  std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                  std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "MixtralTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<MixtralInferenceState> state_;
    MixtralTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<MixtralISampler> sampler_;
    ...
    MixtralKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(MixtralKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(MixtralISampler* sampler, const MixtralSamplingParams& params,
    ...
    std::unique_ptr<MixtralISampler> make_step_sampler(const MixtralSamplingParams& params);
