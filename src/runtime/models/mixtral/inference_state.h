...
// MixtralInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   MixtralKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — MixtralKvCache + family-owned recurrent state composed
...
class MixtralInferenceState {
  public:
    virtual ~MixtralInferenceState() = default;
    ...
    // MixtralKvCache -> true. Family-owned recurrent state -> false.
