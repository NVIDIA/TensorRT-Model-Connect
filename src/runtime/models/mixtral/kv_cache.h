...
// MixtralKvCache: autoregressive KV cache state manager.
...
class MixtralKvCache : public MixtralInferenceState {
  public:
    ...
    MixtralKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                   DType cache_dtype = DType::kFloat32, MixtralKvCacheNames names = {});

    // --- MixtralInferenceState overrides ---
    void reset() override;
    ...
    // --- MixtralKvCache-specific methods (not on the interface) ---
    ...
    // — too big for MixtralKvCache's single-row present buffer. The caller reads
