...
#include "runtime/models/qwen3_5/kv_cache.h"
...
Qwen35KvCache::Qwen35KvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim,
                             cudaStream_t stream, DType cache_dtype, Qwen35KvCacheNames names)
...
void Qwen35KvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t Qwen35KvCache::preferred_cache_rows() const {
    if (!dynamic_binding_enabled_)
        return max_length_;
    return round_up_rows(std::max(position_, 1), kRuntimeBucketRows, max_length_);
}

void Qwen35KvCache::rebind_cache_rows(int32_t cache_rows) {
...
std::vector<int64_t> Qwen35KvCache::mask_shape_for_engine(int32_t mask_width,
...
void Qwen35KvCache::write_position_input(TensorMap& inputs, int32_t seq_len) {
...
void Qwen35KvCache::write_batched_mask(TensorMap& inputs, int32_t seq_len) {
...
void Qwen35KvCache::write_bidirectional_mask(TensorMap& inputs, int32_t seq_len) {
...
void Qwen35KvCache::write_decode_mask(TensorMap& inputs) {
    const int32_t valid = std::max(0, std::min(position_, max_length_));
...
void Qwen35KvCache::prepare_step(TensorMap& inputs, int32_t seq_len) {
    if (seq_len <= 0)
        seq_len = 1;
...
void Qwen35KvCache::prepare_bidirectional_step(TensorMap& inputs, int32_t seq_len) {
    if (seq_len <= 0)
        seq_len = 1;
    write_position_input(inputs, seq_len);
...
void Qwen35KvCache::bind_to(TrtModule& module) {
...
void Qwen35KvCache::bind_cache_inputs(TrtModule& module) {
    has_position_input_ = module.has_input(names_.position_id);
    for (int32_t i = 0; i < num_layers_; ++i) {
        auto li = static_cast<std::size_t>(i);
        module.bind_external(names_.cache_k[li], cache_k_[li].data());
...
void Qwen35KvCache::write_prefill_kv(const std::vector<const void*>& prefill_k,
                                     const std::vector<const void*>& prefill_v, int32_t seq_len) {
    if (seq_len <= 0)
        return;
    if (seq_len > max_length_)
        throw std::runtime_error("Qwen35KvCache::write_prefill_kv: seq_len exceeds max_length");
    if (static_cast<int32_t>(prefill_k.size()) != num_layers_ ||
        static_cast<int32_t>(prefill_v.size()) != num_layers_) {
        throw std::runtime_error(
            "Qwen35KvCache::write_prefill_kv: per-layer pointer count mismatch");
    }
...
void Qwen35KvCache::append_prefill_kv(const std::vector<const void*>& prefill_k,
                                      const std::vector<const void*>& prefill_v, int32_t seq_len) {
    if (seq_len <= 0)
        return;
    if (position_ + seq_len > max_length_)
        throw std::runtime_error("Qwen35KvCache::append_prefill_kv: append exceeds max_length");
    if (static_cast<int32_t>(prefill_k.size()) != num_layers_ ||
        static_cast<int32_t>(prefill_v.size()) != num_layers_) {
        throw std::runtime_error(
            "Qwen35KvCache::append_prefill_kv: per-layer pointer count mismatch");
    }
    const auto row_bytes = static_cast<std::size_t>(kv_dim_) * cache_element_size_;
    const auto block_bytes = static_cast<std::size_t>(seq_len) * row_bytes;
...
void Qwen35KvCache::set_position(int32_t position) {
    position_ = std::max(0, std::min(position, max_length_));
}

void Qwen35KvCache::advance(int32_t n_tokens) {
    // For now, only single-token advance is supported.
    // n_tokens > 1 reserved for future batched prefill (TASK-10).
    assert(n_tokens == 1 && "Qwen35KvCache::advance: only n_tokens==1 supported");
    (void)n_tokens;
