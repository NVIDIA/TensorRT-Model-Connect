...
// Qwen35InferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   Qwen35KvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — Qwen35KvCache + family-owned recurrent state composed
...
class Qwen35InferenceState {
  public:
    virtual ~Qwen35InferenceState() = default;
    ...
    // Qwen35KvCache -> true. Family-owned recurrent state -> false.
