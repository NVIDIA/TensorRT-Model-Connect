#include "runtime/models/qwen3_5/sampler.h"
...
Qwen35SampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                      int32_t eos_token_id) {
    Qwen35SampleResult result;
...
bool greedy_equivalent(const Qwen35SamplingParams& params) {
...
class Qwen35GreedySampler final : public Qwen35ISampler {
  public:
    Qwen35SampleResult sample(const float* logits, int32_t vocab_size,
                              const Qwen35SamplingParams& params) override {
    ...
    Qwen35LogitsLocation logits_location() const override { return Qwen35LogitsLocation::HOST; }
    const char* sampler_type() const override { return "qwen35_greedy"; }
};

class Qwen35TopKSampler final : public Qwen35ISampler {
  public:
    explicit Qwen35TopKSampler(uint64_t initial_seed)
    ...
    Qwen35LogitsLocation logits_location() const override { return Qwen35LogitsLocation::HOST; }
    const char* sampler_type() const override { return "qwen35_top_k"; }
...
std::unique_ptr<Qwen35ISampler> create_qwen35_sampler(const Qwen35SamplingParams& params) {
    ...
    if (params.top_k <= 1 && top_p >= 1.0F - kSamplingEpsilon && min_p <= 0.0F && params.seed < 0) {
        return std::make_unique<Qwen35GreedySampler>();
    }

    const uint64_t seed = (params.seed >= 0) ? static_cast<uint64_t>(params.seed) : 42ULL;
    return std::make_unique<Qwen35TopKSampler>(seed);
}
