...
Cosmos3Pipeline::Cosmos3Pipeline(std::unique_ptr<TrtModule> reasoner,
    ...
    : reasoner_(std::move(reasoner)),
      reasoner_state_(std::move(reasoner_state)),
      dm_generator_(std::move(dm_generator)),
      vae_decoder_(std::move(vae_decoder)),
      vit_encoder_(std::move(vit_encoder)),
...
DiffusionResult Cosmos3Pipeline::generate_diffusion(const std::string& /*prompt*/,
    ...
    throw std::runtime_error(
        "Cosmos3Pipeline::generate_diffusion not yet implemented.\n"
