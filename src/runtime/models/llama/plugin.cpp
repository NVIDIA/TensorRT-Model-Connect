  ...
  private:
    ...
    static void build_kv_names(const PipelineContext& ctx, const IoMap& io,
        ...
        for (int32_t i = 0; i < ctx.config.num_layers; ++i) {
            ...
            kv_names.present_k.push_back(llama_expand_layer_name(io.present_k_pattern, i));
            kv_names.present_v.push_back(llama_expand_layer_name(io.present_v_pattern, i));
        }
    }
    ...
    static std::vector<LlamaTextGenerationPipeline::DecoderContext>
    ...
    static std::unique_ptr<LlamaInferenceState>
    build_inference_state(const PipelineContext& ctx, const KvCacheRuntimeSizing& sizing,
                          LlamaTriAttentionConfig& tri_cfg, DType cache_dtype, int32_t kv_dim,
                          LlamaKvCacheNames& kv_names, cudaStream_t stream) {
        ...
        if (tri_cfg.enabled) {
            ...
            LlamaTriAttentionStats tri_stats = llama_parse_triattention_stats_json(
                stats_json, ctx.config.num_heads, ctx.config.num_kv_heads, ctx.config.num_layers);
            state = std::make_unique<LlamaTriAttentionKvCache>(
            ...
            state =
                std::make_unique<LlamaKvCache>(ctx.config.num_layers, sizing.runtime_rows, kv_dim,
                                               stream, cache_dtype, std::move(kv_names));
        }
        if (!state->ok())
            throw std::runtime_error("Failed to create LlamaKvCache");
        return state;
    }
    ...
    static void
    populate_text_gen_config(const PipelineContext& ctx, LlamaTextGenConfig& tgc, const IoMap& io,
                             const LlamaTextGenerationPipeline::DecoderContext& first_dec,
    ...
    static void apply_chat_template_format(const BundleFile& bundle, LlamaTextGenConfig& tgc) {
        std::string chat_tpl;
        ...
        tgc.chat_template_format = llama_detect_chat_template_format(chat_tpl);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_llama_plugin, DecoderPlugin,
