...
// LlamaISampler: token selection abstraction for autoregressive generation.
//
...
//
// LlamaLogitsLocation tells the pipeline whether to D2H transfer logits before
...
class LlamaISampler {
  public:
    ...
    virtual LlamaLogitsLocation logits_location() const = 0;
...
/// Build LlamaSamplingParams from GenerateConfig fields.
...
std::unique_ptr<LlamaISampler> create_llama_sampler(const LlamaSamplingParams& params,
                                                    const LlamaSamplerFactoryOptions& options);
