...
enum class LlamaTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class LlamaTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct LlamaTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    LlamaTriAttentionScoreAggregation score_aggregation{LlamaTriAttentionScoreAggregation::kMean};
    LlamaTriAttentionScoreAggregation per_layer_aggregation{
        LlamaTriAttentionScoreAggregation::kMean};
...
struct LlamaTriAttentionHeadStats {
...
struct LlamaTriAttentionStats {
    int32_t head_dim{0};
    LlamaTriAttentionRopeStyle rope_style{LlamaTriAttentionRopeStyle::kHalf};
    ...
    std::vector<LlamaTriAttentionHeadStats> layer_stats;
};

struct LlamaTriAttentionCompactionProfile {
...
// Build a LlamaTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class LlamaTriAttentionKvCache : public LlamaInferenceState {
  public:
    LlamaTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                             int32_t kv_dim, cudaStream_t stream, LlamaTriAttentionConfig config,
                             LlamaTriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(LlamaTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const LlamaTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const LlamaTriAttentionCompactionProfile* profile_ptr) const;
    ...
    std::vector<int32_t> select_keep_indices(int32_t keep_budget,
                                             LlamaTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             LlamaTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                LlamaTriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const LlamaTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const LlamaTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const LlamaTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    LlamaTriAttentionConfig config_;
    LlamaTriAttentionStats stats_;
