// LlamaISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// LlamaTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/llama/argmax_kernel.h"
#include "runtime/models/llama/sparse_multinomial_kernel.h"
...
static bool greedy_equivalent(const LlamaSamplingParams& params) {
...
class GreedySampler final : public LlamaISampler {
  public:
    LlamaSampleResult sample(const float* logits, int32_t vocab_size,
                             const LlamaSamplingParams& params) override {
...
class TopKSampler final : public LlamaISampler {
  public:
...
class TorchCudaMultinomialSampler final : public LlamaISampler {
  public:
    ...
    LlamaSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        llama_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const LlamaTorchMultinomialExecutionPolicy policy =
                llama_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public LlamaISampler {
  public:
    ...
    LlamaSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        llama_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    LlamaLogitsLocation logits_location() const override { return LlamaLogitsLocation::DEVICE; }
...
create_llama_sampler(const LlamaSamplingParams& params,
                     [[maybe_unused]] const LlamaSamplerFactoryOptions& options) {
...
std::unique_ptr<LlamaISampler> create_llama_sampler(const LlamaSamplingParams& params) {
    return create_llama_sampler(params, LlamaSamplerFactoryOptions{});
}

std::unique_ptr<LlamaISampler> create_llama_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
