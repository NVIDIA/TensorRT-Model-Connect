...
#include "runtime/models/llama/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/llama/triattention_kernels.h"
#endif
...
LlamaTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return LlamaTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return LlamaTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(LlamaTriAttentionScoreAggregation value) {
    switch (value) {
    case LlamaTriAttentionScoreAggregation::kMean:
        return "mean";
    case LlamaTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

LlamaTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return LlamaTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return LlamaTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, LlamaKvCacheNames& names) {
...
// override helpers that patched LlamaTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     LlamaTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(LlamaTriAttentionConfig& cfg,
...
void fill_debug_from_registry(LlamaTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(LlamaTriAttentionConfig& cfg, const std::string& config_json,
...
static void validate_triattention_config(const LlamaTriAttentionConfig& cfg,
                                         int32_t max_cache_length) {
    if (cfg.kv_budget < 1)
...
LlamaTriAttentionConfig
llama_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                       const ::trtmc::config::ConfigBundle* runtime_config) {
    LlamaTriAttentionConfig cfg;
...
void fill_stats_header(LlamaTriAttentionStats& stats, const json& root, int32_t num_attention_heads,
...
void fill_stats_inv_freq(LlamaTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(LlamaTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(LlamaTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(LlamaTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(LlamaTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(LlamaTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(LlamaTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(LlamaTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(LlamaTriAttentionStats& stats, const std::vector<int32_t>& group_counts,
                           int32_t half_dim) {
    bool any_stats = false;
    for (int32_t layer = 0; layer < stats.num_layers; ++layer) {
        auto& layer_stats = stats.layer_stats[static_cast<std::size_t>(layer)];
...
void parse_sparse_stats(LlamaTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
LlamaTriAttentionStats llama_parse_triattention_stats_json(const std::string& stats_json,
                                                           int32_t num_attention_heads,
                                                           int32_t num_key_value_heads,
                                                           int32_t num_layers) {
    LlamaTriAttentionStats stats;
...
void LlamaTriAttentionKvCache::validate_shapes() {
...
void LlamaTriAttentionKvCache::normalize_sampled_heads() {
...
void LlamaTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void LlamaTriAttentionKvCache::allocate_layer_tensors() {
...
LlamaTriAttentionKvCache::LlamaTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                   int32_t max_length, int32_t kv_dim,
                                                   cudaStream_t stream,
                                                   LlamaTriAttentionConfig config,
                                                   LlamaTriAttentionStats stats, DType cache_dtype,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void LlamaTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void LlamaTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void LlamaTriAttentionKvCache::initialize_gpu_state() {
...
bool LlamaTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool LlamaTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool LlamaTriAttentionKvCache::can_use_gpu_selection() const {
...
void LlamaTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t LlamaTriAttentionKvCache::preferred_cache_rows() const {
...
void LlamaTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void LlamaTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> LlamaTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows, LlamaTriAttentionCompactionProfile* profile) const {
...
void LlamaTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t LlamaTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> LlamaTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t> LlamaTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
LlamaTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                              LlamaTriAttentionCompactionProfile* profile) {
...
int32_t LlamaTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t LlamaTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> LlamaTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void LlamaTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, LlamaTriAttentionCompactionProfile* profile) const {
...
bool LlamaTriAttentionKvCache::layer_stats_shapes_valid(
    const LlamaTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void LlamaTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                             int32_t half_dim, float& k_rot_real,
                                             float& k_rot_imag) const {
    if (stats_.rope_style == LlamaTriAttentionRopeStyle::kInterleaved) {
...
float LlamaTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == LlamaTriAttentionScoreAggregation::kMax)
...
float LlamaTriAttentionKvCache::score_one_row(
    const float* row_ptr, const LlamaTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void LlamaTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const LlamaTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void LlamaTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void LlamaTriAttentionKvCache::accumulate_layer_fallback(
...
void LlamaTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == LlamaTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void LlamaTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
LlamaTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void LlamaTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == LlamaTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void LlamaTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> LlamaTriAttentionKvCache::build_keep_indices_per_head(
...
bool LlamaTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, LlamaTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> LlamaTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, LlamaTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void LlamaTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                      int32_t total_tokens) const {
...
void LlamaTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void LlamaTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
LlamaTriAttentionKvCache::GpuLayerResult LlamaTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = llama_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == LlamaTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == LlamaTriAttentionScoreAggregation::kMax,
...
bool LlamaTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool LlamaTriAttentionKvCache::upload_gpu_trig_phases(int32_t num_offsets, int32_t half_dim,
                                                      LlamaTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void LlamaTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> LlamaTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool LlamaTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> LlamaTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, LlamaTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void LlamaTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void LlamaTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool LlamaTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool LlamaTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                     std::size_t row_bytes) {
    const bool ok_k = llama_triattention_compact_rows_gpu(
    ...
    const bool ok_v = llama_triattention_compact_rows_gpu(
...
bool LlamaTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void LlamaTriAttentionKvCache::compact_layer_on_host(
...
void LlamaTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    LlamaTriAttentionCompactionProfile profile;
    LlamaTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void LlamaTriAttentionKvCache::finalize_repack_profile(
    LlamaTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
LlamaTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
LlamaTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                    int32_t keep_count) const {
...
int32_t LlamaTriAttentionKvCache::count_prefix_positions(
...
void LlamaTriAttentionKvCache::log_compact_debug(
...
void LlamaTriAttentionKvCache::log_compact_profile(
    const LlamaTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool LlamaTriAttentionKvCache::should_emit_keep_dump(
    const LlamaTriAttentionCompactionProfile* profile_ptr) const {
...
void LlamaTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "LlamaTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void LlamaTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void LlamaTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void LlamaTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool LlamaTriAttentionKvCache::ok() const {
