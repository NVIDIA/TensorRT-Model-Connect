...
// MagpieKvCache: autoregressive KV cache state manager.
...
class MagpieKvCache : public MagpieInferenceState {
  public:
    ...
    MagpieKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, MagpieKvCacheNames names = {});

    // --- MagpieInferenceState overrides ---
    void reset() override;
    ...
    // --- MagpieKvCache-specific methods (not on the interface) ---

    // DEPRECATED: Use prepare_step() instead.
    // Kept for backward compatibility with tests that call this directly.
    void magpie_build_attention_mask(std::vector<float>& mask) const;
    ...
    // — too big for MagpieKvCache's single-row present buffer. The caller reads
