...
// EagleVlmInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   EagleVlmKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — EagleVlmKvCache + family-owned recurrent state composed
...
class EagleVlmInferenceState {
  public:
    virtual ~EagleVlmInferenceState() = default;
    ...
    // EagleVlmKvCache -> true. Family-owned recurrent state -> false.
