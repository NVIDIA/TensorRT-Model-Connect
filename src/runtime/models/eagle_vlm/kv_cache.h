...
// EagleVlmKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/eagle_vlm/inference_state.h"
...
class EagleVlmKvCache : public EagleVlmInferenceState {
  public:
    ...
    EagleVlmKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                    DType cache_dtype = DType::kFloat32, EagleVlmKvCacheNames names = {});

    // --- EagleVlmInferenceState overrides ---
    void reset() override;
    ...
    // --- EagleVlmKvCache-specific methods (not on the interface) ---
    ...
    // — too big for EagleVlmKvCache's single-row present buffer. The caller reads
