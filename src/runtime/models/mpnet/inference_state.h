...
// MpnetInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   MpnetKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — MpnetKvCache + family-owned recurrent state composed
...
class MpnetInferenceState {
  public:
    virtual ~MpnetInferenceState() = default;
    ...
    // MpnetKvCache -> true. Family-owned recurrent state -> false.
