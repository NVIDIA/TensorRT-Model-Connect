...
// MpnetKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/mpnet/inference_state.h"
...
class MpnetKvCache : public MpnetInferenceState {
  public:
    ...
    MpnetKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                 DType cache_dtype = DType::kFloat32, MpnetKvCacheNames names = {});

    // --- MpnetInferenceState overrides ---
    void reset() override;
    ...
    // --- MpnetKvCache-specific methods (not on the interface) ---
    ...
    // — too big for MpnetKvCache's single-row present buffer. The caller reads
