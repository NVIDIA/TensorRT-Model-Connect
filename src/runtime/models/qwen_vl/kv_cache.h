...
// QwenVlKvCache: autoregressive KV cache state manager.
...
class QwenVlKvCache : public QwenVlInferenceState {
  public:
    ...
    QwenVlKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, QwenVlKvCacheNames names = {});

    // --- QwenVlInferenceState overrides ---
    void reset() override;
    ...
    // Confirm that native prefill updated the user-owned cache in place, then
    // advance the logical cache length.
    void append_prefill_kv(const std::vector<const void*>& prefill_k,
                           const std::vector<const void*>& prefill_v, int32_t seq_len);

    // Prefill and decode share the same full-capacity user-owned cache.
