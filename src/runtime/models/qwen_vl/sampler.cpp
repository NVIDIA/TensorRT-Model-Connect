...
// QwenVlISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// QwenVlPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/qwen_vl/argmax_kernel.h"
#include "runtime/models/qwen_vl/sparse_multinomial_kernel.h"
...
static bool greedy_equivalent(const QwenVlSamplingParams& params) {
...
class GreedySampler final : public QwenVlISampler {
  public:
    QwenVlSampleResult sample(const float* logits, int32_t vocab_size,
                              const QwenVlSamplingParams& params) override {
...
class TopKSampler final : public QwenVlISampler {
  public:
...
class TorchCudaMultinomialSampler final : public QwenVlISampler {
  public:
    ...
    QwenVlSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        qwen_vl_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const QwenVlTorchMultinomialExecutionPolicy policy =
                qwen_vl_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public QwenVlISampler {
  public:
    ...
    QwenVlSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        qwen_vl_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    QwenVlLogitsLocation logits_location() const override { return QwenVlLogitsLocation::DEVICE; }
...
create_qwen_vl_sampler(const QwenVlSamplingParams& params,
                       [[maybe_unused]] const QwenVlSamplerFactoryOptions& options) {
...
std::unique_ptr<QwenVlISampler> create_qwen_vl_sampler(const QwenVlSamplingParams& params) {
    return create_qwen_vl_sampler(params, QwenVlSamplerFactoryOptions{});
}

std::unique_ptr<QwenVlISampler> create_qwen_vl_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
