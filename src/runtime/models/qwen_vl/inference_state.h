...
// QwenVlInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   QwenVlKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — QwenVlKvCache + family-owned recurrent state composed
...
class QwenVlInferenceState {
  public:
    virtual ~QwenVlInferenceState() = default;
