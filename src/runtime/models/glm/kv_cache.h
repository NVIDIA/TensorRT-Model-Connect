...
// Runtime-owned full-capacity BF16 cache for TensorRT native KV updates.
...
class GlmKvCache : public GlmInferenceState {
  public:
    ...
    // The prefill engine aliases its present outputs to the same cache storage.
    // Completion therefore validates aliases and advances only logical state.
