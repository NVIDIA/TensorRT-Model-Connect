...
// GlmKvCache: autoregressive KV cache state manager.
...
class GlmKvCache : public GlmInferenceState {
  public:
    ...
    GlmKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
               DType cache_dtype = DType::kFloat32, GlmKvCacheNames names = {});

    // --- GlmInferenceState overrides ---
    void reset() override;
    ...
    // --- GlmKvCache-specific methods (not on the interface) ---
    ...
    // — too big for GlmKvCache's single-row present buffer. The caller reads
