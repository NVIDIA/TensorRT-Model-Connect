...
// GlmISampler: token selection abstraction for autoregressive generation.
//
...
//
// GlmLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// glm-owned sampler interface.
class GlmISampler {
  public:
    virtual ~GlmISampler() = default;
    ...
    virtual GlmSampleResult sample(const float* logits, int32_t vocab_size,
                                   const GlmSamplingParams& params) = 0;
    ...
    virtual GlmLogitsLocation logits_location() const = 0;
...
/// Build GlmSamplingParams from GenerateConfig fields.
/// Forward-declared here; defined in sampler.cpp alongside the factory.
struct GenerateConfig; // defined in trtmc/pipeline.h

GlmSamplingParams glm_sampling_params_from_config(const GenerateConfig& cfg,
                                                  int32_t default_eos = -1);

/// Factory: create sampler from GlmSamplingParams.
...
std::unique_ptr<GlmISampler> create_glm_sampler(const GlmSamplingParams& params,
                                                const GlmSamplerFactoryOptions& options);
