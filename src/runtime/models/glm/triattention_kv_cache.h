...
#include "runtime/models/glm/inference_state.h"
...
enum class GlmTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class GlmTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct GlmTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    GlmTriAttentionScoreAggregation score_aggregation{GlmTriAttentionScoreAggregation::kMean};
    GlmTriAttentionScoreAggregation per_layer_aggregation{GlmTriAttentionScoreAggregation::kMean};
...
struct GlmTriAttentionHeadStats {
...
struct GlmTriAttentionStats {
    int32_t head_dim{0};
    GlmTriAttentionRopeStyle rope_style{GlmTriAttentionRopeStyle::kHalf};
    ...
    std::vector<GlmTriAttentionHeadStats> layer_stats;
};

struct GlmTriAttentionCompactionProfile {
...
// Build a GlmTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class GlmTriAttentionKvCache : public GlmInferenceState {
  public:
    GlmTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                           int32_t kv_dim, cudaStream_t stream, GlmTriAttentionConfig config,
                           GlmTriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(GlmTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const GlmTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const GlmTriAttentionCompactionProfile* profile_ptr) const;
    ...
    std::vector<int32_t> select_keep_indices(int32_t keep_budget,
                                             GlmTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             GlmTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const GlmTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const GlmTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const GlmTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    GlmKvCacheNames names_;
    GlmTriAttentionConfig config_;
    GlmTriAttentionStats stats_;
