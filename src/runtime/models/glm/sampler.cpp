// GlmISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// GlmTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/glm/argmax_kernel.h"
#include "runtime/models/glm/sparse_multinomial_kernel.h"
...
static GlmSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                          int32_t eos_token_id) {
    GlmSampleResult result;
...
static bool greedy_equivalent(const GlmSamplingParams& params) {
...
static FilteredDistribution build_filtered_distribution(const float* logits, int32_t vocab_size,
                                                        const GlmSamplingParams& params) {
...
class GreedySampler final : public GlmISampler {
  public:
    GlmSampleResult sample(const float* logits, int32_t vocab_size,
                           const GlmSamplingParams& params) override {
    ...
    GlmLogitsLocation logits_location() const override { return GlmLogitsLocation::HOST; }
...
class TopKSampler final : public GlmISampler {
  public:
...
class TorchCudaMultinomialSampler final : public GlmISampler {
  public:
    ...
    GlmSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        glm_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const GlmTorchMultinomialExecutionPolicy policy =
                glm_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public GlmISampler {
  public:
    ...
    GlmSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        glm_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    GlmLogitsLocation logits_location() const override { return GlmLogitsLocation::DEVICE; }
...
GlmSamplingParams glm_sampling_params_from_config(const GenerateConfig& cfg, int32_t default_eos) {
    GlmSamplingParams p;
...
create_glm_sampler(const GlmSamplingParams& params,
                   [[maybe_unused]] const GlmSamplerFactoryOptions& options) {
...
std::unique_ptr<GlmISampler> create_glm_sampler(const GlmSamplingParams& params) {
    return create_glm_sampler(params, GlmSamplerFactoryOptions{});
}

std::unique_ptr<GlmISampler> create_glm_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
