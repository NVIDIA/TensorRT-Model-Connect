...
//
// Composes: TrtModule (decoder) + GlmKvCache + ITokenizer for this runtime
...
class GlmTextGenerationPipeline final : public IPipeline {
  public:
    ...
    GlmTextGenerationPipeline(std::vector<DecoderContext> decoders,
                              std::unique_ptr<GlmInferenceState> state, GlmTextGenConfig config,
                              cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer = nullptr,
                              std::string model_id_str = "",
                              std::unique_ptr<GlmISampler> sampler = nullptr,
  ...
  private:
    ...
    GlmKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(GlmKvCache& kv, TrtModule& prefill, int32_t sq);
