...
// Sam3InferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   Sam3KvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — Sam3KvCache + family-owned recurrent state composed
...
class Sam3InferenceState {
  public:
    virtual ~Sam3InferenceState() = default;
    ...
    // Sam3KvCache -> true. Family-owned recurrent state -> false.
