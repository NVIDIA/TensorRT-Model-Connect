...
// Sam3KvCache: autoregressive KV cache state manager.
...
#include "runtime/models/sam3/inference_state.h"
...
class Sam3KvCache : public Sam3InferenceState {
  public:
    ...
    Sam3KvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                DType cache_dtype = DType::kFloat32, Sam3KvCacheNames names = {});

    // --- Sam3InferenceState overrides ---
    void reset() override;
    ...
    // --- Sam3KvCache-specific methods (not on the interface) ---
    ...
    // — too big for Sam3KvCache's single-row present buffer. The caller reads
