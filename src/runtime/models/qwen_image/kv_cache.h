...
// QwenImageKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/qwen_image/inference_state.h"
...
class QwenImageKvCache : public QwenImageInferenceState {
  public:
    ...
    QwenImageKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                     DType cache_dtype = DType::kFloat32, QwenImageKvCacheNames names = {});

    // --- QwenImageInferenceState overrides ---
    void reset() override;
    ...
    // --- QwenImageKvCache-specific methods (not on the interface) ---
    ...
    // — too big for QwenImageKvCache's single-row present buffer. The caller reads
