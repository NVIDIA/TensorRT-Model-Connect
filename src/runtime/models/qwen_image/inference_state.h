...
// QwenImageInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   QwenImageKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — QwenImageKvCache + family-owned recurrent state composed
...
class QwenImageInferenceState {
  public:
    virtual ~QwenImageInferenceState() = default;
    ...
    // QwenImageKvCache -> true. Family-owned recurrent state -> false.
