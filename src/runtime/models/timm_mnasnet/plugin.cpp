...
// TimmMnasnetPlugin: handles timm MNASNet image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_mnasnet/pipeline.h"
...
TimmMnasnetPreprocessConfig make_timm_mnasnet_preprocess_config(const std::string& json) {
    TimmMnasnetPreprocessConfig cfg;
...
class TimmMnasnetPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmMnasnetImageClassificationPipeline>(
            std::move(loaded.module), make_timm_mnasnet_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_mnasnet_plugin, TimmMnasnetPlugin,
                                       "timm_mnasnet_image_classification");
