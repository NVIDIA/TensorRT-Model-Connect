...
// Phi4MultimodalISampler: token selection abstraction for autoregressive generation.
//
...
//
// Phi4MultimodalLogitsLocation tells the pipeline whether to D2H transfer logits before
...
struct Phi4MultimodalSamplingParams {
...
struct Phi4MultimodalSamplerFactoryOptions {
    bool prefer_torch_cuda_multinomial{true};
};

/// Where the sampler expects logits to live.
enum class Phi4MultimodalLogitsLocation {
...
struct Phi4MultimodalSampleResult {
    int32_t token_id{0};
...
/// phi4_multimodal-owned sampler interface.
class Phi4MultimodalISampler {
  public:
    virtual ~Phi4MultimodalISampler() = default;
    ...
    virtual Phi4MultimodalSampleResult sample(const float* logits, int32_t vocab_size,
                                              const Phi4MultimodalSamplingParams& params) = 0;
    ...
    virtual Phi4MultimodalLogitsLocation logits_location() const = 0;
...
/// Build Phi4MultimodalSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from Phi4MultimodalSamplingParams.
...
std::unique_ptr<Phi4MultimodalISampler>
create_phi4_multimodal_sampler(const Phi4MultimodalSamplingParams& params,
                               const Phi4MultimodalSamplerFactoryOptions& options);
