...
void Phi4MultimodalKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t Phi4MultimodalKvCache::preferred_cache_rows() const {
    if (!dynamic_binding_enabled_)
        return max_length_;
    return round_up_rows(std::max(position_, 1), kRuntimeBucketRows, max_length_);
}

void Phi4MultimodalKvCache::rebind_cache_rows(int32_t cache_rows) {
...
std::vector<int64_t> Phi4MultimodalKvCache::mask_shape_for_engine(int32_t mask_width,
...
void Phi4MultimodalKvCache::write_batched_mask(TensorMap& inputs, int32_t seq_len) {
...
void Phi4MultimodalKvCache::write_bidirectional_mask(TensorMap& inputs, int32_t seq_len) {
...
void Phi4MultimodalKvCache::write_decode_mask(TensorMap& inputs) {
    const int32_t valid = std::max(0, std::min(position_, max_length_));
...
void Phi4MultimodalKvCache::prepare_bidirectional_step(TensorMap& inputs, int32_t seq_len) {
    if (seq_len <= 0)
        seq_len = 1;
    write_position_input(inputs, seq_len);
...
void Phi4MultimodalKvCache::write_prefill_kv(const std::vector<const void*>& prefill_k,
    ...
    if (seq_len > max_length_)
        throw std::runtime_error(
            "Phi4MultimodalKvCache::write_prefill_kv: seq_len exceeds max_length");
    if (static_cast<int32_t>(prefill_k.size()) != num_layers_ ||
        static_cast<int32_t>(prefill_v.size()) != num_layers_) {
        throw std::runtime_error(
            "Phi4MultimodalKvCache::write_prefill_kv: per-layer pointer count mismatch");
    }
...
void Phi4MultimodalKvCache::append_prefill_kv(const std::vector<const void*>& prefill_k,
                                              const std::vector<const void*>& prefill_v,
                                              int32_t seq_len) {
    if (seq_len <= 0)
        return;
    if (position_ + seq_len > max_length_)
        throw std::runtime_error(
            "Phi4MultimodalKvCache::append_prefill_kv: append exceeds max_length");
    if (static_cast<int32_t>(prefill_k.size()) != num_layers_ ||
        static_cast<int32_t>(prefill_v.size()) != num_layers_) {
        throw std::runtime_error(
            "Phi4MultimodalKvCache::append_prefill_kv: per-layer pointer count mismatch");
    }
    const auto row_bytes = static_cast<std::size_t>(kv_dim_) * cache_element_size_;
    const auto block_bytes = static_cast<std::size_t>(seq_len) * row_bytes;
...
void Phi4MultimodalKvCache::set_position(int32_t position) {
...
void Phi4MultimodalKvCache::advance(int32_t n_tokens) {
    ...
    assert(n_tokens == 1 && "Phi4MultimodalKvCache::advance: only n_tokens==1 supported");
    (void)n_tokens;
