...
// Phi4MultimodalISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// Phi4MultimodalPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/phi4_multimodal/argmax_kernel.h"
#include "runtime/models/phi4_multimodal/sparse_multinomial_kernel.h"
...
static Phi4MultimodalSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
...
static bool greedy_equivalent(const Phi4MultimodalSamplingParams& params) {
...
class GreedySampler final : public Phi4MultimodalISampler {
  public:
    Phi4MultimodalSampleResult sample(const float* logits, int32_t vocab_size,
                                      const Phi4MultimodalSamplingParams& params) override {
    ...
    Phi4MultimodalLogitsLocation logits_location() const override {
        return Phi4MultimodalLogitsLocation::HOST;
    }
...
class TopKSampler final : public Phi4MultimodalISampler {
  public:
...
class TorchCudaMultinomialSampler final : public Phi4MultimodalISampler {
  public:
    ...
    Phi4MultimodalSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        phi4_multimodal_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep,
                                                           initial_seed_, current_offset_,
                                                           total_threads_, d_token_id_, stream_);
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const Phi4MultimodalTorchMultinomialExecutionPolicy policy =
                phi4_multimodal_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public Phi4MultimodalISampler {
  public:
    ...
    Phi4MultimodalSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        phi4_multimodal_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    Phi4MultimodalLogitsLocation logits_location() const override {
        return Phi4MultimodalLogitsLocation::DEVICE;
    }
...
Phi4MultimodalSamplingParams phi4_multimodal_sampling_params_from_config(const GenerateConfig& cfg,
                                                                         int32_t default_eos) {
    Phi4MultimodalSamplingParams p;
...
std::unique_ptr<Phi4MultimodalISampler> create_phi4_multimodal_sampler(
    const Phi4MultimodalSamplingParams& params,
    [[maybe_unused]] const Phi4MultimodalSamplerFactoryOptions& options) {
...
std::unique_ptr<Phi4MultimodalISampler>
create_phi4_multimodal_sampler(const Phi4MultimodalSamplingParams& params) {
    return create_phi4_multimodal_sampler(params, Phi4MultimodalSamplerFactoryOptions{});
}

std::unique_ptr<Phi4MultimodalISampler> create_phi4_multimodal_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
