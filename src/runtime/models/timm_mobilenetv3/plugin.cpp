...
// TimmMobilenetv3Plugin: handles timm MobileNetV3 image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_mobilenetv3/pipeline.h"
...
TimmMobilenetv3PreprocessConfig make_timm_mobilenetv3_preprocess_config(const std::string& json) {
    TimmMobilenetv3PreprocessConfig cfg;
...
class TimmMobilenetv3Plugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmMobilenetv3ImageClassificationPipeline>(
            std::move(loaded.module), make_timm_mobilenetv3_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_mobilenetv3_plugin, TimmMobilenetv3Plugin,
