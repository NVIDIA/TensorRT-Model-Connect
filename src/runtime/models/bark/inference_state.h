...
// BarkInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   BarkKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — BarkKvCache + family-owned recurrent state composed
...
class BarkInferenceState {
  public:
    virtual ~BarkInferenceState() = default;
    ...
    // BarkKvCache -> true. Family-owned recurrent state -> false.
