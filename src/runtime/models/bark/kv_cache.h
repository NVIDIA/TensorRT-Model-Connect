...
// BarkKvCache: autoregressive KV cache state manager.
...
class BarkKvCache : public BarkInferenceState {
  public:
    ...
    BarkKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                DType cache_dtype = DType::kFloat32, BarkKvCacheNames names = {});

    // --- BarkInferenceState overrides ---
    void reset() override;
    ...
    // --- BarkKvCache-specific methods (not on the interface) ---

    // DEPRECATED: Use prepare_step() instead.
    // Kept for backward compatibility with tests that call this directly.
    void bark_build_attention_mask(std::vector<float>& mask) const;
    ...
    // — too big for BarkKvCache's single-row present buffer. The caller reads
