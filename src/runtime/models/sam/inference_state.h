...
// SamInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   SamKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — SamKvCache + family-owned recurrent state composed
...
class SamInferenceState {
  public:
    virtual ~SamInferenceState() = default;
    ...
    // SamKvCache -> true. Family-owned recurrent state -> false.
