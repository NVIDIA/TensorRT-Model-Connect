...
// SamKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/sam/inference_state.h"
...
class SamKvCache : public SamInferenceState {
  public:
    ...
    SamKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
               DType cache_dtype = DType::kFloat32, SamKvCacheNames names = {});

    // --- SamInferenceState overrides ---
    void reset() override;
    ...
    // --- SamKvCache-specific methods (not on the interface) ---
    ...
    // — too big for SamKvCache's single-row present buffer. The caller reads
