...
// TimmHrnetPlugin: handles timm HRNet image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_hrnet/pipeline.h"
...
TimmHrnetPreprocessConfig make_timm_hrnet_preprocess_config(const std::string& json) {
    TimmHrnetPreprocessConfig cfg;
...
class TimmHrnetPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmHrnetImageClassificationPipeline>(
            std::move(loaded.module), make_timm_hrnet_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_hrnet_plugin, TimmHrnetPlugin,
                                       "timm_hrnet_image_classification");
