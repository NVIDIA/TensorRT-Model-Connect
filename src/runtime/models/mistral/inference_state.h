...
// MistralInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   MistralKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — MistralKvCache + family-owned recurrent state composed
...
class MistralInferenceState {
  public:
    virtual ~MistralInferenceState() = default;
    ...
    // MistralKvCache -> true. Family-owned recurrent state -> false.
