...
MistralTextGenerationPipeline::MistralTextGenerationPipeline(
    ...
    if (decoders_.size() != 1 || !decoders_.front().module || !decoders_.front().module->ok())
        throw std::runtime_error(
            "MistralTextGenerationPipeline requires exactly one native decode module");
    if (!prefill_ || !prefill_->ok())
        throw std::runtime_error(
            "MistralTextGenerationPipeline requires a native split prefill module");
    if (!state_ || !state_->ok()) {
...
namespace {
void gather_prefill_kv_pointers(TrtModule& prefill, const MistralTextGenConfig& cfg,
                                std::vector<const void*>& pk, std::vector<const void*>& pv) {
    if (cfg.num_layers <= 0)
        throw std::runtime_error("Mistral native prefill has no layer geometry");
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        if (pk[li] == nullptr || pv[li] == nullptr) {
            throw std::runtime_error(
                "Mistral native prefill is missing an aliased present_k/present_v output");
        }
    }
}
...
void validate_generation_capacity(const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
    ...
    const auto* kv = dynamic_cast<const MistralKvCache*>(state);
    if (kv == nullptr)
        throw std::runtime_error("Mistral native runtime requires MistralKvCache");
    ...
    if (input_ids.size() > capacity ||
        ...
        throw std::runtime_error(
            "Mistral requested prompt and generation exceed the model's fixed KV cache capacity");
    }
}
} // namespace

void MistralTextGenerationPipeline::run_prefill_chunk(const int32_t* token_ids, int32_t chunk_size,
                                                      MistralKvCache& kv,
    ...
    if (logits_it == outputs.end()) {
        throw std::runtime_error(
            "MistralTextGenerationPipeline: prefill module has no logits output");
    }
    ...
    if (static_cast<std::size_t>(logits_tensor.numel()) < vocab) {
        throw std::runtime_error(
            "MistralTextGenerationPipeline: prefill logits are smaller than vocabulary");
    }
    logits.resize(vocab);
...
void MistralTextGenerationPipeline::log_batched_prefill(int32_t token_count, int32_t chunk_count,
...
void MistralTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
                                                        std::vector<float>& logits,
                                                        bool retain_device_logits) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    if (sq <= 0)
        throw std::invalid_argument("Mistral native prefill requires at least one token");
    auto* kv = dynamic_cast<MistralKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error("Mistral native prefill requires MistralKvCache");
    if (config_.prefill_max_length <= 0)
        throw std::runtime_error("Mistral native prefill engine has no valid profile capacity");
...
void MistralTextGenerationPipeline::run_prefill(const std::vector<int32_t>& input_ids,
    ...
    if (step_trace_enabled()) {
        if (gpu_sampling) {
            throw std::runtime_error(
                "Mistral text trace requires host logits and cannot use GPU-only sampling");
        }
...
MistralTextGenerationPipeline::resolve_generation_mode(const GenerateConfig& cfg) const {
    ...
    if (mode != "auto" && mode != "ar") {
        throw std::runtime_error(
            "Mistral native KV runtime supports autoregressive generation only");
    }
    return mode;
}
