// MistralISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// MistralTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/mistral/argmax_kernel.h"
#include "runtime/models/mistral/sparse_multinomial_kernel.h"
...
static MistralSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                              int32_t eos_token_id) {
    MistralSampleResult result;
...
static bool greedy_equivalent(const MistralSamplingParams& params) {
...
class GreedySampler final : public MistralISampler {
  public:
    MistralSampleResult sample(const float* logits, int32_t vocab_size,
                               const MistralSamplingParams& params) override {
    ...
    MistralLogitsLocation logits_location() const override { return MistralLogitsLocation::HOST; }
...
class TopKSampler final : public MistralISampler {
  public:
...
class TorchCudaMultinomialSampler final : public MistralISampler {
  public:
    ...
    MistralSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        mistral_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const MistralTorchMultinomialExecutionPolicy policy =
                mistral_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public MistralISampler {
  public:
    ...
    MistralSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        mistral_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    MistralLogitsLocation logits_location() const override { return MistralLogitsLocation::DEVICE; }
...
create_mistral_sampler(const MistralSamplingParams& params,
                       [[maybe_unused]] const MistralSamplerFactoryOptions& options) {
...
std::unique_ptr<MistralISampler> create_mistral_sampler(const MistralSamplingParams& params) {
    return create_mistral_sampler(params, MistralSamplerFactoryOptions{});
}
