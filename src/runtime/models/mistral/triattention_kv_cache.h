...
enum class MistralTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class MistralTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct MistralTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    MistralTriAttentionScoreAggregation score_aggregation{
        MistralTriAttentionScoreAggregation::kMean};
    MistralTriAttentionScoreAggregation per_layer_aggregation{
        MistralTriAttentionScoreAggregation::kMean};
...
struct MistralTriAttentionHeadStats {
...
struct MistralTriAttentionStats {
    int32_t head_dim{0};
    MistralTriAttentionRopeStyle rope_style{MistralTriAttentionRopeStyle::kHalf};
    ...
    std::vector<MistralTriAttentionHeadStats> layer_stats;
};

struct MistralTriAttentionCompactionProfile {
...
// Build a MistralTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class MistralTriAttentionKvCache : public MistralInferenceState {
  public:
    MistralTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                               int32_t kv_dim, cudaStream_t stream,
                               MistralTriAttentionConfig config, MistralTriAttentionStats stats,
                               DType cache_dtype = DType::kFloat32, MistralKvCacheNames names = {});
  ...
  private:
    ...
    void finalize_repack_profile(MistralTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const MistralTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const MistralTriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        MistralTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             MistralTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                MistralTriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const MistralTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const MistralTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const MistralTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    MistralKvCacheNames names_;
    MistralTriAttentionConfig config_;
    MistralTriAttentionStats stats_;
