...
#include "runtime/models/mistral/triattention_kernels.h"
#endif
...
MistralTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return MistralTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return MistralTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(MistralTriAttentionScoreAggregation value) {
    switch (value) {
    case MistralTriAttentionScoreAggregation::kMean:
        return "mean";
    case MistralTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

MistralTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return MistralTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return MistralTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, MistralKvCacheNames& names) {
...
// override helpers that patched MistralTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     MistralTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(MistralTriAttentionConfig& cfg,
...
void fill_debug_from_registry(MistralTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(MistralTriAttentionConfig& cfg,
...
static void validate_triattention_config(const MistralTriAttentionConfig& cfg,
...
MistralTriAttentionConfig
mistral_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                         const ::trtmc::config::ConfigBundle* runtime_config) {
    MistralTriAttentionConfig cfg;
...
void fill_stats_header(MistralTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(MistralTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(MistralTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(MistralTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(MistralTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(MistralTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(MistralTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(MistralTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(MistralTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(MistralTriAttentionStats& stats,
...
void parse_sparse_stats(MistralTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
MistralTriAttentionStats mistral_parse_triattention_stats_json(const std::string& stats_json,
                                                               int32_t num_attention_heads,
                                                               int32_t num_key_value_heads,
                                                               int32_t num_layers) {
    MistralTriAttentionStats stats;
...
void MistralTriAttentionKvCache::validate_shapes() {
...
void MistralTriAttentionKvCache::normalize_sampled_heads() {
...
void MistralTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void MistralTriAttentionKvCache::allocate_layer_tensors() {
...
MistralTriAttentionKvCache::MistralTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                       int32_t max_length, int32_t kv_dim,
                                                       cudaStream_t stream,
                                                       MistralTriAttentionConfig config,
                                                       MistralTriAttentionStats stats,
                                                       DType cache_dtype, MistralKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void MistralTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void MistralTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void MistralTriAttentionKvCache::initialize_gpu_state() {
...
bool MistralTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool MistralTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool MistralTriAttentionKvCache::can_use_gpu_selection() const {
...
void MistralTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t MistralTriAttentionKvCache::preferred_cache_rows() const {
...
void MistralTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void MistralTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> MistralTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows, MistralTriAttentionCompactionProfile* profile) const {
...
void MistralTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t MistralTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> MistralTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
MistralTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
MistralTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                                MistralTriAttentionCompactionProfile* profile) {
...
int32_t MistralTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t MistralTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> MistralTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void MistralTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, MistralTriAttentionCompactionProfile* profile) const {
...
bool MistralTriAttentionKvCache::layer_stats_shapes_valid(
    const MistralTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void MistralTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                               int32_t half_dim, float& k_rot_real,
                                               float& k_rot_imag) const {
    if (stats_.rope_style == MistralTriAttentionRopeStyle::kInterleaved) {
...
float MistralTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == MistralTriAttentionScoreAggregation::kMax)
...
float MistralTriAttentionKvCache::score_one_row(
    const float* row_ptr, const MistralTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void MistralTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const MistralTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void MistralTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void MistralTriAttentionKvCache::accumulate_layer_fallback(
...
void MistralTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == MistralTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void MistralTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
MistralTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void MistralTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == MistralTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void MistralTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> MistralTriAttentionKvCache::build_keep_indices_per_head(
...
bool MistralTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, MistralTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> MistralTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, MistralTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void MistralTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                        int32_t total_tokens) const {
...
void MistralTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void MistralTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
MistralTriAttentionKvCache::GpuLayerResult
MistralTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = mistral_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == MistralTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == MistralTriAttentionScoreAggregation::kMax,
...
bool MistralTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool MistralTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, MistralTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void MistralTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> MistralTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool MistralTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> MistralTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, MistralTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void MistralTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void MistralTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool MistralTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool MistralTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                       std::size_t row_bytes) {
    const bool ok_k = mistral_triattention_compact_rows_gpu(
    ...
    const bool ok_v = mistral_triattention_compact_rows_gpu(
...
bool MistralTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void MistralTriAttentionKvCache::compact_layer_on_host(
...
void MistralTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    MistralTriAttentionCompactionProfile profile;
    MistralTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void MistralTriAttentionKvCache::finalize_repack_profile(
    MistralTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
MistralTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
MistralTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                      int32_t keep_count) const {
...
int32_t MistralTriAttentionKvCache::count_prefix_positions(
...
void MistralTriAttentionKvCache::log_compact_debug(
...
void MistralTriAttentionKvCache::log_compact_profile(
    const MistralTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool MistralTriAttentionKvCache::should_emit_keep_dump(
    const MistralTriAttentionCompactionProfile* profile_ptr) const {
...
void MistralTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "MistralTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void MistralTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void MistralTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void MistralTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool MistralTriAttentionKvCache::ok() const {
