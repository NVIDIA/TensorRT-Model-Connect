...
// MistralKvCache: autoregressive KV cache state manager.
...
class MistralKvCache : public MistralInferenceState {
  public:
    ...
    MistralKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                   DType cache_dtype = DType::kFloat32, MistralKvCacheNames names = {});

    // --- MistralInferenceState overrides ---
    void reset() override;
    ...
    // --- MistralKvCache-specific methods (not on the interface) ---
    ...
    // — too big for MistralKvCache's single-row present buffer. The caller reads
