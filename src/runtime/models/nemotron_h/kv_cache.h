...
// NemotronHKvCache: autoregressive KV cache state manager.
...
#include "runtime/models/nemotron_h/inference_state.h"
...
class NemotronHKvCache : public NemotronHInferenceState {
  public:
    ...
    NemotronHKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                     DType cache_dtype = DType::kFloat32, NemotronHKvCacheNames names = {});

    // --- NemotronHInferenceState overrides ---
    void reset() override;
    ...
    // --- NemotronHKvCache-specific methods (not on the interface) ---
    ...
    // — too big for NemotronHKvCache's single-row present buffer. The caller reads
