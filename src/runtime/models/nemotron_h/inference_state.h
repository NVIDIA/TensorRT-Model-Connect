...
// NemotronHInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   NemotronHKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — NemotronHKvCache + family-owned recurrent state composed
...
class NemotronHInferenceState {
  public:
    virtual ~NemotronHInferenceState() = default;
    ...
    // NemotronHKvCache -> true. Family-owned recurrent state -> false.
