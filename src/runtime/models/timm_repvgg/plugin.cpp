...
// TimmRepvggPlugin: handles timm RepVGG image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_repvgg/pipeline.h"
...
TimmRepvggPreprocessConfig make_timm_repvgg_preprocess_config(const std::string& json) {
    TimmRepvggPreprocessConfig cfg;
...
class TimmRepvggPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmRepvggImageClassificationPipeline>(
            std::move(loaded.module), make_timm_repvgg_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_repvgg_plugin, TimmRepvggPlugin,
                                       "timm_repvgg_image_classification");
