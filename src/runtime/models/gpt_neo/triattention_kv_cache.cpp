...
#include "runtime/models/gpt_neo/triattention_kv_cache.h"

#include "trtmc/config/config_bundle.h"
#include "trtmc/config/schema_registry.h"
#include "trtmc/runtime/trt_module.h"
#ifdef TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/gpt_neo/triattention_kernels.h"
#endif
...
GptNeoTriAttentionScoreAggregation parse_score_aggregation(const std::string& value) {
    if (value == "mean")
        return GptNeoTriAttentionScoreAggregation::kMean;
    if (value == "max")
        return GptNeoTriAttentionScoreAggregation::kMax;
    throw std::runtime_error("Unsupported TriAttention score_aggregation: " + value);
}

const char* score_aggregation_name(GptNeoTriAttentionScoreAggregation value) {
    switch (value) {
    case GptNeoTriAttentionScoreAggregation::kMean:
        return "mean";
    case GptNeoTriAttentionScoreAggregation::kMax:
        return "max";
    }
    return "unknown";
}

GptNeoTriAttentionRopeStyle parse_rope_style(const std::string& value) {
    if (value == "interleaved")
        return GptNeoTriAttentionRopeStyle::kInterleaved;
    if (value.empty() || value == "half")
        return GptNeoTriAttentionRopeStyle::kHalf;
...
void maybe_generate_default_names(int32_t num_layers, GptNeoKvCacheNames& names) {
...
// override helpers that patched GptNeoTriAttentionConfig with
...
void apply_aggregation_from_registry(const ::trtmc::config::ConfigBundle& bundle,
                                     ...
                                     GptNeoTriAttentionScoreAggregation& out) {
...
// values.
void overlay_core_runtime_from_registry(GptNeoTriAttentionConfig& cfg,
...
void fill_debug_from_registry(GptNeoTriAttentionConfig& cfg,
                              const ::trtmc::config::ConfigBundle& bundle) {
...
static void fill_core_from_legacy_json(GptNeoTriAttentionConfig& cfg,
...
static void validate_triattention_config(const GptNeoTriAttentionConfig& cfg,
...
GptNeoTriAttentionConfig
gpt_neo_parse_triattention_bundle_config(const std::string& config_json, int32_t max_cache_length,
                                         const ::trtmc::config::ConfigBundle* runtime_config) {
    GptNeoTriAttentionConfig cfg;
...
void fill_stats_header(GptNeoTriAttentionStats& stats, const json& root,
...
void fill_stats_inv_freq(GptNeoTriAttentionStats& stats, const json& root) {
...
void append_sampled_heads_from_array(GptNeoTriAttentionStats& stats, const json& sampled_root,
...
void populate_sampled_heads(GptNeoTriAttentionStats& stats, const json& root,
                            int32_t head_upper_bound) {
...
void copy_dense_head_row(GptNeoTriAttentionHeadStats& dst,
...
bool fill_layer_stats_from_dense(GptNeoTriAttentionHeadStats& dst, const json& layer_node,
...
bool try_parse_dense_layer_stats(GptNeoTriAttentionStats& stats, const json& root,
                                 int32_t half_dim) {
...
void init_layer_stats_empty(GptNeoTriAttentionStats& stats, int32_t half_dim) {
    stats.layer_stats.resize(static_cast<std::size_t>(stats.num_layers));
    const auto flat_size =
...
void accumulate_sparse_entry(GptNeoTriAttentionStats& stats, const json& raw_stats, int32_t layer,
...
bool finalize_sparse_stats(GptNeoTriAttentionStats& stats, const std::vector<int32_t>& group_counts,
                           int32_t half_dim) {
    bool any_stats = false;
    for (int32_t layer = 0; layer < stats.num_layers; ++layer) {
        auto& layer_stats = stats.layer_stats[static_cast<std::size_t>(layer)];
...
void parse_sparse_stats(GptNeoTriAttentionStats& stats, const json& root, int32_t half_dim) {
...
GptNeoTriAttentionStats gpt_neo_parse_triattention_stats_json(const std::string& stats_json,
                                                              int32_t num_attention_heads,
                                                              int32_t num_key_value_heads,
                                                              int32_t num_layers) {
    GptNeoTriAttentionStats stats;
...
void GptNeoTriAttentionKvCache::validate_shapes() {
...
void GptNeoTriAttentionKvCache::normalize_sampled_heads() {
...
void GptNeoTriAttentionKvCache::log_init_debug() const {
    if (!config_.debug)
        return;
...
void GptNeoTriAttentionKvCache::allocate_layer_tensors() {
...
GptNeoTriAttentionKvCache::GptNeoTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads,
                                                     int32_t max_length, int32_t kv_dim,
                                                     cudaStream_t stream,
                                                     GptNeoTriAttentionConfig config,
                                                     GptNeoTriAttentionStats stats,
                                                     DType cache_dtype, GptNeoKvCacheNames names)
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void GptNeoTriAttentionKvCache::allocate_core_selection_buffers(int32_t half_dim) {
...
void GptNeoTriAttentionKvCache::build_layer_gpu_stats(int32_t layer, int32_t half_dim) {
...
void GptNeoTriAttentionKvCache::initialize_gpu_state() {
...
bool GptNeoTriAttentionKvCache::core_selection_buffers_ready() const {
...
bool GptNeoTriAttentionKvCache::layer_gpu_stats_ready(const LayerGpuStats& layer) {
...
bool GptNeoTriAttentionKvCache::can_use_gpu_selection() const {
...
void GptNeoTriAttentionKvCache::build_attention_mask(std::vector<float>& mask) const {
...
int32_t GptNeoTriAttentionKvCache::preferred_cache_rows() const {
...
void GptNeoTriAttentionKvCache::prepare_step(TensorMap& inputs, int32_t /*seq_len*/) {
...
void GptNeoTriAttentionKvCache::bind_to(TrtModule& module) {
...
std::vector<float> GptNeoTriAttentionKvCache::copy_cache_rows_to_host(
    const DeviceTensor& tensor, int32_t rows, GptNeoTriAttentionCompactionProfile* profile) const {
...
void GptNeoTriAttentionKvCache::sync_shared_positions_from_head0() {
...
int32_t GptNeoTriAttentionKvCache::count_prefix_rows() const {
...
std::vector<char> GptNeoTriAttentionKvCache::build_reserve_mask(int32_t total_tokens,
...
std::vector<int32_t>
GptNeoTriAttentionKvCache::broadcast_indices_per_head(std::vector<int32_t> rows,
...
std::vector<int32_t>
GptNeoTriAttentionKvCache::select_keep_indices(int32_t keep_budget,
                                               GptNeoTriAttentionCompactionProfile* profile) {
...
int32_t GptNeoTriAttentionKvCache::compaction_keep_budget(int32_t total_tokens) const {
...
int32_t GptNeoTriAttentionKvCache::compaction_trigger_length() const {
...
std::vector<int32_t> GptNeoTriAttentionKvCache::broadcast_reserved_for_empty_budget(
...
void GptNeoTriAttentionKvCache::precompute_trig_phases(
    std::vector<std::vector<float>>& cos_phase, std::vector<std::vector<float>>& sin_phase,
    int32_t half_dim, GptNeoTriAttentionCompactionProfile* profile) const {
...
bool GptNeoTriAttentionKvCache::layer_stats_shapes_valid(
    const GptNeoTriAttentionHeadStats& layer_stats, int32_t half_dim) const {
...
void GptNeoTriAttentionKvCache::extract_k_rot(const float* row_ptr, int32_t head_offset, int32_t d,
                                              int32_t half_dim, float& k_rot_real,
                                              float& k_rot_imag) const {
    if (stats_.rope_style == GptNeoTriAttentionRopeStyle::kInterleaved) {
...
float GptNeoTriAttentionKvCache::reduce_trig_sums(const std::vector<float>& trig_sums) const {
    if (trig_sums.empty())
        return 0.0F;
    if (config_.score_aggregation == GptNeoTriAttentionScoreAggregation::kMax)
...
float GptNeoTriAttentionKvCache::score_one_row(
    const float* row_ptr, const GptNeoTriAttentionHeadStats& layer_stats, std::size_t stats_base,
...
void GptNeoTriAttentionKvCache::score_rows_for_head(
    std::vector<float>& scores, const std::vector<float>& layer_cache,
    const GptNeoTriAttentionHeadStats& layer_stats, int32_t score_head, int32_t cache_head,
...
void GptNeoTriAttentionKvCache::standardize_scores(std::vector<float>& scores) const {
    if (scores.empty())
        return;
    const float mean =
...
void GptNeoTriAttentionKvCache::accumulate_layer_fallback(
...
void GptNeoTriAttentionKvCache::reduce_group_into_aggregate(
    ...
    const bool use_max = config_.per_layer_aggregation == GptNeoTriAttentionScoreAggregation::kMax;
    (void)cache_head;
    for (int32_t row = 0; row < total_tokens; ++row) {
...
void GptNeoTriAttentionKvCache::accumulate_layer_to_aggregate(
...
std::vector<float>
GptNeoTriAttentionKvCache::compute_fallback_mean(const std::vector<float>& global_fallback_sum,
...
void GptNeoTriAttentionKvCache::finalize_per_head_aggregate(
    std::vector<std::vector<float>>& aggregated_scores,
    ...
    const bool per_layer_max =
        config_.per_layer_aggregation == GptNeoTriAttentionScoreAggregation::kMax;
    for (int32_t cache_head = 0; cache_head < cache_head_count_; ++cache_head) {
...
void GptNeoTriAttentionKvCache::maybe_dump_score_values(
...
std::vector<int32_t> GptNeoTriAttentionKvCache::build_keep_indices_per_head(
...
bool GptNeoTriAttentionKvCache::process_layer_for_host_selection(
    ...
    std::vector<float>& layer_aggregate_dump, GptNeoTriAttentionCompactionProfile* profile) const {
...
std::vector<int32_t> GptNeoTriAttentionKvCache::select_keep_indices_host(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, GptNeoTriAttentionCompactionProfile* profile) const {
...
#ifdef TRTMC_HAS_CUDA_KERNELS
void GptNeoTriAttentionKvCache::standardize_score_rows(float* rows, int32_t num_rows,
                                                       int32_t total_tokens) const {
...
void GptNeoTriAttentionKvCache::accumulate_flat_fallback(const float* rows, int32_t num_rows,
...
void GptNeoTriAttentionKvCache::aggregate_gpu_layer_into_cache_heads(
...
GptNeoTriAttentionKvCache::GpuLayerResult
GptNeoTriAttentionKvCache::process_layer_for_gpu_selection(
    ...
    const bool launched = gpt_neo_triattention_score_candidates_gpu(
        cache_k_[static_cast<std::size_t>(layer)].data(), cache_dtype_, kv_dim_, stats_.head_dim,
        (stats_.rope_style == GptNeoTriAttentionRopeStyle::kInterleaved),
        ...
        config_.score_aggregation == GptNeoTriAttentionScoreAggregation::kMax,
...
bool GptNeoTriAttentionKvCache::upload_candidate_indices_identity(int32_t total_tokens) {
...
bool GptNeoTriAttentionKvCache::upload_gpu_trig_phases(
    int32_t num_offsets, int32_t half_dim, GptNeoTriAttentionCompactionProfile* profile) {
    if (config_.disable_trig)
        return true;
    const auto trig_start = Clock::now();
...
void GptNeoTriAttentionKvCache::finalize_flat_per_head_aggregate(
    std::vector<float>& aggregated_scores,
    const std::vector<int32_t>& contributing_layers_by_cache_head,
...
std::vector<int32_t> GptNeoTriAttentionKvCache::build_keep_from_flat_aggregate(
...
bool GptNeoTriAttentionKvCache::run_gpu_selection_over_layers(
...
std::vector<int32_t> GptNeoTriAttentionKvCache::select_keep_indices_gpu(
    int32_t keep_budget, const std::vector<int32_t>& reserved,
    const std::vector<int32_t>& candidates, GptNeoTriAttentionCompactionProfile* profile) {
    const int32_t need = std::max(0, keep_budget - static_cast<int32_t>(reserved.size()));
    if (need <= 0)
        return broadcast_reserved_for_empty_budget(keep_budget, reserved);
    if (candidates.empty())
...
void GptNeoTriAttentionKvCache::dump_cache_rows_to_file(const DeviceTensor& tensor, int32_t rows,
...
void GptNeoTriAttentionKvCache::maybe_dump_score_cache(int32_t rows, const char* k_pattern,
...
#ifdef TRTMC_HAS_CUDA_KERNELS
bool GptNeoTriAttentionKvCache::gpu_compaction_upload_keep(
...
bool GptNeoTriAttentionKvCache::gpu_compact_one_layer(int32_t layer, int32_t keep_count,
                                                      std::size_t row_bytes) {
    const bool ok_k = gpt_neo_triattention_compact_rows_gpu(
    ...
    const bool ok_v = gpt_neo_triattention_compact_rows_gpu(
...
bool GptNeoTriAttentionKvCache::compact_layer_on_gpu(int32_t layer,
...
void GptNeoTriAttentionKvCache::compact_layer_on_host(
...
void GptNeoTriAttentionKvCache::compact_existing_cache(bool reserve_slot_for_append) {
    const int32_t trigger_length = compaction_trigger_length();
    if (cache_length_ < trigger_length)
        return;

    GptNeoTriAttentionCompactionProfile profile;
    GptNeoTriAttentionCompactionProfile* profile_ptr = profile_enabled_ ? &profile : nullptr;
...
void GptNeoTriAttentionKvCache::finalize_repack_profile(
    GptNeoTriAttentionCompactionProfile* profile_ptr, cudaEvent_t repack_start,
...
GptNeoTriAttentionKvCache::build_new_positions_by_head(const std::vector<int32_t>& keep_indices,
...
std::vector<int32_t>
GptNeoTriAttentionKvCache::collect_dropped_positions(const std::vector<int32_t>& keep_indices,
                                                     int32_t keep_count) const {
...
int32_t GptNeoTriAttentionKvCache::count_prefix_positions(
...
void GptNeoTriAttentionKvCache::log_compact_debug(
...
void GptNeoTriAttentionKvCache::log_compact_profile(
    const GptNeoTriAttentionCompactionProfile* profile_ptr, int32_t keep_count) const {
    if (profile_ptr == nullptr)
        return;
...
bool GptNeoTriAttentionKvCache::should_emit_keep_dump(
    const GptNeoTriAttentionCompactionProfile* profile_ptr) const {
...
void GptNeoTriAttentionKvCache::advance(int32_t n_tokens) {
    assert(n_tokens == 1 && "GptNeoTriAttentionKvCache::advance only supports n_tokens==1");
    (void)n_tokens;
...
void GptNeoTriAttentionKvCache::set_prompt_length(int32_t prompt_length) {
    planned_prompt_length_ = std::max(prompt_length, 0);
}

void GptNeoTriAttentionKvCache::mark_prefill_complete() {
    prompt_end_position_ = std::max(absolute_position_, planned_prompt_length_);
}

void GptNeoTriAttentionKvCache::reset() {
    cache_length_ = 0;
...
bool GptNeoTriAttentionKvCache::ok() const {
