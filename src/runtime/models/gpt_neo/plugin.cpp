...
resolve_kv_cache_runtime_sizing(const PipelineContext& ctx, const TrtModule& module,
                                const GptNeoKvCacheNames& kv_names, DType cache_dtype,
                                const GptNeoTriAttentionConfig& tri_cfg, int32_t kv_dim) {
...
class DecoderPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<GptNeoTextGenerationPipeline>(
  ...
  private:
    ...
    load_split_prefill_module(const PipelineContext& ctx, cudaStream_t stream, const IoMap& io,
                              const GptNeoKvCacheNames& kv_names, int32_t& prefill_profile_idx,
    ...
    static void build_kv_names(const PipelineContext& ctx, const IoMap& io,
        ...
        for (int32_t i = 0; i < ctx.config.num_layers; ++i) {
            kv_names.cache_k.push_back(gpt_neo_expand_layer_name(io.cache_k_pattern, i));
            kv_names.cache_v.push_back(gpt_neo_expand_layer_name(io.cache_v_pattern, i));
            kv_names.present_k.push_back(gpt_neo_expand_layer_name(io.present_k_pattern, i));
            kv_names.present_v.push_back(gpt_neo_expand_layer_name(io.present_v_pattern, i));
        }
    }
    ...
    static std::vector<GptNeoTextGenerationPipeline::DecoderContext>
    build_decoder_contexts(BackendProfileModules profile_modules, int32_t runtime_rows,
        ...
        for (const auto& profile : profile_roles.decode_profiles) {
            ...
            decoders.push_back(GptNeoTextGenerationPipeline::DecoderContext{
    ...
    static std::unique_ptr<GptNeoInferenceState>
    build_inference_state(const PipelineContext& ctx, const KvCacheRuntimeSizing& sizing,
                          GptNeoTriAttentionConfig& tri_cfg, DType cache_dtype, int32_t kv_dim,
                          GptNeoKvCacheNames& kv_names, cudaStream_t stream) {
        std::unique_ptr<GptNeoInferenceState> state;
        if (tri_cfg.enabled) {
            ...
            GptNeoTriAttentionStats tri_stats = gpt_neo_parse_triattention_stats_json(
                stats_json, ctx.config.num_heads, ctx.config.num_kv_heads, ctx.config.num_layers);
            state = std::make_unique<GptNeoTriAttentionKvCache>(
            ...
            state =
                std::make_unique<GptNeoKvCache>(ctx.config.num_layers, sizing.runtime_rows, kv_dim,
                                                stream, cache_dtype, std::move(kv_names));
        }
        if (!state->ok())
            throw std::runtime_error("Failed to create GptNeoKvCache");
        return state;
    }
    ...
    static void
    populate_text_gen_config(const PipelineContext& ctx, GptNeoTextGenConfig& tgc, const IoMap& io,
                             const GptNeoTextGenerationPipeline::DecoderContext& first_dec,
    ...
    static void apply_chat_template_format(const BundleFile& bundle, GptNeoTextGenConfig& tgc) {
        std::string chat_tpl;
        ...
        tgc.chat_template_format = gpt_neo_detect_chat_template_format(chat_tpl);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_gpt_neo_plugin, DecoderPlugin,
                                       "gpt_neo_decoder_kv_cache");
