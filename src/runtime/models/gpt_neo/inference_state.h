...
// GptNeoInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   GptNeoKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — GptNeoKvCache + family-owned recurrent state composed
...
class GptNeoInferenceState {
  public:
    virtual ~GptNeoInferenceState() = default;
    ...
    // GptNeoKvCache -> true. Family-owned recurrent state -> false.
