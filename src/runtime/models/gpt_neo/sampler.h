...
// GptNeoISampler: token selection abstraction for autoregressive generation.
//
...
//
// GptNeoLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// gpt_neo-owned sampler interface.
class GptNeoISampler {
  public:
    virtual ~GptNeoISampler() = default;
    ...
    virtual GptNeoSampleResult sample(const float* logits, int32_t vocab_size,
                                      const GptNeoSamplingParams& params) = 0;
    ...
    virtual GptNeoLogitsLocation logits_location() const = 0;
...
/// Build GptNeoSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from GptNeoSamplingParams.
...
std::unique_ptr<GptNeoISampler> create_gpt_neo_sampler(const GptNeoSamplingParams& params,
                                                       const GptNeoSamplerFactoryOptions& options);
