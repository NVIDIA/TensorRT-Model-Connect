...
#include "runtime/models/gpt_neo/inference_state.h"
...
enum class GptNeoTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class GptNeoTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct GptNeoTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    GptNeoTriAttentionScoreAggregation score_aggregation{GptNeoTriAttentionScoreAggregation::kMean};
    GptNeoTriAttentionScoreAggregation per_layer_aggregation{
        GptNeoTriAttentionScoreAggregation::kMean};
...
struct GptNeoTriAttentionHeadStats {
...
struct GptNeoTriAttentionStats {
    int32_t head_dim{0};
    GptNeoTriAttentionRopeStyle rope_style{GptNeoTriAttentionRopeStyle::kHalf};
    ...
    std::vector<GptNeoTriAttentionHeadStats> layer_stats;
};

struct GptNeoTriAttentionCompactionProfile {
...
// Build a GptNeoTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class GptNeoTriAttentionKvCache : public GptNeoInferenceState {
  public:
    GptNeoTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                              int32_t kv_dim, cudaStream_t stream, GptNeoTriAttentionConfig config,
                              GptNeoTriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(GptNeoTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const GptNeoTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const GptNeoTriAttentionCompactionProfile* profile_ptr) const;
    ...
    select_keep_indices(int32_t keep_budget,
                        GptNeoTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             GptNeoTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    void precompute_trig_phases(std::vector<std::vector<float>>& cos_phase,
                                ...
                                GptNeoTriAttentionCompactionProfile* profile) const;
    bool layer_stats_shapes_valid(const GptNeoTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const GptNeoTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const GptNeoTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    GptNeoKvCacheNames names_;
    GptNeoTriAttentionConfig config_;
    GptNeoTriAttentionStats stats_;
