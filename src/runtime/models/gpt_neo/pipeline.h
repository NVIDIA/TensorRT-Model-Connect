...
//
// Composes: TrtModule (decoder) + GptNeoKvCache + ITokenizer for this runtime
...
class GptNeoTextGenerationPipeline final : public IPipeline {
  public:
    ...
    GptNeoTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                 std::unique_ptr<GptNeoInferenceState> state,
                                 GptNeoTextGenConfig config, cudaStream_t stream,
                                 std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                 std::string model_id_str = "",
                                 std::unique_ptr<GptNeoISampler> sampler = nullptr,
                                 std::shared_ptr<void> distributed_owner = nullptr);
    GptNeoTextGenerationPipeline(std::vector<DecoderContext> decoders,
                                 std::unique_ptr<GptNeoInferenceState> state,
                                 GptNeoTextGenConfig config, cudaStream_t stream,
                                 std::shared_ptr<ITokenizer> tokenizer = nullptr,
                                 std::string model_id_str = "",
    ...
    const char* pipeline_type() const override { return "GptNeoTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<GptNeoInferenceState> state_;
    GptNeoTextGenConfig config_;
    cudaStream_t stream_;
    std::shared_ptr<ITokenizer> tokenizer_;
    std::string model_id_;
    std::unique_ptr<GptNeoISampler> sampler_;
    ...
    GptNeoKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(GptNeoKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    int32_t run_decode_loop(GptNeoISampler* sampler, const GptNeoSamplingParams& params,
    ...
    std::unique_ptr<GptNeoISampler> make_step_sampler(const GptNeoSamplingParams& params);
