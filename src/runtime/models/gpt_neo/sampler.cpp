// GptNeoISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// GptNeoTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/gpt_neo/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/gpt_neo/argmax_kernel.h"
#include "runtime/models/gpt_neo/sparse_multinomial_kernel.h"
...
static GptNeoSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                             int32_t eos_token_id) {
    GptNeoSampleResult result;
...
static bool greedy_equivalent(const GptNeoSamplingParams& params) {
...
class GreedySampler final : public GptNeoISampler {
  public:
    GptNeoSampleResult sample(const float* logits, int32_t vocab_size,
                              const GptNeoSamplingParams& params) override {
    ...
    GptNeoLogitsLocation logits_location() const override { return GptNeoLogitsLocation::HOST; }
...
class TopKSampler final : public GptNeoISampler {
  public:
...
class TorchCudaMultinomialSampler final : public GptNeoISampler {
  public:
    ...
    GptNeoSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        gpt_neo_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const GptNeoTorchMultinomialExecutionPolicy policy =
                gpt_neo_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public GptNeoISampler {
  public:
    ...
    GptNeoSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        gpt_neo_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    GptNeoLogitsLocation logits_location() const override { return GptNeoLogitsLocation::DEVICE; }
...
GptNeoSamplingParams gpt_neo_sampling_params_from_config(const GenerateConfig& cfg,
                                                         int32_t default_eos) {
    GptNeoSamplingParams p;
...
create_gpt_neo_sampler(const GptNeoSamplingParams& params,
                       [[maybe_unused]] const GptNeoSamplerFactoryOptions& options) {
...
std::unique_ptr<GptNeoISampler> create_gpt_neo_sampler(const GptNeoSamplingParams& params) {
    return create_gpt_neo_sampler(params, GptNeoSamplerFactoryOptions{});
}

std::unique_ptr<GptNeoISampler> create_gpt_neo_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
