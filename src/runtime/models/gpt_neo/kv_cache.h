...
// GptNeoKvCache: autoregressive KV cache state manager.
...
class GptNeoKvCache : public GptNeoInferenceState {
  public:
    ...
    GptNeoKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, GptNeoKvCacheNames names = {});

    // --- GptNeoInferenceState overrides ---
    void reset() override;
    ...
    // --- GptNeoKvCache-specific methods (not on the interface) ---
    ...
    // — too big for GptNeoKvCache's single-row present buffer. The caller reads
