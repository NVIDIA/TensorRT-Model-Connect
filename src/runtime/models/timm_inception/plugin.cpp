...
// TimmInceptionPlugin: handles timm Inception image-classification bundles.

#include "plugin_helpers.h"
#include "runtime/models/timm_inception/pipeline.h"
...
TimmInceptionPreprocessConfig make_timm_inception_preprocess_config(const std::string& json) {
    TimmInceptionPreprocessConfig cfg;
...
class TimmInceptionPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<TimmInceptionImageClassificationPipeline>(
            std::move(loaded.module), make_timm_inception_preprocess_config(ctx.config_json),
            ctx.bundle.info.model_id);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_timm_inception_plugin, TimmInceptionPlugin,
                                       "timm_inception_image_classification");
