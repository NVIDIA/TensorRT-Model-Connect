...
// Caller-owned, full-capacity native TensorRT KV state for DeepSeek-OCR.
...
class DeepseekOcrKvCache : public DeepseekOcrInferenceState {
  public:
    DeepseekOcrKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                       DType cache_dtype = DType::kBFloat16, DeepseekOcrKvCacheNames names = {});
  ...
  private:
    void validate_and_bind(TrtModule& module);
