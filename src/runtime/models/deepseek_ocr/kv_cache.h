...
// DeepseekOcrKvCache: autoregressive KV cache state manager.
...
struct DeepseekOcrKvCacheNames {
...
class DeepseekOcrKvCache : public DeepseekOcrInferenceState {
  public:
    ...
    DeepseekOcrKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                       DType cache_dtype = DType::kFloat32, DeepseekOcrKvCacheNames names = {});

    // --- DeepseekOcrInferenceState overrides ---
    void reset() override;
    ...
    // --- DeepseekOcrKvCache-specific methods (not on the interface) ---
    ...
    // — too big for DeepseekOcrKvCache's single-row present buffer. The caller reads
  ...
  private:
    ...
    DeepseekOcrKvCacheNames names_;
