...
// DeepseekOcrISampler: token selection abstraction for autoregressive generation.
//
...
//
// DeepseekOcrLogitsLocation tells the pipeline whether to D2H transfer logits before
...
class DeepseekOcrISampler {
  public:
    ...
    virtual DeepseekOcrLogitsLocation logits_location() const = 0;
...
/// Build DeepseekOcrSamplingParams from GenerateConfig fields.
...
std::unique_ptr<DeepseekOcrISampler>
create_deepseek_ocr_sampler(const DeepseekOcrSamplingParams& params,
                            const DeepseekOcrSamplerFactoryOptions& options);
