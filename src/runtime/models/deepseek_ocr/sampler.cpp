...
// DeepseekOcrISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// DeepseekOcrPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/deepseek_ocr/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/deepseek_ocr/argmax_kernel.h"
#include "runtime/models/deepseek_ocr/sparse_multinomial_kernel.h"
...
static DeepseekOcrSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                                  int32_t eos_token_id) {
    DeepseekOcrSampleResult result;
...
static bool greedy_equivalent(const DeepseekOcrSamplingParams& params) {
...
class GreedySampler final : public DeepseekOcrISampler {
  public:
    DeepseekOcrSampleResult sample(const float* logits, int32_t vocab_size,
                                   const DeepseekOcrSamplingParams& params) override {
    ...
    DeepseekOcrLogitsLocation logits_location() const override {
        return DeepseekOcrLogitsLocation::HOST;
    }
...
class TopKSampler final : public DeepseekOcrISampler {
  public:
...
class TorchCudaMultinomialSampler final : public DeepseekOcrISampler {
  public:
    ...
    DeepseekOcrSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        deepseek_ocr_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const DeepseekOcrTorchMultinomialExecutionPolicy policy =
                deepseek_ocr_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public DeepseekOcrISampler {
  public:
    ...
    DeepseekOcrSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        deepseek_ocr_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    DeepseekOcrLogitsLocation logits_location() const override {
        return DeepseekOcrLogitsLocation::DEVICE;
    }
...
DeepseekOcrSamplingParams deepseek_ocr_sampling_params_from_config(const GenerateConfig& cfg,
                                                                   int32_t default_eos) {
    DeepseekOcrSamplingParams p;
...
create_deepseek_ocr_sampler(const DeepseekOcrSamplingParams& params,
                            [[maybe_unused]] const DeepseekOcrSamplerFactoryOptions& options) {
...
std::unique_ptr<DeepseekOcrISampler>
create_deepseek_ocr_sampler(const DeepseekOcrSamplingParams& params) {
    return create_deepseek_ocr_sampler(params, DeepseekOcrSamplerFactoryOptions{});
}

std::unique_ptr<DeepseekOcrISampler> create_deepseek_ocr_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
