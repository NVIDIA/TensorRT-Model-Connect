...
// DeepseekOcrInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   DeepseekOcrKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — DeepseekOcrKvCache + family-owned recurrent state composed
...
class DeepseekOcrInferenceState {
  public:
    virtual ~DeepseekOcrInferenceState() = default;
