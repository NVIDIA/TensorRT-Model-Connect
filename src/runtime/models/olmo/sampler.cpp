// OlmoISampler implementations: GreedySampler, TopKSampler, and factory.
//
// GreedySampler wraps the same std::max_element logic as the original
// OlmoTextGenerationPipeline::argmax() and select_argmax_token(), producing
...
#include "runtime/models/olmo/sampler.h"

#include "trtmc/pipeline.h"
#if TRTMC_HAS_CUDA_KERNELS
#include "runtime/models/olmo/argmax_kernel.h"
#include "runtime/models/olmo/sparse_multinomial_kernel.h"
...
static OlmoSampleResult argmax_over_logits(const float* logits, int32_t vocab_size,
                                           int32_t eos_token_id) {
    OlmoSampleResult result;
...
static bool greedy_equivalent(const OlmoSamplingParams& params) {
...
class GreedySampler final : public OlmoISampler {
  public:
    OlmoSampleResult sample(const float* logits, int32_t vocab_size,
                            const OlmoSamplingParams& params) override {
    ...
    OlmoLogitsLocation logits_location() const override { return OlmoLogitsLocation::HOST; }
...
class TopKSampler final : public OlmoISampler {
  public:
...
class TorchCudaMultinomialSampler final : public OlmoISampler {
  public:
    ...
    OlmoSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        cudaMemcpyAsync(d_probs_, dist.probs.data(),
                        ...
                        stream_);
        olmo_gpu_sparse_torch_multinomial_exact(d_indices_, d_probs_, dist.keep, initial_seed_,
  ...
  private:
    ...
    void ensure_execution_policy(int32_t vocab_size) {
        if (vocab_size != cached_vocab_size_) {
            const OlmoTorchMultinomialExecutionPolicy policy =
                olmo_compute_torch_multinomial_execution_policy(vocab_size);
...
#if TRTMC_HAS_CUDA_KERNELS
class GpuGreedySampler final : public OlmoISampler {
  public:
    ...
    OlmoSampleResult sample(const float* logits, int32_t vocab_size,
        ...
        olmo_gpu_argmax(logits, vocab_size, d_token_id_, d_logit_val_, stream_);
    ...
    OlmoLogitsLocation logits_location() const override { return OlmoLogitsLocation::DEVICE; }
...
OlmoSamplingParams olmo_sampling_params_from_config(const GenerateConfig& cfg,
                                                    int32_t default_eos) {
    OlmoSamplingParams p;
...
create_olmo_sampler(const OlmoSamplingParams& params,
                    [[maybe_unused]] const OlmoSamplerFactoryOptions& options) {
...
std::unique_ptr<OlmoISampler> create_olmo_sampler(const OlmoSamplingParams& params) {
    return create_olmo_sampler(params, OlmoSamplerFactoryOptions{});
}

std::unique_ptr<OlmoISampler> create_olmo_gpu_greedy_sampler(void* stream) {
#if TRTMC_HAS_CUDA_KERNELS
