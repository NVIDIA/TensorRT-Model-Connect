...
// OlmoInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   OlmoKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — OlmoKvCache + family-owned recurrent state composed
...
class OlmoInferenceState {
  public:
    virtual ~OlmoInferenceState() = default;
    ...
    // OlmoKvCache -> true. Family-owned recurrent state -> false.
