...
bool engine_uses_native_kv_updates(const TrtModule& module, const OlmoKvCacheNames& kv_names) {
    ...
    if (has_write_indices != has_kv_lengths) {
        throw std::runtime_error("Olmo native KV engine must expose both cache_write_indices and "
                                 "key_value_lengths");
    }
...
void validate_native_kv_marker(const std::string& config_json, bool engine_uses_native_kv) {
    ...
    if (!declares_native_kv || !engine_uses_native_kv ||
        ...
        throw std::runtime_error("Olmo native KV metadata does not match the engine contract");
    }
}

bool validate_native_kv_runtime(const PipelineContext& ctx, const TrtModule& module,
                                const OlmoKvCacheNames& kv_names, DType cache_dtype,
                                const OlmoTriAttentionConfig& tri_cfg,
    ...
    if (cache_dtype != DType::kFloat16 || tri_cfg.enabled || tp_cfg.enabled) {
        throw std::runtime_error(
            "Olmo native KV requires FP16, single-GPU, non-TriAttention runtime");
    }
    if (ctx.config.head_dim <= 0) {
        throw std::runtime_error("Olmo native KV requires a positive head_dim");
    }
    ...
    if (module.tensor_shape(kv_names.cache_k.front()) != expected_shape) {
        throw std::runtime_error("Olmo native KV cache shape does not match bundle geometry");
    }
    return true;
}
...
std::uint64_t checked_multiply(std::uint64_t lhs, std::uint64_t rhs) {
    if (lhs != 0 && rhs > std::numeric_limits<std::uint64_t>::max() / lhs)
        throw std::overflow_error("Olmo native KV byte accounting overflow");
    return lhs * rhs;
}
...
void admit_native_kv_allocation(const PipelineContext& ctx, bool native_kv,
    ...
    if (status != cudaSuccess) {
        throw std::runtime_error(std::string("Olmo native KV CUDA memory query failed: ") +
    ...
    if (sizing.cache_bytes > available) {
        throw std::runtime_error(
            "Olmo native KV cache admission failed before allocation: capacity=" +
...
void reject_native_kv_size_override(const PipelineContext& ctx) {
    if (ctx.kv_cache_size_bytes != 0) {
        throw std::invalid_argument(
            "Olmo native TensorRT KV cache allocates the model's complete fixed "
...
void apply_runtime_kv_size_override(const PipelineContext& ctx, const TrtModule& module,
                                    const OlmoKvCacheNames& kv_names,
                                    const OlmoTriAttentionConfig& tri_cfg, int32_t bundle_max_rows,
...
KvCacheRuntimeSizing resolve_kv_cache_runtime_sizing(
    const PipelineContext& ctx, const TrtModule& module, const OlmoKvCacheNames& kv_names,
    DType cache_dtype, const OlmoTriAttentionConfig& tri_cfg, int32_t kv_dim, bool native_kv) {
    KvCacheRuntimeSizing sizing;
    const int32_t bundle_max_rows = ctx.config.max_cache_length;
    if (ctx.config.num_layers <= 0 || kv_dim <= 0 || bundle_max_rows <= 0)
        throw std::runtime_error("Olmo KV geometry must be positive");
...
class DecoderPlugin final : public IPipelinePlugin {
  public:
    std::unique_ptr<IPipeline> create(const PipelineContext& ctx) override {
        ...
        return std::make_unique<OlmoTextGenerationPipeline>(
  ...
  private:
    ...
    load_split_prefill_module(const PipelineContext& ctx, cudaStream_t stream, const IoMap& io,
                              const OlmoKvCacheNames& kv_names, int32_t& prefill_profile_idx,
    ...
    static void build_kv_names(const PipelineContext& ctx, const IoMap& io,
        ...
        for (int32_t i = 0; i < ctx.config.num_layers; ++i) {
            kv_names.cache_k.push_back(olmo_expand_layer_name(io.cache_k_pattern, i));
            kv_names.cache_v.push_back(olmo_expand_layer_name(io.cache_v_pattern, i));
            kv_names.present_k.push_back(olmo_expand_layer_name(io.present_k_pattern, i));
            kv_names.present_v.push_back(olmo_expand_layer_name(io.present_v_pattern, i));
        }
    }
    ...
    static std::vector<OlmoTextGenerationPipeline::DecoderContext>
    build_decoder_contexts(BackendProfileModules profile_modules, int32_t runtime_rows,
        ...
        for (const auto& profile : profile_roles.decode_profiles) {
            ...
            decoders.push_back(OlmoTextGenerationPipeline::DecoderContext{
    ...
    static std::unique_ptr<OlmoInferenceState>
    build_inference_state(const PipelineContext& ctx, const KvCacheRuntimeSizing& sizing,
                          OlmoTriAttentionConfig& tri_cfg, DType cache_dtype, int32_t kv_dim,
                          OlmoKvCacheNames& kv_names, cudaStream_t stream) {
        ...
        if (tri_cfg.enabled) {
            ...
            OlmoTriAttentionStats tri_stats = olmo_parse_triattention_stats_json(
                stats_json, ctx.config.num_heads, ctx.config.num_kv_heads, ctx.config.num_layers);
            state = std::make_unique<OlmoTriAttentionKvCache>(
            ...
            state = std::make_unique<OlmoKvCache>(ctx.config.num_layers, sizing.runtime_rows,
                                                  kv_dim, stream, cache_dtype, std::move(kv_names));
        }
        if (!state->ok())
            throw std::runtime_error("Failed to create OlmoKvCache");
        return state;
    }
    ...
    static void
    populate_text_gen_config(const PipelineContext& ctx, OlmoTextGenConfig& tgc, const IoMap& io,
                             const OlmoTextGenerationPipeline::DecoderContext& first_dec,
    ...
    static void apply_chat_template_format(const BundleFile& bundle, OlmoTextGenConfig& tgc) {
        std::string chat_tpl;
        ...
        tgc.chat_template_format = olmo_detect_chat_template_format(chat_tpl);
    }
};

REGISTER_PIPELINE_PLUGIN_WITH_MANIFEST(register_olmo_plugin, DecoderPlugin,
                                       "olmo_decoder_kv_cache");
