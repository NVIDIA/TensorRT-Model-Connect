...
#include "runtime/models/olmo/inference_state.h"
...
enum class OlmoTriAttentionScoreAggregation {
    kMean,
    kMax,
};

enum class OlmoTriAttentionRopeStyle {
    kHalf,
    kInterleaved,
};

struct OlmoTriAttentionConfig {
    bool enabled{false};
    int32_t kv_budget{0};
    int32_t divide_length{128};
    int32_t recent_window{128};
    OlmoTriAttentionScoreAggregation score_aggregation{OlmoTriAttentionScoreAggregation::kMean};
    OlmoTriAttentionScoreAggregation per_layer_aggregation{OlmoTriAttentionScoreAggregation::kMean};
...
struct OlmoTriAttentionHeadStats {
...
struct OlmoTriAttentionStats {
    int32_t head_dim{0};
    OlmoTriAttentionRopeStyle rope_style{OlmoTriAttentionRopeStyle::kHalf};
    ...
    std::vector<OlmoTriAttentionHeadStats> layer_stats;
};

struct OlmoTriAttentionCompactionProfile {
...
// Build a OlmoTriAttentionConfig from the bundle's JSON plus (optionally) the
...
class OlmoTriAttentionKvCache : public OlmoInferenceState {
  public:
    OlmoTriAttentionKvCache(int32_t num_layers, int32_t num_kv_heads, int32_t max_length,
                            int32_t kv_dim, cudaStream_t stream, OlmoTriAttentionConfig config,
                            OlmoTriAttentionStats stats, DType cache_dtype = DType::kFloat32,
  ...
  private:
    ...
    void finalize_repack_profile(OlmoTriAttentionCompactionProfile* profile_ptr,
    ...
    void log_compact_profile(const OlmoTriAttentionCompactionProfile* profile_ptr,
                             int32_t keep_count) const;
    bool should_emit_keep_dump(const OlmoTriAttentionCompactionProfile* profile_ptr) const;
    ...
    std::vector<int32_t> select_keep_indices(int32_t keep_budget,
                                             OlmoTriAttentionCompactionProfile* profile = nullptr);
    ...
    select_keep_indices_host(int32_t keep_budget, const std::vector<int32_t>& reserved,
                             ...
                             OlmoTriAttentionCompactionProfile* profile = nullptr) const;
    std::vector<int32_t>
    ...
    bool layer_stats_shapes_valid(const OlmoTriAttentionHeadStats& layer_stats,
    ...
    float score_one_row(const float* row_ptr, const OlmoTriAttentionHeadStats& layer_stats,
    ...
    void score_rows_for_head(std::vector<float>& scores, const std::vector<float>& layer_cache,
                             const OlmoTriAttentionHeadStats& layer_stats, int32_t score_head,
    ...
    OlmoKvCacheNames names_;
    OlmoTriAttentionConfig config_;
    OlmoTriAttentionStats stats_;
