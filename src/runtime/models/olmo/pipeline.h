...
//
// Composes: TrtModule (decoder) + OlmoKvCache + ITokenizer for this runtime
...
class OlmoTextGenerationPipeline final : public IPipeline {
  public:
    ...
    OlmoTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                               ...
                               std::unique_ptr<OlmoISampler> sampler = nullptr,
                               std::shared_ptr<void> distributed_owner = nullptr);
    OlmoTextGenerationPipeline(std::vector<DecoderContext> decoders,
                               std::unique_ptr<OlmoInferenceState> state, OlmoTextGenConfig config,
                               cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer = nullptr,
                               std::string model_id_str = "",
                               std::unique_ptr<OlmoISampler> sampler = nullptr,
    ...
    const char* pipeline_type() const override { return "OlmoTextGenerationPipeline"; }
  ...
  private:
    ...
    std::unique_ptr<OlmoISampler> sampler_;
    ...
    TimedGenResult generate_from_ids(const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
                                     const OlmoSamplingParams& params, const GenerateConfig& cfg);
    ...
    OlmoKvCache& require_block_kv_cache();
    void copy_block_logits(const TensorMap& outputs, std::vector<float>& logits) const;
    void append_prefill_kv(OlmoKvCache& kv, TrtModule& prefill, int32_t sq);
    ...
    std::unique_ptr<OlmoISampler> make_step_sampler(const OlmoSamplingParams& params);
    ...
    void run_prefill_chunk(const int32_t* token_ids, int32_t chunk_size, OlmoKvCache& kv,
