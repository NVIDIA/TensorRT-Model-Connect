...
// OlmoISampler: token selection abstraction for autoregressive generation.
//
...
//
// OlmoLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// olmo-owned sampler interface.
class OlmoISampler {
  public:
    virtual ~OlmoISampler() = default;
    ...
    virtual OlmoSampleResult sample(const float* logits, int32_t vocab_size,
                                    const OlmoSamplingParams& params) = 0;
    ...
    virtual OlmoLogitsLocation logits_location() const = 0;
...
/// Build OlmoSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from OlmoSamplingParams.
...
std::unique_ptr<OlmoISampler> create_olmo_sampler(const OlmoSamplingParams& params,
                                                  const OlmoSamplerFactoryOptions& options);
