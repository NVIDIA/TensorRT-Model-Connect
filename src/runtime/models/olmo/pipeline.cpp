...
#include "runtime/models/olmo/pipeline.h"

#include "runtime/models/olmo/chat_templates.h"
#include "runtime/models/olmo/kv_cache.h"
#include "runtime/models/olmo/tensor_names.h"
...
std::vector<OlmoTextGenerationPipeline::DecoderContext>
single_decoder_context(std::unique_ptr<TrtModule> decoder) {
    std::vector<OlmoTextGenerationPipeline::DecoderContext> decoders;
    decoders.push_back(OlmoTextGenerationPipeline::DecoderContext{0, std::move(decoder)});
    return decoders;
}
...
bool greedy_text_diffusion_params(const OlmoSamplingParams& params) {
...
OlmoTextGenerationPipeline::OlmoTextGenerationPipeline(
    std::unique_ptr<TrtModule> decoder, std::unique_ptr<OlmoInferenceState> state,
    OlmoTextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<OlmoISampler> sampler,
    std::shared_ptr<void> distributed_owner)
    : OlmoTextGenerationPipeline(single_decoder_context(std::move(decoder)), std::move(state),
...
OlmoTextGenerationPipeline::OlmoTextGenerationPipeline(
    std::vector<DecoderContext> decoders, std::unique_ptr<OlmoInferenceState> state,
    OlmoTextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<OlmoISampler> sampler,
    ...
    if (decoders_.empty()) {
        throw std::runtime_error("OlmoTextGenerationPipeline: no decoder modules");
    }
    for (const auto& decoder_ctx : decoders_) {
        if (!decoder_ctx.module || !decoder_ctx.module->ok()) {
            throw std::runtime_error("OlmoTextGenerationPipeline: invalid decoder module");
        }
    }
    if (!state_ || !state_->ok()) {
        throw std::runtime_error("OlmoTextGenerationPipeline: invalid inference state");
    }
...
static std::vector<int32_t> encode_prompt(const ITokenizer& tokenizer,
                                          const OlmoTextGenConfig& config,
...
TextResult OlmoTextGenerationPipeline::generate(const std::string& prompt,
                                                const GenerateConfig& cfg) {
    if (!tokenizer_) {
        throw std::runtime_error("OlmoTextGenerationPipeline: no tokenizer configured");
    }
...
OlmoTextGenerationPipeline::generate_ids(const std::vector<int32_t>& input_ids,
                                         const GenerateConfig& cfg) {
...
std::unique_ptr<OlmoISampler>
OlmoTextGenerationPipeline::make_step_sampler(const OlmoSamplingParams& params) {
    ...
    if (prefer_gpu_greedy_ && greedy_params) {
        if (auto gpu = create_olmo_gpu_greedy_sampler(stream_))
            return gpu;
    }
    return create_olmo_sampler(params);
}
...
namespace {
bool gather_prefill_kv_pointers(TrtModule& prefill, const OlmoTextGenConfig& cfg,
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        pk[li] = prefill.device_ptr(olmo_expand_layer_name(cfg.present_k_pattern, i));
        pv[li] = prefill.device_ptr(olmo_expand_layer_name(cfg.present_v_pattern, i));
...
bool batched_prefill_supported(const TrtModule* prefill, const OlmoTextGenConfig& cfg, int32_t sq,
                               OlmoInferenceState* state) {
    if (prefill == nullptr || sq <= 0)
        return false;
    if (cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        return false;
    return dynamic_cast<OlmoKvCache*>(state) != nullptr;
}

void validate_generation_capacity(const std::vector<int32_t>& input_ids, int32_t max_new_tokens,
                                  OlmoInferenceState* state, const TrtModule* prefill,
    ...
    const auto* kv = dynamic_cast<const OlmoKvCache*>(state);
    if (kv == nullptr)
        return;
    ...
    if (input_ids.size() > capacity ||
        ...
        throw std::runtime_error(
            "Olmo requested prompt and generation exceed the model's fixed KV cache capacity");
    }
}

int32_t resolve_batched_prefill_chunk_limit(const OlmoKvCache& kv, const OlmoTextGenConfig& config,
                                            int32_t token_count) {
    ...
    if (config.prefill_max_length <= 0)
        throw std::runtime_error("Olmo native KV prefill engine has no valid profile capacity");
    return config.prefill_max_length;
}
} // namespace

void OlmoTextGenerationPipeline::run_prefill_chunk(const int32_t* token_ids, int32_t chunk_size,
                                                   OlmoKvCache& kv,
    ...
    if (logits_it == outputs.end()) {
        throw std::runtime_error("OlmoTextGenerationPipeline: prefill module has no logits output");
    }
    ...
    if (static_cast<std::size_t>(logits_tensor.numel()) < vocab) {
        throw std::runtime_error(
            "OlmoTextGenerationPipeline: prefill logits are smaller than vocabulary");
    }
    logits.resize(vocab);
    ...
    if (retain_device_logits) {
        ...
        if (device_logits == nullptr) {
            throw std::runtime_error(
                "OlmoTextGenerationPipeline: prefill logits have no device buffer");
        }
...
void OlmoTextGenerationPipeline::log_batched_prefill(int32_t token_count, int32_t chunk_count,
...
bool OlmoTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
                                                     std::vector<float>& logits,
                                                     bool retain_device_logits) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    if (!batched_prefill_supported(prefill_.get(), config_, sq, state_.get()))
        return false;
    auto* kv = static_cast<OlmoKvCache*>(state_.get());
    ...
    if (sq > kv->max_length()) {
        throw std::runtime_error("Olmo sequence exceeds the model's fixed KV cache capacity");
    }
...
void OlmoTextGenerationPipeline::prime_decoder_after_batched_prefill(
...
void OlmoTextGenerationPipeline::run_prefill(const std::vector<int32_t>& input_ids,
...
TrtModule& OlmoTextGenerationPipeline::require_block_prefill(int32_t sq,
                                                             TrtModule* prefill_override) {
    TrtModule* prefill = prefill_override != nullptr ? prefill_override : prefill_.get();
    if (prefill == nullptr)
        throw std::runtime_error(
            "OlmoTextGenerationPipeline: block generation requires prefill module");
    if (sq <= 0)
        throw std::runtime_error("OlmoTextGenerationPipeline: empty block");
    if (config_.prefill_max_length > 0 && sq > config_.prefill_max_length) {
        throw std::runtime_error(
            "OlmoTextGenerationPipeline: block length exceeds prefill profile");
    }
    return *prefill;
}

OlmoKvCache& OlmoTextGenerationPipeline::require_block_kv_cache() {
    auto* kv = dynamic_cast<OlmoKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error(
            "OlmoTextGenerationPipeline: block generation requires OlmoKvCache");
    return *kv;
}

void OlmoTextGenerationPipeline::copy_block_logits(const TensorMap& outputs,
                                                   std::vector<float>& logits) const {
    auto logits_it = outputs.find(config_.logits_output_name);
    if (logits_it == outputs.end())
        throw std::runtime_error("OlmoTextGenerationPipeline: prefill module has no '" +
...
void OlmoTextGenerationPipeline::append_prefill_kv(OlmoKvCache& kv, TrtModule& prefill,
                                                   int32_t sq) {
    std::vector<const void*> pk, pv;
    if (!gather_prefill_kv_pointers(prefill, config_, pk, pv)) {
        throw std::runtime_error("OlmoTextGenerationPipeline: prefill module is missing "
                                 "present_k/present_v outputs");
    }
    kv.append_prefill_kv(pk, pv, sq);
}

void OlmoTextGenerationPipeline::run_prefill_block(const std::vector<int32_t>& input_ids,
                                                   bool bidirectional, bool append_kv,
                                                   std::vector<float>& logits,
                                                   TrtModule* prefill_override) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    TrtModule& prefill = require_block_prefill(sq, prefill_override);
    OlmoKvCache& kv = require_block_kv_cache();
...
std::string OlmoTextGenerationPipeline::resolve_generation_mode(const GenerateConfig& cfg) const {
...
int32_t OlmoTextGenerationPipeline::resolve_text_diffusion_block_length(
    ...
    if (!config_.supports_text_diffusion || config_.mask_token_id < 0)
        throw std::runtime_error(
            "OlmoTextGenerationPipeline: bundle does not support text diffusion");
    ...
    if (require_divisible && max_new_tokens % block_len != 0) {
        throw std::runtime_error("OlmoTextGenerationPipeline: diffusion mode requires "
                                 "max_new_tokens % block_length == 0");
    }
    return block_len;
}

int32_t OlmoTextGenerationPipeline::seed_next_token_from_prefill(
    ...
    if (static_cast<int32_t>(logits.size()) < vocab)
        throw std::runtime_error("OlmoTextGenerationPipeline: missing prefill logits");
    return argmax_with_confidence(logits.data() + logits.size() - static_cast<std::size_t>(vocab),
                                  vocab, 0)
        .token_id;
}

void OlmoTextGenerationPipeline::fill_diffusion_block(std::vector<int32_t>& block,
    ...
    for (int32_t step = 0; step < block_len && has_mask_token(block, config_.mask_token_id);
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "OlmoTextGenerationPipeline: diffusion engine must output full block logits");
        }
...
int32_t OlmoTextGenerationPipeline::verify_diffusion_block(const std::vector<int32_t>& block,
                                                           std::vector<float>& logits,
                                                           int32_t block_len, int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "OlmoTextGenerationPipeline: diffusion engine must output full verify logits");
    }
...
bool OlmoTextGenerationPipeline::append_tokens_until_eos(const std::vector<int32_t>& tokens,
                                                         std::vector<int32_t>& output,
                                                         const OlmoSamplingParams& params) const {
...
void OlmoTextGenerationPipeline::fill_linear_spec_block(std::vector<int32_t>& block,
                                                        std::vector<float>& logits,
                                                        int32_t block_len, int32_t vocab,
    ...
    while (has_mask_token(block, config_.mask_token_id)) {
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "OlmoTextGenerationPipeline: linear_spec engine must output full block logits");
        }
        const auto preds = masked_predictions(logits, block, config_.mask_token_id, vocab);
...
std::vector<int32_t>
OlmoTextGenerationPipeline::verify_linear_spec_block(const std::vector<int32_t>& block,
                                                     std::vector<float>& logits, int32_t block_len,
                                                     int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "OlmoTextGenerationPipeline: linear_spec engine must output full verify logits");
    }
...
int32_t OlmoTextGenerationPipeline::count_linear_spec_accepts(const std::vector<int32_t>& ar_tokens,
                                                              const std::vector<int32_t>& block) {
    if (ar_tokens.empty())
        return 0;
    if (block.size() < 2)
...
bool OlmoTextGenerationPipeline::append_linear_spec_tokens(const std::vector<int32_t>& ar_tokens,
                                                           int32_t emit_count,
                                                           std::vector<int32_t>& output,
                                                           int32_t& generated,
                                                           const OlmoSamplingParams& params) const {
...
OlmoTextGenerationPipeline::TimedGenResult OlmoTextGenerationPipeline::generate_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens, const OlmoSamplingParams& params,
    const GenerateConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    ...
    if (mode != "auto" && mode != "ar")
        throw std::runtime_error("OlmoTextGenerationPipeline: unsupported generation mode '" +
                                 mode + "'");

    OlmoISampler* active_sampler = sampler_.get();
    std::unique_ptr<OlmoISampler> local_sampler;
    if (!active_sampler) {
    ...
    std::vector<float> logits;
    const bool gpu_sampling = (active_sampler->logits_location() == OlmoLogitsLocation::DEVICE);
...
OlmoTextGenerationPipeline::TimedGenResult OlmoTextGenerationPipeline::generate_diffusion_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens, const OlmoSamplingParams& params,
    const GenerateConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error("OlmoTextGenerationPipeline: diffusion mode currently "
...
OlmoTextGenerationPipeline::TimedGenResult
OlmoTextGenerationPipeline::generate_linear_spec_from_ids(const std::vector<int32_t>& input_ids,
    ...
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error("OlmoTextGenerationPipeline: linear_spec mode currently "
                                 "supports greedy temperature=0 "
                                 "generation");
    }
    if (use_lora_draft && linear_spec_lora_prefill_ == nullptr) {
        throw std::runtime_error("OlmoTextGenerationPipeline: linear_spec_lora mode requires a "
    ...
    auto* kv = dynamic_cast<OlmoKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error("OlmoTextGenerationPipeline: linear_spec requires OlmoKvCache");
...
bool OlmoTextGenerationPipeline::should_stop_on_answer(const std::vector<int32_t>& output,
...
void OlmoTextGenerationPipeline::log_decode_summary(int32_t steps, double ms) const {
...
int32_t OlmoTextGenerationPipeline::run_decode_loop(
    OlmoISampler* sampler, const OlmoSamplingParams& params, std::vector<int32_t>& output,
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        ...
        const OlmoSampleResult result = sampler->sample(sample_ptr, vocab_size, params);
...
int32_t OlmoTextGenerationPipeline::select_decoder_index(int32_t desired_rows) const {
    if (decoders_.size() == 1)
        return 0;
...
TrtModule& OlmoTextGenerationPipeline::bind_decoder_for_step() {
...
void OlmoTextGenerationPipeline::run_step(int32_t token_id, std::vector<float>& logits) {
    TensorMap inputs;
    ...
    if (it == outputs.end()) {
        throw std::runtime_error("OlmoTextGenerationPipeline: no '" + logits_output_name_ +
                                 "' output");
    }
...
void OlmoTextGenerationPipeline::run_step_device(int32_t token_id) {
    TensorMap inputs;
...
int32_t OlmoTextGenerationPipeline::argmax(const std::vector<float>& logits) {
    if (logits.empty())
        return 0;
