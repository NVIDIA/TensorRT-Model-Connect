...
void OlmoTextGenerationPipeline::append_prefill_kv(OlmoKvCache& kv, TrtModule& prefill,
    ...
    if (!gather_prefill_kv_pointers(prefill, config_, pk, pv)) {
        throw std::runtime_error(
            "OlmoTextGenerationPipeline: prefill module is missing present_k/present_v outputs");
    }
...
OlmoTextGenerationPipeline::TimedGenResult OlmoTextGenerationPipeline::generate_diffusion_from_ids(
    ...
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error(
            "OlmoTextGenerationPipeline: diffusion mode currently supports greedy temperature=0 "
            "generation");
    }
    const int32_t block_len =
...
OlmoTextGenerationPipeline::generate_linear_spec_from_ids(const std::vector<int32_t>& input_ids,
    ...
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error(
            "OlmoTextGenerationPipeline: linear_spec mode currently supports greedy temperature=0 "
            "generation");
    }
    if (use_lora_draft && linear_spec_lora_prefill_ == nullptr) {
        throw std::runtime_error(
            "OlmoTextGenerationPipeline: linear_spec_lora mode requires a linear-spec LoRA engine");
    }
    const int32_t block_len =
