...
// OlmoKvCache: autoregressive KV cache state manager.
...
class OlmoKvCache : public OlmoInferenceState {
  public:
    ...
    OlmoKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                DType cache_dtype = DType::kFloat32, OlmoKvCacheNames names = {});

    // --- OlmoInferenceState overrides ---
    void reset() override;
    ...
    // --- OlmoKvCache-specific methods (not on the interface) ---
    ...
    // — too big for OlmoKvCache's single-row present buffer. The caller reads
