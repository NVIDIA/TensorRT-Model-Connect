...
def test_generic_reference_has_no_family_dispatch() -> None:
    source = Path(generic_reference.__file__).read_text(encoding="utf-8").casefold()
    for family in (
        "magpie_tts",
        "canary",
    ...
    assert "arguments.family" not in source
...
def repository(tmp_path, monkeypatch):
    ...
    canonical = root / "qualification_tests/benchmark_qualification/performance/config/release.yaml"
...
def test_execute_entry_validates_actual_script_process(repository, tmp_path, monkeypatch, failure):
    ...
    if failure:
        ...
        with pytest.raises(perf.PerfMatrixError, match=expected):
            perf.execute_entry(entry, environment, tmp_path / "run", no_build=True, verbose=False, attempt=1)
    else:
        row = perf.execute_entry(entry, environment, tmp_path / "run", no_build=True, verbose=False, attempt=1)
