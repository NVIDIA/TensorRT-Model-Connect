...
def test_every_release_entry_preserves_its_existing_explicit_timing():
    _, entries, _ = perf.load_suite(perf.REPOSITORY / "qualification_tests/benchmark_qualification/performance/config/release.yaml")
    assert entries
...
def _loader(monkeypatch, actual):
    ...
    def load(*_args):
        ...
        return generic_reference.Session(invoke, "fixture", **actual)
    ...
    monkeypatch.setitem(generic_reference.LOADERS, "hf-transformers-vision", load)
    monkeypatch.setattr(generic_reference, "_measure", measure)
    monkeypatch.setattr(generic_reference, "_environment", lambda: {})
    return calls
...
def test_real_session_metadata_not_family_name_controls_validation(tmp_path, monkeypatch, family):
    ...
    assert generic_reference.run(arguments) == 0
...
def test_session_timing_drift_is_rejected_before_warmup_or_measurement(tmp_path, monkeypatch, actual):
    ...
    with pytest.raises(RuntimeError, match="timing drifted"):
        generic_reference.run(arguments)
    assert calls == ["load"]
...
def test_ambiguous_embedding_reference_requires_existing_explicit_timing_option():
    with pytest.raises(ValueError, match="requires explicit --timing-contract-json"):
        generic_reference._load_embedding(SimpleNamespace(family="bert"), {"prompt": "hello"}, {})
    with pytest.raises(ValueError, match="preloads its assets"):
        generic_reference._load_embedding(
