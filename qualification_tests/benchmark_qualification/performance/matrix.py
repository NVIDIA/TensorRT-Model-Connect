...
from qualification_tests.benchmark_qualification.performance.references.hf_transformers import flatten_config  # noqa: E402
from qualification_tests.benchmark_qualification.performance import reference_protocol  # noqa: E402
from qualification_tests.benchmark_qualification.performance.reporting import write_report  # noqa: E402
...
from qualification_tests.benchmark_qualification.performance.stability import (  # noqa: E402
    measurement_stability as _measurement_stability,
    p50 as _p50,
    timing_stability as _timing_stability,  # noqa: F401 - compatibility for existing tests
)
from qualification_tests.benchmark_qualification.performance.types import (  # noqa: E402
    Environment,
    PerfMatrixError,
    ResolvedEntry,
)
...
def load_suite(path: Path) -> tuple[str, list[dict[str, Any]], set[str]]:
    ...
    canonical = REPOSITORY / "qualification_tests/benchmark_qualification/performance/config/release.yaml"
...
def _baseline_command(
    ...
    *,
    ...
    local_files_only = bool(baseline.get("local_files_only", environment.local_files_only))
    trust_remote_code = bool(entry.manifest.get("trust_remote_code", False))
    common = [
        "--model",
        model,
        "--request-json",
        json.dumps(request_value, ensure_ascii=True, separators=(",", ":")),
        "--precision",
    ...
    if runner == "hf-transformers":
        ...
        return reference_protocol.command(
            python=getattr(environment, "reference_python", Path(sys.executable)),
            generic_runner=environment.task_runner,
            script=_family_script(entry.spec) if "script" in baseline else None,
            family=entry.model.family,
            operation=str(entry.spec["operation"]),
            manifest=entry.model.manifest_path,
            selected_task=(
                _effective_task(entry)
                if "script" in baseline or entry.case.selected_task is not None
                else None
            ),
            testcase_name=entry.case.testcase_name,
            adapter=None if "script" in baseline else str(baseline["adapter"]),
            adapter_options=_adapter_options(entry, environment),
            timing_contract=entry.baseline_timing,
            padding=str(baseline.get("padding", "longest")),
            model=model,
            revision=revision,
            request=request_value,
            precision=entry.reference_precision,
            mode=selected_mode,
            warmup=entry.case.measurement.warmup,
            iterations=entry.case.measurement.iterations,
