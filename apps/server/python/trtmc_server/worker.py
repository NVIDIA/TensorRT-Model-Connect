...
"""Long-lived JSONL subprocess transport for ``trtmc-server _serve-worker``."""
...
# This local server owns only independently loadable, single-process lanes.
# MPI/NCCL rank and rendezvous state must not cross into its child workers.
...
class WorkerLoadOptions:
    """Native load-time options forwarded to every private worker."""

    runtime_root: str | None = None
    kv_cache_size_bytes: int | None = None
    runtime_cache: str | None = None
    cuda_graphs: bool = False

    def __post_init__(self) -> None:
        if self.runtime_root is not None and not self.runtime_root:
            raise ValueError("runtime_root must be non-empty when provided")
        if (
            self.kv_cache_size_bytes is not None
            and (
                isinstance(self.kv_cache_size_bytes, bool)
                or self.kv_cache_size_bytes <= 0
            )
        ):
            raise ValueError("kv_cache_size_bytes must be positive")

    def argv(self) -> list[str]:
        result: list[str] = []
        if self.runtime_root is not None:
            result.extend(("--runtime-root", self.runtime_root))
        if self.kv_cache_size_bytes is not None:
            result.extend(("--kv-cache-size", str(self.kv_cache_size_bytes)))
...
class WorkerGroup:
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "ready": ready,
            "degraded": not closed and 0 < ready_replicas < self.replicas,
