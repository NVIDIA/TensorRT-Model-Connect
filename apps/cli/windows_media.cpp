...
bool reference_limit_ticks(const ReferenceMediaDecodePolicy& policy,
    ...
    ticks = static_cast<std::uint64_t>(policy.maximum_duration_seconds) * kAudioMediaTicksPerSecond;
    return true;
}
...
std::uint64_t rounded_reference_frame_slot(std::uint64_t frame, std::uint32_t fps_numerator,
    ...
    if (fps_numerator == 0 || fps_denominator == 0 ||
        frame >
            std::numeric_limits<std::uint64_t>::max() / policy.target_video_fps / fps_denominator) {
...
std::size_t maximum_reference_audio_scalars(std::uint32_t sample_rate, std::uint32_t channels,
    ...
    const std::uint64_t public_limit =
        static_cast<std::uint64_t>(sample_rate) * channels * policy.maximum_duration_seconds;
...
void reject_reference_timestamp(LONGLONG timestamp, const char* label,
                                const ReferenceMediaDecodePolicy& policy) {
    const auto limit =
        static_cast<std::uint64_t>(policy.maximum_duration_seconds) * kMediaTicksPerSecond;
...
bool detail::reference_audio_event_timestamp_within_padding(
    ...
    return static_cast<std::uint64_t>(timestamp) <= reference_limit + maximum_padding_ticks;
}
...
VideoClipInput read_video_file(const std::string& path, const ReferenceMediaDecodePolicy& policy) {
    ...
    if (declared_duration &&
        *declared_duration >
            static_cast<LONGLONG>(policy.maximum_duration_seconds) * kMediaTicksPerSecond +
                padding.duration) {
        throw std::runtime_error(
    ...
    if (fps_numerator == 0 || fps_denominator == 0 ||
        ...
        throw std::runtime_error("reference video source frame rate exceeds the configured limit");
    }
    ...
    const std::uint64_t maximum_video_frames =
        downsample_video
            ? static_cast<std::uint64_t>(policy.maximum_duration_seconds) * policy.target_video_fps
    ...
    while (!video_done || !audio_done) {
        ...
        if (stream_index == video_stream_index) {
            ...
            if (downsample_video) {
                ...
                const auto end = rounded_reference_frame_slot(
                    source_video_frames + 1, fps_numerator, fps_denominator, policy);
                retain = end > begin;
            }
            ++source_video_frames;
            if (retain) {
...
AudioResult read_audio_file(const std::string& path, const ReferenceMediaDecodePolicy& policy) {
    ...
    while (true) {
        ...
        if ((flags & MF_SOURCE_READERF_STREAMTICK) != 0) {
            ...
            note_empty_source_reader_event(flags, timestamp, last_tick_timestamp,
                                           consecutive_empty_reads, total_empty_reads,
                                           static_cast<std::uint64_t>(padding.duration),
                                           "reference audio source reader", policy);
            continue;
        }
        if (!sample) {
            note_empty_source_reader_event(flags, timestamp, last_tick_timestamp,
