  ...
  public:
    TempDirectory() {
        path_ = std::filesystem::temp_directory_path() /
                ("trtmc-windows-media-test-" + std::to_string(GetCurrentProcessId()) + "-" +
...
int run_test() {
    ...
    constexpr trtmc::ReferenceMediaDecodePolicy compact_policy{
        2, 12, 60, 256, 256ULL * 448, 16, 0.5, 2.0,
    };
    require(trtmc::cli::detail::reference_video_frame_ceiling(compact_policy, 30'000, 1'001) == 60,
    ...
    require(!trtmc::cli::detail::reference_timeline_within_limit(policy, 149'000'000, 2'000'000),
    ...
    const auto& temporary_root = temporary.path();
    ...
    trtmc::cli::io::write_wav(result.audio, wav_path.string());
    ...
    trtmc::cli::io::write_wav(boundary_audio, boundary_wav_path.string());
    ...
    boundary_audio.num_samples = static_cast<int32_t>(boundary_audio.samples.size());
    trtmc::cli::io::write_wav(boundary_audio, over_limit_wav_path.string());
    require_throws_with(
    ...
    require_throws_with(
        ...
        "configured duration", "a true 16-second MP3 must not be accepted as codec padding");
