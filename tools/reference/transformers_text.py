...
_INTERNLM_TOKENIZER_REPO_ID = "internlm/internlm2-step-prover"
_INTERNLM_TOKENIZER_REVISION = "6c727046190546168bf3aba9a1d78d5fb325ff14"
_INTERNLM_TOKENIZER_FILENAME = "tokenizer.json"
_INTERNLM_TOKENIZER_SHA256 = (
    "1193d3a1aa3d9f74866287ca3c1f7bf64fe54dd6ecf015e751f13ebce509e411"
)
_INTERNLM_SOURCE_MODEL_SHA256 = (
...
def _resolve_internlm_reference_tokenizer_json(
    model_dir: Path,
    *,
    local_files_only: bool,
) -> Path | None:
    """Resolve InternLM's official fast tokenizer independently from the DUT."""
    source_model = model_dir / "tokenizer.model"
    if (
        not source_model.is_file()
        or _sha256_file(source_model) != _INTERNLM_SOURCE_MODEL_SHA256
    ):
        return None
    ...
    try:
        ...
        tokenizer_path = Path(
            hf_hub_download(
                repo_id=_INTERNLM_TOKENIZER_REPO_ID,
                filename=_INTERNLM_TOKENIZER_FILENAME,
                revision=_INTERNLM_TOKENIZER_REVISION,
                local_files_only=local_files_only,
            )
        )
    except Exception as exc:
        raise RuntimeError(
            ...
            f"the local Hugging Face cache: {_INTERNLM_TOKENIZER_REPO_ID}@"
            f"{_INTERNLM_TOKENIZER_REVISION}"
        ) from exc

    actual_sha256 = _sha256_file(tokenizer_path)
    if actual_sha256 != _INTERNLM_TOKENIZER_SHA256:
        raise RuntimeError(
            "pinned official InternLM reference tokenizer SHA256 mismatch: "
            f"expected {_INTERNLM_TOKENIZER_SHA256}, got {actual_sha256}: "
            f"{tokenizer_path}"
        )
...
def _load_tokenizer(
    ...
    if arguments.family == "internlm":
        model_ref = _resolve_cached_model_ref(
            arguments.model,
            revision=arguments.model_revision,
            local_files_only=arguments.local_files_only,
        )
        tokenizer_path = _resolve_internlm_reference_tokenizer_json(
            model_ref,
            local_files_only=arguments.local_files_only,
        )
        if tokenizer_path is not None:
            tokenizer_config = _load_json(model_ref / "tokenizer_config.json")
...
def _reproduction_command(arguments: argparse.Namespace) -> list[str]:
    ...
    if arguments.family:
        command.extend(["--family", arguments.family])
    for flag, value in (
...
def _load_runtime(
    ...
    tokenizer = _load_tokenizer(arguments, transformers_module)
