...
def _load_tokenizer(
    ...
    family = str(getattr(arguments, "family", "") or "")
    if family:
        module_name = (
            f"tensorrt_model_connect.models.{family}.tests.tools.transformers_text"
        )
        try:
            owner_tool = import_module(module_name)
        except ModuleNotFoundError as exc:
            if exc.name != module_name:
                raise
        else:
            tokenizer = owner_tool.load_tokenizer(
                arguments,
                transformers_module,
            )
