...
def run(arguments: argparse.Namespace) -> None:
    ...
    owner_runner = load_model_entrypoint(family, "reference_entrypoint")
    if owner_runner is not None:
        responses = owner_runner(arguments, manifest, answers, prompt_rows)
        selected = list(range(len(responses)))
    else:
    ...
    for run_index, source_index in enumerate(selected if owner_runner is None else []):
