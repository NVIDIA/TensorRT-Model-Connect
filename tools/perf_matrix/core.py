...
def load_suite(path: Path) -> tuple[str, list[dict[str, Any]], set[str]]:
    owner = path.parent.parent.name if path.name == 'performance.yaml' and path.parent.name == 'tests' and (path.parent.parent.parent.resolve() == MANIFEST_ROOT.resolve()) else None
...
def _load_suite_file(path: Path, *, owner: str | None=None) -> tuple[str, list[dict[str, Any]], set[str]]:
...
def _family_file(family: str, relative: str, label: str) -> Path:
    ...
    if not isinstance(family, str) or not family or family in {'.', '..'} or ('/' in family) or ('\\' in family):
...
def _validate_entry(entry: Mapping[str, Any]) -> None:
    ...
    if not isinstance(baseline, Mapping) or baseline.get('runner') not in {'hf-transformers', 'task-reference'}:
...
def load_environment(path: Path) -> Environment:
    ...
    if not all((isinstance(value, Mapping) for value in (tools, storage, execution, references))):
        raise PerfMatrixError('environment tools, storage, execution, and references must be objects')
    assert isinstance(tools, Mapping)
    assert isinstance(storage, Mapping)
    assert isinstance(execution, Mapping)
    assert isinstance(references, Mapping)
    required_tools = {'trtmc_bench', 'trtmc_worker', 'hf_transformers_runner', 'task_reference_runner'}
    required_storage = {'results_root', 'scratch_root', 'bundle_cache', 'bundle_roots', 'runtime_root'}
    ...
    return Environment(name=name, trtmc_bench=_path(str(tools['trtmc_bench']), 'tools.trtmc_bench'), worker=_path(str(tools['trtmc_worker']), 'tools.trtmc_worker'), hf_runner=_path(str(tools['hf_transformers_runner']), 'tools.hf_transformers_runner'), task_runner=_path(str(tools['task_reference_runner']), 'tools.task_reference_runner'), reference_python=_python_path(str(tools.get('reference_python', sys.executable)), 'tools.reference_python'), results_root=_path(str(storage['results_root']), 'storage.results_root'), scratch_root=_path(str(storage['scratch_root']), 'storage.scratch_root'), bundle_cache=_path(str(storage['bundle_cache']), 'storage.bundle_cache'), bundle_roots=_path_list(storage['bundle_roots'], 'storage.bundle_roots'), runtime_root=_path(str(storage['runtime_root']), 'storage.runtime_root'), bundle_retention=retention, local_files_only=bool(execution.get('local_files_only', False)), timeout_seconds=timeout, references={name: str(references[name]) for name in REFERENCE_FIELDS}, storage_root=_path(storage_root, 'storage.storage_root') if storage_root is not None else None, hf_cache_mode=str(hf_cache_mode), hf_cache_retention=str(hf_cache_retention))
...
def select_entries(entries: Sequence[dict[str, Any]], *, entry_ids: Sequence[str]=(), models: Sequence[str]=(), model_selection: Path | None=None) -> list[dict[str, Any]]:
...
def _coverage(entries: Sequence[Mapping[str, Any]], excluded: set[str]) -> None:
    ...
    ready = {entry.name for entry in catalog_entries if entry.status == 'ready' and '-l0' not in f'-{entry.name}-' and ('-regression-' not in f'-{entry.name}-')}
    ...
    for spec in entries:
        try:
            ...
            if model.family != spec['family']:
                raise PerfMatrixError(f"entry {spec['id']} family {spec['family']} does not match {model.family}")
            ...
            overrides = {f'request.{name}': value for name, value in workload.get('request', {}).items()}
            timing = timing_contract(runner=str(spec['baseline']['runner']), declared=spec['baseline'])
            baseline_timing = _baseline_timing(timing)
            overrides.update({'measurement.warmup': int(spec['measurement']['warmup']), 'measurement.iterations': int(spec['measurement']['iterations']), 'measurement.timing_scope': timing['candidate_timing_scope'], 'measurement.asset_loading_included': baseline_timing['asset_loading_included'], 'telemetry.gpu': 'off'})
            bundle = environment.bundle_cache / model.name / model.bundle_name
            case = resolve_case(model, bundle, case_name=str(workload['testcase']), operation=str(spec['operation']), overrides=overrides).with_values(runtime_root=environment.runtime_root)
        ...
        resolved.append(ResolvedEntry(spec, model, case, manifest, reference_precision, baseline_timing))
    return resolved

def _reference_precision(spec: Mapping[str, Any], testcase_name: str, manifest: Mapping[str, Any], model: Any) -> str:
...
def _baseline_timing(declared: Mapping[str, Any]) -> dict[str, Any]:
    return {'timing_scope': declared['timing_scope'], 'input_preparation_included': declared['input_preparation_included'], 'asset_loading_included': declared['asset_loading_included']}
...
def preflight(
    ...
    *,
    ...
    if environment.storage_root is not None:
        ...
        for label, path in (('results_root', environment.results_root), ('scratch_root', environment.scratch_root), ('bundle_cache', environment.bundle_cache)):
            if not path.is_relative_to(environment.storage_root):
                raise PerfMatrixError(f'{label} must stay below storage_root {environment.storage_root}: {path}')
    ...
    if require_runtime:
        for entry in resolved:
            ...
            baseline_command(entry, environment, environment.scratch_root / f"{_entry_slug(str(entry.spec['id']))}.reference.json")
    return resolved
...
def _candidate_base(entry: ResolvedEntry, environment: Environment) -> list[str]:
    ...
    arguments = [str(environment.trtmc_bench), 'run', '--model', selector, '--case', entry.case.testcase_name, '--operation', str(entry.spec['operation']), '--bundle-cache', str(environment.bundle_cache)]
    ...
    arguments.extend(('--warmup', str(entry.case.measurement.warmup), '--iterations', str(entry.case.measurement.iterations), '--telemetry', 'off', '--set', 'measurement.timing_scope=public_task_call_wall', '--set', 'measurement.asset_loading_included=' + ('true' if entry.case.measurement.asset_loading_included else 'false')))
    return arguments

def candidate_command(entry: ResolvedEntry, environment: Environment, output: Path | None, *, prepare_only: bool=False, no_build: bool=False) -> list[str]:
    arguments = _candidate_base(entry, environment)
    if prepare_only:
        arguments.append('--prepare-only')
        return arguments
    arguments.extend(('--runtime-root', str(environment.runtime_root), '--worker', str(environment.worker)))
    if no_build:
...
def _validate_reference_path(entry: ResolvedEntry, field: str, path: Path) -> None:
    required = {'elf_repo': ('src',), 'lance_repo': ('inference_lance.py',), 'lerobot_repo': ('lerobot/common/policies/act/modeling_act.py',), 'personaplex_repo': ('moshi',), 'fast_foundation_stereo_model': ('core/foundation_stereo.py', 'core/submodule.py', 'weights/23-36-37/model_best_bp2_serialize.pth')}.get(field, ())
    missing = [relative for relative in required if not (path / relative).exists()]
    if missing:
        raise PerfMatrixError(f"entry {entry.spec['id']} reference path {path} is missing: " + ', '.join(missing))
...
def _baseline_command(entry: ResolvedEntry, environment: Environment, output: Path, *, mode: str | None=None) -> list[str]:
    ...
    common = ['--model', str(_adapter_options(entry, environment).get('model_id', entry.model.hf_id)), '--request-json', request, '--precision', entry.reference_precision, '--mode', selected_mode, '--warmup', str(entry.case.measurement.warmup), '--iterations', str(entry.case.measurement.iterations), '--case-name', str(entry.spec['id']), '--output', str(output)]
    if runner == 'hf-transformers':
        arguments = [str(environment.reference_python), str(environment.hf_runner), '--task', str(baseline.get('task', _baseline_task(entry))), '--max-length', str(entry.model.build_settings.get('max_sequence_length', 256)), '--padding', str(baseline.get('padding', 'longest')), '--output-token-policy', str(baseline.get('output_token_policy', 'new-tokens')), *common]
        ...
        executable = [str(_family_script(entry.spec))] if 'script' in baseline else [str(environment.task_runner), '--adapter', str(baseline['adapter'])]
        arguments = [str(getattr(environment, 'reference_python', sys.executable)), *executable, '--family', entry.model.family, '--operation', str(entry.spec['operation']), '--manifest', str(entry.model.manifest_path), '--adapter-options-json', json.dumps(_adapter_options(entry, environment), ensure_ascii=True, separators=(',', ':')), '--timing-contract-json', json.dumps(entry.baseline_timing, ensure_ascii=True, separators=(',', ':')), '--padding', str(baseline.get('padding', 'longest')), *common]
...
def _validate_script_result(entry: ResolvedEntry, environment: Environment, result: Mapping[str, Any]) -> None:
    expected = {'schema_version': 'trtmc.perf-baseline/v1', 'status': 'completed', 'model': str(_adapter_options(entry, environment).get('model_id', entry.model.hf_id)), 'family': entry.model.family, 'operation': str(entry.spec['operation']), 'case_name': str(entry.spec['id']), 'selected_task': _effective_task(entry), 'precision': entry.reference_precision, 'mode': _baseline_mode(entry.spec['baseline'])}
    for field, value in expected.items():
        if result.get(field) != value:
            raise PerfMatrixError(f'family reference result has mismatched {field}')
    measurement = result.get('measurement')
    for field in ('warmup', 'iterations'):
        if not isinstance(measurement, Mapping) or type(measurement.get(field)) is not int or measurement[field] != getattr(entry.case.measurement, field):
            raise PerfMatrixError(f'family reference result has mismatched {field}')
    policy = result.get('measurement_policy')
    for field, value in entry.baseline_timing.items():
        if not isinstance(policy, Mapping) or result.get(field) != value or policy.get(field) != value or (type(result.get(field)) is not type(value)) or (type(policy.get(field)) is not type(value)):
            raise PerfMatrixError(f'family reference result has mismatched {field}')
    samples = result.get('samples_ms')
    if not isinstance(samples, list) or len(samples) != entry.case.measurement.iterations or any((type(value) not in (int, float) or not math.isfinite(value) or value <= 0 for value in samples)):
...
def run_command(arguments: Sequence[str], *, timeout: int, stdout_path: Path, stderr_path: Path, verbose: bool, env: Mapping[str, str] | None=None) -> dict[str, Any]:
    stdout_path.parent.mkdir(parents=True, exist_ok=True)
    if verbose:
        print(shlex.join(_reported_arguments(arguments)), flush=True)
    started = time.monotonic()
    try:
        completed = subprocess.run(list(arguments), cwd=REPOSITORY, env=dict(env) if env is not None else _command_environment(), capture_output=True, text=True, timeout=timeout, check=False)
    ...
    return {'argv': _reported_arguments(arguments), 'cwd': str(REPOSITORY), 'exit_code': code, 'elapsed_seconds': time.monotonic() - started, 'stdout_log': str(stdout_path), 'stderr_log': str(stderr_path)}
...
def _candidate_result(path: Path) -> dict[str, Any]:
    ...
    if result.get('schema_version') != 'trtmc.benchmark-run/v2' or not isinstance(cells, list) or len(cells) != 1 or (not isinstance(cells[0], Mapping)):
    ...
    return {'metrics': cell.get('metrics', {}), 'samples_ms': cell.get('samples_ms', []), 'output_summary': cell.get('output_summary', {}), 'timing_scope': cell.get('timing_scope'), 'asset_loading_included': cell.get('asset_loading_included'), 'preparation': result.get('preparation', {})}
...
def _timing_stability(values: Sequence[Any]) -> dict[str, Any]:
    if len(values) != _TIMING_STABILITY_SAMPLE_COUNT:
        return {'status': 'not_evaluated', 'sample_count': len(values), 'reason': 'requires_10_samples'}
    try:
        samples = [float(value) for value in values]
    except (TypeError, ValueError):
        return {'status': 'not_evaluated', 'sample_count': len(values), 'reason': 'invalid_samples'}
    if not all((math.isfinite(value) and value > 0.0 for value in samples)):
        return {'status': 'not_evaluated', 'sample_count': len(samples), 'reason': 'invalid_samples'}
    ...
    half_change_percent = abs(second_half_median_ms - first_half_median_ms) / first_half_median_ms * 100.0
    samples_within_band = sum((abs(sample - median_ms) / median_ms * 100.0 <= _TIMING_STABILITY_MEDIAN_BAND_PERCENT for sample in samples))
    stable = half_change_percent <= _TIMING_STABILITY_MAX_HALF_CHANGE_PERCENT and samples_within_band >= _TIMING_STABILITY_MIN_IN_BAND
    return {'status': 'stable' if stable else 'unstable', 'sample_count': len(samples), 'median_ms': median_ms, 'first_half_median_ms': first_half_median_ms, 'second_half_median_ms': second_half_median_ms, 'half_median_change_percent': half_change_percent, 'samples_within_band': samples_within_band}

def _measurement_stability(reference: Mapping[str, Any], candidate: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {'status': status, 'policy': {'required_samples': _TIMING_STABILITY_SAMPLE_COUNT, 'max_half_median_change_percent': _TIMING_STABILITY_MAX_HALF_CHANGE_PERCENT, 'median_band_percent': _TIMING_STABILITY_MEDIAN_BAND_PERCENT, 'minimum_samples_within_band': _TIMING_STABILITY_MIN_IN_BAND}, 'reference': reference_result, 'candidate': candidate_result}

def compare(entry: ResolvedEntry, candidate: Mapping[str, Any], baseline: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    if ratio > 1.0 + margin:
        ...
        status = 'yellow'
    value = {'candidate_p50_ms': candidate_p50, 'reference_p50_ms': reference_p50, 'reference_over_candidate_p50': ratio, 'equivalence_margin_percent': margin * 100.0}
    if evidence:
        value['output_contract'] = evidence
    return (status, value)

def _timing_mismatch(entry: ResolvedEntry, candidate: Mapping[str, Any], baseline: Mapping[str, Any]) -> str:
    if candidate.get('timing_scope') != 'public_task_call_wall':
        return 'candidate timing scope is not public_task_call_wall'
    if candidate.get('asset_loading_included') is not bool(entry.case.measurement.asset_loading_included):
...
def _contract_name(entry: ResolvedEntry) -> str:
    ...
    if configured:
        ...
        contract = {'classify': 'classification-top-class', 'embed': 'embedding-shape', 'encode': 'embedding-shape', 'head_scores': 'head-scores-shape', 'geometry': 'metric-geometry-shape', 'predict_structure': 'molecular-structure-shape', 'refine_pose': 'pose-refinement-shape', 'control': 'robot-action-shape', 'segment': 'segmentation-shape', 'solve': 'forecast-shape', 'regress': 'regression-values' if _effective_task(entry) == 'series_to_regression_values' else 'regression-distribution', 'transcribe': 'transcription-text', 'speech_dialogue': 'offline-speech-shape' if _effective_task(entry) == 'offline_speech_dialogue' else ''}.get(str(entry.spec['operation']), '')
    if contract not in OUTPUT_CONTRACTS:
        raise PerfMatrixError(f"entry {entry.spec['id']} has unsupported output contract: {contract or '<none>'}")
    return contract

def _output_contract(entry: ResolvedEntry, candidate: Mapping[str, Any], baseline: Mapping[str, Any]) -> tuple[bool, str, dict[str, Any] | None]:
    ...
    if contract == 'ocr-text':
        ...
        return (distance <= limit, 'OCR text distance exceeds the contract', {'normalized_edit_distance': distance, 'maximum': limit})
    ...
    if contract in {'metric-geometry-shape', 'molecular-structure-shape', 'pose-refinement-shape'}:
        signature = {'metric-geometry-shape': _metric_geometry_signature, 'molecular-structure-shape': _molecular_structure_signature, 'pose-refinement-shape': _pose_refinement_signature}[contract]
    ...
    if contract == 'audio-shape':
        left_shape = (left.get('num_samples', left.get('audio_samples')), left.get('sample_rate'))
        right_shape = (right.get('num_samples', right.get('audio_samples')), right.get('sample_rate'))
    ...
    if contract == 'classification-top-class':
        ...
        matched = isinstance(left_class, int) and (not isinstance(left_class, bool)) and (left_class == right_class)
        return (matched, 'classification top class differs' if not matched else '', None)
    if contract == 'image-features-shape':
        left_shape = (left.get('last_hidden_state_shape'), left.get('pooler_output_shape'))
        right_shape = (right.get('last_hidden_state_shape'), right.get('pooler_output_shape'))
    ...
    if contract == 'reranking-order':
        ...
        right_order = sorted(range(len(right_scores)), key=lambda index: (-right_scores[index], index))
    ...
    if contract == 'robot-action-shape':
        ...
        matched = None not in left_shape and left_shape == right_shape and (left.get('within_training_bounds') is True) and (right.get('finite') is True)
    ...
    if contract == 'head-scores-shape':
        ...
        def signature(value: Mapping[str, Any]) -> tuple[Any, ...] | None:
            ...
            if not isinstance(shape, list) or not shape or any((isinstance(size, bool) or not isinstance(size, int) or size <= 0 for size in shape)) or (not isinstance(values, list)) or (len(values) != math.prod(shape)):
                return None
    ...
    if contract == 'forecast-shape':
        ...
        expected_axes = ['horizon', 'channel'] if task == 'series_to_point_forecast' else ['quantile', 'horizon', 'channel']
        shape_matches = left_shape == right_shape and isinstance(left_shape, list) and (len(left_shape) == len(expected_axes)) and (left.get('axes') == right.get('axes') == expected_axes) and isinstance(left.get('horizon_steps'), list) and (len(left['horizon_steps']) == left_shape[expected_axes.index('horizon')]) and (left.get('horizon_steps') == right.get('horizon_steps')) and (left.get('quantile_levels') == right.get('quantile_levels')) if semantic else isinstance(left_shape, list) and isinstance(right_shape, list) and (sorted(left_shape) == sorted(right_shape))
        if semantic and task == 'series_to_quantile_forecast':
            levels = left.get('quantile_levels')
            shape_matches = shape_matches and isinstance(levels, list) and (len(levels) == left_shape[0])
        matched = isinstance(left_elements, int) and (not isinstance(left_elements, bool)) and (left_elements > 0) and (left_elements == right_elements) and isinstance(left_shape, list) and isinstance(right_shape, list) and shape_matches
    ...
    if contract == 'forecast-parity':
        ...
        if not isinstance(left_values, list) or not isinstance(right_values, list) or (not left_values) or (len(left_values) != len(right_values)):
        ...
        difference = math.sqrt(sum(((candidate - reference) ** 2 for candidate, reference in zip(candidate_values, reference_values, strict=True))))
        reference_norm = math.sqrt(sum((value ** 2 for value in reference_values)))
        relative_l2 = difference / max(reference_norm, 1e-12)
        max_absolute_error = max((abs(candidate - reference) for candidate, reference in zip(candidate_values, reference_values, strict=True)))
        relative_limit = float(entry.spec['baseline']['max_relative_l2'])
        absolute_limit = float(entry.spec['baseline']['max_absolute_error'])
        evidence = {'relative_l2': relative_l2, 'max_relative_l2': relative_limit, 'max_absolute_error': max_absolute_error, 'max_allowed_absolute_error': absolute_limit}
    ...
    if contract == 'regression-values':
        ...
        def signature(value: Mapping[str, Any]) -> tuple[Any, ...] | None:
            ...
            if isinstance(targets, bool) or not isinstance(targets, int) or targets <= 0 or (not isinstance(values, list)) or (len(values) != targets) or (value.get('axes') != ['target']) or (value.get('kind') != 'regression_values'):
                return None
            if any((isinstance(item, bool) or not isinstance(item, (int, float)) or (not math.isfinite(item)) for item in values)):
                return None
            for key in ('target_names', 'target_units'):
                metadata = value.get(key, [])
                if not isinstance(metadata, list) or len(metadata) not in {0, targets} or (not all((isinstance(item, str) for item in metadata))):
                    return None
            return (targets,)
        first, second = (signature(left), signature(right))
        matched = first is not None and first == second
    ...
    if contract == 'regression-distribution':
        ...
        def signature(value: Mapping[str, Any]) -> tuple[Any, ...] | None:
            ...
            if value.get('distribution') not in {'normal', 'student_t', 'negative_binomial'} or isinstance(targets, bool) or (not isinstance(targets, int)) or (targets <= 0) or (not isinstance(parameters, list)) or (not parameters) or (value.get('axes') != ['target']):
                return None
            names = []
            for parameter in parameters:
                if not isinstance(parameter, Mapping) or not isinstance(parameter.get('name'), str) or (not parameter['name']):
                    return None
...
def _metric_geometry_signature(summary: Mapping[str, Any]) -> tuple[Any, ...]:
    ...
    if summary.get('units') != 'meters' or summary.get('camera_axes') != ['right', 'down', 'forward'] or summary.get('intrinsics_coordinates') != 'normalized_uv':
    ...
    if 'intrinsics_artifact' in summary:
        ...
        if not isinstance(calibration, Mapping) or type(calibration.get('height')) is not int or type(calibration.get('width')) is not int or (calibration.get('height') != height) or (calibration.get('width') != width) or (calibration.get('normalized') is not True) or (calibration.get('intrinsics') != intrinsics):
...
def _molecular_structure_signature(summary: Mapping[str, Any]) -> tuple[Any, ...]:
    ...
    if confidence is not None:
        ...
        for name in ('confidence_score', 'ptm', 'iptm', 'ligand_iptm', 'protein_iptm', 'complex_plddt', 'complex_iplddt'):
...
def _pose_refinement_signature(summary: Mapping[str, Any]) -> tuple[Any, ...]:
    ...
    if not isinstance(shape, list) or shape != [count, 4, 4] or any((type(axis) is not int for axis in shape)):
    ...
    for query in queries:
        ...
        if not isinstance(query_shape, list) or len(query_shape) != 3 or query_shape[1:] != [4, 4] or any((type(axis) is not int for axis in query_shape)):
...
def _offline_speech_wav_chunks(payload: bytes) -> dict[bytes, bytes]:
    if len(payload) < 12 or payload[:4] != b'RIFF' or payload[8:12] != b'WAVE' or (int.from_bytes(payload[4:8], 'little') + 8 != len(payload)):
...
def _offline_speech_event(event: Any, rate: int, channels: int) -> tuple[str, int]:
    kinds = {'agent_audio', 'agent_text', 'user_transcript', 'turn_started', 'turn_finished', 'yielded', 'reset', 'input_finished', 'user_speech_started', 'user_speech_stopped', 'input_cleared'}
    ...
    if any((abs(value) > 3.4028234663852886e+38 for value in event['audio'])):
...
def _offline_speech_candidate(summary: Mapping[str, Any]) -> dict[str, Any]:
    ...
    text = ' '.join((piece for turn in turns.values() if (piece := (turn['final'] if turn['final'] is not None else turn['partial']))))
    _offline_speech_duration(summary.get('output_audio_seconds'), len(audio), rate, channels)
    return {'text': text, 'audio': audio, 'sample_rate': rate, 'channels': channels, 'input': _offline_speech_input(summary)}
...
def _offline_speech_reference(summary: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {'text': summary['text'], 'audio': audio, 'sample_rate': rate, 'channels': channels, 'input': _offline_speech_input(summary)}

def _offline_speech_contract(entry: ResolvedEntry, left: Mapping[str, Any], right: Mapping[str, Any]) -> tuple[bool, str, dict[str, Any]]:
    evidence: dict[str, Any] = {'contract': 'offline-speech-shape', 'numerical_parity_checked': False, 'text_parity_checked': False}
    ...
    for side, summary, parse in (('candidate', left, _offline_speech_candidate), ('reference', right, _offline_speech_reference)):
        try:
            value = parse(summary)
        except (ValueError, OSError, OverflowError) as error:
            return (False, f'{side} offline-speech-shape: {error}', evidence)
        evidence[side] = {'text': value['text'], 'audio_samples': len(value['audio']), 'sample_rate': value['sample_rate'], 'channels': value['channels'], 'input_samples': value['input'][0], 'input_sample_rate': value['input'][1], 'input_channels': value['input'][2]}
    names = ('audio_samples', 'sample_rate', 'channels', 'input_samples', 'input_sample_rate', 'input_channels')
...
def _localizations(text: str) -> tuple[str, list[tuple[float, ...]]] | None:
    if '<ref>' not in text or '</ref>' not in text:
        return None
    groups = re.findall('<(?:box|point)>(.*?)</(?:box|point)>', text, flags=re.DOTALL)
    if not groups:
        return None
    values = []
    kind = ''
    for group in groups:
        numbers = tuple((float(value) for value in re.findall('-?\\d+(?:\\.\\d+)?', group)))
        if len(numbers) == 4:
            current = 'box'
        elif len(numbers) == 2:
            current = 'point'
...
def _localization_contract(entry: ResolvedEntry, left: Mapping[str, Any], right: Mapping[str, Any]) -> tuple[bool, str, dict[str, Any] | None]:
    ...
    distance = _text_distance(_normalized_text(left.get('text')), _normalized_text(right.get('text')))
...
def _disparity(entry: ResolvedEntry, left: Mapping[str, Any], right: Mapping[str, Any]) -> dict[str, Any]:
    ...
    baseline = entry.spec['baseline']
    passed = finite and reference_finite and (cosine >= float(baseline['min_disparity_cosine'])) and (mean_error <= float(baseline['max_disparity_mean_abs_error'])) and (bad_fraction <= float(baseline['max_disparity_bad_2px_fraction']))
    return {'passed': passed, 'reason': '' if passed else 'disparity parity is outside the contract', 'cosine': cosine, 'mean_abs_error': mean_error, 'bad_2px_fraction': bad_fraction}
...
def _entry_slug(entry_id: str) -> str:
    value = re.sub('[^a-zA-Z0-9_.-]+', '-', entry_id).strip('-')
    return 'entry' if value in {'', '.', '..'} else value

def _execute_entry(entry: ResolvedEntry, environment: Environment, run_directory: Path, *, no_build: bool, verbose: bool, attempt: int) -> dict[str, Any]:
    ...
    try:
        for measurement_attempt in (1, 2):
            ...
            candidate_arguments = candidate_command(entry, environment, candidate_output, no_build=no_build)
            candidate_result = run_command(candidate_arguments, timeout=environment.timeout_seconds, stdout_path=logs / 'candidate.stdout.log', stderr_path=logs / 'candidate.stderr.log', verbose=verbose, env=command_environment)
            ...
            for mode_index, mode in enumerate(modes):
                ...
                mode_output = measurement_dir / f'reference-{mode}.json' if is_fallback else reference_output
                ...
                reference_result = run_command(reference_arguments, timeout=environment.timeout_seconds, stdout_path=logs / f'{log_name}.stdout.log', stderr_path=logs / f'{log_name}.stderr.log', verbose=verbose, env=command_environment)
                command_name = ('reference_fallback' if is_fallback else 'reference') + suffix
                commands[command_name] = reference_result
                reference_attempt = {'measurement_attempt': measurement_attempt, 'mode': mode, 'fallback': is_fallback, 'exit_code': reference_result['exit_code']}
                ...
                if tentative_reference.get('status') != 'completed':
                    reference_attempt['fallback_reason'] = str(tentative_reference.get('error', 'reference failed'))
                    continue
                if 'script' in entry.spec['baseline']:
                    _validate_script_result(entry, environment, tentative_reference)
                tentative_status, tentative_comparison = compare(entry, candidate, tentative_reference)
            ...
            if selected_reference is None:
                if len(modes) == 1:
                    reason = reference_attempts[-1].get('fallback_reason')
                    raise PerfMatrixError(str(reason or 'reference failed'))
            ...
            measurement_attempts.append({'attempt': measurement_attempt, 'reference': {'samples_ms': list(reference.get('samples_ms', [])), 'stability': stability['reference']}, 'candidate': {'samples_ms': list(candidate.get('samples_ms', [])), 'stability': stability['candidate']}})
        ...
        if environment.bundle_retention == 'delete_always' or (environment.bundle_retention == 'delete_on_pass' and passed):
    ...
    row = {'id': entry.spec['id'], 'model': entry.model.name, 'family': entry.model.family, 'operation': entry.spec['operation'], 'testcase': entry.case.testcase_name, 'status': status, 'attempts': attempt, 'artifact_dir': str(artifact.relative_to(run_directory)), 'candidate': candidate, 'reference': reference, 'comparison': comparison, 'bundle_cleanup': cleanup, 'hf_cache_cleanup': cache_cleanup, 'commands': commands, 'reference_attempts': reference_attempts}
    if measurement_stability is not None:
        row['measurement_stability'] = measurement_stability
    return row

def _cleanup_managed_bundle(candidate: Mapping[str, Any] | None, entry: ResolvedEntry, environment: Environment) -> dict[str, Any] | None:
    if candidate is None:
        records: Sequence[Any] = [{'model': entry.model.name, 'bundle': str(entry.case.bundle_path)}]
    else:
...
def _cleanup_entry_work(work: Path, environment: Environment, *, passed: bool) -> dict[str, Any] | None:
    if environment.hf_cache_mode != 'per_entry':
        return None
    ...
    try:
        ...
        raise PerfMatrixError(f'refusing to delete HF cache outside scratch_root: {work}') from error
...
def _initial_results(suite_path: Path, environment_path: Path, suite_name: str, environment: Environment, entries: Sequence[ResolvedEntry], *, no_build: bool) -> dict[str, Any]:
    return {'schema_version': RESULT_SCHEMA, 'status': 'running', 'started_at': _now(), 'suite': suite_name, 'environment': environment.name, 'suite_path': str(suite_path.resolve()), 'environment_path': str(environment_path.resolve()), 'selected_entry_ids': [entry.spec['id'] for entry in entries], 'no_build': no_build, 'rows': []}
...
def _run_rows(run_directory: Path, results: dict[str, Any], entries: Sequence[ResolvedEntry], environment: Environment, *, no_build: bool, verbose: bool) -> int:
    current = {str(row.get('id')): row for row in results.get('rows', []) if isinstance(row, Mapping)}
    for entry in entries:
        ...
        while (artifact_root / f'attempt-{attempt}').exists():
            attempt += 1
        try:
            row = _execute_entry(entry, environment, run_directory, no_build=no_build, verbose=verbose, attempt=attempt)
        except (OSError, PerfMatrixError, subprocess.SubprocessError) as error:
            row = {'id': entry_id, 'model': entry.model.name, 'family': entry.model.family, 'operation': entry.spec['operation'], 'testcase': entry.case.testcase_name, 'status': 'white', 'attempts': attempt, 'error': str(error)}
        current[entry_id] = row
        results['rows'] = [current[str(value.spec['id'])] for value in entries if str(value.spec['id']) in current]
        _write_json(run_directory / 'results.json', results)
        write_report(run_directory, results)
    results['finished_at'] = _now()
    results['status'] = 'completed' if all((row.get('status') in TERMINAL_COMPARISONS for row in results['rows'])) else 'failed'
    _write_json(run_directory / 'results.json', results)
    write_report(run_directory, results)
    return 0 if results['status'] == 'completed' else 1

def write_report(run_directory: Path, results: Mapping[str, Any], preparation: Mapping[str, Any] | None=None) -> dict[str, Any]:
    ...
    counts = {status: sum((row.get('status') == status for row in rows)) for status in ('green', 'yellow', 'red', 'contract-mismatch', 'white')}
    report = {'schema_version': REPORT_SCHEMA, 'source_schema_version': source_schema, 'generated_at': _now(), 'status': results.get('status', 'unknown'), 'suite': results.get('suite'), 'environment': results.get('environment'), 'summary': {'selected': len(selected_ids), 'pending': sum((entry_id not in completed_ids for entry_id in selected_ids)), 'comparable': counts['green'] + counts['yellow'] + counts['red'], **counts}, 'rows': rows}
...
def _measurement_html(value: Any) -> str:
    ...
    measured = sorted((float(sample) for sample in samples if isinstance(sample, (int, float)) and (not isinstance(sample, bool)) and math.isfinite(sample) and (sample > 0))) if isinstance(samples, list) else []
    parts = []
    ...
    scope = value.get('timing_scope') or (policy.get('timing_scope') if isinstance(policy, Mapping) else None)
    parts.append('Scope: ' + html.escape(str(scope or 'not recorded')))
    if isinstance(metrics, Mapping):
        parts.extend((f'{html.escape(name)}: {number:,.3f}' for name, number in metrics.items() if name.endswith('_per_s') and isinstance(number, (int, float)) and (not isinstance(number, bool)) and math.isfinite(number)))
...
def _report_html(report: Mapping[str, Any]) -> str:
    ...
    for row in report.get('rows', []):
        ...
        measurement_stability = row.get('measurement_stability', {}) if isinstance(row, Mapping) else {}
        candidate = _measurement_html(row.get('candidate') or {'metrics': {'latency_ms': {'p50': comparison.get('candidate_p50_ms')}}})
        reference = _measurement_html(row.get('reference', row.get('baseline')) or {'metrics': {'latency_ms': {'p50': comparison.get('reference_p50_ms')}}})
        reason = comparison.get('reason', row.get('error', ''))
        stability = measurement_stability.get('status', '') if isinstance(measurement_stability, Mapping) else ''
        evidence = {key: row[key] for key in ('commands', 'comparison', 'measurement_stability', 'resolved_settings', 'baseline_contract') if key in row}
        detail = html.escape(json.dumps(evidence, indent=2, sort_keys=True))
        link = ''
        artifact = row.get('artifact_dir')
        if isinstance(artifact, str) and artifact and (not Path(artifact).is_absolute()) and ('..' not in Path(artifact).parts) and (not any((value in artifact for value in (':', '\\')))):
            from urllib.parse import quote
            link = f'''<a href="{quote(artifact, safe='/.')}/">Case artifacts</a>'''
        rows.append(f"<tr><td>{html.escape(str(row.get('id', '')))}</td><td>{html.escape(str(row.get('model', '')))}</td><td>{html.escape(str(row.get('operation', '')))}</td><td>{html.escape(str(row.get('status', '')))}</td><td>{html.escape(str(stability))}</td><td>{candidate}</td><td>{reference}</td><td>{html.escape(str(reason))}</td><td><details><summary>Evidence / commands</summary>{link}<pre>{detail}</pre></details></td></tr>")
...
def prepare_entries(entries: Sequence[ResolvedEntry], environment: Environment, output: Path, *, verbose: bool) -> int:
    ...
    for entry in entries:
        ...
        result = run_command(command, timeout=environment.timeout_seconds, stdout_path=log_root / f"{_entry_slug(str(entry.spec['id']))}.stdout.log", stderr_path=log_root / f"{_entry_slug(str(entry.spec['id']))}.stderr.log", verbose=verbose)
        ...
        try:
            ...
            raise PerfMatrixError(f"bundle preparation returned invalid JSON for {entry.spec['id']}") from error
    ...
    receipt = {'schema_version': PREPARATION_SCHEMA, 'created_at': _now(), 'included_in_performance_metrics': False, 'bundles': list(bundles.values())}
...
def _common(arguments: argparse.Namespace) -> tuple[Path, Path, str, list[dict[str, Any]], set[str], Environment, list[ResolvedEntry]]:
    ...
    selected = select_entries(all_entries, entry_ids=arguments.entry, models=arguments.model, model_selection=arguments.model_selection)
    environment = load_environment(environment_path)
    if not arguments.allow_partial:
        _coverage(all_entries, excluded)
    resolved = preflight(selected, environment, require_runtime=arguments.command in {'check', 'run'})
