...
def family_dirs(repo_root: Path) -> list[Path]:
    root = repo_root / "python/tensorrt_model_connect/families"
    return sorted(path for path in root.iterdir() if (path / "plugin.py").is_file())
...
def planned_moves(repo_root: Path) -> list[Move]:
    moves: list[Move] = []
    tests_root = repo_root / "tests/builder/families"
    tools_root = repo_root / "tools/families"

    for family_dir in family_dirs(repo_root):
...
def _normalize_consolidated_models(repo_root: Path) -> None:
    for family_dir in family_dirs(repo_root):
...
def migrate(repo_root: Path) -> None:
    ...
    _move_paths(moves)
    mapping.update(_consolidate_model_cores(repo_root, reverse_paths))
    _normalize_consolidated_models(repo_root)
    ...
    print(
        f"migrated_families={len(family_dirs(repo_root))} "
        f"moved_entries={len(moves)} module_rewrites={len(mapping)}"
    )


def layout_violations(repo_root: Path) -> list[str]:
    violations: list[str] = []
    for family_dir in family_dirs(repo_root):
        names = {
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "migrate":
        migrate(repo_root)
    violations = layout_violations(repo_root)
    if violations:
        print("\n".join(violations), file=sys.stderr)
        return 1
    print(f"canonical_family_layouts={len(family_dirs(repo_root))}")
    return 0
