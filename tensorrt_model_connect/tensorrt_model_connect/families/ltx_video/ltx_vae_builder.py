...
def _trt_dtype(precision: str) -> trt.DataType:
    ...
    if precision == "bf16":
        if not hasattr(trt_module, "bfloat16"):
            raise RuntimeError("LTX VAE bf16 builds require TensorRT BF16 support")
...
def _add_constant_for_trt_dtype(
    ...
    if hasattr(trt_module, "bfloat16") and dtype == trt_module.bfloat16:
        try:
            import ml_dtypes
        except ModuleNotFoundError as exc:
...
def build_ltx_vae_decoder_engine(
    ...
    *,
    ...
    dec_upsample_type = (
        list(reversed(upsample_type)) if upsample_type is not None else None
    )
    dec_upsample_factor = (
        list(reversed(upsample_factor)) if upsample_factor is not None else None
    )
    ...
    for block_idx in range(block_count):
        ...
        if f"{up_prefix}.weight" in weights and _sequence_at(
            dec_scaling, block_idx, True
        ):
            stride = _upsample_stride(
                _sequence_at(dec_upsample_type, block_idx, "spatiotemporal")
            )
            ...
            if _sequence_at(dec_upsample_residual, block_idx, False):
                residual = _ltx2_upsample_residual(
                    ...
                    stride=stride,
                    upscale_factor=int(
                        _sequence_at(dec_upsample_factor, block_idx, 1)
                    ),
                )
                x = network.add_elementwise(
                    x, residual, trt.ElementWiseOperation.SUM
                ).get_output(0)
...
def _pixel_shuffle_output_channels(
    weight: np.ndarray, stride: tuple[int, int, int]
) -> int:
...
def _reflect_pad_axis(
    ...
    *,
    ...
    if pad >= axis_len:
        raise ValueError(
            f"reflect padding {pad} on axis {axis} requires input size > padding, "
            f"got {axis_len}"
        )
    left = [
        _slice_single_index(network, inp, axis=axis, index=index)
        for index in range(pad, 0, -1)
    ]
    right = [
        _slice_single_index(network, inp, axis=axis, index=index)
    ...
    *,
    ...
    def conv3d(
        ...
        *,
        ...
        if causal:
            return _conv3d_causal(network, inp, weight, bias, dtype=dtype)
...
def _pixel_unshuffle_3d(
    ...
    *,
    ...
    if frames % st or height % sh or width % sw:
        raise ValueError(
            "LTX2 VAE pixel unshuffle dimensions must be divisible by stride"
        )
    r1 = network.add_shuffle(inp)
    r1.reshape_dims = (
        1,
...
def _channel_group_mean(
    ...
    *,
    ...
    mean = network.add_reduce(
        r.get_output(0), trt.ReduceOperation.AVG, 1 << 2, keep_dims=False
    )
...
def _ltx2_downsample3d(
    ...
    *,
    ...
    hidden = _conv3d_causal(network, padded, weight, bias, dtype=dtype)
