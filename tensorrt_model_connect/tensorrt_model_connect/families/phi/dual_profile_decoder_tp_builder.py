"""Tensor-parallel dual-profile decoder engine builder for the Phi-3 family.
...
Scope: Phi-3-mini-4k-instruct (3.8B, MHA num_kv_heads == num_heads, full
RoPE partial_rotary_factor == 1.0, no biases). Mechanically identical to
the Llama-shape TP recipe — the Phi-3 family plugin handles fused
``qkv_proj`` and ``gate_up_proj`` *at weight-load time*, so by the time
this builder sees the weights they are already split into
``w_q/w_k/w_v`` and ``w_gate/w_up``.

Sliding-window attention is not modeled here — Phi-3-mini sets
``sliding_window=2047`` in config but the model serves correctly with
full attention at 4k context.

Phi-3 variants with partial RoPE (some Phi-3.5 / Phi-4 lines) are out of
scope — they need a separate sibling builder that threads
``partial_rotary_factor`` from config, mirroring the phi4_multimodal TP
builder.
...
def build_dual_profile_tp_decoder_engine(
    ...
    *,
    ...
    """Build a rank-local TP engine for Phi-3-family decoders.
    ...
    follow-up). Partial RoPE variants are rejected (see module docstring).
    """
    ...
    if quant_ctx is not None:
        raise ValueError("Tensor-parallel Phi-3 builds do not support quantization yet")
    partial = float(getattr(config, "raw", {}).get("partial_rotary_factor", 1.0)
                    if hasattr(config, "raw") else 1.0)
    if partial != 1.0:
        raise ValueError(
            f"Phi-3 TP builder requires partial_rotary_factor == 1.0, got {partial}. "
            "Variants with partial RoPE need a separate sibling TP builder "
            "(see families/phi4_multimodal/dual_profile_decoder_tp_builder.py).")
    ...
    rotary_embedding_dim = head_dim  # Phi-3-mini uses full rotary
    ...
    if verbose:
        print(f"[trtmc-build] Building Phi-3 TP decoder engine "
    ...
    if plan is None:
        raise RuntimeError("Phi-3 tensor-parallel engine build failed")
    return bytes(plan)
