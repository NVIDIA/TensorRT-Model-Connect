...
def build_dual_profile_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp16', opt_prefill_length: int=64, max_prefill_length: int | None=None, quant_ctx: 'QuantContext | None'=None, norm_type: str='rmsnorm', mlp_type: str='swiglu', position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, round_rope_inv_freq_to_bf16: bool=False, verbose: bool=False, dynamic_kv_profile_rows: list[int] | None=None, profile_mode: str='dual_profile') -> bytes:
    ...
    * ``"prefill"``: one prefill profile only. This is used by split-engine
    ...
    mrope_position_ids = network.add_input('mrope_position_ids', trt.int32, (3, -1)) if mrope_section is not None else None
    ...
    def _add_profile(opt_sq: int, max_sq: int, *, fixed: bool=False, cache_rows_min: int | None=None, cache_rows_opt: int | None=None, cache_rows_max: int | None=None):
        ...
        if mrope_position_ids is not None:
            prof.set_shape('mrope_position_ids', (3, min_sq), (3, opt_sq), (3, max_sq))
    ...
    if position_type == 'rope':
        ...
        cos_half_np = graph_ops.make_rope_table_half_dim(kmax, head_dim, config.rope_theta, True, partial_rotary_factor, interleaved=interleaved_rope, round_inv_freq_to_bf16=round_rope_inv_freq_to_bf16)
        sin_half_np = graph_ops.make_rope_table_half_dim(kmax, head_dim, config.rope_theta, False, partial_rotary_factor, interleaved=interleaved_rope, round_inv_freq_to_bf16=round_rope_inv_freq_to_bf16)
        if round_rope_inv_freq_to_bf16:
            cos_half_table = graph_ops.add_constant(network, cos_half_np.shape, cos_half_np, dtype=np.float32)
            sin_half_table = graph_ops.add_constant(network, sin_half_np.shape, sin_half_np, dtype=np.float32)
            if work_trt_dtype != trt.float32:
                cos_half_table = network.add_cast(cos_half_table, work_trt_dtype).get_output(0)
                sin_half_table = network.add_cast(sin_half_table, work_trt_dtype).get_output(0)
        else:
    ...
    if input_embed_tensor is not None and use_input_embed_tensor is not None:
        ...
        one = _const_in_work_dtype(network, (1, 1), np.array([[1.0]], dtype=work_np_dtype), work_np_dtype, work_trt_dtype)
    ...
    for layer_idx in range(num_layers):
        ...
        normed = _norm_multi(network, hidden_state, hidden, weights[f'{prefix}.input_norm'], weights.get(f'{prefix}.input_norm_beta'), eps_tensor, norm_type, work_np_dtype)
        ...
        if position_type == 'rope':
            if mrope_position_ids is not None and mrope_section is not None:
                q = graph_ops.add_apply_mrope_native_sequence(network, q, num_heads, head_dim, cos_half_table, sin_half_table, mrope_position_ids, mrope_section, rotary_embedding_dim, interleaved_rope)
                k = graph_ops.add_apply_mrope_native_sequence(network, k, num_kv_heads, head_dim, cos_half_table, sin_half_table, mrope_position_ids, mrope_section, rotary_embedding_dim, interleaved_rope)
            else:
        ...
        all_v_cat.axis = 0
        context = graph_ops.add_attention_from_rows(network, q, all_k_cat.get_output(0), all_v_cat.get_output(0), num_heads=num_heads, head_dim=head_dim, num_kv_heads=num_kv_heads, q_seq=None, kv_seq=None, causal=False, mask=mask_4d, scale=attn_scale, tag=f'{prefix}.attn')
        ...
        if parallel_residual:
            ...
            if post_attn_norm_w is not None:
                norm2 = _norm_multi(network, hidden_state, hidden, post_attn_norm_w, weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, norm_type, work_np_dtype)
            else:
                norm2 = normed
        else:
            residual1 = network.add_elementwise(hidden_state, attn_out, trt.ElementWiseOperation.SUM)
            norm2 = _norm_multi(network, residual1.get_output(0), hidden, weights[f'{prefix}.post_attn_norm'], weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, norm_type, work_np_dtype)
        if mlp_type == 'gelu_fc':
    ...
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = _norm_multi(network, hidden_state, hidden, final_norm, weights.get('final_norm_beta'), eps_tensor, norm_type, work_np_dtype)
    ...
    logits = graph_ops.add_matmul_rhs_constant(network, last_hidden, hidden, out_vocab, weights['w_out'], dtype=work_np_dtype)
