...
def build_standard_decoder_engine(config: ModelConfig, weights: WeightDict, max_cache_length: int, *, precision: str='fp32', quant_ctx: QuantContext | None=None, norm_type: str='rmsnorm', mlp_type: str='swiglu', position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, round_rope_inv_freq_to_bf16: bool=False, verbose: bool=False, debug_layer_outputs: bool=False, hidden_state_output: bool=False) -> bytes:
    ...
    if not _dual_profile_disabled_for and (decoder_engine_role in ('dual_profile', 'prefill') or vl_single_engine_fallback):
        return build_dual_profile_decoder_engine(config, weights, max_cache_length, precision=precision, quant_ctx=quant_ctx, norm_type=norm_type, mlp_type=mlp_type, position_type=position_type, activation=activation, partial_rotary_factor=partial_rotary_factor, interleaved_rope=interleaved_rope, parallel_residual=parallel_residual, scale_attn_weights=scale_attn_weights, round_rope_inv_freq_to_bf16=round_rope_inv_freq_to_bf16, verbose=verbose, profile_mode='prefill' if decoder_engine_role == 'prefill' else 'dual_profile')
    ...
    if position_type == 'rope':
        ...
        cos_half_np = graph_ops.make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, True, partial_rotary_factor, interleaved=interleaved_rope, round_inv_freq_to_bf16=round_rope_inv_freq_to_bf16)
        sin_half_np = graph_ops.make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, False, partial_rotary_factor, interleaved=interleaved_rope, round_inv_freq_to_bf16=round_rope_inv_freq_to_bf16)
        rope_storage_dtype = np.float32 if round_rope_inv_freq_to_bf16 else work_np_dtype
        cos_half_tensor = graph_ops.add_constant(network, cos_half_np.shape, cos_half_np, dtype=rope_storage_dtype)
        cos_half_tensor = _cast_work_dtype(cos_half_tensor)
        sin_half_tensor = graph_ops.add_constant(network, sin_half_np.shape, sin_half_np, dtype=rope_storage_dtype)
