...
// LanceISampler: token selection abstraction for autoregressive generation.
//
...
//
// LanceLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// lance-owned sampler interface.
class LanceISampler {
  public:
    virtual ~LanceISampler() = default;
    ...
    virtual LanceSampleResult sample(const float* logits, int32_t vocab_size,
                                     const LanceSamplingParams& params) = 0;
    ...
    virtual LanceLogitsLocation logits_location() const = 0;
...
/// Build LanceSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from LanceSamplingParams.
...
std::unique_ptr<LanceISampler> create_lance_sampler(const LanceSamplingParams& params,
                                                    const LanceSamplerFactoryOptions& options);
