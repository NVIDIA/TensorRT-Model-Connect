...
from tensorrt_model_connect.models.fast_foundation_stereo.tests.e2e_plugins.comparator import (
    StereoDisparityComparator,
)
from tests.e2e_harness.contracts import StageOutput, StageSpec, ThresholdProfile
from tensorrt_model_connect.config import ModelConfig
from tensorrt_model_connect.models.fast_foundation_stereo import model as family_model
from tensorrt_model_connect.models.fast_foundation_stereo.prepare_model import (
...
_FAMILY_SOURCE_DIR = (
    _REPOSITORY_ROOT / "python/tensorrt_model_connect/models/fast_foundation_stereo"
)
_RUNTIME_SOURCE_DIR = _FAMILY_SOURCE_DIR / "runtime"
...
def test_config_adapter_claims_only_complete_source_package(tmp_path: Path) -> None:
    assert family_model.config_from_dir(tmp_path) is None
    model_dir = _model_dir(tmp_path)
    config = family_model.config_from_dir(model_dir)
...
def test_model_module_owns_unique_strategy_and_skips_tokenizer() -> None:
    plugin = family_model
...
def test_flat_hf_checkpoint_is_staged_with_pinned_source(tmp_path: Path, monkeypatch) -> None:
    ...
    assert family_model.config_from_dir(staged) is not None
...
def test_plugin_builds_feature_and_family_owned_post_section(tmp_path: Path, monkeypatch) -> None:
    model_dir = _model_dir(tmp_path)
    raw = family_model.config_from_dir(model_dir)
    assert raw is not None
    config = ModelConfig.from_json(json.dumps(raw))
    plugin = family_model
    weights = plugin.load_weights(str(model_dir), config, precision="fp16")

    from tensorrt_model_connect.models.fast_foundation_stereo import builder
    ...
    from tensorrt_model_connect.models.fast_foundation_stereo import native_plugin_builder
...
def test_production_family_uses_only_native_tensorrt_network_definition() -> None:
    sources = sorted(
        ...
        if path.is_file()
        and "tests" not in path.relative_to(_FAMILY_SOURCE_DIR).parts
...
def test_native_basic_conv_preserves_distilled_boolean_activation_semantics() -> None:
    from tensorrt_model_connect.models.fast_foundation_stereo.native_graph import NativeGraph
...
def test_feature_att_8_folds_exact_three_bn_and_preserves_relu_gate_and_checkpoint() -> None:
    from tensorrt_model_connect.models.fast_foundation_stereo.native_graph import NativeGraph
    from tensorrt_model_connect.models.fast_foundation_stereo.native_post import (
...
def test_cost_aggregation_uses_fixed_default_winner_routes(monkeypatch) -> None:
    from tensorrt_model_connect.models.fast_foundation_stereo import native_post
...
def test_raft_gru_stacks_update_and_reset_convolutions() -> None:
    from tensorrt_model_connect.models.fast_foundation_stereo.native_post import _raft_gru
