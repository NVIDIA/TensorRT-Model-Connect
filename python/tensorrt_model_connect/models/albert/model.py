...
"""ALBERT family model -- encoder-only with cross-layer parameter sharing.
...
def load_weights(model_dir: str, config: ModelConfig) -> WeightDict:
    ...
    for group_idx in range(num_hidden_groups):
        for inner_idx in range(inner_group_num):
            hf_prefix = f"albert.encoder.albert_layer_groups.{group_idx}.albert_layers.{inner_idx}"
...
def build_engine(
    ...
    *,
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="ALBERT tensor-parallel builds")
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete albert bundle inside its owning family module."""
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("albert does not support context-parallel builds")
    if options.get("dynamic_kv_cache") or options.get("triattention_stats_path"):
        raise ValueError("albert does not use a decoder KV-cache runtime")
