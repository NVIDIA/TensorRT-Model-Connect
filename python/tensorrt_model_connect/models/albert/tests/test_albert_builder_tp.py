...
"""Tensor-parallel tests for ALBERT encoder support."""
...
from tensorrt_model_connect.models.albert import model as AlbertModel
from tensorrt_model_connect.parallel_config import ParallelConfig

albert_plugin = importlib.import_module(
    "tensorrt_model_connect.models.albert.model")
...
def _albert_tp_builder_module():
    return pytest.importorskip(
        "tensorrt_model_connect.models.albert.tp_builder",
...
def test_albert_tp_builder_rejects_single_device_mode():
    tp_builder = _albert_tp_builder_module()
...
def test_albert_tp_validation_rejects_bad_config_dimensions(
    parallel,
    overrides,
    message,
):
...
def test_albert_tp_shards_rank_local_encoder_weights():
    tp_builder = _albert_tp_builder_module()
    config = _make_config()
...
def test_albert_plugin_routes_tp_build(monkeypatch):
    tp_builder = _albert_tp_builder_module()
    captured = {}
    ...
    assert plan == b"albert-tp-plan"
    assert captured["config"].model_type == "albert"
...
def test_albert_plugin_rejects_quantized_tp(monkeypatch):
    monkeypatch.setattr(
        albert_plugin,
        "require_tensorrt_11_for_tensor_parallel",
        lambda parallel, *, feature: None,
