...
class QwenVLCalibrationAdapter:
    """Qwen-VL image-and-text calibration and layer-name mapping."""
    ...
    def _calibration_images():
        ...
        for color in ("red", "green", "blue", "orange"):
            ...
            ImageDraw.Draw(image).ellipse([80, 80, 368, 368], fill=color)
            images.append(image)
        return images

    @staticmethod
    def _load_calibration_images():
        """Use representative images when declared, otherwise synthetic inputs."""
        import os

        directory = os.environ.get("TRTMC_CALIB_IMAGE_DIR")
        if directory and os.path.isdir(directory):
            ...
            for filename in sorted(os.listdir(directory)):
                if filename.lower().endswith((".jpg", ".jpeg", ".png", ".webp")):
                    try:
                        images.append(
                            Image.open(os.path.join(directory, filename)).convert("RGB")
                        )
                    except Exception:
                        pass
                if len(images) >= 64:
            ...
            if images:
                print(
                    f"[qwenvl-calib] using {len(images)} real calibration images "
                    f"from {directory}",
                    flush=True,
                )
                return images
        print(
            "[qwenvl-calib] TRTMC_CALIB_IMAGE_DIR unset -> synthetic PLACEHOLDER "
            "images (not representative; set the env for real accuracy)",
            flush=True,
        )
        ...
        *,
        ...
        try:
            ...
            if os.path.isfile(tokenizer_config):
                with open(tokenizer_config, encoding="utf-8") as stream:
                    chat_template = json.load(stream).get("chat_template")
        except Exception:
            chat_template = None
        ...
        if manifest and os.path.isfile(manifest):
            ...
            print(
                f"[qwenvl-calib] paired manifest: {len(entries)} "
                f"(image,prompt) pairs from {manifest}",
                flush=True,
            )
            used = 0
            ...
            print(
                f"[qwenvl-calib] paired manifest batches: {used}/{num_samples}",
                flush=True,
            )
            return
        ...
        pairs = [(image, prompt) for image in images for prompt in prompts]
        used_images = 0
        for image, prompt in itertools.islice(itertools.cycle(pairs), num_samples):
            try:
                messages = [
                    {
                        "role": "user",
        ...
        print(
            f"[qwenvl-calib] image+text batches: {used_images}/{num_samples} "
            f"(rest = text-only fallback); images={len(images)}",
            flush=True,
        )
...
def _build_qwen3_vl_decoder(
    ...
    *,
    ...
    Uses graph_blocks composition instead of default_decoder so
