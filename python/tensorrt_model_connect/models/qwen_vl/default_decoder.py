...
def build_standard_decoder_engine(config: ModelConfig, weights: WeightDict, max_cache_length: int, *, precision: str='fp32', quant_ctx: QuantContext | None=None, norm_type: str='rmsnorm', mlp_type: str='swiglu', position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, verbose: bool=False, debug_layer_outputs: bool=False, hidden_state_output: bool=False) -> bytes:
    ...
    split_decode = active_split and decoder_engine_role == 'decode'
    decode_attention = _decode_attention_backend(config)
    opt_prefill_length, max_prefill_length, builder_workspace_bytes = _decoder_profile_options(config)
    if decode_attention == 'decomposed' and decoder_engine_role != 'prefill' and (not split_decode):
        raise ValueError('qwen_vl_decoder.decode_attention=decomposed requires --decoder-engine-layout split')
    vl_single_engine_fallback = not active_split and decoder_engine_role == 'decode'
    if not _dual_profile_disabled_for and (decoder_engine_role in ('dual_profile', 'prefill') or split_decode or vl_single_engine_fallback):
        return build_dual_profile_decoder_engine(config, weights, max_cache_length, precision=precision, quant_ctx=quant_ctx, norm_type=norm_type, mlp_type=mlp_type, position_type=position_type, activation=activation, partial_rotary_factor=partial_rotary_factor, interleaved_rope=interleaved_rope, parallel_residual=parallel_residual, scale_attn_weights=scale_attn_weights, verbose=verbose, opt_prefill_length=opt_prefill_length, max_prefill_length=max_prefill_length, builder_workspace_bytes=builder_workspace_bytes, force_decomposed_attention=split_decode and decode_attention == 'decomposed', profile_mode='prefill' if decoder_engine_role == 'prefill' else 'decode' if split_decode else 'dual_profile')
    ...
    raw_mrope_section = rope_scaling.get('mrope_section') if isinstance(rope_scaling, dict) else None
    mrope_section = tuple((int(value) for value in raw_mrope_section)) if raw_mrope_section is not None else None
    ...
    if raw_profile_rows:
        ...
        dynamic_kv_profile_rows = []
    builder_context = create_builder_context(verbose=verbose, workspace_bytes=builder_workspace_bytes)
    ...
    mrope_position_ids = network.add_input('mrope_position_ids', trt.int32, (3,)) if mrope_section is not None else None
    ...
    if position_type == 'rope':
        ...
        if mrope_section is not None:
            if len(mrope_section) != 3 or any((value <= 0 for value in mrope_section)):
                raise ValueError('rope_scaling.mrope_section must contain three positive integers')
            if sum(mrope_section) != rotary_embedding_dim // 2:
                raise ValueError(f'rope_scaling.mrope_section must sum to half the rotary dimension; got {mrope_section} for rotary dimension {rotary_embedding_dim}')
        ...
        position_embed_table = graph_ops.add_constant(network, pos_embed_np.shape, pos_embed_np, dtype=work_np_dtype)
    ...
    logits = graph_ops.add_matmul_rhs_constant(network, hidden_state, hidden, out_vocab, weights['w_out'], dtype=work_np_dtype)
