...
``default_decoder`` supports — RMSNorm or LayerNorm; SwiGLU or
...
on ``default_decoder`` for now and are dispatched there from
...
def build_dual_profile_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp16', opt_prefill_length: int=64, max_prefill_length: int | None=None, builder_workspace_bytes: int=1 << 30, quant_ctx: 'QuantContext | None'=None, norm_type: str='rmsnorm', mlp_type: str='swiglu', position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, verbose: bool=False, dynamic_kv_profile_rows: list[int] | None=None, force_decomposed_attention: bool=False, profile_mode: str='dual_profile') -> bytes:
    ...
    * ``"decode"``: one fixed-Sq=1 decode profile only. This is the companion
    ...
    if profile_mode not in ('dual_profile', 'prefill', 'decode'):
        raise ValueError(f"profile_mode must be 'dual_profile', 'prefill', or 'decode', got {profile_mode!r}")
    ...
    matmul = _make_matmul_fn(network, work_np_dtype, quant_ctx, lora_config=lora_config)
    ...
    for layer_idx in range(num_layers):
        ...
        all_v_cat.axis = 0
        context = graph_ops.add_attention_from_rows(network, q, all_k_cat.get_output(0), all_v_cat.get_output(0), num_heads=num_heads, head_dim=head_dim, num_kv_heads=num_kv_heads, q_seq=None, kv_seq=None, causal=False, mask=mask_4d, scale=attn_scale, fp32_accumulation=profile_mode == 'decode', force_decomposed_attention=force_decomposed_attention, tag=f'{prefix}.attn')
    ...
    if verbose:
        mode_label = {'prefill': 'prefill-profile', 'decode': 'decode-profile', 'dual_profile': 'dual-profile'}[profile_mode]
