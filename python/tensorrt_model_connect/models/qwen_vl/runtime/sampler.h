...
// QwenVlISampler: token selection abstraction for autoregressive generation.
//
...
//
// QwenVlLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// qwen_vl-owned sampler interface.
class QwenVlISampler {
  public:
    virtual ~QwenVlISampler() = default;
    ...
    virtual QwenVlSampleResult sample(const float* logits, int32_t vocab_size,
                                      const QwenVlSamplingParams& params) = 0;
    ...
    virtual QwenVlLogitsLocation logits_location() const = 0;
...
/// Build QwenVlSamplingParams from GenerateConfig fields.
...
std::unique_ptr<QwenVlISampler> create_qwen_vl_sampler(const QwenVlSamplingParams& params,
                                                       const QwenVlSamplerFactoryOptions& options);
