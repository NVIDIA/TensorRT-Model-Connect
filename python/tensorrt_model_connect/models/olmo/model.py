...
"""OLMo family model — non-parametric LayerNorm + tied embeddings.
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete olmo bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("olmo does not support context-parallel builds")
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="olmo tensor-parallel builds")

        if quant_ctx is not None:
            raise ValueError("olmo tensor-parallel builds do not support quantization")
        if dynamic_kv_cache:
            raise NotImplementedError(
                "Tensor-parallel olmo builds do not support dynamic KV cache or TriAttention"
            )
