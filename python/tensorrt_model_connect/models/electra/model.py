...
"""Electra family model -- encoder-only discriminator/generator transformer.
...
def load_weights(model_dir: str, config: ModelConfig) -> WeightDict:
    ...
    if _has_tensor(readers, tt_key):
        ...
        if embedding_size != hidden and tt_embed.shape[1] == embedding_size:
            proj_w = _load_tensor(readers, _bpfx(root, "embeddings_project.weight"))
            proj_b = _load_tensor(readers, _bpfx(root, "embeddings_project.bias"))
            tt_embed = tt_embed.astype(np.float32) @ proj_w.T.astype(np.float32) + proj_b.astype(
                np.float32
            )
...
def build_engine(
    ...
    *,
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="ELECTRA tensor-parallel builds")
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete electra bundle inside its owning family module."""
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("electra does not support context-parallel builds")
    if options.get("dynamic_kv_cache") or options.get("triattention_stats_path"):
        raise ValueError("electra does not use a decoder KV-cache runtime")
