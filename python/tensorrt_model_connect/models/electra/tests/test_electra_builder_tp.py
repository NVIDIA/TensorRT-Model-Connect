...
"""Tensor-parallel tests for ELECTRA encoder support."""
...
from tensorrt_model_connect.models.electra import model as ElectraModel
from tensorrt_model_connect.parallel_config import ParallelConfig

electra_plugin = importlib.import_module(
    "tensorrt_model_connect.models.electra.model")
...
def _electra_tp_builder_module():
    return pytest.importorskip(
        "tensorrt_model_connect.models.electra.tp_builder",
...
def test_electra_tp_builder_rejects_single_device_mode():
    tp_builder = _electra_tp_builder_module()
...
def test_electra_tp_validation_rejects_bad_config_dimensions(
    parallel,
    overrides,
    message,
):
...
def test_electra_tp_shards_rank_local_encoder_weights():
    tp_builder = _electra_tp_builder_module()
    config = _make_config()
...
def test_electra_plugin_routes_tp_build(monkeypatch):
    tp_builder = _electra_tp_builder_module()
    captured = {}
    ...
    plan = ElectraModel.build_engine(
        _make_config(),
        _make_encoder_weights(),
        max_cache_length=8,
    ...
    assert plan == b"electra-tp-plan"
    assert captured["config"].model_type == "electra"
