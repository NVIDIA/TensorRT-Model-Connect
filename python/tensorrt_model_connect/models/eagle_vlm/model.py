...
"""Eagle VLM family model — embedding and reranking models.
...
_runtime_strategy = "eagle_vlm_embedding"
...
def build_engine(
    ...
    *,
    ...
    global _runtime_strategy, runtime_strategy
    is_rerank = _is_reranker(config)
    # Store for runtime_strategy property
    _runtime_strategy = "eagle_vlm_reranking" if is_rerank else "eagle_vlm_embedding"
    runtime_strategy = _runtime_strategy
...
def _build_eagle_engine(
    ...
    *,
    ...
    if keep_reranker_tail_fp32 and pad_mask_4d.dtype != trt.float32:
        tail_pad_mask_4d = network.add_cast(pad_mask_4d, trt.float32).get_output(0)
    ...
    for layer_idx in range(num_layers):
        prefix = f"layer.{layer_idx}"
        is_reranker_tail = keep_reranker_tail_fp32 and layer_idx >= max(
            num_layers - _RERANKER_FP32_TAIL_LAYERS, 0
        )
        ...
        layer_cos_half_tensor = tail_cos_half_tensor if is_reranker_tail else cos_half_tensor
        layer_sin_half_tensor = tail_sin_half_tensor if is_reranker_tail else sin_half_tensor
        layer_pad_mask_4d = tail_pad_mask_4d if is_reranker_tail else pad_mask_4d
        ...
        q = graph_ops.add_matmul_rhs_constant(
            network, norm1, hidden, attention_size, weights[f"{prefix}.w_q"], dtype=layer_np_dtype
        )
    ...
    if final_norm is not None and len(final_norm) > 0:
        final_norm_dtype = np.float32 if keep_reranker_tail_fp32 else work_np_dtype
        final_eps_tensor = tail_eps_tensor if keep_reranker_tail_fp32 else eps_tensor
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete eagle_vlm bundle inside its owning family module."""
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("eagle_vlm does not support context-parallel builds")
    if options.get("dynamic_kv_cache") or options.get("triattention_stats_path"):
        raise ValueError("eagle_vlm does not use a decoder KV-cache runtime")
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(
            parallel, feature="eagle_vlm tensor-parallel builds"
        )
        if quant_ctx is not None:
            raise ValueError("eagle_vlm tensor-parallel builds do not support quantization")
