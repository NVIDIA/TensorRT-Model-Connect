...
// NemotronInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   NemotronKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — NemotronKvCache + family-owned recurrent state composed
...
class NemotronInferenceState {
  public:
    virtual ~NemotronInferenceState() = default;
    ...
    // NemotronKvCache -> true. Family-owned recurrent state -> false.
