...
"""Nemotron-4 family model — LayerNorm1P + squared ReLU MLP + GQA + partial RoPE.
...
def load_weights(model_dir: str, config: ModelConfig, *, precision: str = "fp32") -> WeightDict:
    ...
    for layer_idx in range(num_layers):
        ...
        if _has_tensor(readers, up_bias_key):
            weights[f"{prefix}.fc1_bias"] = _load_tensor(readers, up_bias_key).astype(target_dtype)
...
def build_engine(
    ...
    *,
    ...
    if normalize_parallel_config(parallel_config).enabled:
        raise ValueError("nemotron does not support tensor-parallel builds")
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete nemotron bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("nemotron does not support context-parallel builds")
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="nemotron tensor-parallel builds")
        raise ValueError("nemotron does not support tensor-parallel builds")
