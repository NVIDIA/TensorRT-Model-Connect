...
"""GPT-OSS family model — OpenAI GPT-OSS MoE with packed expert weights.
...
def load_weights(model_dir: str, config: ModelConfig, *, precision: str = "fp32") -> WeightDict:
    ...
    print(
        "[trtmc build] Loading GPT-OSS model (MXFP4 auto-dequant) ...", file=sys.stderr, flush=True
    )
    ...
    for layer_idx in range(num_layers):
        ...
        # RMSNorm (no biases)
        weights[f"{prefix}.input_norm"] = state[f"{hf}.input_layernorm.weight"].astype(np.float32)
        weights[f"{prefix}.post_attn_norm"] = state[f"{hf}.post_attention_layernorm.weight"].astype(
            np.float32
        )
...
def build_engine(
    ...
    *,
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="GPT-OSS tensor-parallel builds")
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete gpt_oss bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("gpt_oss does not support context-parallel builds")
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="gpt_oss tensor-parallel builds")

        if quant_ctx is not None:
            raise ValueError("gpt_oss tensor-parallel builds do not support quantization")
        if dynamic_kv_cache:
            raise NotImplementedError(
                "Tensor-parallel gpt_oss builds do not support dynamic KV cache or TriAttention"
            )
