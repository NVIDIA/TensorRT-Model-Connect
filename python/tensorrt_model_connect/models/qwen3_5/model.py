...
"""Qwen3.5 family model -- Hybrid Gated DeltaNet + self-attention decoder.
...
def load_weights(model_dir: str, config: ModelConfig) -> WeightDict:
    ...
    for layer_idx in range(num_layers):
        ...
        if _has_tensor(readers, post_norm_key):
            weights[f"{prefix}.post_attn_norm"] = 1.0 + _load_tensor(readers, post_norm_key).astype(
                np.float32
            )
        else:
        ...
        _load_mlp_weights(readers, weights, prefix, hf_prefix, hidden, mlp_size)
...
def _load_deltanet_weights(
    ...
    weights[f"{prefix}.deltanet_in_proj_qkv"] = _transpose_2d(in_proj_raw, "deltanet_in_proj_qkv")
...
def _load_mlp_weights(
    ...
    if _has_tensor(readers, gate_key):
        weights[f"{prefix}.w_gate"] = _transpose_2d(_load_tensor(readers, gate_key), "gate_proj")
        weights[f"{prefix}.w_up"] = _transpose_2d(_load_tensor(readers, up_key), "up_proj")
        weights[f"{prefix}.w_down"] = _transpose_2d(_load_tensor(readers, down_key), "down_proj")
...
def build_engine(
    ...
    *,
    ...
    cos_half_tensor = graph_ops.add_constant(network, cos_half.shape, cos_half, dtype=work_np_dtype)
    sin_half_tensor = graph_ops.add_constant(network, sin_half.shape, sin_half, dtype=work_np_dtype)
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete qwen3_5 bundle inside its owning family module."""
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("qwen3_5 does not support context-parallel builds")
    if options.get("dynamic_kv_cache") or options.get("triattention_stats_path"):
        raise ValueError("qwen3_5 does not use a decoder KV-cache runtime")
    ...
    if parallel.enabled:
        raise ValueError("qwen3_5 does not support tensor-parallel builds")
