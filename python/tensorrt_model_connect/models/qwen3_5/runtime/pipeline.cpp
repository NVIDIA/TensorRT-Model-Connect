...
static std::vector<int32_t> encode_prompt(const ITokenizer& tokenizer,
    ...
    if (cfg.use_chat_template && !config.chat_template_format.empty()) {
        effective =
            qwen3_5_apply_chat_template(config.chat_template_format, prompt, cfg.enable_thinking);
        templated = true;
    }
...
TextResult RecurrentPipeline::generate(const std::string& prompt, const GenerateConfig& cfg) {
    ...
    auto sp = qwen35_sampling_params_from_config(cfg, eos);
...
std::vector<int32_t> RecurrentPipeline::generate_from_ids(const std::vector<int32_t>& input_ids,
    ...
    Qwen35ISampler* active_sampler = sampler_.get();
    std::unique_ptr<Qwen35ISampler> local_sampler;
    if (!active_sampler) {
        local_sampler = create_qwen35_sampler(params);
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        Qwen35SampleResult result = active_sampler->sample(logits.data(), vocab_size, params);
