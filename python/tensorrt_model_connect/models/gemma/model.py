...
"""Gemma family model — applies +1.0 to RMSNorm gamma and sqrt(hidden) embed scale."""
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete gemma bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("gemma does not support context-parallel builds")
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="gemma tensor-parallel builds")

        if quant_ctx is not None:
            raise ValueError("gemma tensor-parallel builds do not support quantization")
        if dynamic_kv_cache:
            raise NotImplementedError(
                "Tensor-parallel gemma builds do not support dynamic KV cache or TriAttention"
            )
