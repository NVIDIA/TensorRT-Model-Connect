...
def build_personaplex_tp_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp32', quant_ctx=None, activation: str='silu', partial_rotary_factor: float=1.0, parallel_residual: bool=False, scale_attn_weights: bool=True, verbose: bool=False, debug_layer_outputs: bool=False, parallel_config=None, _builder_context_factory: BuilderContextFactory) -> bytes:
    ...
    for i in range(num_layers):
        cache_k_inputs.append(network.add_input(graph_ops.layer_tensor_name('cache_k', i), work_trt_dtype, (max_cache_length, kv_attention_size)))
        cache_v_inputs.append(network.add_input(graph_ops.layer_tensor_name('cache_v', i), work_trt_dtype, (max_cache_length, kv_attention_size)))
    ...
    cos_half_tensor = _cast_work_dtype(graph_ops.add_constant(network, cos_half_np.shape, cos_half_np, dtype=work_np_dtype))
    sin_half_tensor = _cast_work_dtype(graph_ops.add_constant(network, sin_half_np.shape, sin_half_np, dtype=work_np_dtype))
    ...
    if input_embed_tensor is not None and use_input_embed_tensor is not None:
        ...
        inv_flag = network.add_elementwise(one_const, flag_for_math, trt.ElementWiseOperation.SUB).get_output(0)
        tok_part = network.add_elementwise(inv_flag, token_embed, trt.ElementWiseOperation.PROD).get_output(0)
        embed_part = network.add_elementwise(flag_for_math, input_embed_tensor, trt.ElementWiseOperation.PROD).get_output(0)
        hidden_state = network.add_elementwise(tok_part, embed_part, trt.ElementWiseOperation.SUM).get_output(0)
    else:
    ...
    for layer_idx in range(num_layers):
        ...
        result = _add_tp_decoder_layer(network=network, hidden=hidden_state, cache_k=cache_k_inputs[layer_idx], cache_v=cache_v_inputs[layer_idx], attention_mask=attention_mask, position_id=position_id, attention_scale=attn_scale, eps_tensor=eps_tensor, eps=config.rms_norm_eps, weights=weights, prefix=prefix, hidden_size=hidden, attention_size=attention_size, kv_attention_size=kv_attention_size, mlp_size=mlp_size, num_heads=num_heads, num_kv_heads=num_kv_heads, head_dim=head_dim, max_cache_length=max_cache_length, activation=activation, parallel_residual=parallel_residual, dtype=work_np_dtype, quant_ctx=quant_ctx, cos_half_tensor=cos_half_tensor, sin_half_tensor=sin_half_tensor, rotary_embedding_dim=rotary_embedding_dim, tp_size=parallel.tp_size)
    ...
    if lm_bias is not None:
        ...
        logits = graph_ops.add_bias_sum(network, logits, out_vocab, np.zeros(out_vocab, dtype=work_np_dtype), dtype=work_np_dtype)
    if work_trt_dtype != trt.float32:
    ...
    if verbose:
        print(f'[trtmc build] Building PersonaPlex temporal TP rank {parallel.rank}/{parallel.tp_size} ({num_layers}L, h={hidden}, local_heads={num_heads}, local_attn={attention_size}, local_mlp={mlp_size}, cache={max_cache_length}, precision={precision}) ...', file=sys.stderr)
...
def _add_tp_decoder_layer(*, network: trt.INetworkDefinition, hidden: trt.ITensor, cache_k: trt.ITensor, cache_v: trt.ITensor, attention_mask: trt.ITensor, position_id: trt.ITensor, attention_scale: float | None, eps_tensor: trt.ITensor, weights: 'WeightDict', prefix: str, hidden_size: int, attention_size: int, kv_attention_size: int, mlp_size: int, num_heads: int, num_kv_heads: int, head_dim: int, max_cache_length: int, activation: str, parallel_residual: bool, dtype: np.dtype, quant_ctx, cos_half_tensor: trt.ITensor, sin_half_tensor: trt.ITensor, rotary_embedding_dim: int, tp_size: int, eps: float | None=None) -> dict[str, trt.ITensor]:
    attn = graph_blocks.add_attention_block(network, hidden, cache_k, cache_v, attention_mask, position_id, weights=weights, prefix=prefix, hidden_size=hidden_size, attention_size=attention_size, kv_attention_size=kv_attention_size, num_heads=num_heads, num_kv_heads=num_kv_heads, head_dim=head_dim, max_cache_length=max_cache_length, attention_scale=attention_scale, eps_tensor=eps_tensor, eps=eps, position_type='rope', alibi_slopes_tensor=None, alibi_indices_tensor=None, dtype=dtype, quant_ctx=quant_ctx, layer_prefix=prefix, cos_half_tensor=cos_half_tensor, sin_half_tensor=sin_half_tensor, rotary_embedding_dim=rotary_embedding_dim, interleaved_rope=True, ffi_attention_kernel=None, dynamic_kv_cache=False)
    ...
    if parallel_residual:
        ...
        residual1 = network.add_elementwise(hidden, attn_out, trt.ElementWiseOperation.SUM).get_output(0)
        norm2 = _apply_norm(network, residual1, hidden_size, weights[f'{prefix}.post_attn_norm'], weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, 'rmsnorm', dtype=dtype, eps=eps)
    ...
    if parallel_residual:
        sum_attn = network.add_elementwise(hidden, attn_out, trt.ElementWiseOperation.SUM).get_output(0)
        residual2 = network.add_elementwise(sum_attn, mlp_out, trt.ElementWiseOperation.SUM).get_output(0)
        post_attn_tensor = sum_attn
    else:
        residual2 = network.add_elementwise(residual1, mlp_out, trt.ElementWiseOperation.SUM).get_output(0)
        post_attn_tensor = residual1
    return {'hidden': residual2, 'post_attn': post_attn_tensor, 'present_k': attn['present_k'], 'present_v': attn['present_v']}
