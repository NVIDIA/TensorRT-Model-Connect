...
def build_standard_decoder_engine(config: ModelConfig, weights: WeightDict, max_cache_length: int, *, precision: str='fp32', quant_ctx: QuantContext | None=None, position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, verbose: bool=False, debug_layer_outputs: bool=False, hidden_state_output: bool=False, fp32_layers: tuple[int, ...]=(), _builder_context_factory: BuilderContextFactory) -> bytes:
    ...
    invalid_fp32_layers = sorted((layer for layer in selected_fp32_layers if layer < 0 or layer >= num_layers))
    if invalid_fp32_layers:
        raise ValueError(f'PersonaPlex temporal fp32_layers contains out-of-range indices: {invalid_fp32_layers}; expected 0-{num_layers - 1}')
    if selected_fp32_layers and precision != 'fp16':
        raise ValueError('PersonaPlex temporal fp32_layers requires FP16 base precision')
    ...
    for i in range(num_layers):
        ck = network.add_input(graph_ops.layer_tensor_name('cache_k', i), work_trt_dtype, (-1, kv_attention_size) if dynamic_kv_cache else (max_cache_length, kv_attention_size))
        cv = network.add_input(graph_ops.layer_tensor_name('cache_v', i), work_trt_dtype, (-1, kv_attention_size) if dynamic_kv_cache else (max_cache_length, kv_attention_size))
        ...
        if dynamic_kv_profile_rows:
            for profile_rows in dynamic_kv_profile_rows:
                ...
                for i in range(num_layers):
                    ...
                    profile.set_shape(graph_ops.layer_tensor_name('cache_k', i), min_cache_shape, cache_shape, cache_shape)
                    profile.set_shape(graph_ops.layer_tensor_name('cache_v', i), min_cache_shape, cache_shape, cache_shape)
                trt_config.add_optimization_profile(profile)
        else:
            profile = builder.create_optimization_profile()
            profile.set_shape('attention_mask', (1, 2), (1, dynamic_kv_opt_rows + 1), (1, attention_window))
            for i in range(num_layers):
                profile.set_shape(graph_ops.layer_tensor_name('cache_k', i), (1, kv_attention_size), (dynamic_kv_opt_rows, kv_attention_size), (max_cache_length, kv_attention_size))
                profile.set_shape(graph_ops.layer_tensor_name('cache_v', i), (1, kv_attention_size), (dynamic_kv_opt_rows, kv_attention_size), (max_cache_length, kv_attention_size))
            trt_config.add_optimization_profile(profile)
    ...
    embedding_table = graph_ops.add_constant(network, (vocab, hidden), weights['embedding'], dtype=work_np_dtype)
    ...
    if position_type == 'rope':
        ...
        cos_half_np = graph_ops.make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, True, partial_rotary_factor, interleaved=interleaved_rope)
        sin_half_np = graph_ops.make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, False, partial_rotary_factor, interleaved=interleaved_rope)
        cos_half_tensor = graph_ops.add_constant(network, cos_half_np.shape, cos_half_np, dtype=work_np_dtype)
        cos_half_tensor = _cast_work_dtype(cos_half_tensor)
        sin_half_tensor = graph_ops.add_constant(network, sin_half_np.shape, sin_half_np, dtype=work_np_dtype)
        ...
        alibi_indices_tensor = graph_ops.add_constant(network, (max_cache_length,), np.arange(max_cache_length, dtype=np.float32), dtype=np.float32)
    ...
    if embed_norm is not None:
        ...
        hidden_state = graph_ops.add_layer_norm_native(network, hidden_state, hidden, embed_norm, embed_norm_beta, config.rms_norm_eps, dtype=work_np_dtype)
    ...
    if selected_fp32_layers:
        fp32_attention_mask = network.add_cast(attention_mask, trt.float32).get_output(0)
        fp32_eps_tensor = graph_ops.add_constant(network, (1, 1), np.array([config.rms_norm_eps], dtype=np.float32), dtype=np.float32)
        if cos_half_tensor is not None:
            fp32_cos_half_tensor = network.add_cast(cos_half_tensor, trt.float32).get_output(0)
            fp32_sin_half_tensor = network.add_cast(sin_half_tensor, trt.float32).get_output(0)
    for layer_idx in range(num_layers):
        ...
        if layer_is_fp32:
            layer_hidden = network.add_cast(layer_hidden, trt.float32).get_output(0)
            layer_cache_k = network.add_cast(layer_cache_k, trt.float32).get_output(0)
            layer_cache_v = network.add_cast(layer_cache_v, trt.float32).get_output(0)
        ...
        result = _add_decoder_layer(network=network, hidden=layer_hidden, cache_k=layer_cache_k, cache_v=layer_cache_v, attention_mask=layer_attention_mask, position_id=position_id, attention_scale=attn_scale, eps_tensor=layer_eps_tensor, eps=config.rms_norm_eps, weights=weights, prefix=prefix, hidden_size=hidden, attention_size=attention_size, kv_attention_size=kv_attention_size, mlp_size=mlp_size, num_heads=num_heads, num_kv_heads=num_kv_heads, head_dim=head_dim, max_cache_length=max_cache_length, position_type=position_type, activation=activation, parallel_residual=parallel_residual, alibi_slopes_tensor=alibi_slopes_tensor, alibi_indices_tensor=alibi_indices_tensor, dtype=layer_dtype, quant_ctx=quant_ctx, cos_half_tensor=layer_cos_half_tensor, sin_half_tensor=layer_sin_half_tensor, rotary_embedding_dim=rotary_embedding_dim, interleaved_rope=interleaved_rope, ffi_attention_kernel=ffi_attention_kernel, dynamic_kv_cache=dynamic_kv_cache)
        if layer_is_fp32:
            next_layer_is_fp32 = layer_idx + 1 in selected_fp32_layers
            if not next_layer_is_fp32 and layer_idx + 1 < num_layers:
                result['hidden'] = network.add_cast(result['hidden'], work_trt_dtype).get_output(0)
            result['present_k'] = network.add_cast(result['present_k'], work_trt_dtype).get_output(0)
            result['present_v'] = network.add_cast(result['present_v'], work_trt_dtype).get_output(0)
    ...
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = _apply_norm(network, hidden_state, hidden, final_norm, weights.get('final_norm_beta'), tail_eps_tensor, 'rmsnorm', dtype=tail_dtype, eps=config.rms_norm_eps)
    ...
    logits = graph_ops.add_matmul_rhs_constant(network, hidden_state, hidden, out_vocab, weights['w_out'], dtype=tail_dtype)
    lm_bias = weights.get('lm_head_bias')
    if lm_bias is not None:
        logits = graph_ops.add_bias_sum(network, logits, out_vocab, lm_bias, dtype=tail_dtype)
    else:
        b_out = np.zeros(out_vocab, dtype=tail_dtype)
        logits = graph_ops.add_bias_sum(network, logits, out_vocab, b_out, dtype=tail_dtype)
...
def _add_decoder_layer(*, network: trt.INetworkDefinition, hidden: trt.ITensor, cache_k: trt.ITensor, cache_v: trt.ITensor, attention_mask: trt.ITensor, position_id: trt.ITensor, attention_scale: float | None, eps_tensor: trt.ITensor, weights: WeightDict, prefix: str, hidden_size: int, attention_size: int, kv_attention_size: int, mlp_size: int, num_heads: int, num_kv_heads: int, head_dim: int, max_cache_length: int, position_type: str='rope', activation: str='silu', parallel_residual: bool=False, alibi_slopes_tensor: trt.ITensor | None=None, alibi_indices_tensor: trt.ITensor | None=None, dtype: np.dtype=np.float32, quant_ctx: QuantContext | None=None, cos_half_tensor: trt.ITensor | None=None, sin_half_tensor: trt.ITensor | None=None, rotary_embedding_dim: int=0, interleaved_rope: bool=False, ffi_attention_kernel: str | None=None, dynamic_kv_cache: bool=False, eps: float | None=None) -> dict[str, trt.ITensor]:
    ...
    if parallel_residual:
        ...
        if post_attn_norm_w is not None:
            norm2 = _apply_norm(network, hidden, hidden_size, post_attn_norm_w, weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, 'rmsnorm', dtype=dtype, eps=eps)
        else:
            norm2 = attn['normed']
    else:
    ...
    mlp_out = graph_blocks.add_swiglu_mlp(network, norm2, weights=weights, prefix=prefix, hidden_size=hidden_size, mlp_size=mlp_size, dtype=dtype, quant_ctx=quant_ctx, layer_prefix=prefix)
    if parallel_residual:
