...
"""Bark family-owned build implementation -- text-to-audio generation.
...
def _map_bark_decoder_weights(
    ...
    # LM head: default_decoder uses "w_out"
    lm_key = f"{prefix}.lm_head.weight"
    if lm_key in state_dict:
...
_coarse_cfg: dict = {}
_fine_cfg: dict = {}
_codec_seq_length: int = 0
_fine_seq_length: int = 0
...
def load_weights(model_dir: str, config: ModelConfig, *, precision: str = "fp32") -> WeightDict:
    ...
    global _coarse_cfg, _fine_cfg, _semantic_cfg
    ...
    _semantic_cfg = semantic_cfg
    _coarse_cfg = coarse_cfg
    _fine_cfg = fine_cfg
...
def build_extra_engines(
    ...
    *,
    ...
    global _codec_seq_length, _fine_seq_length
    ...
    if parallel.enabled:
        ...
        with timed_trt_compile(build_timing, "extra_bark_coarse_decoder"):
            for rank in range(parallel.tp_size):
                ...
                result[f"coarse_engine_plan_tp_rank{rank}"] = build_bark_tp_decoder_engine(
                    config,
                    coarse_weights,
                    max_cache_length,
                    sub_model="coarse",
    ...
    if embed_parts:
        result["fine_embed"] = np.concatenate([e.ravel() for e in embed_parts]).tobytes()
    pos_key = "fine.position_embedding"
    if pos_key in weights:
        result["fine_position_embed"] = (
...
def get_audio_config(config: ModelConfig) -> dict:
    ...
    if _semantic_cfg:
        cfg["vocab_size"] = _semantic_cfg["vocab_size"]
        cfg["hidden_size"] = _semantic_cfg["hidden_size"]
        cfg["num_hidden_layers"] = _semantic_cfg["num_layers"]
        cfg["num_attention_heads"] = _semantic_cfg["num_heads"]
        cfg["num_key_value_heads"] = _semantic_cfg["num_heads"]
    # Inject coarse sub-model dimensions (detected during load_weights)
    if _coarse_cfg:
        cfg["coarse_hidden_size"] = _coarse_cfg["hidden_size"]
        cfg["coarse_num_layers"] = _coarse_cfg["num_layers"]
        cfg["coarse_num_heads"] = _coarse_cfg["num_heads"]
    # Inject fine sub-model dimensions (detected during load_weights)
    if _fine_cfg:
        cfg["fine_hidden_size"] = _fine_cfg["hidden_size"]
        cfg["fine_num_layers"] = _fine_cfg["num_layers"]
        cfg["fine_num_heads"] = _fine_cfg["num_heads"]
        cfg["fine_codebook_size"] = _fine_cfg.get("codebook_size", 1056)
        cfg["fine_n_lm_heads"] = _fine_cfg.get("n_lm_heads", 7)
        if _fine_seq_length > 0:
            cfg["fine_seq_length"] = _fine_seq_length
    # Codec engine config
    if _codec_seq_length > 0:
        cfg["codec_seq_length"] = _codec_seq_length
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete bark bundle in the owning family module."""
    import json
    import time
