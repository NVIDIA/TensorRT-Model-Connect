...
def add_reflect_pad_1d(
    ...
    if pad_left:
        ...
        left_index_tensor = add_constant(network, (pad_left,), left_indices, dtype=np.int32)
        parts.append(network.add_gather(inp, left_index_tensor, 2).get_output(0))

    parts.append(inp)

    if pad_right:
        right_indices = np.arange(length - 2, length - pad_right - 2, -1, dtype=np.int32)
        right_index_tensor = add_constant(network, (pad_right,), right_indices, dtype=np.int32)
...
def add_lstm_unrolled(
    ...
    # Weight constants
    w_ih_t = add_constant(
        network, (input_size, 4 * H), np.ascontiguousarray(w_ih.T, dtype=np.float32), dtype=dtype
    )
    w_hh_t = add_constant(
        network, (H, 4 * H), np.ascontiguousarray(w_hh.T, dtype=np.float32), dtype=dtype
    )
    ...
    trip_count = network.add_constant((), trt.Weights(np.array(seq_length, dtype=np.int32)))
    ...
    xw = network.add_matrix_multiply(
        x_t, trt.MatrixOperation.NONE, w_ih_t, trt.MatrixOperation.NONE
    )
    hw = network.add_matrix_multiply(h, trt.MatrixOperation.NONE, w_hh_t, trt.MatrixOperation.NONE)
    gates = network.add_elementwise(
        xw.get_output(0), hw.get_output(0), trt.ElementWiseOperation.SUM
    )
    gates = network.add_elementwise(gates.get_output(0), bias_t, trt.ElementWiseOperation.SUM)

    # Split gates: i, f, g, o each [1, H]
    gate_i = network.add_slice(gates.get_output(0), start=(0, 0), shape=(1, H), stride=(1, 1))
    gate_f = network.add_slice(gates.get_output(0), start=(0, H), shape=(1, H), stride=(1, 1))
    gate_g = network.add_slice(gates.get_output(0), start=(0, 2 * H), shape=(1, H), stride=(1, 1))
    gate_o = network.add_slice(gates.get_output(0), start=(0, 3 * H), shape=(1, H), stride=(1, 1))
    ...
    # c_new = f * c + i * g
    fc = network.add_elementwise(f_t, c, trt.ElementWiseOperation.PROD).get_output(0)
    ig = network.add_elementwise(i_t, g_t, trt.ElementWiseOperation.PROD).get_output(0)
    c_new = network.add_elementwise(fc, ig, trt.ElementWiseOperation.SUM).get_output(0)
...
def add_attention_from_rows(
    ...
    *,
    ...
    if logit_softcap is not None and float(logit_softcap) > 0.0:
        ...
        output_dtype = q_4d.dtype
        k_4d = _repeat_kv_heads_4d(
            network, k_4d, num_heads=num_heads, num_kv_heads=kv_heads, head_dim=head_dim
        )
        v_4d = _repeat_kv_heads_4d(
            network, v_4d, num_heads=num_heads, num_kv_heads=kv_heads, head_dim=head_dim
        )
        ...
        scores = network.add_matrix_multiply(
            q_fp32, trt.MatrixOperation.NONE, k_fp32, trt.MatrixOperation.TRANSPOSE
        ).get_output(0)
        scale_t = _scalar_constant_for_trt_dtype(network, (1, 1, 1, 1), scale, trt.float32)
        scores = network.add_elementwise(scores, scale_t, trt.ElementWiseOperation.PROD).get_output(
            0
        )
        if mask_fp32 is not None:
            scores = network.add_elementwise(
                scores, mask_fp32, trt.ElementWiseOperation.SUM
            ).get_output(0)
        probs = network.add_softmax(scores)
        probs.axes = 1 << 3
        ctx_4d = network.add_matrix_multiply(
            probs.get_output(0), trt.MatrixOperation.NONE, v_fp32, trt.MatrixOperation.NONE
        ).get_output(0)
