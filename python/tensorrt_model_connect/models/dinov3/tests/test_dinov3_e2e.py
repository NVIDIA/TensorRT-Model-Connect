...
"""Model-owned E2E entrypoint for the DINOv3 family."""
...
_SPEC = importlib.util.spec_from_file_location(
    f"{Path(__file__).resolve().parent.parent.name}_e2e_runner",
    _RUNNER_PATH,
)
...
def pytest_generate_tests(metafunc):
    if "case_name" in metafunc.fixturenames:
        ...
        if not case_names:
            pytest.skip("No DINOv3 manifests selected", allow_module_level=True)
