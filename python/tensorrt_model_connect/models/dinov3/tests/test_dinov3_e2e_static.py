...
_REPO_ROOT = _MODEL_DIR.parents[4]
_MODELS = {
...
def test_owner_metadata_routes_native_extract_features() -> None:
    with (_MODEL_DIR.parent / "MODEL.toml").open("rb") as stream:
        descriptor = tomllib.load(stream)
    strategy = "dinov3_image_feature_extraction"
    assert descriptor["runtime_strategies"] == [strategy]
    assert runtime_strategy_requires_new_runtime_guard(strategy)
    assert runtime_strategy_task_strategy(strategy) == "image_feature_extraction"
    assert runtime_strategy_cli_commands(strategy) == ("extract-features",)
    assert runtime_strategy_performance_mode(strategy) == "single_pass"
