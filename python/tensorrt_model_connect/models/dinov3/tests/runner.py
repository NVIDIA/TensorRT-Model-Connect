...
"""Model-owned E2E runner for the DINOv3 family."""
...
def _resolve_ld_library_path() -> str:
    ...
    parts = [part for part in (trt_lib_dir, "/usr/local/cuda/lib64", existing) if part]
    return ":".join(parts)
...
def _load_waives(platform: str = "") -> dict[str, tuple[str, str]]:
    ...
    with open(_WAIVES_FILE, encoding="utf-8") as handle:
        for raw_line in handle:
            ...
            selector = parts[0]
            if "/" in selector:
                selected_platform, model_name = selector.split("/", 1)
