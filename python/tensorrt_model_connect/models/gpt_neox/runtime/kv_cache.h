...
// GptNeoxKvCache: autoregressive KV cache state manager.
...
class GptNeoxKvCache : public GptNeoxInferenceState {
  public:
    ...
    // --- GptNeoxInferenceState overrides ---
    void reset() override;
    ...
    // --- GptNeoxKvCache-specific methods (not on the interface) ---
    ...
    // — too big for GptNeoxKvCache's single-row present buffer. The caller reads
