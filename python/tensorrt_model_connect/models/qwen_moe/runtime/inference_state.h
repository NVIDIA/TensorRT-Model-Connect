...
// QwenMoeInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   QwenMoeKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — QwenMoeKvCache + family-owned recurrent state composed
...
class QwenMoeInferenceState {
  public:
    virtual ~QwenMoeInferenceState() = default;
    ...
    // QwenMoeKvCache -> true. Family-owned recurrent state -> false.
