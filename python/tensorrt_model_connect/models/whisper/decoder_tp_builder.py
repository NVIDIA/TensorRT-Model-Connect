...
``model.py`` while making the rank-local TP choices explicit:
...
def _validate_whisper_tp(
    ...
    *,
    ...
    if hidden % tp != 0:
        raise ValueError(
            f"Whisper tensor parallel requires hidden size divisible by tp_size ({hidden} vs {tp})"
        )
    if num_heads % tp != 0:
        raise ValueError(
...
def _add_whisper_tp_decoder_layer(
    *,
    ...
    ca = graph_ops.add_matmul_rhs_constant(
        network, ccf, local_attention_size, hidden_size, weights[f"{prefix}.cross_w_o"], dtype=dtype
    )
...
def build_whisper_tp_decoder_engine(
    ...
    *,
    ...
    if not parallel.enabled:
        raise ValueError(
            "build_whisper_tp_decoder_engine requires tensor_parallel mode with tp_size > 1"
        )
    dec_layers = int(weights["_dec_layers"])
    ...
    embedding_table = graph_ops.add_constant(
        network, (vocab, hidden), rank_weights["dec_embedding"], dtype=work_np_dtype
    )
