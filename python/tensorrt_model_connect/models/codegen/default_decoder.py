...
def build_standard_decoder_engine(config: ModelConfig, weights: WeightDict, max_cache_length: int, *, precision: str='fp32', quant_ctx: QuantContext | None=None, partial_rotary_factor: float=1.0, scale_attn_weights: bool=True, fp32_rope: bool=False, fp32_qk_attention: bool=False, fp32_lm_head: bool=False, embed_input: bool=False, verbose: bool=False, debug_layer_outputs: bool=False, hidden_state_output: bool=False) -> bytes:
    ...
    if not _dual_profile_disabled_for and decoder_engine_role in ('dual_profile', 'prefill'):
        return build_dual_profile_decoder_engine(config, weights, max_cache_length, precision=precision, quant_ctx=quant_ctx, partial_rotary_factor=partial_rotary_factor, scale_attn_weights=scale_attn_weights, fp32_rope=fp32_rope, fp32_qk_attention=fp32_qk_attention, fp32_lm_head=fp32_lm_head, verbose=verbose, profile_mode='prefill' if decoder_engine_role == 'prefill' else 'dual_profile')
    ...
    cos_half_tensor = graph_ops.add_constant(network, cos_half_np.shape, cos_half_np, dtype=rope_np_dtype)
    sin_half_tensor = graph_ops.add_constant(network, sin_half_np.shape, sin_half_np, dtype=rope_np_dtype)
    if not fp32_rope:
    ...
    logits = graph_ops.add_matmul_rhs_constant(network, hidden_state, hidden, out_vocab, weights['w_out'], dtype=lm_head_dtype)
    lm_bias = weights.get('lm_head_bias')
    if lm_bias is not None:
        logits = graph_ops.add_bias_sum(network, logits, out_vocab, lm_bias, dtype=lm_head_dtype)
    else:
        b_out = np.zeros(out_vocab, dtype=lm_head_dtype)
        logits = graph_ops.add_bias_sum(network, logits, out_vocab, b_out, dtype=lm_head_dtype)
...
def _add_decoder_layer(*, network: trt.INetworkDefinition, hidden: trt.ITensor, cache_k: trt.ITensor, cache_v: trt.ITensor, attention_mask: trt.ITensor, position_id: trt.ITensor, attention_scale: float | None, eps_tensor: trt.ITensor, weights: WeightDict, prefix: str, hidden_size: int, attention_size: int, kv_attention_size: int, mlp_size: int, num_heads: int, num_kv_heads: int, head_dim: int, max_cache_length: int, alibi_slopes_tensor: trt.ITensor | None=None, alibi_indices_tensor: trt.ITensor | None=None, dtype: np.dtype=np.float32, quant_ctx: QuantContext | None=None, cos_half_tensor: trt.ITensor | None=None, sin_half_tensor: trt.ITensor | None=None, rotary_embedding_dim: int=0, fp32_rope: bool=False, fp32_qk_attention: bool=False, ffi_attention_kernel: str | None=None, dynamic_kv_cache: bool=False, eps: float | None=None) -> dict[str, trt.ITensor]:
    ...
    if post_attn_norm_w is not None:
        ...
        norm2 = attn['normed']
    mlp_out = graph_blocks.add_gelu_fc_mlp(network, norm2, weights=weights, prefix=prefix, hidden_size=hidden_size, mlp_size=mlp_size, dtype=dtype, quant_ctx=quant_ctx, layer_prefix=prefix)
