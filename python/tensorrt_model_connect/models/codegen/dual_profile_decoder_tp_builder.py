...
def build_dual_profile_tp_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp16', opt_prefill_length: int=64, max_prefill_length: int | None=None, quant_ctx: 'QuantContext | None'=None, partial_rotary_factor: float=1.0, scale_attn_weights: bool=True, fp32_rope: bool=False, fp32_qk_attention: bool=False, fp32_lm_head: bool=False, verbose: bool=False, dynamic_kv_profile_rows: list[int] | None=None, parallel_config=None) -> bytes:
    ...
    if not parallel.enabled:
        raise ValueError('dual_profile_decoder_tp_builder requires parallel.mode=tensor_parallel and tp_size > 1')
    ...
    cos_half_np = graph_ops.make_rope_table_half_dim(kmax, head_dim, config.rope_theta, True, partial_rotary_factor, interleaved=True)
    sin_half_np = graph_ops.make_rope_table_half_dim(kmax, head_dim, config.rope_theta, False, partial_rotary_factor, interleaved=True)
    if fp32_rope:
    ...
    for layer_idx in range(num_layers):
        ...
        q = graph_ops.add_apply_rope_native(network, q, num_heads, head_dim, cos_half_table, sin_half_table, position_id, rotary_embedding_dim, True, sequence_length=None)
        k = graph_ops.add_apply_rope_native(network, k, num_kv_heads, head_dim, cos_half_table, sin_half_table, position_id, rotary_embedding_dim, True, sequence_length=None)
        ...
        all_v_cat.axis = 0
        context = graph_ops.add_attention_from_rows(network, q, all_k_cat.get_output(0), all_v_cat.get_output(0), num_heads=num_heads, head_dim=head_dim, num_kv_heads=num_kv_heads, q_seq=None, kv_seq=None, causal=False, mask=mask_4d, scale=attn_scale, fp32_qk_accumulation=fp32_qk_attention, tag=f'{prefix}.attn')
        ...
        if fc2_bias is not None:
            mlp_out = graph_ops.add_bias_sum(network, mlp_out, hidden, fc2_bias, dtype=work_np_dtype)
    ...
    logits = graph_ops.add_matmul_rhs_constant(network, last_hidden, hidden, out_vocab, weights['w_out'], dtype=lm_head_dtype)
    ...
    if lm_bias is not None:
        ...
        logits = graph_ops.add_bias_sum(network, logits, out_vocab, zero_bias, dtype=lm_head_dtype)
    ...
    if verbose:
        print(f'[trtmc-build] Building tensor-parallel decoder engine (layers={num_layers}, hidden={hidden}, attn={attention_size}, kv={kv_attention_size}, mlp={mlp_size}, cache={max_cache_length}, opt_prefill={opt_prefill_length}, max_prefill={max_prefill_length}, norm=layernorm, mlp_type=gelu_fc, pos=rope, precision={precision}, tp={parallel.tp_size}, rank={parallel.rank}) ...', file=sys.stderr)
