...
"""CodeGen family model — GPT-J-like with parallel residual + partial RoPE.
...
def build_engine(
    ...
    *,
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="CodeGen tensor-parallel builds")
        if quant_ctx is not None:
            raise ValueError("CodeGen tensor-parallel builds do not support quantization")
        if debug_layer_outputs:
            raise ValueError("CodeGen tensor-parallel builds do not support debug_layer_outputs")
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete codegen bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("codegen does not support context-parallel builds")
    ...
    if parallel.enabled:
        ...
        if dynamic_kv_cache:
            raise NotImplementedError(
                "Tensor-parallel codegen builds do not support dynamic KV cache or TriAttention"
            )
