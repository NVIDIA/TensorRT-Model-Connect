...
"""GPT-2 family model — learned positions + fused QKV + Conv1D weights.
...
def load_weights(model_dir: str, config: ModelConfig, *, precision: str = "fp32") -> WeightDict:
    ...
    pos_embed = _load_tensor(readers, _key(root, "wpe.weight"))
    ...
    for layer_idx in range(num_layers):
        ...
        hf_prefix = _key(root, f"h.{layer_idx}")
    ...
    # Final LayerNorm
    ln_f_weight = _load_tensor(readers, _key(root, "ln_f.weight"))
    ln_f_bias = _load_tensor(readers, _key(root, "ln_f.bias"))
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete gpt2 bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("gpt2 does not support context-parallel builds")
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="gpt2 tensor-parallel builds")

        if quant_ctx is not None:
            raise ValueError("gpt2 tensor-parallel builds do not support quantization")
        if dynamic_kv_cache:
            raise NotImplementedError(
                "Tensor-parallel gpt2 builds do not support dynamic KV cache or TriAttention"
            )
