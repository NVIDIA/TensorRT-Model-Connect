...
// VLPlugin: handles "deepseek_ocr_vision_language" strategy.
...
DeepseekOcrKvCacheNames build_kv_cache_names(const BaseConfig& config) {
    ...
    DeepseekOcrKvCacheNames kv_names;
    for (int32_t i = 0; i < config.num_layers; ++i) {
        kv_names.cache_k.push_back(deepseek_ocr_expand_layer_name(io.cache_k_pattern, i));
        kv_names.cache_v.push_back(deepseek_ocr_expand_layer_name(io.cache_v_pattern, i));
        kv_names.present_k.push_back(deepseek_ocr_expand_layer_name(io.present_k_pattern, i));
        kv_names.present_v.push_back(deepseek_ocr_expand_layer_name(io.present_v_pattern, i));
    }
    return kv_names;
}
