...
static bool normalize_to_chw(const std::vector<unsigned char>& resized, int target_size,
                             const DeepseekOcrPreprocessConfig& config,
...
static DeepseekOcrPreprocessedImage
preprocess_merge_group_chw(const LoadedImage& loaded, const DeepseekOcrPreprocessConfig& config) {
    DeepseekOcrPreprocessedImage result;
...
static DeepseekOcrPreprocessedImage
preprocess_simple_chw(const LoadedImage& loaded, const DeepseekOcrPreprocessConfig& config) {
    DeepseekOcrPreprocessedImage result;
    ImageTransformParams params;
...
static DeepseekOcrPreprocessedImage
preprocess_patchify_chw(const LoadedImage& loaded, const DeepseekOcrPreprocessConfig& config) {
    DeepseekOcrPreprocessedImage result;
...
using PreprocessImageFn = DeepseekOcrPreprocessedImage (*)(
    const LoadedImage& loaded, const DeepseekOcrPreprocessConfig& config);
...
run_preprocess_dispatch(const runtime::adapters::io::DecodedImage& image,
    ...
    if (!loaded.ok) {
        return DeepseekOcrPreprocessedImage{};
    }
    return dispatch.preprocess_fn(loaded, config);
}

runtime::adapters::io::DecodedImage deepseek_ocr_decode_image_rgb(const std::string& image_path) {
...
DeepseekOcrPreprocessedImage
deepseek_ocr_preprocess_decoded_image(const runtime::adapters::io::DecodedImage& image,
                                      const DeepseekOcrPreprocessConfig& config) {
...
DeepseekOcrPreprocessedImage
deepseek_ocr_load_and_preprocess_image(const std::string& image_path,
                                       const DeepseekOcrPreprocessConfig& config) {
    return deepseek_ocr_preprocess_decoded_image(deepseek_ocr_decode_image_rgb(image_path), config);
}

std::string deepseek_ocr_format_prompt(const std::string& user_prompt,
                                       const DeepseekOcrPreprocessConfig& config) {
...
static void parse_base_vl_config(const std::string& config_text, DeepseekOcrPreprocessConfig& cfg) {
...
static void maybe_apply_resample_fallback(const std::string& config_text,
                                          ...
                                          DeepseekOcrPreprocessConfig& cfg) {
...
DeepseekOcrPreprocessConfig
deepseek_ocr_parse_preprocess_config(const std::string& config_text,
                                     const std::string& preprocessor_config_text) {
    DeepseekOcrPreprocessConfig cfg;
