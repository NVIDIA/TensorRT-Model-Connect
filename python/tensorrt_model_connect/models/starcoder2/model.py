...
"""StarCoder2 family model — LayerNorm + GELU FC + RoPE.
    ...
    *,
    ...
    if parallel.enabled:
        ...
        if debug_layer_outputs:
            raise ValueError("StarCoder2 tensor-parallel builds do not support debug_layer_outputs")
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete starcoder2 bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("starcoder2 does not support context-parallel builds")
    ...
    if parallel.enabled:
        ...
        if dynamic_kv_cache:
            raise NotImplementedError(
                "Tensor-parallel starcoder2 builds do not support dynamic KV cache or TriAttention"
            )
    ...
    tokenizer_override = tokenizer_json_bundle_override(model_path)
    for filename in (
        "tokenizer.json",
