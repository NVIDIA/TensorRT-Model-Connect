...
def build_dual_profile_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp16', opt_prefill_length: int=64, max_prefill_length: int | None=None, quant_ctx: 'QuantContext | None'=None, partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, attention_layer_types: Sequence[str] | None=None, local_attention_window: int=0, verbose: bool=False, dynamic_kv_profile_rows: list[int] | None=None, profile_mode: str='dual_profile') -> bytes:
    ...
    * ``"prefill"``: one prefill profile only. This is used by split-engine
    ...
    if verbose:
        ...
        print(f'[trtmc build] Building {mode_label} engine (layers={num_layers}, hidden={hidden}, attn={attention_size}, kv={kv_attention_size}, mlp={mlp_size}, cache={max_cache_length}, opt_prefill={opt_prefill_length}, max_prefill={max_prefill_length}, norm=layernorm, mlp_type=gelu_fc, pos=learned, precision={precision}) ...', file=sys.stderr)
