...
def build_dual_profile_tp_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp16', opt_prefill_length: int=64, max_prefill_length: int | None=None, quant_ctx: 'QuantContext | None'=None, partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, attention_layer_types: Sequence[str] | None=None, local_attention_window: int=0, verbose: bool=False, dynamic_kv_profile_rows: list[int] | None=None, parallel_config=None) -> bytes:
    ...
    if not parallel.enabled:
        raise ValueError('default_dual_profile_decoder_tp requires parallel.mode=tensor_parallel and tp_size > 1')
    _validate_tp_quantization(quant_ctx)
    if 'gelu_fc' not in {'swiglu', 'gelu_fc'}:
        raise NotImplementedError('Tensor-parallel decoder builds currently support SwiGLU and GeluFC MLPs only')
    ...
    if attention_layer_types and 'local' in attention_layer_types and (local_attention_window > 0):
        local_mask_2d = graph_ops.add_local_attention_mask_2d(network, attention_mask_work, position_id, max_cache_length=max_cache_length, window_size=local_attention_window)
    ...
    for layer_idx in range(num_layers):
        ...
        layer_mask_4d = local_mask_4d if local_mask_4d is not None and layer_idx < len(attention_layer_types or ()) and (attention_layer_types[layer_idx] == 'local') else mask_4d
        ...
        all_v_cat.axis = 0
        context = graph_ops.add_attention_from_rows(network, q, all_k_cat.get_output(0), all_v_cat.get_output(0), num_heads=num_heads, head_dim=head_dim, num_kv_heads=num_kv_heads, q_seq=None, kv_seq=None, causal=False, mask=layer_mask_4d, scale=attn_scale, tag=f'{prefix}.attn')
    ...
    if verbose:
        print(f'[trtmc-build] Building tensor-parallel decoder engine (layers={num_layers}, hidden={hidden}, attn={attention_size}, kv={kv_attention_size}, mlp={mlp_size}, cache={max_cache_length}, opt_prefill={opt_prefill_length}, max_prefill={max_prefill_length}, norm=layernorm, mlp_type=gelu_fc, pos=learned, precision={precision}, tp={parallel.tp_size}, rank={parallel.rank}) ...', file=sys.stderr)
