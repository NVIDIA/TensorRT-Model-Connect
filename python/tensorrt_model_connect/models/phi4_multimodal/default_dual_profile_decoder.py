    ...
    * ``"prefill"``: one prefill profile only. This is used by split-engine
    ...
    if position_type == 'rope':
        ...
        if rope_scaling.get('type') == 'longrope':
            original_max = int(config.raw.get('original_max_position_embeddings', config.raw.get('max_position_embeddings', kmax)))
        ...
        cos_half_np = graph_ops.make_rope_table_half_dim(kmax, head_dim, config.rope_theta, True, partial_rotary_factor, interleaved=interleaved_rope, frequency_factors=rope_frequency_factors, attention_factor=rope_attention_factor)
        sin_half_np = graph_ops.make_rope_table_half_dim(kmax, head_dim, config.rope_theta, False, partial_rotary_factor, interleaved=interleaved_rope, frequency_factors=rope_frequency_factors, attention_factor=rope_attention_factor)
    ...
    for layer_idx in range(num_layers):
        ...
        all_v_cat.axis = 0
        context = graph_ops.add_attention_from_rows(network, q, all_k_cat.get_output(0), all_v_cat.get_output(0), num_heads=num_heads, head_dim=head_dim, num_kv_heads=num_kv_heads, q_seq=None, kv_seq=None, causal=False, mask=mask_4d, scale=attn_scale, tag=f'{prefix}.attn', explicit_attention=bool(weights.get('_explicit_attention', False)))
