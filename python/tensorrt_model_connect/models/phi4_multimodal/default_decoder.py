...
def build_standard_decoder_engine(config: ModelConfig, weights: WeightDict, max_cache_length: int, *, precision: str='fp32', quant_ctx: QuantContext | None=None, norm_type: str='rmsnorm', mlp_type: str='swiglu', position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, verbose: bool=False, debug_layer_outputs: bool=False, hidden_state_output: bool=False) -> bytes:
    ...
    _dual_profile_disabled_for = debug_layer_outputs or hidden_state_output or bool(requested_fp32_layers) or bool(config.raw.get('dynamic_kv_cache', False)) or (_os.environ.get('TRTMC_NO_DUAL_PROFILE') == '1')
    ...
    if position_type == 'rope':
        ...
        if rope_scaling.get('type') == 'longrope':
            original_max = int(config.raw.get('original_max_position_embeddings', config.raw.get('max_position_embeddings', attention_window)))
            max_positions = int(config.raw.get('max_position_embeddings', original_max))
            extension_factor = max_positions / max(original_max, 1)
            rope_attention_factor = float(rope_scaling.get('attention_factor', 1.0 if extension_factor <= 1.0 else np.sqrt(1.0 + np.log(extension_factor) / np.log(original_max))))
            factor_key = 'long_factor' if attention_window > original_max else 'short_factor'
            rope_frequency_factors = rope_scaling.get(factor_key)
        cos_half_np = graph_ops.make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, True, partial_rotary_factor, interleaved=interleaved_rope, frequency_factors=rope_frequency_factors, attention_factor=rope_attention_factor)
        sin_half_np = graph_ops.make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, False, partial_rotary_factor, interleaved=interleaved_rope, frequency_factors=rope_frequency_factors, attention_factor=rope_attention_factor)
