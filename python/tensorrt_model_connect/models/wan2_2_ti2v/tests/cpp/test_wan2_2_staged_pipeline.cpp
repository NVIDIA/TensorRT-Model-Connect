...
trtmc::TrtUniquePtr<nvinfer1::IHostMemory> build_identity_plan(bool dynamic) {
    ...
    const nvinfer1::Dims input_dims = dynamic ? nvinfer1::Dims2{-1, 4} : nvinfer1::Dims{1, {4}};
    auto* input = network->addInput("x", nvinfer1::DataType::kFLOAT, input_dims);
    if (!input)
        return nullptr;
    if (dynamic) {
    ...
    if (!identity)
        return nullptr;
    identity->getOutput(0)->setName("y");
    network->markOutput(*identity->getOutput(0));
    return trtmc::TrtUniquePtr<nvinfer1::IHostMemory>(
        builder->buildSerializedNetwork(*network, *config));
}

bool test_backend_prebinding_contract() {
    auto static_plan = build_identity_plan(false);
    auto dynamic_plan = build_identity_plan(true);
    std::unique_ptr<trtmc::IBackend, decltype(&trtmc_destroy_backend)> backend(
        trtmc_create_backend(), trtmc_destroy_backend);
    auto* prebound = dynamic_cast<trtmc::IPreboundBackend*>(backend.get());
    if (!static_plan || !dynamic_plan || prebound == nullptr) {
        std::cerr << "FAIL: could not create the TensorRT prebinding test fixture\n";
        return false;
    }

    void* input_buffer = nullptr;
    void* output_buffer = nullptr;
    if (cudaMalloc(&input_buffer, 4 * sizeof(float)) != cudaSuccess ||
        cudaMalloc(&output_buffer, 4 * sizeof(float)) != cudaSuccess) {
        cudaFree(input_buffer);
        cudaFree(output_buffer);
        std::cerr << "FAIL: could not allocate the TensorRT prebinding test buffers\n";
        return false;
    }

    bool ok = true;
    try {
        auto module = prebound->create_module_prebound(
            static_plan->data(), static_plan->size(), {},
            {{"x", input_buffer, 4 * sizeof(float)}, {"y", output_buffer, 4 * sizeof(float)}});
        if (!module || module->device_ptr("x") != input_buffer ||
            module->device_ptr("y") != output_buffer) {
            throw std::runtime_error("external addresses were not installed");
        }
        float input[4] = {1.0F, 2.0F, 3.0F, 4.0F};
        module->forward_async({{"x", {input, {4}, trtmc::DType::kFloat32}}});
        module->sync();
        float output[4] = {};
        if (cudaMemcpy(output, output_buffer, sizeof(output), cudaMemcpyDeviceToHost) !=
                cudaSuccess ||
    ...
    const auto rejects = [prebound](const nvinfer1::IHostMemory& plan,
                                    std::vector<trtmc::ModuleExternalBinding> bindings) {
        try {
            (void)prebound->create_module_prebound(plan.data(), plan.size(), {}, bindings);
            return false;
        } catch (const std::exception&) {
            return true;
        }
    };
    ok = ok && rejects(*static_plan, {{"", input_buffer, 4 * sizeof(float)}}) &&
         rejects(*static_plan, {{"x", nullptr, 4 * sizeof(float)}}) &&
         rejects(*static_plan, {{"missing", input_buffer, 4 * sizeof(float)}}) &&
         rejects(*static_plan, {{"x", input_buffer, 4 * sizeof(float)},
                                {"x", input_buffer, 4 * sizeof(float)}}) &&
         rejects(*static_plan, {{"x", input_buffer, 4 * sizeof(float) - 1}}) &&
         rejects(*dynamic_plan, {{"x", input_buffer, 3 * 4 * sizeof(float)}});
    if (!ok)
        std::cerr << "FAIL: TensorRT prebinding accepted an invalid contract\n";
...
bool test_vae_cache_prebindings() {
    ...
    const auto shape = trtmc::make_wan22_runtime_shape(trtmc::Wan22TI2VRequest{});
    const auto bindings = trtmc::make_wan22_vae_cache_bindings(inputs, outputs, shape);
    ...
    try {
        ...
        (void)trtmc::make_wan22_vae_cache_bindings(short_inputs, outputs, shape);
    ...
    try {
        ...
        (void)trtmc::make_wan22_vae_cache_bindings(inputs, null_outputs, shape);
...
bool test_runtime_shapes_and_l0_cache_prebindings() {
    ...
    if (official.latent_frames != 31 || official.latent_height != 44 ||
        official.latent_width != 80 || official.video_frames != 121 ||
        official.video_height != 704 || official.video_width != 1280 ||
        official.latent_count != 5237760U) {
    ...
    if (l0.latent_frames != 2 || l0.latent_height != 24 || l0.latent_width != 42 ||
        l0.video_frames != 5 || l0.video_height != 384 || l0.video_width != 672 ||
        l0.latent_count != 96768U) {
...
int main() {
    if (!test_backend_prebinding_contract())
        return 1;
