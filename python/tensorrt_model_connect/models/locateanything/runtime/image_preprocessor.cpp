...
static LocateAnythingPreprocessedImage
preprocess_merge_group_chw(const LoadedImage& loaded,
                           const LocateAnythingPreprocessConfig& config) {
    LocateAnythingPreprocessedImage result;
...
static LocateAnythingPreprocessedImage
preprocess_simple_chw(const LoadedImage& loaded, const LocateAnythingPreprocessConfig& config) {
    LocateAnythingPreprocessedImage result;
    ImageTransformParams params;
...
static LocateAnythingPreprocessedImage
preprocess_patchify_chw(const LoadedImage& loaded, const LocateAnythingPreprocessConfig& config) {
    LocateAnythingPreprocessedImage result;
...
using PreprocessImageFn = LocateAnythingPreprocessedImage (*)(
    const LoadedImage& loaded, const LocateAnythingPreprocessConfig& config);
...
run_preprocess_dispatch(const runtime::adapters::io::DecodedImage& image,
    ...
    if (!loaded.ok) {
        return LocateAnythingPreprocessedImage{};
    }
    return dispatch.preprocess_fn(loaded, config);
}

runtime::adapters::io::DecodedImage locateanything_decode_image_rgb(const std::string& image_path) {
...
LocateAnythingPreprocessedImage
locateanything_load_and_preprocess_image(const std::string& image_path,
                                         const LocateAnythingPreprocessConfig& config) {
    return locateanything_preprocess_decoded_image(locateanything_decode_image_rgb(image_path),
                                                   config);
}
...
static void parse_base_vl_config(const std::string& config_text,
                                 LocateAnythingPreprocessConfig& cfg) {
