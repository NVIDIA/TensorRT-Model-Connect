...
void test_retired_diffusion_strategy_is_rejected() {
    trtmc_test::TempDirGuard dir;
    const std::filesystem::path bundle_path =
        std::filesystem::path(dir.path()) / "wan_retired_strategy.bundle";
  ...
  "scheduler": "flow_match_euler"
    ...
    check(pipeline == nullptr, "retired diffusion strategy returns nullptr");

    const char* err = trtmc_last_error();
    check(err != nullptr && std::strlen(err) > 0, "trtmc_last_error set for wan guard path");
    if (err != nullptr) {
        check(std::string(err).find("No plugin registered for runtime_strategy: diffusion") !=
                  std::string::npos,
              "retired diffusion strategy fails closed before model loading");
    }
}
...
int main() {
    test_retired_diffusion_strategy_is_rejected();
