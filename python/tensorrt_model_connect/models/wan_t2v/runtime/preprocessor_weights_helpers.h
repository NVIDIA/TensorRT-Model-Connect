...
namespace trtmc {
namespace wan_preprocessor_weights {
...
inline bool extract_preprocessor_index(const std::vector<char>& data, std::string& index_json,
    ...
    if (data.size() < 4) {
        std::cerr << "[wan] preprocessor_weights section too small\n";
        return false;
    }
    ...
    if (4 + index_len > data.size()) {
        std::cerr << "[wan] preprocessor_weights index length overflow\n";
        return false;
    }
...
inline bool load_preprocessor_floats(const std::string& index_json, const char* blob,
    ...
    if (offset + nbytes > blob_size) {
        std::cerr << "[wan] weight " << key << " overflows blob\n";
        return false;
    }
