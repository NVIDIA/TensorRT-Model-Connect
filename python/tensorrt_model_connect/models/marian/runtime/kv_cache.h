...
// MarianKvCache: autoregressive KV cache state manager.
...
class MarianKvCache : public MarianInferenceState {
  public:
    ...
    MarianKvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
                  DType cache_dtype = DType::kFloat32, MarianKvCacheNames names = {});

    // --- MarianInferenceState overrides ---
    void reset() override;
    ...
    // --- MarianKvCache-specific methods (not on the interface) ---

    // Build the dense decoder mask consumed by this encoder-decoder pipeline.
    ...
    // — too big for MarianKvCache's single-row present buffer. The caller reads
