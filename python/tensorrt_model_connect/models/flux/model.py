...
def _flux2_text_seq_len(config: ModelConfig) -> int:
    text_seq_len = int(
        config.raw.get(
            "text_seq_len",
            config.raw.get("max_cache_length", _FLUX2_TEXT_SEQ_LEN),
        )
    )
    return max(1, min(text_seq_len, _FLUX2_TEXT_SEQ_LEN))
...
def build_components(
    ...
    *,
    ...
    if _is_flux2(tc):
        ...
        return _build_flux2_components(
            model_dir,
            config,
            weights,
            tc=tc,
    ...
    return _build_flux1_components(
        model_dir,
        config,
        weights,
        tc=tc,
...
def _build_flux1_components(
    ...
    *,
    ...
    require_tensorrt_11_for_distributed(parallel, feature="Flux distributed builds")
    ...
    dit_dim = tc.get("num_attention_heads", _DIT_NUM_HEADS) * tc.get(
        "attention_head_dim", _DIT_HEAD_DIM
    )
    num_heads = tc.get("num_attention_heads", _DIT_NUM_HEADS)
    num_layers = tc.get("num_layers", _DIT_NUM_LAYERS)
    num_single_layers = tc.get("num_single_layers", _DIT_NUM_SINGLE_LAYERS)
    tc.get("in_channels", _DIT_IN_CHANNELS)
    tc.get("patch_size", _DIT_PATCH_SIZE)
    tc.get("joint_attention_dim", _DIT_JOINT_ATTN_DIM)
    tc.get("pooled_projection_dim", _DIT_POOLED_PROJ_DIM)
    guidance_embeds = tc.get("guidance_embeds", False)
    tuple(tc.get("axes_dims_rope", _AXES_DIMS_ROPE))
            ...
            if "CLIP" in arch or clip_cfg.get("model_type") == "clip_text_model":
                ...
                with timed_weight_loading(build_timing, "clip_encoder"):
                    clip_weights = load_clip_weights(
                        clip_dir,
                        hidden_size=clip_cfg.get("hidden_size", _CLIP_HIDDEN),
                        num_layers=clip_cfg.get("num_hidden_layers", _CLIP_LAYERS),
                    )
                with timed_trt_compile(build_timing, "clip_encoder"):
                    clip_plan = build_clip_encoder_engine(
                        clip_weights,
                        hidden_size=clip_cfg.get("hidden_size", _CLIP_HIDDEN),
                        num_heads=clip_cfg.get("num_attention_heads", _CLIP_HEADS),
                        intermediate_size=clip_cfg.get("intermediate_size", _CLIP_INTERMEDIATE),
                        num_layers=clip_cfg.get("num_hidden_layers", _CLIP_LAYERS),
                        vocab_size=clip_cfg.get("vocab_size", _CLIP_VOCAB),
                        max_seq_len=clip_cfg.get("max_position_embeddings", _CLIP_MAX_SEQ),
                        verbose=verbose,
                    )
                ...
                with timed_weight_loading(build_timing, "t5_encoder"):
                    t5_weights = load_t5_weights(
                        clip_dir,
                        d_model=clip_cfg.get("d_model", _T5_D_MODEL),
                        num_heads=clip_cfg.get("num_heads", _T5_NUM_HEADS),
                        d_kv=clip_cfg.get("d_kv", _T5_D_KV),
                        d_ff=clip_cfg.get("d_ff", _T5_D_FF),
                        num_layers=clip_cfg.get("num_layers", _T5_NUM_LAYERS),
                        vocab_size=clip_cfg.get("vocab_size", _T5_VOCAB_SIZE),
                        precision=precision,
                    )
    ...
    with timed_trt_compile(build_timing, "flux_dit"):
        if parallel.cp_enabled:
            from .flux_dit_cp_builder import build_flux_dit_engine as build_flux_dit_cp_engine
...
def _build_flux2_components(
    ...
    *,
    ...
    vae_mbs = 1
    from ...build_timing import timed_trt_compile, timed_weight_loading
    from .mistral_encoder_builder import build_mistral_encoder_engine, load_mistral_encoder_weights
    ...
    dit_dim = tc.get("num_attention_heads", _FLUX2_DIT_NUM_HEADS) * tc.get(
        "attention_head_dim", _DIT_HEAD_DIM
    )
    num_heads = tc.get("num_attention_heads", _FLUX2_DIT_NUM_HEADS)
    num_layers = tc.get("num_layers", _FLUX2_DIT_NUM_LAYERS)
    num_single_layers = tc.get("num_single_layers", _FLUX2_DIT_NUM_SINGLE_LAYERS)
    mlp_ratio = tc.get("mlp_ratio", _FLUX2_DIT_MLP_RATIO)
    tuple(tc.get("axes_dims_rope", _FLUX2_AXES_DIMS_ROPE))

    # VAE params
    vc = weights.get("_vae_config", {})
    vae_latent_channels = vc.get("latent_channels", _FLUX2_VAE_LATENT_CHANNELS)
    vc.get("scaling_factor", _VAE_SCALING_FACTOR)
    vc.get("shift_factor", _VAE_SHIFT_FACTOR)
    ...
    pack_size = 2
    num_img_tokens = (h_lat // pack_size) * (w_lat // pack_size)
    text_seq_len = _flux2_text_seq_len(config)
    ...
    m_hidden = _MISTRAL_HIDDEN
    m_heads = _MISTRAL_NUM_HEADS
    m_kv_heads = _MISTRAL_NUM_KV_HEADS
    m_head_dim = _MISTRAL_HEAD_DIM
    m_intermediate = _MISTRAL_INTERMEDIATE
    m_num_layers = _MISTRAL_NUM_LAYERS
    m_vocab = 131072
    m_extract = _MISTRAL_EXTRACT_LAYERS
    ...
    if te_dir:
        ...
        m_hidden = tc_text.get("hidden_size", _MISTRAL_HIDDEN)
        m_heads = tc_text.get("num_attention_heads", _MISTRAL_NUM_HEADS)
        m_kv_heads = tc_text.get("num_key_value_heads", _MISTRAL_NUM_KV_HEADS)
        m_head_dim = tc_text.get("head_dim", m_hidden // m_heads)
        m_intermediate = tc_text.get("intermediate_size", _MISTRAL_INTERMEDIATE)
        m_num_layers = tc_text.get("num_hidden_layers", _MISTRAL_NUM_LAYERS)
    ...
    _cast_dtype = "bf16" if fp8_scales is not None else ("bf16" if precision == "bf16" else "fp16")
    ...
    text_encoder_dim = len(_MISTRAL_EXTRACT_LAYERS) * m_hidden
    ...
    if te_dir:
        print(
            f"[flux] Loading Mistral 3 encoder ({m_hidden}d, {m_num_layers}L) ...", file=sys.stderr
        )
            ...
            for _f in _vae_st_files:
                with _safe_open(str(_f), framework="numpy") as _r:
                    if "bn.running_mean" in _r.keys():
                        _vae_weights["bn.running_mean"] = _r.get_tensor("bn.running_mean").astype(
                            _np.float32
                        )
    ...
    # 5. Serialize preprocessor weights
    preprocessor_weights = _serialize_flux2_preprocessor(dit_weights, vae_bn_weights=_vae_weights)
...
def diffusion_bundle_sections(components: dict, *, parallel_config=None) -> list[tuple[str, bytes]]:
    ...
    if parallel.cp_enabled:
        ...
        for rank in range(parallel.tp_size):
            ...
            if plan is None:
                raise ValueError(f"Missing FLUX {parallel.mode} denoiser rank {rank}")
...
def get_diffusion_config(config: ModelConfig) -> dict:
    ...
    if _is_flux2(tc):
        return _get_flux2_diffusion_config(config, tc)

    return _get_flux1_diffusion_config(config, tc)


def _get_flux1_diffusion_config(config: ModelConfig, tc: dict) -> dict:
    ...
    return {
        ...
        "num_inference_steps": config.raw.get("num_inference_steps", 28 if guidance_embeds else 4),
        ...
        "video_width": img_w,
        "video_num_frames": 1,
        "dit_dim": tc.get("num_attention_heads", _DIT_NUM_HEADS)
        * tc.get("attention_head_dim", _DIT_HEAD_DIM),
        "dit_num_heads": tc.get("num_attention_heads", _DIT_NUM_HEADS),
        "dit_num_layers": tc.get("num_layers", _DIT_NUM_LAYERS),
        ...
        "vae_shift_factor": _VAE_SHIFT_FACTOR,
        "guidance_embeds": 1 if guidance_embeds else 0,
        "axes_dims_rope": list(tc.get("axes_dims_rope", _AXES_DIMS_ROPE)),
        "num_vae_caches": 0,
        "vae_model_id": "sayakpaul/FLUX.1-merged",
    }


def _get_flux2_diffusion_config(config: ModelConfig, tc: dict) -> dict:
    ...
    vae_latent_ch = vc.get("latent_channels", _FLUX2_VAE_LATENT_CHANNELS)
    vae_scaling = vc.get("scaling_factor", _VAE_SCALING_FACTOR)
    vae_shift = vc.get("shift_factor", _VAE_SHIFT_FACTOR)

    # Mistral encoder output dimension: extract_layers * hidden_size
    text_encoder_dim = len(_MISTRAL_EXTRACT_LAYERS) * _MISTRAL_HIDDEN
    ...
    return {
        ...
        "video_width": img_w,
        "video_num_frames": 1,
        "dit_dim": tc.get("num_attention_heads", _FLUX2_DIT_NUM_HEADS)
        * tc.get("attention_head_dim", _DIT_HEAD_DIM),
        "dit_num_heads": tc.get("num_attention_heads", _FLUX2_DIT_NUM_HEADS),
        "dit_num_layers": tc.get("num_layers", _FLUX2_DIT_NUM_LAYERS),
        "patch_size": [1, 2, 2],
        "z_dim": vae_latent_ch,
        "scale_factor_temporal": 1,
        "scale_factor_spatial": 8,
        ...
        "axes_dims_rope": list(tc.get("axes_dims_rope", _FLUX2_AXES_DIMS_ROPE)),
...
def fp8_calibrate(
    ...
    text_seq = _flux2_text_seq_len(config)
    packed_ch = 32 * 4  # z_dim * 2x2 patch
    # Encoder dim = Mistral hidden * num_extract_layers (5120 * 3 = 15360)
    encoder_dim = tc.get("joint_attention_dim", _MISTRAL_HIDDEN * len(_MISTRAL_EXTRACT_LAYERS))
    ...
    def calibration_loop(m: torch.nn.Module) -> None:
        ...
        for t in timesteps:
            for _ in range(4):
                ...
                if done % 4 == 0:
                    print(f"  [fp8-calibrate] {done}/{total} (t={t.item():.0f})", file=sys.stderr)
...
def build(model_dir: str, output_path: str, **options) -> None:
    ...
    if fp8_scales == "auto":
        ...
        fp8_scales = fp8_calibrate(str(model_path), config)
        add_build_timing(
            timing,
            "fp8_calibration_s",
