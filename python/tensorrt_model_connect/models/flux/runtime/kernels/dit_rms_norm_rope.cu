...
namespace trtmc::models::flux::kernels {
...
//   "trtmc.flux_dit_rms_norm_rope_bf16" → __nv_bfloat16
//   "trtmc.flux_dit_rms_norm_rope_fp16" → __half
    ...
    TVMFFIAny const* args, int32_t num_args, TVMFFIAny* result)
{
    ...
    trtmc::models::flux::kernels::launchPerTensorRMSNormRope<T>(
...
struct _AutoRegister {
    _AutoRegister() {
        register_global_func("trtmc.flux_dit_rms_norm_rope_bf16",
                             trtmc_dit_rms_norm_rope_bf16_impl);
        register_global_func("trtmc.flux_dit_rms_norm_rope_fp16",
