...
void load_preprocessor_weights(const std::string& index_json, const char* blob,
    ...
    flux_preprocessor_weights::load_with_fallback(index_json, blob, blob_size,
                                                  "patch_embedding.weight", "x_embedder.weight",
                                                  w.patch_embed_weight);
    flux_preprocessor_weights::load_with_fallback(
...
FluxPreprocessorWeights parse_preprocessor_weights(const std::vector<char>& data) {
    FluxPreprocessorWeights w;
    std::string index_json;
    const char* blob = nullptr;
    std::size_t blob_size = 0;
    if (!flux_preprocessor_weights::extract_preprocessor_index(data, index_json, blob, blob_size)) {
        return w;
    }
    ...
    std::cerr << "[flux] Preprocessor weights loaded: " << (w.valid ? "OK" : "INCOMPLETE")
