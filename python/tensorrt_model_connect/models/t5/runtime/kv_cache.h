...
// T5KvCache: autoregressive KV cache state manager.
...
class T5KvCache : public T5InferenceState {
  public:
    ...
    T5KvCache(int32_t num_layers, int32_t max_length, int32_t kv_dim, cudaStream_t stream,
              DType cache_dtype = DType::kFloat32, T5KvCacheNames names = {});

    // --- T5InferenceState overrides ---
    void reset() override;
    ...
    // --- T5KvCache-specific methods (not on the interface) ---
    ...
    // — too big for T5KvCache's single-row present buffer. The caller reads
