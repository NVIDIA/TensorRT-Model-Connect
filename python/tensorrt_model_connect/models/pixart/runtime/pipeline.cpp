...
#include "pixart_denoising_step_seam.h"
#include "pixart_dpmsolver.h"
#include "pixart_generation_conditioning.h"
#include "pixart_generation_plan.h"
#include "pixart_matmul_policy.h"
...
std::vector<float> prepare_pixart_vae_2d_input(const std::vector<float>& latents,
                                               const PixArtDiffusionConfig& config,
...
bool run_pixart_denoising_loop(
    ...
    return pixart_denoising::run_pixart_video_denoising_steps(
...
void denormalize_pixart_latents(const PixArtDiffusionConfig& config, int32_t z_dim, int32_t t_lat,
...
// ---------------------------------------------------------------------------
// PixArtVideoResult -> ImageResult conversion
// ---------------------------------------------------------------------------

ImageResult video_to_image(const PixArtVideoResult& vr, int32_t default_h, int32_t default_w) {
    ImageResult out;
...
PixArtPipeline::PixArtPipeline(std::unique_ptr<TrtModule> text_encoder,
                               ...
                               PixArtDiffusionConfig config, PixArtPreprocessorWeights weights,
...
bool PixArtPipeline::run_t5_encoder(const std::vector<int32_t>& input_ids,
...
bool PixArtPipeline::run_denoiser(const std::vector<float>& hidden,
...
void PixArtPipeline::compute_timestep_embedding(float timestep, std::vector<float>& temb_6d,
...
void PixArtPipeline::project_text(const std::vector<float>& in, int32_t seq_len,
...
void PixArtPipeline::patchify(const std::vector<float>& latents, int32_t c, int32_t t, int32_t h,
...
void PixArtPipeline::unpatchify(const std::vector<float>& patches, int32_t c, int32_t t, int32_t h,
...
void PixArtPipeline::compute_3d_rope(int32_t nt, int32_t nh, int32_t nw,
...
bool PixArtPipeline::decode_vae_2d(const std::vector<float>& latents, int32_t c, int32_t h_lat,
                                   int32_t w_lat, PixArtVideoResult& result) {
    if (!vae_ || !vae_->ok()) {
        return false;
    }
...
int32_t PixArtPipeline::query_vae_output_temporal_dim() const {
...
void PixArtPipeline::init_vae_caches() {
...
void PixArtPipeline::zero_vae_caches() {
...
void PixArtPipeline::decode_vae_single_frame(const std::vector<float>& latents, int32_t c,
...
bool PixArtPipeline::decode_vae_3d(const std::vector<float>& latents, int32_t c, int32_t t_lat,
                                   int32_t h_lat, int32_t w_lat, PixArtVideoResult& result) {
    if (!vae_ || !vae_->ok()) {
        return false;
    }
...
bool PixArtPipeline::run_pixart_text_conditioning(const std::vector<int32_t>& input_ids,
                                                  int32_t seq_len,
...
bool PixArtPipeline::run_pixart_vae_decode(int32_t z_dim, int32_t t_lat, int32_t h_lat,
...
ImageResult PixArtPipeline::generate_image(const std::string& prompt, const GenerateConfig& cfg) {
    // Tokenize prompt
    ...
    if (!run_pixart_text_conditioning(input_ids, layout.seq_len, text_projected, null_text,
                                      error)) {
