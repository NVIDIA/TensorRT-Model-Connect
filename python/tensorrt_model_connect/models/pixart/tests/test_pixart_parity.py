...
from tensorrt_model_connect.models.pixart.tests.e2e_plugins.runners import diffusion
...
def test_hf_and_trtmc_resolve_the_same_initial_latent(tmp_path) -> None:
    from tensorrt_model_connect.models.pixart.tests.e2e_plugins.parity import ensure_initial_latents
...
def test_trtmc_runner_consumes_and_reports_shared_initial_latent(
    ...
    ctx = RunContext(
        ...
        model_plugin_dir="/model-plugins/pixart",
    )
    monkeypatch.setattr(
    ...
    assert command[command.index("--model-plugin-dir") + 1] == "/model-plugins/pixart"
