...
// LlamaInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   LlamaKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — LlamaKvCache + family-owned recurrent state composed
...
class LlamaInferenceState {
  public:
    virtual ~LlamaInferenceState() = default;
    ...
    // LlamaKvCache -> true. Family-owned recurrent state -> false.
