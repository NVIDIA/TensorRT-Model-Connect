...
// GraniteInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   GraniteKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — GraniteKvCache + family-owned recurrent state composed
...
class GraniteInferenceState {
  public:
    virtual ~GraniteInferenceState() = default;
    ...
    // GraniteKvCache -> true. Family-owned recurrent state -> false.
