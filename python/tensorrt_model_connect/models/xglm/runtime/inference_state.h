...
// XglmInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   XglmKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — XglmKvCache + family-owned recurrent state composed
...
class XglmInferenceState {
  public:
    virtual ~XglmInferenceState() = default;
    ...
    // XglmKvCache -> true. Family-owned recurrent state -> false.
