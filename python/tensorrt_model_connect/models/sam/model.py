...
"""SAM (Segment Anything Model) family model -- prompted segmentation.
...
def load_weights(model_dir: str, config: ModelConfig) -> WeightDict:
    ...
    weights["prompt.no_mask_embed"] = (
        _load_tensor(readers, "prompt_encoder.no_mask_embed.weight").flatten().astype(np.float32)
    )
    ...
    for i in range(3):
        ...
        if _has_tensor(readers, f"{hf_conv}.weight"):
            ...
            weights[f"prompt.mask_down.{i}.bias"] = _load_tensor(readers, f"{hf_conv}.bias").astype(
                np.float32
            )
    ...
    for layer_idx in range(decoder_depth):
        ...
        for proj in ("q", "k", "v"):
            proj_name = {"q": "q_proj", "k": "k_proj", "v": "v_proj"}[proj]
            w = _load_tensor(readers, f"{hf_prefix}.cross_attn_token_to_image.{proj_name}.weight")
        ...
        t2i_o_w = _load_tensor(readers, f"{hf_prefix}.cross_attn_token_to_image.out_proj.weight")
        ...
        for proj in ("q", "k", "v"):
            proj_name = {"q": "q_proj", "k": "k_proj", "v": "v_proj"}[proj]
            w = _load_tensor(readers, f"{hf_prefix}.cross_attn_image_to_token.{proj_name}.weight")
        ...
        i2t_o_w = _load_tensor(readers, f"{hf_prefix}.cross_attn_image_to_token.out_proj.weight")
    ...
    *,
    ...
    for layer_idx in range(num_layers):
        ...
        normed = graph_ops.add_layer_norm(
            network, reshape_2d.get_output(0), hidden, norm1_w, norm1_b, eps_t, dtype=work_np_dtype
        )
        ...
        if use_window:
            ...
            attn_out_4d = _build_windowed_attention(
                network,
            ...
            # Global attention
            attn_out_4d = _build_global_attention(
                network,
                normed_4d.get_output(0),
                weights,
                w_prefix,
    ...
    neck_c1_b = weights.get("encoder.neck.conv1.bias", np.zeros(decoder_hidden, dtype=np.float32))
    ...
    neck_c2_b = weights.get("encoder.neck.conv2.bias", np.zeros(decoder_hidden, dtype=np.float32))
...
def build_vision_engine(
    ...
    *,
    ...
    B = 2.0 * np.pi * B
    image_pe_flat = np.concatenate([np.sin(B), np.cos(B)], axis=-1).astype(np.float32)  # [H*W, 256]
    ...
    no_mask_c = graph_ops.add_constant(network, no_mask_4d.shape, no_mask_4d, dtype=work_np_dtype)
    ...
    for layer_idx in range(decoder_depth):
        ...
        mlp_out = graph_ops.add_matmul_rhs_constant(
            network, queries, decoder_hidden, decoder_mlp_dim, weights[f"{w_prefix}.mlp.fc1.weight"]
        )
        ...
        mlp_out = graph_ops.add_matmul_rhs_constant(
            network, mlp_out, decoder_mlp_dim, decoder_hidden, weights[f"{w_prefix}.mlp.fc2.weight"]
        )
        mlp_out = graph_ops.add_bias_sum(
        ...
        keys = network.add_elementwise(keys, i2t_out, trt.ElementWiseOperation.SUM).get_output(0)
    ...
    queries = network.add_elementwise(queries, final_t2i, trt.ElementWiseOperation.SUM).get_output(
        0
    )
    queries = graph_ops.add_layer_norm(
        network,
    ...
    # GELU activation (HF: activation = nn.GELU())
...
def _build_windowed_attention(
    ...
    if f"{w_prefix}.attn.rel_pos_h" in weights:
        ...
        rp_h = _get_rel_pos(grid_size, grid_size, rel_pos_h)  # [H, H, head_dim]
        rp_w = _get_rel_pos(grid_size, grid_size, rel_pos_w)  # [W, W, head_dim]
    ...
    ctx = network.add_matrix_multiply(
        softmax.get_output(0), trt.MatrixOperation.NONE, v_h.get_output(0), trt.MatrixOperation.NONE
    )
...
def _build_global_attention(
    ...
    return _build_windowed_attention(
        network, inp_4d, weights, w_prefix, grid_size, hidden, num_heads, head_dim, 0, dtype=dtype
    )
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete sam bundle inside its owning family module."""
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("sam does not support context-parallel builds")
    if options.get("dynamic_kv_cache") or options.get("triattention_stats_path"):
        raise ValueError("sam does not use a decoder KV-cache runtime")
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="sam tensor-parallel builds")
        if quant_ctx is not None:
            raise ValueError("sam tensor-parallel builds do not support quantization")
