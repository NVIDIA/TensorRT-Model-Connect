...
#include "sam3_video_processor.h"

#ifdef TRTMC_HAS_CUDA_KERNELS
#include "sam3_video_kernels.h"
#endif
...
bool tracker_sparse_outputs_can_hold(const TrtModule& engine, std::size_t mask_values,
                                     std::size_t pointer_values, std::size_t batch_size) {
    return tracker_output_can_hold(engine, "pred_masks", mask_values) &&
           ...
           tracker_output_can_hold(engine, "selected_iou", batch_size);
}
...
bool try_forward_tracker_head_sparse(TrtModule& engine, const TensorMap& inputs,
    ...
    if (!tracker_sparse_outputs_can_hold(engine, mask_values, pointer_values, batch_size)) {
        return false;
    }
...
EncodedMemory parse_encoded_memory(const TensorMap& outputs, const char* memory_name,
    ...
    bfloat16_round_in_place(result.position);
    return result;
}
...
std::vector<VisionFeatureBinding> vision_feature_bindings(
    ...
    TrtModule& tracker_memory_engine, TrtModule& tracker_hard_memory_engine,
    TrtModule& tracker_hard_memory_batch2_engine, TrtModule* tracker_step_batch2_engine,
    TrtModule* tracker_memory_batch2_engine, TrtModule* parallel_tracker_init_engine) {
    std::vector<VisionFeatureBinding> bindings;
    bindings.reserve(16 + (tracker_step_batch2_engine == nullptr ? 0 : 4) +
...
class Sam3VideoProcessorState {
  ...
  public:
    Sam3VideoProcessorState(TrtModule& vision_encoder, TrtModule& core_engine,
                            ...
                            TrtModule& tracker_memory_engine, TrtModule& tracker_hard_memory_engine,
                            TrtModule& tracker_hard_memory_batch2_engine, Sam3Config config,
  ...
  private:
    ...
    bool has_device_hard_memory() const {
        return device_resident_recurrent_memory_ &&
               has_device_memory_pair(tracker_hard_memory_engine_, "new_memory_features",
                                      "new_memory_position");
    }

    bool has_device_hard_batch2_memory() const {
        return device_resident_recurrent_memory_ &&
    ...
    void initialize_device_resident_memory_capabilities() {
        ...
        device_resident_hard_memory_ = has_device_hard_memory();
        device_resident_hard_batch2_memory_ = has_device_hard_batch2_memory();
#endif
    }
    ...
    void retain_device_frame_zero_tracker_feature_2() {
        ...
        frame_zero_tracker_feature_2_host_.clear();
    }

    void retain_host_frame_zero_tracker_feature_2(const Sam3FrameFeatures& features) {
        const auto source = tracker_feature(features.vision_outputs, 2);
        frame_zero_tracker_feature_2_host_ = copy_float_tensor(source);
        frame_zero_tracker_feature_2_shape_ = source.shape;
        if (frame_zero_tracker_feature_2_host_.empty() ||
            frame_zero_tracker_feature_2_host_.size() !=
                static_shape_values(frame_zero_tracker_feature_2_shape_)) {
            throw std::runtime_error("SAM3 frame-zero tracker feature host copy failed");
        }
    }

    void retain_frame_zero_tracker_feature_2(const Sam3FrameFeatures& features) {
        ...
        retain_host_frame_zero_tracker_feature_2(features);
    }
    ...
    TrackerNeuralOutput initialize_track_with_engine(const Sam3FrameFeatures& features,
        ...
        return parse_tracker_init_output(tracker_init_engine.forward(inputs));
    }
    ...
    void prepare_tracker_step_scratch(const std::vector<TrackerStepRequest>& requests,
        ...
        for (std::size_t batch = begin; batch < begin + count; ++batch) {
            ...
            for (std::size_t memory_index = 0; memory_index < request.memory_records.size();
                 ++memory_index) {
                const auto* record = request.memory_records[memory_index];
                if (!use_device_memory) {
    ...
    Tensor tracker_memory_tensor(bool /*batch2_layout*/, bool use_device_memory, std::size_t count,
                                 std::size_t memory_count, std::size_t spatial_tokens) {
        Tensor tensor;
    ...
    std::vector<TrackerNeuralOutput> propagate_track_requests(
        ...
        prepare_tracker_step_scratch(requests, begin, count, memory_values, memory_count,
...
#endif
        const auto outputs = memory_engine.forward(inputs);
    ...
    encode_batch2_device_memories(TrtModule& memory_engine, const TensorMap& inputs,
                                  const char* producer) {
    ...
    split_batch2_host_memories(const TensorMap& outputs, const char* producer) {
        const auto memory_output = require_output(outputs, "new_memory_features", producer);
        const auto position_output = require_output(outputs, "new_memory_position", producer);
        validate_batch2_memory_host_shapes(memory_output, position_output);
        auto combined =
            parse_encoded_memory(outputs, "new_memory_features", "new_memory_position", producer);
    ...
    std::array<EncodedMemory, kTrackerStepBatch2Size> encode_memories_batch2(
        ...
        const auto outputs = memory_engine.forward(inputs);
        return split_batch2_host_memories(outputs, producer);
    }
    ...
    Sam3FrameFeatures restore_frame_zero_tracker_feature_2() {
        ...
        if (device_resident_vision_features_) {
            ...
            if (!frame_zero_tracker_feature_2_.ok() || destination == nullptr ||
                ...
                vision_encoder_.tensor_dtype(output_name) != DType::kFloat32) {
                throw std::runtime_error(
                    "SAM3 frame-zero tracker feature retention is unavailable");
            }
            check_cuda(cudaMemcpyAsync(destination, frame_zero_tracker_feature_2_.data(),
                                       frame_zero_tracker_feature_2_.nbytes(),
                                       cudaMemcpyDeviceToDevice, vision_encoder_.stream()),
        ...
        if (frame_zero_tracker_feature_2_host_.empty() ||
            static_shape_values(frame_zero_tracker_feature_2_shape_) !=
                frame_zero_tracker_feature_2_host_.size()) {
            throw std::runtime_error(
                "SAM3 frame-zero tracker feature host retention is unavailable");
        }
        features.vision_outputs.emplace(
            output_name, Tensor{frame_zero_tracker_feature_2_host_.data(),
                                frame_zero_tracker_feature_2_shape_, DType::kFloat32});
        return features;
    }
    ...
    DeferredFrameResult advance_frame_with_features(const Sam3VideoFrame& frame,
        ...
        if (frame.frame_idx == 0 && !tracks_.empty())
            retain_frame_zero_tracker_feature_2(features);
    ...
    bool device_resident_hard_memory_{false};
    bool device_resident_hard_batch2_memory_{false};
    ...
    std::vector<float> frame_zero_tracker_feature_2_host_;
