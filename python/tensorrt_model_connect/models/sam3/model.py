...
"""SAM3 family-owned build implementation for text-prompt PCS bring-up.

This is intentionally separate from the legacy ``sam`` family.  SAM3's image
...
def matches(config) -> bool:
    ...
    return normalized in {"sam3", "sam3_video"}
...
def build_extra_engines(
    ...
    *,
    ...
    plans = {
        "sam3_core_engine_plan": build_sam3_core_engine(
            core_weights,
            **core_kwargs,
        )
    }
...
def get_segmentation_config(config: ModelConfig) -> dict:
    ...
    return {
        ...
        "sam3_text_projection_dim": int(cfg.get("_text_projection_dim", cfg["detr_hidden_size"])),
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete sam3 bundle in the owning family module."""
    import json
    import time
    ...
    segmentation_config = get_segmentation_config(config)
    if segmentation_config is not None:
        runtime_config.update(segmentation_config)
