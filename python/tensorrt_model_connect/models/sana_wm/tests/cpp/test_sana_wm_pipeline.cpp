...
std::size_t ucpe_raymat_offset(int32_t chunk, int32_t y, int32_t x, int32_t row, int32_t col,
                               int32_t h, int32_t w) {
    return (((((static_cast<std::size_t>(chunk) * static_cast<std::size_t>(h) +
                static_cast<std::size_t>(y)) *
                   static_cast<std::size_t>(w) +
               static_cast<std::size_t>(x)) *
                  4U +
...
class FakeConnectorTextModule final : public trtmc::ITrtModule {
  public:
    FakeConnectorTextModule(std::vector<float> context, std::vector<int64_t> context_shape,
                            std::vector<float> mask, std::vector<int64_t> mask_shape)
        : context_(std::move(context)), context_shape_(std::move(context_shape)),
          mask_(std::move(mask)), mask_shape_(std::move(mask_shape)) {}
    ...
    int32_t profile_idx() const override { return 0; }
    std::vector<trtmc::TensorInfo> input_info() const override {
        return {{"input_ids", {1, 256}, trtmc::DType::kInt32, true},
                {"attention_mask", {1, 256}, trtmc::DType::kInt32, true}};
    }
    std::vector<trtmc::TensorInfo> output_info() const override {
        return {{"v_context", context_shape_, trtmc::DType::kFloat32, false},
                {"v_attention_mask", mask_shape_, trtmc::DType::kFloat32, false}};
    }
    bool has_input(const std::string& name) const override {
        return name == "input_ids" || name == "attention_mask";
    }
    bool has_output(const std::string& name) const override {
        return name == "v_context" || name == "v_attention_mask";
    }
    ...
    std::vector<int64_t> tensor_shape(const std::string& name) const override {
        if (name == "input_ids" || name == "attention_mask")
            return {1, 256};
        if (name == "v_context")
            return context_shape_;
        if (name == "v_attention_mask")
            return mask_shape_;
        return {};
    }
  ...
  private:
    ...
    std::vector<int64_t> context_shape_;
...
class FakeDecoderTextModule final : public trtmc::ITrtModule {
  public:
    explicit FakeDecoderTextModule(int64_t max_tokens = 2, bool debug_layer_outputs = false,
                                   bool prefill_hidden = false)
        : max_tokens_(max_tokens), debug_layer_outputs_(debug_layer_outputs),
    ...
    trtmc::TensorMap forward(const trtmc::TensorMap& inputs) override {
        ...
        if (prefill_hidden_ && token_values.size() > 1) {
            token_ids.insert(token_ids.end(), token_values.begin(), token_values.end());
            position_ids.insert(position_ids.end(), position_values.begin(), position_values.end());
            hidden_state_.clear();
            debug_embed_.clear();
            debug_hidden_0_.clear();
            debug_hidden_1_.clear();
            for (std::size_t i = 0; i < token_values.size(); ++i) {
                const auto token = token_values[i];
                const auto position = i < position_values.size() ? position_values[i] : -1;
                hidden_state_.push_back(static_cast<float>(token * 10 + position));
                hidden_state_.push_back(static_cast<float>(token * 10 + position + 1));
                debug_embed_.push_back(static_cast<float>(token * 100 + position));
                debug_embed_.push_back(static_cast<float>(token * 100 + position + 1));
                debug_hidden_0_.push_back(static_cast<float>(1000 + token * 100 + position));
                debug_hidden_0_.push_back(static_cast<float>(1000 + token * 100 + position + 1));
                debug_hidden_1_.push_back(static_cast<float>(2000 + token * 100 + position));
                debug_hidden_1_.push_back(static_cast<float>(2000 + token * 100 + position + 1));
            }
            const std::vector<int64_t> shape{static_cast<int64_t>(token_values.size()), 2};
            trtmc::TensorMap outputs{
                {"hidden_state",
                 trtmc::Tensor{hidden_state_.data(), shape, trtmc::DType::kFloat32}},
            };
            if (debug_layer_outputs_) {
                outputs["debug_embed"] =
                    trtmc::Tensor{debug_embed_.data(), shape, trtmc::DType::kFloat32};
                outputs["debug_hidden_0"] =
                    trtmc::Tensor{debug_hidden_0_.data(), shape, trtmc::DType::kFloat32};
                outputs["debug_hidden_1"] =
                    trtmc::Tensor{debug_hidden_1_.data(), shape, trtmc::DType::kFloat32};
            }
            return outputs;
        }
        ...
        debug_embed_ = {static_cast<float>(token_ids.back() * 100 + position_ids.back()),
                        static_cast<float>(token_ids.back() * 100 + position_ids.back() + 1)};
        debug_hidden_0_ = {
            static_cast<float>(1000 + token_ids.back() * 100 + position_ids.back()),
            static_cast<float>(1000 + token_ids.back() * 100 + position_ids.back() + 1)};
        debug_hidden_1_ = {
            static_cast<float>(2000 + token_ids.back() * 100 + position_ids.back()),
            static_cast<float>(2000 + token_ids.back() * 100 + position_ids.back() + 1)};
        ...
        if (debug_layer_outputs_) {
            outputs["debug_embed"] =
                trtmc::Tensor{debug_embed_.data(), {1, 2}, trtmc::DType::kFloat32};
            outputs["debug_hidden_0"] =
                trtmc::Tensor{debug_hidden_0_.data(), {1, 2}, trtmc::DType::kFloat32};
            outputs["debug_hidden_1"] =
                trtmc::Tensor{debug_hidden_1_.data(), {1, 2}, trtmc::DType::kFloat32};
        }
        return outputs;
    }
    trtmc::DeviceTensorMap forward_device(const trtmc::DeviceTensorMap&) override { return {}; }
    ...
    std::vector<trtmc::TensorInfo> input_info() const override {
        if (prefill_hidden_) {
            return {
                {"token_id", {max_tokens_}, trtmc::DType::kInt32, true},
                {"position_id", {max_tokens_}, trtmc::DType::kInt32, true},
                {"attention_mask", {max_tokens_, max_tokens_ * 2}, trtmc::DType::kFloat32, true},
    ...
    std::vector<trtmc::TensorInfo> output_info() const override {
        if (prefill_hidden_) {
            std::vector<trtmc::TensorInfo> info{
                {"hidden_state", {max_tokens_, 2}, trtmc::DType::kFloat32, false},
            };
            if (debug_layer_outputs_) {
                info.push_back({"debug_embed", {max_tokens_, 2}, trtmc::DType::kFloat32, false});
                info.push_back({"debug_hidden_0", {max_tokens_, 2}, trtmc::DType::kFloat32, false});
                info.push_back({"debug_hidden_1", {max_tokens_, 2}, trtmc::DType::kFloat32, false});
            }
            return info;
        }
        std::vector<trtmc::TensorInfo> info{
            {"hidden_state", {1, 2}, trtmc::DType::kFloat32, false},
            {"present_k_0", {1, 2}, trtmc::DType::kFloat32, false},
            {"present_v_0", {1, 2}, trtmc::DType::kFloat32, false},
        };
        if (debug_layer_outputs_) {
            info.push_back({"debug_embed", {1, 2}, trtmc::DType::kFloat32, false});
            info.push_back({"debug_hidden_0", {1, 2}, trtmc::DType::kFloat32, false});
            info.push_back({"debug_hidden_1", {1, 2}, trtmc::DType::kFloat32, false});
        }
        return info;
    }
    bool has_input(const std::string& name) const override {
  ...
  private:
    ...
    bool debug_layer_outputs_{false};
    bool prefill_hidden_{false};
    std::vector<float> hidden_state_;
    std::vector<float> debug_embed_;
    std::vector<float> debug_hidden_0_;
    std::vector<float> debug_hidden_1_;
    std::vector<float> present_k_;
    std::vector<float> present_v_;
};

class FakeTextConnectorModule final : public trtmc::ITrtModule {
  public:
    FakeTextConnectorModule(std::vector<float> context, std::vector<int64_t> context_shape,
                            std::vector<float> mask, std::vector<int64_t> mask_shape)
        : context_(std::move(context)), context_shape_(std::move(context_shape)),
          mask_(std::move(mask)), mask_shape_(std::move(mask_shape)) {}
    ...
    int32_t profile_idx() const override { return 0; }
    std::vector<trtmc::TensorInfo> input_info() const override {
        return {{"text_hidden_states", {1, 4, 6}, trtmc::DType::kFloat32, true},
                {"attention_mask", {1, 4}, trtmc::DType::kInt32, true}};
    }
    std::vector<trtmc::TensorInfo> output_info() const override {
        return {{"v_context", context_shape_, trtmc::DType::kFloat32, false},
                {"v_attention_mask", mask_shape_, trtmc::DType::kFloat32, false}};
    ...
    bool has_input(const std::string& name) const override {
        return name == "text_hidden_states" || name == "attention_mask";
    }
    bool has_output(const std::string& name) const override {
        return name == "v_context" || name == "v_attention_mask";
    }
    ...
    std::unordered_map<std::string, trtmc::DType> last_input_dtypes;
    std::vector<std::unordered_map<std::string, std::vector<float>>> input_value_calls;
    std::unordered_map<std::string, std::vector<int32_t>> int_value_calls;
...
void test_action_rollout_matches_model_card_frame_count_and_translation() {
    ...
    check(near(poses[81].c2w[3], -0.0549999F, 1.0e-4F),
          "sana wm action: j strafes left under unified controls");
    check(near(poses[121].c2w[3], -2.251665F, 1.0e-4F),
          "sana wm action: j strafe accumulates with smoothing");
    check(near(poses.back().c2w[2], 0.0F, 1.0e-6F),
          "sana wm action: model-card action has no yaw after j/l remap");
    check(near(poses.back().c2w[3], 1.168354F, 1.0e-4F) &&
              near(poses.back().c2w[11], 17.599983F, 1.0e-4F),
          "sana wm action: final smoothed strafe/forward position matches upstream");
}
...
void test_runtime_config_parses_native_sana_wm_fields() {
    const auto cfg = trtmc::parse_sana_wm_config(
        R"json({
          ...
          "sana_wm_stage1_text_encoder_hf_id": "local/stage1-text",
    ...
    check(near(cfg.flow_shift, 9.8F) && cfg.seed == 42 && cfg.refiner_seed == 314,
    ...
    check(cfg.refiner_text_encoder_max_length == 1024,
          "sana wm config: refiner text encoder length parsed");
    check(cfg.text_encoder_dim == 2304, "sana wm config: text encoder dim parsed");
    check(cfg.stage1_text_encoder_hf_id == "local/stage1-text",
          "sana wm config: stage1 text encoder model parsed");
...
void test_camera_conditions_match_upstream_shapes_and_raymap() {
    ...
    check(conditions.raymats.size() == 128, "sana wm camera: UCPE ray matrix shape");
    check(conditions.raymats_inv.size() == 128, "sana wm camera: inverse UCPE ray matrix shape");
    for (int32_t row = 0; row < 4; ++row) {
        for (int32_t col = 0; col < 4; ++col) {
            const float expected = row == col ? 1.0F : 0.0F;
            check(near(conditions.raymats[static_cast<std::size_t>(row * 4 + col)], expected),
                  "sana wm camera: first UCPE ray matrix is identity");
            check(near(conditions.raymats_inv[static_cast<std::size_t>(row * 4 + col)], expected),
                  "sana wm camera: first inverse UCPE ray matrix is identity");
        }
    }
...
void test_camera_conditions_inverse_ucpe_raymat_matches_bfloat16_packed_inverse() {
    trtmc::SanaWmPose first;
    first.c2w = {1.0F, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, 0.0F, 0.0F,
                 0.0F, 0.0F, 1.0F, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F};
    trtmc::SanaWmPose second;
    second.c2w = {1.0F, 0.0F, 0.0F, 1.2345F,  0.0F, 1.0F, 0.0F, -0.3333F,
                  0.0F, 0.0F, 1.0F, 2.71828F, 0.0F, 0.0F, 0.0F, 1.0F};

    const std::vector<trtmc::SanaWmPose> poses{first, second};
    const std::vector<trtmc::SanaWmIntrinsics> intrinsics(poses.size(), {2.0F, 2.0F, 0.0F, 0.0F});
    const auto conditions = trtmc::sana_wm_prepare_camera_conditions(poses, intrinsics, 4, 4, 1, 2);

    auto matrix_at = [&](const std::vector<float>& values, int32_t x, int32_t row, int32_t col) {
        return values[ucpe_raymat_offset(1, 0, x, row, col, 2, 2)];
    };

    const float rounded_origin_x = round_to_bfloat16(second.c2w[3]);
    check(near(matrix_at(conditions.raymats_inv, 0, 0, 3), rounded_origin_x, 0.0F),
          "sana wm camera: inverse UCPE translation is bfloat16-rounded");
    check(!near(matrix_at(conditions.raymats_inv, 0, 0, 3), second.c2w[3], 0.0F),
          "sana wm camera: inverse UCPE translation is not stored as unrounded origin");

    bool inverse_matches_packed_raymat = true;
    for (int32_t row = 0; row < 3; ++row) {
        for (int32_t col = 0; col < 3; ++col) {
            const float expected = round_to_bfloat16(matrix_at(conditions.raymats, 1, col, row));
            inverse_matches_packed_raymat =
                inverse_matches_packed_raymat &&
                near(matrix_at(conditions.raymats_inv, 1, row, col), expected, 0.0F);
        }
        const float expected_translation = round_to_bfloat16(
            -(matrix_at(conditions.raymats, 1, 0, row) * matrix_at(conditions.raymats, 1, 0, 3) +
              matrix_at(conditions.raymats, 1, 1, row) * matrix_at(conditions.raymats, 1, 1, 3) +
              matrix_at(conditions.raymats, 1, 2, row) * matrix_at(conditions.raymats, 1, 2, 3)));
        inverse_matches_packed_raymat =
            inverse_matches_packed_raymat &&
            near(matrix_at(conditions.raymats_inv, 1, row, 3), expected_translation, 0.0F);
    }
    check(inverse_matches_packed_raymat,
          "sana wm camera: inverse UCPE ray matrix matches bfloat16 packed inverse");
}
...
void test_stage1_latents_seed42_matches_pytorch_cuda_bfloat16_randn() {
    ...
    check(latents.values.size() == expected.size(),
          "sana wm latents: pytorch cuda bf16 randn sample count");
    check(near(latents.values[0], 0.0F),
          "sana wm latents: pytorch cuda bf16 randn still anchors frame zero");
    bool all_match = latents.values.size() == expected.size();
    for (std::size_t i = 1; i < latents.values.size() && i < expected.size(); ++i)
        all_match = all_match && near(latents.values[i], expected[i], 0.0F);
    check(all_match, "sana wm latents: seed 42 matches pytorch cuda bf16 randn");
}
...
void test_stage1_latents_seed42_matches_pytorch_cuda_grid_stride_lanes() {
    ...
    check(near(latents.values[622592], 1.921875F, 0.0F),
          "sana wm latents: seed 42 matches pytorch cuda normal4 lane two");
    check(near(latents.values[933888], -1.234375F, 0.0F),
          "sana wm latents: seed 42 matches pytorch cuda normal4 lane three");
    check(near(latents.values[1245184], 0.138671875F, 0.0F),
          "sana wm latents: seed 42 matches pytorch cuda next grid-stride offset");
}
...
void test_pipeline_requires_native_tensor_rt_modules() {
    ...
    cfg.default_image = "asset/sana_wm/demo_0.png";
...
void test_native_module_sections_require_complete_native_set() {
    trtmc::SanaWmRuntimeConfig cfg;
    cfg.hf_id = "Efficient-Large-Model/SANA-WM_bidirectional";
    cfg.default_image = "/tmp/trtmc_sana_wm_missing_input.png";
...
void test_segmented_stage1_denoiser_modules_count_as_native_stage1() {
    trtmc::SanaWmRuntimeConfig cfg;
    cfg.hf_id = "Efficient-Large-Model/SANA-WM_bidirectional";

    trtmc::SanaWmNativeModules complete;
    complete.text_encoder = std::make_unique<FakeTrtModule>();
    complete.vae_encoder = std::make_unique<FakeTrtModule>();
    for (int i = 0; i < 5; ++i)
        complete.stage1_denoiser_segments.push_back(std::make_unique<FakeTrtModule>());
    trtmc::SanaWmPipeline complete_pipeline(cfg, std::move(complete));
    check(complete_pipeline.has_native_stage1(),
          "sana wm native: complete segmented stage1 denoiser is accepted");

    trtmc::SanaWmNativeModules partial;
    partial.text_encoder = std::make_unique<FakeTrtModule>();
    partial.vae_encoder = std::make_unique<FakeTrtModule>();
    partial.stage1_denoiser_segments.push_back(std::make_unique<FakeTrtModule>());
    trtmc::SanaWmPipeline partial_pipeline(cfg, std::move(partial));
    check(!partial_pipeline.has_native_stage1(),
          "sana wm native: partial segmented stage1 denoiser is incomplete");
}

void test_native_default_requires_refiner_plan_set() {
    trtmc::SanaWmRuntimeConfig cfg;
    cfg.hf_id = "Efficient-Large-Model/SANA-WM_bidirectional";
    cfg.default_image = "/tmp/trtmc_sana_wm_default_requires_refiner.png";
    ...
    try {
        ...
        refiner_required_reported =
            ...
            message.find("refiner text, denoiser, VAE, and tokenizer") != std::string::npos;
    }
...
void test_native_stage1_solver_decodes_with_native_modules() {
    ...
    trtmc::SanaWmRuntimeConfig cfg;
    cfg.hf_id = "Efficient-Large-Model/SANA-WM_bidirectional";
    cfg.default_image = image_path.string();
    cfg.action = "w-1";
    cfg.height = 4;
    cfg.width = 4;
    cfg.num_frames = 2;
    ...
    cfg.refiner_text_encoder_max_length = 2;
    ...
    check(denoiser_ptr->last_input_shapes["raymats"] == std::vector<int64_t>({2, 8, 4, 4}),
          "sana wm native: denoiser UCPE ray matrix shape");
    check(denoiser_ptr->last_input_shapes["raymats_inv"] == std::vector<int64_t>({2, 8, 4, 4}),
          "sana wm native: denoiser inverse UCPE ray matrix shape");
    ...
    if (denoiser_ptr->input_value_calls.size() == 2) {
        ...
        const auto& second_t = denoiser_ptr->input_value_calls[1]["timestep"];
        check(first_t.size() == 4 && near(first_t[0], 0.0F) && near(first_t[2], 0.0F),
              "sana wm native: anchor frame timestep is zero for CFG batches");
        check(second_t.size() == 4 && second_t[1] > 80.0F && second_t[1] < 95.0F,
              "sana wm native: CLI flow_shift override reaches stage1 scheduler");
...
void test_native_stage1_accepts_decoder_style_text_encoder() {
    ...
    if (text_encoder_ptr->input_value_calls.size() >= 4) {
        ...
        const auto& first_pad_mask = text_encoder_ptr->input_value_calls[2]["attention_mask"];
        const auto& second_pad_mask = text_encoder_ptr->input_value_calls[3]["attention_mask"];
        check(first_pad_mask.size() == 3 && near(first_pad_mask[0], -10000.0F) &&
                  near(first_pad_mask[1], -10000.0F) && near(first_pad_mask[2], -10000.0F),
              "sana wm decoder text: first pad token is masked from the current slot");
        check(second_pad_mask.size() == 3 && near(second_pad_mask[0], -10000.0F) &&
                  near(second_pad_mask[1], -10000.0F) && near(second_pad_mask[2], -10000.0F),
              "sana wm decoder text: pad cache rows stay masked");
...
void test_native_stage1_uses_decoder_prefill_hidden_state() {
    const auto image_path =
        std::filesystem::temp_directory_path() / "trtmc_sana_wm_prefill_text_input_test.png";
    trtmc::io::save_png(image_path.string(),
                        {
                            1.0F,
                            0.0F,
    ...
    trtmc::SanaWmNativeModules modules;
    auto text_encoder = std::make_unique<FakeDecoderTextModule>(2, false, true);
    auto* text_encoder_ptr = text_encoder.get();
    modules.text_encoder = std::move(text_encoder);
    auto denoiser = std::make_unique<FakeTrtModule>(
        std::vector<float>(32, 0.25F), std::vector<int64_t>{2, 2, 2, 2, 2},
    ...
    check(result.num_frames == 2, "sana wm decoder prefill text: generated native video");
    check(text_encoder_ptr->call_count == 2,
          "sana wm decoder prefill text: encoder invoked once per prompt");
    check(text_encoder_ptr->token_ids == std::vector<int32_t>({1, 2, 0, 0}),
          "sana wm decoder prefill text: token ids are fed as full sequences");
    check(text_encoder_ptr->position_ids == std::vector<int32_t>({0, 1, 0, 1}),
          "sana wm decoder prefill text: position ids cover each full sequence");
    check(text_encoder_ptr->last_input_shapes["token_id"] == std::vector<int64_t>({2}),
          "sana wm decoder prefill text: token input shape");
    check(text_encoder_ptr->last_input_shapes["attention_mask"] == std::vector<int64_t>({2, 4}),
          "sana wm decoder prefill text: attention mask covers cache and prompt");
    if (text_encoder_ptr->input_value_calls.size() >= 2) {
        const auto& cond_mask = text_encoder_ptr->input_value_calls[0]["attention_mask"];
        check(cond_mask.size() == 8 && near(cond_mask[0], -10000.0F) &&
                  near(cond_mask[1], -10000.0F) && near(cond_mask[2], 0.0F) &&
                  near(cond_mask[3], -10000.0F) && near(cond_mask[6], 0.0F) &&
                  near(cond_mask[7], 0.0F),
              "sana wm decoder prefill text: condition prompt uses causal key mask");
        const auto& neg_mask = text_encoder_ptr->input_value_calls[1]["attention_mask"];
        check(neg_mask.size() == 8 && std::all_of(neg_mask.begin(), neg_mask.end(),
                                                  [](float v) { return near(v, -10000.0F); }),
              "sana wm decoder prefill text: negative pad prompt masks all keys");
    }
    if (!denoiser_ptr->input_value_calls.empty()) {
        const auto& text = denoiser_ptr->input_value_calls.front()["y"];
        check(text.size() == 8 && near(text[0], 0.0F) && near(text[1], 1.0F) &&
                  near(text[2], 1.0F) && near(text[3], 2.0F) && near(text[4], 10.0F) &&
                  near(text[5], 11.0F) && near(text[6], 21.0F) && near(text[7], 22.0F),
              "sana wm decoder prefill text: hidden states feed stage1 text conditioning");
    }
...
void test_native_refiner_decodes_and_drops_sink_frame() {
    ...
    modules.vae_encoder = std::make_unique<FakeTrtModule>(
        ...
        std::vector<int64_t>{1, 2, 1, 2, 2});
    auto refiner_text = std::make_unique<FakeConnectorTextModule>(
        std::vector<float>{0.1F, 0.2F, 0.3F, 0.4F}, std::vector<int64_t>{1, 2, 2},
        std::vector<float>{1.0F, 0.0F}, std::vector<int64_t>{1, 2});
    ...
    refiner_video[refiner_cthw_index(0, 1, 0, 0)] = 0.0019683837890625F;
    ...
    check(result.num_frames == 1 && result.pixels.size() == 48 &&
              near(result.pixels[0], 128.0F / 255.0F) && near(result.pixels[1], 127.0F / 255.0F) &&
              near(result.pixels[2], 1.0F),
          "sana wm native refiner: drops sink CTHW frame and matches BF16 pixel conversion");
    ...
    check(refiner_denoiser_ptr->last_input_shapes["v_attention_mask"] ==
              std::vector<int64_t>({1, 2}),
          "sana wm native refiner: connector attention mask shape");
    if (!refiner_denoiser_ptr->input_value_calls.empty()) {
        const auto& mask = refiner_denoiser_ptr->input_value_calls.front()["denoise_mask"];
        const auto& text_mask = refiner_denoiser_ptr->input_value_calls.front()["v_attention_mask"];
        const auto& positions = refiner_denoiser_ptr->input_value_calls.front()["positions"];
        ...
        check(text_mask.size() == 2 && near(text_mask.front(), 1.0F) &&
                  near(text_mask.back(), 0.0F),
              "sana wm native refiner: passes connector attention mask to denoiser");
        check(positions.size() == 48 && near(positions[8], 0.25F) && near(positions[9], 2.25F),
              "sana wm native refiner: CLI fps override reaches position encoding");
    }
...
void test_native_refiner_uses_split_text_connector() {
    const auto image_path =
        std::filesystem::temp_directory_path() / "trtmc_sana_wm_refiner_connector_test.png";
    trtmc::io::save_png(image_path.string(),
                        {
                            1.0F,
                            0.0F,
    ...
    cfg.vae_spatial_stride = 2;
    cfg.text_encoder_max_length = 2;
    cfg.refiner_text_encoder_max_length = 4;
    cfg.text_encoder_dim = 2;
    cfg.num_steps = 1;
    cfg.default_intrinsics = {2.0F, 2.0F, 1.0F, 1.0F};
    ...
    auto refiner_text = std::make_unique<FakeDecoderTextModule>(4, true, true);
    auto* refiner_text_ptr = refiner_text.get();
    modules.refiner_text_encoder = std::move(refiner_text);
    auto text_connector = std::make_unique<FakeTextConnectorModule>(
        std::vector<float>{7.0F, 8.0F, 9.0F, 10.0F}, std::vector<int64_t>{1, 2, 2},
        std::vector<float>{1.0F, 1.0F}, std::vector<int64_t>{1, 2});
    auto* text_connector_ptr = text_connector.get();
    modules.refiner_text_connector = std::move(text_connector);
    auto refiner_denoiser = std::make_unique<FakeTrtModule>(
        std::vector<float>(8, 0.0F), std::vector<int64_t>{1, 4, 2},
        std::vector<std::string>{"latent", "clean_latent", "denoise_mask", "positions", "v_context",
                                 "v_attention_mask", "sigma"});
    ...
    check(refiner_text_ptr->call_count == 1,
          "sana wm refiner connector: Gemma text encoder uses one full prefill");
    check(refiner_text_ptr->position_ids.front() == 0 && refiner_text_ptr->position_ids.back() == 3,
          "sana wm refiner connector: prefill positions cover the padded sequence");
    check(refiner_text_ptr->last_input_shapes["attention_mask"] == std::vector<int64_t>({4, 8}),
          "sana wm refiner connector: prefill mask covers cache and sequence");
    if (!refiner_text_ptr->input_value_calls.empty()) {
        const auto& prefill_mask = refiner_text_ptr->input_value_calls.front()["attention_mask"];
        check(prefill_mask.size() == 32 && near(prefill_mask[22], 0.0F) &&
                  near(prefill_mask[23], -10000.0F) && near(prefill_mask[30], 0.0F) &&
                  near(prefill_mask[31], 0.0F),
              "sana wm refiner connector: prefill mask preserves causal left padding");
    }
    check(text_connector_ptr->call_count == 1,
          "sana wm refiner connector: LTX text connector invoked once");
    check(text_connector_ptr->last_input_shapes["text_hidden_states"] ==
              std::vector<int64_t>({1, 4, 6}),
          "sana wm refiner connector: hidden-state stack shape");
    check(text_connector_ptr->last_input_shapes["attention_mask"] == std::vector<int64_t>({1, 4}),
          "sana wm refiner connector: attention mask shape");
    check(text_connector_ptr->last_input_dtypes["attention_mask"] == trtmc::DType::kInt32,
          "sana wm refiner connector: attention mask dtype");
    const auto mask_it = text_connector_ptr->int_value_calls.find("attention_mask");
    check(mask_it != text_connector_ptr->int_value_calls.end() &&
    ...
    if (!text_connector_ptr->input_value_calls.empty()) {
        ...
        check(hidden.size() == 24 && near(hidden[0], 0.0F) && near(hidden[1], 1000.0F) &&
                  near(hidden[5], 1.0F) && near(hidden[12], 102.0F) && near(hidden[13], 1102.0F) &&
                  near(hidden[14], 12.0F) && near(hidden[18], 203.0F),
              "sana wm refiner connector: hidden states are packed with layer dimension fastest");
    }
    check(refiner_denoiser_ptr->last_input_shapes["v_context"] == std::vector<int64_t>({1, 2, 2}),
          "sana wm refiner connector: denoiser consumes connector context");
    if (!refiner_denoiser_ptr->input_value_calls.empty()) {
        const auto& text_mask = refiner_denoiser_ptr->input_value_calls.front()["v_attention_mask"];
        check(text_mask.size() == 2 && near(text_mask[0], 1.0F) && near(text_mask[1], 1.0F),
              "sana wm refiner connector: denoiser consumes connector attention mask");
    }
...
void test_native_refiner_accepts_decoder_style_text_encoder() {
    ...
    cfg.vae_spatial_stride = 2;
    cfg.text_encoder_max_length = 2;
    cfg.refiner_text_encoder_max_length = 256;
    cfg.text_encoder_dim = 2;
    cfg.num_steps = 1;
    cfg.default_intrinsics = {2.0F, 2.0F, 1.0F, 1.0F};
    ...
    if (!refiner_denoiser_ptr->input_value_calls.empty()) {
        ...
        check(text.size() == 512 && near(text[0], 0.0F) && near(text[1], 1.0F) &&
                  near(text[2], 1.0F) && near(text[3], 2.0F) && near(text[508], 264.0F) &&
                  near(text[510], 275.0F),
              "sana wm refiner decoder text: left-padded hidden states feed refiner conditioning");
    }
...
int main() {
    ...
    test_camera_conditions_inverse_ucpe_raymat_matches_bfloat16_packed_inverse();
    ...
    test_stage1_latents_seed42_matches_pytorch_cuda_bfloat16_randn();
