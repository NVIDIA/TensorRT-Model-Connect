...
"""SANA-WM-owned image-to-video contract plugin."""
...
def _make_pass(stage_name: str, metrics, rule: str) -> CompareResult:
    return CompareResult(
        ...
        metrics=metrics,
        composite_rule=rule,
        message="SANA-WM image-to-video contract verified",
    )
...
class SanaWmDiffusionVideoPlugin:
    ...
    def verify(self, trt_output, ref_output, case, threshold):
        ...
        if stage in final_stages:
            min_frames = int(threshold.metrics.get("contract_min_frame_count", 1))
        ...
        return _make_fail(stage, metrics, rule, "SANA-WM image-to-video contract failed")


plugin = SanaWmDiffusionVideoPlugin()
