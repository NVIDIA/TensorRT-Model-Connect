...
"""Shared deterministic inputs for Qwen-Image HF-to-TRTMC parity."""
...
def _parity_root(ctx: RunContext) -> Path:
    if not ctx.artifacts_dir:
        return Path(tempfile.gettempdir()) / "trtmc_qwen_image_parity"
...
def ensure_initial_latents(case: E2ECase, ctx: RunContext) -> InitialLatents:
    ...
    if height <= 0 or width <= 0 or height % 8 != 0 or width % 8 != 0:
        raise ValueError(
            "Qwen-Image parity dimensions must be positive and divisible by 8: "
            f"{height}x{width}"
        )
