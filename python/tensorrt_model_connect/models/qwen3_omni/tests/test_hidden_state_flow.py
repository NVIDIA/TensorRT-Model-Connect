...
MODEL = ROOT / "python/tensorrt_model_connect/models/qwen3_omni/model.py"
...
def test_qwen3_omni_runtime_feeds_generated_text_to_official_talker() -> None:
    source = (
        ROOT / "python/tensorrt_model_connect/models/qwen3_omni/runtime/pipeline.cpp"
    ).read_text()
...
def test_qwen3_omni_runtime_requires_official_code2wav_and_python_talker() -> None:
    source = (
        ROOT / "python/tensorrt_model_connect/models/qwen3_omni/runtime/plugin.cpp"
    ).read_text()
    builder = MODEL.read_text()
    ...
    assert 'overrides["omni_talker_model_id"] = _talker_model_id' in builder
...
def test_qwen3_omni_runtime_has_no_retired_talker_recurrent_state() -> None:
    runtime = ROOT / "python/tensorrt_model_connect/models/qwen3_omni/runtime"
    plugin = (runtime / "plugin.cpp").read_text()

    assert "Qwen3OmniKvCache" in plugin
    assert "Qwen3OmniRecurrentState" not in plugin
    assert '#include "recurrent_state.h"' not in plugin
