...
"""Qwen3-Omni family-owned build implementation.
...
_thinker_cfg: dict = {}
_talker_cfg: dict = {}
_audio_encoder_cfg: dict = {}
_code2wav_cfg: dict = {}
_talker_model_id = ""
_talker_model_revision = ""
...
def load_weights(model_dir: str, config: ModelConfig, *, precision: str = "fp32") -> WeightDict:
    ...
    _talker_model_id, _talker_model_revision = _talker_model_locator(model_dir_path)
    ...
    for layer_idx in range(num_layers):
        ...
        if _has_tensor(readers, router_key):
            ...
            weights[f"{prefix}.router"] = _transpose_2d(router_raw, "router", precision)
            del router_raw

            expert_gate = np.empty((num_experts, hidden, intermediate_size), dtype=target_dtype)
            expert_up = np.empty((num_experts, hidden, intermediate_size), dtype=target_dtype)
            expert_down = np.empty((num_experts, intermediate_size, hidden), dtype=target_dtype)
            for e in range(num_experts):
                ...
                expert_gate[e] = _transpose_2d(gate_raw, f"expert_{e}_gate", precision)
                expert_up[e] = _transpose_2d(up_raw, f"expert_{e}_up", precision)
                expert_down[e] = _transpose_2d(down_raw, f"expert_{e}_down", precision)
    ...
    if lm_head_key is not None:
        weights["w_out"] = _transpose_2d(_load_tensor(readers, lm_head_key), "lm_head", precision)
    else:
        weights["w_out"] = _transpose_2d(weights["embedding"], "embedding_tied", precision)
    ...
    audio_cfg = _detect_audio_encoder(readers)
    _audio_encoder_cfg = audio_cfg
    weights["_audio_encoder_cfg"] = audio_cfg

    # Detect Talker config
    talker_cfg = _detect_talker(readers, config)
    _talker_cfg = talker_cfg
    weights["_talker_cfg"] = talker_cfg

    # Detect Code2Wav config
    code2wav_cfg = _detect_code2wav(readers, config)
    _code2wav_cfg = code2wav_cfg
    ...
    for reader in readers:
        for key in reader.keys():
            ...
            # Code2Wav weights
                ...
                weights[f"code2wav.{key[len('model.code2wav.') :]}"] = _load_tensor([reader], key)
...
def _detect_audio_encoder(readers) -> dict:
...
def _detect_talker(readers, config: ModelConfig) -> dict:
...
def _detect_code2wav(readers, config: ModelConfig) -> dict:
...
def build_engine(
    ...
    *,
    ...
    if input_embed_tensor.dtype != work_trt_dtype:
        input_embed_tensor = network.add_cast(input_embed_tensor, work_trt_dtype).get_output(0)
    if use_input_embed_tensor.dtype != work_trt_dtype:
        use_input_embed_tensor = network.add_cast(
            use_input_embed_tensor, work_trt_dtype
        ).get_output(0)
    ...
    if verbose:
        print(
            f"[trtmc build] Building dual-profile Qwen3-Omni Thinker MoE engine "
...
def get_bundle_config_overrides(config: ModelConfig) -> dict:
    ...
    serialized JSON (the model-owned bundle builder handles this).
    """
    overrides = {}
    ...
    # Thinker MoE config
    overrides["num_local_experts"] = _thinker_cfg.get("num_experts", 8)
    overrides["num_experts_per_tok"] = _thinker_cfg.get("num_experts_per_tok", 2)
    # Audio output config (flat keys matching C++ fast_path_config parser)
    overrides["audio_sample_rate"] = 24000
    overrides["omni_n_codebooks"] = _talker_cfg.get("n_codebooks", 16)
    overrides["omni_codebook_size"] = _talker_cfg.get("codebook_size", 2048)

    # Talker config (flat keys for C++ parser)
    overrides["omni_talker_hidden_size"] = _talker_cfg.get("hidden_size", 0)
    overrides["omni_talker_num_layers"] = _talker_cfg.get("num_layers", 0)
    overrides["omni_talker_max_cache_length"] = 1024
    overrides["omni_talker_model_id"] = _talker_model_id
    overrides["omni_talker_model_revision"] = _talker_model_revision
    ...
    overrides["omni_code2wav_max_frames"] = _code2wav_cfg.get("max_frames", 32)
    overrides["omni_code2wav_upsample_factor"] = _code2wav_cfg.get("upsample_factor", 1920)
    overrides["omni_code2wav_output_delay"] = _code2wav_cfg.get("output_delay", 555)

    # Audio encoder config (flat keys for C++ parser)
    overrides["omni_audio_embed_dim"] = _audio_encoder_cfg.get("embed_dim", 1280)
    overrides["omni_audio_num_mel"] = _audio_encoder_cfg.get("num_mel_bins", 128)
    overrides["omni_audio_num_layers"] = _audio_encoder_cfg.get("num_layers", 0)
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete qwen3_omni bundle in the owning family module."""
    import json
    import time
