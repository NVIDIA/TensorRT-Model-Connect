...
``default_decoder`` for now. The VL ``embed_input`` path is included
...
def build_dual_profile_tp_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp16', opt_prefill_length: int=64, max_prefill_length: int | None=None, quant_ctx: 'QuantContext | None'=None, partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, verbose: bool=False, dynamic_kv_profile_rows: list[int] | None=None, parallel_config=None) -> bytes:
    ...
    multi_device_preview = getattr(trt.PreviewFeature, 'MULTIDEVICE_RUNTIME_10_16', None)
    ...
    if input_embed_tensor is not None and use_input_embed_tensor is not None:
        ...
        one_const = _const_in_work_dtype(network, (1, 1), np.array([[1.0]], dtype=work_np_dtype), work_np_dtype, work_trt_dtype)
        inv_flag = network.add_elementwise(one_const, embed_selector, trt.ElementWiseOperation.SUB)
        tok_part = network.add_elementwise(inv_flag.get_output(0), token_embed, trt.ElementWiseOperation.PROD)
        embed_part = network.add_elementwise(embed_selector, input_embed, trt.ElementWiseOperation.PROD)
    ...
    for layer_idx in range(num_layers):
        ...
        normed = _norm_multi(network, hidden_state, hidden, weights[f'{prefix}.input_norm'], weights.get(f'{prefix}.input_norm_beta'), eps_tensor, 'rmsnorm', work_np_dtype)
        ...
        if parallel_residual:
            ...
            if post_attn_norm_w is not None:
                norm2 = _norm_multi(network, hidden_state, hidden, post_attn_norm_w, weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, 'rmsnorm', work_np_dtype)
            else:
                norm2 = normed
        else:
            residual1 = network.add_elementwise(hidden_state, attn_out, trt.ElementWiseOperation.SUM)
            norm2 = _norm_multi(network, residual1.get_output(0), hidden, weights[f'{prefix}.post_attn_norm'], weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, 'rmsnorm', work_np_dtype)
    ...
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = _norm_multi(network, hidden_state, hidden, final_norm, weights.get('final_norm_beta'), eps_tensor, 'rmsnorm', work_np_dtype)
    ...
    if verbose:
        print(f'[trtmc-build] Building tensor-parallel decoder engine (layers={num_layers}, hidden={hidden}, attn={attention_size}, kv={kv_attention_size}, mlp={mlp_size}, cache={max_cache_length}, opt_prefill={opt_prefill_length}, max_prefill={max_prefill_length}, norm=rmsnorm, mlp_type=swiglu, pos=rope, precision={precision}, tp={parallel.tp_size}, rank={parallel.rank}) ...', file=sys.stderr)
