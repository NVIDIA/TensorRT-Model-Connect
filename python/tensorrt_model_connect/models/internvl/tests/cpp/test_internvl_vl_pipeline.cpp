...
// Intent:         InternVlPipeline text-only generation with mock engines; constructor
//                 validation (null decoder/cache); InternVlConfig sync from
//                 InternVlPreprocessConfig; zero max_tokens early exit; string-based
...
// =============================================================================
// Test suite: InternVlPipeline — vision-language generation
// =============================================================================
...
static void test_vl_text_only() {
    ...
    trtmc::InternVlPipeline pipeline(std::move(decoder), nullptr, std::move(cache), cfg, vl_pp,
                                     stream);
    check(std::string(pipeline.pipeline_type()) == "InternVlPipeline", "vl name");
...
static void test_vl_validates_decoder() {
    ...
    bool threw = false;
    try {
        trtmc::InternVlPipeline p(nullptr, nullptr, std::move(cache), cfg, vl_pp, stream);
...
static void test_vl_validates_cache() {
    ...
    bool threw = false;
    try {
        trtmc::InternVlPipeline p(std::move(decoder), nullptr, nullptr, cfg, vl_pp, stream);
    } catch (const std::exception&) {
        threw = true;
    }
...
static void test_vl_config_sync() {
    // InternVlPreprocessConfig has image_token_id=1 and vision_output_dim=64,
    // InternVlConfig has image_token_id=-1 and vision_output_dim=0.
    ...
    check(std::string(pipeline.pipeline_type()) == "InternVlPipeline", "config_sync: type correct");
...
static void test_vl_generate_with_vision_encoder() {
    ...
    //   convert_float_to_decoded (lines 64-79), internvl_preprocess_decoded_image with simple_chw,
...
static void test_vl_generate_with_embed_decoder() {
    // Covers InternVlPipeline::run_text_step_with_embed() full body (lines 285-361).
