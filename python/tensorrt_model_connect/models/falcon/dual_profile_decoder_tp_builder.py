...
def build_dual_profile_tp_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp16', opt_prefill_length: int=64, max_prefill_length: int | None=None, quant_ctx: 'QuantContext | None'=None, position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, alibi_bias_scale: float=1.0, verbose: bool=False, dynamic_kv_profile_rows: list[int] | None=None, parallel_config=None) -> bytes:
    ...
    if position_type == 'alibi':
        mask_4d = graph_ops.add_alibi_mask_4d(network, attention_mask_work, position_id, alibi_slopes_tensor, alibi_cache_positions_fp32, num_heads, target_dtype=work_trt_dtype, bias_scale=float(alibi_bias_scale))
    else:
