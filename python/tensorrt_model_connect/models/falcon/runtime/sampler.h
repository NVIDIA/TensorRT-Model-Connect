...
// FalconISampler: token selection abstraction for autoregressive generation.
//
...
//
// FalconLogitsLocation tells the pipeline whether to D2H transfer logits before
...
/// falcon-owned sampler interface.
class FalconISampler {
  public:
    virtual ~FalconISampler() = default;
    ...
    virtual FalconSampleResult sample(const float* logits, int32_t vocab_size,
                                      const FalconSamplingParams& params) = 0;
    ...
    virtual FalconLogitsLocation logits_location() const = 0;
...
/// Build FalconSamplingParams from GenerateConfig fields.
...
/// Factory: create sampler from FalconSamplingParams.
...
std::unique_ptr<FalconISampler> create_falcon_sampler(const FalconSamplingParams& params,
                                                      const FalconSamplerFactoryOptions& options);
