...
"""Focused tests for Phi-MoE tensor-parallel support."""
...
try:
    phi_moe_module = importlib.import_module(
        "tensorrt_model_connect.models.phi_moe.model")
    from tensorrt_model_connect.models.phi_moe import tp_builder
...
def _weights(top_k: int = 2) -> dict:
    ...
    weights: dict[str, object] = {
        ...
        "lm_head_bias": np.zeros((32,), dtype=np.float32),
        "layer.0.input_norm": np.ones((hidden,), dtype=np.float32),
        "layer.0.input_norm_beta": np.zeros((hidden,), dtype=np.float32),
        "layer.0.post_attn_norm": np.ones((hidden,), dtype=np.float32),
        "layer.0.post_attn_norm_beta": np.zeros((hidden,), dtype=np.float32),
...
def test_phi_moe_tp_slices_attention_biases_and_experts():
    parallel = ParallelConfig(mode="tensor_parallel", tp_size=4, rank=2)
    weights = _weights()

    sharded = tp_builder.shard_phi_moe_weights(
...
def test_phi_moe_tp_validation_rejects_non_divisible_kv_heads():
    with pytest.raises(ValueError, match="num_key_value_heads"):
        tp_builder._validate_phi_moe_tp(
...
def test_phi_moe_tp_validation_rejects_non_top2_sparsemixer():
...
def test_phi_moe_plugin_routes_parallel_builds(monkeypatch):
    ...
    def fake_build(config, weights, max_cache_length, **kwargs):
        ...
        return b"phi-moe-tp-plan"

    monkeypatch.setattr(
        phi_moe_module, "require_tensorrt_11_for_tensor_parallel", fake_require)
    ...
    assert result == b"phi-moe-tp-plan"
    assert calls["require"][0] == parallel
    assert "Phi-MoE tensor-parallel" in calls["require"][1]
