...
def build_standard_decoder_engine(config: ModelConfig, weights: WeightDict, max_cache_length: int, *, precision: str='fp32', quant_ctx: QuantContext | None=None, norm_type: str='rmsnorm', mlp_type: str='swiglu', position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, parallel_residual: bool=False, scale_attn_weights: bool=True, embed_input: bool=False, verbose: bool=False, debug_layer_outputs: bool=False, hidden_state_output: bool=False) -> bytes:
    ...
    if position_type == 'rope':
        ...
        cos_half_np = graph_ops.make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, True, partial_rotary_factor, interleaved=True)
        sin_half_np = graph_ops.make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, False, partial_rotary_factor, interleaved=True)
...
def _add_decoder_layer(*, network: trt.INetworkDefinition, hidden: trt.ITensor, cache_k: trt.ITensor, cache_v: trt.ITensor, attention_mask: trt.ITensor, position_id: trt.ITensor, attention_scale: float | None, eps_tensor: trt.ITensor, weights: WeightDict, prefix: str, hidden_size: int, attention_size: int, kv_attention_size: int, mlp_size: int, num_heads: int, num_kv_heads: int, head_dim: int, max_cache_length: int, norm_type: str='rmsnorm', mlp_type: str='swiglu', position_type: str='rope', activation: str='silu', parallel_residual: bool=False, alibi_slopes_tensor: trt.ITensor | None=None, alibi_indices_tensor: trt.ITensor | None=None, dtype: np.dtype=np.float32, quant_ctx: QuantContext | None=None, cos_half_tensor: trt.ITensor | None=None, sin_half_tensor: trt.ITensor | None=None, rotary_embedding_dim: int=0, ffi_attention_kernel: str | None=None, dynamic_kv_cache: bool=False, eps: float | None=None) -> dict[str, trt.ITensor]:
    ...
    if parallel_residual:
        ...
        if post_attn_norm_w is not None:
            norm2 = _apply_norm(network, hidden, hidden_size, post_attn_norm_w, weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, norm_type, dtype=dtype, eps=eps)
        else:
            norm2 = attn['normed']
    else:
        residual1 = network.add_elementwise(hidden, attn_out, trt.ElementWiseOperation.SUM)
        norm2 = _apply_norm(network, residual1.get_output(0), hidden_size, weights[f'{prefix}.post_attn_norm'], weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, norm_type, dtype=dtype, eps=eps)
    if mlp_type == 'gelu_fc':
