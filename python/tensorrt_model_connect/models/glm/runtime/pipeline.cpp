...
std::vector<GlmTextGenerationPipeline::DecoderContext>
single_decoder_context(std::unique_ptr<TrtModule> decoder) {
    std::vector<GlmTextGenerationPipeline::DecoderContext> decoders;
    decoders.push_back(GlmTextGenerationPipeline::DecoderContext{0, std::move(decoder)});
    return decoders;
}
...
bool greedy_text_diffusion_params(const GlmSamplingParams& params) {
...
GlmTextGenerationPipeline::GlmTextGenerationPipeline(std::unique_ptr<TrtModule> decoder,
                                                     std::unique_ptr<GlmInferenceState> state,
    ...
    : GlmTextGenerationPipeline(single_decoder_context(std::move(decoder)), std::move(state),
...
GlmTextGenerationPipeline::GlmTextGenerationPipeline(
    std::vector<DecoderContext> decoders, std::unique_ptr<GlmInferenceState> state,
    GlmTextGenConfig config, cudaStream_t stream, std::shared_ptr<ITokenizer> tokenizer,
    std::string model_id_str, std::unique_ptr<GlmISampler> sampler,
    ...
    if (decoders_.empty()) {
        throw std::runtime_error("GlmTextGenerationPipeline: no decoder modules");
    }
    for (const auto& decoder_ctx : decoders_) {
        if (!decoder_ctx.module || !decoder_ctx.module->ok()) {
            throw std::runtime_error("GlmTextGenerationPipeline: invalid decoder module");
        }
    }
    if (!state_ || !state_->ok()) {
        throw std::runtime_error("GlmTextGenerationPipeline: invalid inference state");
    }
...
static std::vector<int32_t> encode_prompt(const ITokenizer& tokenizer,
                                          const GlmTextGenConfig& config, const std::string& prompt,
                                          const GenerateConfig& cfg) {
    std::string effective = prompt;
    bool templated = false;
    if (cfg.use_chat_template && !config.chat_template_format.empty()) {
        effective =
            glm_apply_chat_template(config.chat_template_format, prompt, cfg.enable_thinking);
        templated = true;
    }
...
GlmTextGenerationPipeline::make_step_sampler(const GlmSamplingParams& params) {
    ...
    if (prefer_gpu_greedy_ && greedy_params) {
        if (auto gpu = create_glm_gpu_greedy_sampler(stream_))
            return gpu;
    }
...
namespace {
bool gather_prefill_kv_pointers(TrtModule& prefill, const GlmTextGenConfig& cfg,
    ...
    for (int32_t i = 0; i < cfg.num_layers; ++i) {
        ...
        pk[li] = prefill.device_ptr(glm_expand_layer_name(cfg.present_k_pattern, i));
        pv[li] = prefill.device_ptr(glm_expand_layer_name(cfg.present_v_pattern, i));
...
bool batched_prefill_supported(const TrtModule* prefill, const GlmTextGenConfig& cfg, int32_t sq,
                               GlmInferenceState* state) {
    ...
    if (cfg.num_layers <= 0 || cfg.vocab_size <= 0)
        return false;
    return dynamic_cast<GlmKvCache*>(state) != nullptr;
}
} // namespace

bool GlmTextGenerationPipeline::run_prefill_batched(const std::vector<int32_t>& input_ids,
                                                    std::vector<float>& logits) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    if (!batched_prefill_supported(prefill_.get(), config_, sq, state_.get()))
        return false;
    auto* kv = static_cast<GlmKvCache*>(state_.get());
...
void GlmTextGenerationPipeline::prime_decoder_after_batched_prefill(
...
TrtModule& GlmTextGenerationPipeline::require_block_prefill(int32_t sq,
                                                            TrtModule* prefill_override) {
    TrtModule* prefill = prefill_override != nullptr ? prefill_override : prefill_.get();
    if (prefill == nullptr)
        throw std::runtime_error(
            "GlmTextGenerationPipeline: block generation requires prefill module");
    if (sq <= 0)
        throw std::runtime_error("GlmTextGenerationPipeline: empty block");
    if (config_.prefill_max_length > 0 && sq > config_.prefill_max_length) {
        throw std::runtime_error("GlmTextGenerationPipeline: block length exceeds prefill profile");
    }
    return *prefill;
}

GlmKvCache& GlmTextGenerationPipeline::require_block_kv_cache() {
    auto* kv = dynamic_cast<GlmKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error("GlmTextGenerationPipeline: block generation requires GlmKvCache");
    return *kv;
}

void GlmTextGenerationPipeline::copy_block_logits(const TensorMap& outputs,
                                                  std::vector<float>& logits) const {
    auto logits_it = outputs.find(config_.logits_output_name);
    if (logits_it == outputs.end())
        throw std::runtime_error("GlmTextGenerationPipeline: prefill module has no '" +
...
void GlmTextGenerationPipeline::append_prefill_kv(GlmKvCache& kv, TrtModule& prefill, int32_t sq) {
    std::vector<const void*> pk, pv;
    if (!gather_prefill_kv_pointers(prefill, config_, pk, pv)) {
        throw std::runtime_error(
            "GlmTextGenerationPipeline: prefill module is missing present_k/present_v outputs");
    }
    kv.append_prefill_kv(pk, pv, sq);
}

void GlmTextGenerationPipeline::run_prefill_block(const std::vector<int32_t>& input_ids,
                                                  bool bidirectional, bool append_kv,
                                                  std::vector<float>& logits,
                                                  TrtModule* prefill_override) {
    const auto sq = static_cast<int32_t>(input_ids.size());
    TrtModule& prefill = require_block_prefill(sq, prefill_override);
    GlmKvCache& kv = require_block_kv_cache();
...
std::string GlmTextGenerationPipeline::resolve_generation_mode(const GenerateConfig& cfg) const {
...
int32_t GlmTextGenerationPipeline::resolve_text_diffusion_block_length(
    ...
    if (!config_.supports_text_diffusion || config_.mask_token_id < 0)
        throw std::runtime_error(
            "GlmTextGenerationPipeline: bundle does not support text diffusion");
    ...
    if (require_divisible && max_new_tokens % block_len != 0) {
        throw std::runtime_error("GlmTextGenerationPipeline: diffusion mode requires "
...
int32_t
GlmTextGenerationPipeline::seed_next_token_from_prefill(const std::vector<int32_t>& input_ids,
    ...
    if (static_cast<int32_t>(logits.size()) < vocab)
        throw std::runtime_error("GlmTextGenerationPipeline: missing prefill logits");
    return argmax_with_confidence(logits.data() + logits.size() - static_cast<std::size_t>(vocab),
                                  vocab, 0)
        .token_id;
}

void GlmTextGenerationPipeline::fill_diffusion_block(std::vector<int32_t>& block,
    ...
    for (int32_t step = 0; step < block_len && has_mask_token(block, config_.mask_token_id);
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "GlmTextGenerationPipeline: diffusion engine must output full block logits");
        }
...
int32_t GlmTextGenerationPipeline::verify_diffusion_block(const std::vector<int32_t>& block,
                                                          std::vector<float>& logits,
                                                          int32_t block_len, int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "GlmTextGenerationPipeline: diffusion engine must output full verify logits");
    }
...
bool GlmTextGenerationPipeline::append_tokens_until_eos(const std::vector<int32_t>& tokens,
                                                        std::vector<int32_t>& output,
                                                        const GlmSamplingParams& params) const {
...
void GlmTextGenerationPipeline::fill_linear_spec_block(std::vector<int32_t>& block,
                                                       std::vector<float>& logits,
                                                       int32_t block_len, int32_t vocab,
    ...
    while (has_mask_token(block, config_.mask_token_id)) {
        ...
        if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
            throw std::runtime_error(
                "GlmTextGenerationPipeline: linear_spec engine must output full block logits");
        }
        const auto preds = masked_predictions(logits, block, config_.mask_token_id, vocab);
...
std::vector<int32_t>
GlmTextGenerationPipeline::verify_linear_spec_block(const std::vector<int32_t>& block,
                                                    std::vector<float>& logits, int32_t block_len,
                                                    int32_t vocab) {
    run_prefill_block(block, /*bidirectional=*/false, /*append_kv=*/true, logits);
    if (static_cast<int32_t>(logits.size()) < block_len * vocab) {
        throw std::runtime_error(
            "GlmTextGenerationPipeline: linear_spec engine must output full verify logits");
    }
...
int32_t GlmTextGenerationPipeline::count_linear_spec_accepts(const std::vector<int32_t>& ar_tokens,
                                                             const std::vector<int32_t>& block) {
    if (ar_tokens.empty())
        return 0;
    if (block.size() < 2)
...
bool GlmTextGenerationPipeline::append_linear_spec_tokens(const std::vector<int32_t>& ar_tokens,
                                                          int32_t emit_count,
                                                          std::vector<int32_t>& output,
                                                          int32_t& generated,
                                                          const GlmSamplingParams& params) const {
...
GlmTextGenerationPipeline::TimedGenResult GlmTextGenerationPipeline::generate_diffusion_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens, const GlmSamplingParams& params,
    const GenerateConfig& cfg) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error(
            "GlmTextGenerationPipeline: diffusion mode currently supports greedy temperature=0 "
            "generation");
    }
    const int32_t block_len =
...
GlmTextGenerationPipeline::TimedGenResult GlmTextGenerationPipeline::generate_linear_spec_from_ids(
    const std::vector<int32_t>& input_ids, int32_t max_new_tokens, const GlmSamplingParams& params,
    const GenerateConfig& cfg, bool use_lora_draft) {
    using Clock = std::chrono::steady_clock;
    if (!greedy_text_diffusion_params(params)) {
        throw std::runtime_error(
            "GlmTextGenerationPipeline: linear_spec mode currently supports greedy temperature=0 "
            "generation");
    }
    if (use_lora_draft && linear_spec_lora_prefill_ == nullptr) {
        throw std::runtime_error(
            "GlmTextGenerationPipeline: linear_spec_lora mode requires a linear-spec LoRA engine");
    }
    const int32_t block_len =
    ...
    auto* kv = dynamic_cast<GlmKvCache*>(state_.get());
    if (kv == nullptr)
        throw std::runtime_error("GlmTextGenerationPipeline: linear_spec requires GlmKvCache");
...
void GlmTextGenerationPipeline::log_decode_summary(int32_t steps, double ms) const {
...
int32_t
GlmTextGenerationPipeline::run_decode_loop(GlmISampler* sampler, const GlmSamplingParams& params,
    ...
    for (int32_t step = 0; step < max_new_tokens; ++step) {
        ...
        const GlmSampleResult result = sampler->sample(sample_ptr, vocab_size, params);
...
int32_t GlmTextGenerationPipeline::select_decoder_index(int32_t desired_rows) const {
    if (decoders_.size() == 1)
        return 0;
...
TrtModule& GlmTextGenerationPipeline::bind_decoder_for_step() {
...
void GlmTextGenerationPipeline::run_step(int32_t token_id, std::vector<float>& logits) {
    ...
    if (it == outputs.end()) {
        throw std::runtime_error("GlmTextGenerationPipeline: no '" + logits_output_name_ +
                                 "' output");
    }
