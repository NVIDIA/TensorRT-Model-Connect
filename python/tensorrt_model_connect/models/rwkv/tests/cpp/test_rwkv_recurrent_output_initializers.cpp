...
namespace under_test = trtmc::rwkv_recurrent;
...
void test_rwkv_recurrent_contracts() {
    ...
    check(under_test::validate_state_layer_count(ok_states, 2),
          "rwkv contract accepts matching layer count");
    check(!under_test::validate_state_layer_count(bad_layer_states, 2),
          "rwkv contract rejects layer count mismatch");
    ...
    check(under_test::validate_state_tensor_sizes(ok_specs, 2),
          "rwkv contract accepts matching tensor sizes");
    check(!under_test::validate_state_tensor_sizes(bad_specs, 2),
          "rwkv contract rejects tensor size mismatch");

    std::vector<std::vector<float>> outputs;
    under_test::initialize_layer_outputs(3, 2, outputs);
    check(outputs.size() == 3, "rwkv contract allocates layer outputs");
    check(outputs[0] == std::vector<float>({0.0F, 0.0F}),
          "rwkv contract initializes outputs to zero");
}
