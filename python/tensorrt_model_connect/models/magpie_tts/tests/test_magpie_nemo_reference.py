...
from tensorrt_model_connect.models.magpie_tts.tests.e2e_plugins.references.nemo_reference import (
...
def test_reference_profile_matches_pinned_magpie_checkpoint() -> None:
    family_root = Path(__file__).resolve().parents[5] / (
        "python/tensorrt_model_connect/models/magpie_tts"
    )
    lock = (
        family_root
...
def test_magpie_reference_maps_upstream_url_to_pre_warmed_file(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        "tensorrt_model_connect.models.magpie_tts.tests.e2e_plugins.references.nemo_reference.subprocess.run",
        fake_run,
    )
