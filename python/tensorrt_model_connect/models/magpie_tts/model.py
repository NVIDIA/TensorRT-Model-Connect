...
"""Magpie TTS family-owned build implementation.
...
def load_weights(model_dir: str, config: ModelConfig, *, precision: str = "fp32") -> WeightDict:
    ...
    global _audio_config, _nemo_path
    weights = WeightDict()
    ...
    for i in range(enc_layers):
        ...
        w_q, w_k, w_v = _split_fused_qkv(state_dict[f"{src}.self_attention.qkv_net.weight"], hidden)
    ...
    for i in range(dec_layers):
        ...
        weights[f"{pfx}.norm_xattn_query"] = _to_np(state_dict[f"{src}.norm_xattn_query.weight"])
        weights[f"{pfx}.norm_xattn_memory"] = _to_np(state_dict[f"{src}.norm_xattn_memory.weight"])
    ...
    if "baked_context_embedding.weight" in state_dict:
        weights["baked_context_embedding"] = _to_np(state_dict["baked_context_embedding.weight"])
    ...
    try:
        ...
        codec_keys = {
            ...
            and (k.startswith("codec.audio_decoder.") or k.startswith("codec.vector_quantizer."))
        }
        if codec_keys:
...
def build_engine(
    ...
    *,
    ...
    if precision == "fp16":
        ...
        raise ValueError(f"Unsupported MagpieTTS precision {precision!r}; expected fp32 or fp16")
    ...
    pos_table = graph_ops.add_constant(network, dec_pos_np.shape, dec_pos_np, dtype=work_np_dtype)
...
def build_extra_engines(
    ...
    *,
    ...
    if "_lt_hidden" in weights:
        ...
        result["lt_out_projections"] = np.concatenate(out_proj_parts).astype(np.float32).tobytes()
    ...
    if _nemo_path:
        try:
            ipa_assets = _extract_ipa_assets(_nemo_path)
...
def get_audio_config(config: ModelConfig) -> dict:
    """Return audio config dict injected into bundle config.json."""
    return _audio_config.copy()
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete magpie_tts bundle in the owning family module."""
    import json
    import time
