...
// MagpieInferenceState: unified interface for autoregressive inference state.
//
...
//
// Implementations:
//   MagpieKvCache          — dense append-only (current default)
//   Family-owned recurrent state — recurrent tensor state
//   Family-owned hybrid state — MagpieKvCache + family-owned recurrent state composed
...
class MagpieInferenceState {
  public:
    virtual ~MagpieInferenceState() = default;
    ...
    // MagpieKvCache -> true. Family-owned recurrent state -> false.
