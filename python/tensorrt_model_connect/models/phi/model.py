...
"""Phi-3 family model — handles fused qkv_proj and gate_up_proj splitting."""
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete phi bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("phi does not support context-parallel builds")
    ...
    if parallel.enabled:
        ...
        if dynamic_kv_cache:
            raise NotImplementedError(
                "Tensor-parallel phi builds do not support dynamic KV cache or TriAttention"
            )
