...
#pragma once
// Z-Image-owned CPU math helpers.
...
namespace trtmc {
namespace z_image_math {

/// CPU matrix multiply: out[M,N] = A[M,K] * B[K,N] + bias[N]
inline void cpu_matmul_bias(
...
inline void fill_sinusoidal_embedding(
    float value, int32_t freq_dim, std::vector<float>& embedding)
{
...
inline void compute_timestep_mlp(
    float timestep,
    int32_t freq_dim,
    int32_t dim,
    ...
    const std::vector<float>& emb_2_bias,
    std::vector<float>& output)
{
    ...
    cpu_matmul_bias(sinusoidal.data(), emb_0_weight.data(), emb_0_bias.data(),
                    hidden.data(), 1, freq_dim, dim);
    ...
    cpu_matmul_bias(hidden.data(), emb_2_weight.data(), emb_2_bias.data(),
                    output.data(), 1, dim, dim);
}
