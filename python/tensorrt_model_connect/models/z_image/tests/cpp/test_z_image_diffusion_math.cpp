...
// test_z_image_diffusion_math.cpp — Unit tests for
//   python/tensorrt_model_connect/models/z_image/runtime/diffusion_math.h
//
// Purpose:
//   Validates the inline CPU math helpers used by the Z-Image pipeline:
...
//
// Dependencies:
//   - diffusion_math.h  (header-only, CPU)
//   No TRT, CUDA, or GPU required.

#include "diffusion_math.h"
...
static bool test_matmul_1x1_with_bias() {
    ...
    trtmc::z_image_math::cpu_matmul_bias(A, B, bias, out, 1, 1, 1);
...
static bool test_matmul_2x2_no_bias() {
    ...
    trtmc::z_image_math::cpu_matmul_bias(A, B, nullptr, out, 2, 2, 2);
...
static bool test_silu_values() {
    ...
    trtmc::z_image_math::cpu_silu_inplace(data, 3);
...
static bool test_silu_empty() {
    trtmc::z_image_math::cpu_silu_inplace(nullptr, 0);
    return true;
}
...
static bool test_gelu_tanh_values() {
    ...
    trtmc::z_image_math::cpu_gelu_tanh_inplace(data, 3);
...
static bool test_sinusoidal_embedding_zero_value() {
    ...
    trtmc::z_image_math::fill_sinusoidal_embedding(0.0F, 4, emb);
...
static bool test_sinusoidal_embedding_range() {
    std::vector<float> emb;
    trtmc::z_image_math::fill_sinusoidal_embedding(3.14F, 8, emb);
...
static bool test_timestep_mlp_output_size() {
    ...
    trtmc::z_image_math::compute_timestep_mlp(1.0F, freq_dim, dim, w0, b0, w2, b2, output);
...
static bool test_timestep_mlp_zero_weights() {
    ...
    std::vector<float> output;
    trtmc::z_image_math::compute_timestep_mlp(5.0F, freq_dim, dim, zeros_4x2, zeros_2, zeros_2x2,
                                              zeros_2, output);
...
int main() {
    ...
    std::cout << "test_z_image_diffusion_math:" << std::endl;
    ...
    if (all_passed) {
        std::cout << "test_z_image_diffusion_math passed" << std::endl;
        return 0;
    }
    std::cerr << "test_z_image_diffusion_math FAILED" << std::endl;
    return 1;
}
