...
"""BLOOM family model — ALiBi positions + fused QKV + embedding LayerNorm.
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete bloom bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("bloom does not support context-parallel builds")
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="bloom tensor-parallel builds")

        if quant_ctx is not None:
            raise ValueError("bloom tensor-parallel builds do not support quantization")
        if dynamic_kv_cache:
            raise NotImplementedError(
                "Tensor-parallel bloom builds do not support dynamic KV cache or TriAttention"
            )
