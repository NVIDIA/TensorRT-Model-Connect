...
"""TensorRT graph builders for timm Inception-ResNet classifiers.

The residual blocks concatenate several convolution branches, project the
result with a biased 1x1 convolution that has no norm, scale it by a constant,
and add it back to the block input. That needs concatenation, a constant scale,
and an average pool that keeps its spatial size.
"""
...
def add_avg_pool2d(network, x, kernel: int, stride: int, padding: int,
    ...
    """Average pool; Inception-ResNet excludes the zero padding."""
    pool = network.add_pooling_nd(x, trt.PoolingType.AVERAGE, (kernel, kernel))
    pool.stride_nd = (stride, stride)
    pool.padding_nd = (padding, padding)
    pool.average_count_excludes_padding = not count_include_pad
    return pool.get_output(0)


def add_constant_scale(network, x, factor: float, *, dtype=np.float32):
    """Multiply by a constant. The residual scale is architecture, not a weight."""
    layer = network.add_scale(
        x,
