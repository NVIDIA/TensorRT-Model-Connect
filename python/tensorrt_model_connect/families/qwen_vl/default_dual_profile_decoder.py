...
def build_dual_profile_decoder_engine(
    ...
    * ``"prefill"``: one prefill profile only. This is used by split-engine
    ...
    mrope_section = (
        ...
        if raw_mrope_section is not None
        else None
    )
    if mrope_section is not None:
        ...
        if sum(mrope_section) != rotary_embedding_dim // 2:
            raise ValueError(
                f"rope_scaling.mrope_section must sum to half the rotary dimension; got {mrope_section} for rotary dimension {rotary_embedding_dim}"
            )
    ...
    if input_embed_tensor is not None and use_input_embed_tensor is not None:
        ...
        inverse_selector = network.add_elementwise(
            one, embed_selector, trt.ElementWiseOperation.SUB
        ).get_output(0)
        token_part = network.add_elementwise(
            inverse_selector, token_embed, trt.ElementWiseOperation.PROD
        ).get_output(0)
        embed_part = network.add_elementwise(
            embed_selector, input_embed, trt.ElementWiseOperation.PROD
        ).get_output(0)
    ...
    if verbose:
        ...
        print(
            f"[trtmc build] Building {mode_label} engine (layers={num_layers}, hidden={hidden}, attn={attention_size}, kv={kv_attention_size}, mlp={mlp_size}, cache={max_cache_length}, opt_prefill={opt_prefill_length}, max_prefill={max_prefill_length}, norm={norm_type}, mlp_type={mlp_type}, pos={position_type}, precision={precision}) ...",
            file=sys.stderr,
        )
