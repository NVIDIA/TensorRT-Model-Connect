...
def build_qwen_vl_tp_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp32', quant_ctx=None, norm_type: str='rmsnorm', mlp_type: str='swiglu', position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, deepstack_num_levels: int=0, verbose: bool=False, debug_layer_outputs: bool=False, parallel_config=None) -> bytes:
    ...
    if deepstack_num_levels > 0:
        for level in range(deepstack_num_levels):
            deepstack_embed_inputs.append(network.add_input(f'deepstack_embed_{level}', trt.float32, (1, hidden)))
        deepstack_active_tensor = network.add_input('deepstack_active', trt.float32, (1,))
    cache_k_inputs = []
    cache_v_inputs = []
    ...
    if input_embed_tensor is not None and use_input_embed_tensor is not None:
        ...
        tok_part = network.add_elementwise(inv_flag, token_embed_for_math, trt.ElementWiseOperation.PROD).get_output(0)
        ...
        embed_part = network.add_elementwise(flag_for_math, input_embed_for_math, trt.ElementWiseOperation.PROD).get_output(0)
    ...
    if verbose:
        print(f'[trtmc build] Building Qwen-VL TP rank {parallel.rank}/{parallel.tp_size} ({num_layers}L, h={hidden}, local_heads={num_heads}, local_attn={attention_size}, local_mlp={mlp_size}, cache={max_cache_length}, precision={precision}) ...', file=sys.stderr)
