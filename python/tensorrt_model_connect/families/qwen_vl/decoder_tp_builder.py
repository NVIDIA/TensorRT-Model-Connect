    ...
    *,
    ...
    for layer_idx in range(num_layers):
        ...
        result = _add_tp_decoder_layer(
            ...
            deepstack_embed=deepstack_embed_inputs[layer_idx]
    ...
    if verbose:
        print(
            f"[trtmc build] Building Qwen-VL TP rank {parallel.rank}/{parallel.tp_size} ({num_layers}L, h={hidden}, local_heads={num_heads}, local_attn={attention_size}, local_mlp={mlp_size}, cache={max_cache_length}, precision={precision}) ...",
            file=sys.stderr,
        )
        ...
        if deepstack_embed is not None and deepstack_active is not None:
            ...
            if deepstack_for_math.dtype != residual1.dtype:
                deepstack_for_math = network.add_cast(
                    deepstack_for_math, residual1.dtype
                ).get_output(0)
            ...
            residual1 = network.add_elementwise(
                residual1, ds_scaled, trt.ElementWiseOperation.SUM
            ).get_output(0)
        norm2 = _apply_norm(
            network,
            residual1,
