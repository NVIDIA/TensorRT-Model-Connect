...
def add_apply_mrope_native(
    ...
    return add_apply_rope_native_sequence(
        ...
        num_heads,
        head_dim,
        build_cache(cos_cache_2d),
        build_cache(sin_cache_2d),
        rotary_embedding_dim,
        interleaved,
        sequence_length=1,
    )
    ...
    def build_cache(cache: trt.ITensor) -> trt.ITensor:
        ...
        for axis, width in enumerate(sections):
            ...
            column_indices = add_constant(
                network, (width,), np.arange(offset, offset + width, dtype=np.int32), dtype=np.int32
            )
...
def _add_decomposed_attention_core(
    ...
    *,
    ...
    if logit_softcap is not None and float(logit_softcap) > 0.0:
        scores = add_tanh_softcap(network, scores, float(logit_softcap), scalar_shape=(1, 1, 1, 1))
...
def add_attention_from_rows(
    ...
    *,
    ...
    if scale is None:
        scale = float(1.0 / np.sqrt(head_dim)) if head_dim > 0 else 1.0
    use_decomposed_attention = logit_softcap is not None and float(logit_softcap) > 0.0
    if use_decomposed_attention:
        if causal:
            raise NotImplementedError("decomposed attention requires an explicit additive mask")
