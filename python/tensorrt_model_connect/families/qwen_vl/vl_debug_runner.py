...
class TrtRunner:
    ...
    def __init__(
        ...
        for i in range(self.engine.num_io_tensors):
            ...
            if name == "input_embed":
                ...
                embed_shape = _profile_min_shape(self.engine, name, self._profile_index)
                ...
                selector_shape = _profile_min_shape(self.engine, name, self._profile_index)
                self._h_use_input_embed = np.zeros(selector_shape, dtype=np.float32)
                err, self._d_use_input_embed = cudart.cudaMalloc(self._h_use_input_embed.nbytes)
                _check_cuda(err)
        ...
        for i in range(self.engine.num_io_tensors):
            ...
            if name.startswith("deepstack_embed_"):
                ...
                shape = _profile_min_shape(self.engine, name, self._profile_index)
                ...
                active_shape = _profile_min_shape(self.engine, name, self._profile_index)
                self._h_deepstack_active = np.zeros(active_shape, dtype=np.float32)
                err, self._d_deepstack_active = cudart.cudaMalloc(self._h_deepstack_active.nbytes)
                _check_cuda(err)
    ...
    def step(
        ...
        if self._has_embed_input:
            ...
            cudart.cudaMemcpyAsync(
                self._d_use_input_embed,
                self._h_use_input_embed.ctypes.data,
    ...
    *,
    ...
    if config is None or header is None or engine_plan is None:
        if bundle_path is None:
            raise TypeError("bundle_path is required when parsed Qwen-VL data is absent")
    ...
    return TrtRunner(
        ...
        num_layers=int(header.get("num_layers", config.get("num_hidden_layers", 1))),
