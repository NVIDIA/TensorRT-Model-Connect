...
"""Load Marian checkpoint tensors in the layout its TRT builders consume."""
...
def _transpose_2d(array: np.ndarray, name: str) -> np.ndarray:
    """Transpose a Marian projection from HF to TRT layout."""
    if array.ndim != 2:
...
class WeightDict(dict):
    """Marian encoder/decoder weights in TRT projection layout."""
...
class _Readers(list):
    def __init__(self, readers: list, tensor_map: dict[str, object] | None = None):
        ...
        self.tensor_map = tensor_map or {
            key: reader for reader in readers for key in reader.keys()
        }


def _open_safetensors(model_dir: Path) -> _Readers:
    """Open a single/sharded safetensors or legacy Marian PyTorch checkpoint."""
    ...
    if pytorch_bin.exists():
        return _Readers([_TorchBinReader(pytorch_bin)])
