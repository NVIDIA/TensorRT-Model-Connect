...
def _validate_marian_tp(
    ...
    if hidden % tp != 0:
        raise ValueError(
            f"Marian tensor parallel requires hidden size divisible by tp_size ({hidden} vs {tp})"
        )
    if dec_heads % tp != 0:
        raise ValueError(
...
def build_marian_tp_decoder_engine(
    ...
    *,
    ...
    logits = graph_ops.add_bias_sum(network, logits, vocab, rank_weights["final_logits_bias"])
    logits.name = "logits"
