...
"""TensorRT graph builders for timm SENet classifiers.

The SENet stack is convolutional, so it needs a different op set from the
...
def add_sigmoid(network, x):
    return network.add_activation(x, trt.ActivationType.SIGMOID).get_output(0)
