...
def add_attention_core(
    ...
    """Add MPNet's bidirectional scaled dot-product attention."""
...
def add_attention_from_rows(
    ...
    *,
    ...
    """Add MPNet attention for row-major Q/K/V tensors."""
...
def _validate_encoder_tp(
    ...
    if (
        "relative_position_bias" in weights
        and weights["relative_position_bias"].shape[0] % tp_size != 0
    ):
...
def shard_encoder_weights(
    ...
    *,
    ...
    """Return rank-local MPNet encoder weights."""
    ...
    for key, value in weights.items():
        ...
        elif key.endswith((".w_o", ".w_fc2")) or key == "relative_position_bias":
            out[key] = _slice_first_dim(value, parallel.rank, parallel.tp_size)
        else:
            out[key] = value
...
def build_encoder_engine(
    ...
    *,
    ...
    """Build a single-device or rank-local tensor-parallel MPNet engine."""
    ...
    if precision == "fp16":
        ...
        raise ValueError(f"Unsupported MPNet precision: {precision}")
    ...
    ones_mask = add_constant(
        network,
        (1,),
        np.array([1.0], dtype=work_np_dtype),
        dtype=work_np_dtype,
    )
    ...
    if "relative_position_bias" in weights:
        ...
        rel_pos_bias_tensor = add_constant(network, rpb.shape, rpb, dtype=work_np_dtype)
    ...
    if verbose:
        print(
            f"[trtmc build] Building MPNet encoder TRT engine "
...
def build_tp_encoder_engine(
    ...
    *,
    ...
    """Build one rank-local MPNet engine."""
    parallel = normalize_parallel_config(parallel_config)
    if not parallel.enabled:
...
def _add_encoder_layer(
    *,
    ...
    """Add one bidirectional MPNet encoder layer with post-norm."""
    attention_size = num_heads * head_dim
    ...
    # applies scaling internally through add_attention_from_rows, so
