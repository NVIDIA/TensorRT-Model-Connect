...
"""TensorRT graph builders for timm EfficientNet classifiers.

EfficientNet adds two things to the plain convolutional op set: the SiLU (swish)
activation and a squeeze-and-excite gate. Unlike MobileNetV3 the gate uses a
plain sigmoid and the inner activation is SiLU, so the two families keep their
own copies rather than sharing one parameterised helper.
"""
...
def add_silu(network, x):
    """SiLU / swish: x * sigmoid(x)."""
    gate = network.add_activation(x, trt.ActivationType.SIGMOID).get_output(0)
...
def add_squeeze_excite(
    ...
    *,
    ...
    Spatial mean, a 1x1 reduce convolution with SiLU, a 1x1 expand convolution,
    then a sigmoid gate multiplied back into x. The pooled branch broadcasts
    ...
    reduced = add_silu(network, reduced)
    expanded = add_conv2d(
