...
"""timm EfficientNet image-classification family plugin.
...
def _discover_layout(readers) -> dict:
    ...
    if strides is None:
        raise ValueError(
            f"No EfficientNet stride schedule for {len(stages)} stages")
...
class TimmEfficientnetPlugin:
    name = "timm_efficientnet"
    runtime_strategy = "timm_efficientnet_image_classification"
    requires_tokenizer = False

    def matches(self, model_type: str) -> bool:
        mt = (model_type or "").lower()
        if mt == "timm_efficientnet":
            return True
        return mt.startswith("efficientnet")
        ...
        *,
        ...
        if quant_ctx is not None:
            raise NotImplementedError(
                "timm_efficientnet does not support quantized builds yet")
        if parallel_config is not None and getattr(parallel_config, "enabled", False):
            raise NotImplementedError(
                "timm_efficientnet does not support tensor-parallel builds")
        ...
        if image_h % total_stride != 0 or image_w % total_stride != 0:
            raise ValueError(
                f"timm_efficientnet input {image_h}x{image_w} must be divisible "
                f"by {total_stride}")
        ...
        hidden = graph_ops.add_conv2d(
            network, hidden, head_w, None, int(head_w.shape[0]), (1, 1),
            dtype=work_np_dtype)
