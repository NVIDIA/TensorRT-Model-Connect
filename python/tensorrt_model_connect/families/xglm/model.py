...
"""XGLM family model — sinusoidal positions + GELU FC MLP.
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete xglm bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("xglm does not support context-parallel builds")
    ...
    if parallel.enabled:
        ...
        if dynamic_kv_cache:
            raise NotImplementedError(
                "Tensor-parallel xglm builds do not support dynamic KV cache or TriAttention"
            )
