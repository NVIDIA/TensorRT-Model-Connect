...
"""Family-owned TensorRT graph builders for XGLM decoders."""
...
def add_gelu(
    ...
    """Apply XGLM's tanh-approximated GELU in the input tensor dtype."""
    shape = (1,) * max(1, len(tuple(inp.shape)))

    def constant(value: float) -> trt.ITensor:
        tensor = add_constant(network, shape, np.array([value]), dtype=dtype)
        return _cast_back_to_trt_dtype(network, tensor, inp.dtype)

    squared = network.add_elementwise(inp, inp, trt.ElementWiseOperation.PROD)
    cubed = network.add_elementwise(squared.get_output(0), inp, trt.ElementWiseOperation.PROD)
    cubic_term = network.add_elementwise(
        cubed.get_output(0), constant(0.044715), trt.ElementWiseOperation.PROD
    )
    inner = network.add_elementwise(inp, cubic_term.get_output(0), trt.ElementWiseOperation.SUM)
    scaled = network.add_elementwise(
        inner.get_output(0), constant(float(np.sqrt(2.0 / np.pi))), trt.ElementWiseOperation.PROD
    )
    tanh = network.add_activation(scaled.get_output(0), trt.ActivationType.TANH)
    shifted = network.add_elementwise(
        tanh.get_output(0), constant(1.0), trt.ElementWiseOperation.SUM
    )
    half_x = network.add_elementwise(inp, constant(0.5), trt.ElementWiseOperation.PROD)
    return network.add_elementwise(
        half_x.get_output(0), shifted.get_output(0), trt.ElementWiseOperation.PROD
    ).get_output(0)
...
#
# Native LayerNorm and attention replace their manual primitive chains.
# ---------------------------------------------------------------------------
...
def add_layer_norm_native(
    ...
    INormalizationLayer computes the mean-centered LayerNorm used by XGLM.
...
def add_attention_core(
    ...
    """Apply scaled, additively masked TensorRT attention."""
...
def add_attention_from_rows(
    ...
    *,
    ...
    """Apply XGLM self-attention to row-major Q/K/V tensors."""
...
def apply_norm(
    ...
    """Apply XGLM LayerNorm, using the native TRT layer when possible."""
    if beta is None:
        raise ValueError("XGLM LayerNorm requires beta weights")
    if eps is not None:
...
def add_attention_block(
    ...
    *,
    ...
    """Add XGLM pre-norm attention without applying the residual."""
    ...
    k = matmul(normed, hidden_size, attention_size, weights[f"{prefix}.w_k"], f"{_lp}.w_k")
    v = matmul(normed, hidden_size, attention_size, weights[f"{prefix}.w_v"], f"{_lp}.w_v")
...
def add_gelu_fc_mlp(
    ...
    *,
    ...
    """Add XGLM's biased ``fc1 -> GELU -> fc2`` MLP."""
    matmul = _make_matmul_fn(network, dtype, quant_ctx)
    _lp = layer_prefix or prefix
    ...
    activated = add_gelu(network, fc1, dtype=dtype)
    fc2 = matmul(activated, mlp_size, hidden_size, weights[f"{prefix}.w_fc2"], f"{_lp}.w_fc2")
    return add_bias_sum(network, fc2, hidden_size, weights[f"{prefix}.fc2_bias"], dtype=dtype)
...
def norm_multi(
    ...
    """Apply XGLM LayerNorm to a dynamic-sequence tensor."""
    if beta is None:
        raise ValueError("XGLM LayerNorm requires beta weights")
...
def build_standard_decoder_engine(
    ...
    *,
    ...
    """Build XGLM's fixed-Sq debug engine or its normal dual-profile engine."""
    import os as _os
    ...
    if config.num_key_value_heads != num_heads:
        raise ValueError("XGLM requires num_key_value_heads == num_attention_heads")
    ...
    position_embed = network.add_gather(position_embed_table, position_id, 0).get_output(0)
    hidden_state = network.add_elementwise(
        token_embed, position_embed, trt.ElementWiseOperation.SUM
    ).get_output(0)
...
def _supports_config(config: "ModelConfig", weights: "WeightDict") -> None:
    """Reject a non-XGLM or grouped-query configuration."""
    if config.model_type.lower() != "xglm":
        raise ValueError(f"Expected model_type='xglm', got {config.model_type!r}")
    if config.num_key_value_heads != config.num_attention_heads:
        raise ValueError("XGLM requires num_key_value_heads == num_attention_heads")
    for name in ("embedding", "position_embedding", "final_norm", "w_out"):
        if name not in weights:
            raise ValueError(f"Missing required XGLM weight: {name}")
...
def build_dual_profile_decoder_engine(
    ...
    *,
    ...
    """Build XGLM with dynamic prefill and fixed-token decode profiles.

    ``profile_mode`` controls which profiles are emitted:
    ...
    * ``"prefill"``: one prefill profile only. This is used by split-engine
    ...
    Dynamic-KV cache inputs are enabled only when decode buckets are supplied.
    """
    ...
    if verbose:
        ...
        print(
            f"[trtmc build] Building {mode_label} XGLM engine (layers={num_layers}, hidden={hidden}, attn={attention_size}, mlp={mlp_size}, cache={max_cache_length}, opt_prefill={opt_prefill_length}, max_prefill={max_prefill_length}, precision={precision}) ...",
            file=sys.stderr,
        )
    plan = builder.build_serialized_network(network, trt_config)
    if plan is None:
