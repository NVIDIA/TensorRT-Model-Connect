...
"""Rank-local tensor-parallel XGLM graph builder."""
...
def _gelu_fc_mlp(
    ...
    *,
    ...
    activated = graph_ops.add_gelu(network, fc1, dtype=work_np_dtype)
...
def build_dual_profile_tp_decoder_engine(
    ...
    **_family_contract,
    ...
    """Build one rank-local XGLM engine with prefill/decode profiles."""
    ...
    if verbose:
        print(
            f"[trtmc-build] Building tensor-parallel XGLM engine (layers={num_layers}, hidden={hidden}, attn={attention_size}, mlp={mlp_size}, cache={max_cache_length}, opt_prefill={opt_prefill_length}, max_prefill={max_prefill_length}, precision={precision}, tp={parallel.tp_size}, rank={parallel.rank}) ...",
            file=sys.stderr,
        )
