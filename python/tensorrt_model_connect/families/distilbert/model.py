...
"""DistilBERT family model — encoder-only bidirectional transformer.
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete distilbert bundle inside its owning family module."""
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("distilbert does not support context-parallel builds")
    if options.get("dynamic_kv_cache") or options.get("triattention_stats_path"):
        raise ValueError("distilbert does not use a decoder KV-cache runtime")
