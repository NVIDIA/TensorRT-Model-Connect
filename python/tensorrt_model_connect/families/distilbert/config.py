...
"""DistilBERT architecture fields used by the family implementation."""
...
class ModelConfig:
    ...
    def from_json(cls, text: str) -> ModelConfig:
        raw = json.loads(text)
        hidden_size = int(raw.get("hidden_size") or raw.get("dim") or 0)
        return cls(
            model_type=str(raw.get("model_type") or ""),
            vocab_size=int(raw.get("vocab_size") or 0),
            hidden_size=hidden_size,
            intermediate_size=int(
                raw.get("intermediate_size") or raw.get("hidden_dim") or hidden_size * 4
            ),
            num_hidden_layers=int(raw.get("num_hidden_layers") or raw.get("n_layers") or 0),
            num_attention_heads=int(raw.get("num_attention_heads") or raw.get("n_heads") or 1),
            ...
            hidden_act=str(raw.get("hidden_act") or raw.get("activation") or ""),
            raw=raw,
        )
