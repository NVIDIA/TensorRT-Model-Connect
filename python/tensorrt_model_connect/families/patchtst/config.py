...
"""PatchTST family model configuration."""
...
@dataclass
class ModelConfig:
    """Fields consumed by the PatchTST builder and build metadata."""
    ...
    def from_json(text: str) -> ModelConfig:
        ...
        return ModelConfig(
            model_type=data.get("model_type", "patchtst") or "patchtst",
            architectures=architectures,
            vocab_size=int(data.get("vocab_size", 0)),
            hidden_size=hidden_size,
            intermediate_size=int(data.get("ffn_dim", hidden_size * 4)),
            ...
            hidden_act=data.get("activation_function", "gelu") or "gelu",
            _head_dim=int(data.get("head_dim", 0)),
            raw=data,
        )

    @classmethod
    def create_tiny(cls, model_type: str = "patchtst", **overrides) -> ModelConfig:
        data = {
