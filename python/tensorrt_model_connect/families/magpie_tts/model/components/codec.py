...
def _build_length_mask(network, current_len, max_len):
    ...
    arange = graph_ops.add_constant(
        network, (1, 1, max_len), np.arange(max_len, dtype=np.float32).reshape(1, 1, max_len)
    )
...
def _add_snake(network, x, alpha_np, channels):
    ...
    alpha = graph_ops.add_constant(network, tuple(alpha_np.shape), alpha_np.astype(np.float32))
...
def _add_causal_conv1d_wn(network, x, sd, prefix, out_ch, kernel, mask, dilation=1):
    ...
    if pad_left > 0:
        ...
        inp = p.get_output(0)
    w4 = np.ascontiguousarray(weight.reshape(weight.shape[0], weight.shape[1], 1, kernel))
    cw = trt.Weights(w4)
    cb = trt.Weights(np.ascontiguousarray(bias)) if bias is not None else trt.Weights()
...
def _add_causal_conv_t1d_wn(network, x, sd, prefix, out_ch, kernel, stride, groups, mask):
    ...
    ri.reshape_dims = (1, c_in, 1, length)
    opg = out_ch // groups
    w4 = np.ascontiguousarray(weight.reshape(c_in, opg, 1, kernel), dtype=np.float32)
    cw = trt.Weights(w4)
    cb = trt.Weights(np.ascontiguousarray(bias)) if bias is not None else trt.Weights()
...
def build_nanocodec_decoder_engine(
    ...
    *,
    ...
    fsq_table = graph_ops.add_constant(
        network, (FSQ_TOTAL_CODES, FSQ_DIM_PER_GROUP), _make_fsq_lookup_table(FSQ_LEVELS)
    )
    group_latents = []
    ...
    x = _add_causal_conv1d_wn(network, x, sd, pp, pv.shape[0], pv.shape[2], mask)
    cur_ch = pv.shape[0]
    ...
    for si, rate in enumerate(UPSAMPLE_RATES):
        ...
        # Then ConvTranspose
        x = _add_causal_conv_t1d_wn(network, x, sd, up, oc, uk, rate, groups=oc, mask=mask)
        cur_ch = oc
        ...
        for j in range(num_res_groups):
            ...
            for k, dil in enumerate(RES_DILATIONS):
                ...
                gx = _add_snake(
                    network, gx, sd[f"{r}.input_activation.activation.snake_act.alpha"], cur_ch
                )
                ic = f"{r}.input_conv.conv"
                ick = sd[f"{ic}.parametrizations.weight.original1"].shape[2]
                gx = _add_causal_conv1d_wn(network, gx, sd, ic, cur_ch, ick, mask, dilation=dil)

                gx = _add_snake(
                    network, gx, sd[f"{r}.skip_activation.activation.snake_act.alpha"], cur_ch
                )
                sc = f"{r}.skip_conv.conv"
                sck = sd[f"{sc}.parametrizations.weight.original1"].shape[2]
                gx = _add_causal_conv1d_wn(network, gx, sd, sc, cur_ch, sck, mask)
        ...
        nc = graph_ops.add_constant(
            network, (1, 1, 1), np.array([1.0 / num_res_groups], dtype=np.float32)
        )
    ...
    x = _add_causal_conv1d_wn(network, x, sd, pp2, pv2.shape[0], pv2.shape[2], mask)
