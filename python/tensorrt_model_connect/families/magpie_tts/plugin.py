...
from .weights import magpie_tokenizer
...
def _extract_ipa_assets(nemo_path: str) -> dict[str, bytes] | None:
    ...
    try:
        ...
        print(f"[trtmc build]   Warning: failed to read .nemo for IPA: {e}", file=sys.stderr)
        return None
    ...
    if phoneme_dict_text is None:
        print("[trtmc build]   Warning: could not find IPA phoneme dict", file=sys.stderr)
        return None
...
class MagpieTTSPlugin:
    ...
    def load_weights(self, model_dir: str, config: ModelConfig) -> WeightDict:
        ...
        audio_options = (
            family_options.get("audio_magpie", {}) if isinstance(family_options, dict) else {}
        )
        override_max_pos = int(audio_options.get("max_source_positions", 0) or 0)
        if 0 < override_max_pos < max_positions:
            print(
                f"[trtmc build]   Shrinking max_source_positions: "
                f"{max_positions} -> {override_max_pos}",
                file=sys.stderr,
            )
        ...
        try:
            ...
            weights["_codec_state_dict"] = {
                k: _to_np(v)
                for k, v in codec_sd.items()
                if k.startswith("audio_decoder.") or k.startswith("vector_quantizer.")
            }
        except Exception as e:
            print(f"[trtmc build]   Warning: could not load NanoCodec: {e}", file=sys.stderr)
            ...
            codec_keys = {
                ...
                and (
                    k.startswith("codec.audio_decoder.") or k.startswith("codec.vector_quantizer.")
                )
            }
            if codec_keys:
    ...
    def build_engine(
        ...
        *,
        ...
        if parallel.enabled:
            ...
            from .model.parallel import build_magpie_tp_decoder_engine
        ...
        logits = graph_ops.add_bias_sum(network, logits, output_size, weights["w_out_bias"])
        logits.name = "logits"
    ...
    def build_extra_engines(
        ...
        *,
        ...
        if codec_sd:
            ...
            if verbose:
                print(
                    f"[trtmc build]   Building NanoCodec engine "
                    f"(max_frames={max_codec_frames}) ...",
                    file=sys.stderr,
                )

            from .model.components.codec import build_nanocodec_decoder_engine
            ...
            try:
                ...
                if ipa_assets is not None:
                    ...
                    if verbose:
                        for key, data in ipa_assets.items():
                            print(
                                f"[trtmc build]   Baked {key} ({len(data)} bytes)", file=sys.stderr
                            )
            except Exception as e:
                print(
                    f"[trtmc build]   Warning: IPA extraction failed, "
...
def _build_magpie_encoder(weights: WeightDict, *, verbose: bool = False) -> bytes:
    ...
    text_embed_table = graph_ops.add_constant(
        network, (weights["_text_vocab_size"], hidden), weights["text_embedding"]
    )
    pos_embed_table = graph_ops.add_constant(
        network, (max_pos, hidden), weights["enc_pos_embedding"]
    )
...
def _add_causal_self_attention(
    ...
    *,
    ...
    q = graph_ops.add_matmul_rhs_constant(network, hidden, hidden_size, hidden_size, w_q)
    k = graph_ops.add_matmul_rhs_constant(network, hidden, hidden_size, hidden_size, w_k)
    v = graph_ops.add_matmul_rhs_constant(network, hidden, hidden_size, hidden_size, w_v)
    ...
    # Output projection
    out = graph_ops.add_matmul_rhs_constant(network, context_flat, hidden_size, hidden_size, w_o)
...
def _add_magpie_decoder_layer(
    *,
    ...
    k = graph_ops.add_matmul_rhs_constant(
        network, normed, hidden_size, attention_size, weights[f"{prefix}.w_k"]
    )
    v = graph_ops.add_matmul_rhs_constant(
        network, normed, hidden_size, attention_size, weights[f"{prefix}.w_v"]
    )
...
def _build_local_transformer_engine(  # pragma: no cover
    ...
    *,
    ...
    qkv = graph_ops.add_matmul_rhs_constant(
        network, normed, lt_hidden, 3 * lt_hidden, weights["lt_qkv_net"]
    )
    ...
    sa = graph_ops.add_matmul_rhs_constant(network, cf, lt_hidden, lt_hidden, weights["lt_o_net"])
