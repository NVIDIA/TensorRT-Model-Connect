...
"""Deepseek V2-owned debug runner adapter."""
...
def load_engine_from_bundle(
    ...
    with open(bundle_path, "rb") as bundle:
        ...
        engine_plan = bundle.read(engine_meta["size"])
...
def load_section_from_bundle(bundle_path: str, section_name: str) -> bytes | None:
    ...
    with open(bundle_path, "rb") as bundle:
        ...
        return bundle.read(section["size"])
...
def runner_from_bundle(
    *,
    ...
    del bundle_path
    if runtime_strategy != "deepseek_v2_decoder_kv_cache":
        return None
