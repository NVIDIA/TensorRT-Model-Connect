...
class TimmMobilenetv2Plugin:
    name = "timm_mobilenetv2"
    runtime_strategy = "timm_mobilenetv2_image_classification"
    requires_tokenizer = False

    def matches(self, model_type: str) -> bool:
        mt = (model_type or "").lower()
        if mt == "timm_mobilenetv2":
            return True
        return mt.startswith("mobilenetv2")
    ...
    def build_engine(
        ...
        *,
        ...
        if quant_ctx is not None:
            raise NotImplementedError("timm_mobilenetv2 does not support quantized builds yet")
        if parallel_config is not None and getattr(parallel_config, "enabled", False):
            raise NotImplementedError("timm_mobilenetv2 does not support tensor-parallel builds")
    ...
    def get_bundle_config_overrides(self, config: ModelConfig) -> dict:
        cfg = config.raw.get("_timm_mobilenetv2_config") or _resolve_config(config.raw)
        return {
