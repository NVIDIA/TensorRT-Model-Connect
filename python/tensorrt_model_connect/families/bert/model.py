...
"""Complete family-owned BERT model construction."""
...
def load_weights(model_dir: str, config: ModelConfig) -> WeightDict:
    """Load BERT checkpoint weights using the family-owned mapper."""
...
def build_engine(
    ...
    *,
    ...
    """Build one BERT engine without routing through a plugin object."""
    parallel = normalize_parallel_config(parallel_config)
    if parallel.enabled:
...
def _ensure_tokenizer_json(model_dir) -> None:
    import tempfile
    from pathlib import Path
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete BERT bundle from checkpoint to serialized artifact."""
    import json
    import re
    import time
