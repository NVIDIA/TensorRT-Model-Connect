...
"""DeepSeek-OCR-2 family model — VL model: SAM ViT + Qwen2 encoder + MoE decoder.
...
def build_engine(
    ...
    *,
    ...
    if precision == "fp16":
        ...
        raise ValueError(f"Unsupported DeepSeek-OCR precision {precision!r}; expected fp32 or fp16")
    ...
    if work_trt_dtype != trt.float32:
        ...
        cache_k_inputs = [network.add_cast(x, work_trt_dtype).get_output(0) for x in cache_k_inputs]
        cache_v_inputs = [network.add_cast(x, work_trt_dtype).get_output(0) for x in cache_v_inputs]
    if io_trt_dtype != trt.float32:
        input_embed_tensor = network.add_cast(input_embed_tensor, io_trt_dtype).get_output(0)
        use_input_embed_tensor = network.add_cast(use_input_embed_tensor, io_trt_dtype).get_output(
            0
        )
...
def _add_moe_with_shared_experts(
    ...
    for e in range(n_routed_experts):
        ...
        selected = network.add_elementwise(
            top_indices, expert_index, trt.ElementWiseOperation.EQUAL
        ).get_output(0)
        selected = network.add_cast(selected, final_weights.dtype).get_output(0)
        selected_weights = network.add_elementwise(
            final_weights, selected, trt.ElementWiseOperation.PROD
        ).get_output(0)
        expert_weight = network.add_reduce(
            selected_weights, trt.ReduceOperation.SUM, 1 << 1, keep_dims=True
        ).get_output(0)
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete deepseek_ocr bundle inside its owning family module."""
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("deepseek_ocr does not support context-parallel builds")
    if options.get("dynamic_kv_cache") or options.get("triattention_stats_path"):
        raise ValueError("deepseek_ocr does not use a decoder KV-cache runtime")
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(
            parallel, feature="deepseek_ocr tensor-parallel builds"
        )
        if quant_ctx is not None:
            raise ValueError("deepseek_ocr tensor-parallel builds do not support quantization")
