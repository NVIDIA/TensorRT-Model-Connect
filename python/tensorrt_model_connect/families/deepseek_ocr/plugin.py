    ...
    def build_engine(
        ...
        *,
        ...
        if parallel.enabled:
            ...
            from .model.parallel import build_deepseek_ocr_tp_engine
...
def _add_moe_with_shared_experts(
    ...
    for k in range(num_experts_per_tok):
        ...
        w_slice = network.add_slice(final_weights, start=(0, k), shape=(1, 1), stride=(1, 1))
...
def _build_sam_attention(
    ...
    scale_c = graph_ops.add_constant(network, (1, 1, 1), np.array([attn_scale], dtype=np.float32))
    ...
    ctx = network.add_matrix_multiply(
        softmax.get_output(0), trt.MatrixOperation.NONE, v_r.get_output(0), trt.MatrixOperation.NONE
    )
...
def _load_vision_weights(
    ...
    # --- Qwen2 encoder ---
    vw["qwen2.queries"] = _load_tensor(readers, "model.qwen2_model.query_1024.weight").astype(
        np.float32
    )
...
def _build_deepseek_ocr_vision_engine(
    ...
    *,
    ...
    rms_eps_t = graph_ops.add_constant(network, (1, 1), np.array([1e-6], dtype=np.float32))
    ...
    for layer_idx in range(sam_layers):
        ...
        fc1 = graph_ops.add_matmul_rhs_constant(
            network, normed2, sam_hidden, sam_mlp_dim, vw[f"{wp}.mlp.fc1.weight"]
        )
        fc1 = graph_ops.add_bias_sum(network, fc1, sam_mlp_dim, vw[f"{wp}.mlp.fc1.bias"])
        gelu = graph_ops.add_gelu_erf(network, fc1)
        fc2 = graph_ops.add_matmul_rhs_constant(
            network, gelu, sam_mlp_dim, sam_hidden, vw[f"{wp}.mlp.fc2.weight"]
        )
        fc2 = graph_ops.add_bias_sum(network, fc2, sam_hidden, vw[f"{wp}.mlp.fc2.bias"])
    ...
    cos_half_c = graph_ops.add_constant(network, cos_half_np.shape, cos_half_np)
    sin_half_c = graph_ops.add_constant(network, sin_half_np.shape, sin_half_np)
    ...
    qwen2_mask_np = np.full((qwen2_total_seq, qwen2_total_seq), -1e9, dtype=np.float32)
    ...
    for layer_idx in range(qwen2_layers):
        ...
        # Pre-attention RMSNorm
        norm1 = _apply_norm(
            network, enc_state, qwen2_hidden, vw[f"{wp}.input_norm"], None, rms_eps_t, "rmsnorm"
        )
        ...
        attn_out = graph_ops.add_matmul_rhs_constant(
            network, ctx_flat, qwen2_q_dim, qwen2_hidden, vw[f"{wp}.w_o"]
        )
        ...
        gated = network.add_elementwise(swish.get_output(0), up, trt.ElementWiseOperation.PROD)
        down = graph_ops.add_matmul_rhs_constant(
            network, gated.get_output(0), qwen2_mlp_dim, qwen2_hidden, vw[f"{wp}.w_down"]
        )
    ...
    enc_state = _apply_norm(
        network, enc_state, qwen2_hidden, vw["qwen2.final_norm"], None, rms_eps_t, "rmsnorm"
    )
    ...
    projected = graph_ops.add_matmul_rhs_constant(
        network, query_out.get_output(0), proj_in, proj_out, vw["proj.weight"]
    )
    projected = graph_ops.add_bias_sum(network, projected, proj_out, vw["proj.bias"])
    # [256, 1280]

    # ===================================================================
    # Stage 4: View Separator
    # ===================================================================
    view_sep_c = graph_ops.add_constant(network, (1, proj_out), vw["view_sep"].reshape(1, -1))
