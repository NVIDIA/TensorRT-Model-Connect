...
def _add_distributed_rms_norm(
    ...
    *,
    ...
    global_sum = add_all_reduce_sum(network, local_sum.get_output(0), tp_size)
    inv_hidden = graph_ops.add_constant(
        network, (1, 1), np.array([1.0 / float(global_hidden_size)], dtype=np.float32)
    )
    mean = network.add_elementwise(global_sum, inv_hidden, trt.ElementWiseOperation.PROD)
    ...
    gamma_t = graph_ops.add_constant(network, (1, local_hidden_size), gamma, dtype=np.float32)
...
def build_olmo2_tp_engine(
    ...
    *,
    ...
    if not parallel.enabled:
        raise ValueError("build_olmo2_tp_engine requires tensor_parallel mode with tp_size > 1")
    ...
    attention_size: int = weights.get("_attention_size", config.attention_size // parallel.tp_size)
    mlp_size: int = weights.get("_mlp_size", config.intermediate_size // parallel.tp_size)
    ...
    cos_tensor = graph_ops.add_constant(network, cos_table_np.shape, cos_table_np)
    sin_tensor = graph_ops.add_constant(network, sin_table_np.shape, sin_table_np)
    ...
    for layer_idx in range(num_layers):
        ...
        q = graph_ops.add_matmul_rhs_constant(
            network, hidden_state, hidden, attention_size, weights[f"{prefix}.w_q"]
        )
        k = graph_ops.add_matmul_rhs_constant(
            network, hidden_state, hidden, kv_attention_size, weights[f"{prefix}.w_k"]
        )
        v = graph_ops.add_matmul_rhs_constant(
            network, hidden_state, hidden, kv_attention_size, weights[f"{prefix}.w_v"]
        )
        ...
        normed_attn = graph_ops.add_rms_norm(
            network, attn_out, hidden, weights[f"{prefix}.post_attn_norm"], eps_tensor
        )
        ...
        normed_mlp = graph_ops.add_rms_norm(
            network, mlp_out, hidden, weights[f"{prefix}.post_ff_norm"], eps_tensor
        )
    ...
    if verbose:
        print(
            f"[trtmc build] Building OLMo2 TP engine "
