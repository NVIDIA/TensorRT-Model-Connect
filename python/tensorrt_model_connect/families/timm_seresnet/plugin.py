...
"""timm SE-ResNet image-classification family plugin.

Supports timm SE-ResNet and SE-ResNeXt classifiers stored in HF Hub format.
...
def _discover_layout(readers) -> dict:
    ...
    if not any(name.startswith("layer1.0.se.") for name in names):
        raise ValueError(
            "Checkpoint has no squeeze-excite gate; this family only builds "
            "SE-ResNet variants")
    return {"blocks": blocks, "bottleneck": bottleneck}


class TimmSeresnetPlugin:
    name = "timm_seresnet"
    runtime_strategy = "timm_seresnet_image_classification"
    ...
    def load_weights(
        ...
        *,
        ...
        raw["_timm_seresnet_config"] = resnet_cfg
    ...
    def _add_block(self, network, x, weights, prefix, stride, bottleneck, dtype,
                   spatial):
        """One residual block with a squeeze-excite gate on its output.

        timm places the spatial stride on the 3x3 convolution, and applies the
        gate after the final batch norm but before the residual add.
        """
        identity = x
        ...
        *,
        ...
        if quant_ctx is not None:
            raise NotImplementedError(
                "timm_seresnet does not support quantized builds yet")
        if parallel_config is not None and getattr(parallel_config, "enabled", False):
            raise NotImplementedError(
                "timm_seresnet does not support tensor-parallel builds")
        ...
        if precision == "fp16":
            ...
            raise ValueError(f"Unsupported timm_seresnet precision: {precision}")

        cfg = config.raw.get("_timm_seresnet_config")
        if cfg is None:
            raise RuntimeError(
        ...
        if image_h % 32 != 0 or image_w % 32 != 0:
            raise ValueError(
                f"timm_seresnet input {image_h}x{image_w} must be divisible by 32")
        ...
        cur_h, cur_w = image_h // 4, image_w // 4
        for stage_idx, (stage, count) in enumerate(zip(_STAGES, blocks)):
            for block in range(count):
                ...
                hidden = self._add_block(
                    ...
                    stride, bottleneck, work_np_dtype, (cur_h, cur_w))
        ...
        if plan is None:
            raise RuntimeError("TensorRT timm_seresnet engine build failed")
        return bytes(plan)

    def get_bundle_config_overrides(self, config: ModelConfig) -> dict:
        cfg = config.raw.get("_timm_seresnet_config") or _resolve_resnet_config(config.raw)
        return {
