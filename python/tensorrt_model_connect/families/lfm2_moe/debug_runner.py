...
"""Family-owned fixed-step debug runner for native LFM2-MoE bundles."""
...
def _require_runtime() -> None:
    if trt is None:
        raise ImportError("tensorrt is required for the LFM2-MoE debug runner")
    if cudart is None:
        raise ImportError("cuda-python is required for the LFM2-MoE debug runner")
...
def _tensor_nbytes(engine, name: str) -> int:
    ...
    if any(value <= 0 for value in shape):
        raise ValueError(f"LFM2-MoE debug tensor {name!r} must have a static shape")
...
class Lfm2MoeTrtRunner:
    ...
    def __init__(
        ...
        if self.max_cache_length <= 0:
            raise ValueError("LFM2-MoE debug max_cache_length must be positive")

        logger = trt.Logger(trt.Logger.WARNING)
        runtime = trt.Runtime(logger)
        self.engine = runtime.deserialize_cuda_engine(engine_plan)
        if self.engine is None:
            raise RuntimeError("Failed to deserialize LFM2-MoE TensorRT engine")
        self.context = self.engine.create_execution_context()
        if self.context is None:
            raise RuntimeError("Failed to create LFM2-MoE TensorRT execution context")
    ...
    def step(self, token_id: int) -> dict[str, np.ndarray]:
        if self.cache_length >= self.max_cache_length:
            raise RuntimeError("LFM2-MoE sequence exceeds the fixed native KV capacity")
        ...
        if not self.context.execute_async_v3(self.stream):
            raise RuntimeError("LFM2-MoE TensorRT execution failed")
...
def runner_from_bundle(
    *,
    ...
    del bundle_path
    expected = "lfm2_moe_hybrid_conv_attention"
    if runtime_strategy != expected:
        raise ValueError(
            f"LFM2-MoE debug runner requires runtime_strategy={expected!r}, got {runtime_strategy!r}"
        )
    return Lfm2MoeTrtRunner(
