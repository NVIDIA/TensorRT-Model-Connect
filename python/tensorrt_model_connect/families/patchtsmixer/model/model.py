...
"""PatchTSMixer-owned TensorRT graph helpers."""
...
def add_matmul_rhs_constant(
    ...
    *,
    ...
    shape = (lhs_width, rhs_width) if rank <= 2 else (1,) * (rank - 2) + (lhs_width, rhs_width)
    rhs = add_constant(
        network,
        shape,
        np.asarray(rhs_weights).reshape(shape),
        dtype=dtype,
    )
...
def build_serialized_network(
    ...
    *,
    ...
    if verbose:
        print(
            f"[trtmc build] patchtsmixer: building native TRT network "
            f"({network.num_layers} layers, precision={precision}) ...",
            file=sys.stderr,
        )
    plan = builder.build_serialized_network(network, config)
    if plan is None:
        raise RuntimeError("TensorRT patchtsmixer engine build failed")
    return bytes(plan)
