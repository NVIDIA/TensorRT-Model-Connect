...
"""TensorRT graph builders for timm ResNeSt classifiers.
...
def add_avg_pool2d(network, x, kernel: int, stride: int, padding: int):
    pool = network.add_pooling_nd(x, trt.PoolingType.AVERAGE, trt.Dims2(kernel, kernel))
    pool.stride_nd = trt.Dims2(stride, stride)
    pool.padding_nd = trt.Dims2(padding, padding)
    pool.average_count_excludes_padding = False
    return pool.get_output(0)


def add_reshape(network, x, shape: tuple[int, ...]):
    layer = network.add_shuffle(x)
    layer.reshape_dims = trt.Dims(shape)
    return layer.get_output(0)


def add_permute(network, x, permutation: tuple[int, ...]):
    layer = network.add_shuffle(x)
    layer.second_transpose = trt.Permutation(permutation)
    return layer.get_output(0)


def add_reduce_sum(network, x, axis: int):
    """Sum over one axis, dropping it."""
    return network.add_reduce(
        x, trt.ReduceOperation.SUM, 1 << axis, False).get_output(0)


def add_mean_spatial(network, x, axes: tuple[int, int]):
    """Mean over two axes, keeping them as size 1."""
    mask = 0
    for axis in axes:
        mask |= 1 << axis
    return network.add_reduce(x, trt.ReduceOperation.AVG, mask, True).get_output(0)


def add_softmax(network, x, axis: int):
    layer = network.add_softmax(x)
    layer.axes = 1 << axis
    return layer.get_output(0)


def add_product(network, a, b):
    return network.add_elementwise(a, b, trt.ElementWiseOperation.PROD).get_output(0)
