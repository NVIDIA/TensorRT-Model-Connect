...
def add_attention_block(network: trt.INetworkDefinition, hidden: trt.ITensor, cache_k: trt.ITensor, cache_v: trt.ITensor, attention_mask: trt.ITensor, position_id: trt.ITensor, *, weights: WeightDict, prefix: str, hidden_size: int, attention_size: int, num_heads: int, head_dim: int, max_cache_length: int, eps_tensor: trt.ITensor, kv_attention_size: int | None=None, num_kv_heads: int | None=None, attention_scale: float | None=None, eps: float | None=None, alibi_slopes_tensor: trt.ITensor | None=None, alibi_indices_tensor: trt.ITensor | None=None, dtype: np.dtype=np.float32, quant_ctx: QuantContext | None=None, layer_prefix: str='', cos_half_tensor: trt.ITensor | None=None, sin_half_tensor: trt.ITensor | None=None, rotary_embedding_dim: int=0, interleaved_rope: bool=False, ffi_attention_kernel: str | None=None, dynamic_kv_cache: bool=False) -> dict[str, trt.ITensor]:
    ...
    normed = apply_norm(network, hidden, hidden_size, weights[f'{prefix}.input_norm'], weights.get(f'{prefix}.input_norm_beta'), eps_tensor, 'layernorm', dtype=dtype, eps=eps)
    ...
    if cos_half_tensor is None or sin_half_tensor is None:
        raise ValueError('RoPE attention requires half-dimension cos/sin tensors for TRT native IRotaryEmbeddingLayer')
    rope_dim = rotary_embedding_dim or head_dim
    rope_dim = validate_native_rope_dim(rope_dim)
    q = add_apply_rope_native(network, q, num_heads, head_dim, cos_half_tensor, sin_half_tensor, position_id, rope_dim, interleaved_rope)
    k = add_apply_rope_native(network, k, num_kv_heads, head_dim, cos_half_tensor, sin_half_tensor, position_id, rope_dim, interleaved_rope)
    present_k = k
    present_v = v
    ...
    if use_native_attention:
        ...
        if alibi_slopes_tensor is not None:
            ...
            mask_4d = add_alibi_mask_4d(network, attention_mask, position_id, alibi_slopes_tensor, alibi_indices_tensor, num_heads)
        else:
            mask_4d = add_2d_mask_to_4d(network, attention_mask)
        context = add_attention_from_rows(network, q, all_k.get_output(0), all_v.get_output(0), num_heads=num_heads, head_dim=head_dim, num_kv_heads=num_kv_heads, q_seq=1, kv_seq=kv_seq, causal=False, mask=mask_4d, scale=attention_scale)
    elif ffi_attention_kernel is not None:
        if num_kv_heads != num_heads:
            raise ValueError('FFI decoder attention requires num_kv_heads == num_heads; use TRT native attention for compact GQA/MQA KV cache')
        context = add_decoder_attention_ffi(network, q, all_k.get_output(0), all_v.get_output(0), kernel_name=ffi_attention_kernel, num_heads=num_heads, head_dim=head_dim, attention_window=attention_window)
...
def add_gelu_fc_mlp(network: trt.INetworkDefinition, inp: trt.ITensor, *, weights: WeightDict, prefix: str, hidden_size: int, mlp_size: int, dtype: np.dtype=np.float32, quant_ctx: QuantContext | None=None, layer_prefix: str='') -> trt.ITensor:
    ...
    activated = add_activation(network, fc1, 'gelu_new', dtype=dtype)
...
def build_standard_decoder_engine(config: ModelConfig, weights: WeightDict, max_cache_length: int, *, precision: str='fp32', quant_ctx: QuantContext | None=None, partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, embed_input: bool=False, verbose: bool=False, debug_layer_outputs: bool=False, hidden_state_output: bool=False) -> bytes:
    ...
    _dual_profile_disabled_for = embed_input or debug_layer_outputs or hidden_state_output or bool(config.raw.get('dynamic_kv_cache', False)) or (_os.environ.get('TRTMC_NO_DUAL_PROFILE') == '1')
    if decoder_engine_role == 'prefill' and _dual_profile_disabled_for:
        raise NotImplementedError('split prefill engine is not supported for this standard decoder configuration')
    if not _dual_profile_disabled_for and decoder_engine_role in ('dual_profile', 'prefill'):
        return build_dual_profile_decoder_engine(config, weights, max_cache_length, precision=precision, quant_ctx=quant_ctx, partial_rotary_factor=partial_rotary_factor, interleaved_rope=interleaved_rope, parallel_residual=parallel_residual, scale_attn_weights=scale_attn_weights, verbose=verbose, profile_mode='prefill' if decoder_engine_role == 'prefill' else 'dual_profile')
    ...
    kv_attention_size = infer_kv_attention_size(weights, num_kv_heads=num_kv_heads, head_dim=head_dim)
    attention_window = max_cache_length + 1
    ...
    attention_mask = network.add_input('attention_mask', trt.float32, (1, -1) if dynamic_kv_cache else (1, attention_window))
    ...
    for i in range(num_layers):
        ck = network.add_input(layer_tensor_name('cache_k', i), work_trt_dtype, (-1, kv_attention_size) if dynamic_kv_cache else (max_cache_length, kv_attention_size))
        cv = network.add_input(layer_tensor_name('cache_v', i), work_trt_dtype, (-1, kv_attention_size) if dynamic_kv_cache else (max_cache_length, kv_attention_size))
    ...
    if dynamic_kv_cache:
        if dynamic_kv_profile_rows:
            for profile_rows in dynamic_kv_profile_rows:
                ...
                min_rows = 1
                profile.set_shape('attention_mask', (1, min_rows + 1), (1, profile_rows + 1), (1, profile_rows + 1))
                for i in range(num_layers):
                    min_cache_shape = (min_rows, kv_attention_size)
                    cache_shape = (profile_rows, kv_attention_size)
                    profile.set_shape(layer_tensor_name('cache_k', i), min_cache_shape, cache_shape, cache_shape)
                    profile.set_shape(layer_tensor_name('cache_v', i), min_cache_shape, cache_shape, cache_shape)
                trt_config.add_optimization_profile(profile)
        else:
            profile = builder.create_optimization_profile()
            profile.set_shape('attention_mask', (1, 2), (1, dynamic_kv_opt_rows + 1), (1, attention_window))
            for i in range(num_layers):
                profile.set_shape(layer_tensor_name('cache_k', i), (1, kv_attention_size), (dynamic_kv_opt_rows, kv_attention_size), (max_cache_length, kv_attention_size))
                profile.set_shape(layer_tensor_name('cache_v', i), (1, kv_attention_size), (dynamic_kv_opt_rows, kv_attention_size), (max_cache_length, kv_attention_size))
            trt_config.add_optimization_profile(profile)
    ...
    embedding_table = add_constant(network, (vocab, hidden), weights['embedding'], dtype=work_np_dtype)
    ...
    cos_half_np = make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, True, partial_rotary_factor, interleaved=interleaved_rope)
    sin_half_np = make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, False, partial_rotary_factor, interleaved=interleaved_rope)
    ...
    eps_tensor = add_constant(network, (1, 1), np.array([config.rms_norm_eps], dtype=work_np_dtype), dtype=work_np_dtype)
    ...
    if embed_input and input_embed_tensor is not None and (use_input_embed_tensor is not None):
        ...
        one_const = const_in_work_dtype(network, (1, 1), np.array([1.0], dtype=work_np_dtype), work_np_dtype, work_trt_dtype)
        inv_flag = network.add_elementwise(one_const, flag_for_math, trt.ElementWiseOperation.SUB)
        tok_part = network.add_elementwise(inv_flag.get_output(0), token_embed_for_math, trt.ElementWiseOperation.PROD)
        input_embed_for_math = input_embed_tensor
        if input_embed_for_math.dtype != work_trt_dtype:
            input_embed_for_math = network.add_cast(input_embed_for_math, work_trt_dtype).get_output(0)
        embed_part = network.add_elementwise(flag_for_math, input_embed_for_math, trt.ElementWiseOperation.PROD)
        hidden_state_sum = network.add_elementwise(tok_part.get_output(0), embed_part.get_output(0), trt.ElementWiseOperation.SUM)
    ...
    if embed_norm is not None:
        ...
        hidden_state = add_layer_norm_native(network, hidden_state, hidden, embed_norm, embed_norm_beta, config.rms_norm_eps, dtype=work_np_dtype)
    ...
    for layer_idx in range(num_layers):
        ...
        result = _add_decoder_layer(network=network, hidden=hidden_state, cache_k=cache_k_inputs[layer_idx], cache_v=cache_v_inputs[layer_idx], attention_mask=attention_mask, position_id=position_id, attention_scale=attn_scale, eps_tensor=eps_tensor, eps=config.rms_norm_eps, weights=weights, prefix=prefix, hidden_size=hidden, attention_size=attention_size, kv_attention_size=kv_attention_size, mlp_size=mlp_size, num_heads=num_heads, num_kv_heads=num_kv_heads, head_dim=head_dim, max_cache_length=max_cache_length, parallel_residual=parallel_residual, alibi_slopes_tensor=alibi_slopes_tensor, alibi_indices_tensor=alibi_indices_tensor, dtype=work_np_dtype, quant_ctx=quant_ctx, cos_half_tensor=cos_half_tensor, sin_half_tensor=sin_half_tensor, rotary_embedding_dim=rotary_embedding_dim, interleaved_rope=interleaved_rope, ffi_attention_kernel=ffi_attention_kernel, dynamic_kv_cache=dynamic_kv_cache)
    ...
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = _apply_norm(network, hidden_state, hidden, final_norm, weights.get('final_norm_beta'), eps_tensor, 'layernorm', dtype=work_np_dtype, eps=config.rms_norm_eps)
    ...
    logits = add_matmul_rhs_constant(network, hidden_state, hidden, out_vocab, weights['w_out'], dtype=work_np_dtype)
    ...
    if verbose:
        print(f'[trtmc build] Building TRT engine ({num_layers} layers, hidden={hidden}, attn={attention_size}, kv={kv_attention_size}, mlp={mlp_size}, cache={max_cache_length}, precision={precision}) ...', file=sys.stderr)
...
def _add_decoder_layer(*, network: trt.INetworkDefinition, hidden: trt.ITensor, cache_k: trt.ITensor, cache_v: trt.ITensor, attention_mask: trt.ITensor, position_id: trt.ITensor, attention_scale: float | None, eps_tensor: trt.ITensor, weights: WeightDict, prefix: str, hidden_size: int, attention_size: int, kv_attention_size: int, mlp_size: int, num_heads: int, num_kv_heads: int, head_dim: int, max_cache_length: int, parallel_residual: bool=False, alibi_slopes_tensor: trt.ITensor | None=None, alibi_indices_tensor: trt.ITensor | None=None, dtype: np.dtype=np.float32, quant_ctx: QuantContext | None=None, cos_half_tensor: trt.ITensor | None=None, sin_half_tensor: trt.ITensor | None=None, rotary_embedding_dim: int=0, interleaved_rope: bool=False, ffi_attention_kernel: str | None=None, dynamic_kv_cache: bool=False, eps: float | None=None) -> dict[str, trt.ITensor]:
    ...
    if parallel_residual:
        ...
        if post_attn_norm_w is not None:
            norm2 = _apply_norm(network, hidden, hidden_size, post_attn_norm_w, weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, 'layernorm', dtype=dtype, eps=eps)
        else:
            norm2 = attn['normed']
    else:
        residual1 = network.add_elementwise(hidden, attn_out, trt.ElementWiseOperation.SUM)
        norm2 = _apply_norm(network, residual1.get_output(0), hidden_size, weights[f'{prefix}.post_attn_norm'], weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, 'layernorm', dtype=dtype, eps=eps)
    mlp_out = add_gelu_fc_mlp(network, norm2, weights=weights, prefix=prefix, hidden_size=hidden_size, mlp_size=mlp_size, dtype=dtype, quant_ctx=quant_ctx, layer_prefix=prefix)
    if parallel_residual:
    ...
    return {'hidden': residual2.get_output(0), 'post_attn': post_attn_tensor, 'present_k': present_k, 'present_v': present_v}


def _gelu_fc_mlp(network: trt.INetworkDefinition, inp: trt.ITensor, *, matmul, weights: 'WeightDict', prefix: str, hidden: int, mlp_size: int, work_np_dtype: np.dtype) -> trt.ITensor:
    ...
    activated = add_activation(network, fc1, 'gelu_new', dtype=work_np_dtype)
...
def build_dual_profile_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp16', opt_prefill_length: int=64, max_prefill_length: int | None=None, quant_ctx: 'QuantContext | None'=None, partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, verbose: bool=False, dynamic_kv_profile_rows: list[int] | None=None, profile_mode: str='dual_profile') -> bytes:
    ...
    * ``"prefill"``: one prefill profile only. This is used by split-engine
    ...
    if profile_mode == 'prefill':
        ...
        if _reverse:
            ...
            if multi_bucket_decode:
                ...
                _add_profile(1, 1, fixed=True)
    embedding_table = const_in_work_dtype(network, (vocab, hidden), weights['embedding'], work_np_dtype, work_trt_dtype)
    cos_half_table: trt.ITensor | None = None
    sin_half_table: trt.ITensor | None = None
    kmax = max_cache_length + max_prefill_length
    validate_native_rope_dim(rotary_embedding_dim)
    cos_half_np = make_rope_table_half_dim(kmax, head_dim, config.rope_theta, True, partial_rotary_factor, interleaved=interleaved_rope)
    sin_half_np = make_rope_table_half_dim(kmax, head_dim, config.rope_theta, False, partial_rotary_factor, interleaved=interleaved_rope)
    cos_half_table = const_in_work_dtype(network, cos_half_np.shape, cos_half_np, work_np_dtype, work_trt_dtype)
    sin_half_table = const_in_work_dtype(network, sin_half_np.shape, sin_half_np, work_np_dtype, work_trt_dtype)
    eps_tensor = add_constant(network, (1, 1), np.array([[config.rms_norm_eps]], dtype=np.float32), dtype=np.float32)
    eps_tensor_per_head = add_constant(network, (1, 1, 1), np.array([[[config.rms_norm_eps]]], dtype=np.float32), dtype=np.float32)
    attn_scale = 1.0 / np.sqrt(max(head_dim, 1)) if scale_attn_weights else 1.0
    ...
    if embed_norm is not None:
        ...
        hidden_state = norm_multi(network, hidden_state, hidden, embed_norm, embed_norm_beta, eps_tensor, 'layernorm', work_np_dtype)
    mask_4d = add_2d_mask_to_4d(network, attention_mask_work)
    present_k_outs: list[trt.ITensor] = []
    present_v_outs: list[trt.ITensor] = []
    for layer_idx in range(num_layers):
        prefix = f'layer.{layer_idx}'
        normed = norm_multi(network, hidden_state, hidden, weights[f'{prefix}.input_norm'], weights.get(f'{prefix}.input_norm_beta'), eps_tensor, 'layernorm', work_np_dtype)
        ...
        q_norm = weights.get(f'{prefix}.q_norm')
        if q_norm is not None:
            q = add_rms_norm_per_head(network, q, num_heads, head_dim, q_norm, eps_tensor_per_head, dtype=work_np_dtype, sequence_length=None)
        k_norm = weights.get(f'{prefix}.k_norm')
        if k_norm is not None:
            k = add_rms_norm_per_head(network, k, num_kv_heads, head_dim, k_norm, eps_tensor_per_head, dtype=work_np_dtype, sequence_length=None)
        q = add_apply_rope_native(network, q, num_heads, head_dim, cos_half_table, sin_half_table, position_id, rotary_embedding_dim, interleaved_rope, sequence_length=None)
        k = add_apply_rope_native(network, k, num_kv_heads, head_dim, cos_half_table, sin_half_table, position_id, rotary_embedding_dim, interleaved_rope, sequence_length=None)
        ...
        all_v_cat.axis = 0
        context = add_attention_from_rows(network, q, all_k_cat.get_output(0), all_v_cat.get_output(0), num_heads=num_heads, head_dim=head_dim, num_kv_heads=num_kv_heads, q_seq=None, kv_seq=None, causal=False, mask=mask_4d, scale=attn_scale, tag=f'{prefix}.attn')
        ...
        if parallel_residual:
            post_attn_norm_w = weights.get(f'{prefix}.post_attn_norm')
            if post_attn_norm_w is not None:
                norm2 = norm_multi(network, hidden_state, hidden, post_attn_norm_w, weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, 'layernorm', work_np_dtype)
            else:
                norm2 = normed
        else:
            residual1 = network.add_elementwise(hidden_state, attn_out, trt.ElementWiseOperation.SUM)
            norm2 = norm_multi(network, residual1.get_output(0), hidden, weights[f'{prefix}.post_attn_norm'], weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, 'layernorm', work_np_dtype)
        mlp_out = _gelu_fc_mlp(network, norm2, matmul=matmul, weights=weights, prefix=prefix, hidden=hidden, mlp_size=mlp_size, work_np_dtype=work_np_dtype)
        if parallel_residual:
    ...
    final_norm = weights.get('final_norm')
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = norm_multi(network, hidden_state, hidden, final_norm, weights.get('final_norm_beta'), eps_tensor, 'layernorm', work_np_dtype)
    ...
    out_vocab = weights['w_out'].shape[1] if isinstance(weights['w_out'], np.ndarray) else vocab
    logits = add_matmul_rhs_constant(network, last_hidden, hidden, out_vocab, weights['w_out'], dtype=work_np_dtype)
    lm_bias = weights.get('lm_head_bias')
    if lm_bias is not None:
        logits = add_bias_sum(network, logits, out_vocab, lm_bias, dtype=work_np_dtype)
    else:
    ...
    if verbose:
        ...
        print(f'[trtmc build] Building {mode_label} engine (layers={num_layers}, hidden={hidden}, attn={attention_size}, kv={kv_attention_size}, mlp={mlp_size}, cache={max_cache_length}, opt_prefill={opt_prefill_length}, max_prefill={max_prefill_length}, norm=layernorm, mlp_type=gelu_fc, pos=rope, precision={precision}) ...', file=sys.stderr)
    plan = builder.build_serialized_network(network, trt_config)
    if plan is None:
