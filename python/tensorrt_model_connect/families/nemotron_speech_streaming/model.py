...
"""Nemotron Speech Streaming family-owned build implementation.
...
def load_weights(model_dir: str, config: ModelConfig, *, precision: str = "fp32") -> WeightDict:
    global _bundle_config, _vl_config
    del precision
    w = WeightDict()
    ...
    if has_prompt_kernel:
        ...
        if pk_w0.ndim != 2 or pk_w2.ndim != 2:
            raise ValueError(f"Unexpected prompt_kernel weight ranks: {pk_w0.shape}, {pk_w2.shape}")
...
def get_vl_config(config: ModelConfig) -> dict | None:
    del config
    return _vl_config or {
        "has_vision_engine": True,
        "num_mel_bins": 128,
        "max_source_positions": 375,
        "encoder_layers": 24,
        "mel_length": _DEFAULT_MEL_LENGTH,
        "subsampling_factor": 8,
        "sample_rate": 16000,
    }
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete nemotron_speech_streaming bundle in the owning family module."""
    import json
    import time
