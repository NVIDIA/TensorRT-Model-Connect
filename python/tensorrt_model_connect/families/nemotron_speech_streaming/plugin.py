...
from .model.parallel import build_nemotron_streaming_tp_predictor
...
def _rel_shift_streaming(network, x, heads: int, query_len: int, pos_len: int, key_len: int):
    zeros = graph_ops.add_constant(
        network, (heads, query_len, 1), np.zeros((heads, query_len, 1), dtype=np.float32)
    )
...
def _add_streaming_conv_module(
    network, hs, time_cache, weights, pfx, hidden, kern, query_len, eps, conv_norm_type
):
    normed = graph_ops.add_layer_norm(
...
def _build_streaming_encoder(
    weights: WeightDict, right_context: int, *, first_step: bool = False, verbose: bool = False
) -> bytes:
...
def _build_predictor(weights: WeightDict, *, verbose: bool = False) -> bytes:
    ...
    for layer in range(pred_layers):
        ...
        hidden, c_new = _add_lstm_cell(
            network, hidden, h_in, c_in, weights, f"pred.{layer}", pred_hidden
        )
...
def _build_joint(weights: WeightDict, *, verbose: bool = False) -> bytes:
    ...
    enc_proj = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(
            network, enc, enc_hidden, joint_hidden, weights["joint_enc_w"]
        ),
        joint_hidden,
        weights["joint_enc_b"],
    )
    pred_proj = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(
            network, pred, pred_hidden, joint_hidden, weights["joint_pred_w"]
        ),
        joint_hidden,
    ...
    logits = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(
            network, joint, joint_hidden, vocab_total, weights["joint_out_w"]
        ),
        vocab_total,
        weights["joint_out_b"],
    )
