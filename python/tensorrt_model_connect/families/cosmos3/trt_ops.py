...
def attention(
    ...
    *,
    ...
    """Non-decomposable native TensorRT GQA for the full Cosmos3 sequence."""
    ...
    *,
    ...
    def _fused(q4, k4, v4, *, causal: bool, mask=None):
        q4 = network.add_elementwise(
            cast(network, q4, trt.float16), scale, trt.ElementWiseOperation.PROD
        ).get_output(0)
        layer = network.add_attention(
            q4,
            cast(network, k4, trt.float16),
            cast(network, v4, trt.float16),
            trt.AttentionNormalizationOp.SOFTMAX,
            causal,
        )
        if layer is None:
        ...
        layer.decomposable = False
        if mask is not None:
            layer.mask = cast(network, mask, trt.float16)
    ...
    def _decomposed_causal(q4, k4, v4):
        ...
        return network.add_matrix_multiply(
            cast(network, probabilities.get_output(0), trt.float16),
            trt.MatrixOperation.NONE,
            v4,
            trt.MatrixOperation.NONE,
        ).get_output(0)
