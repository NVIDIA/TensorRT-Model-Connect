...
def runtime_binding_specs(
    config: NativeTTSConfig = EXACT_CONFIG,
) -> dict[str, dict[str, tuple[str, tuple[int, ...]]]]:
    """Return the C++ binding contract without importing TensorRT."""
    inputs: dict[str, tuple[str, tuple[int, ...]]] = {
        "prev_codes": ("int32", (config.num_quantizers,)),
        "subword_id": ("int32", (1,)),
        "subword_mask": ("float32", (1,)),
        "position_id": ("int32", (1,)),
        "attention_mask": ("float32", (1, 1, 1, -1)),
        "mixture_uniform": (
            "float32",
            (NUM_REFINEMENT_STEPS, config.mog_num_predictions),
        ),
        "mog_noise": ("float32", (NUM_REFINEMENT_STEPS, config.latent_size)),
        "audio_prompt_latent": ("float32", (config.hidden_size,)),
        "audio_prompt_mode": ("float32", (1,)),
        "bos_flag": ("float32", (1,)),
    }
    outputs: dict[str, tuple[str, tuple[int, ...]]] = {
        "rvq_codes": ("int32", (config.num_quantizers,)),
    }
    for layer_idx in range(config.num_hidden_layers):
        inputs[f"cache_k_{layer_idx}"] = ("work", (2, -1, config.kv_width))
        inputs[f"cache_v_{layer_idx}"] = ("work", (2, -1, config.kv_width))
        outputs[f"present_k_{layer_idx}"] = ("work", (2, 1, config.kv_width))
        outputs[f"present_v_{layer_idx}"] = ("work", (2, 1, config.kv_width))
...
def build_native_tts_engine_from_weights(
    ...
    *,
    ...
    if hasattr(builder_config, "clear_flag") and hasattr(trt, "BuilderFlag"):
...
def _load_aria_warmup_assets(
    ...
    recipe: dict[str, Any] = {
        "speaker": "Aria",
        "num_steps": 37,
        "embedding_section": "tts_aria_prompt_latents",
        "embedding_dtype": "float32_le",
        "embedding_shape": [37, EXACT_CONFIG.hidden_size],
        "subword_ids": subword_ids,
        "subword_mask": subword_mask,
        "audio_prompt_mode": [1] * 36 + [0],
        "bos_flags": bos_flags,
        "position_ids": list(range(37)),
        "prev_codes_policy": ("ignored_for_steps_0_to_35_then_tts_silence_codes_for_step_36"),
        "first_generation_code_section": "tts_first_code_input",
        "first_generation_position_id": 37,
        "first_generation_subword_context": "current_thinker_token_only",
        "cfg_cache_batch_order": ["conditional", "unconditional"],
        "first_step_cache_policy": "one_masked_dummy_row_then_discard_dummy",
...
def build_tts_sections(
    ...
    *,
    ...
    return [
        ("tts_engine_plan", engine),
        ("tts_silence_codes", silence.tobytes()),
        ("tts_control_codes", control.tobytes()),
        ("tts_first_code_input", first_code.tobytes()),
        ("tts_aria_prompt_latents", aria_warmup.tobytes()),
        (
