...
"""TensorRT graph builders for timm CrossViT classifiers."""
...
def add_patch_conv(network, inp, weight, bias, out_channels, patch, dtype=np.float32):
    """Non-overlapping patch convolution: stride equals the kernel size."""
    conv = network.add_convolution_nd(
        inp, out_channels, trt.Dims2(*patch),
        trt.Weights(np.ascontiguousarray(weight, dtype=dtype)),
        trt.Weights(np.ascontiguousarray(bias, dtype=dtype)))
    conv.stride_nd = trt.Dims2(*patch)
    return conv.get_output(0)


def add_reshape(network, inp, shape):
    layer = network.add_shuffle(inp)
    layer.reshape_dims = trt.Dims(shape)
    return layer.get_output(0)


def add_permute_reshape(network, inp, first_reshape, permutation, second_reshape):
    """One shuffle doing reshape then transpose, plus an optional reshape after."""
    layer = network.add_shuffle(inp)
    if first_reshape is not None:
        layer.reshape_dims = trt.Dims(first_reshape)
    layer.second_transpose = trt.Permutation(permutation)
    out = layer.get_output(0)
    return add_reshape(network, out, second_reshape) if second_reshape is not None else out


def add_token_slice(network, inp, start, count):
    """Take `count` tokens starting at `start` from a [1, N, C] tensor."""
    shape = [int(d) for d in inp.shape]
    starts, sizes = [0] * len(shape), list(shape)
    starts[1], sizes[1] = start, count
    return network.add_slice(
        inp, trt.Dims(starts), trt.Dims(sizes),
        trt.Dims([1] * len(shape))).get_output(0)


def add_token_concat(network, tensors):
    layer = network.add_concatenation(tensors)
    layer.axis = 1
    return layer.get_output(0)


def add_bicubic_resize(network, inp, size):
    """Bicubic resize matching torch.nn.functional.interpolate(align_corners=False).

    The coefficient and the coordinate convention both have to be set: TensorRT
    and PyTorch pick different defaults, and a mismatch shifts every sampled
    pixel slightly rather than failing.
    """
...
def add_mean_tokens(network, inp):
    """Mean over the token axis of a [1, N, C] tensor, dropping it."""
    return network.add_reduce(
        inp, trt.ReduceOperation.AVG, 1 << 1, False).get_output(0)


def add_scaled_sum(network, tensors, scale, dtype=np.float32):
    """Sum tensors then multiply by a scalar."""
    total = tensors[0]
    for other in tensors[1:]:
        total = network.add_elementwise(
            total, other, trt.ElementWiseOperation.SUM).get_output(0)
    shape = (1,) * len(tuple(total.shape))
    factor = network.add_constant(
        shape, trt.Weights(np.array([scale], dtype=dtype).reshape(shape))).get_output(0)
    if factor.dtype != total.dtype:
        factor = network.add_cast(factor, total.dtype).get_output(0)
    return network.add_elementwise(
        total, factor, trt.ElementWiseOperation.PROD).get_output(0)
