...
def build_tp_xlnet_engine(
    ...
    *,
    ...
    if not parallel.enabled:
        raise ValueError("build_tp_xlnet_engine requires tensor_parallel mode and tp_size > 1")
    ...
    scale_t = graph_ops.add_constant(
        network,
        (1, 1, 1),
        np.array([scale], dtype=work_np_dtype),
        dtype=work_np_dtype,
    )

    # Precompute sinusoidal relative positional embeddings: [2*qlen, hidden]
    pos_emb_np = graph_ops._compute_sinusoidal_pos_emb(qlen, hidden)
...
def _add_xlnet_layer(
    *,
    ...
    q = graph_ops.add_matmul_rhs_constant(
        network, hidden, hidden_size, attn_size, weights[f"{prefix}.w_q"], dtype=dtype
    )
    k = graph_ops.add_matmul_rhs_constant(
        network, hidden, hidden_size, attn_size, weights[f"{prefix}.w_k"], dtype=dtype
    )
    v = graph_ops.add_matmul_rhs_constant(
        network, hidden, hidden_size, attn_size, weights[f"{prefix}.w_v"], dtype=dtype
    )

    # Position key: [2*qlen, hidden] @ [hidden, attn_size] -> [2*qlen, attn_size]
    k_r = graph_ops.add_matmul_rhs_constant(
        network, pos_emb, hidden_size, attn_size, weights[f"{prefix}.w_r"], dtype=dtype
    )
    ...
    bd = graph_ops._add_rel_shift(network, bd_raw.get_output(0), num_heads, seq_length, klen)
    ...
    seg_embed = graph_ops.add_constant(
        network, (2, num_heads, d_head), weights[f"{prefix}.seg_embed"], dtype=dtype
    )
    ...
    # Select the same/different score with the one-hot segment matrix.
    ...
    w_o = graph_ops.add_constant(
        network, (hidden_size, attn_size), weights[f"{prefix}.w_o"], dtype=dtype
    )
    ...
    residual1 = network.add_elementwise(hidden, attn_out_sum, trt.ElementWiseOperation.SUM)
    normed1 = graph_ops.add_layer_norm_native(
        network,
