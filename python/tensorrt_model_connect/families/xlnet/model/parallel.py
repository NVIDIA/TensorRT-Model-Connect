...
def build_tp_xlnet_engine(
    ...
    *,
    ...
    scale_t = graph_ops.add_constant(network, (1, 1, 1), np.array([scale], dtype=np.float32))

    # Precompute sinusoidal relative positional embeddings: [2*qlen, hidden]
    pos_emb_np = _compute_sinusoidal_pos_emb(qlen, hidden)
    pos_emb_const = graph_ops.add_constant(network, pos_emb_np.shape, pos_emb_np)
...
def _add_xlnet_layer(
    *,
    ...
    q = graph_ops.add_matmul_rhs_constant(
        network, hidden, hidden_size, attn_size, weights[f"{prefix}.w_q"]
    )
    k = graph_ops.add_matmul_rhs_constant(
        network, hidden, hidden_size, attn_size, weights[f"{prefix}.w_k"]
    )
    v = graph_ops.add_matmul_rhs_constant(
        network, hidden, hidden_size, attn_size, weights[f"{prefix}.w_v"]
    )
    ...
    w_o = graph_ops.add_constant(network, (hidden_size, attn_size), weights[f"{prefix}.w_o"])
