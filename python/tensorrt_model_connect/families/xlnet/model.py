...
"""XLNet family model -- encoder-only bidirectional transformer with relative positional encoding.
...
def build_engine(
    ...
    *,
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="XLNet tensor-parallel builds")
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete xlnet bundle inside its owning family module."""
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("xlnet does not support context-parallel builds")
    if options.get("dynamic_kv_cache") or options.get("triattention_stats_path"):
        raise ValueError("xlnet does not use a decoder KV-cache runtime")
