...
def add_attention_block(network: trt.INetworkDefinition, hidden: trt.ITensor, cache_k: trt.ITensor, cache_v: trt.ITensor, attention_mask: trt.ITensor, position_id: trt.ITensor, *, weights: WeightDict, prefix: str, hidden_size: int, attention_size: int, num_heads: int, head_dim: int, max_cache_length: int, eps_tensor: trt.ITensor, kv_attention_size: int | None=None, num_kv_heads: int | None=None, attention_scale: float | None=None, eps: float | None=None, alibi_slopes_tensor: trt.ITensor | None=None, alibi_indices_tensor: trt.ITensor | None=None, dtype: np.dtype=np.float32, quant_ctx: QuantContext | None=None, layer_prefix: str='', cos_half_tensor: trt.ITensor | None=None, sin_half_tensor: trt.ITensor | None=None, rotary_embedding_dim: int=0, ffi_attention_kernel: str | None=None, dynamic_kv_cache: bool=False) -> dict[str, trt.ITensor]:
    ...
    normed = apply_norm(network, hidden, hidden_size, weights[f'{prefix}.input_norm'], weights.get(f'{prefix}.input_norm_beta'), eps_tensor, 'rmsnorm', dtype=dtype, eps=eps)
    ...
    q = add_apply_rope_native(network, q, num_heads, head_dim, cos_half_tensor, sin_half_tensor, position_id, rope_dim, False)
    k = add_apply_rope_native(network, k, num_kv_heads, head_dim, cos_half_tensor, sin_half_tensor, position_id, rope_dim, False)
    present_k = k
    present_v = v
...
def build_standard_decoder_engine(config: ModelConfig, weights: WeightDict, max_cache_length: int, *, precision: str='fp32', quant_ctx: QuantContext | None=None, verbose: bool=False, debug_layer_outputs: bool=False, hidden_state_output: bool=False) -> bytes:
    ...
    _dual_profile_disabled_for = debug_layer_outputs or hidden_state_output or bool(config.raw.get('dynamic_kv_cache', False)) or (_os.environ.get('TRTMC_NO_DUAL_PROFILE') == '1')
    ...
    if not _dual_profile_disabled_for and decoder_engine_role in ('dual_profile', 'prefill'):
        return build_dual_profile_decoder_engine(config, weights, max_cache_length, precision=precision, quant_ctx=quant_ctx, verbose=verbose, profile_mode='prefill' if decoder_engine_role == 'prefill' else 'dual_profile')
    ...
    rotary_embedding_dim = int(head_dim * 1.0)
    validate_native_rope_dim(rotary_embedding_dim)
    cos_half_np = make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, True, 1.0, interleaved=False)
    sin_half_np = make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, False, 1.0, interleaved=False)
    ...
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = _apply_norm(network, hidden_state, hidden, final_norm, weights.get('final_norm_beta'), eps_tensor, 'rmsnorm', dtype=work_np_dtype, eps=config.rms_norm_eps)
...
def _add_decoder_layer(*, network: trt.INetworkDefinition, hidden: trt.ITensor, cache_k: trt.ITensor, cache_v: trt.ITensor, attention_mask: trt.ITensor, position_id: trt.ITensor, attention_scale: float | None, eps_tensor: trt.ITensor, weights: WeightDict, prefix: str, hidden_size: int, attention_size: int, kv_attention_size: int, mlp_size: int, num_heads: int, num_kv_heads: int, head_dim: int, max_cache_length: int, alibi_slopes_tensor: trt.ITensor | None=None, alibi_indices_tensor: trt.ITensor | None=None, dtype: np.dtype=np.float32, quant_ctx: QuantContext | None=None, cos_half_tensor: trt.ITensor | None=None, sin_half_tensor: trt.ITensor | None=None, rotary_embedding_dim: int=0, ffi_attention_kernel: str | None=None, dynamic_kv_cache: bool=False, eps: float | None=None) -> dict[str, trt.ITensor]:
    ...
    norm2 = _apply_norm(network, residual1.get_output(0), hidden_size, weights[f'{prefix}.post_attn_norm'], weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, 'rmsnorm', dtype=dtype, eps=eps)
    mlp_out = add_swiglu_mlp(network, norm2, weights=weights, prefix=prefix, hidden_size=hidden_size, mlp_size=mlp_size, dtype=dtype, quant_ctx=quant_ctx, layer_prefix=prefix)
...
def build_dual_profile_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp16', opt_prefill_length: int=64, max_prefill_length: int | None=None, quant_ctx: 'QuantContext | None'=None, verbose: bool=False, dynamic_kv_profile_rows: list[int] | None=None, profile_mode: str='dual_profile') -> bytes:
    ...
    * ``"prefill"``: one prefill profile only. This is used by split-engine
    ...
    validate_native_rope_dim(rotary_embedding_dim)
    cos_half_np = make_rope_table_half_dim(kmax, head_dim, config.rope_theta, True, 1.0, interleaved=False)
    sin_half_np = make_rope_table_half_dim(kmax, head_dim, config.rope_theta, False, 1.0, interleaved=False)
    ...
    for layer_idx in range(num_layers):
        prefix = f'layer.{layer_idx}'
        normed = norm_multi(network, hidden_state, hidden, weights[f'{prefix}.input_norm'], weights.get(f'{prefix}.input_norm_beta'), eps_tensor, 'rmsnorm', work_np_dtype)
        ...
        q = add_apply_rope_native(network, q, num_heads, head_dim, cos_half_table, sin_half_table, position_id, rotary_embedding_dim, False, sequence_length=None)
        k = add_apply_rope_native(network, k, num_kv_heads, head_dim, cos_half_table, sin_half_table, position_id, rotary_embedding_dim, False, sequence_length=None)
        ...
        norm2 = norm_multi(network, residual1.get_output(0), hidden, weights[f'{prefix}.post_attn_norm'], weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, 'rmsnorm', work_np_dtype)
        mlp_out = _swiglu_mlp(network, norm2, matmul=matmul, weights=weights, prefix=prefix, hidden=hidden, mlp_size=mlp_size)
        residual2 = network.add_elementwise(residual1.get_output(0), mlp_out, trt.ElementWiseOperation.SUM)
        hidden_state = residual2.get_output(0)
    final_norm = weights.get('final_norm')
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = norm_multi(network, hidden_state, hidden, final_norm, weights.get('final_norm_beta'), eps_tensor, 'rmsnorm', work_np_dtype)
    ...
    if verbose:
        ...
        print(f'[trtmc build] Building {mode_label} engine (layers={num_layers}, hidden={hidden}, attn={attention_size}, kv={kv_attention_size}, mlp={mlp_size}, cache={max_cache_length}, opt_prefill={opt_prefill_length}, max_prefill={max_prefill_length}, norm={'rmsnorm'}, mlp_type={'swiglu'}, pos={'rope'}, precision={precision}) ...', file=sys.stderr)
    plan = builder.build_serialized_network(network, trt_config)
    if plan is None:
