"""Tensor-parallel dual-profile decoder engine builder for the Granite family.
...
Scope: Granite 3.x (IBM). Granite uses the LLaMA-shape decoder with four
extra scaling factors (`embedding_multiplier`, `attention_multiplier`,
`residual_multiplier`, `logits_scaling`) that the Granite plugin absorbs
into the weight tensors at `load_weights` time. After load_weights the
weights look identical to a plain LLaMA model, so this TP builder is the
standard LLaMA TP graph with no Granite-specific runtime ops.
...
def build_dual_profile_tp_decoder_engine(
    ...
    *,
    ...
    """Build a rank-local TP engine for Granite-family decoders.
    ...
    if quant_ctx is not None:
        raise ValueError("Tensor-parallel Granite builds do not support quantization yet")
    ...
    rotary_embedding_dim = head_dim  # Granite uses full rotary
    ...
    if verbose:
        print(f"[trtmc-build] Building Granite TP decoder engine "
    ...
    if plan is None:
        raise RuntimeError("Granite tensor-parallel engine build failed")
    return bytes(plan)
