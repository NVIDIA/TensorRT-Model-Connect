...
"""Routing contract for dense Granite models using TensorRT native KV cache."""
...
def _checked_product(label: str, *values: int) -> int:
    ...
    for value in values:
        if value <= 0 or product > _UINT64_MAX // value:
            raise ValueError(f"native Granite KV {label} exceeds uint64")
        product *= value
    return product
...
def native_kv_cache_geometry(
    ...
    *,
    ...
    if capacity != context:
        raise ValueError(
            "native Granite KV requires max_cache_length == "
...
def _validate_rope(raw: dict, reasons: list[str]) -> None:
    ...
    if rope_type in ("", "default"):
        ...
        return
    reasons.append(f"native Granite does not support rope_type={rope_type!r}")


def native_kv_architecture_capability(
    config: object,
) -> NativeKvCapability:
    """Accept any model size that retains the dense Granite graph contract."""

    model_type = str(getattr(config, "model_type", "")).lower()
    if not model_type.startswith("granite"):
        return _result(applicable=False)
    if model_type != "granite":
        return _result(reasons=["model_type must be exactly 'granite'"])

    raw = _raw(config)
    reasons: list[str] = []
    if tuple(getattr(config, "architectures", ()) or ()) != ("GraniteForCausalLM",):
        reasons.append("architectures must contain exactly GraniteForCausalLM")
    ...
    try:
        ...
        if head_dim not in (64, 128):
            reasons.append("native Granite attention requires head_dim=64 or 128")
    except ValueError as exc:
        reasons.append(str(exc))

    if str(getattr(config, "hidden_act", "")).lower() != "silu":
        reasons.append("native Granite requires hidden_act='silu'")
    ...
    if enabled:
        reasons.append("unsupported Granite fields: " + ", ".join(enabled))
    try:
        if _integer(raw.get("pretraining_tp", 1), "pretraining_tp") != 1:
            reasons.append("native Granite requires pretraining_tp=1")
    except ValueError as exc:
        reasons.append(str(exc))
    try:
        if float(raw.get("partial_rotary_factor", 1.0)) != 1.0:
            reasons.append("native Granite requires full rotary embeddings")
    except (TypeError, ValueError, OverflowError):
    ...
    if layer_types is not None and (
        ...
        reasons.append("native Granite does not support hybrid layer types")
...
def native_kv_build_capability(
    ...
    *,
    ...
    if str(precision).lower() != "fp16":
        reasons.append("native Granite requires FP16")
    if str(raw.get("_decoder_engine_layout", "split")) != "split":
        reasons.append("native Granite requires split prefill/decode engines")
    if raw.get("_rtx_build_requested"):
        reasons.append("native Granite requires the standard TensorRT backend")
    if parallel_enabled or raw.get("_parallel_build_enabled"):
        reasons.append("native Granite does not support tensor parallel builds")
    if dynamic_kv_cache or raw.get("_runtime_dynamic_kv_requested") or raw.get("dynamic_kv_cache"):
        reasons.append("native Granite uses one fixed physical KV capacity")
    if quantized or raw.get("quantization_config") or raw.get("_quantized_build_requested"):
        reasons.append("native Granite does not support quantized builds")
    if raw.get("_fp32_layers"):
        reasons.append("native Granite does not support FP32 layer overrides")
    if debug_layer_outputs:
        reasons.append("native Granite does not support debug layer outputs")
    try:
...
def prefer_native_default(
    ...
    """Claim all Granite-family builds so unsupported variants fail closed."""
