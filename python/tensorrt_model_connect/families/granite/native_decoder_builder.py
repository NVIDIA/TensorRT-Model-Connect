...
"""Native-only split prefill/decode builder for dense Granite models."""
...
def _infer_kv_attention_size(
    ...
    *,
    ...
    if explicit is not None and int(explicit) != expected:
        raise ValueError(
            "Compact K/V width must equal num_kv_heads * head_dim "
            f"({expected}), got {int(explicit)}"
        )
    w_k = weights.get("layer.0.w_k")
    if isinstance(w_k, np.ndarray) and w_k.ndim == 2 and int(w_k.shape[1]) != expected:
        raise ValueError(
            f"layer.0.w_k must use compact K/V width {expected}, got {int(w_k.shape[1])}"
        )
    return expected


def _make_matmul(network, dtype):
    def matmul(lhs, lhs_width, rhs_width, rhs_weights, _weight_name):
...
def build_native_decoder_engine(
    ...
    *,
    ...
    """Build one native-KV prefill or decode engine at fixed full capacity."""

    if precision != "fp16":
        raise ValueError("native Granite decoder requires FP16")
    if profile_mode not in ("prefill", "decode"):
        raise ValueError(
            f"native Granite profile_mode must be 'prefill' or 'decode', got {profile_mode!r}"
        )
    ...
    standard_attention_scale = float(1.0 / np.sqrt(head_dim))
    ...
    matmul = _make_matmul(network, work_np_dtype)
    ...
    for layer_idx in range(num_layers):
        ...
        native_attention = graph_ops.add_native_kv_cache_attention_from_rows(
            ...
            head_dim=head_dim,
            q_seq=None,
            scale=standard_attention_scale,
            tag=f"{prefix}.attn",
        )
    ...
    # The model-owned C++ pipeline samples ``float`` logits on both host and
    # device. Keep the decoder ABI explicit while all transformer math and KV
    # storage remain FP16.
    ...
    if verbose:
        print(
            f"[trtmc build] Building native Granite {profile_mode} engine "
            f"(layers={num_layers}, hidden={hidden}, heads={num_heads}, "
            f"kv_heads={num_kv_heads}, cache={max_cache_length}, "
            f"max_prefill={max_prefill_length}, precision=fp16) ...",
            file=sys.stderr,
        )

    plan = builder.build_serialized_network(network, trt_config)
    if plan is None:
        raise RuntimeError(f"native Granite {profile_mode} engine build failed")
    return bytes(plan)
