...
"""Mapped-weight contract for Granite's TensorRT native KV graph."""
...
def _shape(value: object, name: str) -> tuple[int, ...]:
    try:
        ...
        raise ValueError(f"native Granite weight {name} has an invalid shape") from exc
...
def _require(
    ...
    if name not in weights:
        raise ValueError(f"missing native Granite weight {name}")
    actual = _shape(weights[name], name)
    if actual != expected:
        raise ValueError(f"native Granite weight {name} must have shape {expected}, got {actual}")
...
def validate_native_kv_weights(
    ...
    if not isinstance(weights, Mapping):
        raise ValueError("native Granite weights must be a mapping")
    ...
    if malformed or layer_indices != set(range(layers)):
        raise ValueError(
            "native Granite weights require continuous layer indices; "
    ...
    for name, expected in (
        ...
        if name in weights and int(weights[name]) != expected:
            raise ValueError(f"native Granite metadata {name} must be {expected}")
    ...
    for layer in range(layers):
        ...
        forbidden.extend(
            f"{prefix}.{suffix}" for suffix in _BIAS_SUFFIXES if f"{prefix}.{suffix}" in weights
        )

    if forbidden:
        raise ValueError(
            "native dense Granite does not support bias weights: " + ", ".join(sorted(forbidden))
        )
