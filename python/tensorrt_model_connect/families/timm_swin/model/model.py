...
def add_reshape(network: trt.INetworkDefinition, inp: trt.ITensor, shape: tuple[int, ...]):
    layer = network.add_shuffle(inp)
    layer.reshape_dims = trt.Dims(shape)
    return layer.get_output(0)


def add_permute_reshape(
    network: trt.INetworkDefinition,
    inp: trt.ITensor,
    first_reshape: tuple[int, ...] | None,
    permutation: tuple[int, ...],
    second_reshape: tuple[int, ...] | None,
):
    """One shuffle doing reshape, transpose, and reshape in that order."""
    layer = network.add_shuffle(inp)
    if first_reshape is not None:
        layer.reshape_dims = trt.Dims(first_reshape)
    layer.second_transpose = trt.Permutation(permutation)
    if second_reshape is not None:
        # set_output_type cannot express a post-transpose reshape, so the
        # second reshape needs its own shuffle.
        out = layer.get_output(0)
        return add_reshape(network, out, second_reshape)
    return layer.get_output(0)
...
def add_roll(
    ...
    TensorRT has no roll operator, so each axis is split at the shift point
    and the two pieces are concatenated in the opposite order.

    Follows torch.roll: a positive shift moves elements towards higher
    indices, so the split point is measured from the *end*. Getting this sign
    backwards still produces a well-formed tensor, just the wrong one.
    """
    ...
    for shift, axis in zip(shifts, axes):
        ...
        for start, size in ((offset, extent - offset), (0, offset)):
            ...
            slice_layer = network.add_slice(
                out, trt.Dims(starts), trt.Dims(sizes), trt.Dims([1] * len(shape)))
            pieces.append(slice_layer.get_output(0))
...
def add_global_avg_pool_nhwc(network: trt.INetworkDefinition, inp: trt.ITensor):
    """Mean over the two spatial axes of an NHWC tensor, keeping [N, C]."""
    reduce_layer = network.add_reduce(
        inp, trt.ReduceOperation.AVG, (1 << 1) | (1 << 2), False)
