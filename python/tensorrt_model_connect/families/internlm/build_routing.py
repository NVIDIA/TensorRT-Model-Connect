...
"""Fail-closed routing contract for InternLM's TensorRT native KV path."""
...
def _checked_product(label: str, *values: int) -> int:
    ...
    for value in values:
        if value <= 0 or product > _UINT64_MAX // value:
            raise ValueError(f"native InternLM KV {label} exceeds uint64")
        product *= value
    return product
...
def native_kv_cache_geometry(
    ...
    *,
    ...
    """Return byte geometry for the required full-context BF16 cache."""

    capacity = _integer(capacity, "max_cache_length")
    context = _positive(config, "max_position_embeddings")
    if capacity != context:
        raise ValueError(
            "native InternLM KV requires max_cache_length == "
...
def _validate_internlm_rope(raw: dict, reasons: list[str]) -> None:
    """Accept InternLM2's dynamic-NTK declaration within official capacity.

    InternLM2 only changes its dynamic-NTK inverse frequencies after the
    active position exceeds ``max_position_embeddings``. Native KV capacity
    is exactly that value, so default RoPE and dynamic NTK are equivalent
    throughout the supported range. Linear scaling is not equivalent.
    """
    ...
    if rope_type not in ("", "default", "dynamic"):
        reasons.append(
            "native InternLM2 supports only default or dynamic-NTK RoPE")
        return
    unexpected = sorted(
        set(rope) - {"type", "rope_type", "factor"})
    if unexpected:
        reasons.append(
            "unsupported InternLM2 RoPE fields: " + ", ".join(unexpected))
    try:
        factor = float(rope.get("factor", 1.0))
    except (TypeError, ValueError, OverflowError):
        factor = 0.0
    if not math.isfinite(factor) or factor < 1.0:
        reasons.append("InternLM2 dynamic RoPE factor must be finite and >= 1")


def native_kv_architecture_capability(
    config: object,
) -> NativeKvCapability:
    """Accept any dense InternLM size with the qualified attention contract."""

    model_type = str(getattr(config, "model_type", "")).lower()
    if not model_type.startswith("internlm"):
        return _result(applicable=False)
    if model_type != "internlm2":
        return _result(reasons=["model_type must be exactly 'internlm2'"])
    ...
    if tuple(getattr(config, "architectures", ()) or ()) != (
        ...
        reasons.append(
            "architectures must contain exactly InternLM2ForCausalLM")
    ...
    try:
        ...
        if head_dim != 128:
            reasons.append("native InternLM attention requires head_dim=128")
    except ValueError as exc:
        reasons.append(str(exc))

    if str(getattr(config, "hidden_act", "")).lower() != "silu":
        reasons.append("native InternLM requires hidden_act='silu'")
    # InternLM2Config defaults bias=True when the field is absent. Require the
    # checkpoint to opt into the bias-free graph explicitly so the mapper
    # cannot silently ignore projection biases.
    if raw.get("bias", True) is not False:
        reasons.append("native InternLM requires explicit bias=false")
    ...
    if enabled:
        reasons.append("unsupported InternLM fields: " + ", ".join(enabled))
    try:
        if _integer(raw.get("pretraining_tp", 1), "pretraining_tp") != 1:
            reasons.append("native InternLM requires pretraining_tp=1")
    except ValueError as exc:
        reasons.append(str(exc))
    try:
        if float(raw.get("partial_rotary_factor", 1.0)) != 1.0:
            reasons.append("native InternLM requires full rotary embeddings")
    except (TypeError, ValueError, OverflowError):
    ...
    if layer_types is not None and (
        ...
        reasons.append("native InternLM does not support hybrid layer types")
    _validate_internlm_rope(raw, reasons)
...
def native_kv_build_capability(
    ...
    *,
    ...
    if str(precision).lower() != "bf16":
        reasons.append("native InternLM requires BF16")
    if str(raw.get("_decoder_engine_layout", "split")) != "split":
        reasons.append("native InternLM requires split prefill/decode engines")
    if raw.get("_rtx_build_requested"):
        reasons.append("native InternLM requires the standard TensorRT backend")
    if parallel_enabled or raw.get("_parallel_build_enabled"):
        reasons.append("native InternLM does not support tensor parallel builds")
    if (
        dynamic_kv_cache
        or raw.get("_runtime_dynamic_kv_requested")
        or raw.get("dynamic_kv_cache")
    ):
        reasons.append("native InternLM uses one fixed physical KV capacity")
    if (
        quantized
        or raw.get("quantization_config")
        or raw.get("_quantized_build_requested")
    ):
        reasons.append("native InternLM does not support quantized builds")
    if raw.get("_fp32_layers"):
        reasons.append("native InternLM does not support FP32 layer overrides")
    if debug_layer_outputs:
        reasons.append("native InternLM does not support debug layer outputs")
    try:
...
def prefer_native_default(config: object) -> bool:
    """InternLM production routing has no legacy KV fallback."""
