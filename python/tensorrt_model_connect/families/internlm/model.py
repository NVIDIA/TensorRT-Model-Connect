...
"""InternLM2 family model — handles fused wqkv and non-standard key names.
...
def build_engine(
    ...
    *,
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="InternLM tensor-parallel builds")
        if quant_ctx is not None:
            raise ValueError("InternLM tensor-parallel builds do not support quantization")
        if debug_layer_outputs:
            raise ValueError("InternLM tensor-parallel builds do not support debug_layer_outputs")
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete internlm bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("internlm does not support context-parallel builds")
    ...
    if parallel.enabled:
        ...
        if dynamic_kv_cache:
            raise NotImplementedError(
                "Tensor-parallel internlm builds do not support dynamic KV cache or TriAttention"
            )
    ...
    tokenizer_frame = prepare_tokenizer_special_frame(
        ...
        family_ensure=ensure_tokenizer_json,
        family_first=True,
    )
