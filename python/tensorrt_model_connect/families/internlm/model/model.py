...
def make_rope_table_half_dim(
    ...
    """Build InternLM's full-head RoPE cosine or sine table."""
...
def add_apply_rope_native(
    ...
    """Apply InternLM's full, non-interleaved RoPE."""
    rotary_embedding_dim = validate_native_rope_dim(head_dim)
...
def add_attention_core(
    ...
    """Build InternLM's explicitly masked native scaled attention."""
...
def add_attention_block(
    ...
    *,
    ...
    """Build InternLM2 pre-norm GQA with full, non-interleaved RoPE."""
    ...
    if cos_half_tensor is None or sin_half_tensor is None:
        raise ValueError("InternLM RoPE requires cosine and sine tensors")
...
def build_standard_decoder_engine(
    ...
    *,
    ...
    """Build an InternLM2 RMSNorm/RoPE/SwiGLU decoder engine."""
    ...
    if decoder_engine_role == "prefill" and debug_layer_outputs:
        raise NotImplementedError(
            "split prefill engine does not support InternLM debug outputs"
        )
    if not debug_layer_outputs and decoder_engine_role in ("dual_profile", "prefill"):
    ...
    validate_native_rope_dim(head_dim)
...
def _add_decoder_layer(
    *,
    ...
    """Add one sequential InternLM2 decoder layer."""
...
def build_dual_profile_decoder_engine(
    ...
    *,
    ...
    """Build an InternLM2 prefill/decode dynamic-sequence engine."""
    ...
    for layer_idx in range(num_layers):
        ...
        q = add_apply_rope_native(
            network, q, num_heads, head_dim, cos_half_table, sin_half_table,
            position_id, sequence_length=None,
        )
        k = add_apply_rope_native(
            network, k, num_kv_heads, head_dim, cos_half_table, sin_half_table,
            position_id, sequence_length=None,
        )
