...
"""Native-only split prefill/decode builder for dense InternLM models."""
...
def _swiglu_mlp(
    ...
    *,
    ...
    return matmul(
        gated.get_output(0), mlp_size, hidden,
...
def build_native_decoder_engine(
    ...
    *,
    ...
    if precision != "bf16":
        raise ValueError("native InternLM decoder requires BF16")
    if profile_mode not in ("prefill", "decode"):
        raise ValueError(
            "native InternLM profile_mode must be 'prefill' or 'decode', "
            f"got {profile_mode!r}")
    if max_cache_length < 2:
        raise ValueError(
            "native InternLM split prefill requires max_cache_length >= 2")

    if max_prefill_length is None:
        max_prefill_length = min(
            max_cache_length, _NATIVE_PREFILL_CHUNK_TOKENS)
    max_prefill_length = max(
        2, min(max_prefill_length, max_cache_length))
    opt_prefill_length = max(
        2, min(opt_prefill_length, max_prefill_length))
    ...
    for layer_idx in range(num_layers):
        ...
        q = matmul(
            normed, hidden, attention_size,
            weights[f"{prefix}.w_q"], f"{prefix}.w_q")
        k = matmul(
            normed, hidden, kv_attention_size,
    ...
    if verbose:
        print(
            f"[trtmc build] Building native InternLM {profile_mode} engine "
            f"(layers={num_layers}, hidden={hidden}, heads={num_heads}, "
            f"kv_heads={num_kv_heads}, cache={max_cache_length}, "
            f"max_prefill={max_prefill_length}, precision=bf16) ...",
            file=sys.stderr,
        )
    ...
    if plan is None:
        raise RuntimeError(
            f"native InternLM {profile_mode} engine build failed")
    return bytes(plan)
