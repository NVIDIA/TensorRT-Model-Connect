"""Tensor-parallel encoder-only engine builder (ALBERT-style).
...
def _validate_encoder_tp(
    ...
    if config.num_attention_heads % tp != 0:
        raise ValueError(
            "ALBERT tensor parallel requires num_attention_heads divisible by "
            f"tp_size ({config.num_attention_heads} vs {tp})"
        )
    if config.intermediate_size % tp != 0:
        raise ValueError(
            "ALBERT tensor parallel requires intermediate_size divisible by "
    ...
    if "relative_position_bias" in weights and weights["relative_position_bias"].shape[0] % tp != 0:
        raise ValueError("relative_position_bias head dim must be divisible by tp_size")
...
def shard_encoder_weights(
    ...
    *,
    ...
    """Return rank-local ALBERT-style encoder weights for the TP builder."""
...
def build_tp_encoder_engine(
    ...
    *,
    ...
    """Build a rank-local TRT engine plan for an ALBERT-style encoder.

    Args:
        config: Model architecture from config.json.
        weights: Loaded weight dict from ALBERT plugin.
    ...
    eps = config.rms_norm_eps  # Actually layer_norm_eps for ALBERT
    ...
    # -------------------------------------------------------------------
    embedding_size = weights["embedding"].shape[
        1
    ]  # may differ from hidden (e.g. ALBERT 128 vs 768)
    ...
    graph_ops.add_constant(network, (1, 1), np.array([eps], dtype=np.float32))
    ...
    if "relative_position_bias" in weights:
        ...
        rel_pos_bias_tensor = graph_ops.add_constant(network, rpb.shape, rpb)
    ...
    if verbose:
        print(
            f"[trtmc build] Building ALBERT encoder TRT engine "
...
def _add_encoder_layer(
    *,
    ...
    """Add one ALBERT encoder layer with POST-norm.

    ALBERT architecture per layer:
    ...
    if rel_pos_bias is not None:
        additive_mask = network.add_elementwise(
            rel_pos_bias, attn_mask, trt.ElementWiseOperation.SUM
        ).get_output(0)
