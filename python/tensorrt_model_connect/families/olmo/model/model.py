...
def add_layer_norm(
    ...
    """Apply OLMo's mean-centered LayerNorm in FP32."""
    ...
    denom_in = network.add_elementwise(
        variance.get_output(0), eps_tensor, trt.ElementWiseOperation.SUM
    )
...
def make_rope_table_half_dim(
    ...
    """Build OLMo's full-head RoPE cos or sin half-dimension table."""
    rotary_ndims = validate_native_rope_dim(head_dim)
    half = rotary_ndims // 2
...
def add_apply_rope_native(
    ...
    """Apply OLMo's rotate-half RoPE to row-major attention projections."""
...
def add_attention_core(
    ...
    """Run OLMo's scaled, additive-mask attention."""
...
def add_attention_from_rows(
    ...
    *,
    ...
    """Run native GQA attention on row-major Q/K/V tensors."""
    attention_size = num_heads * head_dim
    ...
    ctx_4d = add_attention_core(network, q_4d, k_4d, v_4d, mask, scale)
...
def add_attention_block(
    ...
    *,
    ...
    """Build OLMo pre-norm attention and the updated KV rows."""
    ...
    rope_dim = validate_native_rope_dim(rotary_embedding_dim)
...
def create_builder_context(verbose: bool):
...
def build_standard_decoder_engine(
    ...
    *,
    ...
    """Build a OLMo decoder engine, including debug and split-engine modes."""
    del norm_type  # Kept for the established family builder call contract.
    ...
    _dual_profile_disabled_for = (
        ...
        or (os.environ.get("TRTMC_NO_DUAL_PROFILE") == "1")
    )
    ...
    if raw_profile_rows:
        ...
        dynamic_kv_profile_rows = []
    builder, network, trt_config = create_builder_context(verbose)
    if precision == "fp16":
    ...
    cos_half_tensor = _cast_work_dtype(
        add_constant(network, cos_half_np.shape, cos_half_np, dtype=work_np_dtype)
    )
    sin_half_tensor = _cast_work_dtype(
        add_constant(network, sin_half_np.shape, sin_half_np, dtype=work_np_dtype)
    )
    ...
    if verbose:
        print(
            f"[trtmc build] Building OLMo engine ({num_layers} layers, hidden={hidden}, "
            f"attn={attention_size}, kv={kv_attention_size}, mlp={mlp_size}, "
...
def build_dual_profile_decoder_engine(
    ...
    *,
    ...
    """Build OLMo's prefill-only or combined prefill/decode engine."""
    ...
    elif os.environ.get("TRTMC_DECODE_ONLY_DEBUG") == "1":
        _add_profile(1, 1, fixed=True)
    else:
        _reverse = os.environ.get("TRTMC_REVERSE_PROFILE_ORDER", "0") == "1"
        if _reverse:
            _add_profile(1, 1, fixed=True)
            _add_profile(opt_prefill_length, max_prefill_length)
        else:
    ...
    if verbose:
        ...
        print(
            f"[trtmc build] Building {mode_label} OLMo engine "
