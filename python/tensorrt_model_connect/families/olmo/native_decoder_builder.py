...
"""Native-only split prefill/decode builder for dense OLMo models."""
...
def build_native_decoder_engine(
    ...
    *,
    ...
    if precision != "fp16":
        raise ValueError("native OLMo decoder requires FP16")
    if profile_mode not in ("prefill", "decode"):
        raise ValueError(
            f"native OLMo profile_mode must be 'prefill' or 'decode', got {profile_mode!r}"
        )
    ...
    if verbose:
        print(
            f"[trtmc build] Building native OLMo {profile_mode} engine "
    ...
    if plan is None:
        raise RuntimeError(f"native OLMo {profile_mode} engine build failed")
    return bytes(plan)
