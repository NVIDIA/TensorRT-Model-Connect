...
"""Fail-closed routing contract for OLMo's TensorRT native KV path."""
...
def resolved_rotary_dim(config: object) -> int:
    """Return OLMo v1's full-head RoPE width."""
    head_dim = resolved_head_dim(config)
    if head_dim < 2 or head_dim % 2:
        raise ValueError("head_dim must be even for OLMo RoPE")
    return head_dim
...
def _checked_product(label: str, *values: int) -> int:
    ...
    for value in values:
        if value <= 0 or product > _UINT64_MAX // value:
            raise ValueError(f"native OLMo KV {label} exceeds uint64")
        product *= value
    return product
...
def native_kv_cache_geometry(
    ...
    *,
    ...
    if capacity != context:
        raise ValueError(
            "native OLMo KV requires max_cache_length == "
...
def native_kv_architecture_capability(
    ...
    """Accept the dense OLMo v1 graph qualified by this family."""

    model_type = str(getattr(config, "model_type", "")).lower()
    if not model_type.startswith("olmo"):
        return _result(applicable=False)
    if model_type != "olmo":
        return _result(reasons=["model_type must be exactly 'olmo'"])
    ...
    if tuple(getattr(config, "architectures", ()) or ()) != ("OlmoForCausalLM",):
        reasons.append("architectures must contain exactly OlmoForCausalLM")
    ...
    try:
        ...
        if dimensions["num_key_value_heads"] != dimensions["num_attention_heads"]:
            reasons.append(
                "native OLMo currently requires MHA "
                "(num_key_value_heads == num_attention_heads)"
            )
        if head_dim % 8 or head_dim > 128:
            reasons.append(
                "native OLMo attention requires head_dim to be an "
    ...
    if str(getattr(config, "hidden_act", "")).lower() != "silu":
        reasons.append("native OLMo requires hidden_act='silu'")
    ...
    if raw.get("attention_bias") is not False:
        reasons.append("native OLMo requires attention_bias=false")
    if raw.get("partial_rotary_factor", 1.0) not in (None, 1, 1.0):
        reasons.append("native OLMo requires full-head RoPE")
    if _enabled(raw.get("clip_qkv")):
        reasons.append("native OLMo does not support clip_qkv")
    if _enabled(raw.get("use_parallel_residual")):
        reasons.append("native OLMo requires sequential residuals")
    ...
    if enabled:
        reasons.append("unsupported OLMo fields: " + ", ".join(enabled))
    if not bool(getattr(config, "tie_word_embeddings", False)):
        reasons.append("native OLMo requires tied word embeddings")
...
def native_kv_build_capability(
    ...
    *,
    ...
    if str(precision).lower() != "fp16":
        reasons.append("native OLMo requires FP16")
    if str(raw.get("_decoder_engine_layout", "split")) != "split":
        reasons.append("native OLMo requires split prefill/decode engines")
    if raw.get("_rtx_build_requested"):
        reasons.append("native OLMo requires the standard TensorRT backend")
    if parallel_enabled or raw.get("_parallel_build_enabled"):
        reasons.append("native OLMo does not support tensor parallel builds")
    if dynamic_kv_cache or raw.get("_runtime_dynamic_kv_requested") or raw.get("dynamic_kv_cache"):
        reasons.append("native OLMo uses one fixed physical KV capacity")
    if quantized or raw.get("quantization_config") or raw.get("_quantized_build_requested"):
        reasons.append("native OLMo does not support quantized builds")
    if raw.get("_fp32_layers"):
        reasons.append("native OLMo does not support FP32 layer overrides")
    if debug_layer_outputs:
        reasons.append("native OLMo does not support debug layer outputs")
    try:
...
def prefer_native_default(config: object) -> bool:
    """Prefer native KV for supported OLMo architectures."""
