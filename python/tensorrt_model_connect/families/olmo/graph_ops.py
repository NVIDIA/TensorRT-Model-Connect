...
def make_native_active_rope_inv_freq(
    ...
    *,
    ...
    if not np.isfinite(rope_theta) or rope_theta <= 0.0:
        raise ValueError(
            "TRT native OLMo RoPE requires rope_theta to be finite "
            f"and positive; got {rope_theta}")
    try:
        import torch
    except (ImportError, OSError) as exc:
        raise RuntimeError(
            "TensorRT native OLMo KV build requires PyTorch in the "
...
def add_native_kv_cache_attention_from_rows(
    ...
    *,
    ...
    if not hasattr(network, "add_kv_cache_update") or not hasattr(
        ...
        raise RuntimeError(
            "OLMo native KV cache requires TensorRT "
    ...
    if update_k is None or update_v is None:
        raise RuntimeError(
            "TensorRT failed to create OLMo KV-cache update layers")
    if tag:
    ...
    if q_4d.dtype != trt.float16:
        raise ValueError("OLMo native KV attention requires FP16 queries")
    ...
    if attention is None:
        raise RuntimeError("TensorRT failed to create OLMo native attention")
