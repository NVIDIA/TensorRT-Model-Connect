...
"""Native-only split prefill/decode builder for dense OPT models."""
...
def build_native_decoder_engine(
    ...
    *,
    ...
    if precision != "fp16":
        raise ValueError("native OPT decoder requires FP16")
    if profile_mode not in ("prefill", "decode"):
        raise ValueError(
            f"native OPT profile_mode must be 'prefill' or 'decode', got {profile_mode!r}"
        )
    ...
    position_embedding_table = const_in_work_dtype(
        network,
    ...
    position_embedding = network.add_gather(
    ...
    if verbose:
        print(
            f"[trtmc build] Building native OPT {profile_mode} engine "
    ...
    if plan is None:
        raise RuntimeError(f"native OPT {profile_mode} engine build failed")
    return bytes(plan)
