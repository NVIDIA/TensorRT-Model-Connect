...
"""Family-owned TensorRT graph builders for OPT decoders."""
...
#
# Two primitives replace manual primitive chains:
...
def add_attention_from_rows(
    ...
    *,
    ...
    """Apply OPT self-attention to row-major Q/K/V tensors."""
...
def apply_norm(
    ...
    """Apply OPT LayerNorm, using the native TRT layer when possible."""
    if beta is None:
        raise ValueError("OPT LayerNorm requires beta weights")
    if eps is not None:
...
def add_attention_block(
    ...
    *,
    ...
    """Add OPT pre-norm attention without applying the residual."""
...
def add_relu_mlp(
    ...
    *,
    ...
    """Add OPT's biased ``fc1 -> ReLU -> fc2`` MLP."""
    matmul = _make_matmul_fn(network, dtype, quant_ctx)
    _lp = layer_prefix or prefix
...
def norm_multi(
    ...
    """Apply OPT LayerNorm to a dynamic-sequence tensor."""
    if beta is None:
        raise ValueError("OPT LayerNorm requires beta weights")
...
def build_standard_decoder_engine(
    ...
    *,
    ...
    """Build OPT's fixed-Sq debug engine or its normal dual-profile engine."""
    import os as _os
...
def _supports_config(config, weights: "WeightDict") -> None:
    """Reject a non-OPT or grouped-query configuration."""
...
def build_dual_profile_decoder_engine(
    ...
    *,
    ...
    """Build OPT with dynamic prefill and fixed-token decode profiles.
    ...
    * ``"prefill"``: one prefill profile only. This is used by split-engine
    ...
    if verbose:
        ...
        print(
            f"[trtmc build] Building {mode_label} OPT engine (layers={num_layers}, hidden={hidden}, attn={attention_size}, kv={kv_attention_size}, mlp={mlp_size}, cache={max_cache_length}, opt_prefill={opt_prefill_length}, max_prefill={max_prefill_length}, precision={precision}) ...",
            file=sys.stderr,
        )
    plan = builder.build_serialized_network(network, trt_config)
    if plan is None:
