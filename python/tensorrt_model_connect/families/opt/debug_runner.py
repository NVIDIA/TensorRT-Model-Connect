...
"""OPT-owned debug runner adapter."""
...
def _bundle_header(bundle_path: str) -> tuple[dict, int]:
    with open(bundle_path, "rb") as bundle:
        ...
        return json.loads(bundle.read(header_size).decode("utf-8")), 16 + header_size


def load_section_from_bundle(bundle_path: str, section_name: str) -> bytes | None:
    header, payload_start = _bundle_header(bundle_path)
    section = header.get("sections", {}).get(section_name)
    if section is None:
        return None
    with open(bundle_path, "rb") as bundle:
        bundle.seek(payload_start + section["offset"])
...
def runner_from_bundle(
    *,
    ...
    del bundle_path
    if runtime_strategy != "opt_decoder_kv_cache":
        return None
