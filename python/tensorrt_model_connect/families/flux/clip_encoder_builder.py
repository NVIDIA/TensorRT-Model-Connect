...
def build_clip_encoder_engine(
    ...
    *,
    ...
    tok_const = graph_ops.add_constant(
        network, tok_embed_w.shape, tok_embed_w)
    ...
    pos_const = graph_ops.add_constant(
        network, pos_embed_w.shape, pos_embed_w)
    pos_gather = network.add_gather(pos_const, pos_ids_const, axis=0)
    hidden = network.add_elementwise(
        hidden, pos_gather.get_output(0),
    ...
    for i in range(num_layers):
        ...
        q = graph_ops.add_bias_sum(network, q, hidden_size,
            weights[f"{p}.self_attn.q_proj.bias"])

        k = graph_ops.add_matmul_rhs_constant(
            network, normed, hidden_size, hidden_size,
            weights[f"{p}.self_attn.k_proj.weight"])
        k = graph_ops.add_bias_sum(network, k, hidden_size,
