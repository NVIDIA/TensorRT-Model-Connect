...
# The explicit BF16 trt.Weights constructor borrows the numpy buffer.
...
def add_constant(
    ...
    """Add a constant tensor, including explicit BF16 weight handling."""
...
def _add_fp8_qdq(
    ...
    *,
    ...
    dq = network.add_dequantize(q.get_output(0), scale_t, output_dtype or tensor.dtype)
...
def add_attention_core(
    ...
    def _scale_attention_input(tensor: trt.ITensor, factor: float) -> trt.ITensor:
        ...
        scale_t = add_constant(
            network, (1, 1, 1, 1), np.array([[[[factor]]]]), dtype=scale_np_dtype
        )
        if tensor.dtype == trt.bfloat16:
            scale_t = network.add_cast(scale_t, trt.bfloat16).get_output(0)
        return network.add_elementwise(tensor, scale_t, trt.ElementWiseOperation.PROD).get_output(0)
    ...
    if quant_scales:
        ...
        q_attn = _add_fp8_qdq(
            network, q_attn, float(quant_scales["q_bmm_scale"]), output_dtype=output_dtype
        )
        k_attn = _add_fp8_qdq(
            network, k_attn, float(quant_scales["k_bmm_scale"]), output_dtype=output_dtype
        )
        v_attn = _add_fp8_qdq(
            network, v_attn, float(quant_scales["v_bmm_scale"]), output_dtype=output_dtype
        )
    ...
    if quant_scales and quant_scales.get("bmm2_output_scale") is not None:
        result = _add_fp8_qdq(
            network, result, float(quant_scales["bmm2_output_scale"]), output_dtype=output_dtype
        )
    return result
...
def add_attention_from_rows(
    ...
    *,
    ...
    if logit_softcap is not None and float(logit_softcap) > 0.0:
        if quant_scales:
            raise NotImplementedError("quantized IAttention with logit_softcap is not supported")
        if causal:
...
def add_tvm_ffi_kernel(
    ...
    """Add a TVM-FFI kernel call as a TensorRT plugin layer."""
    import json
    ...
    fields = [
        ...
        trt.PluginField(
            "shape_spec",
            json.dumps(spec_dict).encode("utf-8"),
            trt.PluginFieldType.CHAR,
        ),
    ]
    plugin = creator.create_plugin(
        "tvm_ffi_kernel", trt.PluginFieldCollection(fields)
    )
