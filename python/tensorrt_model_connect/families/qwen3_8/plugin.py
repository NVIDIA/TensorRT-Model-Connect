...
"""Qwen3.8 family plugin -- Hybrid Gated DeltaNet + self-attention decoder.
...
`Qwen38Plugin.matches_config` for the config-body discriminator.
...
class Qwen38Plugin:
    ...
    # Config keys that Qwen3.8 text configs carry and Qwen3.5 text configs do
    # not. Qwen3.8 ships `model_type: "qwen3_5"` and
    # `architectures: ["Qwen3_5ForConditionalGeneration"]`, so the checkpoint
    # strings are useless for telling the two families apart and the config
    # body is the only available signal.
    _QWEN38_MARKER_KEYS = ("output_gate_type",)
    # Conversely, every Qwen3.5 text config carries `mlp_only_layers`; Qwen3.8
    # dropped it. Requiring its absence keeps a future Qwen3.5 refresh that
    # gained `output_gate_type` from being mis-claimed by this family.
    _QWEN35_MARKER_KEYS = ("mlp_only_layers",)

    def matches(self, model_type: str) -> bool:
        mt = model_type.lower()
        return mt in {"qwen3_8", "qwen3.8", "qwen38"}

    def matches_config(self, config: object) -> bool:
        """Claim Qwen3.8 checkpoints that masquerade as `model_type: qwen3_5`.

        Returning False here is what lets a genuine Qwen3.5 checkpoint fall
        through to the qwen3_5 family: family discovery consults
        `architecture_patterns` first, and only an affirmative
        `matches_config` binds the model to this plugin.
        """
        ...
        arch_ok = any(
            "qwen3_5forconditionalgeneration" in str(value).lower()
            for value in architectures
        )
        model_type_ok = str(raw.get("model_type", "")).lower() in {
            "qwen3_5", "qwen3.5", "qwen3_8", "qwen3.8",
        }
        if not arch_ok and not model_type_ok:
            return False

        text_cfg = raw.get("text_config")
        if not isinstance(text_cfg, dict):
            text_cfg = raw
        if any(key in text_cfg for key in self._QWEN35_MARKER_KEYS):
            return False
        return any(key in text_cfg for key in self._QWEN38_MARKER_KEYS)
    ...
    def get_bundle_config_overrides(self, config: ModelConfig) -> dict:
        ...
        # Python side.
        #
        # `eos_token_id` is deliberately not republished: the builder already
        # emits a top-level list from generation_config.json, which carries all
        # stop ids, whereas text_config holds only one.
        flat_dims = {}
