...
constexpr std::array<nvinfer1::DataType, TrackerMemoryFfiPlugin::kInputCount> kInputTypes{
...
bool valid_content_addressed_name(std::string_view name, int32_t batch_size) {
    ...
    if (!name.starts_with(kPrefix))
        return false;
    const auto policy_end = name.find('.', kPrefix.size());
...
bool output_dimensions_valid(const nvinfer1::Dims& dims, int32_t batch_size) {
    if (batch_size == 1)
        return exact_dimensions(dims, {2, TrackerMemoryFfiPlugin::kSpatialTokens, 1,
                                       TrackerMemoryFfiPlugin::kMemoryChannels});
    if (batch_size == 2)
        return exact_dimensions(dims, {2, 2, TrackerMemoryFfiPlugin::kSpatialTokens,
                                       TrackerMemoryFfiPlugin::kMemoryChannels});
    return false;
}
...
bool descriptor_types_valid(const nvinfer1::PluginTensorDesc* inputs,
    ...
    if (inputs == nullptr || outputs == nullptr)
        return false;
    for (int32_t index = 0; index < TrackerMemoryFfiPlugin::kInputCount; ++index) {
...
bool dynamic_descriptor_contract_valid(const nvinfer1::DynamicPluginTensorDesc* inputs,
    ...
    if (inputs == nullptr || outputs == nullptr ||
        ...
        return false;
    std::array<nvinfer1::PluginTensorDesc, TrackerMemoryFfiPlugin::kInputCount> descriptors{};
    std::array<nvinfer1::PluginTensorDesc, TrackerMemoryFfiPlugin::kInputCount> minima{};
    std::array<nvinfer1::PluginTensorDesc, TrackerMemoryFfiPlugin::kInputCount> maxima{};
...
void report_ffi_error(const std::string& kernel_name) {
    ...
    if (cell->message.data != nullptr && cell->message.size > 0) {
        std::cerr << "[Sam3TrackerMemoryFfi] " << kernel_name << ": "
...
TrackerMemoryFfiPlugin::TrackerMemoryFfiPlugin(std::string kernel_name, int32_t batch_size)
    : kernel_name_(std::move(kernel_name)), batch_size_(batch_size) {}

TrackerMemoryFfiPlugin::TrackerMemoryFfiPlugin(const void* data, std::size_t length) {
...
char const* TrackerMemoryFfiPlugin::getPluginType() const noexcept {
    return kPluginName;
}

char const* TrackerMemoryFfiPlugin::getPluginVersion() const noexcept {
    return kPluginVersion;
}

int32_t TrackerMemoryFfiPlugin::getNbOutputs() const noexcept {
    return kOutputCount;
}

int32_t TrackerMemoryFfiPlugin::initialize() noexcept {
...
void TrackerMemoryFfiPlugin::terminate() noexcept {
    release_kernel();
}

void TrackerMemoryFfiPlugin::destroy() noexcept {
    release_kernel();
    delete this;
}

std::size_t TrackerMemoryFfiPlugin::getSerializationSize() const noexcept {
    return sizeof(uint32_t) + kernel_name_.size() + sizeof(batch_size_);
}

void TrackerMemoryFfiPlugin::serialize(void* buffer) const noexcept {
...
void TrackerMemoryFfiPlugin::setPluginNamespace(char const* plugin_namespace) noexcept {
    namespace_ = plugin_namespace != nullptr ? plugin_namespace : "";
}

char const* TrackerMemoryFfiPlugin::getPluginNamespace() const noexcept {
    return namespace_.c_str();
}

nvinfer1::DataType TrackerMemoryFfiPlugin::getOutputDataType(int32_t, nvinfer1::DataType const*,
                                                             int32_t) const noexcept {
    return nvinfer1::DataType::kFLOAT;
}

TrackerMemoryFfiPlugin* TrackerMemoryFfiPlugin::clone() const noexcept {
    try {
        auto* plugin = new TrackerMemoryFfiPlugin(kernel_name_, batch_size_);
...
nvinfer1::DimsExprs
TrackerMemoryFfiPlugin::getOutputDimensions(int32_t, nvinfer1::DimsExprs const*, int32_t,
    ...
    if (batch_size_ == 1) {
        ...
        output.d[2] = builder.constant(1);
        output.d[3] = builder.constant(kMemoryChannels);
    } else {
        output.d[1] = builder.constant(2);
        output.d[2] = builder.constant(kSpatialTokens);
        output.d[3] = builder.constant(kMemoryChannels);
    }
    return output;
}

bool TrackerMemoryFfiPlugin::supportsFormatCombination(int32_t position,
...
void TrackerMemoryFfiPlugin::configurePlugin(nvinfer1::DynamicPluginTensorDesc const* inputs,
                                             int32_t input_count,
...
std::size_t TrackerMemoryFfiPlugin::getWorkspaceSize(nvinfer1::PluginTensorDesc const*, int32_t,
                                                     nvinfer1::PluginTensorDesc const*,
                                                     int32_t) const noexcept {
    return 0;
}

bool TrackerMemoryFfiPlugin::valid_contract() const noexcept {
    return (batch_size_ == 1 || batch_size_ == 2) &&
           valid_content_addressed_name(kernel_name_, batch_size_);
}

bool TrackerMemoryFfiPlugin::resolve_kernel() noexcept {
    ...
    if (TVMFFIFunctionGetGlobal(&name, &resolved) != 0 || resolved == nullptr) {
        std::cerr << "[Sam3TrackerMemoryFfi] unresolved kernel " << kernel_name_ << '\n';
        return false;
    }
...
void TrackerMemoryFfiPlugin::release_kernel() noexcept {
    if (cached_function_ != nullptr) {
...
int32_t TrackerMemoryFfiPlugin::enqueue(nvinfer1::PluginTensorDesc const* input_desc,
    ...
    const auto fail = [this](const char* reason) {
        std::cerr << "[Sam3TrackerMemoryFfi] " << kernel_name_ << ": enqueue rejected: " << reason
                  << '\n';
        return -1;
    };
...
class TrackerMemoryFfiPluginCreator final : public nvinfer1::IPluginCreator {
  public:
    TrackerMemoryFfiPluginCreator() {
    ...
    char const* getPluginName() const noexcept override {
        return TrackerMemoryFfiPlugin::kPluginName;
    }
    char const* getPluginVersion() const noexcept override {
        return TrackerMemoryFfiPlugin::kPluginVersion;
    }
    ...
    createPlugin(char const*, nvinfer1::PluginFieldCollection const* collection) noexcept override {
        try {
            ...
            if (!valid_content_addressed_name(kernel_name, batch_size))
                return nullptr;
            auto* plugin = new TrackerMemoryFfiPlugin(std::move(kernel_name), batch_size);
...
static nvinfer1::PluginRegistrar<TrackerMemoryFfiPluginCreator> tracker_memory_plugin_registrar{};

} // namespace

} // namespace trtmc::sam3

extern "C" const char* trtmc_sam3_tracker_memory_plugin_version() noexcept {
    return trtmc::sam3::TrackerMemoryFfiPlugin::kPluginVersion;
}
