...
def build_sam3_text_encoder_engine(
    ...
    *,
    ...
    pos_ids = network.add_constant(
        (max_seq_len,), trt.Weights(np.ascontiguousarray(pos_ids_np))
    ).get_output(0)
    ...
    invalid_mask = network.add_elementwise(
        valid_ones, mask_float, trt.ElementWiseOperation.SUB
    ).get_output(0)
    invalid_mask_4d = network.add_shuffle(invalid_mask)
    invalid_mask_4d.reshape_dims = (1, 1, 1, max_seq_len)
    mask_penalty = graph_ops.add_constant(network, (1, 1, 1, 1), np.array([-1e9], dtype=np.float32))
    ...
    attention_bias = network.add_elementwise(
        causal_const, padding_bias.get_output(0), trt.ElementWiseOperation.SUM
    ).get_output(0)
    ...
    for layer_idx in range(num_layers):
        ...
        q = graph_ops.add_matmul_rhs_constant(
            network, normed, hidden_size, hidden_size, weights[f"{prefix}.self_attn.q_proj.weight"]
        )
        q = graph_ops.add_bias_sum(
            network, q, hidden_size, weights[f"{prefix}.self_attn.q_proj.bias"]
        )
        k = graph_ops.add_matmul_rhs_constant(
            network, normed, hidden_size, hidden_size, weights[f"{prefix}.self_attn.k_proj.weight"]
        )
        k = graph_ops.add_bias_sum(
            network, k, hidden_size, weights[f"{prefix}.self_attn.k_proj.bias"]
        )
        v = graph_ops.add_matmul_rhs_constant(
            network, normed, hidden_size, hidden_size, weights[f"{prefix}.self_attn.v_proj.weight"]
        )
        ...
        attn_out = graph_ops.add_bias_sum(
            network, attn_out, hidden_size, weights[f"{prefix}.self_attn.out_proj.bias"]
        )
        ...
        fc2 = graph_ops.add_matmul_rhs_constant(
            network, activated, intermediate_size, hidden_size, weights[f"{prefix}.mlp.fc2.weight"]
        )
        fc2 = graph_ops.add_bias_sum(network, fc2, hidden_size, weights[f"{prefix}.mlp.fc2.bias"])
