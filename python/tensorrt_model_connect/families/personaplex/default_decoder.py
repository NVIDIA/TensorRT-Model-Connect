...
@with_builder_context(
    workspace_bytes=1 << 30, disable_tf32=True, builder_optimization_level=0, max_num_tactics=1
)
def build_standard_decoder_engine(
    ...
    *,
    ...
    if invalid_fp32_layers:
        raise ValueError(
            f"PersonaPlex temporal fp32_layers contains out-of-range indices: {invalid_fp32_layers}; expected 0-{num_layers - 1}"
        )
    ...
    for layer_idx in range(num_layers):
        ...
        if layer_is_fp32:
            ...
            result["present_k"] = network.add_cast(result["present_k"], work_trt_dtype).get_output(
                0
            )
            result["present_v"] = network.add_cast(result["present_v"], work_trt_dtype).get_output(
                0
            )
    ...
    logits = graph_ops.add_matmul_rhs_constant(
        network, hidden_state, hidden, out_vocab, weights["w_out"], dtype=tail_dtype
    )
