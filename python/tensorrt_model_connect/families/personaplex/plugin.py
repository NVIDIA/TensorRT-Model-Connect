...
class PersonaPlexPlugin:
    ...
    def build_extra_engines(
        ...
        *,
        ...
        mimi_encoder_precision = _component_precision(config, _MIMI_ENCODER_COMPONENT, precision)
...
def _build_mimi_encoder_engine(
    ...
    *,
    ...
    x = _add_mimi_conv1d_causal(
        network, x, ds_w, None, hidden_size, 4, stride=compress, dtype=work_np_dtype
    )
    ...
    sem_projected = graph_ops.add_conv1d(
        network, x, sem_input_proj_w, None, codebook_dim, 1, dtype=work_np_dtype
    )
    ...
    cb_const = graph_ops.add_constant(
        network, (codebook_dim, codebook_size), sem_cb.T.copy(), dtype=work_np_dtype
    )
    ...
    half_norms = graph_ops.add_constant(
        network, (1, codebook_size), (-0.5 * cb_norms_sq), dtype=work_np_dtype
    )
    ...
    acou_projected = graph_ops.add_conv1d(
        network, x, acou_input_proj_w, None, codebook_dim, 1, dtype=work_np_dtype
    )
    ...
    for cb_idx in range(31):
        ...
        acou_cb_const = graph_ops.add_constant(
            network, (codebook_dim, codebook_size), acou_cb.T.copy(), dtype=work_np_dtype
        )
        acou_cb_norms_sq = np.sum(acou_cb**2, axis=1, keepdims=True).T
        acou_half_norms = graph_ops.add_constant(
            network, (1, codebook_size), -0.5 * acou_cb_norms_sq, dtype=work_np_dtype
        )
