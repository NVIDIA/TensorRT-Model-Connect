...
Scope: GPT-NeoX LayerNorm, GELU MLP, partial non-interleaved RoPE, and
sequential or parallel residual. Quantized builds thread Q/DQ insertion
through every projection matmul.
...
def build_dual_profile_tp_decoder_engine(
    ...
    *,
    ...
    matmul = graph_ops.make_matmul_fn(network, work_np_dtype, quant_ctx)
    ...
    if verbose:
        print(
            f"[trtmc-build] Building tensor-parallel decoder engine (layers={num_layers}, hidden={hidden}, attn={attention_size}, kv={kv_attention_size}, mlp={mlp_size}, cache={max_cache_length}, opt_prefill={opt_prefill_length}, max_prefill={max_prefill_length}, norm={'layernorm'}, mlp_type={'gelu_fc'}, pos={'rope'}, precision={precision}, tp={parallel.tp_size}, rank={parallel.rank}) ...",
            file=sys.stderr,
        )
