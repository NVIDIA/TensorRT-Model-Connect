...
"""GPT-NeoX family model (Pythia, RedPajama) — parallel residual + partial RoPE.
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete gpt_neox bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("gpt_neox does not support context-parallel builds")
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="gpt_neox tensor-parallel builds")

        if quant_ctx is not None:
            raise ValueError("gpt_neox tensor-parallel builds do not support quantization")
        if dynamic_kv_cache:
            raise NotImplementedError(
                "Tensor-parallel gpt_neox builds do not support dynamic KV cache or TriAttention"
            )
