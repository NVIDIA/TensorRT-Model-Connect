...
"""GPT-NeoX-owned debug runner adapter."""
...
def _read_bundle_section(bundle_path: str, section_name: str) -> tuple[bytes | None, dict]:
...
def load_engine_from_bundle(
    ...
    engine_plan, header = _read_bundle_section(bundle_path, section_name)
...
def load_config_from_bundle(bundle_path: str) -> dict:
    config_json, _ = _read_bundle_section(bundle_path, "config.json")
    return {} if config_json is None else json.loads(config_json.decode("utf-8"))
...
def runner_from_bundle(
    *,
    ...
    del bundle_path
    if runtime_strategy != "gpt_neox_decoder_kv_cache":
        return None
