...
def _gelu_fc_mlp(network: trt.INetworkDefinition, inp: trt.ITensor, *, matmul, weights: 'WeightDict', prefix: str, hidden: int, mlp_size: int, work_np_dtype: np.dtype) -> trt.ITensor:
    ...
    activated = graph_ops.add_gelu_new(network, fc1, dtype=work_np_dtype)
...
def build_dual_profile_tp_decoder_engine(
    ...
    *,
    ...
    if embed_norm is not None:
        ...
        hidden_state = _norm_multi(network, hidden_state, hidden, embed_norm, embed_norm_beta, eps_tensor, work_np_dtype)
    ...
    for layer_idx in range(num_layers):
        ...
        normed = _norm_multi(network, hidden_state, hidden, weights[f'{prefix}.input_norm'], weights.get(f'{prefix}.input_norm_beta'), eps_tensor, work_np_dtype)
        ...
        if post_attn_norm_w is not None:
            norm2 = _norm_multi(network, hidden_state, hidden, post_attn_norm_w, weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, work_np_dtype)
        else:
            norm2 = normed
    ...
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = _norm_multi(network, hidden_state, hidden, final_norm, weights.get('final_norm_beta'), eps_tensor, work_np_dtype)
