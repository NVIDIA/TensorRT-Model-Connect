...
def apply_norm(
    ...
    """Apply CodeGen LayerNorm, using the native layer when epsilon is known."""
    if beta is None:
...
def add_attention_block(network: trt.INetworkDefinition, hidden: trt.ITensor, cache_k: trt.ITensor, cache_v: trt.ITensor, attention_mask: trt.ITensor, position_id: trt.ITensor, *, weights: WeightDict, prefix: str, hidden_size: int, attention_size: int, num_heads: int, head_dim: int, max_cache_length: int, eps_tensor: trt.ITensor, kv_attention_size: int | None=None, num_kv_heads: int | None=None, attention_scale: float | None=None, eps: float | None=None, alibi_slopes_tensor: trt.ITensor | None=None, alibi_indices_tensor: trt.ITensor | None=None, dtype: np.dtype=np.float32, quant_ctx: QuantContext | None=None, layer_prefix: str='', cos_half_tensor: trt.ITensor | None=None, sin_half_tensor: trt.ITensor | None=None, rotary_embedding_dim: int=0, ffi_attention_kernel: str | None=None, dynamic_kv_cache: bool=False) -> dict[str, trt.ITensor]:
    ...
    normed = apply_norm(network, hidden, hidden_size, weights[f'{prefix}.input_norm'], weights.get(f'{prefix}.input_norm_beta'), eps_tensor, dtype=dtype, eps=eps)
    ...
    q = add_apply_rope_native(network, q, num_heads, head_dim, cos_half_tensor, sin_half_tensor, position_id, rope_dim, True)
    k = add_apply_rope_native(network, k, num_kv_heads, head_dim, cos_half_tensor, sin_half_tensor, position_id, rope_dim, True)
    present_k = k
    present_v = v
...
def build_standard_decoder_engine(
    ...
    *,
    ...
    if not _dual_profile_disabled_for and decoder_engine_role in ('dual_profile', 'prefill'):
        return build_dual_profile_decoder_engine(config, weights, max_cache_length, precision=precision, quant_ctx=quant_ctx, partial_rotary_factor=partial_rotary_factor, scale_attn_weights=scale_attn_weights, verbose=verbose, profile_mode='prefill' if decoder_engine_role == 'prefill' else 'dual_profile')
    ...
    cos_half_np = make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, True, partial_rotary_factor, interleaved=True)
    sin_half_np = make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, False, partial_rotary_factor, interleaved=True)
    ...
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = apply_norm(network, hidden_state, hidden, final_norm, weights.get('final_norm_beta'), eps_tensor, dtype=work_np_dtype, eps=config.rms_norm_eps)
...
def _add_decoder_layer(*, network: trt.INetworkDefinition, hidden: trt.ITensor, cache_k: trt.ITensor, cache_v: trt.ITensor, attention_mask: trt.ITensor, position_id: trt.ITensor, attention_scale: float | None, eps_tensor: trt.ITensor, weights: WeightDict, prefix: str, hidden_size: int, attention_size: int, kv_attention_size: int, mlp_size: int, num_heads: int, num_kv_heads: int, head_dim: int, max_cache_length: int, alibi_slopes_tensor: trt.ITensor | None=None, alibi_indices_tensor: trt.ITensor | None=None, dtype: np.dtype=np.float32, quant_ctx: QuantContext | None=None, cos_half_tensor: trt.ITensor | None=None, sin_half_tensor: trt.ITensor | None=None, rotary_embedding_dim: int=0, ffi_attention_kernel: str | None=None, dynamic_kv_cache: bool=False, eps: float | None=None) -> dict[str, trt.ITensor]:
    ...
    if post_attn_norm_w is not None:
        norm2 = apply_norm(network, hidden, hidden_size, post_attn_norm_w, weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, dtype=dtype, eps=eps)
    else:
        norm2 = attn['normed']
...
def build_dual_profile_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp16', opt_prefill_length: int=64, max_prefill_length: int | None=None, quant_ctx: 'QuantContext | None'=None, partial_rotary_factor: float=1.0, scale_attn_weights: bool=True, verbose: bool=False, dynamic_kv_profile_rows: list[int] | None=None, profile_mode: str='dual_profile') -> bytes:
    ...
    * ``"prefill"``: one prefill profile only. This is used by split-engine
    ...
    validate_native_rope_dim(rotary_embedding_dim)
    cos_half_np = make_rope_table_half_dim(kmax, head_dim, config.rope_theta, True, partial_rotary_factor, interleaved=True)
    sin_half_np = make_rope_table_half_dim(kmax, head_dim, config.rope_theta, False, partial_rotary_factor, interleaved=True)
    ...
    for layer_idx in range(num_layers):
        ...
        q = add_apply_rope_native(network, q, num_heads, head_dim, cos_half_table, sin_half_table, position_id, rotary_embedding_dim, True, sequence_length=None)
        k = add_apply_rope_native(network, k, num_kv_heads, head_dim, cos_half_table, sin_half_table, position_id, rotary_embedding_dim, True, sequence_length=None)
