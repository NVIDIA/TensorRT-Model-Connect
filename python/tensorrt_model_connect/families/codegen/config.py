...
@dataclass
class ModelConfig:
    """CodeGen fields consumed by the family-owned weight and graph builders."""

    model_type: str = "codegen"
    vocab_size: int = 0
    hidden_size: int = 0
    ...
    def from_json(cls, text: str) -> ModelConfig:
        raw = json.loads(text)
        hidden_size = raw.get("hidden_size") or raw.get("n_embd") or 0
        num_heads = raw.get("num_attention_heads") or raw.get("n_head") or 1
        return cls(
            model_type=raw.get("model_type") or "codegen",
            ...
            num_hidden_layers=(
                raw.get("num_hidden_layers") or raw.get("n_layer") or 0
            ),
    ...
    @classmethod
    def create_tiny(cls, model_type: str = "codegen", **overrides) -> ModelConfig:
        values = {
