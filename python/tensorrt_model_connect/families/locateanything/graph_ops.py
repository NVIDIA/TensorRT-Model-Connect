...
def make_native_active_rope_inv_freq(
    ...
    if not np.isfinite(rope_theta) or rope_theta <= 0.0:
        raise ValueError(
            "LocateAnything native RoPE requires rope_theta to be finite and positive; "
            f"got {rope_theta}"
        )
    ...
    try:
        ...
        raise RuntimeError(
            "LocateAnything native active-position RoPE requires PyTorch at build time"
        ) from exc
...
def add_native_kv_cache_attention_from_rows(
    ...
    *,
    ...
    if not hasattr(network, "add_kv_cache_update") or not hasattr(
        ...
        raise RuntimeError(
            "LocateAnything native KV cache requires TensorRT add_kv_cache_update "
    ...
    if update_k is None or update_v is None:
        raise RuntimeError("TensorRT failed to create LocateAnything KV-cache update layers")
    if tag:
    ...
    if q_4d.dtype != trt.bfloat16:
        raise ValueError("LocateAnything native KV attention requires BF16 queries")
    if scale is None:
        scale = float(1.0 / np.sqrt(head_dim)) if head_dim > 0 else 1.0
    ...
    if attention is None:
        raise RuntimeError("TensorRT failed to create LocateAnything native attention")
