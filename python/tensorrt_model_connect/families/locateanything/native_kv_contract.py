...
"""Mapped-weight validation for LocateAnything's Qwen2 native KV graph."""
...
def _require(weights: Mapping[str, object], name: str, shape: tuple[int, ...]) -> None:
    if name not in weights:
        raise ValueError(f"missing LocateAnything native KV weight {name}")
    actual = tuple(int(dim) for dim in weights[name].shape)
    if actual != shape:
        raise ValueError(
            f"LocateAnything native KV weight {name} must have shape {shape}, got {actual}")


def validate_native_kv_weights(config: object, weights: Mapping[str, object]) -> None:
    if not isinstance(weights, Mapping):
        raise ValueError("LocateAnything native KV weights must be a mapping")
    ...
    for name, expected in (
        ...
        if int(weights.get(name, expected)) != expected:
            raise ValueError(f"LocateAnything native KV metadata {name} must be {expected}")
