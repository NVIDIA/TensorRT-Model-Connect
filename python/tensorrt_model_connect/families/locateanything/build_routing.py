...
"""Fail-closed build contract for LocateAnything TensorRT native KV cache."""
...
def _positive(config: object, name: str) -> int:
    ...
    if value <= 0:
        raise ValueError(f"LocateAnything native KV requires positive {name}")
    return value


def validate_native_kv_architecture(config: object) -> None:
    if str(getattr(config, "model_type", "")).lower() != "locateanything":
        raise ValueError("LocateAnything native KV requires model_type='locateanything'")
    ...
    if not isinstance(text, dict) or str(text.get("model_type", "")).lower() != "qwen2":
        raise ValueError("LocateAnything native KV requires a Qwen2 text_config")
    ...
    if hidden != heads * head_dim or head_dim != 128:
        raise ValueError(
            "LocateAnything native KV requires hidden_size == num_attention_heads * 128")
    if heads % kv_heads:
        raise ValueError(
            "LocateAnything native KV requires query heads divisible by KV heads")
    if str(getattr(config, "hidden_act", "")).lower() != "silu":
        raise ValueError("LocateAnything native KV requires hidden_act='silu'")
    for name in ("rms_norm_eps", "rope_theta"):
        value = float(getattr(config, name, 0.0))
        if not math.isfinite(value) or value <= 0.0:
            raise ValueError(f"LocateAnything native KV requires positive finite {name}")

    rope = text.get("rope_scaling")
    if rope is not None:
        if not isinstance(rope, dict):
            raise ValueError("LocateAnything native KV rope_scaling must be an object")
        rope_type = str(rope.get("rope_type", rope.get("type", "default"))).lower()
        if rope_type != "default":
            raise ValueError(
                "LocateAnything native KV supports only default RoPE")
...
def validate_native_kv_build(
    ...
    *,
    ...
    if str(precision).lower() != "bf16":
        raise ValueError("LocateAnything native KV requires BF16")
    context = int(getattr(config, "max_position_embeddings"))
    if int(max_cache_length) != context:
        raise ValueError(
            "LocateAnything native KV requires max_cache_length equal to the model "
            f"context ({context})")
    raw = getattr(config, "raw", {})
    ...
    if quantized:
        raise ValueError("LocateAnything native KV does not support quantized builds")
    if debug_layer_outputs:
        raise ValueError("LocateAnything native KV does not support debug layer outputs")

    if bool(getattr(parallel, "enabled", False)):
        tp_size = int(getattr(parallel, "tp_size", 1))
        if tp_size not in (2, 4, 8):
            raise ValueError("LocateAnything native KV TP size must be 2, 4, or 8")
        if int(getattr(config, "num_attention_heads")) % tp_size:
            raise ValueError("LocateAnything query heads must be divisible by TP size")
        if int(getattr(config, "num_key_value_heads")) % tp_size:
            raise ValueError("LocateAnything KV heads must be divisible by TP size")
