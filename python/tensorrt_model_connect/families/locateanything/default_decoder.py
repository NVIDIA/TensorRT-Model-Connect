...
"""Compatibility entrypoint for LocateAnything's native-KV decoder builder.

LocateAnything no longer exposes its legacy cache-concatenation graph. Callers of
...
def build_standard_decoder_engine(
    ...
    *,
    ...
    """Build a split LocateAnything native-KV engine through the legacy entrypoint."""
    if debug_layer_outputs or hidden_state_output:
        raise ValueError(
            "LocateAnything native KV does not support debug or hidden-state outputs")
    ...
    if role not in ("prefill", "decode"):
        raise ValueError(
            "LocateAnything native KV requires an explicit split prefill/decode role")
