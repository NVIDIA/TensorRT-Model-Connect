...
"""Build one single-GPU LocateAnything native-KV TensorRT engine.

Prefill and decode are separate single-profile BF16 engines. TensorRT
``IKVCacheUpdate`` mutates the caller-provided full-capacity cache and a
non-decomposable ``IAttention`` consumes only its runtime-active prefix.
"""
...
def _supports_config(config: "ModelConfig", weights: "WeightDict") -> None:
    """Reject configs the LocateAnything native-KV builder cannot handle."""
    model_type = getattr(config, "model_type", "").lower()
    if "moe" in model_type or "mamba" in model_type or "rwkv" in model_type:
        raise NotImplementedError(
            f"LocateAnything native-KV builder does not support model_type={model_type!r}")
...
def build_dual_profile_decoder_engine(
    ...
    *,
    ...
    """Build one native-KV prefill or decode engine.
    ...
    The legacy concatenation graph, multi-profile layout, KV buckets, and
    quantized route are intentionally rejected.
    """
    _supports_config(config, weights)
    if profile_mode not in ("prefill", "decode"):
        raise ValueError(
            "LocateAnything native KV profile_mode must be 'prefill' or 'decode', "
            f"got {profile_mode!r}")
    if precision != "bf16":
        raise ValueError("LocateAnything native KV cache requires BF16")
    if quant_ctx is not None or dynamic_kv_profile_rows:
        raise ValueError(
            "LocateAnything native KV does not support quantization or KV buckets")
    if position_type != "rope":
        raise NotImplementedError(
            "LocateAnything native KV cache requires rotary position embeddings")
    if norm_type != "rmsnorm" or mlp_type != "swiglu" or activation != "silu":
        raise NotImplementedError(
            "LocateAnything native KV requires Qwen2 RMSNorm/SwiGLU/SILU")
    if parallel_residual or interleaved_rope or partial_rotary_factor != 1.0:
        raise NotImplementedError(
            "LocateAnything native KV requires sequential residual and full non-interleaved RoPE")
    if not scale_attn_weights:
        raise NotImplementedError(
            "LocateAnything native KV requires scaled dot-product attention")
    ...
    if plan is None:
        raise RuntimeError(f"LocateAnything native-KV {profile_mode} engine build failed")
    return bytes(plan)
