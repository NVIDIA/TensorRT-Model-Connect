...
"""Configuration fields consumed by the M2M-100/NLLB family."""
...
class ModelConfig:
    ...
    def from_json(cls, text: str) -> ModelConfig:
        raw = json.loads(text)
        hidden = raw.get("hidden_size") or raw.get("d_model") or 0
        heads = (
            raw.get("num_attention_heads")
            or raw.get("decoder_attention_heads")
            or raw.get("encoder_attention_heads")
            or 1
        )
        return cls(
            ...
            intermediate_size=(
                ...
                or raw.get("decoder_ffn_dim")
                or raw.get("encoder_ffn_dim")
                or hidden * 4
            ),
            num_hidden_layers=(
                raw.get("num_hidden_layers")
                or raw.get("decoder_layers")
                or raw.get("encoder_layers")
                or 0
            ),
