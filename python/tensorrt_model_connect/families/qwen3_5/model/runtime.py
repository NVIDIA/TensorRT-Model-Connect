"""Qwen3.5-owned hybrid debug runner implementation."""
...
class HybridTrtRunner:
    ...
    def __init__(
        ...
        for _ in range(num_mamba_layers):
            for lst, sz in [
                (self._d_conv_state, self._conv_state_bytes),
        ...
        for i in range(num_mamba_layers):
            _check_cuda(
                cudart.cudaMemsetAsync(
                    self._d_conv_state[i], 0, self._conv_state_bytes, self.stream
                )[0]
            )
            _check_cuda(
                cudart.cudaMemsetAsync(self._d_ssm_state[i], 0, self._ssm_state_bytes, self.stream)[
                    0
                ]
            )
    ...
    def step(self, token_id: int) -> dict[str, np.ndarray]:
        ...
        for i in range(self.num_mamba_layers):
            cudart.cudaMemcpyAsync(
                self._d_conv_state[i], self._d_present_conv[i], self._conv_state_bytes, D2D, stream
            )
            cudart.cudaMemcpyAsync(
                self._d_ssm_state[i], self._d_present_ssm[i], self._ssm_state_bytes, D2D, stream
            )
