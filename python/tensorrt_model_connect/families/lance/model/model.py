...
def build_standard_decoder_engine(config: ModelConfig, weights: WeightDict, max_cache_length: int, *, precision: str='fp32', quant_ctx: QuantContext | None=None, norm_type: str='rmsnorm', mlp_type: str='swiglu', position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, verbose: bool=False, debug_layer_outputs: bool=False, hidden_state_output: bool=False) -> bytes:
    ...
    vl_single_engine_fallback = decoder_engine_role == 'decode'
    if not _dual_profile_disabled_for and (decoder_engine_role in ('dual_profile', 'prefill') or vl_single_engine_fallback):
        return build_dual_profile_decoder_engine(config, weights, max_cache_length, precision=precision, quant_ctx=quant_ctx, norm_type=norm_type, mlp_type=mlp_type, position_type=position_type, activation=activation, partial_rotary_factor=partial_rotary_factor, interleaved_rope=interleaved_rope, parallel_residual=parallel_residual, scale_attn_weights=scale_attn_weights, verbose=verbose, profile_mode='prefill' if decoder_engine_role == 'prefill' else 'dual_profile')
    ...
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = _apply_norm(network, hidden_state, hidden, final_norm, weights.get('final_norm_beta'), eps_tensor, norm_type, dtype=work_np_dtype, eps=config.rms_norm_eps)
...
def build_dual_profile_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp16', opt_prefill_length: int=64, max_prefill_length: int | None=None, quant_ctx: 'QuantContext | None'=None, norm_type: str='rmsnorm', mlp_type: str='swiglu', position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, verbose: bool=False, dynamic_kv_profile_rows: list[int] | None=None, profile_mode: str='dual_profile') -> bytes:
    ...
    * ``"prefill"``: one prefill profile only. This is used by split-engine
    ...
    def _add_profile(opt_sq: int, max_sq: int, *, fixed: bool=False, cache_rows_min: int | None=None, cache_rows_opt: int | None=None, cache_rows_max: int | None=None):
        ...
        prof.set_shape('input_embed', (min_sq, hidden), (opt_sq, hidden), (max_sq, hidden))
        prof.set_shape('use_input_embed', (min_sq, 1), (opt_sq, 1), (max_sq, 1))
        if multi_bucket_decode:
    ...
    if position_type == 'learned':
        pos_embed_np = weights['position_embedding']
        position_embed_table = const_in_work_dtype(network, pos_embed_np.shape, pos_embed_np, work_np_dtype, work_trt_dtype)
    alibi_slopes_tensor: trt.ITensor | None = None
    ...
    if input_embed_tensor is not None and use_input_embed_tensor is not None:
        ...
        one = const_in_work_dtype(network, (1, 1), np.array([[1.0]], dtype=work_np_dtype), work_np_dtype, work_trt_dtype)
        inverse_selector = network.add_elementwise(one, embed_selector, trt.ElementWiseOperation.SUB).get_output(0)
        token_part = network.add_elementwise(inverse_selector, token_embed, trt.ElementWiseOperation.PROD).get_output(0)
        embed_part = network.add_elementwise(embed_selector, input_embed, trt.ElementWiseOperation.PROD).get_output(0)
        hidden_state = network.add_elementwise(token_part, embed_part, trt.ElementWiseOperation.SUM).get_output(0)
    if position_type == 'learned' and position_embed_table is not None:
        pos_gather = network.add_gather(position_embed_table, position_id, 0)
        pos_add = network.add_elementwise(hidden_state, pos_gather.get_output(0), trt.ElementWiseOperation.SUM)
        hidden_state = pos_add.get_output(0)
    ...
    for layer_idx in range(num_layers):
        prefix = f'layer.{layer_idx}'
        normed = norm_multi(network, hidden_state, hidden, weights[f'{prefix}.input_norm'], weights.get(f'{prefix}.input_norm_beta'), eps_tensor, norm_type, work_np_dtype)
        ...
        if parallel_residual:
            post_attn_norm_w = weights.get(f'{prefix}.post_attn_norm')
            if post_attn_norm_w is not None:
                norm2 = norm_multi(network, hidden_state, hidden, post_attn_norm_w, weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, norm_type, work_np_dtype)
            else:
                norm2 = normed
        else:
            residual1 = network.add_elementwise(hidden_state, attn_out, trt.ElementWiseOperation.SUM)
            norm2 = norm_multi(network, residual1.get_output(0), hidden, weights[f'{prefix}.post_attn_norm'], weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, norm_type, work_np_dtype)
        if mlp_type == 'gelu_fc':
    ...
    final_norm = weights.get('final_norm')
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = norm_multi(network, hidden_state, hidden, final_norm, weights.get('final_norm_beta'), eps_tensor, norm_type, work_np_dtype)
