...
"""Lance split decoder builder using TensorRT's native KV-cache layers.

The family packages separate prefill and decode engines. Both consume the
same runtime-owned, full-context compact GQA cache and update it in place with
``IKVCacheUpdateLayer`` before invoking ``IAttentionLayer``. There is no
concat/scalar-cache compatibility graph and no selector for one.
...
def _validate_native_config(
    ...
    """Fail closed unless the graph matches Lance's proven native contract."""
    model_type = getattr(config, "model_type", "").lower()
    if model_type != "lance":
        raise ValueError(f"Lance native KV requires model_type='lance', got {model_type!r}")
    if precision != "bf16":
        raise ValueError("Lance native KV requires bf16 precision")
    if profile_mode not in ("prefill", "decode"):
        raise ValueError(
            "Lance native KV requires profile_mode='prefill' or 'decode', "
            f"got {profile_mode!r}")
    if max_cache_length != int(config.max_position_embeddings):
        raise ValueError(
            "Lance native KV cache capacity must equal the model context "
            f"({config.max_position_embeddings})")
    if "embedding" not in weights:
        raise ValueError("Lance native KV requires an embedding weight")
    if "final_norm" not in weights:
        raise ValueError("Lance native KV requires a final_norm weight")

    rope_scaling = config.raw.get("rope_scaling") or {}
    raw_section = rope_scaling.get("mrope_section")
    if not isinstance(raw_section, (list, tuple)):
        raise ValueError("Lance native KV requires rope_scaling.mrope_section")
    mrope_section = tuple(int(value) for value in raw_section)
    ...
    if sum(mrope_section) != config.head_dim // 2:
        raise ValueError(
            "rope_scaling.mrope_section must sum to half the head dimension; "
            f"got {mrope_section} for head dimension {config.head_dim}")
...
def build_dual_profile_decoder_engine(
    ...
    *,
    ...
    """Build one native-KV split role for Lance."""
    mrope_section = _validate_native_config(
        config, weights, max_cache_length, precision, profile_mode)
    ...
    # Compute mRoPE only for the active rows. Engine size therefore stays
    # independent of the 128K physical cache capacity.
    ...
    matmul = _make_matmul_fn(network, work_np_dtype, None)
    ...
    # Prefill carries Lance's bidirectional vision mask. Decode uses the
    # native attention layer's LOWER_RIGHT causal mask and needs no input mask.
    ...
    for layer_idx in range(num_layers):
        ...
        # Lance's understanding expert uses QKV bias.
        ...
        # Lance optionally carries per-head Q/K norm weights.
    ...
    if verbose:
        print(f"[trtmc build] Building Lance native-KV {profile_mode} engine "
    ...
    if plan is None:
        raise RuntimeError(f"Lance native-KV {profile_mode} engine build failed")
    return bytes(plan)
