...
def _add_decomposed_attention_core(
    ...
    *,
    ...
    scale_t = _scalar_constant_for_trt_dtype(network, (1, 1, 1, 1), scale, q_4d.dtype)
...
def add_attention_from_rows(
    ...
    *,
    ...
    decompose_masked_attention = (
        (q_seq != 1 or kv_seq is None) and mask is not None and head_dim == 128 and not causal
    )
    if logit_softcap is not None and float(logit_softcap) > 0.0:
        ...
        ctx_4d = _add_decomposed_attention_core(
            network, q_4d, k_4d, v_4d, mask=mask, scale=scale, fp32_accumulation=fp32_accumulation
        )
    else:
