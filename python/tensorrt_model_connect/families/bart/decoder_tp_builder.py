...
"""Tensor-parallel BART decoder builder.

This duplicates the single-device BART decoder graph and changes only the
...
def _validate_bart_tp(
    ...
    if not parallel.enabled:
        return
    if parallel.rank < 0:
        raise ValueError("BART tensor-parallel decoder build requires a concrete rank")
...
def shard_bart_decoder_weights(
    weights: "WeightDict",
    *,
    parallel: "ParallelConfig",
) -> "WeightDict":
    """Return rank-local decoder weights for the BART TP builder."""
    if not parallel.enabled:
        return weights
...
def _add_bart_tp_decoder_layer(
    *,
    network,
    hidden,
    cache_k,
...
def build_bart_tp_decoder_engine(
    ...
    *,
    ...
    """Build one rank-local BART decoder with tensor-parallel joins."""
    ...
    dec_heads = int(weights["_dec_heads"])
    dec_ffn = int(weights["_dec_ffn"])
    normalize_embedding = bool(weights["_normalize_embedding"])
    hidden = int(config.hidden_size)
    ...
    for layer_idx in range(dec_layers):
        prefix = f"layer.{layer_idx}"
        result = _add_bart_tp_decoder_layer(
            network=network,
            hidden=hidden_state,
    ...
    if plan is None:
        raise RuntimeError("TensorRT BART TP decoder build failed")
    return bytes(plan)
