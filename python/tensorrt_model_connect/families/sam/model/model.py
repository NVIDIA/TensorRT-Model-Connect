...
def add_attention_from_rows(
    ...
    *,
    ...
    """Build SAM's fixed, non-causal multi-head attention."""

    def _to_heads(tensor: trt.ITensor, seq_len: int) -> trt.ITensor:
        heads = network.add_shuffle(tensor)
        heads.reshape_dims = (seq_len, num_heads, head_dim)
        heads.second_transpose = trt.Permutation([1, 0, 2])
        batched = network.add_shuffle(heads.get_output(0))
        batched.reshape_dims = (1, num_heads, seq_len, head_dim)
        return batched.get_output(0)

    q_4d = _to_heads(q, q_seq)
    k_4d = _to_heads(k, kv_seq)
    v_4d = _to_heads(v, kv_seq)

    scale_dtype = np.float16 if q_4d.dtype == trt.float16 else np.float32
    scale_t = add_constant(network, (1, 1, 1, 1), np.array([[[[scale]]]]), dtype=scale_dtype)
    ...
    context = _cast_back_to_trt_dtype(network, attention.get_output(0), q_4d.dtype)
