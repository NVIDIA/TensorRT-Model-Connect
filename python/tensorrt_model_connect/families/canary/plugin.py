...
from .weights import WeightDict, _transpose_2d
...
from .model.parallel import build_canary_tp_decoder_engine
...
def _build_subsampling(network, mel_input, weights, sub_ch, hidden, num_mel_bins, mel_length):
    ...
    return graph_ops.add_bias_sum(network, out, hidden, weights["enc_sub_out_b"])


def _rel_shift(network, x, H, S):
    zeros = graph_ops.add_constant(network, (H, S, 1), np.zeros((H, S, 1), dtype=np.float32))
...
def _add_rel_pos_attention(
    ...
    k = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(network, normed, hidden, hidden, weights[f"{pfx}.w_k"]),
        hidden,
        weights[f"{pfx}.b_k"],
    )
    v = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(network, normed, hidden, hidden, weights[f"{pfx}.w_v"]),
        hidden,
        weights[f"{pfx}.b_v"],
    )
    ...
    bu = graph_ops.add_constant(network, (1, H, D), weights[f"{pfx}.pos_bias_u"])
    bv = graph_ops.add_constant(network, (1, H, D), weights[f"{pfx}.pos_bias_v"])
    ...
    sc = graph_ops.add_constant(
        network, (1, 1, 1), np.array([1.0 / math.sqrt(D)], dtype=np.float32)
    )
    ...
    return graph_ops.add_bias_sum(
        ...
        graph_ops.add_matmul_rhs_constant(
            network, af.get_output(0), hidden, hidden, weights[f"{pfx}.w_o"]
        ),
        hidden,
        weights[f"{pfx}.b_o"],
    )
...
def _add_conv_module(
    ...
    if valid_mask is not None:
        x = network.add_elementwise(
            x, valid_mask, trt.ElementWiseOperation.PROD
        ).get_output(0)
...
def _add_half_ffn(network, hs, weights, pfx, hidden, ffn, eps):
    ...
    fc1 = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(network, normed, hidden, ffn, weights[f"{pfx}.w1"]),
        ffn,
        weights[f"{pfx}.b1"],
    )
    ...
    fc2 = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(network, act, ffn, hidden, weights[f"{pfx}.w2"]),
        hidden,
        weights[f"{pfx}.b2"],
    )
    half = graph_ops.add_constant(network, (1, 1), np.array([0.5], dtype=np.float32))
...
def _add_decoder_layer(
    *,
    ...
    n = graph_ops.add_layer_norm(
        network, hidden, hsz, weights[f"{pfx}.input_norm"], weights[f"{pfx}.input_norm_b"], eps
    )
    q = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(network, n, hsz, hsz, weights[f"{pfx}.w_q"]),
        hsz,
        weights[f"{pfx}.q_bias"],
    )
    k = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(network, n, hsz, hsz, weights[f"{pfx}.w_k"]),
        hsz,
        weights[f"{pfx}.k_bias"],
    )
    v = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(network, n, hsz, hsz, weights[f"{pfx}.w_v"]),
        hsz,
    ...
    sa = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(network, cf, hsz, hsz, weights[f"{pfx}.w_o"]),
        hsz,
    ...
    cn = graph_ops.add_layer_norm(
        network, psa, hsz, weights[f"{pfx}.xattn_norm"], weights[f"{pfx}.xattn_norm_b"], eps
    )
    cq = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(network, cn, hsz, hsz, weights[f"{pfx}.xw_q"]),
        hsz,
        weights[f"{pfx}.xb_q"],
    )
    ck = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(network, cross_k, hsz, hsz, weights[f"{pfx}.xw_k"]),
        hsz,
        weights[f"{pfx}.xb_k"],
    )
    cv = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(network, cross_v, hsz, hsz, weights[f"{pfx}.xw_v"]),
        hsz,
        weights[f"{pfx}.xb_v"],
    )
    ccf = graph_ops.add_attention_from_rows(
        network, cq, ck, cv, num_heads=nheads, head_dim=hdim, q_seq=1, kv_seq=maxsrc
    )
    ca = graph_ops.add_bias_sum(
        network,
        graph_ops.add_matmul_rhs_constant(network, ccf, hsz, hsz, weights[f"{pfx}.xw_o"]),
        hsz,
        weights[f"{pfx}.xb_o"],
    )
    ...
    fn = graph_ops.add_layer_norm(
        network, pca, hsz, weights[f"{pfx}.ffn_norm"], weights[f"{pfx}.ffn_norm_b"], eps
    )
    mlp = graph_blocks.add_gelu_fc_mlp(
        network, fn, weights=weights, prefix=pfx, hidden_size=hsz, mlp_size=ffn
    )
    ...
    def build_engine(
        ...
        *,
        ...
        for i in range(dl):
            cki.append(
                net.add_input(
                    graph_ops.layer_tensor_name("cache_k", i), trt.float32, (max_cache_length, h)
                )
            )
            cvi.append(
                net.add_input(
                    graph_ops.layer_tensor_name("cache_v", i), trt.float32, (max_cache_length, h)
                )
            )
            xki.append(
                net.add_input(graph_ops.layer_tensor_name("cross_k", i), trt.float32, (es, h))
            )
            xvi.append(
                net.add_input(graph_ops.layer_tensor_name("cross_v", i), trt.float32, (es, h))
            )

        emb_table = graph_ops.add_constant(net, (v, h), weights["dec_emb"])
        pos_np = weights["dec_pos"]
        pos_table = graph_ops.add_constant(net, pos_np.shape, pos_np)
        ...
        hs = graph_ops.add_layer_norm(net, hs, h, weights["emb_ln"], weights["emb_ln_b"], eps)
        ...
        hs = graph_ops.add_layer_norm(
            net, hs, h, weights["final_norm"], weights["final_norm_b"], eps
        )
        logits = graph_ops.add_bias_sum(
            net,
            graph_ops.add_matmul_rhs_constant(net, hs, h, v, weights["w_out"]),
            v,
            weights["out_bias"],
        )
        logits.name = "logits"
        ...
        if verbose:
            print(
                f"[trtmc build] Building Canary decoder ({dl}L, h={h}, heads={dh})", file=sys.stderr
            )
...
def _build_encoder(config, weights, *, verbose=False):
    ...
    if not mask_2d:
        ...
        valid_mask = net.add_elementwise(
            enc_mask, scale, trt.ElementWiseOperation.PROD
        ).get_output(0)
        valid_mask = net.add_elementwise(
            valid_mask, one, trt.ElementWiseOperation.SUM
        ).get_output(0)
        valid_mask = net.add_elementwise(
            valid_mask, zero, trt.ElementWiseOperation.MAX
        ).get_output(0)
        valid_mask = net.add_elementwise(
            valid_mask, one, trt.ElementWiseOperation.MIN
        ).get_output(0)
