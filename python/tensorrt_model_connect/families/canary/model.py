...
"""Canary family-owned build implementation -- FastConformer ASR.
...
def load_weights(model_dir: str, config: ModelConfig, *, precision: str = "fp32") -> WeightDict:
    global _prompt_metadata, _vl_config
    w = WeightDict()
    ...
    if tokenizer_model.exists():
        _prompt_metadata = _canary_prompt_metadata(ncfg, tokenizer_model)
...
def build_engine(
    ...
    *,
    ...
    emb_table = graph_ops.add_constant(net, (v, h), weights["dec_emb"], dtype=decoder_io_np_dtype)
...
def get_bundle_config_overrides(config: ModelConfig) -> dict | None:
    ...
    overrides.update(_prompt_metadata)
    return overrides
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build the complete canary bundle in the owning family module."""
    import json
    import time
