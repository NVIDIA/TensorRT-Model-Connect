...
def add_gated_mlp(
    ...
    *,
    ...
    fc1_gate = matmul(
        inp, hidden_size, mlp_size, weights[f"{prefix}.w_fc1_gate"], f"{_lp}.w_fc1_gate"
    )
    fc1_gate_bias = weights.get(f"{prefix}.fc1_gate_bias")
    if fc1_gate_bias is not None:
        fc1_gate = graph_ops.add_bias_sum(network, fc1_gate, mlp_size, fc1_gate_bias, dtype=dtype)
    ...
    # Output projection
    fc2 = matmul(
        gated.get_output(0), mlp_size, hidden_size, weights[f"{prefix}.w_fc2"], f"{_lp}.w_fc2"
    )
    fc2_bias = weights.get(f"{prefix}.fc2_bias")
    if fc2_bias is not None:
        fc2 = graph_ops.add_bias_sum(network, fc2, hidden_size, fc2_bias, dtype=dtype)
...
def add_dit_block(
    ...
    *,
    ...
    # Gate and residual
    gated_self_attn = network.add_elementwise(self_attn_out, gate1, trt.ElementWiseOperation.PROD)
    hidden = network.add_elementwise(
        hidden, gated_self_attn.get_output(0), trt.ElementWiseOperation.SUM
    ).get_output(0)
    ...
    if cross_norm_gamma is not None:
        cross_normed = graph_ops.add_layer_norm_native(
            ...
            hidden_size,
            cross_norm_gamma,
            weights.get(f"{prefix}.norm_cross.beta", np.zeros(hidden_size, dtype=np.float32)),
            eps,
            dtype=dtype,
        )
    else:
    ...
    hidden = network.add_elementwise(
        hidden, cross_attn_out, trt.ElementWiseOperation.SUM
    ).get_output(0)
    ...
    # Gate and residual
    gated_ffn = network.add_elementwise(ffn_out, gate2, trt.ElementWiseOperation.PROD)
    hidden = network.add_elementwise(
        hidden, gated_ffn.get_output(0), trt.ElementWiseOperation.SUM
    ).get_output(0)
