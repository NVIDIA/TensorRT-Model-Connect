...
from .vae_builder import OFFICIAL_VAE_WORKSPACE_GIB, load_vae_cuda_plugin
...
def vae_step_cache_bytes(profile: Wan22VaeStepProfile) -> int:
    """Return bytes in one FP32 32-cache set."""

    return sum(int(np.prod(spec.shape(profile))) * 4 for spec in VAE_STEP_CACHE_SPECS)
...
def _configure_step_builder_config(
    trt,
    config,
    *,
    workspace_gib: int,
    ...
    """Configure the recurrent builder without silently changing production tactics."""

    if workspace_gib <= 0:
        raise ValueError(f"workspace_gib must be positive, got {workspace_gib}")
    if max_aux_streams is not None and max_aux_streams < 0:
        raise ValueError(f"max_aux_streams must be non-negative, got {max_aux_streams}")
...
def _add_native_final_conv3d(
    ...
    *,
    ...
    """Causal 3D convolution through the bounded Wan22VaeConv3d plugin."""

    batch, input_channels, frames, height, width = tuple(tensor.shape)
    output_channels = int(weight.shape[0])
    concat = network.add_concatenation([cache, tensor])
    concat.axis = 2
    full_temporal = concat.get_output(0)
    cache_output = network.add_slice(
        full_temporal,
        start=(0, 0, frames, 0, 0),
        shape=(batch, input_channels, 2, height, width),
        stride=(1, 1, 1, 1, 1),
    ).get_output(0)
    ...
    weight_layer = network.add_constant(tuple(weight.shape), weight)
    bias_layer = network.add_constant(tuple(bias.shape), bias)
    weight_layer.name = f"{label}_weight"
    bias_layer.name = f"{label}_bias"
    ...
    values = {
        ...
        "output_channels": np.asarray([output_channels], dtype=np.int32),
        "input_depth": np.asarray([2 + frames], dtype=np.int32),
        "input_height": np.asarray([height + 2], dtype=np.int32),
        "input_width": np.asarray([width + 2], dtype=np.int32),
    }
    fields = [
        trt.PluginField(name, value, trt.PluginFieldType.INT32) for name, value in values.items()
    ]
    plugin = creator.create_plugin(label, trt.PluginFieldCollection(fields))
    if plugin is None:
        raise RuntimeError(f"Could not create {label}")
    layer = network.add_plugin_v2(
        [padded, weight_layer.get_output(0), bias_layer.get_output(0)],
        plugin,
    )
    if layer is None:
        raise RuntimeError(f"Could not add {label}")
    layer.name = label
    return layer.get_output(0), cache_output
...
def build_vae_step_engine(
    ...
    *,
    ...
    current_height = profile.latent_height
    current_width = profile.latent_width
    current_scale = 1
    ...
    def add_resnet(tensor, *, prefix: str, input_channels: int, output_channels: int, native: bool):
        ...
        if native:
            conv1, cache_outputs[cache1_index] = _add_native_final_conv3d(
                trt,
                network,
                act1,
                cache1,
                ...
                label=f"wan22_vae_step_{prefix.replace('.', '_')}_conv1",
            )
        else:
        ...
        if native:
            conv2, cache_outputs[cache2_index] = _add_native_final_conv3d(
                trt,
                network,
                act2,
                cache2,
                ...
                label=f"wan22_vae_step_{prefix.replace('.', '_')}_conv2",
            )
        else:
    ...
    for middle in range(2):
        ...
        x = add_resnet(x, prefix=prefix, input_channels=1024, output_channels=1024, native=False)
        if middle == 0:
    ...
    print(
        ...
        f"caches={len(VAE_STEP_CACHE_SPECS)}, cache_bytes={vae_step_cache_bytes(profile)}",
        file=sys.stderr,
    )
