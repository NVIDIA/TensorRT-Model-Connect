...
This module implements only GPT-Neo's learned positions, LayerNorm, GELU
feed-forward block, sequential residuals, and family-owned projection biases.
Quantized builds thread Q/DQ insertion through every projection matmul.
...
def build_dual_profile_tp_decoder_engine(
    ...
    *,
    ...
    if verbose:
        print(
            f"[trtmc-build] Building tensor-parallel decoder engine (layers={num_layers}, hidden={hidden}, attn={attention_size}, kv={kv_attention_size}, mlp={mlp_size}, cache={max_cache_length}, opt_prefill={opt_prefill_length}, max_prefill={max_prefill_length}, norm={'layernorm'}, mlp_type={'gelu_fc'}, pos={'learned'}, precision={precision}, tp={parallel.tp_size}, rank={parallel.rank}) ...",
            file=sys.stderr,
        )
