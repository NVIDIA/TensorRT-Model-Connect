...
def build_standard_decoder_engine(config: ModelConfig, weights: WeightDict, max_cache_length: int, *, precision: str='fp32', quant_ctx: QuantContext | None=None, partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, attention_layer_types: Sequence[str] | None=None, local_attention_window: int=0, embed_input: bool=False, verbose: bool=False, debug_layer_outputs: bool=False, hidden_state_output: bool=False) -> bytes:
    ...
    if not _dual_profile_disabled_for and decoder_engine_role in ('dual_profile', 'prefill'):
        return build_dual_profile_decoder_engine(config, weights, max_cache_length, precision=precision, quant_ctx=quant_ctx, partial_rotary_factor=partial_rotary_factor, interleaved_rope=interleaved_rope, parallel_residual=parallel_residual, attention_layer_types=attention_layer_types, local_attention_window=local_attention_window, verbose=verbose, profile_mode='prefill' if decoder_engine_role == 'prefill' else 'dual_profile')
    ...
    if attention_layer_types and 'local' in attention_layer_types and (local_attention_window > 0):
        local_attention_mask = graph_ops.add_local_attention_mask_2d(network, attention_mask, position_id, max_cache_length=max_cache_length, window_size=local_attention_window)
