...
"""TensorRT graph builders for timm DenseNet classifiers.

DenseNet is pre-activation: each layer runs batch norm, ReLU, then convolution,
and concatenates its output onto the running feature stack. That needs two ops
the residual families did not: channel concatenation and average pooling in the
transitions.
"""
...
def add_avg_pool2d(network, x, kernel: int, stride: int):
