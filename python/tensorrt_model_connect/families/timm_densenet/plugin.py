...
"""timm DenseNet image-classification family plugin.
...
class TimmDensenetPlugin:
    name = "timm_densenet"
    runtime_strategy = "timm_densenet_image_classification"
        ...
        *,
        ...
        if quant_ctx is not None:
            raise NotImplementedError(
                "timm_densenet does not support quantized builds yet")
        if parallel_config is not None and getattr(parallel_config, "enabled", False):
            raise NotImplementedError(
                "timm_densenet does not support tensor-parallel builds")
        ...
        for block_index, count in enumerate(block_layers, start=1):
            ...
            if block_index < len(block_layers):
                ...
                hidden = graph_ops.add_conv2d(
                    network, hidden, tw, None, int(tw.shape[0]), (1, 1),
                    dtype=work_np_dtype)
