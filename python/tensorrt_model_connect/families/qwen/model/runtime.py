"""Qwen-owned debug runner adapter."""
...
def _check_cuda(status):
    """Raise on CUDA error."""
    if cudart is None:
        raise RuntimeError("cuda-python is required for Qwen debug_runner execution")
...
def _require_trt_runtime() -> None:
    if trt is None:
        raise ImportError("tensorrt is required for Qwen debug_runner execution")
    if cudart is None:
        raise ImportError("cuda-python is required for Qwen debug_runner execution")
...
class TriAttentionTrtRunner(TrtRunner):
    ...
    def __init__(
        ...
        if triattention_config.kv_budget > max_cache_length:
            raise ValueError("TriAttention kv_budget cannot exceed the engine max_cache_length")
    ...
    def _compact_existing_cache(self) -> None:
        ...
        sampled_layers = sorted(
            {int(layer) for layer, _ in self.triattention_selector.sampled_heads}
        )
...
def runner_from_bundle(
    ...
    *,
    ...
    """Create a Qwen-owned TRT runner from either bundle bytes or parsed parts."""
    if config is None or header is None or engine_plan is None:
        if bundle_path is None:
            raise TypeError("bundle_path is required when parsed Qwen bundle parts are absent")
        config = load_config_from_bundle(bundle_path)
        engine_plan, header = load_engine_from_bundle(
            bundle_path,
            section_name=engine_section,
        )

    runtime_strategy = str(runtime_strategy or config.get("runtime_strategy") or "")
    if runtime_strategy != "qwen_decoder_kv_cache":
        return None
    num_layers = header.get("num_layers", config.get("num_hidden_layers", 1))
    tri_cfg = config.get("triattention", {}) or {}
    if tri_cfg.get("enabled"):
        if bundle_path is None:
            raise TypeError("bundle_path is required for Qwen TriAttention stats")
