...
"""Capsule-owned Qwen3-4B-Instruct-2507 TensorRT Edge-LLM build subprocess."""
...
_PLUGIN_ENV = "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_EDGE_LLM_PLUGIN_LIBRARY"
_EDGE_BUILD_DIR_ENV = "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_EDGE_LLM_BUILD_DIR"
_TRT_INCLUDE_ENV = "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_TENSORRT_INCLUDE_DIR"
_TRT_LIBRARY_ENV = "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_TENSORRT_LIBRARY"
_ONNX_PARSER_INCLUDE_ENV = "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_ONNX_PARSER_INCLUDE_DIR"
_ONNX_PARSER_LIBRARY_ENV = "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_ONNX_PARSER_LIBRARY"
_CUDA_INCLUDE_ENV = "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_CUDA_INCLUDE_DIR"
_CUDART_LIBRARY_ENV = "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_CUDART_LIBRARY"
_CUDA_DRIVER_LIBRARY_ENV = "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_CUDA_DRIVER_LIBRARY"
_ALLOW_FAKE_RUNTIME_BUILD_ENV = "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_ALLOW_FAKE_RUNTIME_BUILD"
...
_EXPORTER_PROFILE_NAME = "qwen3-4b-instruct-2507-edgellm-v09-exporter"
...
_EXPECTED_ENGINE_BUILDER_CONFIG = {
    "max_input_len": 1024,
...
def _runtime_source_root() -> Path:
    ...
    source = (
        ...
        / "qwen"
        / "edge_llm_adapter"
        / "qwen3_4b_instruct_2507_fp16_a100_pcie80_sm80"
    )
    if source.is_dir():
        return source
    raise AdapterError(
...
def _validate_engine_directory(path: Path) -> tuple[Path, int]:
    ...
    for field in ("tp_size", "tp_rank"):
        if field in config:
            raise AdapterError(f"Edge-LLM engine config.{field} must be absent")
...
def _resolve_edge_source(output: Path, runtime_build: Mapping[str, Any]) -> Path:
    ...
    configured = _runtime_setting(
        runtime_build,
        "edge_llm_source_dir",
        "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_EDGE_LLM_SOURCE_DIR",
    )
    if configured:
...
def _build_runtime_dso(
    ...
    configured_sdk_include = _runtime_setting(
        runtime_build,
        "sdk_include_dir",
        "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_SDK_INCLUDE_DIR",
    )
    ...
    json_include = _runtime_setting(
        runtime_build,
        "nlohmann_json_include_dir",
        "_TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_NLOHMANN_JSON_INCLUDE_DIR",
        json_default,
    )
    if not json_include:
        raise AdapterError(
            "Fake Qwen runtime builds require runtime_build.nlohmann_json_include_dir "
            "or _TRTMC_INTERNAL_QWEN3_4B_INSTRUCT_2507_NLOHMANN_JSON_INCLUDE_DIR"
        )
    configure = [
        cmake_launcher,
        "-S",
...
def main(argv: list[str] | None = None) -> int:
    try:
        ...
        print(f"Qwen3-4B-Instruct-2507 Edge-LLM adapter error: {exc}", file=sys.stderr)
        return 1
