...
"""StableLM-2 tensor-parallel prefill/decode engine builder."""
...
def _validate_weights(weights: "WeightDict") -> None:
    """Reject incomplete StableLM weight maps."""
...
def build_dual_profile_tp_decoder_engine(
    ...
    *,
    ...
    """Build one rank-local StableLM-2 engine with prefill/decode profiles."""
    ...
    rotary_embedding_dim = graph_ops.validate_native_rope_dim(int(head_dim * partial_rotary_factor))
    ...
    for layer_idx in range(num_layers):
        ...
        for name, width in (
            ...
            if bias is None:
                continue
            if name == "q":
                q = graph_ops.add_bias_sum(network, q, width, bias, dtype=work_np_dtype)
            elif name == "k":
                k = graph_ops.add_bias_sum(network, k, width, bias, dtype=work_np_dtype)
            else:
                v = graph_ops.add_bias_sum(network, v, width, bias, dtype=work_np_dtype)
    ...
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = graph_ops.apply_norm(
            network,
            hidden_state,
            hidden,
            final_norm,
    ...
    if verbose:
        print(
            f"[trtmc-build] Building StableLM tensor-parallel decoder engine "
