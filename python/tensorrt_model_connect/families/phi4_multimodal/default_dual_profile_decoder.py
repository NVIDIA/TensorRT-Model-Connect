...
"""Phi-4 Multimodal split decoder builder with TensorRT native KV cache.

The bundle contains one multi-token prefill engine and one single-token
decode engine. Both expose the model's complete advertised context as a fixed
physical KV capacity while ``key_value_lengths`` limits attention to the
logical prefix used by the current enqueue. The runtime owns one cache buffer
per layer and aliases each ``present`` output back to that same address, so no
present-to-cache copy or dense attention mask is needed.

Tensor contract (``Sq`` is dynamic only for the prefill engine):
  Inputs
    token_id             int32   (Sq,)
    position_id          int32   (Sq,)
    cache_write_indices  int32   ()
    key_value_lengths    int32   ()
    cache_k_i            fp16    (1, Hkv, capacity, Dh)
    cache_v_i            fp16    (1, Hkv, capacity, Dh)
  Outputs
    logits               float32 (1, vocab)
    present_k_i          fp16    (1, Hkv, capacity, Dh), aliases cache_k_i
    present_v_i          fp16    (1, Hkv, capacity, Dh), aliases cache_v_i
"""
...
def _supports_config(config: "ModelConfig", weights: "WeightDict") -> None:
    ...
    if "moe" in model_type or "mamba" in model_type or "rwkv" in model_type:
        raise NotImplementedError(
            f"Phi-4 Multimodal native decoder does not support model_type={model_type!r}")
...
def build_dual_profile_decoder_engine(
    ...
    *,
    ...
    """Build one half of the native-KV split decoder bundle.

    ``profile_mode`` is either ``"prefill"`` for a dynamic multi-token engine
    or ``"decode"`` for a fixed single-token engine. Both engines expose the
    same full-context TensorRT native KV contract and update runtime-owned cache
    buffers in place.
    """
    _supports_config(config, weights)
    if profile_mode not in ("prefill", "decode"):
        raise ValueError(
            "profile_mode must be 'prefill' or 'decode', "
            f"got {profile_mode!r}")
    if max_prefill_length is None:
        max_prefill_length = min(max_cache_length, _NATIVE_PREFILL_CHUNK_TOKENS)
    ...
    native_rope_attention_factor = 1.0
    ...
    if rope_scaling.get("type") == "longrope":
        ...
        extension_factor = max_cache_length / max(original_max, 1)
        native_rope_attention_factor = float(rope_scaling.get(
            "attention_factor",
            1.0 if extension_factor <= 1.0 else np.sqrt(
                1.0 + np.log(extension_factor) / np.log(original_max))))
        factor_key = "long_factor" if max_cache_length > original_max else "short_factor"
        frequency_factors = rope_scaling.get(factor_key)
    ...
    cos_half_table, sin_half_table = graph_ops.add_active_rope_cache(
        ...
        work_trt_dtype,
        attention_factor=native_rope_attention_factor,
    )
    ...
    input_embed = network.add_cast(input_embed_tensor, work_trt_dtype).get_output(0)
    embed_selector = network.add_cast(
    ...
    # The embedding table is stored as FP16 in the native decoder.
    ...
    for layer_idx in range(num_layers):
        ...
        normed = _norm_multi(
            ...
            eps_tensor, "rmsnorm", work_np_dtype)
    ...
    if verbose:
        mode_label = f"{profile_mode}-profile"
    ...
    if plan is None:
        raise RuntimeError("Phi-4 Multimodal native split decoder build failed")
    return bytes(plan)
