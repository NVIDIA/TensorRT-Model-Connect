...
class SmolLM3Plugin:
    name = "smollm3"
    runtime_strategy = "smollm3_decoder_kv_cache"
    runtime_capabilities = {"decoder_kv"}

    def matches(self, model_type: str) -> bool:
        return model_type.lower() == "smollm3"
    ...
    def default_max_cache_length(self, config: ModelConfig) -> int:
        """Use the model's complete context for native SmolLM3."""
        capability = native_kv_architecture_capability(config)
    ...
    def build_engine(
        ...
        if capability.eligible:
            ...
            if role not in ("prefill", "decode"):
                raise ValueError(
                    "native SmolLM3 requires explicit split engine role "
