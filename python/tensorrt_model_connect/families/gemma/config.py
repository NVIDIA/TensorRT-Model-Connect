...
"""Gemma family model configuration."""
...
@dataclass
class ModelConfig:
    """Gemma fields consumed by builders and bundle metadata."""
    ...
    def from_json(text: str) -> ModelConfig:
        ...
        return ModelConfig(
            model_type=data.get("model_type", "gemma") or "gemma",
            ...
            hidden_act=data.get("hidden_act", "gelu_pytorch_tanh") or "gelu_pytorch_tanh",
            _head_dim=int(data.get("head_dim", 0)),
            raw=data,
        )

    @classmethod
    def create_tiny(cls, model_type: str = "gemma", **overrides) -> ModelConfig:
        data = {
