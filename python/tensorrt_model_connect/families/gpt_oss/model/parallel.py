...
def _validate_gpt_oss_tp(
    ...
    for name, value in checks.items():
        if value % tp != 0:
            raise ValueError(
                f"GPT-OSS tensor parallel requires {name} divisible by tp_size ({value} vs {tp})"
            )
...
def _add_gpt_oss_expert_local(
    ...
    limit_const = graph_ops.add_constant(network, (1, 1), np.array([limit], dtype=np.float32))
    neg_limit_const = graph_ops.add_constant(network, (1, 1), np.array([-limit], dtype=np.float32))
    ...
    alpha_const = graph_ops.add_constant(network, (1, 1), np.array([alpha], dtype=np.float32))
    ...
    return graph_ops.add_matmul_rhs_constant(network, gated, intermediate_size, hidden_size, w_down)
...
def _add_weighted_down_bias(
    ...
    *,
    ...
    bias_arrays = [
        weights.get(f"{prefix}.expert.{expert_idx}.down_bias") for expert_idx in range(num_experts)
    ]
    if not all(isinstance(bias, np.ndarray) for bias in bias_arrays):
        return None

    bias_tensors = [
        graph_ops.add_constant(network, (1, hidden_size), bias.reshape(1, hidden_size))
    ...
    for top_idx in range(top_k):
        ...
        scaled = network.add_elementwise(
            bias.get_output(0), w_slice.get_output(0), trt.ElementWiseOperation.PROD
        )
        if result is None:
...
def _add_gpt_oss_tp_moe_block(
    ...
    if router_bias is not None:
        router_logits = graph_ops.add_bias_sum(network, router_logits, num_experts, router_bias)
    ...
    if bias is None:
        return reduced
    with_bias = network.add_elementwise(reduced, bias, trt.ElementWiseOperation.SUM)
    ...
    *,
    ...
    if sinks is None:
        ...
        max_val = network.add_reduce(
            combined.get_output(0), trt.ReduceOperation.MAX, 1 << 2, keep_dims=True
        )
        stable = network.add_elementwise(
            combined.get_output(0), max_val.get_output(0), trt.ElementWiseOperation.SUB
        )
        ...
        context_heads = network.add_matrix_multiply(
            scores.get_output(0), trt.MatrixOperation.NONE, v_heads_t, trt.MatrixOperation.NONE
        )
    ...
    attn_out = graph_ops.add_matmul_rhs_constant(
        network, context_tensor, attention_size, hidden_size, weights[f"{prefix}.w_o"]
    )
...
def _make_rope_tables(
    ...
    if rope_type == "yarn":
        ...
        yarn_orig_max = int(rope_params.get("original_max_position_embeddings", 4096))
    ...
    return (
        graph_ops.make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, True),
        graph_ops.make_rope_table_half_dim(attention_window, head_dim, config.rope_theta, False),
    )
...
def build_gpt_oss_tp_engine(
    ...
    *,
    ...
    if not parallel.enabled:
        raise ValueError("build_gpt_oss_tp_engine requires tensor_parallel mode with tp_size > 1")
    ...
    cos_half_np, sin_half_np = _make_rope_tables(config, attention_window, head_dim)
