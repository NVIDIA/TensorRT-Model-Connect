...
def _detect_vit_prefix(weights: "WeightDict") -> str:
    ...
    for prefix in candidates:
        ...
        if weights.get(f"{prefix}.0.ln1.weight") is not None:
            return prefix
    raise RuntimeError(f"Cannot detect InternViT layer prefix. Tried: {candidates}")


def _detect_ln_style(weights: "WeightDict", layer_prefix: str) -> str:
    """Detect LayerNorm naming: 'hf' (layernorm_before/after) or 'legacy' (ln1/ln2)."""
...
def _detect_attn_style(weights: "WeightDict", layer_prefix: str) -> str:
    """Detect attention naming: 'hf' (attention.q_proj) or 'legacy' (attn.q_proj/attn.qkv)."""
...
def _add_gelu_mlp(
    ...
    fc1 = graph_ops.add_matmul_rhs_constant(network, normed, embed_dim, mlp_hidden, fc1_w)
    ...
    fc2 = graph_ops.add_matmul_rhs_constant(network, activated, mlp_hidden, embed_dim, fc2_w)
...
def _add_layer_scale(
    ...
    scale_const = graph_ops.add_constant(
        network, (1, embed_dim), scale_weights.astype(np.float32).reshape(1, embed_dim)
    )
    scaled = network.add_elementwise(inp, scale_const, trt.ElementWiseOperation.PROD)
...
def build_internvit_vision_engine(
    ...
    *,
    ...
    # Detect CLS token
    has_cls = (
        weights.get("vision_tower.embeddings.cls_token") is not None
    ...
    actual_select_layer = select_layer if select_layer >= 0 else num_layers + select_layer
    ...
    if verbose:
        print(
            f"[trtmc build] InternViT: image={fixed_image_size}, "
    ...
    eps_tensor = graph_ops.add_constant(network, (1, 1), np.array([eps_val], dtype=np.float32))

    # ---------------------------------------------------------------
    # Input: pixel_values [C, H, W]
    # ---------------------------------------------------------------
    pixel_values = network.add_input(
        "pixel_values", trt.float32, (in_channels, fixed_image_size, fixed_image_size)
    )
    ...
    conv_w = trt.Weights(np.ascontiguousarray(patch_embed_w.astype(np.float32)))
    conv_b = trt.Weights()
    if patch_embed_b is not None:
        conv_b = trt.Weights(np.ascontiguousarray(patch_embed_b.astype(np.float32)))
    ...
    # ---------------------------------------------------------------
    if has_cls:
        cls_token_np = _get_weight(weights, "vision_tower.embeddings.cls_token", "visual.cls_token")
        if cls_token_np is not None:
            cls_const = graph_ops.add_constant(
                network, (1, embed_dim), cls_token_np.astype(np.float32).reshape(1, embed_dim)
            )
    ...
    if pos_embed_w is not None:
        ...
        pos_const = graph_ops.add_constant(network, (seq_len, embed_dim), pos_embed_np)
        pos_add = network.add_elementwise(hidden, pos_const, trt.ElementWiseOperation.SUM)
    ...
    for layer_idx in range(num_layers):
        ...
        ln1_beta = (
            ln1_beta_raw.astype(np.float32)
            if ln1_beta_raw is not None
        ...
        attn_out = _add_vit_attention(
            network, normed, weights, prefix, embed_dim, num_heads, seq_len, attn_style
        )
        ...
        if has_layer_scale:
            ...
            if lambda_1 is not None:
                attn_out = _add_layer_scale(network, attn_out, lambda_1, embed_dim)

        # Residual
        res1 = network.add_elementwise(hidden, attn_out, trt.ElementWiseOperation.SUM)
        ...
        ln2_beta = (
            ln2_beta_raw.astype(np.float32)
            if ln2_beta_raw is not None
            else np.zeros(embed_dim, dtype=np.float32)
        )
        normed2 = graph_ops.add_layer_norm(
            network, res1.get_output(0), embed_dim, ln2_gamma, ln2_beta, eps_tensor
        )

        # GELU MLP
        mlp_out = _add_gelu_mlp(network, normed2, weights, prefix, embed_dim, mlp_hidden)

        # Layer scale on MLP output
        if has_layer_scale:
            lambda_2 = weights.get(f"{prefix}.lambda_2")
            if lambda_2 is not None:
                mlp_out = _add_layer_scale(network, mlp_out, lambda_2, embed_dim)

        # Residual
        res2 = network.add_elementwise(res1.get_output(0), mlp_out, trt.ElementWiseOperation.SUM)
    ...
    if has_cls:
        slice_out = network.add_slice(
            hidden, start=(1, 0), shape=(num_patches, embed_dim), stride=(1, 1)
        )
    ...
    # Try multi_modal_projector.* (InternVL3-8B-hf) first, then mlp1.* (legacy)
    # ---------------------------------------------------------------
    proj_ln_w = _get_weight(
        weights, "multi_modal_projector.layer_norm.weight", "mlp1.1.weight", "visual.mlp1.1.weight"
    )
    proj_ln_b = _get_weight(
        weights, "multi_modal_projector.layer_norm.bias", "mlp1.1.bias", "visual.mlp1.1.bias"
    )

    proj_fc1_w = _get_weight(
        weights, "multi_modal_projector.linear_1.weight", "mlp1.0.weight", "visual.mlp1.0.weight"
    )
    proj_fc1_b = _get_weight(
        weights, "multi_modal_projector.linear_1.bias", "mlp1.0.bias", "visual.mlp1.0.bias"
    )

    proj_fc2_w = _get_weight(
        weights, "multi_modal_projector.linear_2.weight", "mlp1.3.weight", "visual.mlp1.3.weight"
    )
    proj_fc2_b = _get_weight(
        weights, "multi_modal_projector.linear_2.bias", "mlp1.3.bias", "visual.mlp1.3.bias"
    )

    if proj_fc1_w is None:
        raise RuntimeError(
            "Missing MLP projector weights (multi_modal_projector.linear_1.weight or mlp1.0.weight)"
        )
    ...
    # Legacy: Linear(embed_dim, proj_fc1_out_dim) -> LayerNorm -> GELU -> Linear(proj_fc1_out_dim, llm_hidden)
    ...
    is_hf_projector = (
        proj_ln_w is not None and proj_ln_w.shape[0] == concat_dim and proj_fc1_in_dim == concat_dim
    )

    if is_hf_projector:
        # InternVL3-8B-hf: LayerNorm -> fc1 -> GELU -> fc2
        proj_ln_beta = (
            proj_ln_b.astype(np.float32)
            if proj_ln_b is not None
            else np.zeros(concat_dim, dtype=np.float32)
        )
        hidden = graph_ops.add_layer_norm(
            network, hidden, concat_dim, proj_ln_w.astype(np.float32), proj_ln_beta, eps_tensor
        )

        fc1 = graph_ops.add_matmul_rhs_constant(
            network, hidden, concat_dim, proj_fc1_out_dim, proj_fc1_w.astype(np.float32).T.copy()
        )
        ...
        fc2 = graph_ops.add_matmul_rhs_constant(
            network, activated, proj_fc1_out_dim, llm_hidden, proj_fc2_w.astype(np.float32).T.copy()
        )
        if proj_fc2_b is not None:
            fc2 = graph_ops.add_bias_sum(network, fc2, llm_hidden, proj_fc2_b.astype(np.float32))
    else:
        # Legacy: fc1 -> LayerNorm -> GELU -> fc2
        fc1 = graph_ops.add_matmul_rhs_constant(
            network, hidden, concat_dim, proj_fc1_out_dim, proj_fc1_w.astype(np.float32).T.copy()
        )
        if proj_fc1_b is not None:
    ...
    if verbose:
        print(
            f"[trtmc build] Building InternViT vision TRT engine "
            f"({actual_select_layer + 1} layers, embed={embed_dim}, "
            f"patches={num_patches}, output_tokens={num_output_tokens}, "
            f"llm_hidden={llm_hidden}) ...",
            file=sys.stderr,
        )
