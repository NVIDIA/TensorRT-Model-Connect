...
"""Build one InternVL native-KV TensorRT engine for a TP rank.
...
def _supports_config(config: "ModelConfig", weights: "WeightDict") -> None:
    ...
    if "moe" in model_type or "mamba" in model_type or "rwkv" in model_type:
        raise NotImplementedError(
            f"InternVL TP native-KV builder does not support model_type={model_type!r}")
...
def build_dual_profile_tp_decoder_engine(
    ...
    *,
    ...
    if profile_mode not in ("prefill", "decode"):
        raise ValueError(
            "InternVL TP native KV requires profile_mode='prefill' or 'decode'")
    if precision != "bf16":
        raise ValueError("InternVL TP native KV cache requires BF16")
    if quant_ctx is not None or dynamic_kv_profile_rows:
        raise ValueError(
            "InternVL TP native KV cache does not support quantization or KV buckets")
    if position_type != "rope":
        raise NotImplementedError(
            "InternVL TP native KV cache requires rotary position embeddings")
    if norm_type != "rmsnorm" or mlp_type != "swiglu" or activation != "silu":
        raise NotImplementedError(
            "InternVL TP native KV requires Qwen2 RMSNorm/SwiGLU/SILU")
    if parallel_residual or interleaved_rope or partial_rotary_factor != 1.0:
        raise NotImplementedError(
            "InternVL TP native KV requires sequential residual and full non-interleaved RoPE")
    if not scale_attn_weights:
        raise NotImplementedError(
            "InternVL TP native KV requires scaled dot-product attention")
