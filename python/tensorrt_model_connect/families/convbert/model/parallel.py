...
def build_tp_convbert_encoder_engine(
    ...
    *,
    ...
    # Shared constants
    # -------------------------------------------------------------------
    embedding_table = graph_ops.add_constant(network, (vocab, embedding_size), weights["embedding"])
...
def _add_convbert_layer(
    *,
    ...
    zero_col = graph_ops.add_constant(network, (S, 1), np.zeros((S, 1), dtype=np.float32))
    ...
    conv_out = graph_ops.add_bias_sum(
        network, conv_out, all_head_size, weights[f"{prefix}.conv_out_bias"]
    )
    ...
    attn_out = graph_ops.add_matmul_rhs_constant(
        network, cat_flat.get_output(0), 2 * all_head_size, hidden_size, weights[f"{prefix}.w_o"]
    )
