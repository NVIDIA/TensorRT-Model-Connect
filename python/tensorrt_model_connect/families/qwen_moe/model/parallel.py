...
def _validate_qwen_moe_tp(
    ...
    for name, value in checks.items():
        if value % tp != 0:
            raise ValueError(
                f"Qwen-MoE tensor parallel requires {name} divisible by tp_size ({value} vs {tp})"
            )
    ...
    *,
    ...
    for key, value in weights.items():
        ...
        if key.endswith(".experts.w_gate") or key.endswith(".experts.w_up"):
            ...
            out[key] = _slice_first_dim(reshaped, parallel.rank, parallel.tp_size).reshape(-1)
...
def _add_swiglu_tp(
    ...
    *,
    ...
    down = _add_swiglu_local(
        network, inp, hidden_size, intermediate_size, w_gate, w_up, w_down, dtype=dtype
    )
...
def _add_packed_swiglu_experts_tp(
    ...
    *,
    ...
    gate = network.add_matrix_multiply(
        batched_inp.get_output(0), trt.MatrixOperation.NONE, gate_w, trt.MatrixOperation.NONE
    )
    up = network.add_matrix_multiply(
        batched_inp.get_output(0), trt.MatrixOperation.NONE, up_w, trt.MatrixOperation.NONE
    )
...
def _add_qwen_moe_tp_block(
    ...
    if has_shared_expert:
        ...
        combined = network.add_elementwise(local_result, shared_out, trt.ElementWiseOperation.SUM)
...
def build_qwen_moe_tp_engine(
    ...
    *,
    ...
    if not parallel.enabled:
        raise ValueError("build_qwen_moe_tp_engine requires tensor_parallel mode with tp_size > 1")
    ...
    shared_expert_intermediate = int(rank_weights["_shared_expert_intermediate_size"])
    ...
    embedding_table = graph_ops.add_constant(
        network, (vocab, hidden), rank_weights["embedding"], dtype=work_np_dtype
    )
