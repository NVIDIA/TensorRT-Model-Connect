...
"""TensorRT graph builders for timm RegNet classifiers.

RegNetY is a grouped-convolution bottleneck stack with a squeeze-and-excite
gate. The gate uses a ReLU inner activation and a plain sigmoid, which is a
third combination distinct from the MobileNetV3 and EfficientNet ones, so this
family keeps its own copy.
"""
...
def add_squeeze_excite(
    ...
    *,
    ...
    Spatial mean, a 1x1 reduce convolution with ReLU, a 1x1 expand convolution,
    then a sigmoid gate multiplied back into x. timm names the two projections
    fc1 and fc2 here even though both are 1x1 convolutions.
    """
    ...
    reduced = add_conv2d(
        network, squeezed, fc1_w, fc1_b, int(fc1_w.shape[0]), (1, 1), dtype=dtype)
    reduced = add_relu(network, reduced)
    expanded = add_conv2d(
        network, reduced, fc2_w, fc2_b, int(fc2_w.shape[0]), (1, 1), dtype=dtype)
    gate = network.add_activation(expanded, trt.ActivationType.SIGMOID).get_output(0)
