...
_STAGE1_DEBUG_MLP_INPUT_INDEX_ENV = "TRTMC_SANA_WM_STAGE1_DEBUG_MLP_INPUT_INDEX"
...
def _stage1_debug_mlp_input_index() -> int | None:
    value = os.environ.get(_STAGE1_DEBUG_MLP_INPUT_INDEX_ENV)
    if value is None or not value.strip():
        return None
    block_index = int(value)
    if block_index < 0:
        raise ValueError(f"{_STAGE1_DEBUG_MLP_INPUT_INDEX_ENV} must be a non-negative block index")
    return block_index
...
def _stage1_env_flag(name: str, *, default: bool = False) -> bool:
...
def _add_silu_for_stage1_mlp(
    ...
    *,
    ...
    if dtype != np.float32 and _stage1_env_flag("TRTMC_SANA_WM_MLP_SILU_FP32", default=True):
...
def _can_use_trt_loop(network: Any, trt_module: Any) -> bool:
    return (
        _stage1_env_flag("TRTMC_SANA_WM_TRT_LOOP", default=False)
        and hasattr(network, "add_loop")
...
def _get_sana_wm_plugin_creator_with_get_creator(trt_module: Any, plugin_name: str) -> Any | None:
    global _SANA_WM_GDN_PLUGIN_LOAD_ATTEMPTED
    registry_fn = getattr(trt_module, "get_plugin_registry", None)
    if registry_fn is None:
        return None
...
def _get_sana_wm_gdn_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmGdnScan")
...
def _get_sana_wm_timestep_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmTimestepEmbed")


def _get_sana_wm_gate_proj_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmGateProj")


def _get_sana_wm_decay_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmDecay")


def _get_sana_wm_frame_mean_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmFrameMean")


def _get_sana_wm_t2i_modulate_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmT2IModulate")


def _get_sana_wm_caption_embed_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmCaptionEmbed")


def _get_sana_wm_cross_attention_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmCrossAttention")


def _get_sana_wm_softmax_attention_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(
        trt_module,
...
def _get_sana_wm_frame_gate_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmFrameGate")


def _get_sana_wm_layer_norm_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmLayerNorm")


def _get_sana_wm_short_conv_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmShortConv")
...
def _get_sana_wm_cam_prep_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmCamPrep")


def _get_sana_wm_torch_cam_prep_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmTorchCamPrep")
...
def _get_sana_wm_ucpe_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmUcpe")


def _get_sana_wm_torch_conv2d_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmTorchConv2d")


def _get_sana_wm_glumbconvtemp_plugin_creator(trt_module: Any) -> Any | None:
    return _get_sana_wm_plugin_creator_with_get_creator(trt_module, "SanaWmGlumbconvTemp")
...
def _apply_sana_wm_frame_mean_plugin(
    ...
    *,
    ...
    if os.environ.get("TRTMC_SANA_WM_FRAME_MEAN_PLUGIN", "1") in (
        "0",
        "false",
        "False",
    ):
    ...
    try:
        plugin = creator.create_plugin("sana_wm_frame_mean", collection)
    except TypeError:
        return None
...
def _add_sana_wm_bf16_linear_plugin(
    ...
    *,
    ...
    if not _is_bf16_dtype(dtype) or os.environ.get(env_var, default_value) in (
        "0",
        "false",
        "False",
    ):
...
def _add_sana_wm_output_gate_plugin(
    ...
    *,
    ...
    if not _is_bf16_dtype(dtype) or os.environ.get("TRTMC_SANA_WM_OUTPUT_GATE_PLUGIN", "0") in (
        "0",
        "false",
        "False",
    ):
...
def _create_sana_wm_frame_gate_plugin(trt_module: Any) -> Any | None:
    if os.environ.get("TRTMC_SANA_WM_FRAME_GATE_PLUGIN", "0") in (
        "0",
        "false",
        "False",
    ):
    ...
    collection = trt_module.PluginFieldCollection([])
    try:
        return creator.create_plugin("sana_wm_frame_gate", collection)
    except TypeError:
        return None
...
def _create_sana_wm_caption_embed_plugin(
    ...
    *,
    ...
    if os.environ.get("TRTMC_SANA_WM_CAPTION_EMBED_PLUGIN", "0") in (
        "0",
        "false",
        "False",
    ):
    ...
    try:
        plugin = creator.create_plugin("sana_wm_caption_embed", collection)
...
def _apply_sana_wm_cross_attention_plugin(
    ...
    *,
    ...
    if not _is_bf16_dtype(dtype) or os.environ.get("TRTMC_SANA_WM_CROSS_ATTN_PLUGIN", "0") in (
        "0",
        "false",
        "False",
    ):
    ...
    if add_plugin is None or creator is None or not hasattr(trt_module, "PluginField"):
        return None
    heads_value = np.asarray([num_heads], dtype=np.int32)
    collection = trt_module.PluginFieldCollection(
        [
    ...
    try:
        plugin = creator.create_plugin("sana_wm_cross_attention", collection)
    except TypeError:
        return None
...
def _create_sana_wm_torch_conv2d_plugin(
    ...
    *,
    ...
    collection = trt_module.PluginFieldCollection(fields)
    try:
        plugin = creator.create_plugin("sana_wm_torch_conv2d", collection)
        _BF16_WEIGHT_REFS.extend(refs)
        return plugin
    except TypeError:
        return None
...
def _create_sana_wm_glumbconvtemp_plugin(
    ...
    *,
    ...
    collection = trt_module.PluginFieldCollection(fields)
    try:
        plugin = creator.create_plugin("sana_wm_glumbconvtemp", collection)
        _BF16_WEIGHT_REFS.extend(refs)
        return plugin
    except TypeError:
        return None
...
def _create_sana_wm_qk_rope_plugin(
    ...
    *,
    ...
    fields.append(
        trt_module.PluginField("torch_rms", torch_rms_value, trt_module.PluginFieldType.INT32)
    )
...
def _sana_wm_camera_prep_plugin_mode() -> str:
    value = os.environ.get("TRTMC_SANA_WM_CAMERA_PREP_PLUGIN", "v").strip().lower()
    if value in ("", "0", "false", "off", "none"):
        return "off"
    if value in ("1", "true", "on", "full", "all"):
        return "all"
    if value in ("v", "v_only", "v-only"):
        return "v"
    if value in ("hybrid", "torch_qk_cuda_v", "torch-qk-cuda-v"):
        return "hybrid"
    return value
...
def _create_sana_wm_softmax_attention_plugin(
    ...
    *,
    ...
    if os.environ.get("TRTMC_SANA_WM_SOFTMAX_ATTN_PLUGIN", "1") in (
        "0",
        "false",
        "False",
    ):
    ...
    try:
        plugin = creator.create_plugin(
            "sana_wm_softmax_attention", trt_module.PluginFieldCollection(fields)
        )
        _BF16_WEIGHT_REFS.extend(refs)
        return plugin
    except TypeError:
...
def _create_sana_wm_cam_prep_plugin(
    ...
    *,
    ...
    if _sana_wm_camera_prep_plugin_mode() == "off":
        return None
    if use_torch_plugin is None:
        use_torch_plugin = _stage1_env_flag(
            "TRTMC_SANA_WM_TORCH_CAMERA_PREP",
            default=False,
        )
    creator = (
...
def _create_sana_wm_camera_beta_discount_plugin(
    ...
    *,
    ...
    collection = trt_module.PluginFieldCollection(fields)
    try:
        plugin = creator.create_plugin("sana_wm_camera_beta_discount", collection)
        _BF16_WEIGHT_REFS.extend(refs)
        return plugin
    except TypeError:
        return None
...
def _add_patch_embed3d(
    ...
    *,
    ...
    if (
        ...
        and _stage1_env_flag("TRTMC_SANA_WM_PATCH_EMBED_TORCH_CONV", default=True)
    ):
        ...
        weight_2d = np.ascontiguousarray(np.asarray(weight[:, :, 0, :, :], dtype=np.float32))
        torch_weights: WeightDict = WeightDict()
        torch_weights[f"{prefix}.proj_torch_conv.weight"] = weight_2d
        if bias is not None:
        ...
        if conv2d is not None:
            ...
            return (
                _set_tensor_name(flatten.get_output(0), f"{prefix}.tokens"),
                token_count,
                out_channels,
            )
...
def _lower_sana_wm_qk_rope_plugin(
    ...
    *,
    ...
    if add_plugin is None:
        return None
    q_norm_weight = np.ascontiguousarray(
        np.asarray(
            _required_weight(weights, f"{prefix}.attn.q_norm.weight"),
            dtype=np.float32,
        )
    )
    k_norm_weight = np.ascontiguousarray(
        np.asarray(
    ...
    if (
        ...
        if plugin is not None:
            ...
            return SanaWmStage1CameraPreamble(
                q=_set_tensor_name(layer.get_output(0), f"{prefix}.q_cam_bhdn.qk_norm_plugin"),
                k=_set_tensor_name(layer.get_output(1), f"{prefix}.k_cam_bhdn.qk_norm_plugin"),
...
def _apply_ray_projection_to_bhnd(
    ...
    *,
    ...
    token_count = shape.latent_frames * shape.latent_height * shape.latent_width
    if bf16_projection and hasattr(trt_module, "bfloat16"):
...
def _apply_ucpe_inverse_rope_plugin_to_bhnd(
    ...
    *,
    ...
    if dtype == np.float32 or os.environ.get("TRTMC_SANA_WM_UCPE_INVERSE_ROPE_PLUGIN", "0") in (
        "0",
        "false",
        "False",
    ):
...
def _apply_ucpe_block_diagonal_to_bhnd(
    ...
    *,
    ...
    if add_plugin is not None and not split_inverse_rope:
        plugin = _create_sana_wm_ucpe_plugin(
            trt_module,
            frames=shape.latent_frames,
            spatial=shape.latent_height * shape.latent_width,
...
def lower_sana_wm_stage1_camera_ucpe(
    ...
    *,
    ...
    hybrid_eligible = (
        ...
        and getattr(network, "add_plugin_v2", None) is not None
    )
    ...
    camera_q = camera.q
    camera_k = camera.k
    if dtype != np.float32 and os.environ.get("TRTMC_SANA_WM_CAMERA_QK_BF16_BRIDGE", "0") not in (
        "0",
        "false",
        "False",
    ):
    ...
    if camera_prep_plugin_mode == "hybrid" and hybrid_eligible:
        ...
        v_bhdn = cuda_fused.v
        if _stage1_env_flag(
            "TRTMC_SANA_WM_CAMERA_BETA_DISCOUNT_PLUGIN",
            default=False,
        ):
...
def _lower_sana_wm_stage1_bidirectional_gdn_core_raw_plugin(
    ...
    *,
    ...
    if not _stage1_env_flag("TRTMC_SANA_WM_RAW_GDN_PLUGIN", default=True):
        return None
    add_plugin = getattr(network, "add_plugin_v2", None)
    if (
        add_plugin is None
    ...
    out = layer.get_output(0)
    debug_output = os.environ.get("TRTMC_SANA_WM_GDN_DEBUG_OUTPUT", "")
    debug_output_for_block = bool(debug_output) and (
        _stage1_debug_stop_after_block() == block_index
    )
    if dtype != np.float32 and not debug_output_for_block:
        out = _cast_to_dtype(network, out, _trt_dtype_for_np(trt_module, dtype))
...
def _lower_sana_wm_stage1_camera_single_path_combined_plugin(
    ...
    *,
    ...
    if not _stage1_env_flag("TRTMC_SANA_WM_CAMERA_COMBINED_PLUGIN", default=True):
        return None
    add_plugin = getattr(network, "add_plugin_v2", None)
    if add_plugin is None:
        return None
    ...
    *,
    ...
    if gated is not None:
        ...
        projected = _add_sana_wm_bf16_linear_plugin(
            ...
            trt_module=trt_module,
            dtype=dtype,
            env_var="TRTMC_SANA_WM_ATTN_PROJ_PLUGIN",
            name=f"{prefix}.proj",
        )
...
def _lower_sana_wm_torch_softmax_attention(
    ...
    *,
    ...
    if not _is_bf16_dtype(dtype) or getattr(network, "add_plugin_v2", None) is None:
        return None
...
def _apply_frame_gate_to_tokens(
    ...
    *,
    ...
    if _is_bf16_dtype(dtype):
        ...
        add_plugin = getattr(network, "add_plugin_v2", None)
        if plugin is not None and add_plugin is not None:
...
def _add_stage1_mlp_conv2d(
    ...
    *,
    ...
    use_torch_conv = force_torch_conv or _stage1_env_flag(
        "TRTMC_SANA_WM_MLP_TORCH_CONV", default=False
    )
    if use_torch_conv:
...
def _lower_sana_wm_stage1_glumbconvtemp_mlp(
    ...
    *,
    ...
    if debug_return not in internal_debug_returns and _stage1_env_flag(
        "TRTMC_SANA_WM_MLP_PLUGIN", default=True
    ):
    ...
    inverted = _add_stage1_mlp_conv2d(
        ...
        trt_module=trt_module,
        dtype=dtype,
        force_torch_conv=_stage1_env_flag("TRTMC_SANA_WM_MLP_INVERTED_TORCH_CONV", default=False),
    )
    ...
    *,
    ...
    if frontend.plucker_tokens is not None and _model_bool(
        ...
        plucker = _add_sana_wm_bf16_linear_plugin(
            ...
            trt_module=trt_module,
            dtype=dtype,
            env_var="TRTMC_SANA_WM_PLUCKER_PROJ_PLUGIN",
