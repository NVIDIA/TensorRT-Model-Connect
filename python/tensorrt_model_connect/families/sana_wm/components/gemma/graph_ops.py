...
def _require_sana_wm_gemma_plugin_creator(name: str):
    ...
    if get_creator is not None:
        try:
            creator = get_creator(name, "1", "")
        except TypeError:
            creator = get_creator(name, "1")
    if creator is None:
        raise RuntimeError(
            f"{name} plugin is not registered; load "
            "libtrtmc_sana_wm_gdn_plugin.so before building the text encoder"
        )
    return creator


def add_sana_wm_gemma_rms_norm(
    network: trt.INetworkDefinition,
    inp: trt.ITensor,
    gamma: np.ndarray,
    eps: float,
) -> trt.ITensor:
    """Apply Gemma RMSNorm with the exact ATen FP32 reduction sequence."""
    creator = _require_sana_wm_gemma_plugin_creator("SanaWmGemmaRmsNorm")
    gamma = np.ascontiguousarray(gamma, dtype=np.float32).reshape(-1)
    gamma_tensor = add_constant(network, gamma.shape, gamma, dtype=np.float32)
    fields = [
        trt.PluginField("eps", np.asarray([eps], dtype=np.float32), trt.PluginFieldType.FLOAT32)
    ]
    plugin = creator.create_plugin("sana_wm_gemma_rms_norm", trt.PluginFieldCollection(fields))
    return network.add_plugin_v2([inp, gamma_tensor], plugin).get_output(0)


def add_sana_wm_gemma_rms_norm_per_head(
    network: trt.INetworkDefinition,
    inp: trt.ITensor,
    num_heads: int,
    head_dim: int,
    ...
    """Apply exact Gemma3 Q/K RMSNorm independently to each head."""
    gamma = np.asarray(gamma, dtype=np.float32).reshape(-1)
    if gamma.size == num_heads * head_dim:
        per_head = gamma.reshape(num_heads, head_dim)
        if not np.array_equal(per_head, np.broadcast_to(per_head[:1], per_head.shape)):
            raise ValueError("exact Gemma3 per-head RMSNorm requires shared head weights")
        gamma = per_head[0]
    elif gamma.size != head_dim:
        raise ValueError(
            f"Gemma3 per-head RMSNorm expected {head_dim} or "
            f"{num_heads * head_dim} weights, got {gamma.size}"
        )
    reshaped = network.add_shuffle(inp)
    reshaped.reshape_dims = (-1, num_heads, head_dim)
    normalized = add_sana_wm_gemma_rms_norm(
        network,
        reshaped.get_output(0),
        gamma,
        eps,
    )
...
def add_sana_wm_gemma_gated_gelu(
    network: trt.INetworkDefinition,
    gate: trt.ITensor,
    up: trt.ITensor,
) -> trt.ITensor:
    """Apply Gemma's BF16 tanh-GELU gate and product through ATen."""
    creator = _require_sana_wm_gemma_plugin_creator("SanaWmGemmaGatedGelu")
    plugin = creator.create_plugin("sana_wm_gemma_gated_gelu", trt.PluginFieldCollection([]))
    return network.add_plugin_v2([gate, up], plugin).get_output(0)


def add_sana_wm_gemma_rope(
    network: trt.INetworkDefinition,
    inp: trt.ITensor,
    num_heads: int,
    head_dim: int,
    ...
    """Apply Gemma RoPE with the exact ATen BF16 operation sequence."""
    creator = _require_sana_wm_gemma_plugin_creator("SanaWmGemmaRope")
    cos = network.add_gather(cos_table, position_id, 0).get_output(0)
    sin = network.add_gather(sin_table, position_id, 0).get_output(0)
    fields = [
        trt.PluginField(
            "heads", np.asarray([num_heads], dtype=np.int32), trt.PluginFieldType.INT32
        ),
        trt.PluginField(
            "head_dim", np.asarray([head_dim], dtype=np.int32), trt.PluginFieldType.INT32
        ),
        trt.PluginField(
            "rotary_dim",
            np.asarray([rotary_embedding_dim], dtype=np.int32),
            trt.PluginFieldType.INT32,
        ),
        trt.PluginField(
            "interleaved",
            np.asarray([int(interleaved)], dtype=np.int32),
            trt.PluginFieldType.INT32,
        ),
    ]
    plugin = creator.create_plugin("sana_wm_gemma_rope", trt.PluginFieldCollection(fields))
    return network.add_plugin_v2([inp, cos, sin], plugin).get_output(0)


def add_sana_wm_gemma_attention(
    network: trt.INetworkDefinition,
    q: trt.ITensor,
    k: trt.ITensor,
    v: trt.ITensor,
    ...
    *,
    ...
    """Run Gemma attention through ATen SDPA on TensorRT's CUDA stream."""
    creator = _require_sana_wm_gemma_plugin_creator("SanaWmGemmaAttention")
    fields = [
        trt.PluginField(
            "heads", np.asarray([num_heads], dtype=np.int32), trt.PluginFieldType.INT32
        ),
        trt.PluginField(
            "kv_heads",
            np.asarray([num_kv_heads], dtype=np.int32),
            trt.PluginFieldType.INT32,
        ),
        trt.PluginField(
            "head_dim", np.asarray([head_dim], dtype=np.int32), trt.PluginFieldType.INT32
        ...
        trt.PluginField(
            "scale", np.asarray([scale], dtype=np.float32), trt.PluginFieldType.FLOAT32
        ),
    ]
    plugin = creator.create_plugin("sana_wm_gemma_attention", trt.PluginFieldCollection(fields))
    return network.add_plugin_v2([q, k, v, mask], plugin).get_output(0)
