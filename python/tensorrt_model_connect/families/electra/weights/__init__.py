...
"""Checkpoint readers and tensor container used by the ELECTRA weight mapper."""
...
class WeightDict(dict):
    """ELECTRA encoder weights keyed by the local graph builder's logical names."""
...
def _open_safetensors(model_dir: Path) -> list:
    """Open a standard Hugging Face ELECTRA checkpoint."""
    ...
    # Older ELECTRA repositories may predate safetensors.
    bin_single = model_dir / "pytorch_model.bin"
    if bin_single.exists():
        return _ReaderCollection([_TorchBinReader(bin_single)])

    raise FileNotFoundError(
        f"No ELECTRA model.safetensors, index, or pytorch_model.bin in {model_dir}"
    )
...
def _detect_electra_prefix(readers: list) -> str:
...
def load_electra_weights(model_dir: str | Path, config: ModelConfig) -> WeightDict:
    """Map Hugging Face ELECTRA tensors to the local encoder graph contract."""
    readers = _open_safetensors(Path(model_dir))
    hidden = config.hidden_size
    root = _detect_electra_prefix(readers)
