...
"""Fail-closed routing contract for Mistral's TensorRT native KV path."""
...
def _checked_product(label: str, *values: int) -> int:
    ...
    for value in values:
        if value <= 0 or product > _UINT64_MAX // value:
            raise ValueError(f"native Mistral KV {label} exceeds uint64")
        product *= value
    return product
...
def native_kv_cache_geometry(
    ...
    *,
    ...
    if capacity != context:
        raise ValueError(
            "native Mistral KV requires max_cache_length == "
...
def _validate_default_rope(raw: dict, reasons: list[str]) -> None:
    ...
    if rope_type not in ("", "default") or any(
        ...
        reasons.append("native Mistral supports only unscaled default RoPE")


def native_kv_architecture_capability(
    config: object,
) -> NativeKvCapability:
    """Accept any dense Mistral size with the qualified attention contract."""

    model_type = str(getattr(config, "model_type", "")).lower()
    if not model_type.startswith("mistral"):
        return _result(applicable=False)
    if model_type != "mistral":
        return _result(reasons=["model_type must be exactly 'mistral'"])
    ...
    if tuple(getattr(config, "architectures", ()) or ()) != (
        ...
        reasons.append("architectures must contain exactly MistralForCausalLM")
    ...
    try:
        ...
        if head_dim != 128:
            reasons.append("native Mistral attention requires head_dim=128")
    except ValueError as exc:
        reasons.append(str(exc))

    if str(getattr(config, "hidden_act", "")).lower() != "silu":
        reasons.append("native Mistral requires hidden_act='silu'")
    ...
    if enabled:
        reasons.append("unsupported Mistral fields: " + ", ".join(enabled))
    try:
        if _integer(raw.get("pretraining_tp", 1), "pretraining_tp") != 1:
            reasons.append("native Mistral requires pretraining_tp=1")
    except ValueError as exc:
        reasons.append(str(exc))
    try:
        if float(raw.get("partial_rotary_factor", 1.0)) != 1.0:
            reasons.append("native Mistral requires full rotary embeddings")
    except (TypeError, ValueError, OverflowError):
    ...
    if layer_types is not None and (
        ...
        reasons.append("native Mistral does not support hybrid layer types")
...
def native_kv_build_capability(
    ...
    *,
    ...
    if str(precision).lower() != "bf16":
        reasons.append("native Mistral requires BF16")
    if str(raw.get("_decoder_engine_layout", "split")) != "split":
        reasons.append("native Mistral requires split prefill/decode engines")
    if raw.get("_rtx_build_requested"):
        reasons.append("native Mistral requires the standard TensorRT backend")
    if parallel_enabled or raw.get("_parallel_build_enabled"):
        reasons.append("native Mistral does not support tensor parallel builds")
    if (
        dynamic_kv_cache
        or raw.get("_runtime_dynamic_kv_requested")
        or raw.get("dynamic_kv_cache")
    ):
        reasons.append("native Mistral uses one fixed physical KV capacity")
    if (
        quantized
        or raw.get("quantization_config")
        or raw.get("_quantized_build_requested")
    ):
        reasons.append("native Mistral does not support quantized builds")
    if raw.get("_fp32_layers"):
        reasons.append("native Mistral does not support FP32 layer overrides")
    if debug_layer_outputs:
        reasons.append("native Mistral does not support debug layer outputs")
    try:
...
def prefer_native_default(config: object) -> bool:
    """Mistral production routing has no legacy KV fallback."""
