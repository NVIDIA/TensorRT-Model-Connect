...
"""Native-only split prefill/decode builder for dense Mistral models."""
...
def build_native_decoder_engine(
    ...
    *,
    ...
    if precision != "bf16":
        raise ValueError("native Mistral decoder requires BF16")
    if profile_mode not in ("prefill", "decode"):
        raise ValueError(
            f"native Mistral profile_mode must be 'prefill' or 'decode', got {profile_mode!r}"
        )
    ...
    if verbose:
        print(
            f"[trtmc build] Building native Mistral {profile_mode} engine "
    ...
    if plan is None:
        raise RuntimeError(f"native Mistral {profile_mode} engine build failed")
    return bytes(plan)
