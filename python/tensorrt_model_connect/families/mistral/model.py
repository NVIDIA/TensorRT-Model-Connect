...
def build_engine(
    ...
    *,
    ...
    if normalize_parallel_config(parallel_config).enabled:
        raise ValueError("mistral does not support tensor-parallel builds")
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete mistral bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("mistral does not support context-parallel builds")
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="mistral tensor-parallel builds")
        raise ValueError("mistral does not support tensor-parallel builds")
