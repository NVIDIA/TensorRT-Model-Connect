...
def make_native_active_rope_inv_freq(
    ...
    try:
        ...
        raise RuntimeError(
            "TensorRT native Mistral KV build requires PyTorch in the Model "
...
def add_native_kv_cache_attention_from_rows(
    ...
    *,
    ...
    """Update a user-owned full-capacity cache and attend over its active prefix."""

    if not hasattr(network, "add_kv_cache_update") or not hasattr(network, "add_attention_v2"):
        raise RuntimeError(
            "Mistral native KV cache requires TensorRT add_kv_cache_update "
    ...
    if update_k is None or update_v is None:
        raise RuntimeError("TensorRT failed to create Mistral KV-cache update layers")
    if tag:
    ...
    if q_4d.dtype != trt.bfloat16:
        raise ValueError("Mistral native KV attention requires BF16 queries")
    ...
    if attention is None:
        raise RuntimeError("TensorRT failed to create Mistral native attention")
