...
"""Mapped-weight contract for Mistral's TensorRT native KV graph."""
...
def _shape(value: object, name: str) -> tuple[int, ...]:
    try:
        ...
        raise ValueError(
            f"native Mistral weight {name} has an invalid shape"
        ) from exc
...
def _require(
    ...
    if name not in weights:
        raise ValueError(f"missing native Mistral weight {name}")
    actual = _shape(weights[name], name)
    if actual != expected:
        raise ValueError(
            f"native Mistral weight {name} must have shape "
...
def validate_native_kv_weights(
    ...
    if not isinstance(weights, Mapping):
        raise ValueError("native Mistral weights must be a mapping")
    ...
    if malformed or layer_indices != set(range(layers)):
        raise ValueError(
            "native Mistral weights require continuous layer indices; "
    ...
    for name, expected in (
        ...
        if name in weights and int(weights[name]) != expected:
            raise ValueError(
                f"native Mistral metadata {name} must be {expected}"
            )
    ...
    if forbidden:
        raise ValueError(
            "native dense Mistral does not support bias weights: "
