    ...
    * ``"prefill"``: one prefill profile only. This is used by split-engine
    ...
    def _add_profile(opt_sq: int, max_sq: int, *, fixed: bool = False,
        ...
        if multi_bucket_decode:
            ...
            for i in range(num_layers):
                for name in (layer_tensor_name("cache_k", i),
                             layer_tensor_name("cache_v", i)):
                    prof.set_shape(
                        name,
...
def build_standard_decoder_engine(
    ...
    *,
        ...
        if dynamic_kv_profile_rows:
            for profile_rows in dynamic_kv_profile_rows:
                ...
                for i in range(num_layers):
                    ...
                    profile.set_shape(layer_tensor_name("cache_k", i),
                                      min_cache_shape, cache_shape, cache_shape)
                    profile.set_shape(layer_tensor_name("cache_v", i),
                                      min_cache_shape, cache_shape, cache_shape)
    ...
    lm_bias = weights.get("lm_head_bias")
    if lm_bias is not None:
        logits = add_bias_sum(network, logits, out_vocab, lm_bias,
                                        dtype=work_np_dtype)
    else:
        b_out = np.zeros(out_vocab, dtype=work_np_dtype)
        logits = add_bias_sum(network, logits, out_vocab, b_out,
                                        dtype=work_np_dtype)
