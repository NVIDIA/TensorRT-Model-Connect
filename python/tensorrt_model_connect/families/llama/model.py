...
def build_engine(
    ...
    *,
    ...
    if normalize_parallel_config(parallel_config).enabled:
        raise ValueError("llama does not support tensor-parallel builds")
...
def build(model_dir: str, output_path: str, **options) -> None:
    """Build a complete llama bundle from checkpoint to serialized artifact."""
    import json
    import time
    ...
    if parallel.cp_enabled:
        raise NotImplementedError("llama does not support context-parallel builds")
    ...
    if parallel.enabled:
        require_tensorrt_11_for_tensor_parallel(parallel, feature="llama tensor-parallel builds")
        raise ValueError("llama does not support tensor-parallel builds")
