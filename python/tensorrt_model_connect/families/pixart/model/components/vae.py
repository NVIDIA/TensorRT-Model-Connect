...
def _open_vae_safetensors(model_dir: str) -> list:
    ...
    if index.exists():
        ...
        return [safe_open(str(d / f), framework=fw) for f in shards]
...
def _add_group_norm_4d(
    ...
    from ..model import add_constant
...
def _add_resnet_block_2d(
    network, inp, weights, prefix: str, in_ch: int, out_ch: int, h: int, w: int
):
    """ResNetBlock2D: norm1 -> SiLU -> conv1 -> norm2 -> SiLU -> conv2 + shortcut."""
    from tensorrt_model_connect import trt_compat

    trt = trt_compat.get_trt()
    from ..model import add_conv2d, add_silu
...
def _add_self_attention_2d(network, inp, weights, prefix: str, ch: int, h: int, w: int):
...
def _add_upsample_2d(network, inp, weights, prefix: str, ch: int, h: int, w: int):
...
def build_vae_2d_decoder_engine(
    ...
    *,
    ...
    try:
        ...
        x = add_conv2d(
            network, inp, weight=pqc_w, bias=pqc_b, out_channels=latent_channels, kernel_size=(1, 1)
        )
    except KeyError:
    ...
    x = _add_resnet_block_2d(
        network, x, readers, "decoder.mid_block.resnets.0", ch_last, ch_last, cur_h, cur_w
    )

    # SelfAttention2D(512)
    x = _add_self_attention_2d(
        network, x, readers, "decoder.mid_block.attentions.0", ch_last, cur_h, cur_w
    )

    # ResNetBlock2D(512, 512)
    x = _add_resnet_block_2d(
        network, x, readers, "decoder.mid_block.resnets.1", ch_last, ch_last, cur_h, cur_w
    )
