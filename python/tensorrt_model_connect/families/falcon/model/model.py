...
def build_dual_profile_decoder_engine(config: 'ModelConfig', weights: 'WeightDict', max_cache_length: int, *, precision: str='fp16', opt_prefill_length: int=64, max_prefill_length: int | None=None, quant_ctx: 'QuantContext | None'=None, position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, alibi_bias_scale: float=1.0, verbose: bool=False, dynamic_kv_profile_rows: list[int] | None=None, profile_mode: str='dual_profile') -> bytes:
    ...
    * ``"prefill"``: one prefill profile only. This is used by split-engine
    ...
    if profile_mode == 'prefill':
        ...
        if _reverse:
            ...
            if multi_bucket_decode:
                ...
                _add_profile(1, 1, fixed=True)
    embedding_table = _const_in_work_dtype(network, (vocab, hidden), weights['embedding'], work_np_dtype, work_trt_dtype)
    ...
    if position_type == 'alibi':
        alibi_slopes_np = compute_alibi_slopes(num_heads) * float(alibi_bias_scale)
        alibi_slopes_tensor = add_constant(network, (num_heads, 1, 1), alibi_slopes_np.reshape(num_heads, 1, 1), dtype=np.float32)
        alibi_cache_positions_fp32 = add_constant(network, (max_cache_length,), np.arange(max_cache_length, dtype=np.float32), dtype=np.float32)
    ...
    if embed_norm is not None:
        ...
        hidden_state = _norm_multi(network, hidden_state, hidden, embed_norm, embed_norm_beta, eps_tensor, 'layernorm', work_np_dtype)
    if position_type == 'alibi':
        mask_4d = add_alibi_mask_4d(network, attention_mask_work, position_id, alibi_slopes_tensor, alibi_cache_positions_fp32, num_heads, target_dtype=work_trt_dtype)
    else:
    ...
    for layer_idx in range(num_layers):
        ...
        normed = _norm_multi(network, hidden_state, hidden, weights[f'{prefix}.input_norm'], weights.get(f'{prefix}.input_norm_beta'), eps_tensor, 'layernorm', work_np_dtype)
        ...
        if parallel_residual:
            ...
            if post_attn_norm_w is not None:
                norm2 = _norm_multi(network, hidden_state, hidden, post_attn_norm_w, weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, 'layernorm', work_np_dtype)
            else:
                norm2 = normed
        else:
            residual1 = network.add_elementwise(hidden_state, attn_out, trt.ElementWiseOperation.SUM)
            norm2 = _norm_multi(network, residual1.get_output(0), hidden, weights[f'{prefix}.post_attn_norm'], weights.get(f'{prefix}.post_attn_norm_beta'), eps_tensor, 'layernorm', work_np_dtype)
    ...
    if final_norm is not None and len(final_norm) > 0:
        hidden_state = _norm_multi(network, hidden_state, hidden, final_norm, weights.get('final_norm_beta'), eps_tensor, 'layernorm', work_np_dtype)
...
def build_standard_decoder_engine(config: ModelConfig, weights: WeightDict, max_cache_length: int, *, precision: str='fp32', quant_ctx: QuantContext | None=None, norm_type: str='layernorm', mlp_type: str='gelu_fc', position_type: str='rope', activation: str='silu', partial_rotary_factor: float=1.0, interleaved_rope: bool=False, parallel_residual: bool=False, scale_attn_weights: bool=True, alibi_bias_scale: float=1.0, embed_input: bool=False, verbose: bool=False, debug_layer_outputs: bool=False, hidden_state_output: bool=False) -> bytes:
    ...
    if not _dual_profile_disabled_for and decoder_engine_role in ('dual_profile', 'prefill'):
        return build_dual_profile_decoder_engine(config, weights, max_cache_length, precision=precision, quant_ctx=quant_ctx, position_type=position_type, activation=activation, partial_rotary_factor=partial_rotary_factor, interleaved_rope=interleaved_rope, parallel_residual=parallel_residual, scale_attn_weights=scale_attn_weights, alibi_bias_scale=alibi_bias_scale, verbose=verbose, profile_mode='prefill' if decoder_engine_role == 'prefill' else 'dual_profile')
    ...
    if position_type == 'rope':
        ...
        pos_embed_np = weights['position_embedding']
        position_embed_table = add_constant(network, pos_embed_np.shape, pos_embed_np, dtype=work_np_dtype)
        ...
        alibi_indices_tensor = add_constant(network, (max_cache_length,), np.arange(max_cache_length, dtype=np.float32), dtype=np.float32)
    ...
    if embed_input and input_embed_tensor is not None and (use_input_embed_tensor is not None):
        ...
        one_const = add_constant(network, (1, 1), np.array([1.0], dtype=work_np_dtype), dtype=work_np_dtype)
        ...
        tok_part = network.add_elementwise(inv_flag.get_output(0), token_embed, trt.ElementWiseOperation.PROD)
        embed_part = network.add_elementwise(flag_for_math, input_embed_tensor, trt.ElementWiseOperation.PROD)
...
def _add_decoder_layer(*, network: trt.INetworkDefinition, hidden: trt.ITensor, cache_k: trt.ITensor, cache_v: trt.ITensor, attention_mask: trt.ITensor, position_id: trt.ITensor, attention_scale: float | None, eps_tensor: trt.ITensor, weights: WeightDict, prefix: str, hidden_size: int, attention_size: int, kv_attention_size: int, mlp_size: int, num_heads: int, num_kv_heads: int, head_dim: int, max_cache_length: int, position_type: str='rope', activation: str='silu', parallel_residual: bool=False, alibi_slopes_tensor: trt.ITensor | None=None, alibi_indices_tensor: trt.ITensor | None=None, dtype: np.dtype=np.float32, quant_ctx: QuantContext | None=None, cos_half_tensor: trt.ITensor | None=None, sin_half_tensor: trt.ITensor | None=None, rotary_embedding_dim: int=0, interleaved_rope: bool=False, ffi_attention_kernel: str | None=None, dynamic_kv_cache: bool=False, eps: float | None=None) -> dict[str, trt.ITensor]:
    ...
    mlp_out = add_gelu_fc_mlp(network, norm2, weights=weights, prefix=prefix, hidden_size=hidden_size, mlp_size=mlp_size, activation=activation, dtype=dtype, quant_ctx=quant_ctx, layer_prefix=prefix)
    if parallel_residual:
