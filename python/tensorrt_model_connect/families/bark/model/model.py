...
def add_gelu_erf(
    ...
    one_plus_erf = network.add_elementwise(
        _const(1.0), erf_out.get_output(0), trt.ElementWiseOperation.SUM
    )
    half_x = network.add_elementwise(_const(0.5), inp, trt.ElementWiseOperation.PROD)
    result = network.add_elementwise(
...
def add_attention_block(
    ...
    *,
    ...
    if use_native_attention:
        ...
        if num_kv_heads != num_heads:
            raise ValueError(
                "FFI decoder attention requires num_kv_heads == num_heads; use TRT native attention for compact GQA/MQA KV cache"
            )
