...
from .model.parallel import build_bark_tp_decoder_engine
...
def _load_bark_state_dict(model_dir: str) -> dict:
    ...
    if not model_path.exists():
        # Try safetensors
        from .weights import _open_safetensors
...
class BarkPlugin:
    ...
    def build_engine(
        ...
        *,
        ...
        return _build_bark_standard_engine(
            weights, "semantic", sem_cfg, max_cache_length, verbose=verbose
        )
    ...
    def build_extra_engines(
        ...
        *,
        ...
        if parallel.enabled:
            ...
            with timed_trt_compile(build_timing, "extra_bark_coarse_decoder"):
                coarse_plan = _build_bark_standard_engine(
                    weights, "coarse", coarse_cfg, max_cache_length, verbose=verbose
                )
        ...
        if state_dict is not None:
            from .model.components.codec import build_encodec_decoder_engine
...
def _build_bark_standard_engine(
    weights: WeightDict, sub_model: str, sub_cfg: dict, max_cache_length: int, verbose: bool = False
) -> bytes:
    ...
    return build_standard_decoder_engine(sub_mc, sub_weights, max_cache_length, verbose=verbose)
...
def _build_bark_fine_engine(
    ...
    for layer_idx in range(num_layers):
        ...
        attn_out = graph_ops.add_matmul_rhs_constant(
            network, ctx_flat, attention_size, hidden, weights[f"{lp}.w_o"]
        )
    ...
    for j in range(n_lm_heads):
        logits_j = graph_ops.add_matmul_rhs_constant(
            network, hidden_state, hidden, codebook_size, weights[f"{prefix}w_lm_head_{j}"]
        )
