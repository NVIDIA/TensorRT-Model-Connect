...
"""Mapped-weight contract for GLM's TensorRT native KV graph."""
...
def _shape(value: object, name: str) -> tuple[int, ...]:
    try:
        ...
        raise ValueError(f"native GLM weight {name} has an invalid shape") from exc
...
def _require(
    ...
    if name not in weights:
        raise ValueError(f"missing native GLM weight {name}")
    actual = _shape(weights[name], name)
    if actual != expected:
        raise ValueError(f"native GLM weight {name} must have shape {expected}, got {actual}")
...
def validate_native_kv_weights(
    ...
    """Validate mapped tensors before TensorRT graph construction."""

    if not isinstance(weights, Mapping):
        raise ValueError("native GLM weights must be a mapping")
    ...
    if malformed or layer_indices != set(range(layers)):
        raise ValueError(
            "native GLM weights require continuous layer indices; "
    ...
    for name, expected in (
        ...
        if name in weights and int(weights[name]) != expected:
            raise ValueError(f"native GLM metadata {name} must be {expected}")

    _require(weights, "embedding", (vocab, hidden))
    _require(weights, "final_norm", (hidden,))
    _require(weights, "w_out", (hidden, vocab))
    forbidden = [name for name in ("final_norm_beta", "lm_head_bias") if name in weights]
    ...
    for layer in range(layers):
        ...
        forbidden.extend(
            f"{prefix}.{suffix}"
            for suffix in _FORBIDDEN_BIAS_SUFFIXES
            if f"{prefix}.{suffix}" in weights
        )
        forbidden.extend(
            name for name in (f"{prefix}.q_norm", f"{prefix}.k_norm") if name in weights
        )

    if forbidden:
        raise ValueError("native GLM found unsupported weights: " + ", ".join(sorted(forbidden)))
