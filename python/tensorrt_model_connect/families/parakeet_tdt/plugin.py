...
"""Parakeet TDT family plugin -- FastConformer plus token-duration transducer.
...
the checkpoint and builds three TensorRT plans:
  ...
  * ``engine_plan``: TDT prediction network
  * ``joint_engine_plan``: TDT joint network

The C++ runtime strategy is ``parakeet_tdt_speech_to_text`` and performs greedy TDT
...
def _build_streaming_encoder(weights: WeightDict, right_context: int, *,
    ...
    if verbose:
        ...
        print(f"[trtmc build] Building TDT streaming encoder {step} "
              f"(right={right_context}, mel={mel_len}, frames={query_len})",
              file=sys.stderr)
    plan = builder.build_serialized_network(network, config)
    if plan is None:
        raise RuntimeError(
            f"TDT streaming encoder build failed for right={right_context}, "
            f"first_step={first_step}")
    return bytes(plan)


class ParakeetTDTPlugin:
    name = "parakeet_tdt"
    runtime_strategy = "parakeet_tdt_speech_to_text"
    ...
    def load_weights(self, model_dir: str, config: ModelConfig, *, precision: str = "fp32") -> WeightDict:
        del precision
        w = WeightDict()
        hf_checkpoint = (Path(model_dir) / "model.safetensors").is_file()
        sd, ncfg = (_load_hf_as_nemo(model_dir) if hf_checkpoint
                    else _load_nemo_archive(model_dir))
    ...
    def build_engine(self, config: ModelConfig, weights: WeightDict, max_cache_length: int,
        ...
        if parallel.enabled:
            raise ValueError("Parakeet TDT currently supports single-device engine builds")
