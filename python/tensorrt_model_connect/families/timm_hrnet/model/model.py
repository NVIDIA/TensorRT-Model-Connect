...
"""TensorRT graph builders for timm HRNet classifiers.

The HRNet stack is convolutional, so it needs a different op set from the
transformer families: convolution, folded batch norm, ReLU, pooling, and a
final fully-connected head. Fusing the parallel branches also needs a
...
def add_nearest_upsample(network, x, scale: int):
    ...
    layer.scales = [1.0, 1.0, float(scale), float(scale)]
    return layer.get_output(0)
