...
def _redact_text(value: str) -> str:
    value = _QUERY_STRING_PATTERN.sub(f"?{_REDACTED}", value)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="MODEL",
        help="Fail startup unless every MODEL replica proves native streaming support (repeatable)",
    )
    parser.add_argument(
        "--model-replicas",
        action="append",
    ...
    parser.add_argument(
        "--runtime-root",
        metavar="DIR",
        help="Directory containing the exact family and backend runtime DSOs",
    )
    parser.add_argument("--kv-cache-size", type=_positive_int, metavar="BYTES")
...
def resolve_runtime_root(value: str | None) -> Path:
    if value:
        ...
        if candidate.is_dir():
            return candidate.resolve()
    raise ValueError("--runtime-root must name an existing directory")
    ...
    try:
        ...
        if api_key is not None:
            ...
            if not api_key:
                api_key = None
        if api_key is None:
            raise ValueError("authentication requires --api-key or TRTMC_SERVE_TOKEN")
...
def _install_secret_log_filter() -> None:
    ...
    for logger_name in ("uvicorn", "uvicorn.error", "uvicorn.access"):
        ...
        for handler in logger.handlers:
            ...
            if logger_name == "uvicorn.access" and isinstance(handler, logging.StreamHandler):
