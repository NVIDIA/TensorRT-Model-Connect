...
class ModelRegistry:
    ...
    def __init__(
        ...
        *,
        ...
        if load_options is None and worker_factory is None:
            raise ValueError("native workers require explicit runtime load options")
            ...
            try:
                for name in self._order:
                    ...
                    for replica in range(self._replicas[name]):
                        ...
                        self._validate_metadata(name, spec.kind, payload)
                    ...
                    capabilities = list(ready_payload["capabilities"])
                    metadata = {
                    ...
                    metadata["streaming_transcription"] = (
                        "transcription_streaming" in capabilities
                    )
                    if (
                        name in self._required_streaming
                        and not metadata["streaming_transcription"]
                    ):
                        raise WorkerProtocolError(
                            f"worker {name!r} does not advertise transcription_streaming"
                        )
                    if name in self._required_streaming:
                        for worker in workers:
                            ...
                            if (
                                ...
                                or set(probe) != {"supported"}
                                or probe["supported"] is not True
                            ):
                                raise WorkerProtocolError(
                                    f"worker {worker.name!r} returned an invalid streaming probe"
                                )
            ...
            return [
                {
                    ...
                    "owned_by": "trtmc",
                    "capabilities": list(
                        self._metadata.get(name, {}).get("capabilities", ())
                    ),
                    "metadata": {
    ...
    def status(self) -> dict[str, Any]:
        with self._lock:
            ...
            ready = (
                ...
                and bool(self._groups)
                and len(self._groups) == len(self._specs)
                and all(model["ready"] for model in statuses.values())
            )
            available = self._started and any(model["ready"] for model in statuses.values())
            degraded = available and any(
                model["ready_replicas"] < model["replicas"]
                for model in statuses.values()
            )
        return {"ready": ready, "degraded": degraded, "models": statuses}
    ...
    def _make_worker(self, spec: ModelSpec, replica: int) -> WorkerProcess:
        ...
        assert self._load_options is not None
    ...
    def _validate_metadata(
        name: str, kind: ModelKind, metadata: Mapping[str, Any]
    ) -> None:
        allowed = {
            "event",
            "protocol_version",
        ...
        if unknown:
            raise WorkerProtocolError(
                f"worker {name!r} metadata has unsupported field(s): {', '.join(unknown)}"
            )
        if metadata.get("event") != "ready":
            raise WorkerProtocolError(f"worker {name!r} metadata.event must be 'ready'")
        ...
        if (
            ...
            raise WorkerProtocolError(
                f"worker {name!r} metadata.capabilities must be unique non-empty strings"
            )
        required = "text_generation" if kind == "chat" else "transcription"
        if required not in capabilities:
            raise WorkerProtocolError(
                f"worker {name!r} does not advertise required capability {required!r}"
            )
        default_max_new_tokens = metadata.get("default_max_new_tokens")
        if "default_max_new_tokens" in metadata and (
