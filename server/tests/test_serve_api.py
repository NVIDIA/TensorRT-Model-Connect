...
from trtmc_server.app import (
    ServerConfig,
    _BodyLimitMiddleware,
    _HTTP_ENVELOPE_OVERHEAD_BYTES,
    _worker_request,
    create_app,
)
...
from trtmc_server.worker import WorkerLoadOptions, WorkerProcess, WorkerSession


FAKE_TRTMC = Path(__file__).with_name("fake_serve_worker.py")


def _http_scope(*, content_length: int | None = None) -> dict[str, object]:
...
def test_body_limit_rejects_declared_and_chunked_ingress_before_response() -> None:
    async def exercise_declared() -> tuple[bool, list[dict[str, object]]]:
        called = False
        ...
        async def receive() -> dict[str, object]:
            raise AssertionError("oversized Content-Length must not read the body")

        async def send(message: dict[str, object]) -> None:
            sent.append(message)

        middleware = _BodyLimitMiddleware(inner, limits={"/limited": 4})
        await middleware(_http_scope(content_length=5), receive, send)  # type: ignore[arg-type]
        return called, sent

    async def exercise_chunked() -> tuple[bool, list[dict[str, object]]]:
        completed = False
        sent: list[dict[str, object]] = []
        chunks = iter(
            (
        ...
        async def inner(_scope: object, receive: object, _send: object) -> None:
            nonlocal completed
        ...
        middleware = _BodyLimitMiddleware(inner, limits={"/limited": 4})
        await middleware(_http_scope(), receive, send)  # type: ignore[arg-type]
        return completed, sent

    declared_called, declared_messages = asyncio.run(exercise_declared())
    chunked_completed, chunked_messages = asyncio.run(exercise_chunked())
    for messages in (declared_messages, chunked_messages):
        assert messages[0]["status"] == 413
        assert b"request_body_too_large" in messages[-1]["body"]
    assert declared_called is False
    assert chunked_completed is False


def test_private_v3_text_and_invalid_request_shapes_are_strict() -> None:
...
def test_usage_rejects_non_finite_numbers() -> None:
...
def test_registry_start_is_one_transaction(tmp_path: Path) -> None:
    ...
    def make_worker(spec: ModelSpec) -> WorkerProcess:
        ...
        return WorkerProcess(
            ...
            startup_timeout=1,
            request_timeout=1,
            load_options=WorkerLoadOptions(runtime_root=str(tmp_path)),
        )
...
def test_registry_rejects_mismatched_replicas_and_rolls_back(tmp_path: Path) -> None:
    ...
    incompatible = tmp_path / "protocol-v2-asr.bundle"
...
@pytest.mark.parametrize(
    ("bundle_stem", "message"),
    (
        ("wrong-capability-chat", "required capability"),
        ("duplicate-capability-chat", "unique non-empty strings"),
        ("unknown-ready-field-chat", "unsupported field"),
    ),
)
def test_registry_rejects_noncanonical_v3_ready_metadata(
    tmp_path: Path, bundle_stem: str, message: str
) -> None:
...
def test_worker_request_cancellation_retains_lane_until_native_completion() -> None:
    ...
    async def exercise() -> None:
        ...
        request = asyncio.create_task(_worker_request(session, "generate", {}))  # type: ignore[arg-type]
        ...
        session.future.set_result({"text": "completed after disconnect"})
        for _ in range(10):
            if session.closed:
                break
            await asyncio.sleep(0)
...
def test_health_readiness_models_and_bearer_auth(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert health.json() == {"status": "ok", "degraded": False}
        ...
        assert readiness_payload["degraded"] is False
...
def test_chat_generation_and_unsupported_semantics_are_explicit(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert stopped.json()["error"]["param"] == "stop"
        ...
        assert multi_turn.status_code == 400
        assert multi_turn.json()["error"]["param"] == "messages"
...
def test_ready_capabilities_control_exposed_capability_and_startup(tmp_path: Path) -> None:
    ...
    with pytest.raises(WorkerProtocolError, match="does not advertise transcription_streaming"):
        unsupported.start()
    assert not unsupported.ready

    old_protocol = make_single_asr_registry(tmp_path, "protocol-v2-asr")
    with pytest.raises(WorkerProtocolError, match="unsupported protocol version"):
        old_protocol.start()
    assert not old_protocol.ready


def test_required_streaming_probe_failure_rolls_back_every_replica(tmp_path: Path) -> None:
    bundle = tmp_path / "advertises-stream-but-probe-fails-asr.bundle"
    bundle.write_bytes(b"asr")
    workers: list[WorkerProcess] = []
    ...
    registry = ModelRegistry(
        ...
        trtmc_binary=FAKE_TRTMC,
        model_replicas={"asr": 2},
        required_streaming_transcription=("asr",),
        worker_factory=make_worker,
    )
...
def test_health_remains_ok_while_one_replica_can_serve(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert client.get("/healthz").json() == {"status": "ok", "degraded": True}
        assert client.get("/readyz").status_code == 200
        assert client.get("/readyz").json()["degraded"] is True
...
def test_worker_failures_are_http_errors_and_clear_readiness(
    ...
    with TestClient(app) as client:
        ...
        assert health.json() == {"status": "unavailable", "degraded": False}
...
def test_multipart_ingress_limit_precedes_upload_processing(tmp_path: Path) -> None:
    registry = make_registry(tmp_path)
    app = create_app(registry, config=ServerConfig(max_upload_bytes=4))
    oversized = b"x" * (_HTTP_ENVELOPE_OVERHEAD_BYTES + 5)
...
def test_realtime_reconfiguration_starts_a_fresh_item(tmp_path: Path) -> None:
    app = create_app(make_registry(tmp_path), config=ServerConfig(api_key="test-token"))
    with TestClient(app) as client:
        with client.websocket_connect(
            "/v1/realtime?intent=transcription&access_token=test-token"
            ...
            assert first_delta["delta"] == "1 samples"
            first_item_id = first_delta["item_id"]

            websocket.send_json(
                {
                    "type": "session.update",
                    "session": {"trtmc": {"sample_rate_hz": 16000}},
                }
            )
            assert websocket.receive_json()["type"] == "session.updated"

            websocket.send_json({"type": "input_audio_buffer.append", "audio": audio})
            next_delta = websocket.receive_json()
            assert next_delta["delta"] == "1 samples"
            assert next_delta["transcript"] == "1 samples"
            assert next_delta["item_id"] != first_item_id


def test_realtime_reset_clears_local_item_without_active_lease() -> None:
    class Registry:
    ...
    connection.transcript = "stale transcript"
    previous_item_id = connection.item_id
    connection.total_audio_bytes = 2

    asyncio.run(connection._release_stream(reset=True))  # noqa: SLF001

    assert connection.transcript == ""
    assert connection.item_id != previous_item_id
    assert connection.total_audio_bytes == 2


def test_realtime_forwards_validated_audio_wire_text_without_reencoding(
    tmp_path: Path,
) -> None:
    encoded = "AQB="
    assert base64.b64encode(base64.b64decode(encoded, validate=True)).decode() != encoded
    app = create_app(
        make_single_asr_registry(tmp_path, "echo-wire-asr"),
    )
    with TestClient(app) as client:
        with client.websocket_connect("/v1/realtime?intent=transcription") as websocket:
            ...
            delta = websocket.receive_json()
            assert delta["transcript"] == encoded
...
def test_realtime_unexpected_error_logs_only_safe_context(
    caplog: pytest.LogCaptureFixture,
) -> None:
    private_detail = "access_token=client-secret /private/model client-prompt"
    ...
    caplog.set_level(logging.ERROR, logger="trtmc_server.realtime")

    asyncio.run(connection.run())

    failure = websocket.events[-1]["error"]  # type: ignore[index]
    assert failure == {
        "type": "server_error",
        "code": "internal_error",
        "message": "realtime transcription session failed",
    }
    record = next(
        record
        for record in caplog.records
        if record.message.startswith("Unexpected realtime session failure")
    )
    assert connection.connection_id in record.message
    assert "RuntimeError" in record.message
    assert "receive_json" in record.message
    assert record.exc_info is None
    rendered_events = repr(websocket.events)
    for sensitive in (
        "access_token",
        "authorization",
        "client-secret",
        ...
        assert sensitive not in rendered_events
    assert record.message.count(" <- ") <= 7
