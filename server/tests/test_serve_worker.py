...
def make_worker(
    ...
    *,
    ...
    return WorkerProcess(
        ...
        load_options=load_options or WorkerLoadOptions(runtime_root=str(tmp_path)),
    )
...
def test_worker_ready_request_ids_metadata_stderr_and_cleanup(tmp_path: Path) -> None:
    ...
    assert worker.ready_payload["protocol_version"] == 3
    assert worker.ready_payload["capabilities"] == ["text_generation"]
...
def test_worker_environment_is_an_explicit_runtime_allowlist(
    ...
    for name in (
        ...
        monkeypatch.setenv(name, "distributed-test-value")
    worker = make_worker(tmp_path, "inspect-environment-chat")
    worker.start()
    try:
        assert worker.ready_payload["serve_token_present"] is False
        assert worker.ready_payload["secret_environment_present"] is False
        assert worker.ready_payload["distributed_environment_present"] is False
...
def test_worker_forwards_native_load_options_verbatim(tmp_path: Path) -> None:
    ...
    worker = make_worker(tmp_path, "inspect-argv-chat", load_options=options)
    worker.start()
    try:
...
def test_worker_close_interrupts_an_active_request(tmp_path: Path) -> None:
    worker = make_worker(tmp_path, "slow-chat", request_timeout=5)
    worker.start()
    with ThreadPoolExecutor(max_workers=1) as executor:
        request = executor.submit(worker.request, "generate", {"prompt": "wait"})
        deadline = time.monotonic() + 1
        while not worker.busy and time.monotonic() < deadline:
            time.sleep(0.005)
        assert worker.busy

        started = time.monotonic()
        worker.close(grace_period=0.05)
...
def test_worker_rejects_malformed_v3_response(tmp_path: Path, mode: str, message: str) -> None:
