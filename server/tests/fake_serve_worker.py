...
def main() -> int:
    ...
    capabilities = (
        ["transcription", "transcription_streaming"]
        if kind == "transcription"
        else ["text_generation"]
    )
    if "no-stream" in mode:
        capabilities.remove("transcription_streaming")
    ready = {
        "event": "ready",
        "protocol_version": 2 if "protocol-v2" in mode else 3,
        "capabilities": capabilities,
    }
    if kind == "chat":
        ready["default_max_new_tokens"] = 64
    if "wrong-capability" in mode:
        ready["capabilities"] = ["transcription"]
    if "duplicate-capability" in mode:
        ready["capabilities"] = [capabilities[0], capabilities[0]]
    if "unknown-ready-field" in mode:
        ready["private_detail"] = "must be rejected"
    if "inspect-environment" in mode:
        ready.update(
            {
                ...
                "distributed_environment_present": any(
                    name in os.environ
                    for name in (
    ...
    if "inspect-argv" in mode:
        ready["worker_args"] = worker_args
        ...
        try:
            if op == "generate":
                ...
                if request.get("config") != {
                    ...
                    raise ValueError("invalid streaming probe config")
                if (
                    kind != "transcription"
                    or "no-stream" in mode
                    or "probe-fails" in mode
                ):
                ...
                result = {
                    "text": (
                        request["audio"]
                        if "echo-wire" in mode
                        else f"{active_samples} samples"
                    )
                }
