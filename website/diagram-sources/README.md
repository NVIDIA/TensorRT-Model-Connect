...
interaction sources use the deliberately small `*.sequence.json` schema.
Generated SVGs are used for the illustrated guides; compact dependency
diagrams in the architecture contract may remain Mermaid source beside the
text. The `.txt` suffix keeps DOT assets outside the repository's
source-language legal-header classifier; each DOT file still carries SPDX
...
- choose direction from the relationship being shown and state whether arrows
  mean dependency, control flow, or returned data;
