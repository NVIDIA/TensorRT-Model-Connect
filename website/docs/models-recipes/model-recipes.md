---
title: Model Recipes
description: Browse declared TensorRT-Model-Connect recipes by task and model family.
---
...
1. Choose a task using the Hugging Face-derived task taxonomy below.
...
3. Open a family to see its exact manifest recipes, applicable `trtmc` CLI
   commands, source metadata (including Hugging Face `model_type` and
   architecture or pipeline class when applicable), TRTMC task/head contract,
   and every family-owned `--set` configuration key.
