...
`trtmc build` does not choose the native builder first. It resolves the owning
family and probes that family's optimized-runtime implementations for an exact
...
the request, the command falls back to the native TensorRT builder. Once a
profile claims the request, a failure in its build is terminal rather than a
