...
| Context parallelism (CP) | The denoiser's sequence or media-token dimension, using Ulysses exchanges. | One shared `denoiser_plan_cp` containing distributed collectives. | FLUX.1-schnell and Cosmos3-Nano in the current multi-device manifests. |
...
## Model-owned topologies

The requested TP or CP size is the launcher world size. A model can divide that
world into explicit subgroups when the bundle records enough metadata to
reconstruct them.

Cosmos3-Nano uses this mechanism on qualified B200 builds. Requested CP4 and
CP8 worlds become two classifier-free-guidance lanes whose denoisers use CP2
and CP4, respectively. Paired cross-lane groups exchange float predictions
before the scheduler update. CP2 and unqualified devices retain one pure CP
group. The public input remains `--context-parallel-size`; there is no generic
classifier-free-parallel CLI setting.

This distinction matters for observability:

| Value | Meaning |
| --- | --- |
| `context_parallel_size` | Total launcher world requested by the user. |
| `denoiser_context_parallel_size` | Ranks collaborating on one denoiser branch. |
| `classifier_free_parallel_size` | Concurrent classifier-free branches. |
| Global rank | Process and CUDA device identity. |
| Group rank | Position inside one model-owned communicator. |

Do not copy Cosmos3 subgroup metadata into another family. The owning builder
selects, validates, serializes, and tests the topology as one vertical slice.
