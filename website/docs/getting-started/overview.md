...
Getting Started has one goal: run one GPT-2 request through
...
| 4 | [Quick Start](quick-start.md) | `./gpt2.bundle` is built and returns generated text. |

A successful run proves the selected environment can resolve the checkpoint,
build a bundle, load the GPT-2 family runtime, and execute one text request. It
does not qualify every model or hardware profile on that machine.
