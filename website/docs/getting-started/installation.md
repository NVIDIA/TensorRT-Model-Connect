...
The wheel depends on the matching TensorRT Python package, contains the native `trtmc` executable, and packages the `libtrtmc*.so` backend artifacts generated by the CI build. CUDA driver/runtime libraries still come from the host environment, so run the wheel on a compatible GPU system.
