...
flowchart LR
  ...
  Descriptor --> NativeDefault{"model-owned native<br/>default route?"}
  NativeDefault -->|yes| Plugin["native family Python plugin"]
...
resolution, a model-owned `default_build_route` may select the native plugin
before provider probing; eligible dense Qwen3 and Llama currently do so. Other
requests let `runtime_provider/` probe only that family's optimized
...
platform-profile, and session-request provenance. The native
`PipelineFactory` currently gives the resolver the materialized `config.json`
text plus `LoadOptions.config_path` and `LoadOptions.set_tokens`. A `defaults`
object extracted from that config text contributes bundle defaults; the load
options contribute a session request. The factory does not inject separate
build-time or platform-profile contributions. The general layer types
therefore describe resolver capacity, not five layers that every native load
actually supplies. Environment-variable or platform-profile behavior should
not be inferred unless a current caller explicitly provides it.
