...
| **Section 9: Software unit verification** | Each software unit shall be verified against its design. Test methods: requirements-based testing, interface testing, structural coverage. | Implemented | 61 C++ test files in `tests/cpp/`, 74 Python test files in `tests/builder/`, 20 Python test files in `tests/tools/`. All linked to ARCH-* contracts via `website/docs/wiki/Traceability-Matrix.md`. | Coverage metrics tracked but not yet gated in CI at a specific threshold. |
...
| **Section 10: Software integration and verification** | Integration tests shall verify correct interaction between units. | Implemented | Unified E2E test suite: `tests/test_e2e.py` with 84 model manifests in `tests/e2e/models/`. Orchestrator in `tests/e2e_harness/orchestrator.py` coordinates full lifecycle (build, run, compare). | None for QM |
