...
1. The Python package resolves a Hugging Face checkpoint and family. A
   model-owned default route may select native construction immediately;
   otherwise an exact-qualified optimized-runtime profile may delegate artifact
   construction to that family's adapter, with the native family plugin as the
   fallback.
