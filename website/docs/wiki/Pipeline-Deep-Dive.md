...
1. A CLI, C ABI, or C++ caller provides a `.trtfb` path and optional runtime
   configuration.
2. `PipelineFactory` reads the bundle metadata and `config.json`.
3. It extracts `runtime_strategy`. If absent, it accepts only a generated
...
Each `src/runtime/models/<family>/MODEL.toml` declares a library, registration
