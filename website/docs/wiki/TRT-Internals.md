...
Before provider probing, the build API honors a model-owned
`default_build_route`; eligible dense Qwen3 and Llama architectures go directly
to native construction. Other requests probe optimized implementations only
under the resolved family. One exact-qualified model/revision/target/options
profile may claim the request. Its isolated adapter invokes the delegated
runtime's builder and packages `optimized_runtime.json`, opaque implementation
metadata, and an integrity-bound artifact tree containing the exact
