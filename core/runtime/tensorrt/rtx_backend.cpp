...
std::ifstream open_runtime_cache_file(const char* path) {
...
std::streamoff runtime_cache_file_size(std::ifstream& file, const char* path) {
    const std::streamoff size = file.tellg();
    if (size < 0) {
        throw std::runtime_error("[trtmc] Failed to size RTX runtime cache: " + std::string(path));
    }
...
std::vector<char> read_runtime_cache_bytes(std::ifstream& file, std::streamoff size,
                                           const char* path) {
    std::vector<char> bytes(static_cast<std::size_t>(size));
    file.seekg(0, std::ios::beg);
    if (!file) {
        throw std::runtime_error("[trtmc] Failed to seek RTX runtime cache: " + std::string(path));
    }
    file.read(bytes.data(), static_cast<std::streamsize>(size));
    if (!file || file.gcount() != static_cast<std::streamsize>(size)) {
        throw std::runtime_error("[trtmc] Failed to read RTX runtime cache: " + std::string(path));
    }
    return bytes;
}

void load_runtime_cache(nvinfer1::IRuntimeCache& runtime_cache, const char* path) {
    std::ifstream file = open_runtime_cache_file(path);
    if (!file)
        return;

    const std::streamoff size = runtime_cache_file_size(file, path);
    if (size == 0)
        return;

    const std::vector<char> bytes = read_runtime_cache_bytes(file, size, path);
    if (!runtime_cache.deserialize(bytes.data(), bytes.size())) {
        throw std::runtime_error("[trtmc] TensorRT-RTX rejected the serialized runtime cache: " +
                                 std::string(path));
    }
    std::cerr << "[trtmc] RTX runtime cache loaded: " << path << " (" << size << " bytes)\n";
}

std::unique_ptr<nvinfer1::IRuntimeCache> create_runtime_cache(nvinfer1::IRuntimeConfig& config,
                                                              const char* path) {
    std::unique_ptr<nvinfer1::IRuntimeCache> runtime_cache(config.createRuntimeCache());
    if (runtime_cache == nullptr)
        throw std::runtime_error("[trtmc] Failed to create RTX runtime cache");
    load_runtime_cache(*runtime_cache, path);
...
class RtxBackend final : public IBackend {
  ...
  private:
    ...
    void ensure_runtime_cache(nvinfer1::IRuntimeConfig* cfg, const char* path) {
        ...
        if (!runtime_cache_)
            runtime_cache_ = create_runtime_cache(*cfg, path).release();
